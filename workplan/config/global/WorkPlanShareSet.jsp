...
int ssharetype = Util.getIntValue(request.getParameter("ssharetype"), 0);
...
<div class="advancedSearchDiv" id="advancedSearchDiv">
	<FORM id="frmmain" name="frmmain" method="post" action="WorkPlanShareSet.jsp">
	<wea:layout type="4col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(1361, user.getLanguage())%>' >
			...
			<wea:item>
				 <%=SystemEnv.getHtmlLabelName(WorkPlanShare.SHARE_TYPE==2?15525:882,user.getLanguage()) +(user.getLanguage()==8?" ":"") +SystemEnv.getHtmlLabelName(21956,user.getLanguage())%>
			</wea:item>
			<wea:item>
				<select class=inputstyle  name="ssharetype" id="ssharetype" style="width:80px;">
					<option value="0" <%if(ssharetype == 0) {%>selected<%} %>></option>
					  <option value="1" <%if(ssharetype == 1) {%>selected<%} %>><%=SystemEnv.getHtmlLabelName(179,user.getLanguage())%></option>
					  <option value="2" <%if(ssharetype == 2) {%>selected<%} %>><%=SystemEnv.getHtmlLabelName(141,user.getLanguage())%></option>
					  <option value="3" <%if(ssharetype == 3) {%>selected<%} %>><%=SystemEnv.getHtmlLabelName(124,user.getLanguage())%></option>
					  <option value="4" <%if(ssharetype == 4) {%>selected<%} %>><%=SystemEnv.getHtmlLabelName(122,user.getLanguage())%></option>
					  <option value="5" <%if(ssharetype == 5) {%>selected<%} %>><%=SystemEnv.getHtmlLabelName(1340,user.getLanguage())%></option>
	...
	<%
		...
		String sqlForm = "WorkPlanShareSet left join workPlanType on WorkPlanShareSet.planid=workPlanType.workPlanTypeID";
		String sqlWhere = "WHERE settype=0 ";	
		if(planType > -2){
		...
		if(ssharetype > 0){
			sqlWhere += " and WorkPlanShareSet.ssharetype = '" + ssharetype + "' ";
		}												
		String orderby = "WorkPlanShareSet.planid";
		
		String tableString=""+
			"<table pagesize=\""+PageIdConst.getPageSize(PageIdConst.WP_WorkPlanShareSet,user.getUID())+"\" tabletype=\"checkbox\">"+
		    "<sql backfields=\"" + backFields + "\" sqlform=\"" + sqlForm + "\" sqlprimarykey=\"WorkPlanShareSet.ID\" sqlorderby=\"" + orderby + "\"  sqlsortway=\"DESC\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"/>"+
		    "<head>"+
		    ...
		    "<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelNames("15525,33234",user.getLanguage())+"\" column=\"SSHARETYPE\" orderkey=\"SSHARETYPE\" otherpara=\""+user.getLanguage()+"\" transmethod=\"weaver.splitepage.transform.SptmForWorkPlan.getWorkPlanShareTypes\"/>"+						    
		    "<col width=\"14%\"  text=\""+SystemEnv.getHtmlLabelName(WorkPlanShare.SHARE_TYPE==2?15525:882,user.getLanguage())+"\" column=\"SSHARETYPE\" otherpara=\"column:SUSERID+column:SSUBCOMPANYID+column:SDEPARTMENTID+column:SROLEID+column:SROLELEVEL+"+user.getLanguage()+"+column:sjobtitleid+column:sjoblevel+column:sjoblevelvalue\" transmethod=\"weaver.splitepage.transform.SptmForWorkPlan.getWorkPlanShareTypeDesc\"/>"+
		    "<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(683,user.getLanguage())+"\" column=\"Sseclevel\" orderkey=\"Sseclevel\" otherpara=\"column:sseclevelMax+column:SSHARETYPE\" transmethod=\"weaver.splitepage.transform.SptmForWorkPlan.getSeclevel\" />"+
		    "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(18495,user.getLanguage())+"\" column=\"SHARETYPE\" orderkey=\"SHARETYPE\" otherpara=\""+user.getLanguage()+"+column:companyVirtual\" transmethod=\"weaver.splitepage.transform.SptmForWorkPlan.getWorkPlanShareTypes\"/>"+	
			...
			"<operates>"+
				...
				"<operate href=\"javascript:delplan();\" isalwaysshow=\"true\" text=\""+SystemEnv.getHtmlLabelName(91,user.getLanguage())+"\" target=\"_self\" index=\"1\"/>"+
			"</operates>"+
		"</table>";
	%>
...
function doDelete(ids){
	$.post("PlanShareOperation.jsp?types=0&method=delete",{id:ids},function(datas){
		window.location.href="/workplan/config/global/WorkPlanShareSet.jsp";
	});
}
...
function add(){
	...
	diag_vote.URL = "/workplan/config/global/WorkPlanShareAdd.jsp";
	diag_vote.show();
}
function doSearch() {
