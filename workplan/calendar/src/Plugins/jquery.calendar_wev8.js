...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass}" id="${id}" title="${title}" style="color:#fff;"><div class="dhdV" style="display:none">${data}</div><div class="${extendClass} rb-m" style="background-color:${color}">${extendHTML}<div class="rb-i">${content}</div></div></div>';
        ...
        function BuildDayEvent(theme, e, index) {
            ...
            var eP = gP(e.et.hour-option.defBgTime, e.et.minute);
            p.top = sP + "px";
            p.left = (e.left * 100) + "%";
            p.width = (e.aQ * 100) + "%";
            if(eP - sP>4){
        ...
        function parseED(data) {
            if (data.length > 6) {
                ...
                e.push(data[0], data[1], newda1, newda2, parseInt(data[4]), parseInt(data[5]), parseInt(data[6]), data[7] != undefined ? parseInt(data[7]) : -1, data[8] != undefined ? parseInt(data[8]) : 0, data[9], data[10]);
                return e;
            }
            return null;
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                temparr.push(i18n.xgcalendar.example, '</div></td></tr></tbody></table><input id="bbit-cal-start" type="hidden"/><input id="bbit-cal-end" type="hidden"/><input id="bbit-cal-allday" type="hidden"/>[<span id="bbit-cal-quickAddBTN" class="lk" >');
                temparr.push(i18n.xgcalendar.create_event, '</span> ]&nbsp; <SPAN id="bbit-cal-editLink" class="lk">');
                ...
                calbutton.click(function(e) {
                    ...
                    if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
                        ...
                        $.post(option.quickAddUrl+option.selectedUser+"&isShare="+option.isShare, param, function(data) {
                            if (data) {
                                if (data.IsSuccess ) {
                                	newdata.push(-1, what);
