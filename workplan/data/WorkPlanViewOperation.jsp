...
<%
	...
	{	    	    
	    ...
	    if(!"".equals(remindBeforeStart) && null != remindBeforeStart)
	    {
	    	workPlan.setRemindTimesBeforeStart(Util.getIntValue(request.getParameter("remindDateBeforeStart"),0)*60+Util.getIntValue(Util.null2String(request.getParameter("remindTimeBeforeStart")),0));  //开始前提醒时间p
	    }
	    else
	    {
	     ...
	     if(!"".equals(remindBeforeEnd) && null != remindBeforeEnd)
	    {
	    	workPlan.setRemindTimesBeforeEnd(Util.getIntValue(request.getParameter("remindDateBeforeEnd"),0)*60+Util.getIntValue(Util.null2String(request.getParameter("remindTimeBeforeEnd")),0));  //结束前提醒时间p
	    }
	    else
	    {
    	...
    	*/
    	...
    	result.put("IsSuccess",new Boolean(true));
	...
	{
		String id = request.getParameter("id");
		boolean isPass=workPlanHandler.finishWorkPlan(id);
		Map result=new HashMap();
		result.put("IsSuccess",new Boolean(isPass));
		out.clearBuffer();
	...
	{  
		...
		if(recordSet.next())
		{
	    	...
	    	result.put("type",new Integer(recordSet.getInt("type_n")));
	    	result.put("id",new Integer(recordSet.getInt("id")));
	    	result.put("planName",Util.convertInput2DB2(Util.null2String((recordSet.getString("name")))));
	    	...
	    	result.put("remindBeforeStartMinute",new Integer(temptimeb));
	    	result.put("remindBeforeStartHour",new Integer(temmhourb));
	    	result.put("remindBeforeEnd",remindBeforeEnd);
	    	result.put("remindBeforeEndMinute",new Integer(temptimee));
	    	result.put("remindBeforeEndHour",new Integer(temmhoure));
	    	...
	    	if(description.indexOf("<br>")==-1)
				description = Util.convertInput2DB2(description);
	    	...
	    	result.put("shareLevel",new Integer(recordSet.getInt("shareLevel")));
	    	result.put("exchangeCount",new Integer(0));
	    	...
	    	if (null != executeId && !"".equals(executeId)) 
			{
				...
				{
					executeName+=resourceComInfo.getResourcename((String)executeList.get(i));
					executeName+=" ";
				}
			}
	    	...
	    	if (null != crmId && !"".equals(crmId))
			{
				...
				{
					crmIdName += customerInfoComInfo.getCustomerInfoname((String)crmIdList.get(i));
					crmIdName += " ";
				}
			}
	    	...
	    	if (null != docId && !"".equals(docId))
			{
				...
				{
					docIdName += (docComInfo.getDocname((String)docIdList.get(i)));
					docIdName += (" ");
				}
			}
	    	...
	    	if (null != projectId && !"".equals(projectId))
			{
				...
				{
					projectIdName += (projectInfoComInfo.getProjectInfoname((String)projectIdList.get(i)));
					projectIdName +=(" ");
				}
			}
	    	...
	    	if (null != workFlowId && !"".equals(workFlowId))
			{
				...
				{
					requestIdName += (requestComInfo.getRequestname((String)workFlowIdList.get(i)));
					requestIdName += (" ");
				}
			}
	...
	else if("deleteCalendarItem".equals(method))
	{
		...
		int level2=WorkPlanShareUtil.getShareLevel(id, user);
		if(level2 == 2)
		//判断是否对提交的数据有编辑权限
		{
	...
	//得到共享信息
	{
		...
		while(recordSet.next())
		{
            ...
            //
            {
                item.put("shareTypeName",SystemEnv.getHtmlLabelName(15762,user.getLanguage()));
                shareContent="";
                if (1 == shareLevel)
                {
                    shareContent+=(SystemEnv.getHtmlLabelName(367,user.getLanguage()));
	...
	{				
		...
		if(recordSet1.next())
		{
	       ...
	       {     
	    	   out.clearBuffer();
	           out.print("{\"shareId\":\""+recordSet.getInt("id")+"\",\"isSuccess\":true}");
	       }else{
	    	   out.clearBuffer();
	           out.print("{\"shareId\":\""+recordSet.getInt("id")+"\",\"isSuccess\":false}");
	       }
		}
	}
