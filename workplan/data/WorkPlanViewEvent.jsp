...
<%
	...
	if (planStatus.equals("")) planStatus=Constants.WorkPlan_Status_Unfinished;
	int countCell = 0;
	...
	String beginDate = Util.add0(cal.get(Calendar.YEAR), 4) + "-" +
			    Util.add0(cal.get(Calendar.MONTH) + 1, 2) + "-" +
			    Util.add0(cal.get(Calendar.DAY_OF_MONTH), 2);
	int cYear = Util.getIntValue(Util.add0(cal.get(Calendar.YEAR), 4));
	int cMonth = Util.getIntValue(Util.add0(cal.get(Calendar.MONTH), 2));
	int cDate = Util.getIntValue(Util.add0(cal.get(Calendar.DAY_OF_MONTH), 2));
	pre.set(cYear,cMonth,cDate);
	...
	switch (viewType)
	{
		...
		case 2:
			cal.add(Calendar.WEEK_OF_YEAR, 1);
		    pre.add(Calendar.WEEK_OF_YEAR, 1);
			cal.add(Calendar.DATE, -1);
...
<FORM name="frmmain" method="post" action="WorkPlanViewEvent.jsp">
		...
		<TABLE class="ListStyle" width="100%" id="WorkPlanTable" height="100%" cellspacing="1">
			...
			<TR class="Header" > 
				<TD height="5%">
					...
					<SELECT size="1" id="opt" style="width:80" title="" name="planType" onchange="changeTitle('opt', this.value)">
						<OPTION value="" <%if(planStatus.equals("-1")){%>selected<%}%> ></OPTION>
						<%
		  				...
		  				%>
						<OPTION value=<%= planTypeOption %> <%if(planType.equals(planTypeOption)){%>selected<%}%> ><%= Util.forHtml(recordSet.getString("workPlanTypeName")) %></OPTION>
		  				<%
		  					}
		  				%>	
					</SELECT>
					...
					<%
						...
						switch (viewType)
						{
							case 1://日
								...
								break;
							case 2: //周
								viewDate = pre.get(Calendar.YEAR) + SystemEnv.getHtmlLabelName(445,user.getLanguage()) + "-" + SystemEnv.getHtmlLabelName(15323,user.getLanguage()) + weekOfYear + SystemEnv.getHtmlLabelName(1926,user.getLanguage()) + "("+beginDate+")" ;
								break ;
							case 3: //月
...
{
	document.frmmain.action = "WorkPlanView.jsp#rushHour";
	document.frmmain.submit();
}

function changeToDayEventView(paraDate)
{
	document.frmmain.action = "WorkPlanViewEvent.jsp";
