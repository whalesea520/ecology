...
String sqlwhere1 = xssUtil.put(" and formid="+formid);
String sqlwhere2 = xssUtil.put(" and exists(select 1 from workflow_base where workflow_base.formid=WorkFlow_Bill.id and workflow_base.id="+wfid+") ");
...
function getWfBrowserUrl(data){
	...
	var sqlwhere="";
	if(wftype=="2"){
		sqlwhere='<%=xssUtil.put("where isbill=1 and isvalid=1 and formid<0 and exists ( select 1 from workflow_billfield t2 where t2.billid=workflow_base.formid and t2.fieldhtmltype=3 and t2.type=8 and formid not in(74) )") %>';
	}else if(wftype=="3"){
		sqlwhere='<%=xssUtil.put("where isbill=1 and isvalid=1 and formid<0 and exists ( select 1 from workflow_billfield t2 where t2.billid=workflow_base.formid and t2.fieldhtmltype=3 and t2.type=129 and formid not in(152) )") %>';
	}else if(wftype=="1"){
		sqlwhere='<%=xssUtil.put("where isbill=1 and isvalid=1 and formid<0 ") %>';
	}else{
		sqlwhere='<%=xssUtil.put("where 1=1 and isvalid=1 ") %>';
	}
...
function getWfCompleteUrl(data){
	...
	var sqlwhere="";
	if(wftype=="2"){
		sqlwhere='<%=xssUtil.put(" isbill=1 and isvalid=1 and formid<0 and exists ( select 1 from workflow_billfield t2 where t2.billid=workflow_base.formid and t2.fieldhtmltype=3 and t2.type=8 and formid not in(74) )") %>';
	}else if(wftype=="3"){
		sqlwhere='<%=xssUtil.put(" isbill=1 and isvalid=1 and formid<0 and exists ( select 1 from workflow_billfield t2 where t2.billid=workflow_base.formid and t2.fieldhtmltype=3 and t2.type=129 and formid not in(152) )") %>';
	}else if(wftype=="1"){
		sqlwhere='<%=xssUtil.put(" isbill=1 and isvalid=1 and formid<0 ") %>';
	}else{		
		sqlwhere='<%=xssUtil.put(" 1=1 and isvalid=1 ") %>';
	}
	var formid=$("#formid").val();
	if(formid!=""){
		sqlwhere+='<%=sqlwhere1 %>';
