...
<%
if(!HrmUserVarify.checkUserRight("Prj:WorkflowSetting", user)){
	response.sendRedirect("/notice/noright.jsp");	
	return;
}
String wftype="1";//项目创建流程配置
...
String pageId=Util.null2String(PropUtil.getPageId("prj_prjcreatewfconftab"));
%>
...
operatorInfo.put("operatortype", "prj_createwf");//操作项类型
...
String tableString=""+
        ...
        "<head>";
            ...
            tableString+=
              ...
              "<col width=\"5%\"  text=\""+SystemEnv.getHtmlLabelName(586,user.getLanguage())+"\" column=\"prjtype\" orderkey=\"prjtype\" transmethod='weaver.proj.Maint.ProjectTypeComInfo.getProjectTypename' />"+                             
		...
		jQuery.ajax({
			...
			success: function do4Success(msg){
				if(msg&&msg.referenced){
					...
					window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelNames("83742",user.getLanguage())%>:\n"+info);
				}else{
...
function newDialog(type,id){
	...
	var title="<%=SystemEnv.getHtmlLabelNames("83744",user.getLanguage())%>";
	openDialog(url,title,500,350,false);
}
function onAdd(){
	var url="/proj/conf/prjwfadd.jsp?wftype=<%=wftype %>&isdialog=1";
	title="<%=SystemEnv.getHtmlLabelNames("83744",user.getLanguage())%>";
