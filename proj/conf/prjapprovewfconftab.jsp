...
String pageId=Util.null2String(PropUtil.getPageId("prj_prjapprovewfconftab"));
%>
...
operatorInfo.put("operatortype", "prj_approvewf");//操作项类型
...
function newDialog(type,id){
	...
	var title="<%=SystemEnv.getHtmlLabelNames("83740",user.getLanguage())%>";
	openDialog(url,title,500,350,false);
}
function onAdd(){
	var url="/proj/conf/prjwfadd.jsp?wftype=<%=wftype %>&isdialog=1";
	title="<%=SystemEnv.getHtmlLabelNames("83740",user.getLanguage())%>";
