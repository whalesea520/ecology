...
<tr>
	...
	<td valign="top">
		...
		<td valign="top">
		   ...
		   <tr>  
			 ...
			 <TD class=Field colspan="2">&nbsp;
					  ...
					  <input type="hidden" name="begindate01" value="<%=begindate01%>">- &nbsp;<BUTTON class=calendar id=SelectDate onclick=getDate('begindate02span','begindate02')></BUTTON>&nbsp;
			 ...
			 <TD class=Field colspan="2">&nbsp;
					  ...
					  <input type="hidden" name="enddate01" value="<%=enddate01%>">- &nbsp;<BUTTON class=calendar id=SelectDate onclick=getDate('enddate02span','enddate02')></BUTTON>&nbsp;
		...
		%>
			...
			<TD><input type="checkbox" name="check_per" value="<%=ids%>" <%=ischecked%> onClick="CheckItOut(this.checked,'<%=ids%>')"></TD>
...
Sub BrowseTable_onclick()
   ...
   If e.TagName = "TD" Then   	
   	...
   	if obj.checked then
   		...
   		tasknames = replace(tasknames,","&TRIM(e.parentelement.cells(2).innerText),"")
   	else
   		obj.checked = true
   		taskids = taskids & "," & e.parentelement.cells(0).innerText
   		tasknames = tasknames & "," & TRIM(e.parentelement.cells(2).innerText)
   	end if
  '   window.parent.returnvalue = Array(e.parentelement.cells(0).innerText,e.parentelement.cells(1).innerText)
   ...
   ElseIf e.TagName = "A" Then
   	...
   	if obj.checked then
   		...
   		tasknames = replace(tasknames,","&TRIM(e.parentelement.parentelement.cells(2).innerText),"")
   	else
   		obj.checked = true
   		taskids = taskids & "," & e.parentelement.parentelement.cells(0).innerText
   		tasknames = tasknames & "," & TRIM(e.parentelement.parentelement.cells(2).innerText)
   	end if
