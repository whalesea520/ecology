...
<form name=weaver method=post action="/proj/process/DspMember.jsp">
...
<TBODY>
  ...
  <col width="10%">
	    ...
	    </TR><TR class=Line><Th class="Line1"></Th>
		<Th class="Line1"></Th>
		<Th class="Line1"></Th>
		<Th class="Line1"></Th>
		<Th class="Line1"></Th>
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD nowrap></TD>
          <TD nowrap></TD>
          <TD nowrap><%if(!RecordSet.getString("begindate").equals("x")){%><a href=#><%=RecordSet.getString("begindate")%></a><%}%></TD>
          <TD nowrap><%if(!RecordSet.getString("enddate").equals("-")){%><a href=#><%=RecordSet.getString("enddate")%></a>		  <%}%></TD>
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD>
			...
			<img src="/images/project_rank_wev8.gif"><a href="/proj/process/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><%=RecordSet2.getString("subject")%></a></TD>
          <TD nowrap>
<%
		String temptaskid="";
...
%>
			<%if(!temprequestcount.equals("0")){%>
				<a href="/proj/process/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><img src="/images/prj_request_wev8.gif" border=0 alt="<%=SystemEnv.getHtmlLabelName(259,user.getLanguage())%>"></a><%=temprequestcount%> 
			<%}%>
			<%if(!tempdoccount.equals("0")){%>
				<a href="/proj/process/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><img src="/images/prj_doc_wev8.gif" border=0 alt="<%=SystemEnv.getHtmlLabelName(58,user.getLanguage())%>"></a><%=tempdoccount%>
			<%}%>
		  </TD>
          ...
          <TD height="100%">
            <%if((RecordSet2.getString("finish")).equals("0")){%>                  
            <%}else{%>
            <TABLE height="100%" cellSpacing=0 
                <%if(Util.getIntValue(RecordSet2.getString("finish"),0)==100){%>
                class=redgraph 
                <%}else{%>
                class=greengraph 
                <%}%>
              width="<%=RecordSet2.getString("finish")%>%">
            <TBODY>
            <TR>
          ...
          <TD>
              <%if(Util.getIntValue(RecordSet2.getString("finish"),0)==100){%>
              <font class=fontred><%=Util.getIntValue(RecordSet2.getString("finish"),0)%>%</font>
              <%}else{%>
              <%=Util.getIntValue(RecordSet2.getString("finish"),0)%>%
              <%}%>
          </TD>
          <TD nowrap><%=RecordSet2.getString("workday")%></TD>
          <TD nowrap>
			  <%if(!RecordSet2.getString("begindate").equals("x")){%>
				  <%if(Util.getIntValue(RecordSet2.getString("finish"),0)==0 && CurrentDate.compareTo( RecordSet2.getString("begindate"))>=0){%>
						<font class=fontred><%=RecordSet2.getString("begindate")%></font>
				  <%} else {%>
          ...
          <TD nowrap>
			  <%if(!RecordSet2.getString("enddate").equals("-")){%>
				  <%if(Util.getIntValue(RecordSet2.getString("finish"),0)!=100 && CurrentDate.compareTo( RecordSet2.getString("enddate"))>=0){%>
						<font class=fontred><%=RecordSet2.getString("enddate")%></font>
				  <%} else {%>
