...
String sqlwhere="";
sqlwhere=" where prjid = "+ProjID+" and  finish = '100' and isdelete<>'1' ";
...
<table class=viewform>
  <tr>
     <th width="118" align=left><%=SystemEnv.getHtmlLabelName(15289,user.getLanguage())%></th>
      <td>	<%RCMenu += "{-}";
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
         ...
         <TR>
          <TD 
          <%if(RecordSet.getString("finish").equals("100")){%>
          class=redgraph
          <%}else{%>
          class=greengraph
          <%}%>
          width="<%=RecordSet.getString("finish")%>%"></td><td><%=RecordSet.getString("finish")%>%</TD>
