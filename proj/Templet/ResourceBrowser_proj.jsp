...
String sql_mem="select proMember from Prj_Template where id="+ProjID ;
RecordSet.executeSql(sql_mem);
RecordSet.next();
String Members=RecordSet.getString("proMember");
if(Members.indexOf(",")==0){
	Members=Members.substring(1);
}
//System.out.println("Members.indexOf(\",\")"+Members.indexOf(","));
String sqlstr = "select HrmResource.id,lastname,resourcetype,startdate,enddate,jobtitlename,departmentid "+
			    "from HrmResource left join  HrmJobTitles on HrmJobTitles.id = HrmResource.jobtitle where  HrmResource.id in"+ "("+Members+")" ;
%>
<BODY>
...
<TR class=DataHeader>
	  ...
	  <TR class=Line><Th colspan="4" style="padding:0;"></Th></TR> 
<%
int i=0;
