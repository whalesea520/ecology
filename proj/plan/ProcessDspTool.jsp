...
<%
ProcPara = ProjID;
RecordSet.executeProc("Prj_Tool_SumProcess",ProcPara);
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD class=fontred><a href="/cpt/capital/CptCapital.jsp?id=<%=RecordSet.getString("relateid")%>"><%=CapitalComInfo.getCapitalname(RecordSet.getString("relateid"))%></a></TD>
...
<%
			isLight = !isLight;
	ProcPara = ProjID + flag + RecordSet.getString("relateid");
	RecordSet2.executeProc("Prj_Tool_SelectAllProcess",ProcPara);
