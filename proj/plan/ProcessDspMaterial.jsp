...
<TBODY>
	    <TR class=Header>
		  ...
		  <th><%=SystemEnv.getHtmlLabelName(1329,user.getLanguage())%></th>
...
<%
ProcPara = ProjID ;
RecordSet.executeProc("Prj_Material_SumProcess",ProcPara);
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD class=fontred><%=RecordSet.getString("material")%></TD>
          <TD class=fontred></TD>
          <TD class=fontred></TD>
          <TD class=fontred><%=RecordSet.getString("quantity")%></TD>
          <TD class=fontred><%=RecordSet.getString("cost")%></TD>

    </TR>
<%
			isLight = !isLight;
	ProcPara = ProjID + flag + RecordSet.getString("material");
	RecordSet2.executeProc("Prj_Material_SelectAllProcess",ProcPara);
...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD>
			...
			<img src="/images/project_rank_wev8.gif" alt="<%=RecordSet2.getString("wbscoding")%>"><a href="/proj/plan/ViewTaskProcess.jsp?taskrecordid=<%=RecordSet2.getString("taskrecordid")%>" title="<%=RecordSet2.getString("wbscoding")%>"><%=RecordSet2.getString("subject")%></a></TD>
          <TD><%=RecordSet2.getString("unit")%></TD>
          <TD><%=RecordSet2.getString("cost")%></TD>
          <TD><%=RecordSet2.getString("quantity")%></TD>
          <TD><%=(RecordSet2.getFloat("cost")*RecordSet2.getInt("quantity"))%></TD>
