...
String tablename2 = "prj_fielddata";//项目类型自定义字段
...
<tr style='display:none;'>
	...
	<td></td>
		...
		if(fromType&&proTypeId>0){//项目类型(包括项目通用自定义字段,和项目类型自己的自定义字段)
			...
			sql= "select '1' as fieldtype,t1.id,t1.isopen,t1.ismand,t1.isused,t1.groupid,t1.issystem,t1.allowhide,t1.fieldname,t1.fieldlabel,t1.fielddbtype,t1.fieldhtmltype,t1.type,t1.dsporder,t1.textheight,t1.imgwidth,t1.imgheight,t1.childfieldid,t1.qfws from prjDefineField t1 where t1.prjtype=-1 and t1.viewtype=0  ";
		}
          ...
          <TR class=<%=trClass%>>
			...
			<td NOWRAP>
				  ...
				  <div id="div5_<%=rowsum%>" <%if(fieldhtmltype.equals("5")&&!"1".equals(issystem)&&(proTypeId==-1||(proTypeId>0&&"2".equals(fieldtype)))){%>style="display:inline"<%}else{%>style="display:none"<%}%> >
					...
					<div id="div5_5_<%=rowsum%>" <%if(fieldhtmltype.equals("5")&&!"1".equals(issystem)&&(proTypeId==-1||(proTypeId>0))){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				  	<table class='ViewForm' id='choiceTable_<%=rowsum%>' cols=7 border=0>
						...
						<col width="0%">
				  		...
				  		RecordSet1.executeSql("select "+backfields+" from "+selecttablename+" where  fieldid="+
				  		"(select id from prjDefineField where prjtype='-1' and fieldname=(select fieldname from prjDefineField where id='"+fieldid+"'))"+" order by selectvalue ");
				  		while(RecordSet1.next()){
				  		recordchoicerowindex+=1;
				  		%>
				  		<tr>
				  		<td><input type="checkbox" name="chkField" index="<%=recordchoicerowindex%>" value="0" <%if(isFieldUsed||("1".equals(fieldtype)&&!(-1==proTypeId))){%>disabled<%}%>>
				  		<td><input class="InputStyle" value="<%=RecordSet1.getString("selectname")%>" type="text" size="10" <%if("1".equals(fieldtype)&&!(-1==proTypeId)){%>disabled<%} %> name="field_<%=rowsum%>_<%=recordchoicerowindex%>_name" style="width=90%" onchange="checkinput('field_<%=rowsum%>_<%=recordchoicerowindex%>_name','field_<%=rowsum%>_<%=recordchoicerowindex%>_span');setChange(<%=rowsum%>)">
				  		...
				  		<td><input class="InputStyle" type="text" size="4" <%if("1".equals(fieldtype)&&!(-1==proTypeId)){%>disabled<%} %> value = "<%=RecordSet1.getString("listorder")%>" name="field_count_<%=rowsum%>_<%=recordchoicerowindex%>_name" style="width=90%" onchange="setChange(<%=rowsum%>)" onKeyPress="ItemNum_KeyPress('field_count_<%=rowsum%>_<%=recordchoicerowindex%>_name')"></td>
				  		<td><input type="checkbox" name="field_checked_<%=rowsum%>_<%=recordchoicerowindex%>_name" onchange='setChange(<%=rowsum%>)' <%if("1".equals(fieldtype)&&!(-1==proTypeId)){%>disabled<%} %> onclick="if(this.checked){this.value=1;}else{this.value=0}" <%if(RecordSet1.getString("isdefault").equals("y")){%>checked<%}%> value="1"></td>
