...
<script type="text/javascript" src="/proj/js/jquery-ui_wev8.js"></script>
...
<script language=javascript>
    ...
    <%if(message.equals("1")){%>
    window.Dialog.alert("<%=SystemEnv.getHtmlLabelName(18758,user.getLanguage())%>");
 <%}else if(message.equals("-1")){%>
 window.Dialog.alert("<%=SystemEnv.getHtmlLabelName(83912,user.getLanguage())%>");
 <%}else if(message.equals("-2")){%>
 window.Dialog.alert("<%=SystemEnv.getHtmlLabelName(83912,user.getLanguage())%>现有字段不存在！");
 <%}else if(message.equals("-3")){%>
 window.Dialog.alert("保存现有字段出错，字段类型不一致！");
 <%}%>
});
rowindex = "<%=rowsum%>";
delids = ",";
			...
			thisObj.next("span").find(".sbToggle").bind("click",function(){
			   if(thisObj.next("span").find(".sbToggle").hasClass("sbToggle-btc-reverse")){
			   	  ...
			   	  btc.init({
					  ...
					  contentURL:"/workflow/field/BTCAjax.jsp?action=queryContent&isFromMode=0",
			  	  ...
			  	  btc.setContainerStyle(['top','right','min-height'],['-165px','auto','155px']);
			  	  try{
...
function setBTC(){
            var url = "/systeminfo/BrowserMain.jsp?url=/workflow/field/browsertypesetting.jsp?isFromMode=0";
	...
	for(i=0;i<changeRows;i++){//主字段检查
			...
			if(check_formself(frmMain,check_String)){
				...
				if($G("itemFieldType_"+j).value==3&&($G("broswerType_"+j).value==256||$G("broswerType_"+j).value==257)){
					if($G("defineTreeBroswerType_"+j).value==""){//树形字段浏览框必填
...
<script language="javascript" src="/proj/js/btc_wev8.js"></script>
