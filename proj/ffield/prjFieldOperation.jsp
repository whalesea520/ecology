...
synchronized(this){
  		...
  		String index = ""+i;
				  ...
				  if(!new_OR_modify.equals("")){//不为空表示是编辑字段，为空表示新添加字段。
					  ...
					  if(fromType){
						  ...
						  int fieldid=1+Util.getIntValue(RecordSetTrans.getString("maxid"),0);
							  sql="insert into cus_formdict(id,fielddbtype,fieldhtmltype,type,fieldname,fieldlabel,scope,qfws) values('"+fieldid+"','"+fielddbtype+"','"+fieldhtmltype+"','"+type+"','"+fieldname+"','"+fieldlabel+"','ProjCustomField',"+qfws+")";
							  RecordSetTrans.executeSql(sql);
								sql=""+
						  ...
						  RecordSetTrans.executeSql("INSERT INTO prjDefineField(prjtype,groupid,isopen,ismand,billid,fieldname,fieldlabel,fielddbtype,fieldhtmltype,type,dsporder,viewtype,detailtable,textheight,childfieldid,imgwidth,imgheight,qfws) "+
								  " VALUES ('"+prjtype+"','"+groupid+"','"+isopen+"','"+ismand+"',"+formid+",'"+fieldname+"',"+fieldlabel+",'"+fielddbtype+"',"+fieldhtmltype+","+type+","+dsporder+","+viewtype+",'"+detailtable+"',"+textheight+","+childfieldid_tmp+","+imgwidth+","+imgheight+","+qfws+")");
					  }
            ...
            String checkLength = "";
              ...
              if(!actualType.equals(checkType)
                  ...
                  RecordSetTrans.executeSql("update prjDefineField set fielddbtype ='"+checkType+"("+actualLength+")"+"' where id=(select max(id) as id from prjDefineField)");
              }
              if(!fromType){
	              if(isoracle) {
	  ...
	  response.sendRedirect("/proj/ffield/editPrjFieldBatch.jsp?ajax=1&paraid="+prjtype+"&from="+from+"&message="+message);
