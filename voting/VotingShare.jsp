...
<%@ include file="/systeminfo/RightClickMenuConent_wev8.jsp" %>
    <%
        ...
        if(status.equals("0") && cancreate){
        	RCMenu += "{"+SystemEnv.getHtmlLabelName(611,user.getLanguage())+",javascript:addShare("+votingid+"),_top} " ;
			...
			<td class="rightSearchSpan">
			    <%if(status.equals("0") && cancreate){ %>
					<span title="<%=SystemEnv.getHtmlLabelName(611,user.getLanguage()) %>" style="font-size: 12px;cursor: pointer;">
						<input class="e8_btn_top middle" onclick="addShare(<%=votingid %>)" type="button" jQuery1392950000546="32" value="<%=SystemEnv.getHtmlLabelName(611,user.getLanguage()) %>"/>
					</span>
...
<input type=hidden name="method" value="finish">
		...
		<%
			...
			//当前记录是否可删除
			String votingShareDel = cancreate+"+"+status;
			//判断当前分享类型
			String votingShareTypeLink = "column:sharetype+column:resourceid+column:subcompanyid+column:departmentid+column:roleid+column:seclevel+column:rolelevel+column:foralluser+"+user.getLanguage()+"+column:jobtitles+column:joblevel+column:jobsubcompany+column:jobdepartment";
			//获取分享类型名称
			String votingShareTypeName = "column:sharetype+"+user.getLanguage();
			//获取安全级别名称
			...
			String tableString=""+
			   ...
			   " <checkboxpopedom  id=\"checkbox\" showmethod=\"weaver.voting.VotingManager.getVotingShareDel\"  popedompara=\""+votingShareDel+"\" />"+
			   "<sql backfields=\"*\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlform=\"votingshare\" sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"desc\"  sqldistinct=\"true\" />";
	  	...
	  	tableString += operateString;
			   ...
			   tableString +="<head>"+							 
			   "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(21956,user.getLanguage())+"\" column=\"resourceid\" orderkey=\"resourceid\" otherpara=\""+votingShareTypeName+"\" transmethod=\"weaver.voting.VotingManager.votingShareTypeName\"/>"+
			   "<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(106,user.getLanguage())+"\" column=\"sharetype\" orderkey=\"sharetype\" otherpara=\""+votingShareTypeLink+"\" transmethod=\"weaver.voting.VotingManager.votingShareTypeLink\"/>"+
...
function addShare(votingid){
	...
	dialog.Title = "<%=SystemEnv.getHtmlLabelNames("611,25435,19467", user.getLanguage())%>";
	dialog.URL = "/voting/VotingShareAdd.jsp?votingid="+votingid;
	dialog.Width = 576;
	dialog.Height = 260;
	dialog.Drag = true;
 ...
 function doDel(id,otherpara){
	 ...
	 if(ids.length==0){
			...
			window.top.Dialog.confirm('<%=SystemEnv.getHtmlLabelName(82017,user.getLanguage())%>',function(){
				$.post("/voting/VotingShareOperation.jsp?method=delete&shareid="+ids+"&votingid=<%=votingid%>",{},function(){
					_table.reLoad();	
				})
	   	});
	 }
