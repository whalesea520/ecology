...
$(function(){
    $('.e8_box').Tabs({
        ...
        staticOnLoad:true,
        <%if(istemplate==1){%>
            objName:"<%=SystemEnv.getHtmlLabelName(33702,user.getLanguage()) %>"
        <%} else{%>
	        <%if(viewResult==1){%>
	          objName:"<%=SystemEnv.getHtmlLabelName(20042,user.getLanguage()) %>"
	        <%}else{%>
	          objName:"<%=SystemEnv.getHtmlLabelName(33699,user.getLanguage()) %>"
	        <%}%>
	    <%}%>
    });
...
function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		var url = "VotingListFrame.jsp?"+requestParameters;
		/** if($(this).attr("status"))
...
<body scroll="no">
	...
	<div class="e8_boxhead">
		...
		<ul class="tab_menu">
			<li class="">
				<a href="" status='0' target="tabcontentframe">&nbsp;</a>
			</li>
		</ul>
