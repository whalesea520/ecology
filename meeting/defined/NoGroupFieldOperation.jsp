...
if(!HrmUserVarify.checkUserRight("Meeting:fieldDefined", user)){
    ...
    int scopeId = Util.getIntValue(request.getParameter("scopeId"));
    if(scopeId==2||scopeId==3){
    	RecordSet.execute("select id from meeting_fieldgroup where grouptype="+scopeId+" order by id asc");
    	RecordSet.next();
    	String groupid=RecordSet.getString("id");
	    ...
	    if(method.equals("add")||method.equals("edit")){
	        ...
	        for(int i=0; i<fieldid.length ; i++){
		        if(Util.null2String(fieldid[i]).length()==0){
		        	...
		        	temId = mfm.addField(fieldname[i], fp.getFielddbtype(), fieldhtmltype[i], type[i], ""+labelid, filedorder[i], ismand[i], isuse[i], groupid,scopeId,definebroswerType[i],treebroswerType[i],isfixed[i]);
		        	}else{
		        	temId=Util.getIntValue(fieldid[i]);
		        	//只允许更改显示属性，数据库层面的不允许修改
		        	mfm.editField(fieldid[i],filedorder[i], ismand[i], isuse[i], groupid);
		        }
	        ...
	        if(temId>=0)
	            response.sendRedirect("NoGroupEditFieldBatch.jsp?scopeId="+scopeId+"&message=1");
	        else
	            response.sendRedirect("NoGroupEditFieldBatch.jsp?scopeId="+scopeId+"&message="+temId);
	        return;
	        ...
	        response.sendRedirect("NoGroupEditFieldBatch.jsp?scopeId="+scopeId);
	        return;
	    }
    }else{
    	response.sendRedirect("NoGroupEditFieldBatch.jsp?scopeId=-1"); 
    }
%>