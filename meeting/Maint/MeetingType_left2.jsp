...
String nodeid=null;
String rem=(String)session.getAttribute("MeetingTypeleft");
        if(rem==null){
        Cookie[] cks= request.getCookies();

        for(int i=0;i<cks.length;i++){
        //System.out.println("ck:"+cks[i].getName()+":"+cks[i].getValue());
        if(cks[i].getName().equals("MeetingTypeleft"+uid)){
        rem=cks[i].getValue();
        break;
        }
        }
...
session.setAttribute("MeetingTypeleft",rem);
Cookie ck = new Cookie("MeetingTypeleft"+uid,rem);
...
<table height="100%" width=100% cellspacing="0" cellpadding="0" class="flowsTable" valign="top">
  ...
  <FORM NAME=SearchForm STYLE="margin-bottom:0" action="MeetingTypeTab.jsp" method=post target="contentframe">
...
function initTree(){
CXLoadTreeItem("", "MeetingTypeXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
