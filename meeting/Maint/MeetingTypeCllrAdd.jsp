			...
			<FORM id=weaverA name=weaverA action="MeetingTypeOperation.jsp"
				...
				<wea:layout type="2col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(1361, user.getLanguage())%>' >
						...
						<wea:item>
							<SELECT class="InputStyle" style="width:100px;" name="callertype" id="callertype" onchange="onChangePermissionType()">
						...
						<wea:item attributes="{'samePair':\"objtr\"}" >
							...
							<span id=showrolelevel name=showrolelevel style="display:none;width:100%">
...
if("<%=isclose%>"=="1"){
	var dialog = parent.parent.getDialog(window.parent);
	var parentWin = parent.parent.getParentWindow(window.parent);
	parentWin.location="/meeting/Maint/MeetingTypeEdit.jsp?id=<%=mid %>&method=caller";
...
function onChangePermissionType() {
	thisvalue=jQuery("#callertype").val();
 	showEle("sectr");
	showEle("objtr");
	showEle("jobtr");
	if (thisvalue == 1) {
...
function check_by_permissiontype() {
    var re=/^\d+$/;
    var thisvalue=jQuery("#callertype").val();
