...
String nodeid=null;
String rem=(String)session.getAttribute("AddMeetingleft");
        if(rem==null){
        Cookie[] cks= request.getCookies();

        for(int i=0;i<cks.length;i++){
        //System.out.println("ck:"+cks[i].getName()+":"+cks[i].getValue());
        if(cks[i].getName().equals("AddMeetingleft"+uid)){
        rem=cks[i].getValue();
        break;
        }
        }
...
session.setAttribute("AddMeetingleft",rem);
Cookie ck = new Cookie("AddMeetingleft"+uid,rem);
...
<BODY onload="initTree()" scroll=no>
	<FORM NAME=SearchForm STYLE="margin-bottom:0" action="AddMeeting_left.jsp" method=post target="contentframe">
...
function initTree(){
CXLoadTreeItem("", "AddMeetingXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
