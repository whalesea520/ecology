...
if(rs.next() && isapproved.equals("3")){
    canschedule=true;
}
...
<%
RCMenu += "{"+SystemEnv.getHtmlLabelName(2191,user.getLanguage())+",javascript:doSave(),_self} " ;
...
<FORM id=weaver name=weaver action="/meeting/data/MeetingScheduleOperation.jsp" method=post >
	...
	<TD vAlign=top>
	  ...
	  <TABLE class=viewForm>
        ...
        <TR>
          ...
          <TD class=Field> <BUTTON class=Calendar onclick="onShowDate(BeginDatespan,begindate)"></BUTTON> <SPAN id=BeginDatespan ><%=begindate%></SPAN> <input type="hidden" name="begindate" value="<%=begindate%>"></TD>
        </TR>
        <TR><TD class=Line colSpan=2></TD></TR> 
        <TR>
          <TD><%=SystemEnv.getHtmlLabelName(742,user.getLanguage())%></TD>
          <TD class=Field><button class=Clock onclick="onShowTime(BeginTimespan,begintime)"></button><span id="BeginTimespan"><%=begintime%></span><INPUT type=hidden name="begintime" value="<%=begintime%>"></TD>
        </TR>
        <TR><TD class=Line colSpan=2></TD></TR> 
		<TR>
          <TD><%=SystemEnv.getHtmlLabelName(741,user.getLanguage())%></TD>
          <TD class=Field> <BUTTON class=Calendar onclick="onShowDate(EndDatespan,enddate)"></BUTTON> <SPAN id=EndDatespan ><%=enddate%></SPAN> <input type="hidden" name="enddate" value="<%=enddate%>"></TD>
        </TR>
        <TR><TD class=Line colSpan=2></TD></TR> 
        <TR>
          <TD><%=SystemEnv.getHtmlLabelName(743,user.getLanguage())%></TD>
          <TD class=Field><button class=Clock onclick="onShowTime(EndTimespan,endtime)"></button><span id="EndTimespan"><%=endtime%></span><INPUT type=hidden name="endtime" value="<%=endtime%>"></TD>
        </TR>
        <TR><TD class=Line colSpan=2></TD></TR> 
        <TR>
          ...
          <TD class=Field>	
              ...
              <SPAN id=addressspan><A href='/meeting/Maint/MeetingRoom.jsp'><%=MeetingRoomComInfo.getMeetingRoomInfoname(address)%></a></SPAN> 
...
function doSave(){
	if(check_form(document.weaver,'address,begindate,begintime,endtime')){
		document.weaver.method.value="save";
...
function doEnd(){
	if(check_form(document.weaver,'address,begindate,begintime,endtime')){
		document.weaver.method.value="end";
		document.weaver.submit();
	}
}
function submitData() {
