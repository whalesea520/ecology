    ...
    function getIframeDocument(){
    	...
    	if(!!_contentDocument){
			...
			jQuery("#agenda").bind("click",function(){
				...
				changeBtnSet("agenda",_contentDocument);
			});
			...
			jQuery("#service").bind("click",function(){
				...
				changeBtnSet("service",_contentDocument);
			});
...
<%
	String url = "/meeting/data/EditMeeting.jsp";
...
function changeBtnSet(target,obj){
	var divh=$("#"+target+"Div",obj).css("height").replace("px","");
	var contenth=$(".zDialog_div_content",obj).css("height").replace("px","");
	
	if(target=="agenda"){
		target="topic";
	}
	if(parseInt(divh)>parseInt(contenth)){
		$("#"+target+"delbtn",obj).css("margin-right","20px");
	}else{
		$("#"+target+"delbtn",obj).css("margin-right","");
	}
}
</script>