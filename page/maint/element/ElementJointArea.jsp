...
<SCRIPT type="text/javascript" src="/js/jscolor/jscolor_wev8.js"></script>
  ...
  <%
	RCMenu += "{<span>"+SystemEnv.getHtmlLabelName(19613,user.getLanguage())+"</span>,javascript:toggleELib(this) ,_self} " ;
...
<div>
 	<%=wpc.getAllElementList("",user)%>
	<span  id="spanContent" style="display:block;width:100%;vertical-align:top;float:left;">		
		  <%//pu.getBaseHpStr(hpid,layoutid,styleid,user,"hp",subCompanyId,isSetting)%> 
		  <%//wpc.getHpAllElement(hpid,user,isSetting) %>
		  <%=wpc.getAreaElement(fromModule,user) %>
	</span>	
</div>
...
function onAddElement(ebaseid,styleid){
	if($(".item").length==10){
		if(confirm("<%=SystemEnv.getHtmlLabelName(84090,user.getLanguage())%>")){
			url="/homepage/element/ElementPreview.jsp?ebaseid="+ebaseid+"&styleid="+styleid+"&hpid=0&subCompanyId=0&fromModule=<%=fromModule%>&layoutflag=A";	
...
function onDel(eid){	
    ...
    $.get("/homepage/element/EsettingOperate.jsp",
    	  {method: "delElement", hpid: "0",eid:eid,delFlag:flag,delAreaElement:eids,subCompanyId:"0"},
		  function(data){		  	  
    		  if($.trim(data)=="") 	{
    		  	$("#item_"+eid).remove();
    		  } else {
...
function MoveEData(srcFlag,targetFlag){
	...
	var url="/homepage/element/EsettingOperate.jsp?method=editLayout&hpid=0&subCompanyId=0&srcFlag="+srcFlag+"&targetFlag="+targetFlag+"&srcStr="+srcItemEids+"&targetStr="+targetItemEids;		
