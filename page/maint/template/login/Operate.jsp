	...
	try{
		...
		while(i.hasNext()) {
			...
			if(!item.isFormField()){
				...
				if(item.getFieldName().equals("templateid")) templateid=Util.null2String(item.getString("UTF-8"));
		...
		if(zipItem!=null){
			...
			if("add".equals(method)){
				...
				if(rs.next()){
					String tempdir = Util.null2String(rs.getString("dir"));
					zipTmp = tempdir;
...
if("add".equals(method)){
	...
	try {			
		...
		if(!"".equals(dir)){
			...
			if(plu.checkLoginTemplate(dirTemplate,dir)){
				String sqlStr="insert into pagetemplate(templatename,templatedesc,templatetype,templateusetype,dir,zipname) values ('"+templatename+"','"+templatedesc+"','cus','beforelogin','"+dir+"','"+zipTmp+"/"+zipName+"')";
				rs.executeSql(sqlStr);
				
				log.setItem("PortalTheme");
				log.setType("insert");
				log.setSql(sqlStr);
				log.setDesc("新建站点模板");
	...
	response.sendRedirect("/page/maint/template/login/PortalTemplateEdit.jsp?closeDialog=close&message="+message);
	...
	if(targetFile!=null){
		...
		if(!"".equals(dir)){
			...
			if(!dir.endsWith("/"))dir = dir+"/";
			if(plu.checkLoginTemplate(dirTemplate,dir)){
				String sqlStr="update  pagetemplate set templatename='"+templatename+"',templatedesc='"+templatedesc+"', dir='"+dir+"' ,zipName='"+zipTmp+"/"+zipName+"' where id="+templateid;
				rs.executeSql(sqlStr);			
	...
	//删除数据库记录
	String sqlStr="delete  pagetemplate  where id="+templateid;
	rs.executeSql(sqlStr);
	log.setItem("PortalTheme");
	log.setType("update");
	log.setSql(sqlStr);
	...
	log.savePortalOperationLog();
	//跳转至列表页面
	//baseBean.writeLog("List.jsp?message="+message);
