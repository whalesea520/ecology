...
if("add".equals(method)){
	...
	if(targetFile!=null){
		try {		
			...
			//检查jsp文件中是否含有恶意内容
			try{
				String[] allowTypesTemp  = new String[]{".jpg",".jpeg",".gif",".ico",".bmp",".png",".flv",".mp3",".swf",".mp4",".wmv",".htm",".jsp",".html",".js",".css",".xml"};
				List<String> allowTypesList = new ArrayList<String>();
				for(String allowType:allowTypesTemp){
					allowTypesList.add(allowType);
				}
				String zipFileName = zipName.substring(0,zipName.lastIndexOf("."));
				File tempFile = new File(rarPath+zipFileName);
				if(tempFile!=null && tempFile.exists()){
					if(!Util.isValidFile(rarPath + zipFileName, allowTypesList)) {
						FileHelper.recursiveRemoveDir(tempFile);
						message="1";
						response.sendRedirect("LayoutEdit.jsp?closeDialog=close&message="+message);
						return;
					}
		...
		if(!"".equals(dir)){
			...
			if(plu.replaceLayoutTemplate(dirLayout,dir)){
				rs.execute("insert into pagelayout (layoutname,layoutdesc,layouttype,layoutdir,zipname,layoutimage,layoutcode,allowArea,ftl) values ('"+layoutname+"','"+layoutdesc+"','cus','"+dir+"','"+zipTmp+"/"+zipName+"','"+dirLayout+dir+"priview_wev8.png"+"',' ','"+plu.getAllowArea()+" ','"+dir+"index.htm')");
		        log.setItem("PortalPage");
		    	log.setType("insert");
		    	log.setSql("insert into pagelayout (layoutname,layoutdesc,layouttype,layoutdir,zipname,layoutimage,layoutcode,allowArea,ftl) values ('"+layoutname+"','"+layoutdesc+"','cus','"+dir+"','"+zipTmp+"/"+zipName+"','"+dirLayout+dir+"priview_wev8.png"+"',' ','"+plu.getAllowArea()+" ','"+dir+"index.htm')");
	...
	if(targetFile!=null){
		...
		if(!dir.equals("")){
			...
			if(!dir.endsWith("/"))dir = dir+"/";
			if(plu.replaceLayoutTemplate(dirLayout,dir)){
				String sqlStr = "update pagelayout set layoutname='"+layoutname+"', layoutdesc='"+layoutdesc+"',layoutdir='"+dir+"', zipname='"+zipTmp+"/"+zipName+"',allowArea='"+plu.getAllowArea()+"', ftl='"+dir+"index.htm',layoutimage='"+dirLayout+dir+"priview_wev8.png"+"' where id="+id;
				rs.execute(sqlStr);
		        log.setItem("PortalPage");
		    	log.setType("update");
		    	log.setSql(sqlStr);
