...
//	ArrayList tabWhereList = new ArrayList();
 ...
 //校验当前tab信息
  ...
  String url = "/page/element/compatible/RssTabContentData.jsp";
...
<div id="tabnavprev_<%=eid%>" style="cursor:hand;position:relative;float:left;left:-5px;top:-5px;" class="picturebackhp" onclick="backMarqueeDiv1('#tabContainer_<%=eid%>');">&nbsp;</div>


<div id="tabContainer_<%=eid%>" class='tab2' style="width:100%;overflow:hidden;display:none;float:left;"><!--height:32px;-->
		...
		<tr>
			<%for(int i=0;i<tabIdList.size();i++){ 
				...
				<%if(currenttab.equals(tabIdList.get(i))){ 
						String temp_html=Util.toHtml2(((String)tabTitleList.get(i)).replaceAll("&","&amp;"));
				%>
					<td title="<%=temp_html %>" style="padding-top:5px;vertical-align:top;" id="tab_<%=eid%>" tabId=<%=tabIdList.get(i) %> class='tab2selected' onclick="loadContent('<%=eid%>','<%=url%>','<%=queryString%>',event)">
						<span class="ellipsis"><%=temp_html%></span>
					</td>
				<% }else{
						String temp_html=Util.toHtml2(((String)tabTitleList.get(i)).replaceAll("&","&amp;")); 
					%>
					<td title="<%=temp_html %>"   style="padding-top:5px;vertical-align:top;" id="tab_<%=eid+"_"+tabIdList.get(i)%>" tabId=<%=tabIdList.get(i) %> class='tab2unselected' onclick="loadContent('<%=eid%>','<%=url%>','<%=queryString%>',event)">
						<span class="ellipsis"><%=temp_html%></span>
					</td>
			<%	 }
				sumLength += length;
...
<% if(titleState.equals("hidden")){ %>
   <div id="tabnavnext_<%=eid%>" style="cursor:hand;position:relative;float:left;right:-6px;top:-6px;" class="picturenexthp" onclick="nextMarqueeDiv1('#tabContainer_<%=eid%>');">&nbsp;</div>
...
<%}else{ %>
    ...
    <div id="tabnavnext_<%=eid%>" style="cursor:hand;position:relative;float:right;right:-5px;top:-5px;" class="picturenexthp" onclick="nextMarqueeDiv1('#tabContainer_<%=eid%>');">&nbsp;</div>
<%} %>
