...
nameList.add("workflow");
nameList.add("meeting");
nameList.add("workplan");
nameList.add("mail");
nameList.add("showsub");
...
if(!rs_Setting.next()){
	...
	for(int i=0;i<nameList.size();i++){
		...
		if(rs_Setting.next()){
			...
			maxId++;
			String insertStr = "insert into hpElementSetting (id,eid,name,value) values("+(maxId+i)+","+eid+",'"+nameList.get(i)+"','1')";
			rs_Setting.execute(insertStr);
			valueList.add("1");
		}
	}
