...
if(!key_word.equals("")){
	...
	if(searchtype.equals("hrm")){
		...
		temptable = "prjstemptable"+ Util.getNumberRandom() ;
		String SearchSql = "";
		...
		sql.append(" order by t1.id desc"+tsql2+") A"
				+ " union all"
				+ " select B.id,B.name,B.status from "
				+ " (select top "+rows+" id,lastname as name,0 as status from HrmResource where (status =0 or status = 1 or status = 2 or status = 3) "
...
<%!
	public String getSubTaskIds(String taskid,String subids) throws Exception{
        ...
        rs.executeSql("select id from GM_GoalInfo where (deleted=0 or deleted is null) and parentid="+taskid);
        while (rs.next()) {
