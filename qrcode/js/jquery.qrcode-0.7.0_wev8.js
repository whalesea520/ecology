...
var parentdiv;
	...
	var QRCode = function (text, level, version, quiet) {
		...
		drawBackgroundImage = function (qr, context, settings) {
	   		//getBackgroundImagesize(settings);
				var size = settings.size,
			//w = settings.image.naturalWidth || 1,
			//h = settings.image.naturalHeight || 1,
				w = settings.image.naturalWidth || 	settings.image.width,
				h = settings.image.naturalHeight || settings.image.height,
				sh = settings.mSize,
				sw = sh * w / h,
				sl = (1 - sw) * settings.mPosX,
		...
		drawBackground = function (qr, context, settings) {
			...
			if ((mode === 1 || mode === 2)&&!isltie9()) {
				drawBackgroundLabel(qr, context, settings);
				
			} else if ((mode === 3 || mode === 4)&&!isltie9()&&!!settings.image) {
		...
		// Returns a `canvas` element representing the QR code for the given settings.
		 isltie9 = function () {
			...
			//if(browser=="Microsoft Internet Explorer" && (trim_Version=="MSIE7.0"||trim_Version=="MSIE6.0"||trim_Version=="MSIE8.0"||trim_Version=="MSIE9.0"||trim_Version=="MSIE10.0"))return true;
			if(navigator.userAgent.indexOf("MSIE 6.0")>0 || navigator.userAgent.indexOf("MSIE 7.0")>0 
					|| navigator.userAgent.indexOf("MSIE 8.0")>0  )//|| navigator.userAgent.indexOf("MSIE 9.0")>0 || navigator.userAgent.indexOf("MSIE 10.0")>0 
				return true;
			return false;
		},
