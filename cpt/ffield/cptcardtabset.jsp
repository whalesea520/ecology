...
<FORM id=frmain name=frmain method=post action="/cpt/ffield/cptcardtabsetop.jsp"    >
...
<div class="subgroupmain" style="width: 100%;margin-left:0px;"></div>
 <script>
     ...
     var option= {
    		 ...
    		 canDrag:true,
    		 useajax:true,
    		 ajaxurl:'/cpt/ffield/cptcardtabsetop.jsp',
...
/test.jsp?a=1&b=2<br/>
You can also write a placeholder {#id} asset ID, like this: <br/>
/test.jsp?a=1&cptid={#id}&mypara2={#id}<br/> 
If not write placeholder, the default will also receive 2 fixed parameter: <br/>
isfromCapitalTab=1&capitalid=ID	
	<%
...
可以帶上參數傳參,像這樣:<br/>
/test.jsp?a=1&b=2<br/>
也可以寫占位符{#id}傳資産id,像這樣:<br/>
/test.jsp?a=1&cptid={#id}&mypara2={#id}<br/>
即使不寫占位符,默認也會接收到2個固定的參數:<br/>
isfromCapitalTab=1&capitalid=資産id	
	<%
}else{
	%>
...
可以带上参数传参,像这样:<br/>
/test.jsp?a=1&b=2<br/>
也可以写占位符{#id}传资产id,像这样:<br/>
/test.jsp?a=1&cptid={#id}&mypara2={#id}<br/>
即使不写占位符,默认也会接收到2个固定的参数:<br/>
isfromCapitalTab=1&capitalid=资产id
