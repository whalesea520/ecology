...
if(!HrmUserVarify.checkUserRight("DeptDefineInfo1:DeptMaintain1", user)){
  ...
  if(src.equals("addfieldbatch")){//批量添加字段
	            ...
	            int imgheight = Util.getIntValue(request.getParameter("imgheight_"+index),0);
				  ...
				  if(!new_OR_modify.equals("")){//不为空表示是编辑字段，为空表示新添加字段。
					...
					String issystem_t = "";
					  ...
					  //插入字段信息
					  if("1".equals(isfixed))
						  RecordSetTrans.executeSql("INSERT INTO cptDefineField(groupid,isopen,ismand,billid,fieldname,fieldlabel,fielddbtype,fieldhtmltype,type,dsporder,viewtype,detailtable,textheight,childfieldid,imgwidth,imgheight,issystem,allowhide) "+
						  " VALUES ('"+groupid+"','"+isopen+"','"+ismand+"',"+formid+",'"+fieldname+"',"+fieldlabel+",'"+fielddbtype+"',"+fieldhtmltype+","+type+","+dsporder+","+viewtype+",'"+detailtable+"',"+textheight+","+childfieldid_tmp+","+imgwidth+","+imgheight+",1,1)");
					  else
				  ...
				  if(new_OR_modify.equals("")){//新添加字段
					...
					&& !( "number".equals(actualType) && "integer".equals(checkType)) ){
	                          ...
	                          RecordSetTrans.executeSql("update cptDefineField set fielddbtype ='"+checkType+"("+actualLength+")"+"' where id=(select max(id) as id from cptDefineField)");
	                      }
                      }
                      else{
                          //更新主表结构
	  ...
	  response.sendRedirect("/cpt/ffield/editCptFieldBatch.jsp?ajax=1&message="+message);
