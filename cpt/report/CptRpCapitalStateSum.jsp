...
String url="/cpt/report/CptRpCapitalStateSum.jsp?isfromCptTab=1";
...
$(function(){
	try{
		parent.setTabObjName("<%=SystemEnv.getHtmlLabelName(830,user.getLanguage()) %>");
	}catch(e){}
});
</script>
</head>
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
	...
	<wea:layout type="4col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(15774,user.getLanguage())%>'>
	     <wea:item><%=SystemEnv.getHtmlLabelName(830,user.getLanguage())%></wea:item>
	    <wea:item>
		    <brow:browser viewType="0" name="stateid" browserValue='<%= ""+stateid %>' 
					browserUrl="/systeminfo/BrowserMain.jsp?url=/cpt/maintenance/CapitalStateBrowser.jsp?resourceids="
					hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='1'
					completeUrl="/data.jsp?type=243" 
					browserSpanValue='<%=CapitalStateComInfo . getCapitalStatename  (stateid) %>'>
			</brow:browser>
	    </wea:item>
...
<% 
String sqlwhere=" where t1.isdata='2'   and exists(select 1 from CptCapitalShareInfo t2 where t2.relateditemid=t1.id and ("+CommonShareManager.getShareWhereByUser("cpt", user)+") ) ";
if(!"".equals(stateid)){
	sqlwhere+=" and t1.stateid='"+stateid+"' ";
}
if(!fromdate.equals("")){
...
String backfields=" t1.stateid,COUNT(t1.id) AS resultcount ";
String sqlform = " CptCapital  t1  ";
String groupby=" t1.stateid ";
String orderby=" resultcount ";
String primarykey=" t1.stateid ";
