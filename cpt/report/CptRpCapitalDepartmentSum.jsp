...
String url="/cpt/report/CptRpCapitalDepartmentSum.jsp?isfromCptTab=1";
...
$(function(){
	try{
		parent.setTabObjName("<%=SystemEnv.getHtmlLabelName(16599,user.getLanguage()) %>");
	}catch(e){}
});
</script>
</head>
	...
	<wea:layout type="4col">
	    ...
	    <wea:item>
		    <brow:browser viewType="0" name="deptid" browserValue='<%= ""+deptid %>' 
					...
					browserSpanValue='<%=DepartmentComInfo . getDepartmentname (deptid) %>'>
			</brow:browser>
	    </wea:item>
...
<% 
String sqlwhere=" where t1.isdata='2' and t1.departmentid>0  and exists(select 1 from CptCapitalShareInfo t2 where t2.relateditemid=t1.id and ("+CommonShareManager.getShareWhereByUser("cpt", user)+") ) ";
if(!"".equals(deptid)){
...
String backfields=" t1.departmentid,COUNT(t1.id) AS resultcount ";
String sqlform = " CptCapital  t1  ";
String groupby=" t1.departmentid ";
String orderby=" resultcount ";
String primarykey=" t1.departmentid ";
...
String tableString=""+
   ...
   "<head>"+							 
		 "<col width=\"10%\" text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"departmentid\"  orderkey=\"departmentid\" transmethod='weaver.hrm.company.DepartmentComInfo.getDepartmentname' />"+
