...
if(!HrmUserVarify.checkUserRight("DeptDefineInfo1:DeptMaintain1", user)){
  ...
  String wftype = Util.null2String(request.getParameter("wftype"));//apply,fetch,mend,move,discard,back,lend,loss,change
...
if(wftype.length()>1){
  ...
  if(src.equals("editgroupbatch")){//批量编辑分组
	...
	if(dtJsonArray!=null&&dtJsonArray.size()>0){
		...
		for(int i=0;i<dtJsonArray.size() ;i++){
			...
			if(dtJsonArray2!=null&&dtJsonArray2.size()>=15){
				...
				if("change".equalsIgnoreCase(wftype)){
					cptno=Util.null2String( dtJsonArray2.getJSONObject(11).getString("cptno"));
					//zclx=Util.null2String( dtJsonArray2.getJSONObject(12).getString("zclx"));
					rkrq=Util.null2String( dtJsonArray2.getJSONObject(12).getString("rkrq"));
					ssbm=Util.null2String( dtJsonArray2.getJSONObject(13).getString("ssbm"));
					acttype=""+Util.getIntValue( dtJsonArray2.getJSONObject(14).getString("acttype"),-1);
					actnode=""+Util.getIntValue( dtJsonArray2.getJSONObject(15).getString("actnode"),0);
					actlink=""+Util.getIntValue( dtJsonArray2.getJSONObject(16).getString("actlink"),0);
					actmethod=""+Util.getIntValue( dtJsonArray2.getJSONObject(17).getString("actmethod"),0);
					
					groupid= Util.getIntValue( dtJsonArray2.getJSONObject(18).getString("groupid"),0);
				} else {
				...
				String sql="";
				if(groupid>0){
					sql="UPDATE "+grouptablename+" SET wftype = '"+wftype+"',wfid = '"+wfid+"',sqr = '"+sqr+"',zczl = '"+zczl+"',zc = '"+zc+"',sl = '"+sl+"',zcz = '"+zcz+"',jg = '"+jg+"',rq = '"+rq+"',ggxh = '"+ggxh+"',cfdd = '"+cfdd+"',bz = '"+bz+"',wxqx = '"+wxqx+"',wxdw = '"+wxdw+"',cptno = '"+cptno+"',rkrq = '"+rkrq+"',ssbm='"+ssbm+"',isasync = '"+isasync+"',actname = '"+(acttype+","+actnode+","+actlink+","+actmethod)+"' where id="+groupid;
					RecordSet.executeSql(sql);
				}else{
					sql=" INSERT INTO "+grouptablename+"(wftype,wfid,sqr,zczl,zc,sl,zcz,jg,rq,ggxh,cfdd,bz,wxqx,wxdw,cptno,rkrq,ssbm,isasync,actname) "+
						" VALUES ('"+wftype+"','"+wfid+"','"+sqr+"','"+zczl+"','"+zc+"','"+sl+"','"+zcz+"','"+jg+"','"+rq+"','"+ggxh+"','"+cfdd+"','"+bz+"','"+wxqx+"','"+wxdw+"','"+cptno+"','"+rkrq+"','"+ssbm+"','"+isasync+"','"+(acttype+","+actnode+","+actlink+","+actmethod)+"') ";
					RecordSet.executeSql(sql);
					//老的流程归档节点插action
	...
	while(RecordSet.next()){
		...
		if("change".equalsIgnoreCase(wftype)){
			jsonObject=new JSONObject();
			jsonObject.put("name", "cptno");
			jsonObject.put("value", RecordSet.getString("cptno"));
			jsonObject.put("label", Util.null2String(SystemEnv.getHtmlLabelName(Util.getIntValue( hm.get(RecordSet.getString("cptno")),0),user.getLanguage())));
			jsonObject.put("iseditable", true);
			jsonObject.put("type", "browser");
			jsonArray.add(jsonObject);
			
			/**jsonObject=new JSONObject();
			jsonObject.put("name", "zclx");
			jsonObject.put("value", RecordSet.getString("zclx"));
			jsonObject.put("label", Util.null2String(SystemEnv.getHtmlLabelName(Util.getIntValue( hm.get(RecordSet.getString("zclx")),0),user.getLanguage())));
			jsonObject.put("iseditable", true);
			jsonObject.put("type", "browser");
			jsonArray.add(jsonObject);**/
			
			jsonObject=new JSONObject();
			jsonObject.put("name", "rkrq");
			jsonObject.put("value", RecordSet.getString("rkrq"));
			jsonObject.put("label", Util.null2String(SystemEnv.getHtmlLabelName(Util.getIntValue( hm.get(RecordSet.getString("rkrq")),0),user.getLanguage())));
			jsonObject.put("iseditable", true);
			jsonObject.put("type", "browser");
			jsonArray.add(jsonObject);
			
			jsonObject=new JSONObject();
			jsonObject.put("name", "ssbm");
			jsonObject.put("value", RecordSet.getString("ssbm"));
			jsonObject.put("label", Util.null2String(SystemEnv.getHtmlLabelName(Util.getIntValue( hm.get(RecordSet.getString("ssbm")),0),user.getLanguage())));
			jsonObject.put("iseditable", true);
			jsonObject.put("type", "browser");
			jsonArray.add(jsonObject);
		}
