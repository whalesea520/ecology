...
<wea:layout type="2col">
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("83550",user.getLanguage())%>' attributes="{'samePair':'codedetail_g','itemAreaDisplay':'true'}" >
		<wea:item><%=SystemEnv.getHtmlLabelName(22298,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true" onClick=linkcheck("subcompanyflow","22289") name="subcompanyflow" value="1" <%if ("1".equals(subcompanyflow)) out.println("checked");%> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(22299,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true"  onClick =linkcheck("departmentflow","22290") name="departmentflow" value="1" <%if ("1".equals(departmentflow)) out.println("checked");%> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(22300,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true" onClick =linkcheck("capitalgroupflow","20344") name="capitalgroupflow" value="1" <%if ("1".equals(capitalgroupflow)) out.println("checked");%> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(22907,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true" onClick =linkcheck("assetdataflow","22877") name="assetdataflow" value="1" <%if ("1".equals(assetdataflow)) out.println("checked");%> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(22301,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true" onClick =linkcheck("capitaltypeflow","22291") name="capitaltypeflow" value="1" <%if ("1".equals(capitaltypeflow)) out.println("checked");%> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(22302,user.getLanguage())%></wea:item>
		<wea:item>
			<input class="InputStyle" type="checkbox" tzCheckbox="true"  onClick =linkcheck("buydate","22292","buydateselect","1") name="buydate" value="1" <%if ("1".equals(buydate)) out.println("checked");%> >
			<input class="InputStyle" style="width:50px!important" type="radio" onClick=linkcheck("buydate","22292")  name="buydateselect" value="1" <%if ("1".equals(buydateselect)||"".equals(buydateselect)) out.println("checked");%> ><%=SystemEnv.getHtmlLabelName(445,user.getLanguage())%>
			<input class="InputStyle" style="width:50px!important" type="radio" onClick=linkcheck("buydate","22293")  name="buydateselect" value="2" <%if ("2".equals(buydateselect)) out.println("checked");%> ><%=SystemEnv.getHtmlLabelName(6076,user.getLanguage())%>
			<input class="InputStyle" style="width:50px!important" type="radio" onClick=linkcheck("buydate","22294")  name="buydateselect" value="3" <%if ("3".equals(buydateselect)) out.println("checked");%> ><%=SystemEnv.getHtmlLabelName(390,user.getLanguage())%>
		</wea:item>
...
function linkcheck(name,label,rname,flag){
	if(jQuery("input[name="+name+"]").attr("checked")==true){
	
		if(rname){
			var checkvalue = jQuery("input[name="+rname+"]:checked").val();
			if(flag==1){
				if(checkvalue==1){
					label = '22292';
		...
		for (var index=0;index< TR_members.length;index++ ){      
		    ...
		    var codevalue = jQuery(TR_member).children()[0].codevalue.replace(/(^\s+|\s+$)/g,"");
		    if(codevalue==label){
		    	
		    	jQuery(TR_member).children().find("span").first().addClass("tzCheckBox checked");
		  		jQuery(TR_member).children().find("input[id^=chk]").attr("checked","checked");
