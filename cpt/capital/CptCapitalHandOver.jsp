...
<BUTTON type="button" class=btnSave accessKey=S onclick="onSubmit()"><U>S</U>-<%=SystemEnv.getHtmlLabelName(1381,user.getLanguage())%></BUTTON>
<BUTTON type="button" class=btn accessKey=B onclick='window.history.back(-1)'><U>B</U>-<%=SystemEnv.getHtmlLabelName(1390,user.getLanguage())%></BUTTON>
  <TABLE class=form>
    ...
    <tr> 
      <td><%=SystemEnv.getHtmlLabelName(1401,user.getLanguage())%></td>
      <td class=Field><button type="button" class=Calendar id=selecthandoverdate onClick="gethandoverDate()"></button> 
        <span id=handoverdatespan ><IMG src="/images/BacoError_wev8.gif" align=absMiddle></span> 
        <input type="hidden" name="handoverdate">
      </td>
    </tr>
    <tr> 
...
sub onShowResourceID()
 ...
 function onSubmit(){
 	if(check_form(document.frmain,'handoverdate,location,departmentid,costcenterid')){
		document.frmain.action="CapitalHandOverOperation.jsp"
