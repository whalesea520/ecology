...
if(!isBatch){
	...
	if(isfromView){
		...
		if("-2".equals(checkstate)){//已收回的入库单,跳转到编辑入库单页面
			response.sendRedirect("/cpt/capital/CapitalBlankTab.jsp?title="+java.net.URLEncoder.encode(SystemEnv.getHtmlLabelNames("83575",user.getLanguage()),"utf-8")+"&url=/cpt/capital/CptCapitalInstock1tab.jsp?isdialog=1%26id="+id);
			return;
		}
	}
}
   ...
   if(check_form(document.weaver,needcheck)) {
	   ...
	   jQuery.ajax({
		...
		success: function do4Success(data){
			   ...
			   jQuery.ajax({ 
			    	   url : "/cpt/capital/CapitalInStockCheck.jsp",
		               type : "post",
		               async : true,
					   processData : true,
			    	   ...
			    	   if(data.msg1!="yes"){
			    		  document.weaver.submit();
			    		   ...
			    		   $("#btn_confirm").css("background-color","#30b5ff");
			    		   $("#rightMenuIframe").show();
			    		   window.top.Dialog.alert(data.msg2);
			    		   return;
