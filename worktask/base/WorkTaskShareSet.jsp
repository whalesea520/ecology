...
<HEAD>
    ...
    <input type="hidden"  name="wtid" value="<%=wtid%>">
		...
		String tableString="<table  pagesize=\""+perpage+"\" tabletype=\"checkbox\" valign=\"top\" >"
		  ...
		  + "<sql backfields=\"*\" sqlform=\" from worktaskshareset \" sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"asc\" sqlwhere=\"taskid="+wtid+"\" sqlisdistinct=\"false\" />"+
			"       <head>"+
			"    <col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(21947,user.getLanguage())+"\" column=\"taskstatus\" otherpara=\""+user.getLanguage()+"\"  orderkey=\"taskstatus\" transmethod=\"weaver.worktask.worktask.WTRequestUtil.getWTShareStatus\" />"+
			"    <col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(16539, user.getLanguage())+SystemEnv.getHtmlLabelName(882, user.getLanguage())+"\" column=\"ssharetype\" otherpara=\"column:ssubcompanyid+column:sdepartmentid+column:sroleid+column:suserid+column:srolelevel+"+user.getLanguage()+"\" transmethod=\"weaver.worktask.worktask.WTRequestUtil.getWTShareCreator\" />"+
			"    <col width=\"12%\"  text=\""+SystemEnv.getHtmlLabelName(683,user.getLanguage())+"\" column=\"sseclevel\" orderkey=\"sseclevel\"  />"+
			"    <col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(106, user.getLanguage())+"\" column=\"sharetype\" otherpara=\"column:subcompanyid+column:departmentid+column:roleid+column:userid+column:rolelevel+"+user.getLanguage()+"\" transmethod=\"weaver.worktask.worktask.WTRequestUtil.getWTShareCreator\" />"+
			"    <col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(683,user.getLanguage())+"\" column=\"seclevel\" orderkey=\"seclevel\"  />"+
			"    <col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(3005,user.getLanguage())+"\" column=\"sharelevel\" orderkey=\"sharelevel\" otherpara=\""+user.getLanguage()+"\"  transmethod=\"weaver.worktask.worktask.WTRequestUtil.getWTShareLevel\"  />"+
			"       </head>"
	 	+"</table>";			
...
function doAdd(obj){ 
	showDialogWin("<%=SystemEnv.getHtmlLabelName(611,user.getLanguage())%>","/worktask/base/WorkTaskShareAdd.jsp?wtid=<%=wtid%>",700,600)
}
...
function useSetto(){
    ...
    dialog.URL = "/worktask/base/WorktaskList.jsp?wtid=<%=wtid%>&usesettotype=2";
	dialog.Width = 660;
	dialog.Height = 660;
	dialog.Drag = true;
...
function doDeletePrm(ids){
	$.post("/worktask/base/WorkTaskShareOperation.jsp",{method:"delShare",ids:ids},function(datas){
