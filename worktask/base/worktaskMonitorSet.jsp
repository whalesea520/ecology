...
<HEAD>
	...
	<form  name="frmAdd" method="post" action="worktaskMonitorOperator.jsp">
    ...
    <input type="hidden"  name="wtid" value="<%=wtid%>">
		...
		String tableString="<table  pagesize=\""+perpage+"\" tabletype=\"checkbox\" valign=\"top\" >"
		  ...
		  + "<sql backfields=\"* \" sqlform=\" from worktask_monitor \" sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"asc\" sqlwhere=\"taskid="+wtid+"\" sqlisdistinct=\"false\" />"+
			"       <head>"+
			"           <col width=\"45%\"  text=\""+SystemEnv.getHtmlLabelName(21968,user.getLanguage())+"\" column=\"monitor\" otherpara=\""+user.getLanguage()+"\"  orderkey=\"monitor\" transmethod=\"weaver.worktask.worktask.WTRequestUtil.getMonitor\" />"+
			"           <col width=\"45%\"  text=\""+SystemEnv.getHtmlLabelName(21969, user.getLanguage())+"\" column=\"monitortype\" otherpara=\""+user.getLanguage()+"\" transmethod=\"weaver.worktask.worktask.WTRequestUtil.getMonitorType\" />"+
			"       </head>"
	 	+"</table>";			
...
function doAdd(obj){ 
	showDialogWin("<%=SystemEnv.getHtmlLabelName(611,user.getLanguage())%>","/worktask/base/WorkTaskMonitorAdd.jsp?wtid=<%=wtid%>",500,400)
}
...
function useSetto(){
    ...
    dialog.URL = "/worktask/base/WorktaskList.jsp?wtid=<%=wtid%>&usesettotype=5";
	dialog.Width = 660;
	dialog.Height = 660;
	dialog.Drag = true;
...
function doDeletePrm(ids){
	$.post("/worktask/base/worktaskMonitorOperator.jsp",{method:"delmonitor",ids:ids},function(datas){
