...
<script>
   	  ...
   	  function resetbanner(objid, typeid){
			...
			//动态查询
			var href="RequestCheckList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type_d=<%=type_d%>&objId=<%=objId%>";
...
function init(objid,typeid){
	...
	o.location="RequestCheckList.jsp?wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&quarters=<%=quarters%>&weeks=<%=weeks%>&isfromleft=<%=isfromleft%>";
}
 </script>
<body scroll="no">
   ...
   <input name='worktaskStatus'  type='hidden'  value='<%=worktaskStatus%>'>
	...
	<div class="e8_box demo2" id="rightContent">
		<div class="e8_boxhead">
			...
			<div class="e8_ultab">
				<div class="e8_navtab" id="e8_navtab"><a class='dateitemwrapper'><span class='dateitem'></span></a>
					<span id="objName"></span><select class='wtselect' name="wtid" onchange="resetbanner(5,5);" style="width:100%;">
											  ...
											  <select name="worktaskStatus" onchange="resetbanner(6,6)" style="width:100%" disabled='true'>
													<%
														RecordSet.execute("select * from SysPubRef where masterCode='WorkTaskStatus' and flag=1 ");
														while(RecordSet.next()){
		...
		<script type="text/javascript">
			...
			var src="RequestCheckList.jsp?wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&quarters=<%=quarters%>&weeks=<%=weeks%>&isfromleft=<%=isfromleft%>";
            ...
            function  searchDataByTime(){
			  ...
			  var worktaskStatus=$("select[name='worktaskStatus']").val();
			   ...
			   var href="RequestCheckList.jsp?type_d="+type_d+"&objId="+objId+"&wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&years="+year;
			...
			$(document).ready(function(){
			  ...
			  $(".tab_menu").find("a").click(function(e){
					 ...
					 var wtid=$("select[name='wtid']").val();
					 var worktaskStatus=$("select[name='worktaskStatus']").val();
					 var newhref="RequestCheckList.jsp?type_d="+type_d+"&objId="+objId+"&wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&"+currentitem.attr("dateparam");
					 ...
					 if(datetype==='day'){
						 var href="RequestCheckList.jsp?type_d="+type_d+"&objId="+objId+"&wtid="+wtid+"&worktaskStatus="+worktaskStatus;
						 var cdate=new Date();
