...
if(RecordSet.next()){
	String formid = RecordSet.getString("id");
	RecordSet.execute("select * from modeinfo where formid='"+formid+"'");
	if(RecordSet.next()){
...
String journal = "";
String periods = "";
RecordSet.executeSql("select * from uf_xxcb_pbInfo where id="+fwid);
if(RecordSet.next()){
	gwid = Util.null2String(RecordSet.getString("gwid"));
	journal = Util.null2String(RecordSet.getString("journal"));
	fbzt = Util.null2String(RecordSet.getString("fbzt"));
	//laiwen = Util.null2String(RecordSet.getString("laiwen"));
	//sourcedocsubject = Util.null2String(RecordSet.getString("docsubject"));
	sourcedocsubject = Util.null2String(RecordSet.getString("title"));
	periods = Util.null2String(RecordSet.getString("periods"));
}
...
if(editType == 2){
	if(!"".equals(gwid)){//编辑模式下判断文档签出状态
		...
		if(checkOutStatus!=null&&(checkOutStatus.equals("1")||checkOutStatus.equals("2"))&&!(checkOutUserId==user.getUID()&&checkOutUserType!=	null&&checkOutUserType.equals(user.getLogintype()))){
		    ...
		    response.sendRedirect("/govern/information/InformationAdd.jsp?fwid="+fwid+"&editType=0&checkOutMessage="+checkOutMessage);
		    return ;
...
RecordSet.executeSql("select * from uf_xxcb_kxSet where id="+journal);
if(RecordSet.next()){
...
String mbvid = mbVersionId+"_"+mbid;
...
List<String> lwa = new ArrayList<String>();
RecordSet.executeSql("select p.dbxx id,u.zw content,u.bt title,u.bsdw unit,u.lm typeid,l.name type from uf_xxcb_pbInfo_dt1 p left join uf_xxcb_dbInfo u on p.dbxx=u.id left join uf_xxcb_kxSet_dt1 l on p.lanmu=l.id where p.mainid='"+fwid+"' order by p.showOrder");
...
laiwen = "".equals(laiwen)?"":laiwen.substring(1);
for(int i=0;i<lwa.size();i++){
	Map<String,String> map = allMap.get(lwa.get(i));
	String id = Util.null2String(map.get("id"));
	String title = Util.null2String(map.get("title"));
	String unit = Util.null2String(map.get("unit"));
	String typeid = Util.null2String(map.get("typeid"));
	String type = Util.null2String(map.get("type"));
	String content = Util.null2String(map.get("content"));
	...
	if(!"".equals(typeid)){
		...
		//Map<String,String> map = new HashMap<String,String>();
		map.put("index",list.size()+"");
		map.put("groupIndex",typeid + "-" + list.size());
		list.add(map);
...
String jsonListData  = JSON.toJSONString(allDataList,SerializerFeature.DisableCircularReferenceDetect);
...
function onEdit(){
	location.href='InformationAdd.jsp?fwid=<%=fwid%>&editType=2';  //编辑
}
...
function initMenu(){
  ...
  <%}else if(editType==1){%>
    weaver.WebOffice.AppendMenu("1","<%=SystemEnv.getHtmlLabelName(86,languageId)%>");//提交
...
function onLoadContent(){
	try{
		//if(!gwid){
			...
			weaver.WebOffice.WebObject.Bookmarks("Body").Range.text=con;
			//weaver.WebOffice.WebObject.Application.Selection.Range.InsertAfter(con);
			//weaver.WebOffice.WebObject.Bookmarks("Body").Select//Bookmarks("Body").
		//}
	}catch(e){console.log(e)}
}
...
var selectedIds = "<%=laiwen%>";
var startIndex = 0;
function onLoadEnd(){
	try{
		var office = weaver.WebOffice.WebObject;
		var bookMarkName = "Body";
		if(!gwid){
			loadContent();
			/*try{
				office.Bookmarks("Body");
			}catch(e){
				alert("模板有误，排版失败，请联系管理员");
				return;
			}
			try{
				var range = office.Bookmarks("periods").range;
				range.text="<%=periods%>";	
				office.Bookmarks.Add("periods",range);
			}catch(e){alert(e);}
			for(var obj in data){
				var dataList = data[obj] ? data[obj].data : [];
				var id = data[obj] ? data[obj].id : "";
				...
				if(id && name){
					var con = "【" + name + "】\n";
					var bookMark = office.Bookmarks(bookMarkName);	
					var end = bookMark.end;
					if(!startIndex)
						startIndex = end;
					var newRange = office.Range(end,end);
					newRange.Font.Name = "仿宋_GB2312";
					newRange.Font.Size = 14;
					newRange.font.Bold = true;
					newRange.text = con;
					office.Bookmarks.Add("lm_"+id,newRange);
					bookMarkName = "lm_"+id;
					for(var i=0;i<dataList.length;i++){
						var cid = dataList[i].id;
						var title = dataList[i].title;
						var content = dataList[i].content;
						var unit = dataList[i].unit;
						unit = unit?("（"+unit+"）"):"";
						bookMark = office.Bookmarks(bookMarkName);
						end = bookMark.end;
						newRange = office.Range(end,end);
						newRange.Font.Name = "仿宋_GB2312";
						newRange.Font.Size = 14;
						newRange.font.Bold = true;
						newRange.text = "▲" + title + "。";
						office.Bookmarks.Add("nr_"+cid+"_title",newRange);
						var _end = office.Bookmarks("nr_"+cid+"_title").end;
						newRange = office.Range(_end,_end);
						newRange.font.Bold = false;
						newRange.Font.Name = "仿宋_GB2312";
						newRange.Font.Size = 14;
						newRange.text = content+unit+"\n";
						//newRange = office.Range(end,office.Bookmarks("body").end-1);
						office.Bookmarks.Add("nr_"+cid,newRange);
						bookMarkName = "nr_"+cid;
					}
				}
			}
			office.Bookmarks("Body").Delete();
			bookMark = office.Bookmarks(bookMarkName);
			var end = bookMark.end;
			office.Bookmarks.Add("Body",office.Range(end,end));*/
		}else{
			for(var obj in data){
				try{
					var id = data[obj] ? data[obj].id : "";
					var name = data[obj] ? data[obj].name : "";
					if(id && name){
						startIndex = office.Bookmarks("lm_"+id).start;
						break;
					}
				}catch(e){
					alert(e);
...
function contentReload(){
	window.top.Dialog.confirm("您确认要重新从模板生成文档吗？",function(){
	    //weaver.WebOffice.WebUrl="<%=mServerUrl%>";
	    weaver.WebOffice.RecordID="<%=mbvid%>";
	    //weaver.WebOffice.Template="";
	    //weaver.WebOffice.FileName="";
		//try{
		//	weaver.WebOffice.Compatible  = false;
	    ...
	    //weaver.WebOffice.FileType="<%=docType%>";
		//weaver.WebOffice.EditType="<%=editType%><%=canPostil%>";
		var flag = weaver.WebOffice.WebOpen();
		if(flag){
			loadContent();
		}else{
			alert("模板加载失败，请联系管理员。");
...
function checkStyle(){
	//weaver.WebOffice.WebObject.Paragraphs(14).range.ParagraphFormat.CharacterUnitFirstLineIndent=2;//首行缩进
	//weaver.WebOffice.WebObject.Paragraphs(14).range.ParagraphFormat.LineSpacingRule=1;//行间距
	//SpaceBefore//段落前间距
	//var office = weaver.WebOffice.WebObject;
	//var count = 
	/*var paragraphCount = weaver.WebOffice.WebObject.Paragraphs.count;
	for(var i=1;i<=paragraphCount;i++){
		try{
			var range = weaver.WebOffice.WebObject.Paragraphs(i).range;
			if(range.text.trim()){
...
function checkBody(){
	...
	try{//修改时判断body标签是否存在
		office.Bookmarks("Body");
	}catch(e){//如果不存在  重新在最后一个书签后面追加  Body书签
		try{
			var count = office.Bookmarks.count;
			var end = 0;
			for(var i=1;i<=count;i++){//循环找到最后一个书签的位置
				var indexname = office.Bookmarks(i).name;
				if(indexname != "periods"){
					var _end = office.Bookmarks(i).range.end;
					if(_end > end)
						end = _end;
				}
			}
			if(end == 0){
				office.select();
				end = office.Application.Selection.Range.end - 32;//默认给页尾预留32个字符位置
			}
			var newRange = office.Range(end,end);
			office.Bookmarks.Add("Body",newRange);
		}catch(e){
			return false;
		}
	}
...
function changeContent(jsonData){
	var _data = jsonData.returnMap;
	var returnList = jsonData.returnList;
	selectedIds = jsonData.returnStr;
	var bookMarkName = "Body";
	var sbookMarkId = "";//第一个书签的开始位置
	if(!checkBody){
		alert("文档已经损坏");
		return;
	}
	try{
		...
		for(var obj in _data){
			try{
				flag = true;
				var dataList = _data[obj] ? _data[obj].data : [];
				var id = _data[obj] ? _data[obj].id : "";
				var name = _data[obj] ? _data[obj].name : "";
				var dataListOld = [];
				if(id && name){
					if(obj in data){
						office.Bookmarks("lm_"+id).range.text = "";
						dataListOld = data[obj].data;
					}
					if(!sbookMarkId)
						sbookMarkId = id;
					var con = "【" + name + "】\n";
					...
					for(var i=0;i<dataList.length;i++){
						...
						if(checkArrayContain(listData,cid)){//不是新增的文档 取当前内容中信息
							content = office.Bookmarks("nr_"+cid).range.text+"\n";
							//title = office.Bookmarks("nr_"+cid+"_title").range.text;
							office.Bookmarks("nr_"+cid).range.text = "";
							office.Bookmarks("nr_"+cid+"_title").range.text = "";
						}
						bookMark = office.Bookmarks(bookMarkName);
						end = bookMark.end;
						newRange = office.Range(end,end);
		...
		for(var i=0;i<listData.length;i++){
			try{
				if(!checkArrayContain(returnList,listData[i].id)){
					office.Bookmarks("nr_"+listData[i].id).range.text = "";
					office.Bookmarks("nr_"+listData[i].id+"_title").range.text = "";
				}
			}catch(e){console.log(e);}
		}
		for(var obj in data){
			try{
				if(!(obj in _data)){
					office.Bookmarks("lm_"+data[obj].id).range.text = "";
				}
			}catch(e){console.log(e);}
		}
		if(flag){
			...
			try{
				office.Range(startIndex,office.Bookmarks("lm_"+sbookMarkId).start).text="";
			}catch(e){
				console.log(e);
			}
		}
...
function showMore(){
	...
	dialog.URL = "/systeminfo/BrowserMain.jsp?url="+escape("/govern/information/ContentSelect.jsp?fwid=<%=fwid%>&kx=<%=journal%>&sid="+selectedIds+"&loadFormTop=1");
...
function changeRead(){
	var office = weaver.WebOffice.WebObject;
	var count = office.Shapes.count;
	for(var i=1;i<=count;i++){
		try{
			var type = office.Shapes.Range(i).type;
			if(type == 9){
				office.Shapes.Range(i).line.visible = flag;
			}else if(type == 17){
				if(office.Shapes.Range(i).name != "periods")
					office.Shapes.Range(i).visible = flag;
				//office.Shapes.Range(i).TextFrame.TextRange.text = "";
			}
		}catch(e){}
	}
	flag = !flag;
...
<%
	...
	if(editType==0){
		...
		RCMenu += "{"+SystemEnv.getHtmlLabelName(15846,user.getLanguage())+",javascript:contentReload(),_top} " ;
		RCMenuHeight += RCMenuHeightStep ;
...
<input type="hidden" name="docsubject" id="docsubject" value="<%=("".equals(docsubject)||!sourcedocsubject.equals(docsubject))?sourcedocsubject:docsubject%>">
...
<div id="divContentTab" style="width:100%;height:100%;">
	...
	<tr>
		...
		<%}else if(editType==1||editType==2){%>
			...
			<input type=button class="e8_btn_top" onclick="contentReload();" value="<%=SystemEnv.getHtmlLabelName(15846, user.getLanguage())%>"></input>
...
function checkArrayContain(list,cid){
...
function saveDetail(){
	var list = [];
	for(var i=0;i<listData.length;i++){
		var obj = {id:listData[i].id,index:listData[i].index,typeid:listData[i].typeid};
		list.push(obj);
	}
	var data = {list:list};
	jQuery.ajax({    
		...
		cache:false,    
		dataType:'json',
		data:{jsonData:JSON.stringify(data),billid:<%=fwid%>},
		async : false,
...
function onSave(){
	...
	<% 
	RecordSet.executeSql("update uf_xxcb_pbInfo  set kw_zt = '0' where id="+fwid);
	%>
}
