...
String nodeid=null;
String rem=(String)session.getAttribute("PlatformLeft");
        if(rem==null){
        Cookie[] cks= request.getCookies();

        for(int i=0;i<cks.length;i++){
        //System.out.println("ck:"+cks[i].getName()+":"+cks[i].getValue());
        if(cks[i].getName().equals("PlatformLeft"+uid)){
        rem=cks[i].getValue();
        break;
        }
        }
...
session.setAttribute("PlatformLeft",rem);
Cookie ck = new Cookie("PlatformLeft"+uid,rem);
...
<BODY onload="initTree()" style="overflow-y: hidden;" scroll=no>
	<FORM NAME=SearchForm STYLE="margin-bottom:0" action="platformListTab.jsp" method=post target="contentframe">
...
<table height="100%" width=100% class="flowsTable" valign="top">
	...
	<TR>
	<td height="100%" style="width:23%;" class="flowMenusTd">
			<div class="flowMenuDiv"  >
				<div style="overflow:hidden;height:300px;position:relative;" id="overFlowDiv">
					<div class="ulDiv">
						<div id="deeptree" class="cxtree" CfgXMLSrc="/css/TreeConfig.xml" style="width:219px;margin-top:-35px;" />
					</div>
				</div>
			</div>
...
function initTree(){
CXLoadTreeItem("", "PlatformXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
