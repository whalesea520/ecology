...
private boolean hasUsed(weaver.conn.RecordSet rs, String fieldname){
	...
	rs.execute("select count(*) from CRM_CustomerInfo where "+fieldname+"<>'' and "+fieldname+" is not null ");
	if(rs.next()){
...
<input type="hidden" name="whickField" value="c1"/>
...
<TABLE cellSpacing=0 cellPadding=1 width="100%" border=0 id="inputface" class=ListStyle>
  	...
  	<%
  	//老字段信息
		RecordSet.executeSql("select * from Base_FreeField where tablename = 'c1'");
		RecordSet.first();
...
CustomFieldManager cfm = new CustomFieldManager("CrmCustomFieldByInfoType",84);
cfm.getCustomFields();
    ...
    while(cfm.next()){
    	boolean isUsed = CustomFieldManager.fieldIsUsed("CrmCustomFieldByInfoType", 84, cfm.getId());
    %>
    <tr>
  	<td style="border-bottom:silver 1pt solid">
