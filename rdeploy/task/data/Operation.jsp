...
<%
	...
	if("add".equals(operation)){
		...
		if(res){
			...
			<script type="text/javascript">
				if(<%=saveType%>==0){
					try{
						top.IM_Ext.showMsg("创建成功");
						top.IMUtil.doHideSlideDiv(top.document,function(){
							top.IM_Ext._addMsgTag(2, "<%=taskId%>", "<%=msgid%>");
						});
					}catch(e){
						window.close();
					}
					...
					alert("保存成功");
					location.href="/workrelate/task/data/Add.jsp?saveType=1";
				}else if(<%=saveType%>==3){
		...
		if(!taskId.equals("") && !status.equals("")){
			...
			if(status.equals("2")){
				...
				sql = "insert into TM_TaskFeedback (taskid,content,hrmid,docids,wfids,crmids,projectids,meetingids,fileids,createdate,createtime)"
					+" values("+taskId+",'"+content+"',"+user.getUID()+",'','','','','','','"+currentDate+"','"+currentTime+"')";
				rs.executeSql(sql);
				restr.append("$<tr><td class='data fbdata'><div class='feedbackshow'><div class='feedbackinfo'>"
						+"<div class='fbimg'><img src='"+rc.getMessagerUrls(user.getUID()+"")+"' width='43' height='43'/></div>"
						+"<div class='fbuser'>"+cmutil.getPerson(user.getUID()+"")+"</div>"
						+"<div class='fbtime'>"+currentDate+" "+currentTime+"</div></div>"
	...
	if("add_feedback".equals(operation)){
		...
		if(!taskId.equals("") && !content.equals("")){
			...
			restr.append("<div class='feedbackinfo'><div class='fbimg'><img src='"+rc.getMessagerUrls(user.getUID()+"")+"' width='43' height='43'/></div>");
			restr.append("<div class='fbuser'>"+rc.getLastname(user.getUID()+"")+"</div>");
			restr.append("<div class='fbtime'>"+currentDate+" "+currentTime);
			restr.append("</div>");
			restr.append("<div class='fboperate'>");
			restr.append("<img class='fb_replyimg' src='/rdeploy/task/img/reply1.png'/><a href=\"javascript:doReply('"+fbid+"')\">回复</a>");
			restr.append(this.getDelOperate(fbid,userid,userid,currentDate,currentTime)+"</div>");
			restr.append("</div>");
			//restr.append("<div class='pointleft'></div>");
	...
	if("get_feedback".equals(operation)){
		...
		while(rs.next()){
			...
			restr.append("<tr><td id='fbdata_"+fbid+"' class='data fbdata'><div class='feedbackshow'>");
			restr.append("<div class='feedbackinfo'><div class='fbimg'><img src='"+rc.getMessagerUrls(rs.getString("hrmid"))+"' width='43' height='43'/></div>");
			restr.append("<div class='fbuser'>"+rc.getLastname(rs.getString("hrmid"))+"</div>");
			restr.append("<div class='fbtime'>"+Util.null2String(rs.getString("createdate"))+" "+Util.null2String(rs.getString("createtime")));
			...
			restr.append("<div class='fboperate'>");
			restr.append("<img class='fb_replyimg' src='/rdeploy/task/img/reply1.png'/><a href=\"javascript:doReply('"+fbid+"')\">回复</a>");
			restr.append(this.getDelOperate(fbid,rs.getString("hrmid"),userid,rs.getString("createdate"),rs.getString("createtime"))+"</div>");
			restr.append("</div>");
			//restr.append("<div class='pointleft'></div>");
			restr.append("<div class='feedbackrelate'>");
			restr.append("	<div>"+Util.null2String(rs.getString("content"))+"</div>");
			if(!"".equals(docids)){
	...
	if("get_toplog".equals(operation)){
		...
		if(logcount>0){
			String ssql = "select top 7 id,type,operator,operatedate,operatetime,operatefiled,operatevalue from TM_TaskLog where taskid=" + taskId +" order by id desc";
			if(rs.getDBType().equals("oracle")){
	...
	%>
			<tr id="item_tr_<%=taskid%>" class="item_tr" _taskid="<%=taskid%>">
				<td class='item_td'>
					<div class="newshownamediv">
						<div class="showIndex"></div>
						<div id="status_<%=taskid %>" class="status status<%=_status%>">&nbsp;</div>
						<div style="float:left;padding-left:2px;" id="div_<%=taskid%>"><%=Util.null2String(rs.getString("name")) %></div>
						<div id="enddate_<%=taskid %>" class="div_enddate" title='<%=enddate%>'><%=this.convertdate(enddate) %></div>
					</div>
					<div class="newshowzrrdiv" id="zrr_<%=taskid%>">
						负责人:<%=rc.getLastname(rs.getString("principalid")) %>
					</div>
				</td>
			</tr>
		<%if(sorttype==4){ %>
...
<%			
		...
		if(ifMySelf==1){//查看的是下属的
			...
			if(rc.isManager(Util.getIntValue(userid),hrmid)){
				...
				%>
					...
					%>
						<td>
							<table class="datatable" style="background:#fff;">
					...
					%>
								<tr>
									<td>
										<div onclick="refreshDetail(<%=taskId %>)" style="font-weight: bold;border-bottom:1px solid #e4e4e4;margin-right:10px;margin-left:10px;margin-bottom:10px;line-height:40px;height:40px;"><%=taskName %></div>
										<div class="taskFb">
											<%for(int j=0;j<fbList.size();j++){
												String[] fbArrays = (String[])fbList.get(j);
											%>
												<div class="feedbackrelate" style="margin-bottom:5px;border:1px solid #e4e4e4;;margin-left:10px;">
					...
					<%}else{%>	
						<td><div style="height:40px;background:#fff;border:1px solid #e4e4e4;"><span class="spanTips">当日暂无反馈记录!</span></div></td>
					<%}%>
					</tr>
					<%	
					}
	...
	%>
	<tr>
		<td class="data" style="border-bottom:none;background:#f7f7f7;border-top:none;">
			<div class="feedbackshow" style="border:1px solid #e4e4e4;">
				<div class="feedbackinfo" onclick="refreshDetail(<%=Util.null2String(rs.getString("taskid")) %>)"
					 style="font-weight: bold;border-bottom:1px solid #e4e4e4;margin-right:10px;margin-left:10px;margin-bottom:10px;line-height:40px;height:40px;">
				<%=Util.null2String(rs.getString("name")) %>
				</div>
				<div class="feedbackinfo" style="margin-left:10px;">
					<div class="fbimg"><img src="<%=rc.getMessagerUrls(rs.getString("hrmid")) %>" width="43" height="43"/></div>
					<div class="fbuser"><%=rc.getLastname(rs.getString("hrmid")) %> </div>
					<div class="fbtime"><%=Util.null2String(rs.getString("createdate")) %>  <%=Util.null2String(rs.getString("createtime")) %>
					<%if(isnew){ %><font style='color: red;margin-left: 5px;font-style: italic;font-size: 11px;cursor: pointer;' title='新反馈'>new</font><%} %>
					</div>
				</div>
				<div class="feedbackrelate" style="border:none;background:#fff;">
...
%>
			<tr id="item_tr_<%=taskid%>" class="item_tr" _taskid="<%=taskid%>">
				<td class='item_td'>
					<div class="newshownamediv">
						<div class="showIndex"></div>
						<div id="status_<%=taskid %>" class="status status<%=status%>">&nbsp;</div>
						<div style="float:left;padding-left:2px;" id="div_<%=taskid%>"><%=Util.null2String(rs.getString("name")) %></div>
						<div id="enddate_<%=taskid %>" class="div_enddate" title='<%=enddate%>'><%=this.convertdate(enddate) %></div>
					</div>
					<div class="newshowzrrdiv" id="zrr_<%=taskid%>">
...
<%!
	...
	*/
	...
	private String getlogtext(String userid,String datetime,int type,String field,String value,Map fn,String taskId){
		String logtxt = "<div class='logitem'><div class='logtitle'>"+getLink("hrmid",userid+"",taskId)+"&nbsp;&nbsp;<font class='datetxt'>"+datetime+"</font></div><div class='logcontent'>";
	...
	*/
	...
	public String getSubTask(String maintaskid,User user,int colspan,int showallsub) throws Exception{
		...
		//if(rs.getCounts()>0){
			res.append("<tr class='subtable_tr'><td style='padding:0px;padding-left:20px;border:0px;height:auto'><table class='subdatalist' cellpadding='0' cellspacing='0' border='0' align='center'>");
			String statustitle = "";
			int _status = 0;
			boolean canEdit = true;
			int right = 0;
			...
			while(rs.next()){
				...
				res.append("<td class='item_td'><div class='newshownamediv'>");
				res.append("<div class=''></div>");
				res.append("<div id='status_"+taskid+"' class='status status"+_status+"'>&nbsp;</div>");
				res.append("<div style='float:left;padding-left:2px;' id='div_"+taskid+"'>"+Util.null2String(rs.getString("name"))+"</div>");
				res.append("<div id='enddate_"+taskid+"' class='div_enddate' title='"+enddate+"'>"+this.convertdate(enddate)+"</div>");
				res.append("</div>");
				res.append("<div class='newshowzrrdiv' id='zrr_"+taskid+"'>");
				res.append("负责人:"+rc.getLastname(rs.getString("principalid")));
				res.append("</div>");
				res.append("</td>");
				res.append("</tr>");
