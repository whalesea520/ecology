...
function FileProgress(file, targetID) {
	try {
		...
		if(!filetype && file.name){
		  filetype = file.name.indexOf(".") > -1 ? file.name.substring(file.name.lastIndexOf(".")) : "";
		}
		if (filetype == ".doc") {
            filetype = "doc";
            ...
            filetype = "zip";
        }else if (filetype == ".html" || filetype == ".htm" || filetype == ".htmlx") {
            filetype = "html";
        } else if (filetype == ".jpg" || filetype == ".png" || filetype == ".bpm" || filetype == ".gif") {
            filetype = "jpg";
        } else {
            filetype = "general_icon";
        }
        filetype += ".png";
		fileIcon.attr("src", "/rdeploy/assets/img/cproj/doc/fileicon/small/"+filetype);
...
function cancelUploadFile(file_id) {
         if(file_id.indexOf("SWFUpload_0_") == 0){
			oUploadannexupload.cancelUpload(file_id, false);
         }else if(file_id.indexOf("SWFUpload_1_") == 0){
            oUploadannexuploadLi.cancelUpload(file_id, false);
         }else if(file_id.indexOf("SWFUpload_2_") == 0){
            oUploadannexuploadAdd.cancelUpload(file_id, false);
         }
		$("#" + file_id).fadeOut("slow",function(){
				   $(this).remove();
