...
<%
  ...
  int formatHeight = 0;
		...
		while (ite.hasNext()) {
			...
			if (!item.isFormField()) {
				...
				if (item.getFieldName().equals("id"))
					id = Util.null2String(item.getString("UTF-8"));
	...
	if ("delete".equals(method)) {
		//String strSql="update hrmresource set messagerurl='' where id='"+id+"'";		
		//rs.executeSql(strSql);
		ResourceComInfo resourceComInfo=new ResourceComInfo();
		//resourceComInfo.updateResourceInfoCache(""+user.getUID());

		out.println("<script>window.location='GetUserIcon.jsp?id="+id+"&isclosed=true&requestFrom="+requestFrom+"&iconUrl='</script>");
		
	}
	
	if ("usericon".equals(method)) {
		String iconName="id"+TimeUtil.getFormartString(Calendar.getInstance(),"yyyyMMddHHmmss")+".jpg";
		//生成缩略图		
        ...
        if(filerealpath==null || filerealpath.equals("")){
            out.println("<script>window.location='GetUserIcon.jsp?id="+temploginid+"&requestFrom="+trmphomepage+"'</script>");
            return;
          }
		...
		//id、account字段整合  qc:128484
		//if(mode.equals("ldap")) whereSql = "and account='"+id+"'";
		//if(mode.equals("ldap")) whereSql = "and id='"+id+"'";
		//String strSql="update hrmresource set messagerurl='/messager/usericon/"+iconName+"' where 1=1 "+whereSql;	
		//rs.executeSql(strSql);
		//ResourceComInfo resourceComInfo=new ResourceComInfo();
		//resourceComInfo.updateResourceInfoCache(""+user.getUID());
		out.println("<script>window.location='GetUserIcon.jsp?id="+id+"&isclosed=true&requestFrom="+requestFrom+"&imageid="+imagefileid+"&iconUrl=/messager/usericon/"+iconName+"'</script>");
