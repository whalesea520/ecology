...
(function () {
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            var remote = false, list = [], id, tabs = $G('tabhead').children,imagefileid;
            for (var i = 0; i < tabs.length; i++) {
				...
				if(id==='remote'){
                  ...
                  remote && editor.fireEvent("catchRemoteImage");
					...
					for(var i=0;i<list.length;i++){
					   imagefileid=list[i].imagefileid;
					   //插入图片
					   editor.execCommand('inserthtml', "<img src='/weaver/weaver.file.FileDownload?fileid="+imagefileid+"' alt='' />"); 
					}
                    remote && editor.fireEvent("catchRemoteImage");
				}
            }
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                //上传url地址
                actionUrl = "/docs/docs/DocImgUploadOnlyForPic.jsp?userid="+window.top.udesid+"&usertype="+window.top.utype+"&docfiletype=1",
