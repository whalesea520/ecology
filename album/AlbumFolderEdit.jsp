...
if("<%=isclose%>"=="1"){
	try{
		...
		/**
		if("<%=from %>"=="photoview"){
			parent.window.reloadPhotoInfo(<%=id %>);
		}**/
	}catch(e){}
}
</script>
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=folderName %>"/>
</jsp:include>
...
<wea:layout>
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("1361",user.getLanguage())%>' attributes="" >
		...
		<wea:item><input type="text" name="photoDescription" class="inputstyle" maxlength="20" value='<%=folderDescription%>' /></wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(338,user.getLanguage())%></wea:item>
		<wea:item><input type="text" name="orderNum" class="inputstyle" maxlength="3" value='<%=orderNum%>' /></wea:item>
	</wea:group>
</wea:layout>
</form>
