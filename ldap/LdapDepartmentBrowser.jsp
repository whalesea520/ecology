...
<HEAD>
	...
	<script type="text/javascript" src="/wui/common/jquery/plugin/zTree/js/jquery.ztree.excheck.min_wev8.js"></script>
		...
		try{
			parent.setTabObjName("<%=SystemEnv.getHtmlLabelNames("19438",user.getLanguage())%>");
		}catch(e){
...
String hrmtempselectedids = Util.null2String(request.getParameter("selectedids"));
String selectedids = "";
RecordSet.executeSql("select departmentid from HrmResourceTemp where id="+hrmtempselectedids);
if(RecordSet.next()) {
	selectedids = RecordSet.getString("departmentid");
} else {
	selectedids = "";
}
int deptlevel = 0;
...
<script type="text/javascript">
	...
	function onSave() {
	    ...
	    if(trunStr != "") {
			...
			var returnjson = {id:returnVBArray[0], name:returnVBArray[1]};
         	if(dialog){
				try{
					//alert(returnjson.id+":"+returnjson.name);
					
					updateDepartment(returnjson.id);
	...
	function updateDepartment(departmentid) {
		$.ajax({
			url : "/ldap/DesignateOperation.jsp?operation=department&ids="+"<%=hrmtempselectedids%>" +"&departmentid="+departmentid,
			type : "post",
			datatype : "json",
