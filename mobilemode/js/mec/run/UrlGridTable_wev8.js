Mobile_NS.UrlGridTable = {};

Mobile_NS.UrlGridTable.onload = function(mec_id){
	...
	this.fixTableWrapHeightStr(mec_id);
};

Mobile_NS.UrlGridTable.bindEvent = function(mec_id){
	var that = this;
...
Mobile_NS.UrlGridTable.delHtmlTag = function(str){
	return str.replace(/<[^>]+>/g,"");//去掉所有的html标记
};

Mobile_NS.UrlGridTable.groupSum = function(mec_id){
	...
	$(".gtWrap .gtContentWrap > table.gtContent tr.content_tr", $container).each(function(i){
		...
		$row.children("td").each(function(j){
			...
			if(!isNaN(dv) && !isNaN(sv)){
				sumArr[j] = Mobile_NS.UrlGridTable.accAdd(sv, dv);
			}else{
				dv = dv.replace(/,/g,"");
				if(!isNaN(dv)){// 金额千分位
					sv = sv.replace(/,/g,"");
					var sv = Mobile_NS.UrlGridTable.accAdd(sv, dv);
					sumArr[j] = Mobile_NS.UrlGridTable.changeToThousands(sv);
				}
			}
		});
...
Mobile_NS.UrlGridTable.accAdd = function(num1,num2){// 两个浮点数求和
...
Mobile_NS.UrlGridTable.initAdvancedSearch = function(mec_id){
	var $container = $("#NMEC_" + mec_id);
...
Mobile_NS.UrlGridTable.showAdvancedSearch = function(id){
	...
	Mobile_NS.setViewOpen(Mobile_NS.UrlGridTable.hideAdvancedSearch);
};

Mobile_NS.UrlGridTable.hideAdvancedSearch = function(){
...
Mobile_NS.UrlGridTable.initDate = function(eleid, eletype){
...
Mobile_NS.UrlGridTable.initAdvancedSearchContent = function(mec_id){
	...
	Mobile_NS.ajax(url, Mobile_NS.pageParams, function(responseText){
		...
		for(var i = 0; i < data.length; i++){
			...
			if(htmltype == "5"){	/*select*/
						...
						"<div search=\"true\" fieldtype=\"date\" fieldname=\""+fieldname+"\" fieldid=\""+fieldid+"\" style=\"height:100%;\">" +
							"<table class=\"rangeSearchTable\">" +
								"<tr>" +
									...
									"<input class=\"needsclick\" id=\"_as_fieldstart"+fieldid+"\" type=\"text\" placeholder=\"开始日期\" data-clear-btn=\"false\" onchange=\"Mobile_NS.UrlGridTable.showClearBtn('fieldstart', "+fieldid+");\">" +
									...
									"<input class=\"needsclick\" id=\"_as_fieldend"+fieldid+"\" type=\"text\" placeholder=\"结束日期\" data-clear-btn=\"false\" onchange=\"Mobile_NS.UrlGridTable.showClearBtn('fieldend', "+fieldid+");\">" +
					...
					$fieldContent.append(
						"<div search=\"true\" fieldtype=\"time\" fieldname=\""+fieldname+"\" fieldid=\""+fieldid+"\" style=\"height:100%;\">" +
							"<table class=\"rangeSearchTable\">" +
								"<tr>" +
									"<td class=\"rangeSearchcol1\">" +
									"<input class=\"needsclick\" id=\"_as_fieldstart"+fieldid+"\" type=\"text\" placeholder=\"开始时间\" data-clear-btn=\"false\" onchange=\"Mobile_NS.UrlGridTable.showClearBtn('fieldstart', "+fieldid+");\">" +
									"<div class=\"dataClean\" id=\"fieldstart"+fieldid+"\"></div>" +
									"</td>" +
									"<td class=\"rangeSearchcol2\"><div class=\"field_range_separater\"></div></td>" +
									"<td class=\"rangeSearchcol3\">" +
									"<input class=\"needsclick\" id=\"_as_fieldend"+fieldid+"\" type=\"text\" placeholder=\"结束时间\" data-clear-btn=\"false\" onchange=\"Mobile_NS.UrlGridTable.showClearBtn('fieldend', "+fieldid+");\">" +
									"<div class=\"dataClean\" id=\"fieldend"+fieldid+"\"></div>" +
									"</td>" +
								"</tr>" +
							"</table>" +
				...
				$fieldContent.append("<input type=\"text\" placeholder=\"请输入...\" fieldtype=\"textarea\" search=\"true\" fieldname=\""+fieldname+"\" fieldid=\""+fieldid+"\"/>");
			}else if(htmltype == "1"){/*单行*/
				var textType = oneData["textType"];
				if(textType == "1"){//文本
					$fieldWrap.addClass("text-field");
					$fieldContent.append("<input type=\"text\" placeholder=\"请输入...\" fieldtype=\"text\" search=\"true\" fieldname=\""+fieldname+"\" fieldid=\""+fieldid+"\"/>");
					...
					$fieldContent.append(
						"<div search=\"true\" fieldtype=\"thousandnumb\" fieldname=\""+fieldname+"\" fieldid=\""+fieldid+"\" style=\"height:100%;\">" +
							"<table class=\"rangeSearchTable\">" +
								"<tr>" +
									"<td class=\"rangeSearchcol1\"><input id=\"_as_fieldstart"+fieldid+"\" type=\"number\" placeholder=\"大于等于\" data-clear-btn=\"false\"></td>" +
									"<td class=\"rangeSearchcol2\"><div class=\"field_range_separater\"></div></td>" +
									"<td class=\"rangeSearchcol3\"><input id=\"_as_fieldend"+fieldid+"\" type=\"number\" placeholder=\"小于等于\" data-clear-btn=\"false\"></td>" +
								"</tr>" +
							"</table>" +
						"</div>"
					);
			...
			if(isDateTime){
				Mobile_NS.UrlGridTable.initDate("_as_fieldstart"+fieldid, dateTimeType);
				Mobile_NS.UrlGridTable.initDate("_as_fieldend"+fieldid, dateTimeType);
			}
		}
	...
	$searchBtn.on("click", function(){
		...
		setTimeout(function(){
			Mobile_NS.UrlGridTable.refresh(mec_id, listparams);
		}, 300);
	});
...
Mobile_NS.UrlGridTable.resetGridColumnWidth = function(mec_id){
	var that = this;
...
Mobile_NS.UrlGridTable.getHorizontalPadding = function($obj){
...
Mobile_NS.UrlGridTable.getVerticalPadding = function($obj){
...
Mobile_NS.UrlGridTable.getDynamicParam = function(mec_id){
...
Mobile_NS.UrlGridTable.loadMoreData = function(mec_id, totalPageCount){
	var that = this;
...
Mobile_NS.UrlGridTable.refresh = function(mec_id, paramsstr, callbackFn){
	var that = this;
	...
	Mobile_NS.ajax(url, requestParam, function(data){
		...
		that.resetGridColumnWidth(mec_id);
...
Mobile_NS.UrlGridTable.search = function(mec_id){
	var that = this;
...
Mobile_NS.UrlGridTable.bindDataEvent = function(mec_id){
	var $container = $("#NMEC_" + mec_id);
...
Mobile_NS.UrlGridTable.buildFixedColumn = function(mec_id){
...
Mobile_NS.UrlGridTable.updateFixedContentColumn = function(mec_id){
	var that = this;
...
Mobile_NS.UrlGridTable.showClearBtn = function(z, fieldid){
...
Mobile_NS.UrlGridTable.fixCloneTableWidth = function($table, mec_id){
...
Mobile_NS.UrlGridTable.fixTableWidthStr = function(mec_id){
	var that = this;
...
Mobile_NS.UrlGridTable.changeToThousands = function(sv){
	sv = sv.replace(/\s+/g,""); 
	if(sv != ""){
		sv = Mobile_NS.UrlGridTable.commafy(sv);
	}
	return sv;
};

Mobile_NS.UrlGridTable.addZero = function(aNumber,precision){
 ...
 * @param{Object}num  
 */ 
Mobile_NS.UrlGridTable.commafy = function(num){
	num = num + "";   
...
Mobile_NS.UrlGridTable.fixTableWrapHeightStr = function(mec_id){
	var that = this;
	var $container = $("#NMEC_" + mec_id);
	$(".gtOutWrap", $container).css("height", $(".gtWrap", $container).height()+"px");
};