...
Mobile_NS.buildTree = function(mec_id, treeid, pid, pname, unCreateTitle){
	...
	if($treepage.length == 0){
		...
		var url = jionActionUrl("com.weaver.formmodel.mobile.mec.servlet.MECAction", "action=getTreeData&treeid="+treeid+"&pid="+pid+"&appid="+appid+"&mec_id="+mec_id);
...
Mobile_NS.bindTreeBack = function(mec_id, treeid){
	var $treeContainer = $("#tree" + mec_id);
	...
	$toParent.click(function(e){
		...
		if($preNode.length > 0){
			...
			var nodeid = $preNode.attr("nodeid");
			var nodename = $preNode.attr("nodename");
			$preNode.nextAll().remove();
			Mobile_NS.buildTree(mec_id, treeid, nodeid, nodename, true);
		}
		e.stopPropagation(); 
	});
...
Mobile_NS.bindTreeSearch = function(mec_id, treeid){
	...
	$searchBtn.click(function(e){
		...
		if(searchKeyV != ""){
			...
			var url = jionActionUrl("com.weaver.formmodel.mobile.mec.servlet.MECAction", "action=getTreeDataWithSearch&treeid="+treeid+"&appid="+appid+"&mec_id="+mec_id+"&searchKey="+encodeURI(encodeURI(searchKeyV)));
			$.post(url, null, function(responseText){
				$treeloading.hide();
				...
				$(".treepage:not(.searchpage)", $treeContainer).each(function(){
					if(!Mobile_NS.isHidden($(this))){
						$(this).attr("prev_treepage", "true").hide();
					}
				});
				...
				$searchText.show();
				var $searchCanel = $();
				$searchInnerTextWrap.children(".searchCanel").each(function(){
					var _cev = $(this).attr("click_event");
					if(!_cev || _cev != "true"){
						$searchCanel = $searchCanel.add($(this));
					}
				});
