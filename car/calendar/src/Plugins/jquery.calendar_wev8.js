...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight) {
            ...
            for (var j = 0; j < rc; j++) {
                //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
                htb.push("<div id='mvrow_", j, "' style=\"HEIGHT:", pht, "px; TOP:", p * j, "%\"  class=\"month-row\">");
                htb.push("<table class=\"st-bg-table\" cellSpacing=\"0\" cellPadding=\"0\"><tbody><tr>");
                var dMax = B[j];

                for (var i = 0; i < 7; i++) {
                    var day = C[j * 7 + i];
                    htb.push("<td abbr='", dateFormat.call(day, i18n.xgcalendar.dateformat.fulldayvalue), "' style=\"HEIGHT:", (pht-1), "px;\" ch='qkadd' axis='00:00' title=''");
        ...
        function populate() {
            ...
            if (option.url && option.url != "") {
                ...
                $.ajax({
                    type: option.method, //
                    url: option.url+option.selectedUser+"&meetingType="+option.meetingType+"&selectedType="+option.selectedType+"&selectedDept="+option.selectedDept+"&selectedSub="+option.selectedSub+"&isShare="+option.isShare,
                    data: param,				   
                ...
                //$("#listframe").attr("src","/meeting/data/GetMeetingList.jsp?selectedType="+option.selectedType+"&selectedDept="+option.selectedDept+"&selectedSub="+option.selectedSub+"&selectUser="+option.selectedUser+"&isShare="+option.isShare+"&selectdate="+dateFormat.call(option.showday, i18n.xgcalendar.dateformat.fulldayvalue)+"&viewtype="+option.view+"&timezone="+zone);
            }
            else {
        ...
        function dochange() {
            ...
            if (!loaded) {
                populate();
            		...
            		option.onOtherOperate && option.onOtherOperate();
            }
        }
             ...
             if (!option.EditCmdhandler) {
             	...
             	if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                	 option.EditCmdhandler.call(this, 
                            		['0', dateFormat.call(start, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm"), 
