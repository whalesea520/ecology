; (function($) {
    ...
    $.fn.datepickernew = function(o) {
        ...
        if (cp.length == 0) {
            ...
            cpHA.push("<td colspan='4'><button id='BBIT-DP-MP-OKBTN' class='bbit-dp-mp-ok'>", def.btnOk, "</button><span style='height:20px;top:1px;line-height:20px;position:relative;color:#cccccc;margin-left: -3px;'>|</span><button id='BBIT-DP-MP-CANCELBTN' class='bbit-dp-mp-cancel'>", def.btnCancel, "</button></td>");
            cpHA.push("</tr>");
        ...
        function mpok() {
            ...
            $("#BBIT-DP-MP").animate({ top: -193 }, { duration: 200, complete: function() { $("#BBIT-DP-MP").hide(); } });
            if(def.selectMode == "1") {
				cp.data("indata", stringtodate(def.Year+"-"+def.Month + "-" + def.Day));
				returndate();
			} else {
				writecb();
				return false;
        ...
        return $(this).each(function() {
			...
			picker.attr("selectMode", def.selectMode);
            picker.click(function(e) {
				def.selectMode = $(this).attr("selectMode");
