...
String nodeid=null;
String rem=(String)session.getAttribute("CarInfoleft");
        if(rem==null){
        Cookie[] cks= request.getCookies();

        for(int i=0;i<cks.length;i++){
        if(cks[i].getName().equals("CarInfoleft"+uid)){
        rem=cks[i].getValue();
        break;
        }
        }
...
session.setAttribute("CarInfoleft",rem);
Cookie ck = new Cookie("CarInfoleft"+uid,rem);
...
function initTree(){
//CXLoadTreeItem("", "CarInfoXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
