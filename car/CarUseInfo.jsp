...
if(operation.equals("cancel")){
    ...
    String tablename = Util.null2String(Util.null2String(request.getParameter("tablename")),"CarUseApprove");
    String cancelname = Util.null2String(Util.null2String(request.getParameter("cancelname")),"cancel");
	if(!id.equals("")){
		...
		String sql = "update "+tablename+" set "+cancelname+"='1' where id="+id;
...
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(82290,user.getLanguage())+",javascript:newCarUse(),_self} " ;//TODO 车辆信息新建
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<BODY>
<FORM id=frmmain name=frmmain method=post action="CarUseInfo.jsp">
...
%>
    ...
    <div id="calhead" class="calHd" style="height:36px;padding-left:0px;min-width:900px !important;background-color: #f7f7f7;">
      		<div style="float:left;margin-left:10px;border:none;height:24px;"><!-- 显示/隐藏分部树 -->
      		...
      		<div  id="firstButtons" style="float:left;min-width:40px !important;">
      		    <%//TODO 新建车辆使用 %><!-- 新增用车申请 -->
      			<div id="faddbtn" class="calHdBtn faddbtn" title="<%=SystemEnv.getHtmlLabelName(82290,user.getLanguage())%>" style="border:none;height:25px;">
      			</div>
...
jQuery(document).ready(function(){
  ...
  $("#showdaybtn").click(function(e) {
   ...
   //Add a new car
   $("#faddbtn").click(function(e) { //TODO 增加车辆使用
   	 	//var url = "/car/CarInfoAdd.jsp?subCompanyId=-1&isUseCar=1";
   	 	newCarUse();
   });
...
function showCompanyTree(){
	...
	if($("#subCompanytifm").attr("src") == ""){
		$("#subCompanytifm").attr("src","/car/CarSubCompanyTree.jsp?rightStr=Car:Maintenance");
	}
...
function showCarinfoList(carid,event){
	this.carid = "";
	var selected = false;
	...
	if(jQuery(startElement).parent("td.carnames").hasClass("car-selecting")){
		...
		$("#listframe").attr("src","getCarInfoList.jsp?bywhat="+$GetEle("bywhat").value+"&datenow="+$GetEle("datenow").value+"&subids="+$GetEle("subids").value+"&content="+$GetEle("content").value);
	} else{
		...
		$("#listframe").attr("src","getCarInfoList.jsp?bywhat="+$GetEle("bywhat").value+"&datenow="+$GetEle("datenow").value+"&subids="+$GetEle("subids").value+"&content="+$GetEle("content").value+"&carid="+carid);
	}
...
function newCarUse() {
	var url = "/car/CarUseCreate.jsp";
   	newCarinfo(url);
}

function newCarinfo(url){
	...
	<%if(user.getLanguage() == 8){%>
	diag_vote.Title = "<%=SystemEnv.getHtmlLabelName(82290,user.getLanguage())%>";//新增用车申请
	<%} else {%>
	diag_vote.Title = "<%=SystemEnv.getHtmlLabelName(82290,user.getLanguage())%>";//新增用车申请
	<%}%>
	diag_vote.URL = url;
...
function doCancel(id,tablename,cancelname){
	document.frmmain.action="CarUseInfo.jsp?operation=cancel&id="+id+"&tablename="+tablename+"&cancelname="+cancelname;
...
function getCarUseInfo(){
    //得到车辆使用信息
    $.post("/car/AjaxCaruse.jsp",{bywhat:$GetEle("bywhat").value,currentdate:$GetEle("currentdate").value,movedate:$GetEle("movedate").value,content:encodeURI($GetEle("content").value),subids:$GetEle("subids").value},function(datas){
...
function setWindowSize(_document){
	...
	jQuery("#tablediv").css("max-height",bodyheight*0.7);
...
function clearVal(obj){
	if(obj.value=='<%=SystemEnv.getHtmlLabelName(82291,user.getLanguage())%>'){ //搜索车辆
		obj.value="";
...
function recoverVal(obj){
	if(obj.value==''||obj.value==null){
		obj.value="<%=SystemEnv.getHtmlLabelName(82291,user.getLanguage())%>";//搜索车辆
