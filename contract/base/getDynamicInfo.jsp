...
<%
    ...
    if ("getConsComment".equals(action)) {//获取合同评论
        ...
        dynamicServiceImpl.sava(dynamic);
            ...
            String sql = "insert into "+Constant.COMMENT_TABLENAME+" (dataid,moduleid,content,create_date,create_time,create_usr,requestId) values('" + cid + "','" + moduleid + "','" + content + "','" + date + "','" + time + "','" + userid + "','"+requestId+"')";
            RecordSet rs = new RecordSet();
            rs.executeSql(sql);
            RecordSet rs1 = new RecordSet();
            String id = "";
            rs1.executeSql("select id from "+Constant.COMMENT_TABLENAME+" where requestId = '"+requestId+"'");
            while(rs1.next()){
                 id = rs1.getString("id");
            }
            if(!"".equals(id)){
        ...
        String sql = "insert into "+Constant.COMMENT_TABLENAME+" (pid,dataid,moduleid,content,create_date,create_time,create_usr) values('" + pid + "','" + cid + "','" + moduleid + "','" + content + "','" + date + "','" + time + "','" + userid + "')";
        RecordSet rs = new RecordSet();
        rs.executeSql(sql);
        if("comment".equals(dynamicType)){
        	 comment.setDataid(cid);
        ...
        String sql = "delete from "+Constant.COMMENT_TABLENAME+" where id='" + data_id + "'";
        RecordSet rs = new RecordSet();
        rs.executeSql(sql);
        if("comment".equals(dynamicType)){
        	 comment.setDataid(cid);
    	...
    	String data_id = request.getParameter("data_id");
        String content = URLDecoder.decode(request.getParameter("content"), "UTF-8");//内容
        String sql = "update "+Constant.COMMENT_TABLENAME+" set content='" + content + "' where id=" + data_id;
        RecordSet rs = new RecordSet();
        rs.executeSql(sql);
        if("comment".equals(dynamicType)){
        	 comment.setDataid(cid);
