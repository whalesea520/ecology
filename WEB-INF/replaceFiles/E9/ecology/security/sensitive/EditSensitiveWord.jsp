...
rs.executeQuery("select * from sensitive_words where id = ?",id);
String word = "";
if(rs.next()){
...
<wea:layout>
	<wea:group context="<%=SystemEnv.getHtmlLabelName(61,user.getLanguage())+SystemEnv.getHtmlLabelName(87,user.getLanguage())%>">
		...
		<wea:item>
			<wea:required id="wordspan" required="true" value="<%=word%>">
				<INPUT maxLength=500 size=500 class=InputStyle name="word" id="word"  onchange="checkinput('word','wordspan')" temptitle="<%=SystemEnv.getHtmlLabelName(131596,user.getLanguage())%>"  value="<%=word%>">
			</wea:required>
		</wea:item>
	</wea:group>
</wea:layout>
