...
<div class="e8_box demo2">
	<div class="e8_boxhead">
	    ...
	    <div class="tab_box">
        <div>
        		<IFRAME name=frame1 id=frame1 src="<%=searchUrl %>" width="100%" height="180px" frameborder=no scrolling=no>
...
function changeTab(tabId){
	if(tabId==1){
	    ...
	    document.frame2.resetCondition(1,true);
	}else if(tabId==2){
	   document.getElementById("frame1").src = "SearchByGroup.jsp";
	   document.frame2.resetCondition(2,false);
	}else if(tabId==3){
		document.getElementById("frame1").src = "Search.jsp";
		document.frame2.resetCondition(3,false);
	}
	if(tabId==3){
		$("#frame2").contents().find("#btnsearch").show();
		if($("#frame2").contents().find("#btnsearch").next().attr("class")=="e8_sep_line"){
			$("#frame2").contents().find("#btnsearch").next().show();
		}
	}else{
		$("#frame2").contents().find("#btnsearch").hide();
		if($("#frame2").contents().find("#btnsearch").next().attr("class")=="e8_sep_line"){
			$("#frame2").contents().find("#btnsearch").next().hide();
		}
	}
}
