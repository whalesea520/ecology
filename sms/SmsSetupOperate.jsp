...
<%
	...
	if("save".equals(operate)){//添加	编辑
		...
		if(!"".equals(id)&&!"0".equals(id)){//更新
			if(rs.executeSql("update sms_reminder_set set prefix='"+prefix+"', suffix='"+suffix+"',prefixConnector='"+prefixConnector+"',suffixConnector='"+suffixConnector+"' where id="+id)){
				out.print(true);
			}else{
				out.print(false);
			}
			...
			for(int i=0;i<types.length;i++){
				rs.executeSql("insert into sms_reminder_set(prefix,suffix,prefixConnector,suffixConnector,type,def) values('"+prefix+"','"+suffix+"','"+prefixConnector+"','"+suffixConnector+"','"+types[i]+"','0')");
			}
			out.print(true);
		} 
		...
		if(IDS!=null&&!"".equals(IDS)){
			rs.executeSql("delete sms_reminder_set where def=0 and id in("+IDS+")");
			out.print(true);
		}else{
			out.print(false);
		}
