(function($) {
	$.fn.FuzzyQuery = function(options) {
		var defaults = {
			...
			currentid:"",
			param:"",
			result:null, //选择结果后回调函数
			intervalTop:0   //搜索结果框距离输入框上部间隔
		};
		...
		this.createLoadPanel = function() {
			...
			if((top+num*24)>$("#main").height()){
				...
				var top=offset.top+obj.height()+this.options.intervalTop;
				this.show_panel.css("left",left);
				this.show_panel.css("top",top);
			}else{
		...
		this.createQueryPanel = function() {
			...
			if (_valid_data == null) {
			    if(this.options.iscreate)
				  div.html("<div style='height:20px;line-height:22px;color:#C0C0C0;padding-left:10px;'>创建\""+$(this).val()+"\"</div>");
				else
			...
			for (var i = 0; i < _valid_data.length; i++) {
				...
				tr = $("<tr/>");
					...
					tr.append(td);
					if(searchtype=="task"||searchtype=="search"){
						//td = $("<td class='t_status'>" + obj["status"] + "</td>");
						td = $("<td class='t_status'>" + obj["typename"] + "</td>");
						tr.append(td);
					}
					tr.hover(function() {
					...
					function() {
						...
						if(updatename=="objname"){
							if(_row_data["type"]==2){
								searchList(_row_data["id"],_row_data["name"]);
							}else{
								loadDetail(_row_data["id"],_row_data["name"]);
							}
						}else if(updatename=="tag"){
							//me.val(_row_data["name"]);
							selectUpdate("tag",_row_data["name"],_row_data["name"],"str");
						}else{
							alert(updatename+"---");
							selectUpdate(updatename,_row_data["id"],_row_data["name"],updatetype);
						}
						*/
						me.options.result(_row_data,updatename,updatetype);  //选择后回调函数
						me.blur();
					});
				tab.append(tr)
			}
		...
		this.confirm_row = function() {
			...
			this.options.result(_row_data,updatename,updatetype);  //选择后回调函数
			me.blur();
		};
