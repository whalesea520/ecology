...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        var def = {
             ...
             */
            weekstartday: 7,  //start from Monday by default
            theme: 0, //theme no
            /**
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            ...
            var html = [];
            html.push("<div id=\"dvwkcontaienr\" style='height:100%' class=\"wktopcontainer\">");
            html.push("<table class=\"wk-top\" style='height:100%' border=\"0\" cellpadding=\"0\" cellspacing=\"0\">");
            BuildWT(html, days, allDayEvents, dM);
            html.push("</table>");
            html.push("</div>");

            //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
            html.push("<div id=\"dvtec\"  style='display:none' class=\"scolltimeevent\"><table style=\"table-layout: fixed;", jQuery.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
        ...
        function PropareEvents(dayarrs, events, aDE, sDE) {
            ...
            for (var i = 0; i < l; i++) { //to deal with more-than-one-day event //计算跨天
                var de = deB[i];
                if (de.length > 0) { //           
                    for (var j = 0; j < de.length; j++) {
                    	//lrdate 表示开始日期所在周最后一天
                        var end = DateDiff("d", lrdate, de[j].event[3]) > 0 ? lrdate : de[j].event[3];
                        //alert(DateDiff("d", dayarrs[i].date, end));
                        //计算跨天天数
                        de[j].colSpan = DateDiff("d", dayarrs[i].date, end) + 1
                        /*
                        if(DateDiff("d", lrdate, de[j].event[3]) > 0)  //如果本周结束日期大于日程结束日期
                           de[j].colSpan = DateDiff("d", dayarrs[i].date, end)+1;
                        else
                           de[j].colSpan = DateDiff("d", dayarrs[i].date, end);
                        */   
                        //de[j].colSpan=2;
                        //alert("de[j].colSpan:"+de[j].colSpan+" end:"+end);
                    }
                }
                de = null;
            }
            //for all-day events 计算全天
            for (var i = 0; i < l; i++) {
                var de = deA[i];
                if (de.length > 0) { 
                    var x = []; 
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            //2:          
            ht.push("<tr>");
            ht.push("<td valign='top' class=\"wk-allday\"");
            ...
            //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
            var h = allheight - 30;
            ht.push("><div style='height:"+ h +"px' id=\"weekViewAllDaywk\" ><table class=\"st-grid\" style='height:100%' cellpadding=\"0\" cellspacing=\"0\"><tbody>");
        ...
        function BuildMonthDayEvent(e, cday, length) {
            ...
            var sf = e.event[2] < cday;
           	// alert("e.event[2]"+e.event[2]+" e.event[3]:"+e.event[3]+" length:"+length+" cday:"+cday);
        ...
        function populate() {
            ...
            if (option.url && option.url != "") {
                ...
                $.ajax({
                    type: option.method, //
                    url: option.url+option.selectedUser+"&isShare="+option.isShare,
                    data: param,				   
        ...
        function parseED(data) {
            ...
            if (data.length > 6) {
                var e = [];
                var tasktype=data[data.length-1];//任务类型参数
                e.push(data[0], data[1], new Date(data[2]), new Date(data[3]), parseInt(data[4]), parseInt(data[5]), parseInt(data[6]), data[7] != undefined ? parseInt(data[7]) : -1, data[8] != undefined ? parseInt(data[8]) : 0, data[9], data[10],tasktype);
                    ...
                    if (bud.length == 0) {
                        ...
                        calbutton.click(function() {
                            ...
                            if (option.DeleteCmdhandler && $.isFunction(option.DeleteCmdhandler)) {
                            		var obj = jQuery(this);
                               		var taskid = data[0];
                               		var tasktype = data[data.length-1];
                            		$.post('/express/calendar/getData.jsp?method=deleteCalendarItem&taskid='+taskid+"&tasktype="+tasktype,function(data){
        ...
        function moreshow(mv) {
            ...
            if (top + height+42 >= maxtop) {
                top = maxtop - height - 52;
            }
        ...
        function dayupdate(data, start, end) {
            ...
            if (option.quickUpdateUrl != "" && data[8] == 1 && option.readonly != true) {
                ...
                if (option.quickUpdateHandler && $.isFunction(option.quickUpdateHandler)) {
                    ...
                    $.post(option.quickUpdateUrl+option.selectedUser+"&isShare="+option.isShare+"&taskid="+taskid+"&tasktype="+tasktype, param, function(data) {
                        if (data) {
                            if (data.IsSuccess) {
                                option.isloading = false;
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                temparr.push('<table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><th id="_title" class="cb-key">');
                temparr.push(i18n.xgcalendar.time, ':</th><td class=cb-value><div id="bbit-cal-buddle-timeshow"></div></td></tr><tr><th class="cb-key">');
                temparr.push("标 题", ':</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><input id="bbit-cal-what" class="textbox-fill-input"/></div></div><div class="cb-example">');
                temparr.push('</div></td></tr></tbody></table><input id="bbit-cal-start" type="hidden"/><input id="bbit-cal-end" type="hidden"/><input id="bbit-cal-allday" type="hidden"/><input id="bbit-cal-quickAddBTN" value="');
                temparr.push(i18n.xgcalendar.create_event, '" type="button"/>&nbsp; <SPAN id="bbit-cal-editLink" class="lk">');
                temparr.push("修改任务详细信息", ' <StrONG>&gt;&gt;</StrONG></SPAN></div></div></div><tr><td><div id="bl1" class="bubble-corner"><div class="bubble-sprite bubble-bl"></div></div><td><div class="bubble-bottom"></div><td><div id="br1" class="bubble-corner"><div class="bubble-sprite bubble-br"></div></div></tr></tbody></table><div id="bubbleClose1" class="bubble-closebutton"></div><div id="prong2" class="prong"><div class=bubble-sprite></div></div></div>');
                ...
                calbutton.click(function(e) {
                    ...
                    var zone = new Date().getTimezoneOffset() / 60 * -1;
                    var param = [{ "name": "planName", value: encodeURIComponent(what) },
						{ "name": "beginTime", value:  dateFormat.call(strtodate(datestart),"HH:mm")},
						{ "name":"beginDate",value:dateFormat.call(strtodate(datestart),"yyyy-MM-dd")},
						{ "name": "endDate", value:  dateFormat.call(strtodate(dateend),"yyyy-MM-dd")},
						{ "name": "endTime",value: dateFormat.call(strtodate(dateend),"HH:mm")},
                    ...
                    if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
                        ...
                        $.post("/express/calendar/getData.jsp?method=quickAddCalendarItem&selectUser="+option.selectedUser+"&isShare="+option.isShare, param, function(data) {
                            if (data) {
                                if (data.IsSuccess ) {
                                	newdata.push(-1, what);
                                    ...
                                    var diff = DateDiff("d", sd, ed);
                                    newdata.push(sd, ed, allday == "1" ? 1 : 0, diff > 0 ? 1 : 0, "1");
                                    newdata.push(data.Data[7], 0, "", ""); 
                                    tId = Ind(newdata);
                                    realsedragevent();
                                    render();
                                    option.isloading = false;
                                    option.eventItems[tId][0] = data.Data[0];
                                    option.eventItems[tId][8] = data.Data[8];  
                                    render();
                                    option.onAfterRequestData && option.onAfterRequestData(4);
                                }
                                else {
                ...
                lbtn.click(function(e) {
                    ...
                    //dateFormat.call(start, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm")
                    else {
                        if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                        	//$.post("/express/task/data/DetailView.jsp?name="+$("#bbit-cal-what").val(),function(data){}); 
                            option.EditCmdhandler.call(this, 
                            		['0', $("#bbit-cal-what").val(), 
                            		 dateFormat.call(strtodate($("#bbit-cal-start").val()),"yyyy-MM-dd"), 
                            		 dateFormat.call(strtodate($("#bbit-cal-start").val()),"HH:mm"), 
                            		 dateFormat.call(strtodate($("#bbit-cal-end").val()),"yyyy-MM-dd"), 
                            		 dateFormat.call(strtodate($("#bbit-cal-end").val()),"HH:mm"), 
                            		 $("#bbit-cal-allday").val()]);
                       		 }
