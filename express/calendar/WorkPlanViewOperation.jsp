...
<%
    ...
    if("addCalendarItem".equals(method)){
 	   ...
 	   //  String endTime = request.getParameter("endTime");  //结束时间p
 	     ...
 	     int level = 0;
    	// String name =Util.null2String(URLDecoder.decode(request.getParameter("planName"),"UTF-8"));  //标题p
    	 ...
    	 String  sql = "insert into TM_TaskInfo (name,status,creater,createdate,createtime,begindate,enddate,tasklevel,principalid) "
		 ...
		 if(res){
			...
			if(rs.next()){
			 ...
			 rs.executeSql("insert into Task_schedule (userid,taskid,tasktype,taskdate) values ("+userId+","+taskId+","+1+",'"+begindate+"')");
			 }
    	...
    	//JSONObject jsonObj=JSONObject.fromObject(taskId);
    	
    	result.put("IsSuccess","true");
    	
    	data.add(""+taskId);
    	data.add(name);
    	data.add(begindate +" "+"00:00");
    	data.add(enddate+" "+"00:00");
    	data.add("0");
    	data.add("1");//是不是全天
    	...
    	out.flush();
    	//out.print(taskId.toString());
		}
		 
		//{"Data":["2267","范德法特","2012-12-17 00:00","2012-12-20 00:00","0","1","0","1","1","1","1"],"IsSuccess":"true"} 
		//{"Data":["25","你好。。。","2012-12-17 00:00","2012-12-18 00:00","0","1","0","","1","1","1"],"IsSuccess":"true"}
    	//{"Data":["2271","发的发的身份","2012-12-18 00:00","2012-12-18 00:00","0","0","0","1","1","1","1"],"IsSuccess":"true"}
    	//{"Data":["27","佛挡杀佛","2012-12-18 00:00","2012-12-18 00:00","0","1","0","1","1","1","1"],"IsSuccess":"true"}
    }
    if("quickAddCalendarItem".equals(method)){
	     int level = 0;
   	 String name =Util.null2String(URLDecoder.decode(request.getParameter("planName"),"UTF-8"));  //标题p
   	 String principalid = Util.fromScreen3(request.getParameter("principalid"), user.getLanguage());//
   	 principalid = user.getUID()+"";
   	 ...
   	 String begindate = Util.null2String(request.getParameter("beginDate"));
	 String enddate = Util.null2String(request.getParameter("endDate"));  //结束日期p
   	 String  sql = "insert into TM_TaskInfo (name,status,creater,createdate,createtime,begindate,enddate,level,principalid) "
			+"values('"+name+"',1,"+user.getUID()+",'"+currentDate+"','"+currentTime+"','"+begindate+"','"+enddate+"',"+level+","+principalid+")";
   	 boolean res = rs.executeSql(sql);
		 String taskId = "";
		 if(res){
   	...
   	//["92","王企鹅全文","12/29/2011 01:30","12/29/2011 05:00","0","0","0","1","1","Belion",""]
   	//JSONObject jsonObj=JSONObject.fromObject(taskId);
   	//System.out.println("++++++++++++++++:json:"+jsonObj.toString()+"  =="+ taskId);
   	result.put("IsSuccess","true");
   	data.add(""+taskId);
   	data.add(name);
   	data.add(begindate +" "+"00:00");
    ...
    if("delDefault".equals(method)){
    	String taskid = Util.null2String(request.getParameter("taskid"));
    	rs.executeSql("delete Task_schedule where (taskid="+taskid+" and tasktype = 1 and userid = "+ userId+")");
    	rs.executeSql("delete TM_TaskInfo where id = "+taskid);
    	
    }
    
    if("addCalendarItem2".equals(method)){
    	 String begindate = Util.null2String(request.getParameter("beginDate"));
    	 String enddate = Util.null2String(request.getParameter("endDate"));  //结束日期p
    	 int level = 0;
    	 String name =Util.null2String(URLDecoder.decode(request.getParameter("planName"),"UTF-8"));  //标题p
    ...
    //添加
	if("addCalendarItem1".equals(method))
	{	
	...
	else if("editCalendarItemQuick".equals(method))
	{
		...
		rs.executeSql("update Task_schedule set taskdate= '"+startDate+"' where (taskid="+taskid+" and tasktype="+tasktype+" and userid="+userId+")");
		if(tasktype.equals("1")){
			rs.executeSql("update TM_TaskInfo set begindate ='"+startDate +"' ,enddate='"+endDate+"' where id="+taskid);
		}
		if(tasktype.equals("2")){
			rs.executeSql("update  set begindate ='"+startDate +"' ,enddate='"+endDate+"' where id="+taskid);
		}
		if(tasktype.equals("3")){
			rs.executeSql("update  set begindate ='"+startDate +"' ,enddate='"+endDate+"' where id="+taskid);
		}
		if(tasktype.equals("4")){
			rs.executeSql("update  set begindate ='"+startDate +"' ,enddate='"+endDate+"', begintime='"+startTime+"' where id="+taskid);
		}
		if(tasktype.equals("5")){
			rs.executeSql("update cowork_items set begindate ='"+startDate +"' ,enddate='"+endDate+"', begintime='"+startTime+"',endtime='"+endTime+"' where id="+taskid);
		}
	...
	//添加
	else if("editCalendarItemQuick1".equals(method))
	{
		String id =(String) request.getAttribute("id");
		String startDate = (String)request.getAttribute("startDate");
		String startTime = (String)request.getAttribute("startTime");
		...
		recordSet.executeSql(stringBuffer.toString());
		if(recordSet.next()){//判断是否对提交的数据有编辑权限
	...
	else if("overCalendarItem".equals(method))
	{
	    ...
	    if(createrID.equals(userId)){ //提交人完成
		    ...
		    wfTitle += "-" + currentDate;
		    wfRemark = SystemEnv.getHtmlLabelName(83995,user.getLanguage())+ ":<A href=/express/calendar/WorkPlan.jsp?workid=" + planID + ">" + Util.fromScreen2(planName, user.getLanguage()) + "</A>";
	...
	{  
		...
		if(recordSet.next())
		{
	    	...
	    	result.put("description",Util.null2String(recordSet.getString("description")));	
	...
	else if("deleteCalendarItem".equals(method))
	{
		...
		rs.execute("delete from Task_schedule where taskid = "+id +" and tasktype = "+ tasktype);
		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append("SELECT 1 FROM workPlanShareDetail WHERE workId = ");
		stringBuffer.append(id);
		stringBuffer.append(" AND userId = ");
