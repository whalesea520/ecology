...
if("1".equals(type)){ 
	tableString=""+
		       ...
		       "<head>";
		      	if("0".equals(ishtml)) {
		      		tableString = tableString + "<col width=\"10%\"  text=\"流程名称\" column=\"workflowname\" orderkey=\"workflowname\" />"+
		 	 	 	"<col width=\"10%\"  text=\"节点名称\" column=\"nodename\" orderkey=\"nodename\" />";
		      	}
		      	if("5".equals(ishtml)||"4".equals(ishtml)) {
		      		tableString = tableString + "<col width=\"10%\"  text=\""+"模块名称"+"\" column=\"workflowname\" orderkey=\"workflowname\" />"+
		 	 	 	"<col width=\"10%\"  text=\""+"布局名称"+"\" column=\"nodename\" orderkey=\"nodename\" />";
		      	}

		      	if(!"4".equals(ishtml)) {//移动建模没有文件路径
		      		tableString = tableString + "<col width=\"25%\"  text=\"地址\" column=\"file\" orderkey=\"file\" />";
		      	}
	             tableString = tableString + "<col width=\"25%\"  text=\""+"详情"+"\" column=\"detail\"/>"+ 
...
function back() {
	window.location.href="matchruleHtml.jsp?ishtml=<%=ishtml%>&tabtype=<%=tabtype%>&navName=<%=tnavName%>";
}
function replace(id,paras) {
	var contents = paras.split("\+");
	var param = contents[0];
	if(param=="") {
		top.Dialog.alert("路径为空，无法匹配");
		return;
	}
	var htmltype = "<%=ishtml%>"=="4"?1:0;
...
function doRefresh(){
	$("#form1").attr("action","matchruleHtmlResult.jsp");
	$("#form1").submit();
}

function doOpen3(url,title){
	if(typeof dialog  == 'undefined' || dialog==null){
		dialog = new window.top.Dialog();
	}
	dialog.currentWindow = window;
...
function doOpen2(url,title){
	...
	dialog.Drag = true;
	dialog.maxiumnable = true;
	if(url.indexOf("matchruleresultdetailiframe")!=-1){//跳转检查结果页后，关闭结果页，强制对该页面reload
...
function showdetail(param,ruleid,type) {
	if("1" == type) {
		doOpen2("/templetecheck/matchruleresultdetailiframe.jsp?pageid="+param+"&tabtype=<%=tabtype%>&ishtml=<%=ishtml%>&ruleid="+ruleid+"&type="+type,"");
	} else {
		doOpen2("/templetecheck/matchruleresultdetailiframe.jsp?filepath="+param+"&tabtype=<%=tabtype%>&ishtml=<%=ishtml%>&ruleid="+ruleid+"&type="+type,"");
	}
