...
var	rte_toolbar = {
	s1				: {separator: true},
	bold			: {command: 'bold', tags:['b', 'strong'],title:rteLang.bold},
	italic			: {command: 'italic', tags:['i', 'em'],title:rteLang.italic},
	...
	s5				: {separator : true },*/
	orderedList		: {command: 'insertorderedlist', tags: ['ol'],title:rteLang.orderedList },
	unorderedList	: {command: 'insertunorderedlist', tags: ['ul'],title:rteLang.unorderedList },
	s6				: {separator : true },
...
<select>\
	...
	', tag_cmp: lwrte_block_compare, tags: [ 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']},
...
<select>\
	...
	color			: {exec: lwrte_color,title:rteLang.color},
	image			: {exec: lwrte_image, tags: ['img'],title:rteLang.image },
	link			: {exec: lwrte_link, tags: ['a'],title:rteLang.link },
	unlink			: {command: 'unlink',title:rteLang.unlink},
	s8				: {separator : true },
	removeFormat	: {exec: lwrte_unformat,title:rteLang.removeFormat},
	word			: {exec: lwrte_cleanup_word,title:rteLang.word},
	clear			: {exec: lwrte_clear,title:rteLang.clear}
};
...
function lwrte_color(){
	var self = this;
	var panel = self.create_panel(rteLang.color_title, 385);
...
<div class="clear"></div>\
<p class="submit"><button id="ok" style="cursor:pointer">'+rteLang.ok+'</button><button id="cancel" style="cursor:pointer">'+rteLang.cancel+'</button></p>'
).show();
...
function lwrte_image() {
	var self = this;
	var panel = self.create_panel(rteLang.image_title, 385);
	panel.append('\
<p><label>'+rteLang.image_URL+'</label><input type="text" id="url" size="30" value=""><button id="file" style="cursor:pointer">'+rteLang.image_upload+'</button><button id="view">'+rteLang.view+'</button></p>\
<div class="clear"></div>\
<p class="submit"><button id="ok" style="cursor:pointer">'+rteLang.ok+'</button><button id="cancel" style="cursor:pointer">'+rteLang.cancel+'</button></p>'
).show();
	...
	$('#view', panel).click( function() {
			(url.val().length >0 ) ? window.open(url.val()) : alert(rteLang.image_noURLTip);
			return false;
		}
	);
...
function lwrte_clear() {
	if(confirm(rteLang.clearDocument)) 
		this.set_content('');
}
...
function lwrte_link() {
	...
	var self = this;
	var panel = self.create_panel(rteLang.link_title, 385);
	panel.append('\
<p><label>'+rteLang.link_URL+'</label><input type="text" id="url" size="30" value=""><button id="view" style="cursor:pointer">'+rteLang.view+'</button></p>\
<div class="clear"></div>\
<p><label>'+rteLang.tip+'</label><input type="text" id="title" size="30" value=""><label style="display:none">Target</label><select id="target" style="display:none"><option value="">default</option><option value="_blank" selected=true>new</option></select></p>\
<div class="clear"></div>\
<p class="submit"><button id="ok" style="cursor:pointer">'+rteLang.ok+'</button><button id="cancel" style="cursor:pointer">'+rteLang.cancel+'</button></p>'
).show();
	...
	$('#view', panel).click( function() {
		(url.val().length >0 ) ? window.open(url.val()) : alert(rteLang.link_noURLTip);
		return false;
	}
	);
