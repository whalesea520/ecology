...
function FieldAttributeListener(){
	...
	return{
		AddItems : function( menu, tag, tagName ){
			...
			fieldid = fieldid.substr(0, fieldid.indexOf("$"));
			var mgrFieldid = window.parent.document.getElementById("mgrFieldid").value;
			var isMgr = (mgrFieldid == fieldid);
			try{
				if (!isMgr && tagName == 'INPUT' && ( tag.type == 'text' || tag.type == 'password' ) && tag.id.indexOf("$field")==0 && tag.id!="$field-4$"){
					menu.AddSeparator() ;
			...
			try{
				...
				if(!isMgr && canFieldEdit=="1" && index_eFieldids==-1){
