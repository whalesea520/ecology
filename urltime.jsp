...
<%
	...
	Map<String, Object> resp = MonitorXFilter.getFilterData("getALLUrlTimeValues");
    ...
    if ("timePer".equals(action)) {
		...
		application.setAttribute("__urltime_lastcleartime", new java.text.SimpleDateFormat("yyyy'-'MM'-'dd' 'HH:mm:ss").format(new java.util.Date())); 
        MonitorXFilter.setFilterData("clearAllUrlTime", null);
		%>
		<script>location.href = "urltime.jsp";</script>
		<%
...
%>
<a href="cpu.jsp">browse cpu.jsp</a>
&nbsp;
<a href="urltime.jsp?action=clear" title="clear all expire data(not include running threads)">Clear</a>
&nbsp;
<a href="urltime.jsp?action=timeSum">TimeRatio Sort</a>
&nbsp;
<a href="urltime.jsp?action=count">Count Sort</a>
&nbsp;
<a href="urltime.jsp?action=timePer">TimePer Sort</a>
&nbsp;
<span title="every url count average">AVG COUNT = <%=avgCount%></span>
&nbsp;
<span title="every request cpu average">AVG CPU = <%=avgTime%>ms</span>
&nbsp;
<span>LAST CLEAR = <%=application.getAttribute("__urltime_lastcleartime")%></span>
<table border=1>
    <tr>
        <th>url</th>
        <th>time sum(ms)</th>
