...
<%@ page import="weaver.general.Util,java.sql.*" %>
...
detailtablelist = outerdetailtablesArr ;
...
%>
<form action="sdataOperation.jsp" id="weaver" name="weaver">
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("21778,15190",user.getLanguage())+"  ("+maintablename+")" %>'>
	...
	<%
		...
		for(int i = 0 ; i < detailtablelist.size() ; i++){
			...
			//if(null == detailtableidlist||detailtableidlist.size()<1){
			//	isout = true ;
			//}
	%>
		
	<wea:group context='<%=SystemEnv.getHtmlLabelName(19325,user.getLanguage())+(i+1)+SystemEnv.getHtmlLabelName(15190,user.getLanguage())+"  ("+detailtablename+")" %>'>
		<wea:item type="groupHead">
		...
		<wea:item attributes="{'isTableList':'true','colspan':'full'}">
			...
			<%  
				...
				String ajaxDatas = "";
				//if(!isout){
				//	tableid = detailtableidlist.get(i).toString() ;
				//	RecordSet.executeSql("select * from wfec_tablefield where tableid="+tableid+" order by id");//2外部接口
				//	count = RecordSet.getCounts();	
				...
				<input type="hidden" id="dt<%=i %>_tableid" name="dt<%=i %>_tableid" value="<%=tableid %>" />
...
function checkfield(obj,tablename){
	...
	jQuery.ajax({
		...
		success:function(data){
			if(data==1){
				top.Dialog.alert("["+fieldname+"] 字段设置数据库中已存在，请重新输入！");
				try{
					jQuery(obj).val("");
				}catch(e){
				}
