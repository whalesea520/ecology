...
<wea:layout>
        ...
        <wea:group context='<%=SystemEnv.getHtmlLabelName(125442,user.getLanguage()) %>' attributes="{'samePair':'SetInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
            ...
            <wea:item attributes="{'colspan':'full','isTableList':'true'}">
                ...
                int count = 0 ;
                String ajaxDatas = "[]";
                String sql = "SELECT id,wffieldid,wffieldname,wffieldhtmltype,wffieldtype,wffielddbtype,outerfieldname,outfielddbtype,changetype,customtxt,customsql FROM wfec_indatasetdetail where detailindex=0 and mainid="+viewid+" ORDER BY dsporder";
                ...
                <script type="text/javascript">
                    ...
                    jQuery(document).ready(function(){
                        ...
                        var db_fieldbrowser = "<span class='browser'  _callback='settablefieldtype'  _callbackParams='_#rowIndex#' name='outerfieldname'  getBrowserUrlFn='getTableField' getBrowserUrlFnParams='<%=outermaintable%>#0'  isMustInput='2'></span>";
                        var wf_fieldbrowser  = "<span class='browser'  _callback='setwffieldtype' _callbackParams='_#rowIndex#' hasInput='true' name='fieldid' getBrowserUrlFn='getWorkflowTableField' isSingle='true' getBrowserUrlFnParams='0' completeUrl='' ></span><input type='hidden' name='fieldname' id='fieldname'  />" ;
                        var items=[
                    ...
                    function marcallback($this,tr,entry){
                        try{
                            var ct = $this.count ;
                            ...
                            jQuery("#needcheck").val(checkfield+",outerfieldname_"+(ct-1));
                        }catch(e){
        ...
        %>      
            ...
            <wea:item attributes="{'colspan':'full','isTableList':'true'}">
                ...
                int count = 0 ;
                String ajaxDatas = "[]";
                String sql = "SELECT id,wffieldid,wffieldname,wffieldhtmltype,wffieldtype,wffielddbtype,outerfieldname,outfielddbtype,changetype,customtxt,customsql FROM wfec_indatasetdetail where detailindex="+(i+1)+" and mainid="+viewid+" ORDER BY dsporder";
                RecordSet.executeSql(sql);
                count = RecordSet.getCounts() ;
                ...
                <script type="text/javascript">
                    ...
                    jQuery(document).ready(function(){
                        var ajaxDatas = <%=ajaxDatas%> ;
                        //completeUrl='/data.jsp?type=fieldBrowser&wfid=<%=workFlowId%>' isAutoComplete='true'
                        var db_fieldbrowser = "<span class='browser'  _callback='settablefieldtype'  _callbackParams='<%=i %>_#rowIndex#' name='outerfieldname<%=i %>'  getBrowserUrlFn='getTableField' getBrowserUrlFnParams='<%=outdetailname%>#1' isMustInput='2'></span>";
                        var wf_fieldbrowser  = "<span class='browser'  _callback='setwffieldtype' _callbackParams='<%=i %>_#rowIndex#' hasInput='true' name='fieldid<%=i %>' getBrowserUrlFn='getWorkflowTableField' isSingle='true' getBrowserUrlFnParams='<%=(i+1)%>' completeUrl='' ></span>" ;
                        var items=[
                            {width:"20%",display:"",colname:"<%=SystemEnv.getHtmlLabelName(125365,user.getLanguage()) %>",itemhtml:db_fieldbrowser},
                            {width:"10%",colname:"<%=SystemEnv.getHtmlLabelName(84113,user.getLanguage()) %>",itemhtml:"<%=outfieldtypestr%>"},
                            {width:"20%",colname:"<%=SystemEnv.getHtmlLabelName(19372,user.getLanguage()) %>",itemhtml:wf_fieldbrowser},
                    ...
                    function marcallback<%=i %>($this,tr){
                        try{
                            ...
                            var checkfield = jQuery("#needcheck").val();
                            jQuery("#needcheck").val(checkfield+",outerfieldname<%=i %>_"+(idx-1));
                        }catch(e){
...
function getWorkflowTableField(params){
    ...
    var isbill = '<%=isbill%>';
    var isdetail =  params ;
    var urls = "/systeminfo/BrowserMain.jsp?mouldID=workflow&formid="+formid+"&isbill="+isbill+"&url=/workflow/exchange/FormFieldsBrowser.jsp?mouldID=workflow&formid="+formid+"&isbill="+isbill+"&isdetail="+isdetail;
    return urls;
}
...
function settablefieldtype(event,datas,name,_callbackParams){
    ...
    //checkfield(datas.id,datas.tablename,_callbackParams,datas.name);
}
...
function setwffieldtype(event,datas,name,_callbackParams){
    ...
    jQuery.ajax({
        ...
        data:{ruletype:'',htmltype:datas.htmltype,isrdata:0},
...
function checkfield(fieldid,tablename,_callbackParams,showname){
    jQuery.ajax({
        ...
        data:{changetype:'1',type:'checkset',mainid:'<%=viewid %>',tablename:tablename,fieldid:fieldid ,flag:Math.random()},
        success:function(data){
            if(data==1){
                top.Dialog.alert("["+showname+"] <%=SystemEnv.getHtmlLabelName(129041, user.getLanguage())%>");
                try{
                jQuery("#outerfieldname"+_callbackParams).val("");
                jQuery("#outerfieldname"+_callbackParams+"span").html("");
                jQuery("#outerfieldname"+_callbackParams+"spanimg").html("<img align='absmiddle' src='/images/BacoError_wev8.gif'>");
                }catch(e){}
...
function getAllValues(groupid){
  ...
  if(groupid==-1){
    ...
    for(var i = 0; i < len; i++){
        var value = jQuery("#outerfieldname_"+i).val();
        if(value!=''){
            arrValues += ":"+value+":";
        }
    ...
    for(var i = 0; i < len; i++){
        var value = jQuery("#outerfieldname"+groupid+"_"+i).val();
        if(value!=''){
            arrValues += ":"+value+":";
        }
    }   
...
function loopInputs(value,groupid){
  ...
  if(/(:[^:]+:).*?\1/g.test(value)){
    top.Dialog.alert(showname+" <%=SystemEnv.getHtmlLabelName(84684,user.getLanguage()) %>");
    return false ;
  }else{
    return true ;
  }
