...
String rightStr = "WFEC:SETTING";//Util.null2String(request.getParameter("rightStr"));
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="leftTree.jsp" method="post" target="wfleftFrame">
...
<table cellspacing="0" cellpadding="0" class="flowsTable" style="width:100%;height:100%;"  >
	<tr>
		<td class="leftTypeSearch">
			<div>
				<span class="leftType"><%=SystemEnv.getHtmlLabelName(141,user.getLanguage())%><span id="totalDoc"></span></span>
  ...
  <input type="hidden" name="detachable" id="detachable" value="<%=detachable%>">
    <!--########//Search Table End########-->
	</FORM>
<FORM NAME="weaver" id="weaver" STYLE="margin-bottom:0" action="managewfTab.jsp" method="post" target="wfmainFrame">
	<input class=inputstyle type="hidden" name="subcomid" id="subcomid" >
</FORM>

<script language="javascript">
function setHeight(height){
$("#overFlowDivTree").height(height);
$("#overFlowDiv").height(height-$(".leftTypeSearch").height());
}
function initTree(){
CXLoadTreeItem("", "/frameleftXML.jsp?isWorkflowDoc=&rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
...
function setSubcompany(nodeid){ 
    ...
    //if(window.console) console.log("nodeid = "+nodeid +"  subid = "+subid);
...
function reflushlist(objvalue){
	try{
		jQuery("#subcomid").val(objvalue);
