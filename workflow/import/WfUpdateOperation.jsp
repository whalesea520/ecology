...
<jsp:useBean id="WorkflowDataService" class="weaver.workflow.updates.services.WorkflowDataService" scope="page" />
...
int selectworkflowid = Util.getIntValue(fu.getParameter("workflowid"),-1);
int importtype = Util.getIntValue(fu.getParameter("importtype"),-1);
...
if(isaesencrypt.equals("1")){
	checkresult = WorkflowDataService.checkWorkflowXml(xmlfilepath,selectworkflowid,isaesencrypt,aescode);
}else{
	checkresult = WorkflowDataService.checkWorkflowXml(xmlfilepath,selectworkflowid);
}
if(!checkresult.equals("")){//检查的结果不合法
	response.sendRedirect("/workflow/import/WfImport.jsp?checkresult="+checkresult+"&workflowid="+selectworkflowid+"&importtype="+importtype+"&type="+type);
	return;
}

//检查合法，将selectworkflowid更新成实际需要更新的workflowid，不一定是活跃版本的workflowid
selectworkflowid = WorkflowDataService.getXmlWorkflowid(xmlfilepath);

if(isaesencrypt.equals("1")){
	WorkflowDataService.updateWorkflowByXml(xmlfilepath,selectworkflowid,isaesencrypt,aescode);
}else{
	WorkflowDataService.updateWorkflowByXml(xmlfilepath,selectworkflowid);
}
		...
		<td class="rightSearchSpan" style="text-align:right; width:500px!important">
		    ...
		    <script>
		    	function viewWfBaseData(){
		    		window.open("/system/basedata/basedata_workflow.jsp?wfid=<%=selectworkflowid %>","_blank");
		    	}
		    </script>
		</td>
	</tr>
