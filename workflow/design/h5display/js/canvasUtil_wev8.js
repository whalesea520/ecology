...
function roundRect(ctx, x, y, w, h, r, outborder, outborderfill, outborderfill2, innerborder, innerfillImg, lineImg, label1, label2, color) {
	...
	lineImage.onload = function() {
		...
		image.onload = function() {
			...
			ctx.fillStyle = color;
			drawText(ctx, label1, 'normal 12px Microsoft YaHei', x + 6 + (110 - 70) / 2 - 10, y - 4 + 70 / 2 - 2, 70);
...
function diamond(ctx, x, y, w, h, r, outborder, outborderfill, outborderfill2, innerborder, innerfillImg, lineImg, label1, label2, color) {
	...
	lineImage.onload = function() {
		...
		image.onload = function() {
			...
			//console.log(ctx.isPointInPath(270, 90));
			ctx.stroke()
			ctx.fill();
			...
			ctx.fillStyle = color;
			drawText(ctx, label1, 'normal 12px Microsoft YaHei', x + (110 - 70) / 2, y + 100 / 2 - 2, 70);
			drawText(ctx, label2, 'normal 13px Microsoft YaHei', x + (110 - 70) / 2, y + 100 / 2 + 16, 70);
			//console.log(ctx.isPointInPath(270, 120));
		}
	}
}
...
function drawText(cxt, text, style, x, y, maxWidth, color) {
	var temp_text = text;
	cxt.font = style;
...
function dashRect(x, y, width, height, rectname, context) {
	...
	//console.log(x, y + 15, width, rectname);
	drawText(context, rectname, 'normal 13px Microsoft YaHei', x, y + 15, width, '#000');
}
...
function drawBlock(dir, x, y, width, height, blockName, cxt) {
	...
	if (dir == 0) {
		...
		drawText(cxt, blockName, 'normal 13px Microsoft YaHei', x, y + 20, width, '#000');
	}
}