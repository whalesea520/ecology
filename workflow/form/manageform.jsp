...
if(operatelevel>1){
	if(!ajax.equals("1")){
		RCMenu += "{"+SystemEnv.getHtmlLabelName(32136,user.getLanguage())+",javascript:submitData(),_self}" ;
		RCMenuHeight += RCMenuHeightStep ;
	}else{
		RCMenu += "{"+SystemEnv.getHtmlLabelName(32136,user.getLanguage())+",javascript:formnodedel(),_self}" ;
		RCMenuHeight += RCMenuHeightStep ;
	}
}
%>
...
<%}%>
<form id="fromaddtab" name="fromaddtab" method="post" >
<input type=hidden name=formid value="<%= formid %>">
<input type=hidden name=formname value="<%= formname %>">
<input type=hidden name=formdes value="<%= formdes %>">
<input type=hidden name="ajax" value="<%=ajax%>">
<input type=hidden name="operatelevel" value="<%=operatelevel%>">
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.WF_FORM_MANAGEFORM %>"/>
		<TABLE class=Shadow>
		<tr>
		<td valign="top">
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right; width:500px!important">
			...
			<%} %>		
			<%if(operatelevel>1){ %>
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(32136 , user.getLanguage()) %>" class="e8_btn_top" onclick="<%if(!ajax.equals("1")){%>submitData()<%}else{%>formnodedel()<%}%>"/>
			<%}%>
		...
		<div id="tabDiv" >
			...
			<span  class="selectedTitle"><%=SystemEnv.getHtmlLabelName(699,user.getLanguage())%></span>
			...
			<span style="margin-left:-8px;"  onmouseover="spanOver(this)"  onmouseout="spanOut(this)" onclick="location='manageform_sys.jsp'"><%=SystemEnv.getHtmlLabelName(125027, user.getLanguage())%></span>
			</span>
		</div>
...
String sqlWhere = " where not exists (select 1 from ModeFormExtend b where a.id = b.formid and isvirtualform=1) ";
if(detachable==1){  
	if (subCompanyId > 0) {
...
if(!"".equals(formtypeForSearch)){
	if("0".equals(formtypeForSearch)){
		sqlWhere+=" and (isoldornew = 0 or id <0 ) ";
	}else if("1".equals(formtypeForSearch) && !"1".equals(ajax)){
...
String fromSql  = " view_workflowForm_selectAll a "; //这个是视图不是表
...
tableString =   " <table instanceid=\"workflowFormListTable\" tabletype=\"checkbox\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.WF_FORM_MANAGEFORM,user.getUID())+"\" >"+
...
tableString+="       	</head>"+ 
                ...
                "		<operate href=\"javascript:onEdit();\" otherpara=\"column:isoldornew\" text=\""+SystemEnv.getHtmlLabelName(26473,user.getLanguage())+"\" target=\"_self\" index=\"0\"/>"+
...
function searchData(){
	var formname = jQuery("input[name='formnameForSearch']").val();
	var formtype = jQuery("#formtypeForSearch").val();
	window.location="/workflow/form/manageform.jsp";
}

function doReset(){
	jQuery("input[name=flowTitle]",parent.document).val("");
	//jQuery("input[name='flowTitle']",parent.document).val();
	jQuery("input[name=formnameForSearch]").val("");
	jQuery("#formtypeForSearch").selectbox("detach");
	jQuery("#formtypeForSearch").val("");
	jQuery("#formtypeForSearch").selectbox("attach");
}
...
function newDialog(){
	if("<%=ajax%>"!=1){
		  ...
		  //diag_vote.URL = "/workflow/form/addform.jsp?dialog=1&from=addDefineForm&ajax=1";
		  diag_vote.URL = "/workflow/form/addDefineForm.jsp?dialog=1&isnew=1";
		  diag_vote.isIframe=false;
		  diag_vote.show();
...
function formaddtab(){
	window.location = "/workflow/form/addform.jsp?ajax=1";
}

function onEdit(id,obj){
	if(obj=="0"){
		if("<%=ajax%>"=="1"){
			gotab00_new("/workflow/form/editform.jsp?ajax=1&formid="+id,id);        		
		}else{
			if(id>0){
				getnewDialogLink("/workflow/form/addDefineForm.jsp?isoldform=1&formid="+id);
			}else{
				getnewDialogLink("/workflow/form/addDefineForm.jsp?formid="+id);
			}
		...
		if(id<0){
			...
			getnewDialogLink("/workflow/form/addDefineSysForm.jsp?formid="+id);
		}
	}
	//var tempStr[] = obj2.split("+");
	//alert(tempStr[0]);
	//alert(tempStr[1]);
	//alert(tempStr[2]);
...
function onBtnSearchClick(){
	...
	window.location="/workflow/form/manageform.jsp?formnameForSearch="+typename;
}
