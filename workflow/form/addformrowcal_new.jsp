...
<%
    ...
    if(RecordSet.next()){
        ...
        rowcalstr = rowcalstr.replaceAll("\\+","_plus_");//+号标识暂时用字母代替，防止+被当做特殊字符处理
    }
    String sqltable = " select * from workflow_billdetailtable where billid="+formid+" order by orderid";
    if(formid!=0) RecordSet.executeSql(sqltable);
    List<String> listtable = new ArrayList<String>();
    while(RecordSet.next()){
        listtable.add(RecordSet.getString("tablename"));
    }
    sql = "select * from workflow_billfield where "+
    	  "viewtype=1 and fieldhtmltype=1 and (type=2 "+
    	  "or type=3 or type=4 or type=5) and billid="+formid+"  order by detailtable";
    if(formid!=0) RecordSet.executeSql(sql);
    while(RecordSet.next()){
    	if(null==map.get(RecordSet.getString("detailtable"))){
    		map.put(RecordSet.getString("detailtable"),"opList.options[i++] = new Option('','');\n");
    	}
        ...
        String option = "opList.options[i++] = new Option('"+ SystemEnv.getHtmlLabelName(RecordSet.getInt("fieldlabel"),user.getLanguage()) +"','"+RecordSet.getString("id")+"');";
        map.put(RecordSet.getString("detailtable"),map.get(RecordSet.getString("detailtable"))+option+"\n");
    }
                     ...
                     <tr>
                       ...
                       <td>
	                       <a href="#" accessKey="+" onclick="addexp(this)">
			      				<img alt="+" src="/images/wfrowedit/jia_normal_wev8.png" onmouseover="jsChangeBackImg(this,'jia_hot_wev8.png')" onmouseout="jsChangeBackImg(this,'jia_normal_wev8.png')"/>
			      		   </a>    
                       </td>
                       <td>
	                       <a href="#" accessKey="-" onclick="addexp(this)">
...
function onShowChildField_new(){
	var url = "/systeminfo/BrowserMain.jsp?url=" + escape("/workflow/form/searchDetailedField.jsp?billid=<%=formid%>&isbill=1&isdetail=1");
	return url;
}*/
</script>
<%
