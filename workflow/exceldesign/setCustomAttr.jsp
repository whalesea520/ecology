...
<%
	...
	if("row".equals(locatetarget)){
		hideitemname = SystemEnv.getHtmlLabelName(126239, user.getLanguage());
	}else if("col".equals(locatetarget)){
		hideitemname = SystemEnv.getHtmlLabelNames("16636,83959",user.getLanguage());
	}else{
		hideitemname = SystemEnv.getHtmlLabelName(126238, user.getLanguage());
	}
%>
<html>
<head>
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(129024, user.getLanguage())%>"/>
</jsp:include> 
...
<wea:layout type="2col" attributes="{'cw1':'35%','cw2':'65%'}">
	...
	<wea:group context="<%=SystemEnv.getHtmlLabelName(34114, user.getLanguage())%>">
		<wea:item><%=hideitemname %></wea:item>
		<wea:item>
			<input type="checkbox" id="attr_hide" />
			<%if(setDetailColAttr){ %>
			<span class="e8tips" title="<%=SystemEnv.getHtmlLabelName(128070, user.getLanguage())%>">
	...
	<%} %>
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("31649,713",user.getLanguage()) %>'>
		<wea:item attributes="{'colspan':'2'}">
			<div class="blueborder" style="background:rgb(248,248,248);height:30px;line-height:30px;">
				<span style="margin-right:80px;"><%=SystemEnv.getHtmlLabelNames("713,84276",user.getLanguage()) %></span>
				<span><%=SystemEnv.getHtmlLabelName(129025, user.getLanguage())%></span>
			</div>
		</wea:item>
		<%if(!setDetailColAttr){ %>
	...
	<wea:group context="<%=SystemEnv.getHtmlLabelName(129024, user.getLanguage())%>">
			...
			<div id="cusdiv">
				<table id="custable" class="custable" style="width:100%">
					...
					<tr style="background:rgb(248,248,248)">
						...
						<td class="blueborder"><%=SystemEnv.getHtmlLabelNames("713,84276",user.getLanguage()) %></td>
						<td class="blueborder"><%=SystemEnv.getHtmlLabelName(129025, user.getLanguage())%></td>
					</tr>
				</table>
			</div>
		</wea:item>
	...
	jQuery("table#custable").find("tr.cusrecord").each(function(){
		...
		if(_cusname == "" || _cusval == ""){
			...
			if(_cusname.toLowerCase() === "id" || _cusname.toLowerCase() === "name"
				...
				window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(128071, user.getLanguage())%>");
				verifyResult = false;
				return false;
			}
		}
