...
function bindClickEvent(stylediv){
	...
	stylediv.find("div.style_outside").contextMenu({
		...
		onPopup : function(el,e) {
			...
			var menuJson = "{";
			menuJson += "\"children\": [{";
			menuJson += "\"title\" : \"<%=SystemEnv.getHtmlLabelName(93, user.getLanguage())%>\",";
		...
		if(action === "editstyle"){	//编辑
			...
			window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(128088, user.getLanguage())%>",function(){
				$.ajax({
					type : "post",
					url : "/workflow/exceldesign/tabStyleOperation.jsp",
...
function openStyleDesign(title, opertype, styleid){
	...
	var url = "/workflow/exceldesign/tabStyleDesign.jsp?opertype="+opertype;
	if(opertype === "edit")
		url += "&styleid="+styleid;
	dlg.Title = title;
	dlg.Width = 900;
