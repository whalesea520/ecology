...
function showMultiMouldDialog(selectids){
    ...
    config.srchead=["<%=SystemEnv.getHtmlLabelName(685,user.getLanguage()) %>"];
    config.container =jQuery("#colShow");
    config.searchLabel="";
    config.hiddenfield="id";
    config.saveLazy = true;//取消实时保存
    //config.saveurl= "/workflow/exceldesign/chooseTextFieldAjax.jsp?src=save";
    config.srcurl = "/workflow/exceldesign/chooseTextFieldAjax.jsp?src=src";
    config.desturl = "/workflow/exceldesign/chooseTextFieldAjax.jsp?src=dest";
    //config.delteurl="/workflow/exceldesign/chooseTextFieldAjax.jsp?src=save";
    config.pagesize = 10;
