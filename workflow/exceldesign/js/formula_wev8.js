...
function triFormula_delRow(groupid){
	try{
		...
		if(jQuery("#nodesnum"+groupid).val() == "0"){		//明细记录全删除情况
			...
			try{
				...
				if(destcell.indexOf("DETAIL_")==-1){				//赋值给主表字段
					if(formulatxt.match(/DETAIL_\d+\./g)){			//取值字段包含明细表字段
						var reg = /^(EXCEL_AVERAGE|EXCEL_MIN|EXCEL_MAX)\((\s*(MAIN|TAB_\d+|DETAIL_\d+)\.[A-Z]+\d+\s*(\,)?)+\)$/;
...
function calculate_detailSumVal(destcell,formulatxt,cellrange, triTag){
	...
	jQuery("input[type='checkbox'][name='check_node_"+groupid+"']").each(function(){
		try{
			...
			if($.client=='mobile'){
				var rowindex_=jQuery(this).attr("_rowindex");
				if(rowindex_!=null&& rowindex_!=undefined){
				   rowid = rowindex_;
				}
			}
...
function calculate_detailAllRow(destcell,formulatxt,cellrange, triTag){
	...
	var eachRowObj = jQuery("input[type='checkbox'][name='check_node_"+groupid+"']");
	if(triTag === "addRow")
...
function formulaReplaceValue(formulatxt,cellrange,rowid){
	jQuery.each(cellrange,function(index,cellattr){
		...
		if(rowid === "all" && cellattr.indexOf("DETAIL_")>-1){	//需替换明细所有行字段
			...
			jQuery("input[type='checkbox'][name='check_node_"+groupid+"']").each(function(index){
 ...
 * 移动端将计算结果更新到赋值字段中
 */
function setFieldMobileValueById(fieldid,calculateResult){
	 	var fieldElement=jQuery("input#"+fieldid);
		var field_id = fieldid.replace("field","");
	...
	if(jQuery("#fieldformart"+field_id)){
		jQuery("#fieldformart"+field_id).val(fieldvalue);
	}
	if(fieldElement.attr("type")=="hidden"){		//只读
		if(jQuery("span#"+fieldid+"_span").size()>0){
			jQuery("span#"+fieldid+"_span").text(fieldvalue);
			if(jQuery("span#fieldformart"+field_id)){
				jQuery("span#fieldformart"+field_id).text(fieldvalue);
		}
		}
	}
	try{
	  jQuery("input#"+fieldid+"_d").val(fieldvalue);
	  jQuery("input#"+fieldid+"_d").attr("value",fieldvalue);
	  $("#"+$("#"+fieldid.replace("_","").replace("field","")).val()).html(fieldvalue);
	}catch(e){}

	window.setTimeout(function(){			//明细字段触发SQL赋值，需延时执行，why

		if($(fieldElement).attr("onfocus")!=undefined && $(fieldElement).attr("onfocus")!=null){
			   fieldElement.trigger("focus");		//触发合计函数
		}

		if($(fieldElement).attr("onblur")!=undefined && $(fieldElement).attr("onblur")!=null){
			   fieldElement.trigger("blur");		//触发合计函数
		}
		
		if($(fieldElement).attr("onchange")!=undefined && $(fieldElement).attr("onchange")!=null){
...
function setFieldValueById(fieldid,calculateResult){
	if($.client=='mobile'){
         setFieldMobileValueById(fieldid,calculateResult)
		return;
	}
	var fieldElement=jQuery("input#"+fieldid);
	if(fieldElement.size()==0 && jQuery("#"+fieldid)[0].tagName!= "SELECT")	return;
	var fieldvalue = "";
	var regnum = /^(-?\d+)(\.\d+)?$/;
	var isNumber = regnum.test(calculateResult);
	//eval(23*1.4)=32.19999，eval都存在误差，赋值前都toFixed下
	...
	fieldElement.attr("value",fieldvalue);
	if(jQuery("#"+fieldid)[0].tagName== "SELECT"){
		jQuery("#"+fieldid).find("option").each(function(index,ele){
			jQuery(ele).removeAttr("selected");
		});
		if(fieldvalue !== ""){
		    jQuery("#"+fieldid).find("option[value='" + parseInt(fieldvalue,10) + "']").attr("selected","selected");
		}
	}
	if(fieldElement.attr("type")=="hidden" && !(jQuery("#dis"+fieldid).length > 0 && jQuery("#dis"+fieldid)[0].tagName== "SELECT")){		//只读
		if(jQuery("span#"+fieldid+"span").size()>0){
			jQuery("span#"+fieldid+"span").text(fieldvalue);
		}
	}
	if(jQuery("#dis"+fieldid).length > 0 && jQuery("#dis"+fieldid)[0].tagName== "SELECT"){
		jQuery("#dis"+fieldid).find("option").each(function(index,ele){
			jQuery(ele).removeAttr("selected");
		});
		if(fieldvalue !== ""){
		    jQuery("#dis"+fieldid).find("option[value='" + parseInt(fieldvalue,10) + "']").attr("selected","selected");
			fieldElement.attr("value",parseInt(fieldvalue,10));
		}
	}
	window.setTimeout(function(){			//明细字段触发SQL赋值，需延时执行，why
		fieldElement.trigger("change");		//触发合计函数
