	...
	<script type="text/javascript">
		...
		function init() {
			...
			var settings = {
				...
				upload_url: "/weaver/weaver.workflow.exceldesign.ExcelUploadServlet",
		...
		function doChangeImgSrc(obj){
			...
			var zd_btn_confirmBtn = jQuery("#zd_btn_confirm");
			if($(obj).val() === "1"){
				$("#imglabel").text("<%=SystemEnv.getHtmlLabelName(125079, user.getLanguage())%>");
				zd_btn_confirmBtn.show().next().show();
			}else{
				$("#imglabel").text("<%=SystemEnv.getHtmlLabelName(125080, user.getLanguage())%>");
				zd_btn_confirmBtn.hide().next().hide();
			}
		}
		...
		function fileQueued(file) {
			...
			swfu.setUploadURL("/weaver/weaver.workflow.exceldesign.ExcelUploadServlet?method=uploadFile&savefile="+savefile);
		...
		function setPicDesc(){	
			...
			if(!$("[name=imgsrc_"+$("#srcdeal").val()+"]").val()){
				window.top.Dialog.alert('<%=SystemEnv.getHtmlLabelNames("126186,20001", user.getLanguage())%>！');
				return;
			}else{
				dialog.close(rejson);
			}
...
<BODY>
	<jsp:include page="/systeminfo/commonTabHead.jsp">
	   ...
	   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(681, user.getLanguage())%>"/>
	</jsp:include>     	
	
	<wea:layout type="twoCol">
	    <wea:group context="<%=SystemEnv.getHtmlLabelName(681, user.getLanguage())%>">
	    	<wea:item><%=SystemEnv.getHtmlLabelName(63, user.getLanguage())%></wea:item>
	    	<wea:item><select id="srcdtype" name="srcdtype">
	    		<option value="1"><%=SystemEnv.getHtmlLabelName(128045, user.getLanguage())%></option>
	    		<option value="2"><%=SystemEnv.getHtmlLabelName(33440, user.getLanguage())%></option>
	    		<option value="3"><%=SystemEnv.getHtmlLabelName(128046, user.getLanguage())%></option>
	    	</select></wea:item>
	    	<wea:item><%=SystemEnv.getHtmlLabelName(63, user.getLanguage())%></wea:item>
	    	<wea:item><select id="srcdeal" name="srcdeal" onchange="doChangeImgSrc(this);">
	    		<option value="1"><%=SystemEnv.getHtmlLabelNames("128047,74", user.getLanguage())%></option>
	    		<option value="2" selected><%=SystemEnv.getHtmlLabelName(125077, user.getLanguage())%></option>
	    	</select></wea:item>
	    	<wea:item><span id="imglabel"><%=SystemEnv.getHtmlLabelName(125080, user.getLanguage())%></span></wea:item>
	    	<wea:item>
	...
	<div id="uploadDiv" style="display:">
		<form id="frmmain" action="/weaver/weaver.workflow.exceldesign.ExcelUploadServlet?method=uploadFile" method="post" enctype="multipart/form-data">
		...
		<wea:layout needImportDefaultJsAndCss="false">
			<wea:group context=""  attributes="{groupDisplay:none}">
				<wea:item type="toolbar">
					<input type="button" value="<%=SystemEnv.getHtmlLabelName(826, user.getLanguage())%>" id="zd_btn_confirm"  class="zd_btn_cancle" onclick="setPicDesc()">
