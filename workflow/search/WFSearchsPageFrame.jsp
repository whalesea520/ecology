...
<%
	...
	//如果页面时通过左侧菜单打开的，则清除session中的查询条件
	//此从判断主要用于从wfCenterMore.jsp页面重定向到此页面时起作用
	if( "1".equals(fromleftmenu) ){
		SearchClause.setWhereClause("");
		SearchClause.setOrderClause("");
		SearchClause.setWhereclauseOs("");
		SearchClause.setOrderclauseOs("");
	}else{
		request.getRequestDispatcher("WFCustomSearchMiddleHandler.jsp").forward(request, response);
		return;
	}
%>
<html>
	...
	//alert(1);
	 ...
	 loadCustomSearch("<%=wfid%>","<%=customid%>");
	 <%}%>
	 });
...
if(!"customSearch".equals(isfrom)){
	...
	actionurl = "WFCustomSearchMiddleHandler.jsp?offical="+offical+"&officalType="+officalType+"&isfrom="+isfrom+"&workflowid="+wfid;
}
%>
<form id="weaver" name="frmmain" method="post" action="<%=actionurl%>">
	...
	<div id="advancedSearchDiv">
		    ...
		    <wea:group context='<%=SystemEnv.getHtmlLabelName(32905,user.getLanguage())%>'>
		    	...
		    	<wea:item >
			        <span style='float:left'>
						<select class=inputstyle name=creatertype onchange="changeType(this.value,'createridse1span','createridse2span');">
					...
					<span id="createridse1span" style="">
					   ...
					   <input type="hidden" id="createrid1_name" name="createrid_name" value="<%=request.getParameter("createrid_name")%>"/>
					</span>
					<span id="createridse2span" style="display:none;">
						<brow:browser viewType="0" name="createrid2" browserValue="" browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/CRM/data/CustomerBrowser.jsp" hasInput="true" width="135px;float:left" isSingle="true" hasBrowser = "true" isMustInput='1' completeUrl="/data.jsp?type=7"  browserSpanValue=""> 
						</brow:browser> 
						<input type="hidden" id="createrid2_name" name="createrid_name" value="<%=request.getParameter("createrid_name")%>"/>
					</span>
		    	</wea:item>
		     ...
		     <wea:group context='<%=SystemEnv.getHtmlLabelName(32843,user.getLanguage())%>'  >
		    	...
		    	<wea:item>
		    		 <brow:browser viewType="0" name="ownerdepartmentid" browserValue="" browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/company/MutiDepartmentBrowser.jsp?selectedids=" hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1' completeUrl="/data.jsp?type=4" width="80%" browserSpanValue=""> </brow:browser>
		...
		%>
		<%--
		<jsp:include page="/workflow/search/WFCustomSearchGenerate.jsp" flush="true">
			<jsp:param name="workflowid" value="<%=wfid%>" />
			<jsp:param name="customid" value="<%=customid %>" />
			<jsp:param name="issimple" value="true" />
			<jsp:param name="isfrom" value="customSearch" />
		</jsp:include> --%>
		<%//}%>
	</div>
</form>
...
function resetCustom(){
	loadCustomSearch("<%=wfid%>","<%=customid%>");
	//jQuery("#cutomQuerySelect").selectbox("detach");
	//jQuery("#cutomQuerySelect").val("");
	//jQuery("#cutomQuerySelect").selectbox("attach");
}
</script>
</BODY>
</HTML>
