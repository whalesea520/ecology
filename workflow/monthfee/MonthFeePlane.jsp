...
String titlename =  SystemEnv.getHtmlLabelName(15473,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(15464,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<input type=hidden name=src value="plane">
...
<table class="viewform">
  ...
  <td><%=SystemEnv.getHtmlLabelName(15467,user.getLanguage())%></td>
  ...
  <tr>
  <td><%=SystemEnv.getHtmlLabelName(15468,user.getLanguage())%></td>
  <td class=field>
...
<table class=liststyle cellspacing=1  >
  ...
  <tr class=header>
    ...
    <td><%=SystemEnv.getHtmlLabelName(15465,user.getLanguage())%></td>
    <td><%=SystemEnv.getHtmlLabelName(15467,user.getLanguage())%></td>
    <td><%=SystemEnv.getHtmlLabelName(15469,user.getLanguage())%></td>
...
String sql="select t1.* from workflow_form t1,workflow_requestbase t2 where t1.requestid = t2.requestid "+
            " and t2.workflowid=8 and t2.currentnodetype='3'";
if(serialnum.equals("")&&crmid.equals("")&&resourceid.equals("")&&departmentid.equals("")){
    sql+=" and amount=0";
}
else{
    if(!serialnum.equals("")){
...
<script language=vbs>
sub onShowCrm()
	id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/CRM/data/CustomerBrowser.jsp?type=15")
