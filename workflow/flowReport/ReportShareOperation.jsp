...
if(method.equals("delete")){
    ...
    response.sendRedirect("ReportShareSetTab.jsp");
}
if(method.equals("deleteAll")){
	String idss[]=Util.TokenizerString2(ids,",");
	String reportids[] = Util.TokenizerString2(reportid,",");
	for(int i=0;i<idss.length;i++){
		RecordSet.executeProc("WorkflowReportShare_Delete",idss[i]);
		//ReportShare.setReportShareByReport(reportids[i]);
	}
	response.sendRedirect("ReportShareSetTab.jsp");
}
...
else if(method.equals("add"))
{
	...
	//ReportShare.setReportShareByReport(reportid);
	//if("1".equals(dialog)){
	//	response.sendRedirect("/workflow/report/ReportShareAdd.jsp?isclose=1&id="+reportid+"&isBill="+isBill+"&formID="+formID);
	//}else{
	//	response.sendRedirect("/workflow/report/ReportShare.jsp?id="+reportid);
	//}
	response.sendRedirect("/workflow/flowReport/ReportShareSetAdd.jsp?isclose=1");
}
%>
