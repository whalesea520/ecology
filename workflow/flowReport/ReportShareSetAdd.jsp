...
String titlename = SystemEnv.getHtmlLabelName(33666,user.getLanguage()) + SystemEnv.getHtmlLabelName(33508,user.getLanguage()) ;
int needchange=0;
String needfav ="1";
String needhelp ="";
...
<wea:layout type="twoCol">
    <wea:group context='<%=SystemEnv.getHtmlLabelName(2112,user.getLanguage())%>'>
    	<wea:item><%=SystemEnv.getHtmlLabelName(19026,user.getLanguage())%></wea:item>
	    <wea:item>
			<select class=inputstyle  name=reportid style="width: 150px;">
    	...
    	<wea:item>
		    <select class=inputstyle  name=sharetype onchange="onChangeSharetype()" style="float:left;width: 150px;">
				<option value="1"><%=SystemEnv.getHtmlLabelName(179,user.getLanguage())%></option><!-- 人力资源 -->
				<option value="3" selected><%=SystemEnv.getHtmlLabelName(124,user.getLanguage())%></option><!-- 部门 -->
    	...
    	<wea:item>
			<select class="inputstyle"  name="sharelevel" onchange="onChangeShareLevel()" style="float: left;width: 150px;">
				...
				<option value="3"><%=SystemEnv.getHtmlLabelName(18513,user.getLanguage())%></option><!-- 同部门下级部门 -->
				<option value="4"><%=SystemEnv.getHtmlLabelName(25512,user.getLanguage())%></option><!-- 多分部 -->
				<option value="9"><%=SystemEnv.getHtmlLabelName(17006,user.getLanguage())%></option><!-- 多部门 -->
			</select>
			<span id="departmentidsDivSpan" style="display:none;">
				<brow:browser name="departmentids" viewType="0" hasBrowser="true" hasAdd="false" 
						getBrowserUrlFn="onShowMutiDepartment"
						isMustInput="2" isSingle="false" hasInput="true" completeUrl="javascript:getajaxurl2()"  width="150px" browserValue="" browserSpanValue=""/>	
			</span>    
...
if("<%=isclose%>"==1){
	var dialog = parent.getDialog(window);
	var parentWin = parent.getParentWindow(window);
	parentWin.location="/workflow/flowReport/ReportShareSetTab.jsp";
	parentWin.closeDialog();	
}
