...
String sqlwhere = "select t3.id,t2.requestname,t3.defcountprice,t3.countprice from workflow_form t1,workflow_requestbase t2,LgcStockInOut t3 where t1.requestid = t2.requestid and t1.billid = t3.id and t1.billformid = 3";
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="StockOutBrowser.jsp" method=post>
...
<TD width=80% class=field><input name=name value="<%=name%>" style="width:80%"></TD>
</TR>
<TR class="Spacing"><TD class="Line1" colspan=4></TD></TR>
</table>
...
<TH width=80%><%=SystemEnv.getHtmlLabelName(21192,user.getLanguage())%></TH></tr>
