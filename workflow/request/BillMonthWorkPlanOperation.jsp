...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存	
	...
	//操作detail表
	RecordSet.executeProc("bill_monthinfodetail_DeleteByType",""+billid+flag+"1");
	int rowsum1 = Util.getIntValue(Util.null2String(request.getParameter("nodesnum")));
	for(int i=0;i<rowsum1;i++) {	
		String workname = Util.fromScreen(request.getParameter("type_"+i+"_name"),user.getLanguage());
		if(workname.equals("")) continue ;
		String workdesc = Util.fromScreen(request.getParameter("type_"+i+"_desc"),user.getLanguage());
		String curdate = Util.fromScreen(request.getParameter("type_"+i+"_date"),user.getLanguage());
