...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
	...
	for(int i=0;i<rowsum;i++) {		
		...
		String begindate = Util.null2String(request.getParameter("node_"+i+"_begindate"));
		String begintime = Util.null2String(request.getParameter("node_"+i+"_begintime"));
		String enddate = Util.null2String(request.getParameter("node_"+i+"_enddate"));
		String endtime = Util.null2String(request.getParameter("node_"+i+"_endtime"));
		String frompos = Util.null2String(request.getParameter("node_"+i+"_frompos"));
		float beginnumber = Util.getFloatValue(request.getParameter("node_"+i+"_beginnumber"),0);
		float endnumber = Util.getFloatValue(request.getParameter("node_"+i+"_endnumber"),0);
		float number = endnumber-beginnumber;	
		int tmpuserid = Util.getIntValue(Util.null2String(request.getParameter("node_"+i+"_userid")),0);
		int userdepid = Util.getIntValue(Util.null2String(request.getParameter("node_"+i+"_userdepid")),0);
		int isotherplace = Util.getIntValue(Util.null2String(request.getParameter("node_"+i+"_isotherplace")),0);
		
		if(carno != 0){
			Procpara = ""+requestid + flag +date+ flag +driver+ flag +carno+ flag +begindate+ flag +begintime
				+ flag +enddate+ flag +endtime+ flag +frompos+ flag +beginnumber+ flag +endnumber+ flag +number+ flag +tmpuserid+ flag +userdepid+ flag +isotherplace;
			RecordSet.executeProc("bill_CptCarOut_Insert",Procpara);
		}
	}	
...
if(src.equals("submit")&&iscreate.equals("1")) {//新建request且选择提交
	...
	for(int i=0;i<rowsum;i++) {		
		String date = Util.null2String(request.getParameter("node_"+i+"_date"));
		int driver = Util.getIntValue(Util.null2String(request.getParameter("node_"+i+"_driver")),0);
		String carno = Util.null2String(request.getParameter("node_"+i+"_carno"));
		String begindate = Util.null2String(request.getParameter("node_"+i+"_begindate"));
		String begintime = Util.null2String(request.getParameter("node_"+i+"_begintime"));
		String enddate = Util.null2String(request.getParameter("node_"+i+"_enddate"));
		String endtime = Util.null2String(request.getParameter("node_"+i+"_endtime"));
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	for(int i=0;i<rowsum;i++) {
		...
		Procpara = ""+tmpid + flag +date+ flag +driver+ flag +carno+ flag +begindate+ flag +begintime
			+ flag +enddate+ flag +endtime+ flag +frompos+ flag +beginnumber+ flag +endnumber+ flag +number+ flag +tmpuserid+ flag +userdepid+ flag +isotherplace;
			
		out.print("Procpara:::"+Procpara);
		RecordSet.executeProc("bill_CptCarOut_Update",Procpara);
	}	
	...
	String operatortype = "";
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_CptCarOut where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
