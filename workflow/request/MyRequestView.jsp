...
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
		...
		<script type="text/javascript">
			...
			{
				var ajax=ajaxinit();
       			ajax.open("POST", "/workflow/request/MyRequestViewAjax.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>", true);
			...
			function leftMenuClickFn(attr,level,numberType){
				...
				if(level==1){
					...
					window.nodeids=null;
					if(fromAdvancedMenu=="1"){
						url="/workflow/search/wfTabNewFrame.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>&method=myreqeustbywftype&fromadvancedmenu="+fromAdvancedMenu+"&infoId="+infoId+"&wftype="+typeid+"&complete=2&viewcondition="+viewcondition+"&selectedContent="+selectedContent +"&menuType="+menuType +"&date2during="+date2during +"&viewType=4&wftypes=<%=_wftypes %>";
					}else{
						url="/workflow/search/wfTabNewFrame.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>&method=myreqeustbywftype&wftype="+typeid+"&complete=2&viewcondition="+viewcondition+"&date2during="+date2during +"&viewType=4&wftypes=<%=_wftypes %>";
					}
				}else{
					if(numberType==null){
						numberType="flowAll";
					...
					url="/workflow/search/wfTabNewFrame.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>&method=myreqeustbywfid&workflowid="+workflowid+"&complete=2&viewcondition="+viewcondition+"&date2during="+date2during+"&viewType=4&wftypes=<%=_wftypes %>";
				}
			...
			function WfSearchAll(){
				...
				/*清除右侧查询条件并重新查询*/
				document.getElementById("myFrame").src ="/workflow/search/wfTabNewFrame.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>&method=myall&viewType=4&viewScope=mine&wftypes=<%=_wftypes %>&complete=2";
			}
		</script>
		...
		<table cellspacing="0" cellpadding="0" class="flowsTable"  >
			<tr>
				...
				<td rowspan="2">
					<iframe id="myFrame" name="myFrame" src="/workflow/search/wfTabNewFrame.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>&method=myall&viewType=4&viewScope=mine&wftypes=<%=_wftypes %>&complete=2" class="flowFrame" style='height:100%;' frameborder="0" ></iframe>
				</td>
			</tr>
			<tr>
