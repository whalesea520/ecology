...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
    ...
    //加入字段信息	
    String sqltmp = " insert into bill_CptCheckMain(resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from bill_CptCheckMain";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	// add record into bill_CptCheckDetail
	...
	for(int i=0;i<nodesnum;i++) {		
		...
		float realnumber = Util.getFloatValue(request.getParameter("node_"+i+"_number"),0);
		float price = 0;
		String remark1 = Util.null2String(request.getParameter("node_"+i+"_remark"));
		
		String para = ""+billid+flag+cptid+flag+theorynumber+flag+realnumber+flag + price+flag + remark1;
		RecordSet.executeProc("bill_CptCheckDetail_Insert",para);
		
	}					
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update bill_CptCheckMain "+updateclause+" where id = "+billid;
...
if(src.equals("submit")&&iscreate.equals("1")) {//新建request且选择提交
	...
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update bill_CptCheckMain "+updateclause+" where id = "+billid;
	out.print("!!!!!!!!!updateclause:"+updateclause);
	out.print(updateclause);
	RecordSet.executeSql(updateclause);
	...
	String operatortype = "";
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_CptCheckMain where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
