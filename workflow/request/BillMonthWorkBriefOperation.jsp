...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存	
	...
	//更新字段信息     
	String sqltmp = " insert into Bill_workinfo (resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from Bill_workinfo";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	updateclause+= "requestid="+requestid+",billid="+formid+",status='0',createdate='"+CurrentDate+"',";
					
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update Bill_workinfo "+updateclause+" where id = "+billid;
	...
	//操作detail表
	RecordSet.executeProc("bill_monthinfodetail_DeleteByType",""+billid+flag+"2");
	int rowsum1 = Util.getIntValue(Util.null2String(request.getParameter("nodesnum")));
	for(int i=0;i<rowsum1;i++) {	
		String workresult = Util.fromScreen(request.getParameter("type_"+i+"_result"),user.getLanguage());
		if(workresult.equals("")) continue ;
		String curscale = Util.fromScreen(request.getParameter("type_"+i+"_scale"),user.getLanguage());
...
if(src.equals("submit")&&iscreate.equals("1")) {//新建request且选择提交
	...
	for(i=0;i<destnodeids.size();i++){
		...
		if(where.trim().equals(""))	break;
			...
			sql="select * from bill_workinfo where requestid="+requestid+" and "+where;
			RecordSet.executeSql(sql);
...
if(src.equals("delete")&&iscreate.equals("0")){//处理request且选择删除logtype=5
	...
	// 将 bill_workinfo 表的状态改为 2
	RecordSet.executeProc("bill_workinfo_UpdateStatus",""+billid+flag+"2");
}
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	if(nodetype.equals("2")){
		int rowsum1 = Util.getIntValue(Util.null2String(request.getParameter("nodesnum")));
		for(int i=0;i<rowsum1;i++) {	
			String curid = Util.fromScreen(request.getParameter("type_"+i+"_id"),user.getLanguage());
			if(curid.equals("")) continue ;
			String curpoint = Util.fromScreen(request.getParameter("type_"+i+"_point"),user.getLanguage());
			String para = curid + flag + curpoint;
			RecordSet.executeProc("bill_monthinfodetail_UpdateByPoint",para);
		}
	}
	//节点自动赋值操作
    RequestCheckAddinRules.resetParameter();
	...
	if(passedgroups<totalgroups){//当前节点没有完全审批通过，继续停留在当前节点
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_workinfo where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
        ...
        // 如果为结束节点,将 bill_workinfo 表的状态改为 1
    	if(destnodetype.equals("3"))
    		RecordSet.executeProc("bill_workinfo_UpdateStatus",""+billid+flag+"1");
    	//出口自动赋值操作
        RequestCheckAddinRules.resetParameter();
        RequestCheckAddinRules.setRequestid(requestid);
        RequestCheckAddinRules.setObjid(linkid);
...
if(src.equals("active")){//激活request logtype=6
	...
	//加入LOG表信息
        ...
        if(nodetype.equals("3")) 
            ...
            RecordSet.executeProc("bill_workinfo_UpdateStatus",""+billid+flag+"0");
}
