...
if( RequestManager.getNextNodetype().equals("3") ) {
	rs.executeSql(" select * from Bill_HrmDismiss where id = " + billid ) ;
	if( rs.next() ) {
		String resource_n = Util.null2String(rs.getString("resource_n"));		
		String dismissdate =    Util.null2String(rs.getString("dismissdate"));
		String oldjobtitleid = ResourceComInfo.getJobTitle(resource_n);
		String docid = Util.null2String(rs.getString("docid"));
		String dismissreason = Util.null2String(rs.getString("dismissreason")) ;
		if(dismissreason.equals("")) {
			dismissreason = SystemEnv.getHtmlLabelName(6003,user.getLanguage());
		}
		char separator = Util.getSeparator() ;  
		String para = resource_n + separator + dismissdate + separator + dismissreason + separator + docid + separator + ""  + separator + oldjobtitleid + separator + "5" + separator + user.getUID();
        rs.executeProc("HrmResource_Dismiss",para);
	}
}
