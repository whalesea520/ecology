...
RecordSet.executeProc("bill_CptCheckMain_Select",billid+"");
RecordSet.next();
...
<TR class=Section>
  ...
  <tr>
 <table Class=ListShort cols=10 id="oTable">
      	<COLGROUP>
  	<COL width="10%">
  	<COL width="10%">
  	<COL width="10%">
  	...
  	<COL width="20%">
	    ...
	    <TD>备注</TD>
            ...
            <%
	           int linecolor=0;  
	           String sqltmp  = " select * from bill_CptCheckDetail a,CptCapital b where a.cptid = b.id and a.cptcheckid = "+billid;
	...
	while(RecordSet.next()){
		String  capitalid = Util.toScreen(RecordSet.getString("id"),user.getLanguage()) ;
			...
			String	capitalspec=Util.toScreen(RecordSet.getString("capitalspec"),user.getLanguage()) ;
			String	capitalnum=Util.toScreen(RecordSet.getString("capitalnum"),user.getLanguage()) ;
            %>
            <tr <%if(linecolor==0){%> class=datalight <%} else {%> class=datadark <%}%> > 
            <td><%=mark%></td>
            <td><a href='/cpt/capital/CptCapital.jsp?id=<%=RecordSet.getString("cptid")%>'><%=Util.toScreen(CapitalComInfo.getCapitalname(RecordSet.getString("cptid")),user.getLanguage())%> </a>
            <td><%=capitalspec%></td>
           <TD><%=tempcapitalprice%></TD>
           <TD><%=capitalnum%></TD>
            <td><%=RecordSet.getString("realnumber")%></td>
            <TD><%=RecordSet.getFloat("realnumber")-Util.getFloatValue(capitalnum,0)%></TD>
            <td><%=(float)(((int)((RecordSet.getFloat("realnumber")-Util.getFloatValue(capitalnum,0))*tempcapitalprice*100))/100.00)%></td>
            <td><%=Util.toScreen(RecordSet.getString("remark"),user.getLanguage())%></td>
