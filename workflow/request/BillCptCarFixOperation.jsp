...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
	...
	int i=0;	
		...
		if(carno != 0){
			Procpara = ""+requestid + flag +date+ flag +driver+ flag +carno+ flag +fixfee+ flag +remax;
			RecordSet.executeProc("bill_CptCarFix_Insert",Procpara);
		}
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
		...
		if(carno != 0){
			Procpara = ""+tmpid + flag +date+ flag +driver+ flag +carno+ flag +fixfee+ flag +remax;
			out.print(Procpara);
			RecordSet.executeProc("bill_CptCarFix_Update",Procpara);
		}
	...
	String operatortype = "";
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_CptCarFix where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
