...
String overlapagentstrid = Util.null2String((String)session.getAttribute(user.getUID()+"_"+beagenterId+"_"+agenterId));
...
<table width="98%"   border="0"  >
  ...
  <tr  >
    ...
    <tr >
    <td align="left" valign="middle" colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
    <input type="radio"  <%if(method.equals("editAgent")){%> checked="checked" <%} %> name="overlapAgenttype" value="2"><%=SystemEnv.getHtmlLabelName(82670,user.getLanguage()) %>&nbsp;</td>
  </tr>
  <%}else{ %>
	   <tr  >
	    <td   width="60px" rowspan="2" valign="middle" align="right"><img style="margin-right: 10px;" id="Icon_undefined" align="absmiddle" src="/wui/theme/ecology8/skins/default/rightbox/icon_query_wev8.png" width="26" height="26"></td>
		<td   width="331" valign="middle" align="left"><%=SystemEnv.getHtmlLabelName(82666,user.getLanguage()) %>  <a href="#" style="color:#0000FF" onclick="onagentedit()"><%=overlapAgent %></a>  <%=SystemEnv.getHtmlLabelName(82667,user.getLanguage())+"<br>"+SystemEnv.getHtmlLabelName(16631,user.getLanguage()) +SystemEnv.getHtmlLabelName(82670,user.getLanguage())  %>?
...
function sumbitform(){
 ...
 function onagentedit(){
  var   dialog2 = new window.top.Dialog();
		...
		dialog2.currentWindow = window;
		var url = "/workflow/request/wfAgentMyCondit.jsp?agentFlag=0&agented=0&beagenterId=<%=beagenterId%>&agenterId=<%=agenterId%>";
