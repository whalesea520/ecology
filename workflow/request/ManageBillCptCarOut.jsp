...
<form name="frmmain" method="post" action="BillCptCarOutOperation.jsp">
...
<TR class=Section>
 ...
 <% if(!nodetype.equals("0")){%>
    ...
    <td><a href='/cpt/capital/CptCapital.jsp?id=<%=RecordSet.getString("carno")%>'><%=Util.toScreen(CapitalComInfo.getCapitalname(RecordSet.getString("carno")),user.getLanguage())%> </a>
     <td><%=RecordSet.getString("begindate")%></td>
         ...
         <td><a href="javaScript:openhrm(<%=RecordSet.getString("userid")%>);" onclick='pointerXY(event);'>
 ...
 <input type="hidden" name = node_<%=rowindex%>_id value="<%=RecordSet.getString("id")%>">
	...
	<input type='hidden' name='node_<%=rowindex%>_carno' id='node_<%=rowindex%>_carno' value="<%=RecordSet.getString("carno")%>">
     ...
     <td><button class=Browser onClick='getDate(node_<%=rowindex%>_begindatespan,node_<%=rowindex%>_begindate)'></button> 
         <span class=saveHistory id=node_<%=rowindex%>_begindatespan><%=RecordSet.getString("begindate")%></span> 
    	<input type='hidden' name='node_<%=rowindex%>_begindate' id='node_<%=rowindex%>_begindate' value="<%=RecordSet.getString("begindate")%>" >
    </td>
         <td>
         <button class=Browser onClick='onShowTime(node_<%=rowindex%>_begintimespan,node_<%=rowindex%>_begintime)'></button> 
         <span class=saveHistory id=node_<%=rowindex%>_begintimespan><%=RecordSet.getString("begintime")%></span> 
    	<input type='hidden' name='node_<%=rowindex%>_begintime' id='node_<%=rowindex%>_begintime' value="<%=RecordSet.getString("begintime")%>" >
    	</td>
         <td><button class=Browser onClick='getDate(node_<%=rowindex%>_enddatespan,node_<%=rowindex%>_enddate)'></button> 
         <span class=saveHistory id=node_<%=rowindex%>_enddatespan><%=RecordSet.getString("enddate")%></span> 
    	<input type='hidden' name='node_<%=rowindex%>_enddate' id='node_<%=rowindex%>_enddate' value="<%=RecordSet.getString("enddate")%>" >
    </td>
         <td>
         <button class=Browser onClick='onShowTime(node_<%=rowindex%>_endtimespan,node_<%=rowindex%>_endtime)'></button> 
         <span class=saveHistory id=node_<%=rowindex%>_endtimespan><%=RecordSet.getString("endtime")%></span> 
    	<input type='hidden' name='node_<%=rowindex%>_endtime' id='node_<%=rowindex%>_endtime' value="<%=RecordSet.getString("endtime")%>" >
    	</td>
    	<td><input type='text' name='node_<%=rowindex%>_frompos' style=width:100% value="<%=RecordSet.getString("frompos")%>"></td>
         <td><input type='text' style=width:100%  name='node_<%=rowindex%>_beginnumber' onchange='changenumber(<%=rowindex%>)' value="<%=RecordSet.getString("beginnumber")%>"></td>
         <td><input type='text' style=width:100%  name='node_<%=rowindex%>_endnumber' onchange='changenumber(<%=rowindex%>)' value="<%=RecordSet.getString("endnumber")%>"></td>
         <td><span id='node_<%=rowindex%>_number' name='node_<%=rowindex%>_number'><%=RecordSet.getString("number_n")%></span></td>
         <td>
         <button class=Browser onClick='onShowResource(node_<%=rowindex%>_useridspan,node_<%=rowindex%>_userid)'></button>
	<span class=saveHistory id=node_<%=rowindex%>_useridspan><a href="javaScript:openhrm(<%=RecordSet.getString("userid")%>);" onclick='pointerXY(event);'>
              <%=Util.toScreen(ResourceComInfo.getResourcename(RecordSet.getString("userid")),user.getLanguage())%></a></span> 
	<input type='hidden' name='node_<%=rowindex%>_userid' id='node_<%=rowindex%>_userid' value="<%=RecordSet.getString("userid")%>">
	</td>         
         <td>
         <button class=Browser onClick='onShowDepartment(node_<%=rowindex%>_userdepidspan,node_<%=rowindex%>_userdepid)'></button> 
	<span class=saveHistory id=node_<%=rowindex%>_userdepidspan><a 
      href="/hrm/company/HrmDepartmentDsp.jsp?id=<%=RecordSet.getString("userdepid")%>"><%=Util.toScreen(DepartmentComInfo.getDepartmentname(RecordSet.getString("userdepid")),user.getLanguage())%></a></span> 
	<input type='hidden' name='node_<%=rowindex%>_userdepid' id='node_<%=rowindex%>_userdepid' value="<%=RecordSet.getString("userdepid")%>">
         </TD>
         <td>
         <input type='checkbox' name='node_<%=rowindex%>_isotherplace' value='1' <%if(RecordSet.getString("isotherplace").equals("1")){%> checked <%}%>>
         </td> 
         <%}%>
</tr>
<%
