...
//out.println("select * from bill_Discard_Detail where detailrequestid="+requestid);
while(RecordSet.next()){
		%>
		<tr <%if(linecolor==0){%> class=DataLight <%} else {%> class=DataDark <%}%> > 
		<td <%if(dsptypes.indexOf("1_0")!=-1){%> style="display:none" <%}%>>
			<a href="javascript:openFullWindowForXtable('/cpt/capital/CptCapital.jsp?id=<%=RecordSet.getString("capitalid")%>')"><%=Util.toScreen(CapitalComInfo.getCapitalname(RecordSet.getString("capitalid")),user.getLanguage())%></a>
		</td>
	   <td <%if(dsptypes.indexOf("2_0")!=-1){%> style="display:none" <%}%>><%=RecordSet.getString("numbers")%></td>
		<td <%if(dsptypes.indexOf("3_0")!=-1){%> style="display:none" <%}%>><%=RecordSet.getString("dates")%></td>
		<td <%if(dsptypes.indexOf("4_0")!=-1){%> style="display:none" <%}%>><%=RecordSet.getString("fee")%></td>
		<td <%if(dsptypes.indexOf("5_0")!=-1){%> style="display:none" <%}%>><%=RecordSet.getString("remark")%></td>
	   </tr>
	   </tr>
