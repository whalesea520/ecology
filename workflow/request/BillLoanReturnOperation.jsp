...
//得到页面字段的值
String name=Util.fromScreen2(request.getParameter("requestname"),user.getLanguage());
...
String occurdate=Util.fromScreen(request.getParameter("applydate"),user.getLanguage());
String currencyid=Util.fromScreen(request.getParameter("currencyid"),user.getLanguage());
String exchangerate=Util.fromScreen(request.getParameter("exchangerate"),user.getLanguage());
String basictype="3";
String detailtype="1";
...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存	
	...
	//更新字段信息     
	String sqltmp = " insert into Bill_HrmFinance(resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from Bill_HrmFinance";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	while(RecordSet.next()){
		...
		if(fieldname.equals("resourceid")|| fieldname.equals("departmentid")||fieldname.equals("occurdate")|| fieldname.equals("currencyid")|| fieldname.equals("exchangerate")|| fieldname.equals("basictype")) continue ;
    	...
    	if(fieldhtmltype.equals("3") && (fieldtype.equals("1") ||fieldtype.equals("17")))
    		hrmids +=","+request.getParameter("field"+fieldid);
    	if(fieldhtmltype.equals("3") && (fieldtype.equals("7") || fieldtype.equals("18")))
    		crmids +=","+request.getParameter("field"+fieldid);
    	if(fieldhtmltype.equals("3") && fieldtype.equals("8"))
    		prjids +=","+request.getParameter("field"+fieldid);
    	if(fieldhtmltype.equals("3") && fieldtype.equals("9"))
    		docids +=","+request.getParameter("field"+fieldid);
    } 
	updateclause+= "resourceid="+resourceid+",occurdate='"+occurdate+"',departmentid="+departmentid+
					",currencyid="+currencyid+",exchangerate='"+exchangerate+
					"',basictype="+basictype+",detailtype="+detailtype+
					",requestid="+requestid+",billid="+formid+",name='"+name+"',status='0',";
					
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update Bill_HrmFinance "+updateclause+" where id = "+billid;
	...
	Procpara=requestid+""+ flag + workflowid+"" + flag + "" + flag + "" + flag 
			...
			+nodepasstime+flag+nodepasstime+flag+docids+flag+crmids+flag+hrmids+flag+prjids;
...
if(src.equals("delete")&&iscreate.equals("0")){//处理request且选择删除logtype=5
	...
	Procpara=requestid+""+ flag + workflowid+"" + flag + lastnodeid+"" + flag + lastnodetype+"" + flag 
	    		...
	    		+ creatertype + flag +lastoperatortype+flag+nodepasstime+flag+nodelefttime+flag+docids+flag+crmids+flag+hrmids+flag+prjids ;
	...
	// 将 bill_hrmfinance 表的状态改为 2
	RecordSet.executeProc("bill_HrmFinance_UpdateStatus",""+billid+flag+"2");
}
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	while(RecordSet.next()){
		String fieldid=RecordSet.getString("id");
    	String fieldname=RecordSet.getString("fieldname");
    	if(fieldname.equals("basictype")) continue ;
    	String fielddbtype=RecordSet.getString("fielddbtype");
		if(isoracle){ 
			if(fielddbtype.equals("integer"))
				updateclause+=fieldname+" = "+Util.getIntValue(request.getParameter("field"+fieldid),0)+",";
...
if(src.equals("submit")&&iscreate.equals("0")){//处理request且选择提交logtype=2
	...
	if(nodetype.equals("2")){
		//进行财务结算
		RequestTransactionManager.resetParameter();
		RequestTransactionManager.setBillid(billid);
		RequestTransactionManager.setCurrentdate(CurrentDate);
		RequestTransactionManager.setOperator(user.getUID());
		RequestTransactionManager.insertTransaction();
	}
	//先检查passedgroups个数
	...
	if(passedgroups<totalgroups){//当前节点没有完全审批通过，继续停留在当前节点
	    ...
	    Procpara=requestid+""+ flag + workflowid+"" + flag + lastnodeid+"" + flag + lastnodetype+"" + flag 
	        	...
	        	+flag+docids+flag+crmids+flag+hrmids+flag+prjids ;
	    RecordSet.executeProc("workflow_Requestbase_Update",Procpara);
	    //加入LOG表信息
	    Procpara=requestid+"" + flag + workflowid+"" + flag + nodeid+"" + flag + "2" + flag 
		+ CurrentDate + flag + CurrentTime + flag + userid+"" + flag + remark + flag + clientip+flag+operatortype
        ...
        // 如果为结束节点,将 bill_hrmfinance 表的状态改为 1
    	if(destnodetype.equals("3"))
    		RecordSet.executeProc("bill_HrmFinance_UpdateStatus",""+billid+flag+"1");
    	//出口自动赋值操作
        RequestCheckAddinRules.resetParameter();
        RequestCheckAddinRules.setRequestid(requestid);
        RequestCheckAddinRules.setObjid(linkid);
...
if(src.equals("active")){//激活request logtype=6
	...
	Procpara=requestid+""+ flag + workflowid+"" + flag + lastnodeid+"" + flag + lastnodetype+"" + flag 
        		...
        		+nodepasstime+flag+nodelefttime+flag+docids+flag+crmids+flag+hrmids+flag+prjids ; 
        RecordSet.executeProc("workflow_Requestbase_Update",Procpara);
	...
	//加入LOG表信息
        ...
        if(nodetype.equals("3")) 
            ...
            RecordSet.executeProc("bill_HrmFinance_UpdateStatus",""+billid+flag+"0");
}
