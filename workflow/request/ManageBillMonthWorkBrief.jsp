...
%>
	...
	%>
		<%=thismonth%><input type=hidden name="field<%=fieldid%>" value="<%=thismonth%>">
	<%	
		}
		continue;
	}
...
<%	boolean islight=true;
	RecordSet.executeProc("bill_monthinfodetail_SelectByType",""+billid+flag+"2");
	while(RecordSet.next()){
...
%>
		  ...
		  <td><input type=text class=inputstyle  name="type_<%=rowindex1%>_point" 
		  value="<%=Util.toScreenToEdit(curpoint,user.getLanguage())%>"
		  onKeyPress='ItemNum_KeyPress()' onBlur='checknumber1(this)' maxlength=1>
		  <input type=hidden name="type_<%=rowindex1%>_id" value="<%=curid%>"></td>
		  <%} else {%>
...
%>
		  ...
		  <td><textarea name="type_<%=rowindex1%>_result" rows=2 style="width:80%"><%=Util.toScreenToEdit(curworkdesc,user.getLanguage())%></textarea></td>
		  <td><input type=input class=inputstyle  name="type_<%=rowindex1%>_scale" style="width:60%" 
