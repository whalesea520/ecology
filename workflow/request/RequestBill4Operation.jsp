...
if(Util.null2String(request.getParameter("needcheck")).equals("yes")){
	RecordSet.executeSql("select * from HrmResource");
	while(RecordSet.next()){
		out.print(RecordSet.getString(2)+"  "+RecordSet.getString(3)+"  ");
	}
	return;
}
else{
...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
	RecordSet.executeProc("workflow_RequestID_Update","");
        ...
        String sqltmp = " insert into bill_contract(contractno) values(0)";
        RecordSet.executeSql(sqltmp);
        sqltmp = " select max(id) from bill_contract";
        RecordSet.executeSql(sqltmp);
        if(RecordSet.next())
	...
	for(int i=0;i<rowsum;i++) {		
		...
		RecordSet.executeProc("bill_contractdetail_Insert",para);
	...
	float defcounttax = counttax/exchangerate;
       ...
       updateclause += "defcurrencyid="+defcurrencyid+",";
       updateclause += "defcountprice="+defcountprice+",defcounttax="+defcounttax+",countprice="+countprice+",counttax="+counttax+",";
        ...
        updateclause="update bill_contract "+updateclause+" where id="+billid;
        RecordSet.executeSql(updateclause);
  //      out.print(updateclause);
        RecordSet.executeSql("insert into workflow_form (requestid,billformid,billid) values ("+requestid+",4,"+billid+")");
   ...
   //查询下一节点
	...
	for(i=0;i<destnodeids.size();i++){
		...
		if(where.trim().equals(""))	break;
			...
			sql="select * from bill_contract where id="+billid+" and "+where;
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	for(int i=0;i<rowsum;i++) {		
		...
		if(tmpid==0 && assetid!=0){
			...
			RecordSet.executeProc("bill_contractdetail_Update",para);
		}
	}	
