...
%>
<form name="frmmain" method="post" action="BillCptCheckOperation.jsp">
...
%>
	...
	if(fieldhtmltype.equals("1")){
		...
		if(fieldname.equals("assortment")){
		%>
		 <select name="field<%=fieldid%>" <%if(!isedit.equals("1")){%> disabled <%}%> onchange="changetype(this)">
		 <%
		 String sqltmp = "select * from CptCapitalAssortment where supassortmentid = 0 and id <=15";
		 RecordSet.executeSql(sqltmp);
		 while(RecordSet.next()){
		 %>
		 <option value="<%=RecordSet.getString("id")%>"><%=Util.toScreen(RecordSet.getString("assortmentname"),user.getLanguage())%></option>
		 <%}%>
		 </select>
	        <%		
	        continue;
...
<TR class=Section>
  ...
  <tr>
  <table Class=ListShort cols=4 id="oTable">
      	<COLGROUP>
      	<COL width="4%">
  	<COL width="30%">
  	<COL width="30%">
		...
		switch(j) {
			...
			case 1: 
				...
				var sHtml = "<button class=Browser onClick='onShowAsset(node_"+rowindex+"_cptspan,node_"+rowindex+"_cptid)'></button> " + 
        					"<span class=saveHistory id=node_"+rowindex+"_cptspan><img src='/images/BacoError_wev8.gif' align=absmiddle></span> "+
			...
			case 2: 
				var oDiv = document.createElement("div"); 
				var sHtml = "<input type='input' style=width:80%  name='node_"+rowindex+"_number' onKeyPress='ItemCount_KeyPress()' onBlur='checkcount1(this);checkinput1(node_"+rowindex+"_number,node_"+rowindex+"_numberspan)' >"+
				            "<span id='node_"+rowindex+"_numberspan'><img src='/images/BacoError_wev8.gif' align=absmiddle></span>";
				oDiv.innerHTML = sHtml;   
				oCell.appendChild(oDiv);  
				break;
			...
			case 3: 
				var oDiv = document.createElement("div"); 
				var sHtml = "<input type='input'  style=width:80%  name='node_"+rowindex+"_remark'>";
				oDiv.innerHTML = sHtml;   
				oCell.appendChild(oDiv);  
				break;
...
if ("<%=newenddate%>"!="b" && "<%=newfromdate%>"!="a" && document.frmmain.<%=newenddate%>.value != ""){
	...
	function doSave(){
    	...
    	for(i=len-1; i >= 0;i--) {
    		if (document.forms[0].elements[i].name=='check_node')
    			parastr+=",node_"+document.forms[0].elements[i].value+"_number";
    			parastr+=",node_"+document.forms[0].elements[i].value+"_cptid";
    	}
...
sub onShowAsset(spanname,inputname)
	...
	id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/cpt/capital/CapitalBrowser.jsp?sqlwhere=where isdata='2' and capitalgroupid="+groupid)
