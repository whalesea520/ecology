...
String initDetailIframeStrAdd = "";
    ...
    ArrayList seldefieldsdemanage=new ArrayList();
    ArrayList changedefieldsdemanage=new ArrayList();
...
<wea:layout>
		     <wea:group context='<%=SystemEnv.getHtmlLabelName(84496,user.getLanguage()) + (detailno + 1) %>'>
			     <wea:item type="groupHead">
							<%
                            if(dtlmul.equals("1")){%>
                            <input type=button class="sapbtn" accessKey=P onclick="addSapRow<%=detailno%>(<%=detailno%>)"  title="<U>P</U>-SAP"></input>
                            <%}%>
                            
                            <%if(dtladd.equals("1")){%>
                            <input type=button class="addbtn" accessKey=A onclick="addRow<%=detailno%>(<%=detailno%>)"  title="<U>A</U>-<%=SystemEnv.getHtmlLabelName(611,user.getLanguage())%>"></input>
                            <%}%>
                            <%if(dtladd.equals("1")||dtldelete.equals("1")){%>
                            <input type=button class="delbtn" accessKey=E onclick="deleteRow1(<%=detailno%>);"  title="<U>E</U>-<%=SystemEnv.getHtmlLabelName(23777,user.getLanguage())%>"></input>
                            <%}%>
                   ...
                   %>
                    ...
                    %>
                        ...
                        if(rs.next()){
                            ...
                            if((maintable.indexOf("formtable_main_")==0 || maintable.indexOf("uf_")==0) && (tablename.indexOf("formtable_main_")==0 || tablename.indexOf("uf_")==0)){//新表单
                        ...
                        <td width="4%">
                        <input type='checkbox' name='check_node<%=detailno%>' value="<%=derecorderindex%>" <%if(!dtldelete.equals("1")){%>disabled<%}%>>
                        ...
                        <%    }        }
                                     if(defieldhtmltype.equals("1")){                          // 单行文本框
                                        ...
                                        if(defieldtype.equals("1")){                          // 单行文本框中的文本
                                            ...
                                            if(isdeedit.equals("1") && colcount1!=0 ){
                                          %>
                                        ...
                                        if(isdeedit.equals("1") && colcount1!=0 ){
                                            ...
                                            if( !defieldtype.equals("37") ) {    //  多文档特殊处理
                                                ...
                                                if (trrigerdetailfield.indexOf("field"+defieldid)>=0){
                                                    bclick="onShowBrowser2('"+defieldid+"_"+derecorderindex+"','"+url+"','"+linkurl+"','"+defieldtype+"',field"+defieldid+"_"+derecorderindex+".getAttribute('viewtype'));datainputd('field"+defieldid+"_"+derecorderindex+"')";
                                                } else {
                                   ...
                                   hasAdd="false" addOnClick='<%="onNewDoc('" + defieldid + "_" + derecorderindex + "')"%>'
                                   > </brow:browser>
                                   </span>
                                   <!-- 
                                   ...
                                   <%       }
                                        ...
                                        //日期控件不使用brow
                                        if( (defieldtype.equals("2") || defieldtype.equals("19")) || (isdeview.equals("1") && colcount1!=0  && (isdeedit.equals("0") ))){
                                   %>
                                   ...
                                   <%
                                        ...
                                        if(isdeview.equals("1") && colcount1!=0){
                                           %>
                                           <input type=checkbox class=inputstyle viewtype='<%=isdemand%>' temptitle='<%=defieldlable%>' value=1 <%if(isdeedit.equals("0") ){%> DISABLED <%}else{%>  name="field<%=defieldid%>_<%=derecorderindex%>" <%}if(defieldvalue.equals("1")){%> checked <%}%> >
                                           <%
                                               if(changedefieldsdemanage.indexOf(defieldid)>=0){
                                        %>
                                            <input type=hidden name="oldfieldview<%=defieldid%>_<%=derecorderindex%>" value="<%=Util.getIntValue(isdeview,0)+Util.getIntValue(isdeedit,0)+Util.getIntValue(isdemand,0)%>" />
                                        ...
                                        if( isdeedit.equals("0") ||  !isdeview.equals("1") || colcount1==0){
                                           %>
                                    	...
                                    	if(childfieldids.contains(defieldid)){
                                           ...
                                           <select style='width:90%!important;' notBeauty=true class=inputstyle viewtype='<%=isdemand%>' temptitle='<%=defieldlable%>' onBlur="checkinput2('field<%=defieldid%>_<%=derecorderindex%>','field<%=defieldid%>_<%=derecorderindex%>span',this.getAttribute('viewtype'));" <%if(seldefieldsdemanage.indexOf(defieldid)>=0){%>onchange="changeshowattr('<%=defieldid%>_1',this.value,<%=derecorderindex%>,<%=workflowid%>,<%=nodeid%>);<%if (trrigerdetailStr.indexOf("field" + defieldid +"_"+derecorderindex) >= 0) {%>datainputd('field<%=defieldid%>_<%=derecorderindex%>');<%}%><%=onchangeAddStr%>"<%}else{%>onchange="<%if (trrigerdetailStr.indexOf("field" + defieldid +"_"+derecorderindex) >= 0) {%>datainputd('field<%=defieldid%>_<%=derecorderindex%>');<%}%><%=onchangeAddStr%>"<%}%> <%if(isdeedit.equals("0")){%> name="disfield<%=defieldid%>_<%=derecorderindex%>" DISABLED <%}else{%> name="field<%=defieldid%>_<%=derecorderindex%>" <%}%>>
                                           <option value=""></option>
                                           <%
                                        // 查询选择框的所有可以选择的值
                ...
                try{
                    for (int i = 0; i < defieldids.size(); i++) {         // 循环开始
                        ...
                        if (defieldhtmltype.equals("1")) {                          // 单行文本框
                            ...
                            fieldhtml = "<select style='width:90%!important;' notBeauty=true class=inputstyle viewtype='"+isdemand+"' temptitle='"+defieldlable+"' ";
