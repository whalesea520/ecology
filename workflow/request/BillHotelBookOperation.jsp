...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存	
	...
	//更新字段信息     
	String sqltmp = " insert into bill_HotelBook (resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from bill_HotelBook";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	updateclause="update bill_HotelBook "+updateclause+" where id = "+billid;
	...
	for(int i=0 ; i<nodesnum ; i++){
	    String curhotelid=Util.fromScreen(request.getParameter("hotelid_"+i),user.getLanguage());
	    if(curhotelid.equals(""))   continue;
	    String curroomstyle=Util.fromScreen(request.getParameter("roomstyle_"+i),user.getLanguage());
	    String curroomsum=Util.fromScreen(request.getParameter("roomsum_"+i),user.getLanguage());
...
if(src.equals("submit")&&iscreate.equals("1")) {//新建request且选择提交
	...
	for(i=0;i<destnodeids.size();i++){
		...
		if(where.trim().equals(""))	break;
			...
			sql="select * from bill_HotelBook where requestid="+requestid+" and "+where;
			RecordSet.executeSql(sql);
...
if(src.equals("active")){//激活request logtype=6
	...
	//加入LOG表信息
        ...
        if(nodetype.equals("3")) 
            ...
            RecordSet.executeProc("bill_HotelBook_UpdateStatus",""+billid+flag+"0");
}
