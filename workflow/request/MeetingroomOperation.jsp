...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
    ...
    String sqltmp = " insert into Bill_Meetingroom(resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from Bill_Meetingroom";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	while(RecordSet.next()){
    	...
    	if(fieldname.equals("begindate"))
    		begindate = Util.null2String(request.getParameter("field"+fieldid));
    	if(fieldname.equals("begintime"))
    		begintime = Util.null2String(request.getParameter("field"+fieldid));
    	if(fieldname.equals("enddate"))
    		enddate = Util.null2String(request.getParameter("field"+fieldid));
    	if(fieldname.equals("endtime"))
    		endtime = Util.null2String(request.getParameter("field"+fieldid));
    	if(fieldname.equals("meetingroomid"))
    		meetingroomid = Util.null2String(request.getParameter("field"+fieldid));
    ...
    /*
    sql = " select * from Bill_Meetingroom where ((begindate <= '"+begindate+"' and begintime <= '"+begintime+"' and enddate >='"+begindate+"' and endtime >= '"+begintime+"') or (begindate <= '"+enddate+"' and begintime <= '"+endtime+"' and enddate >='"+enddate+"' and endtime >= '"+endtime+"')) and meetingroomid ="+meetingroomid; RecordSet.executeSql(sql);
    out.print(sql);
    if(RecordSet.next()){
    	response.sendRedirect("AddRequest.jsp?workflowid="+workflowid+"&message=1");
    	return;
    }
    */				
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update Bill_Meetingroom "+updateclause+" where id = "+billid;
	...
	if(RecordSet.next())
        ...
        Procpara=requestid+"" + flag + userid+"" + flag + "" + flag + workflowid+"" + flag + workflowtype+ flag + creatertype + flag +"0"+  flag + nodeid +
...
if(src.equals("submit")&&iscreate.equals("1")) {//新建request且选择提交
    ...
    /*
    sql = " select * from Bill_Meetingroom where ((begindate <= '"+begindate+"' and begintime <= '"+begintime+"' and enddate >='"+begindate+"' and endtime >= '"+begintime+"') or (begindate <= '"+enddate+"' and begintime <= '"+endtime+"' and enddate >='"+enddate+"' and endtime >= '"+endtime+"')) and meetingroomid ="+meetingroomid;
    RecordSet.executeSql(sql);
    if(RecordSet.next()){
    	out.print(sql);
    	response.sendRedirect("AddRequest.jsp?workflowid="+workflowid+"&message=1");
	...
	*/       	
	...
	//加入request总表信息        
        ...
        //加入查看人员列表
        Procpara=requestid+"" + flag + userid+"" + flag + "" + flag + workflowid+"" + flag + workflowtype+ flag + creatertype +flag+"2"+  flag + nodeid +
													flag + -1 + flag + "0" + flag + -1+flag+0;
        RecordSet.executeProc("workflow_CurrentOperator_I",Procpara);
	    ...
	    if(RecordSet.next())
	        ...
	        if(!destnodetype.equals("0")){       
		        ...
		        totalgroups = RequestCheckUser.addCurrentoperator();
			 	...
			 	totalgroups = 1;
			 	Procpara=requestid+"" + flag + creater+"" + flag + "" + flag + workflowid+"" + flag + workflowtype+flag+creatertype+flag+"0"+  flag + nodeid +
													flag + -1 + flag + "0" + flag + -1+flag+0;
	        	RecordSet.executeProc("workflow_CurrentOperator_I",Procpara);
        	...
        	if(destnodetype.equals("3")) {
        			RecordSet.executeProc("bill_HrmTime_UpdateStatus",""+billid+flag+statusvalue);
        	}
        	//出口自动赋值操作
	        RequestCheckAddinRules.resetParameter();
	        RequestCheckAddinRules.setRequestid(requestid);
