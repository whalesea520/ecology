...
boolean hasFF = true;
RecordSetFF.executeProc("Base_FreeField_Select","b3");
...
<form name="frmmain" method="post" action="RequestBill3Operation.jsp">
...
%>
 ...
 <input type="hidden" name="node_<%=rowsum%>_id" size=25 value="<%=RecordSet.getString("id")%>">
  ...
  <td>
  <input type='input' style=width:100% name='node_<%=rowsum%>_number' value='<%=RecordSet.getString("number_n")%>'>
  </td>
...
function addRow()
{
	...
	for(j=0; j<ncol; j++) {
		...
		switch(j) {
			...
			case 3: 
				var oDiv = document.createElement("div"); 
				var sHtml = "<input type='input' style=width:100%  name='node_"+rowindex+"_number'>";
				oDiv.innerHTML = sHtml;   
				oCell.appendChild(oDiv);  
				break;
