			...
			case 2: 
				...
				if(edittypes.indexOf("2_1")!=-1){
					...
					jQuery(oDiv).find(".e8Browser").e8Browser({
						   ...
						   linkUrl:"#",
						   isSingle:true,
						   completeUrl:"/data.jsp?type=23&cptstateid=1&inculdeNumZero=0",
						   browserUrl:"",
						   width:'90%',
						   hasAdd:false,
						   isSingle:true,
				...
				if(edittypes.indexOf("4_1")!=-1){
					if(mandtypes.indexOf("4_1")!=-1){
						...
						sHtml += "<img src='/images/BacoError_wev8.gif' align=absmiddle>";
        					...
        					sHtml = "<input type='text'  class=Inputstyle onBlur=checknumber1(this);changeamountsum('node_"+rowindex+"') id='node_"+rowindex+"_unitprice' name='node_"+rowindex+"_unitprice'>";
        				}
				...
				if(edittypes.indexOf("5_1")!=-1){
					if(mandtypes.indexOf("5_1")!=-1){
						...
						sHtml += "<img src='/images/BacoError_wev8.gif' align=absmiddle>";
        					...
        					sHtml = "<input type='text' class=Inputstyle onBlur='checknumber1(this);'  id='node_"+rowindex+"_amount' name='node_"+rowindex+"_amount'>";
        				}
	...
	try
	{
		...
		if (data) 
		{
			if (wuiUtil.getJsonValueByIndex(data, 0) != "") 
			{
				...
				jQuery("#node_"+rowindex+"_cptidspan").find("a").html(data.name);
...
{    
	return "/systeminfo/BrowserMain.jsp?url=/cpt/capital/CapitalBrowser.jsp?sqlwhere=where isdata=2 &cptstateid=1&inculdeNumZero=0";
}
function onSetCptCapital1(event,data,name)
{
	var cptid=jQuery("#"+name).val();
			...
			if (wuiUtil.getJsonValueByIndex(data, 0) != "") 
			{
				if(cptid!=''){
					...
					//	jQuery("#node_"+rowindex+"_capitalidspan").html("<a href='/cpt/capital/CptCapital.jsp?id="+data.id+"' target='_blank'>"+data.name+"</a>");
						jQuery("#node_"+rowindex+"_capitalidspan").find("a").replaceWith("<a href='/cpt/capital/CptCapital.jsp?id="+data.id+"' target='_blank'>"+data.name+"</a>");
						jQuery("#node_"+rowindex+"_capitalid").val(data.id);
				        jQuery("input[name='node_"+rowindex+"_number']").val(data.availablenum);
				        jQuery("#node_"+rowindex+"_capitalcount").val(data.availablenum);
				        if(!edittype){
				       		jQuery("#node_"+rowindex+"_numberspan").html(data.availablenum);
				        }
...
function changeamountsum(noderowindex){
	//console.log("noderowindex:"+noderowindex);
