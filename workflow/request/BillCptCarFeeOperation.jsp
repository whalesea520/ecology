...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
	...
	for(int i=0;i<rowsum;i++) {		
		...
		float oilfee = Util.getFloatValue(request.getParameter("node_"+i+"_oilfee"),0);
		float bridgefee = Util.getFloatValue(request.getParameter("node_"+i+"_bridgefee"),0);
		float fixfee = Util.getFloatValue(request.getParameter("node_"+i+"_fixfee"),0);
		float phonefee = Util.getFloatValue(request.getParameter("node_"+i+"_phonefee"),0);
		float cleanfee = Util.getFloatValue(request.getParameter("node_"+i+"_cleanfee"),0);
		String remax = Util.null2String(request.getParameter("node_"+i+"_remax"));
		
		if(carno != 0){
			Procpara = ""+requestid + flag +date+ flag +driver+ flag +carno+ flag +oilfee+ flag +bridgefee
				+ flag +fixfee+ flag +phonefee+ flag +cleanfee+ flag +remax;
			RecordSet.executeProc("bill_CptCarFee_Insert",Procpara);
		}
	}	
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	for(int i=0;i<rowsum;i++) {		
		...
		Procpara = ""+tmpid + flag +date+ flag +driver+ flag +carno+ flag +oilfee+ flag +bridgefee
				+ flag +fixfee+ flag +phonefee+ flag +cleanfee+ flag +remax;
		out.print("Procpara:::"+Procpara);
		RecordSet.executeProc("bill_CptCarFee_Update",Procpara);
	}	
	...
	String operatortype = "";
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_CptCarFee where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
