...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
	RecordSet.executeProc("workflow_RequestID_Update","");
        ...
        String sqltmp = " insert into LgcStockInOut(outstockno) values(0)";
	...
	for(int i=0;i<rowsum;i++) {		
		...
		String para = ""+billid+flag+assetid+flag+batchmark+flag + number ;
		RecordSet.executeProc("LgcStockInOutDetail_Insert1",para);
	}	
       ...
       updateclause += "modetype = '1',createrid="+userid+",createdate='"+CurrentDate+"',status='0',";
  ...
  //      out.print(updateclause);
        RecordSet.executeSql("insert into workflow_form (requestid,billformid,billid) values ("+requestid+",3,"+billid+")");
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	for(int i=0;i<rowsum;i++) {		
		...
		if(tmpid==0 && assetid!=0){
			...
			String para = ""+tmpid+flag+billid+flag+assetid+flag+batchmark+flag + number ;
			RecordSet.executeProc("LgcStockInOutDetail_Update1",para);
		}
	}	
       ...
       if(nodetype.equals("2")){
		...
		while(RecordSet.next()){
			...
			if(RecordSet1.next()){
				...
				tmpnumber = oldnumber - tmpnumber;
				String sqlup = "update LgcAssetStock set stocknum="+tmpnumber+" where id="+tmpid;
				RecordSet1.executeSql(sqlup);
			}
			else{
				String sqlin = " insert into LgcAssetStock(warehouseid,assetid,stocknum) values("+warehouseid+","+tmpaccetid+","+(0-tmpnumber)+")";
