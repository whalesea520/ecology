...
<wea:layout>
	<wea:group context='<%=SystemEnv.getHtmlLabelName(18913,languageidfromrequest)%>'>
		...
		<wea:item>
			<select class=inputstyle  id="submitNodeId" name=submitNodeId  onChange='nodechange(this.value)'  temptitle="<%=SystemEnv.getHtmlLabelName(18914,languageidfromrequest)%>" notBeauty="true">
                 <option value="" ></option>
                 <%
                 WFNodeMainManager.setWfid(workflowid);
                 WFNodeMainManager.selectWfNode();
...
function openSignPrint() {
  var relaterequest='<%=Util.null2String(request.getParameter("relaterequest"))%>';
  var isrequest='<%=Util.null2String(request.getParameter("isrequest"))%>';
  var redirectUrl = "PrintRequest.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&requestid=<%=requestid%>&isprint=1&fromFlowDoc=1&urger=<%=loga_urger%>&ismonitor=<%=loga_monitor%>&relaterequest="+relaterequest+"&isrequest="+isrequest;
  <%//解决相关流程打印权限问题
...
//解决报表相关流程打印权限问题
  ...
  <%
	...
	if(isnewprint){
  %>
	redirectUrl = "/docs/docs/seconddev/docmouldprint/DocMouldPrintTab.jsp?requestid=<%=requestid%>&nodeid=<%=nodeid%>";
  <%}%>
