...
if(src.equals("save")&&iscreate.equals("1")) {//新建request且选择保存
    ...
    //加入字段信息	
    String sqltmp = " insert into bill_CptPlanMain(resourceid) values(0)";
    RecordSet.executeSql(sqltmp);
    sqltmp = " select max(id) from bill_CptPlanMain";
    RecordSet.executeSql(sqltmp);
    if(RecordSet.next())
	...
	while(RecordSet.next()){
    	...
    	if(fieldname.equals("totalamount")) continue;
		if(isoracle){     		
	...
	// add record into bill_CptPlanDetail
	...
	for(int i=0;i<rowsum;i++) {		
		...
		float number = Util.getFloatValue(request.getParameter("node_"+i+"_number"),0);
		float unitprice = Util.getFloatValue(request.getParameter("node_"+i+"_unitprice"),0);
		float amount = number * unitprice;
		String needdate = Util.null2String(request.getParameter("node_"+i+"_needdate"));
		String purpose = Util.null2String(request.getParameter("node_"+i+"_purpose"));
		String cptdesc = Util.null2String(request.getParameter("node_"+i+"_cptdesc"));
		
		if(amount!=0){
			String para = ""+billid+flag+cptid+flag+number+flag+unitprice+flag + amount+flag+needdate+flag+purpose+flag+cptdesc;
			out.print("Para::::"+para);
			RecordSet.executeProc("bill_CptPlanDetail_Insert",para);
		}
		totalamount += amount;		
	}					
	updateclause += "totalamount = "+totalamount+",";
	updateclause=updateclause.substring(0,updateclause.length()-1);
	updateclause="update bill_CptPlanMain "+updateclause+" where id = "+billid;
...
if(src.equals("save")&&iscreate.equals("0")){//处理request且选择保存logtype=1
	...
	sql = " select sum(amount) from bill_CptPlanDetail where cptplanid="+billid + " group by amount";
	RecordSet.executeSql(sql);
	float totalamount =0;
	if(RecordSet.next())
		totalamount = RecordSet.getFloat(1);
	...
	String operatortype = "";
		...
		for(i=0;i<destnodeids.size();i++){
			...
			if(where.trim().equals(""))	break;
				...
				sql="select * from bill_CptPlanMain where id="+billid+" and "+where;
				RecordSet.executeSql(sql);
				if(RecordSet.next())	break;
			}
		}
