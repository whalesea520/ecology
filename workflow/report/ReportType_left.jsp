...
<FORM NAME="SearchForm" STYLE="margin-bottom:0" action="/workflow/report/ReportManageTab.jsp" method=post target="wfmainFrame">
...
<table cellspacing="0" cellpadding="0" class="flowsTable" style="width:100%;height:100%;"  >
	<tr>
		<td class="leftTypeSearch">
			<div>
				<span class="leftType"><%=SystemEnv.getHtmlLabelName(33664,user.getLanguage()) %><span id="totalDoc"></span></span>
		...
		<td style="width:23%;" class="flowMenusTd">
			<div class="flowMenuDiv"  >
				<div style="overflow:hidden;height:300px;position:relative;" id="overFlowDiv">
					<div class="ulDiv" >
						<div id="deeptree" class="cxtree" CfgXMLSrc="/css/TreeConfig.xml" style="margin-top:0px;"/>
					</div>
				</div>
			</div>
		</td>
...
function initTree(){
	<%
	String cxtree_id = "reportType_1";
	if(!cnodeid.equals("") && !cnodeid.equals("0")) {
		cxtree_id = "reportType_"+cnodeid;
	}
	else {
		if(!Util.null2String(nodeid).equals("") && !nodeid.equals("0"))
			cxtree_id = "reportType_"+nodeid;
	}
	%>
	cxtree_id = '<%=cxtree_id%>';
	
	CXLoadTreeItem("", "/workflow/report/ReportTypeXML.jsp?init=true&subcompanyid=<%=subcompanyid%>&operatelevel=<%=operatelevel%>&nodeid=<%=nodeid%>");
