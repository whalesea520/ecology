...
//String createrdepartmentid = Util.null2String(resourceComInfo.getDepartmentID(""+userid));
...
int departmentId = Util.getIntValue(request.getParameter("departmentId"),0);
    ...
    WorkflowCodeSeqReservedManager.setYearIdDefault(String.valueOf(yearId));
    WorkflowCodeSeqReservedManager.setMonthIdDefault(String.valueOf(monthId));
    WorkflowCodeSeqReservedManager.setDateIdDefault(String.valueOf(dateId));
    WorkflowCodeSeqReservedManager.setFieldIdDefault(String.valueOf(fieldId));
    WorkflowCodeSeqReservedManager.setFieldValueDefault(String.valueOf(fieldValue));
    WorkflowCodeSeqReservedManager.setSupSubCompanyIdDefault(String.valueOf(supSubCompanyId));
    WorkflowCodeSeqReservedManager.setSubCompanyIdDefault(String.valueOf(subCompanyId));
    WorkflowCodeSeqReservedManager.setDepartmentIdDefault(String.valueOf(departmentId));
				...
				if(reservedIdBegin>0&&reservedIdBegin<=reservedIdEnd){
					for(reservedId=reservedIdBegin;reservedId<=reservedIdEnd;reservedId++){
						if(hisReservedIdList.indexOf(""+reservedId)==-1){
							...
							reservedCode=WorkflowCodeSeqReservedManager.getReservedCode(workflowId,formId,isBill,recordId,-1,reservedId,"-1");
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="WFCodeReservedForPara.jsp" method="post">
...
<%
	...
	<%	strindex++;
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(fieldId).equals("-1")){ %>
		...
		<%}else{ %>
		<select class=inputstyle name="selectField_<%=selectindex%>" oldval="<%=codeMemberValue%>" onchange="checkselect('0',this)" disabled style="width:100px!important;">
		<%
		RecordSet.executeSql(selectFieldSql);  
		while(RecordSet.next()){
		int sFieldId = RecordSet.getInt("ID");
		%>
		<option value=<%= sFieldId %> <% if(sFieldId == fieldId) { %> selected <% } %> ><%= Util.null2String(RecordSet.getString("fieldLable")) %></option>
		<%}%>
		</select>
		<%} %>
		</wea:item>
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(departmentId).equals("-1")){ %>
		...
		<%}else{ %>
			<input type=hidden name="<%=browsername%>" class=inputstyle value="<%=departmentId%>" disabled>
			<span name="<%=browsername%>span"><%=DepartmentComInfo.getDepartmentname(String.valueOf(departmentId))%></span>
		<%} %>
		</wea:item>
		<%
		deptindex++;
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(subCompanyId).equals("-1")){ %>
		<brow:browser name='<%=browsername%>' viewType="0" hasBrowser="true" hasAdd="false" browserOnClick=""
		 	   		   browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/company/SubcompanyBrowser.jsp" 
    				   isMustInput="2" isSingle="true" hasInput="true" _callback="changeclickbrow"
     				   completeUrl="javascript:getajaxurl(2)"  width="150px" browserValue="" browserSpanValue=""/>
		<%}else{ %>
			<input type=hidden name="<%=browsername%>" class=inputstyle value="<%=subCompanyId%>" disabled>
			<span name="<%=browsername%>span"><%=SubCompanyComInfo.getSubCompanyname(String.valueOf(subCompanyId))%></span>
		<%} %>
		</wea:item>
		<%
		subindex++;
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(supSubCompanyId).equals("-1")){ %>
		<brow:browser name='<%=browsername%>' viewType="0" hasBrowser="true" hasAdd="false" browserOnClick=""
		 	   		   browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/company/SubcompanyBrowser.jsp" 
    				   isMustInput="2" isSingle="true" hasInput="true" _callback="changeclickbrow"
     				   completeUrl="javascript:getajaxurl(2)"  width="150px" browserValue="" browserSpanValue=""/>
		<%}else{ %>
			<input type=hidden name="<%=browsername%>" class=inputstyle value="<%=supSubCompanyId%>" disabled>
			<span name="<%=browsername%>span"><%=SubCompanyComInfo.getSubCompanyname(String.valueOf(supSubCompanyId))%></span>
		<%} %>
		</wea:item>
		<%
		supsubindex++;
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(yearId).equals("-1")){ %>
			<input type="text" name="selectYear_<%=yindex%>" class=inputstyle value="<%=yearIdToday%>" maxlength="4" size="4"  onChange='proView()'>
		<%}else{ %>
			<input type=text name="selectYear_<%=yindex%>" class=inputstyle value="<%=yearId%>" disabled>
		<%} %>
		</wea:item>
		<%
		yindex++;
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(monthId).equals("-1")){ %>
		...
		<%}else{ %>
		<SELECT class=inputstyle name="selectMonth_<%=mindex%>" onchange="proView()" disabled>
		<%for(int q=1;q<=12;q++){%>
	     	<OPTION value=<%=q%> <%if(q==monthId){%>selected<%}%>><%=q%></OPTION>
		<%}%>
		</SELECT>
		<%} %>
		</wea:item>
		...
		<wea:item attributes='<%=attributes %>'>
		<%if(String.valueOf(dateId).equals("-1")){ %>
		...
		<%}else{ %>
		<SELECT class=inputstyle name="selectDay_<%=dindex%>" onchange="proView()" disabled>
		<%for(int w=1;w<=31;w++){%>
	    <OPTION value=<%=w%> <%if(w==dateId){%>selected<%}%>><%=w%></OPTION>
		<%}%>
		</SELECT>
		<%} %>
		</wea:item>
