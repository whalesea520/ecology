...
if(detachable==0){
    response.sendRedirect("WorkflowCodeSeqReserved_frmtwo.jsp?sqlwhere="+xssUtil.put(sqlwhere));
    return;
}
%>
<HTML><HEAD>
...
<IFRAME name=leftframe id=leftframe src="WorkflowCodeSeqReserved_left.jsp?rightStr=ReservedCode:Maintenance&sqlwhere=<%=xssUtil.put(sqlwhere)%>" width="100%" height="100%" frameborder=no scrolling=no>
...
<IFRAME name=contentframe id=contentframe src="WorkflowCodeSeqReserved_frmtwo.jsp?sqlwhere=<%=xssUtil.put(sqlwhere)%>" width="100%" height="100%" frameborder=no scrolling=yes>
<%=SystemEnv.getHtmlLabelName(15017,user.getLanguage())%></IFRAME>
</td>
</tr>
  </TBODY>
