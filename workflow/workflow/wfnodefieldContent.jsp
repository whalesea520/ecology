...
$(function() {
	$('.e8_box').Tabs({
		...
		staticOnLoad:true,
		objName:"<%=SystemEnv.getHtmlLabelName(33508,user.getLanguage())+SystemEnv.getHtmlLabelName(33473,user.getLanguage())%>"
	});
	attachUrl();
});
...
{
	$("[name='tabcontentframe']").attr("src","/workflow/workflow/addwfnodefield.jsp?design=<%=design %>&ajax=1&wfid=<%=wfid%>&nodeid=<%=nodeid%>");
}
