...
<form name="formWorkflowCodeSeqReserved" method="post" action="/workflow/workflow/WorkflowCodeSeqReserved_right.jsp">
...
<%
        if(workflowId>0){
%>
		    <iframe src="/workflow/workflow/WorkflowCodeSeqReservedSet.jsp?subCompanyId=<%=subCompanyId%>&isFromSubCompanyTree=true&workflowId=<%=workflowId%>&formId=<%=formId%>&isBill=<%=isBill%>" ID="iframeWorkflowCodeSeqReserved" name="iframeWorkflowCodeSeqReserved" frameborder="0" style="width:100%;height:100%;border-right:0px solid #879293;border-bottom:0px solid #879293;border-left:0px solid #879293;padding:10px;padding-right:0" scrolling="auto"></iframe>
<%
        }
    }else{
%>
...
Sub onShowWorkFlowSerach(inputname, spanname)
	...
	If (Not IsEmpty(retValue)) Then
		If retValue(0) <> "0" Then
            ...
            document.formWorkflowCodeSeqReserved.action="/workflow/workflow/WorkflowCodeSeqReserved_right.jsp"
			document.formWorkflowCodeSeqReserved.submit()
