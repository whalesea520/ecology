...
<BODY onload="initTree()" scroll=no>
	<FORM NAME=SearchForm STYLE="margin-bottom:0" action=<%=(isWorkflowDoc.equals("1")?"wfmanage_frm.jsp":"wfmanage_frm.jsp")%> method=post target=<%=(isWorkflowDoc.equals("1")?"contentframe":"contentframe")%> >
...
function initTree(){
CXLoadTreeItem("", "/frameleftXML.jsp?isWorkflowDoc=<%=isWorkflowDoc %>&rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
