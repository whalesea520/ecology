...
List nodesList = Util.TokenizerString(wfid.split("_")[2],",");
%>
<BODY>
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(34102, user.getLanguage())%>"/>
</jsp:include>
...
<TR class=header>
<TH width=30%>
<input type="checkbox" id="checkAllBox" value="1" name="checkbox" onclick='checkAll()'>
</TH>
...
<TR class=DataDark>
	...
	%>
	<TD><input type="checkbox" value="1" name="checkbox_<%=rowNum%>" id="checkbox_<%=rowNum%>" <%=nodesList.contains(tempPrintNodes)?"checked":""%>></TD>	
...
function checkAll(){
	var rowNum = document.getElementById("rowNum").value;
	var checkAll = $('#checkAllBox').attr('checked');
	for(var i = 0; i < rowNum; i++){
		if ( checkAll ){
			jQuery('#checkbox_'+i).attr('checked', 'checked');
			jQuery('#checkbox_'+i).next().attr('class', 'jNiceCheckbox jNiceChecked');
		}else{
			jQuery('#checkbox_'+i).removeAttr('checked');
			jQuery('#checkbox_'+i).next().attr('class', 'jNiceCheckbox');
		}
	}
}
