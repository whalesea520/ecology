...
<%
    ...
    String secName = Util.null2String(secCategoryComInfo.getSecCategoryname(seccategoryid));
...
<wea:layout type="2col" attributes="{'expandAllGroup':'true'}">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(19853,user.getLanguage())%>'>
		<%if(!type.equals("0")){ %>
			...
			<wea:item><%=SystemEnv.getHtmlLabelName(19369,user.getLanguage())%></wea:item>
			<wea:item>
				<select class=inputstyle id="docMouldID" name="docMouldID" onchange="changeBookMark(this.value)">
					<option value="-1"></option>
				<%
					RecordSet.executeSql("select * from DocMould where id in ( " + mouldId+")");
					while(RecordSet.next()){
				    	docMouldID = RecordSet.getString("ID");										            
				    	docMouldName = RecordSet.getString("mouldName");			
				  %>
					<option value="<%= docMouldID %>" <% if(!"".equals(parameterMouldID) && null != parameterMouldID) { 
					if(docMouldID.equals(parameterMouldID)) { finalDocMouldID = parameterMouldID; %> selected <% }} else if (!"-1".equals(docMouldIDEdit)) { if(docMouldID.equals(docMouldIDEdit)) { finalDocMouldID = docMouldIDEdit; %> selected <% }} %>><%= docMouldName %></option>
				<%
				    }			    
				%>
				</select>		
		...
		<%}else{ 
			RecordSet.executeSql("SELECT mouldName FROM DocMould WHERE id = " + mouldId);
		%>
			<wea:item><%=SystemEnv.getHtmlLabelName(25126,user.getLanguage())%></wea:item>
			<wea:item>
...
function changeBookMark(value){
	...
	jQuery.ajax({
		url:"/workflow/workflow/chooseDisplayBookMarkForm.jsp?selectItemID=<%=selectItemID%>&formID=<%=formID%>&flowID=<%=flowID%>&mouldId="+value,
		type:"post",
		dataType:"html",
