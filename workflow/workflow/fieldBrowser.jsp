...
<input type="hidden" id="htmltype" name="htmltype" value="<%=htmltype%>">
<input type="hidden" id="type" name="type" value="<%=type%>">
<input type="hidden" id="fielddbtype" name="fielddbtype" value="<%=fielddbtype%>">
...
<wea:layout type="4col" attributes="{'cw1':'20%','cw2':'30%','cw3':'20%','cw4':'30%'}">
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
		...
		<wea:item>
			<select id="tabletype" name="tabletype">
				<option value="" <%if(tabletype==-1){%>selected<%}%>></option>
				<option value=0 <%if(tabletype==0){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(21778,user.getLanguage())%></option>
				<option value=1 <%if(tabletype==1){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(19325,user.getLanguage())%></option>
			</select>		
		</wea:item>
	</wea:group>
	...
	<wea:group context="" attributes="{'groupDisplay':'none'}">
		<wea:item attributes="{'isTableList':'true','colspan':'full'}">
			...
			<% 
				String tableString=""+
				   "<table pageId=\""+PageIdConst.WF_WORKFLOW_FIELDBROWSER+"\" datasource=\"weaver.workflow.workflow.WfDataSource.getWorkflowTableTriggerField\" sourceparams=\"htmltype:"+htmltype+"+type:"+type+"+isbill:"+isbill+"+isdetail:"+isdetail+"+wfid:"+wfid+"+fieldname:"+fieldname+"+tabletype:"+tabletype+"+fielddbtype:"+fielddbtype+"+sqlwhere:"+Util.toHtmlForSplitPage(sqlwhere)+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.WF_WORKFLOW_FIELDBROWSER,user.getUID())+"\" tabletype=\"none\">"+
