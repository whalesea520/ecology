...
<jsp:useBean id="secCategoryComInfo" class="weaver.docs.category.SecCategoryComInfo" scope="page" />
	...
	<wea:layout type="table" needImportDefaultJsAndCss="false" attributes="{'cols':'3','cws':'20%,50%,30%'}">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			...
			<wea:item type="thead"><%=SystemEnv.getHtmlLabelNames("25126",user.getLanguage())%></wea:item>
				...
				RecordSet.executeSql("select ID, selectValue, selectName, docPath, docCategory,isAccordToSubCom from workflow_SelectItem where fieldid = " + fieldId + " and isBill = "+isbill+" and (cancel is null or cancel<>'1') order by listOrder asc ");
				 ...
				 while(RecordSet.next()){
					...
					String isAccordToSubCom = Util.null2String(RecordSet.getString("isAccordToSubCom"));
				 	...
				 	String sql = "select 1 from workflow_mould where mouldType=1 and workflowid = "+workflowId+"  and seccategory="+secCategory;
					rs.executeSql(sql);
					if(!rs.next()){
						//将该子目录关联的文档插入到中间表中
						rs.executeSql("SELECT docMould.ID, docMould.mouldName FROM DocSecCategoryMould docSecCategoryMould, DocMould docMould WHERE docSecCategoryMould.mouldID = docMould.ID AND docSecCategoryMould.mouldType in(3,7) AND docSecCategoryMould.mouldBind = 2 AND docSecCategoryMould.secCategoryID = " + secCategory);
						if(rs.next()){
							rs.beforFirst();
							while(rs.next()){
								rs.executeSql("insert into workflow_mould(workflowid,mouldId,mouldType,visible,seccategory) values("+workflowId+","+rs.getString("ID")+",1,1,"+secCategory+")");
							}
						}else{
							rs.executeSql("SELECT docMould.ID, docMould.mouldName FROM DocSecCategoryMould docSecCategoryMould, DocMould docMould WHERE docSecCategoryMould.mouldID = docMould.ID AND docSecCategoryMould.mouldType in(3,7) AND docSecCategoryMould.mouldBind = 1 AND docSecCategoryMould.secCategoryID = " + secCategory);
							while(rs.next()){
								rs.executeSql("insert into workflow_mould(workflowid,mouldId,mouldType,visible,seccategory) values("+workflowId+","+rs.getString("ID")+",1,1,"+secCategory+")");
							}
						}
					...
					if(rs.next()){
						...
						rs.executeSql("SELECT docMould.ID, docMould.mouldName FROM DocSecCategoryMould docSecCategoryMould, DocMould docMould WHERE docMould.ID in (select mouldid from workflow_mould where visible=1 and mouldType=1 and seccategory="+secCategory+" and workflowid = "+workflowId+") and docSecCategoryMould.mouldID = docMould.ID AND docSecCategoryMould.mouldType in(3,7) AND docSecCategoryMould.mouldBind = 1 AND docSecCategoryMould.secCategoryID = " + secCategory);
					}
					String mouldIds = "";
					String mouldNames = "";
					while(rs.next()){
					...
					String completeUrl="/data.jsp?type=-99996&mouldType=2&isWorkflowDoc=1"; 
					String browserUrl="/systeminfo/BrowserMain.jsp?url=/docs/mould/DocMouldMutiBrowser.jsp?isWorkflowDoc=1&selectids=";
			...
			%>
					...
					<%=SystemEnv.getHtmlLabelName(22878,user.getLanguage())%>
						...
						<span>
							<brow:browser viewType="0" index='<%=""+i %>' name="multimouldids" browserValue='<%=mouldIds %>' 
								...
								dialogWidth="1100px"
								dialogHeight="1000px"
				                completeUrl='<%=completeUrl %>'  temptitle='<%= SystemEnv.getHtmlLabelName(25126,user.getLanguage())%>'
					...
					<wea:item>
						...
						<%}else{
						%>
						<a href="javascript:setMouldBookMarkMulti(1,<%=Util.null2String(RecordSet.getString("selectValue"))%>,<%=secCategory %>,<%=i %>);"><%=SystemEnv.getHtmlLabelNames("33338",user.getLanguage())%></a>
