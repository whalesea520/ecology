...
<%
    ...
    if(operateLevel < 1) {
        ...
        RecordSet.executeSql("select * from workflow_texttopdfconfig where id="+id);
      	if(RecordSet.next()){
		    ...
		    operationtype=Util.getIntValue(RecordSet.getString("operationtype"),0);
            checktype=Util.null2String(RecordSet.getString("checktype"));
			String[] checktypes = Util.TokenizerString2(checktype, "_");
            checktypea = Util.getIntValue(checktypes[0],0);
            checktypeb = Util.getIntValue(checktypes[1],0);
	        filetopdffile =Util.getIntValue(RecordSet.getString("filetopdffile"),0);
	        filetopdf =Util.getIntValue(RecordSet.getString("filetopdf"),0);
            filemaxsize=Util.null2String(RecordSet.getString("filemaxsize"));
...
<FORM name="formtopdf" method="post" action="TextToPdfOperation.jsp" >
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361, user.getLanguage())%>'>
        ...
        <wea:item>
		 <SELECT class=inputstyle name="topdfnodeid" disabled>
                                            ...
                                            %>
                                                <option value=<%= tempNodeId %> <%if(tempNodeId.equals(""+topdfnodeid)){%>selected<%}%> ><%= RecordSet.getString("nodeName") %></option>
                                            <%
                                                }
                                            %>
                                            </select>
    ...
    <option value=0 <%if(operationtype==0){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(126333,user.getLanguage())%></option>
    <option value=1 <%if(operationtype==1){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(126334,user.getLanguage())%></option>
    </select>&nbsp;
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("126326,68", user.getLanguage())%>'>
		...
		<wea:item>
		<INPUT class=InputStyle type=checkbox   <%if(checktypea==1){%>checked<%}%>  value=1 id=checktypea  name="checktypea" onclick="showarea(1)" ><%=SystemEnv.getHtmlLabelNames("126326,21819", user.getLanguage())%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<INPUT class=InputStyle  type=checkbox  <%if(checktypeb==1){%>checked<%}%>  value=1 id=checktypeb  name="checktypeb" onclick="showarea(2)"  ><%=SystemEnv.getHtmlLabelName(126344, user.getLanguage())%> <span class="e8tips" title="<%=SystemEnv.getHtmlLabelName(126352,user.getLanguage()) %>"><img src="/images/tooltip_wev8.png" align="absMiddle"/></span>
		</wea:item>
    ...
    <option value=0 <%if(catalogtype2.equals("0")){%> selected <%}%> ><%=SystemEnv.getHtmlLabelName(19213,user.getLanguage())%></option>
    <option value=1 <%if(catalogtype2.equals("1")){%> selected <%}%> ><%=SystemEnv.getHtmlLabelName(19214,user.getLanguage())%></option>
    </select>&nbsp;
<%
...
%>
 <span name=checktype1 id=checktype1  <%if(catalogtype2 .equals("0")||catalogtype2 .equals("")){%>style="display:none"<%}%>>
...
%>
        <option value=<%= tempFieldId %> <% if(tempFieldId == selectcatalog2) { %> selected <% } %> ><%= RecordSet.getString("fieldLable") %></option>
    <%
  ...
  <span name=checktype0 id=checktype0  <%if(catalogtype2 .equals("1")){%>style="display:none"<%}%> >
	...
	<brow:browser  name="browser1" viewType="0" hasBrowser="true" hasAdd="false" 
			               browserUrl="#" getBrowserUrlFn="getBrowserUrlFn"  isMustInput="2" isSingle="true" hasInput="true"
			                completeUrl="/data.jsp?type=categoryBrowser" _callback="onShowCatalogData2" width="300px" browserValue="<%=docPath%>" browserSpanValue="<%=docPath%>" />	
	</span>
	</span>
		</wea:item>
		...
		<wea:item attributes="{'samePair':'1seclevel_td','display':''}">
			<SELECT  name="pdffieldid" id="pdffieldid"  onchange="checkSelect1(1)">
                                            ...
                                            %>
                                                <OPTION value=<%= formDictID %> <% if(formDictID.equals(""+pdffieldid)) { %> selected <% } %>  ><%= RecordSet.getString("fieldLable") %></OPTION>
                                            <%
                                              }
                                            %>
                                            </SELECT><span id="pdffieldidspan"><% if(pdffieldid==0) { %><IMG src='/images/BacoError_wev8.gif' align=absMiddle> <% } %></span>
		...
		<wea:item attributes="{'samePair':'1seclevel_td','display':''}">
			<SELECT  name="pdfdocstatus" id="pdfdocstatus">  
			                                    <OPTION value=9 <% if(pdfdocstatus==9) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(21556,user.getLanguage())%></OPTION>
                                                <OPTION value=0 <% if(pdfdocstatus==0) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(220,user.getLanguage())%></OPTION>
                                                <OPTION value=1 <% if(pdfdocstatus==1) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(19563,user.getLanguage())%></OPTION>
                                                <OPTION value=3 <% if(pdfdocstatus==3) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(359,user.getLanguage())%></OPTION>
                                                <OPTION value=5 <% if(pdfdocstatus==5) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(251,user.getLanguage())%></OPTION>
                                                <OPTION value=6 <% if(pdfdocstatus==6) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(19564,user.getLanguage())%></OPTION>
                                                <OPTION value=7 <% if(pdfdocstatus==7) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(15750,user.getLanguage())%></OPTION>
                                                <OPTION value=8 <% if(pdfdocstatus==8) { %> selected <% } %> ><%=SystemEnv.getHtmlLabelName(15358,user.getLanguage())%></OPTION>
												<OPTION value=-1 <% if(pdfdocstatus==-1) { %> selected <% } %>  ></OPTION>
		...
		<wea:item attributes="{'samePair':'seclevel_td1','display':'none'}">
			<select  id=decryptcatalogtype2   name=decryptcatalogtype2 onchange="switchCataLogType2(this.value,'#decryptchecktype0','#decryptchecktype1')"  style="float: left;">
             <option value=0 <%if(decryptcatalogtype2.equals("0")){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(19213,user.getLanguage())%></option>
             <option value=1 <%if(decryptcatalogtype2.equals("1")){%> selected <%}%> ><%=SystemEnv.getHtmlLabelName(19214,user.getLanguage())%></option>
          </select>

 <span name=decryptchecktype1 id=decryptchecktype1  <%if(decryptcatalogtype2 .equals("0")||decryptcatalogtype2 .equals("")){%>style="display:none"<%}%>>
	...
	while(RecordSet.next()){
		tempFieldId = RecordSet.getInt("ID");
%>
        <option value=<%= tempFieldId %> <% if(tempFieldId == decryptselectcatalog2) { %> selected <% } %>  ><%= RecordSet.getString("fieldLable") %></option>
    <%
  ...
  <span name=decryptchecktype0 id=decryptchecktype0 <%if(decryptcatalogtype2 .equals("1")){%>style="display:none"<%}%> >
		
    <span name=decryptmypath2 id=decryptmypath2 >
	
	<brow:browser  name="browser2" viewType="0" hasBrowser="true" hasAdd="false" 
			               browserUrl="#" getBrowserUrlFn="getBrowserUrlFn"  isMustInput="2" isSingle="true" hasInput="true"
			                completeUrl="/data.jsp?type=categoryBrowser" _callback="onShowCatalogData2" width="300px" browserValue="<%=decryptdocPath%>" browserSpanValue="<%=decryptdocPath%>" />
	</span>
  </span>
		</wea:item>
			...
			<SELECT  name="decryptpdffieldid" id="decryptpdffieldid" onchange="checkSelect1(2)"> 
                                            ...
                                            <%
                                              ...
                                              while(RecordSet.next()) {
                                              	String formDictID = RecordSet.getString("ID");
                                            %>
                                                <OPTION value=<%= formDictID %> <% if(formDictID.equals(""+decryptpdffieldid)) { %> selected <% } %>  ><%= RecordSet.getString("fieldLable") %></OPTION>
                                            <%
                                              }
                                            %>
                                            </SELECT><span id=decryptpdffieldidspan><% if(decryptpdffieldid==0) { %><IMG src='/images/BacoError_wev8.gif' align=absMiddle> <% } %></span>
			<span class="e8tips" title="<%=SystemEnv.getHtmlLabelName(128752,user.getLanguage()) %>"><img src="/images/tooltip_wev8.png" align="absMiddle"/></span>
		</wea:item>
		...
		<wea:item attributes="{'samePair':'seclevel_td1','display':'none'}">
			<SELECT  name="decryptpdfdocstatus" id="decryptpdfdocstatus">  
			                                    <OPTION value=9 <% if(decryptpdfdocstatus==9) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(21556,user.getLanguage())%></OPTION>
                                                <OPTION value=0 <% if(decryptpdfdocstatus==0) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(220,user.getLanguage())%></OPTION>
                                                <OPTION value=1 <% if(decryptpdfdocstatus==1) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(19563,user.getLanguage())%></OPTION>
                                                <OPTION value=3 <% if(decryptpdfdocstatus==3) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(359,user.getLanguage())%></OPTION>
                                                <OPTION value=5 <% if(decryptpdfdocstatus==5) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(251,user.getLanguage())%></OPTION>
                                                <OPTION value=6 <% if(decryptpdfdocstatus==6) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(19564,user.getLanguage())%></OPTION>
                                                <OPTION value=7 <% if(decryptpdfdocstatus==7) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(15750,user.getLanguage())%></OPTION>
                                                <OPTION value=8 <% if(decryptpdfdocstatus==8) { %> selected <% } %>  ><%=SystemEnv.getHtmlLabelName(15358,user.getLanguage())%></OPTION>
												<OPTION value=-1 <% if(decryptpdfdocstatus==-1) { %> selected <% } %>  ></OPTION>
       ...
       <wea:item><%=SystemEnv.getHtmlLabelName(126339, user.getLanguage())%></wea:item>
		<wea:item>
		<INPUT class=InputStyle tzCheckbox="true" type=checkbox  value=1 name="filetopdffile"  <% if(filetopdffile==1) { %> checked <% } %> >
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(126340, user.getLanguage())%></wea:item>
		<wea:item>
		<INPUT class=InputStyle tzCheckbox="true" type=checkbox  value=1 name="filetopdf"  <% if(filetopdf==1) { %> checked <% } %>   onclick="showfiletopdfdec()"> <span class="e8tips" title="<%=SystemEnv.getHtmlLabelName(126347,user.getLanguage()) %>"><img src="/images/tooltip_wev8.png" align="absMiddle"/></span> <span id=filetopdfdec <% if(filetopdf!=1) { %> style="display:none" <% } %>   ><INPUT class=InputStyle type=text  value="<%=filemaxsize%>" name="filemaxsize" style="width:50px" ><%=SystemEnv.getHtmlLabelName(126342, user.getLanguage())%></sapn>
		</wea:item>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(85,user.getLanguage())%>'>
		<wea:item>
			...
			<p class="noticeinfo">2、<%=SystemEnv.getHtmlLabelName(128823,user.getLanguage()) %>。</p>
                    ...
                    <INPUT type=hidden id='pdfsavesecid' name='pdfsavesecid' value="<%=pdfsavesecid%>">
                    <INPUT type=hidden id='decryptpdfsavesecid' name='decryptpdfsavesecid' value="<%=decryptpdfsavesecid%>">    
</FORM>
</BODY>
</HTML>
<script>
