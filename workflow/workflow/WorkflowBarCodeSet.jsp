...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(18624,user.getLanguage())%>'>
		<wea:item><%=SystemEnv.getHtmlLabelName(18624,user.getLanguage())%></wea:item>
		<wea:item>
			<input class=inputstyle type="checkbox" tzCheckbox="true" name="isUseBarCodeSet" value="1" onclick="showContentBarCodeSet()"  <%if("1".equals(isUse)) {%> checked <% } %>>		
		</wea:item>
	</wea:group>
<%
...
function showContentBarCodeSet(){
    if($GetEle("isUseBarCodeSet").checked){
    	...
    	hideGroup("contentDivBarCodeSet");
    }
}
