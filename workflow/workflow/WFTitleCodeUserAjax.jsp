...
if(user == null){
	...
	try{
		...
		if(valuestr.equals("")){
			valuestr=""+user.getUID();	
		} 
    	RecordSet.executeSql("select lastname,workcode from hrmresource where id="+valuestr);
		if(RecordSet.next()){
			jc=Util.null2String(RecordSet.getString("lastname"));
			bh=Util.null2String(RecordSet.getString("workcode"));
		}
    	
		if(valuetype.equals("0")){//姓名
			retustr=jc;
    	}if(valuetype.equals("1")){//编号	    		
    		retustr=bh;
    	} 
