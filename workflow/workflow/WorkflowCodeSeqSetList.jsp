	...
	if("1".equals(struSeqAlone)&&"3".equals(struSeqSelect)){//按部门做机构单独流水
	    ...
	    if(subCompanyIds.equals("")){
			...
			if(isFromSubCompanyTree.equals("true")){
				subCompanyIdArray=CheckSubCompanyRight.getSubComByUserRightId(user.getUID(),"StartCode:Maintenance");
			}else{
...
if("1".equals(dateSeqAlone)||"1".equals(fieldSequenceAlone)||"1".equals(struSeqAlone)){
	RCMenu += "{"+SystemEnv.getHtmlLabelName(197,user.getLanguage())+",javascript:onSearchCodeSeqSet(this),_self} " ;
    RCMenuHeight += RCMenuHeightStep;
}

if(canEdit){
    RCMenu += "{"+SystemEnv.getHtmlLabelName(86,user.getLanguage())+",javascript:onSaveCodeSeqSet(this),_self} " ;
    RCMenuHeight += RCMenuHeightStep;
}
...
<form id="formCodeSeqSet" name="formCodeSeqSet" method=post action="WorkflowCodeSeqSetOperation.jsp?isfrom=new" >
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<%if(canEdit){ %>
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(86,user.getLanguage()) %>" id="zd_btn_cancle"  class="e8_btn_top" onclick="onSaveCodeSeqSet(this)">
		<%} %>
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
		...
		%>
<%
if( ("1".equals(dateSeqAlone) && ("1".equals(dateSeqSelect) || "2".equals(dateSeqSelect) || "3".equals(dateSeqSelect))) || (fieldId>0&&"1".equals(fieldSequenceAlone)) || ("1".equals(struSeqAlone))){
%>

<wea:layout type="2Col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(404,user.getLanguage())+SystemEnv.getHtmlLabelName(1321,user.getLanguage())%>'>
<%if("1".equals(dateSeqAlone)&&"1".equals(dateSeqSelect)){ %>
<wea:item><%=SystemEnv.getHtmlLabelName(445,user.getLanguage())%></wea:item>
<wea:item>
	  <input type="text" name="yearIdBeginCodeSeqSet" value="<%=yearIdBeginCodeSeqSet%>" maxlength="4" size="4" style="width:75px!important;" onKeyPress="ItemCount_KeyPress()" onChange='checknumber("yearIdBeginCodeSeqSet");checkinput("yearIdBeginCodeSeqSet","yearIdBeginCodeSeqSetImage")'>
      <SPAN id=yearIdBeginCodeSeqSetImage></SPAN>
	 -
	  <input type="text" name="yearIdEndCodeSeqSet"   value="<%=yearIdEndCodeSeqSet%>" maxlength="4" size="4" style="width:75px!important;" onKeyPress="ItemCount_KeyPress()" onChange='checknumber("yearIdEndCodeSeqSet");checkinput("yearIdEndCodeSeqSet","yearIdEndCodeSeqSetImage")'>
...
%>
 <wea:item>
<%=SystemEnv.getHtmlLabelName(22780,user.getLanguage())%></wea:item>
<wea:item>
...
<wea:group context="" attributes="{'groupDisplay':'none'}">
	<wea:item type="toolbar">
		<input type="submit" value="<%=SystemEnv.getHtmlLabelName(197,user.getLanguage())%>" class="e8_btn_submit" onclick="onSearchCodeSeqSet(this);"/>
...
<%if(canEdit){%>
		<input class=Inputstyle type="text" name="codeSeq<%=rowNum%>_sequenceId" value="<%=tempSequenceId%>" onKeyPress="ItemCount_KeyPress()" onChange='checknumber("codeSeq<%=rowNum%>_sequenceId")'>
<%}else{%>
		<%=tempSequenceId%>
<%}%>
	</td>
...
--%>
<input type="hidden" value="<%=rowNum%>" name="rowNumCodeSeqSet">
...
function onSearchCodeSeqSet(obj) {
	if(checkValueCodeSeqSet()){
		obj.disabled = true;
		document.formCodeSeqSet.action="WorkflowCodeSeqSet.jsp" ;
...
function onSaveCodeSeqSet(obj) {
	if(checkValueCodeSeqSet()){
		obj.disabled = true;
		document.formCodeSeqSet.action="WorkflowCodeSeqSetOperation.jsp" ;
		document.formCodeSeqSet.submit();
	}
}
