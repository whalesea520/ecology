						...
						<tr>
							<td valign="top">
								...
								<FORM name="frmAdd" method="post"
									action="/homepage/maint/LoginMaintOperate.jsp?opt=<%=opt%>">
									...
									<TABLE class=viewform cellspacing=1>
										<TR>
											<td colspan=2><%=SystemEnv.getHtmlLabelName(19484, user.getLanguage())%>:
											</td>
										</TR>
										<TR style="height:1px;">
										...
										<TR>
											...
											<TD class=field>
												<SELECT NAME="seleStyleid" style="width: 38%">
													<%
														esc.setTofirstRow();
														while (esc.next())
														{
										...
										<TR>
											...
											<TD class=field>
												<%
													...
													rs.executeSql("select areaflag,areasize from hplayout where hpid=" + hpid + " and userid=" + userid + " and usertype=0");
													while (rs.next())
													{
												...
												%>
												<SELECT NAME="seleLayoutid" style="width: 38%"
													onchange="onLayoutidChanage(this)">
													<%
															...
															<%
																...
																if (!"".equals(strArea))
																{
																	...
																	for (int i = 0; i < tempList.size(); i++)
																	{
																		...
																		out.println(tempArea + SystemEnv.getHtmlLabelName(15114, user.getLanguage())
																				+ SystemEnv.getHtmlLabelName(203, user.getLanguage()) + ":"
																				+ "<input type=text size=5 class=inputStyle onkeypress=\"ItemCount_KeyPressNoHen()\" name='txtArea_" + tempArea
																				+ "' value='" + tempSizeValue + "'>%<br>");
																	}
																}
															%>
										...
										<TR id="trSetLayoutLine" style="display:<%=display%>;height:1px;">		
											<td colspan=2 class=line></td>
										</TR>
										<TR>
											<td colspan=2>
												<span id=spanLayoutdesc><%=strLayoutdesc%></span>
											</td>
										</TR>
									</TABLE>
								</FORM>
