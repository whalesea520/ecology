...
(function(window){
	...
	var sendNwgui = require('nw.gui');
	var pcGLOBAL_INFOS=sendNwgui.global.GLOBAL_INFOS;
	...
	//var e_dialog = require('nw-dialog');
	//e_dialog.setContext(document) // work in client
    var e_shell = sendNwgui.Shell;
    var currentWindow = sendNwgui.Window.get();
    ...
    var PcSendDirUtils = {
        ...
        _savToPc : function(){
            ...
            pcGLOBAL_INFOS.dirServerConifg = config;
        ...
        startServer : function(){
            ...
            // 注意：页面刷新会自动停止服务
            try {
				this.bindListener(_portArray);
			} catch(e) {
				console.error(e);
			}
        },
		bindListener: function(_portArray){
			var _this = this;
            ...
            $.post('/social/im/SocialIMOperation.jsp', { operation : 'isPcClientOnline', checkId : targetId }, function(data){
                ...
                if(data == 1) {
                    ...
                    if(alreadyPath && typeof alreadyPath === 'string') {
                        ...
                        nwdialog.folderBrowserDialog(function(choosed){
                            if(choosed) {
                                var srcPath = choosed;
    ...
    DirSender.prototype.startSend = function(callback) {
        ...
        if(checkResult.success) {
            ...
            var extra = '{"msg_id":"'+this.msgId+'","senderid":'+M_USERID+',"receiverids":"'+this.targetId+'","sendTime":'+sendTime+'}';
            // content这种写法为了兼容后台发送消息的格式
