(function(global, undefined) {
    ...
    var MD5 = function(e) {
        function n(d) {
            for (var a = 0, b = ""; 3 >= a; a++) {
                b += "0123456789abcdef".charAt(d >> 8 * a + 4 & 15) + "0123456789abcdef".charAt(d >> 8 * a & 15)
            }
            return b
        }
        ...
        e = function(a) {
            for (var b = (a.length + 8 >> 6) + 1, d = Array(16 * b), c = 0; c < 16 * b; c++) {
                d[c] = 0
            }
            for (c = 0; c < a.length; c++) {
                d[c >> 2] |= a.charCodeAt(c) << c % 4 * 8
            }
            d[c >> 2] |= 128 << c % 4 * 8;
            d[16 * b - 2] = 8 * a.length;
            return d
        }(e);
        for (var d = 1732584193, a = -271733879, b = -1732584194, c = 271733878, f = 0; f < e.length; f += 16) {
            var p = d,
                q = a,
                r = b,
                t = c,
    ...
    var io = {
        ...
        func = function() {
            ...
            if ("WebSocket" in global && "ArrayBuffer" in global && WebSocket.prototype.CLOSED === 3 && !global.WEB_SOCKET_FORCE_FLASH && !global['RongOpts']["WEB_XHR_POLLING"]) {
                script.src = global['RongOpts']['PB_URL'];
            } else {
                if (!/opera/i.test(navigator.userAgent) && !global['RongOpts']["WEB_XHR_POLLING"] && (function() {
                    ...
                    script.src = global['RongOpts']['SWF_URL'];
                } else {
                    io._TransportType = "xhr-polling";
                    script.src = global['RongOpts']['XHR_URL'];
                }
            }
    ...
    var binaryHelper = global.RongBinaryHelper = {
        ...
        readUTF: function(_arr) {
            ...
            for (var i = 0, len = _arr.length; i < len; i++) {
                ...
                var one = temp.toString(2),
                    v = one.match(/^1+?(?=0)/);
                if (v && one.length == 8) {
                    var bytesLength = v[0].length,
                        store = one.slice(7 - bytesLength);
    ...
    (function() {
        ...
        Transport.prototype._onData = function(data, header) {
            ...
            if (!Client.Endpoint.userId) {
                Client.Endpoint.userId = val.userId
            }
            if (header) {
    ...
    (function() {
        ...
        WS.prototype.connect = function(url) {
            var self = this;
            this.socket = new WebSocket(global['RongOpts']['WS_PROTOCOL'] + "://" + url);
            ...
            this.socket.onerror = function(evt) {
                if (!evt.data) {
                    bridge._client.reconnectObj.onError(RongIMClient.ConnectErrorStatus.setValue(12));
                    _ConnectionStatusListener.onChanged(RongIMClient.ConnectErrorStatus.setValue(12));
    ...
    (function() {
        ...
        XHR.prototype.disconnect = function(status) {
        ...
        XHR.prototype._request = function(url, method, multipart) {
            ...
            req.open(method || "GET", global['RongOpts']['PROTOCOL'] + "://" + url + "&pid=" + encodeURIComponent(pageId));
    ...
    (function() {
        ...
        XHRPolling.prototype._get = function(symbol, arg) {
            var self = this;
            this._xhr = this._request(symbol + "&timestrap=" + encodeURIComponent(new Date().getTime() + Math.random() + ""), "GET");
    ...
    (function() {
        ...
        Socket.prototype.disconnect = function(callback) {
            if (callback) {
                ...
                if (callback.value && callback.value == 1) {
                    RongIMClient.otherDevice = true
                }
            }
    ...
    function MessageHandler(client) {
        ...
        this.onReceived = function(msg) {
            ...
            if (message.getObjectName() != "RC:TypSts" && message.getObjectName() != "RC:ReadNtf") {
			...
			if(window.localStorage){
			   localStorage.setItem('RongLastestReceviedMsg',JSON.stringify(message));
			}
            onReceived(message)
        };
    ...
    function messageParser(entity, onReceived) {
        ...
        try {
            if (global['RongOpts']["WEB_XHR_POLLING"]) {
        ...
        if (entity.fromUserId == bridge._client.userId) {
            ...
            message.setTargetId(/^[234]$/.test(entity.type || entity.getType()) ? entity.groupId : message.getSenderUserId(bridge._client.userId))
        } if (entity.direction == 1) {
            message.setMessageDirection(RongIMClient.MessageDirection.SEND);
            message.setSenderUserId(bridge._client.userId)
        } else {
        ...
        message.setMessageId(message.getConversationType() + "_" + ~~(Math.random() * 16777215));
        message.setReceivedStatus(new RongIMClient.ReceivedStatus());
        if (RongIMClient.getInstance().getDelaTime() > message.getSentTime()) {
            message.setIsOffLineMessage(true);
        }
        return message
    }
        ...
        this.process = function(status, userId, timestamp) {
            ...
            if (status == 0) {
                ...
                if (self.reconnectObj.onSuccess) {
                    ...
                    if (timestamp > 0) {
                        RongIMClient.getInstance().deltaTime = timestamp
                    } else {
    ...
    function Client(_to, _ap) {
        ...
        this.token = _to;
        this.sdkVer = "1.1.2";
        this.apiVer = Math.floor(Math.random() * 1000000);
        this.channel = null;
        this.handler = null;
        this.userId = "";
        this.reconnectObj = {};
        ...
        function invoke() {
            ...
            if (temp.type != 2) {
                ...
                modules.setIsKeeping(true);
                if (!RongIMClient.keepMsg) {
                    modules.setIsKeeping(false)
                }
        ...
        this.syncTime = function(_type, pullTime) {
            ...
            if (SyncTimeQueue.length == 1 && SyncTimeQueue.state == "complete") {
                var bol = false;
                if (RongIMClient.firstSync) {
                    bol = true;
                    RongIMClient.firstSync = false
                } else {
                    bol = false
    ...
    Client.getServerEndpoint = function(_token, _appId, _onsuccess, _onerror, unignore) {
        ...
        var Url = {
                "navUrl-Release": global['RongOpts']['NAVI_URL']
            },
    ...
    var bridge = function(_appkey, _token, _callback) {
        ...
        this.querySendBoxMsg = function(content, targetId, callback, pbname) {
            bridge._client.queryMessage("pullMsg", content, targetId, Qos.AT_LEAST_ONCE, callback, pbname)
        };
    ...
    function RongIMClient(_appkey) {
        ...
        this.getCurrentUserInfo = function(callback) {
            check(["object"]);
            this.getUserInfo(bridge._client.userId, {
        ...
        this.sendMessage = function(h, v, e, c, u) {
            ...
            if (!i.getMessageId()) {
                i.setMessageId(h + "_" + ~~(Math.random() * 16777215))
            }
        ...
        this.quitChatRoom = function(Id, callback) {
            ...
            var e = new Modules.ChrmInput();
            e.setNothing(1);
            a.queryMsg(17, io.util.arrayFrom(e.toArrayBuffer()), Id, {
        ...
        this.setDiscussionName = function(_discussionId, _name, _callback) {
            ...
            a.queryMsg(12, io.util.arrayFrom(modules.toArrayBuffer()), _discussionId, {
                onSuccess: function(arg) {
                    setTimeout(function() {
                        _callback.onSuccess(arg)
                    })
                },
                onError: function(arg) {
                    setTimeout(function() {
                        _callback.onError(arg)
                    })
                }
            })
        };
        this.removeMemberFromDiscussion = function(_disussionId, _userId, _callback) {
            ...
            a.queryMsg(9, io.util.arrayFrom(modules.toArrayBuffer()), _disussionId, {
                onSuccess: function(arg) {
                    setTimeout(function() {
                        _callback.onSuccess(arg)
                    })
        ...
        this.addMemberToDiscussion = function(_discussionId, _userIdList, _callback) {
            ...
            var modules = new Modules.ChannelInvitationInput();
            modules.setUsers(_userIdList);
            a.queryMsg(0, io.util.arrayFrom(modules.toArrayBuffer()), _discussionId, {
                onSuccess: function(arg) {
                    setTimeout(function() {
                        _callback.onSuccess(arg)
                    })
        ...
        this.getDiscussion = function(_discussionId, _callback) {
            ...
            a.queryMsg(4, io.util.arrayFrom(modules.toArrayBuffer()), _discussionId, {
                onSuccess: function(arg) {
                    setTimeout(function() {
                        _callback.onSuccess(arg)
                    })
        ...
        this.quitDiscussion = function(_discussionId, _callback) {
            ...
            modules.setNothing(1);
            a.queryMsg(7, io.util.arrayFrom(modules.toArrayBuffer()), _discussionId, {
                onSuccess: function(arg) {
                    setTimeout(function() {
                        _callback.onSuccess(arg)
                    })
        ...
        this.removeFromBlacklist = function(userId, callback) {
            ...
            this.getCurrentUserInfo({
                onSuccess: function(info) {
                    var uId = info.getUserId();
                    modules.setUserId(userId);
                    a.queryMsg(22, io.util.arrayFrom(modules.toArrayBuffer()), uId, {
                        onSuccess: function() {
                            setTimeout(function() {
                                _callback.onSuccess()
                            })
        ...
        this.getSendingBox = function(callback) {
            check(["object"]);
            var modules = new Modules.SyncRequestMsg();
            modules.setIspolling(false);
            modules.setIsweb(false);
            modules.setSyncTime(0);
            a.querySendBoxMsg(io.util.arrayFrom(modules.toArrayBuffer()), bridge._client.userId, {
                onSuccess: function(data) {
    ...
    RongIMClient.hasUnreadMessages = function(appkey, token, callback) {
        ...
        xss.src = global['RongOpts']['API_URL'] + "message/exist.js?appKey=" + encodeURIComponent(appkey) + "&token=" + encodeURIComponent(token) + "&callBack=RongIMClient.hasUnreadMessages.RCcallback&_=" + Date.now();
        document.body.appendChild(xss);
        xss.onerror = function() {
            setTimeout(function() {
                callback.onError(RongIMClient.callback.ErrorCode.UNKNOWN_ERROR)
    ...
    RongIMClient.RongIMMessage = function(content) {
        ...
        this.setIsOffLineMessage = function(x){
            offline = x;
        };
        this.getIsOffLineMessage = function(){
            return offline;
        };
