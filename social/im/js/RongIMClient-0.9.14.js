(function(global, undefined) {
	...
	var MD5 = function(e) {
			...
			e = function(a) {
				for (var b = (a.length + 8 >> 6) + 1, d = Array(16 * b), c = 0; c < 16 * b; c++) d[c] = 0;
				for (c = 0; c < a.length; c++) d[c >> 2] |= a.charCodeAt(c) << c % 4 * 8;
				d[c >> 2] |= 128 << c % 4 * 8;
				d[16 * b - 2] = 8 * a.length;
				return d
			}(e);
			for (var d = 1732584193, a = -271733879, b = -1732584194, c = 271733878, f = 0; f < e.length; f += 16) var p = d,
				q = a,
				r = b,
				t = c,
				...
				if (window.FORCE_LOCAL_STORAGE === true) {
					...
					obj = {
						...
						clear: function() {
							...
							if (keys) {
								for (var i = keys.length; i--;) document.cookie = keys[i] + '=0;path=/;expires=' + new Date(0).toUTCString()
							}
						}
					}
				}
	...
	function RongIMClient(_appkey) {
			...
			a.queryMsg(26, io.util.arrayFrom(modules.toArrayBuffer()), bridge._client.userId, {
				onSuccess: function(list) {
					io.util.forEach(list.info, function(x) {
						...
						if (self.bolHistoryMessages && RongIMClient.ConversationType.setValue(mapping[x.type]) != RongIMClient.ConversationType.CHATROOM) {
		...
		this.sendMessage = function(h, v, e, c, u) {
			...
			if (!i.getMessageId()) i.setMessageId(h + "_" + ~~ (Math.random() * 0xffffff));
		...
		this.sendNotification = function(_conversationType, _targetId, _content, _callback) {
			check(["object", "string", "object", "object"]);
			if (_content instanceof RongIMClient.NotificationMessage) this.sendMessage(_conversationType, _targetId, new RongIMClient.MessageContent(_content), null, _callback);
			else throw new Error("Wrong Parameters")
		};
		this.sendStatus = function(_conversationType, _targetId, _content, _callback) {
			check(["object", "string", "object", "object"]);
			if (_content instanceof RongIMClient.StatusMessage) this.sendMessage(_conversationType, _targetId, new RongIMClient.MessageContent(_content), null, _callback);
			else throw new Error("Wrong Parameters")
		};
