...
var ChatUtil = {
    ...
    showImSignPic :function(obj,uid){
        ...
        self.showImSignPicTimer = setTimeout(function(){
            ...
            clearTimeout(self.showImSignPicTimer);
            self.showImSignPicTimer = null;
            var closeImSignPicTimer = setTimeout(function(){
                if(!$("#imSignaturesMessage").is(':hidden')){
                    $("#imSignaturesMessage").hide();
                }
                clearTimeout(closeImSignPicTimer);
            },1000*10);
        }, 600);
    },
    ...
    iconRightClick: function(object,event){
        if(event.button==2){
            ...
            if(object.parent().parent().parent().parent().attr("_targettype")==1){
                ...
                var x= event.clientX;
                var y = event.clientY;
                if(object.find('.chatLItem').length==1){
                    $('#atMessage').css("top",y+12);
                    $('#atMessage').css("left",x+12);
                }
                
                if(object.find('.chatRItem').length==1){
                    $('#atMessage').css("top",y-30);
                    $('#atMessage').css("left",x-120);
                }
                ...
                $('#atMessage').unbind().bind('mouseenter',function(){
                    ...
                    var insert_tpl = "<a href='/hrm/HrmTab.jsp?_fromURL=HrmResource&id="+targetId+"' onclick=\"IMUtil.doOpenLink('/hrm/HrmTab.jsp?_fromURL=HrmResource&id="+targetId+"', '_blank');return false;\" class='hrmecho' _relatedid='"+targetId+"' atsome='@"+targetId+"' unselectable='off' contenteditable='false' target='_blank'>@"+targetName+"</a>"; 
    ...
    sendCountMsg:function(messageid,receiverid,msgFrom){
        ...
        $.post("/social/im/SocialIMOperation.jsp?operation=markMsgRead",{"messageid":messageid,"receiverid":M_USERID,"senderid":receiverid},function(){
            ...
            ClientUtil.sendIMMsg(receiverid,msgObj,6,messageid,0,function(data){
                if(data.issuccess==1){
                        ChatUtil.delCountMsg(messageid,M_USERID);
            ...
            webSql.query("select * from social_IMSyncMsg",function(result){
                ...
                setTimeout(function(){
                    if(syncMsgidsArray.length>0){
                        ...
                        ClientUtil.sendIMMsg(M_USERID,msgObj,6,messageid,0,function(data){
                            if(data.issuccess==1){
    ...
    initChatDiv: function(acceptId, chatType, chatName, headicon){
        ...
        ClientUtil.getClientIns(function(client){
        if(!client){
            $('.dataLoading,.loading1').fadeOut(); //去除loading提示
    ...
    if(ClientSet.ifForbitEmWaterMark == '0'){
      var watermark ;
            var markid ='';
            var textBg = 'white';
            try{
        ...
        // 使用4401测试
        if(true)//M_USERID != '4401' && M_USERID != '646' && M_USERID != '240'
        ...
        if(true){
            ...
            var targetContId = "chatWin_"+chatType+"_"+acceptId;
            var opts = {
                "allowedTypes": ".*",
...
//                    var dept = simpleInfo[simpleInfo.length-5];
    ...
    //确认粘贴的图片
    confirmImg: function(blob, form, chatdiv) {
        if(ClientSet.ifSendPicOrScreenShots == '1') return;
        var IMdlg = $('.IMConfirm');
        if(ChatUtil.cache.reloadPrevImgHandler && ChatUtil.isFromPc() && blob.size == ChatUtil.UploadPreview.imageSize) return;
                ...
                success: function(data) {
                    ...
                    if(!data.content) {
                        IMUtil.getImgUrlFromBlob(blob, function(src){
                            IMUtil.compressImg(src, {maxHeight: 240}, function(base64){
                                data.content = base64;
                                handleData(data);
                            });
                        });
    ...
    removeThisConversation: function(targettype, targetid){
        //移出最近联系人列表
        ClientUtil.removeConversation(targettype, targetid, function(targetid){
    ...
    setThisConversationToTop: function(targettype, targetid, istop){
        if(!istop){ 
           //非置顶状态
            ClientUtil.setConversationToTop(targettype, targetid, function(targetid){
...
// 解散群消息处理
        ...
        if (message && message.objectName == 'RC:DizNtf') {
            ...
            switch((msgType && msgType.value) || msgType){
                ...
                case 2://图片消息
                    ...
                    var imgUrl="";
                    if(message){
                        imgId=message.mssageId;
                        imgUrl= message.imageUri;
                        if(!imgUrl){
                ...
                case 6://群公告,分享类消息,图文消息
                    ...
                    if(objectName=="FW:attachmentMsg"){ //附件
                        ...
                        */
                        ...
                        //tempdiv.find('.chattd').addClass('chatShare');
                        tempTextImg.attr("onclick", "IMUtil.doOpenLink('"+url+"', '_blank');return false");
                ...
                case 3://语音
                    ...
                    if(message){
                        ...
                        if(typeof duration == 'undefined') {
                            try{
                                duration = JSON.parse(message.detail.detail).duration;
                            }catch(err){}
                        }
                    }else{
    ...
    getHistoryMessages: function(targetType,realTargetid,pagesize,isFirst) {
        ClientUtil.getHistoryMessages(targetType,realTargetid,pagesize,function(value){
                    var symbol = value.symbol;
                    var HistoryMessages = value.HistoryMessages;
                    var isSuccess = value.isSuccess;
                    ...
                    for (var i = HistoryMessages.length-1; i >=0; i--) {
                        ...
                        var objName = imMessage.objectName; //消息标识
                        var extra=imMessage.extra;
                        try{
                            var extraObj=eval("("+extra+")");
                        ...
                        var date = new Date();
                        date.setTime(imMessage.sentTime);
                    ...
                    if(historyCount<10&&HistoryMessages.length>0){
                        ...
                        for (var i = 0; i<messageArray.length; i++) {
                            ...
                            var messageid=imMessage.messageId;
                            var senderUserid=imMessage.senderUserId; //发送人id
                            var targetid=imMessage.targetId; //消息来源对象id
                            ...
                            var msgType=imMessage.messageType;
                            var conversationType=imMessage.conversationType; //会话类型
                            var sendtime=imMessage.sentTime; //消息发送时间
                            var objName = imMessage.objectName; //消息标识
                            var messageTag=imMessage.messageTag;
                            var extra=imMessage.extra;
                            var detail=imMessage.detail;
    ...
    initMsgRead:function(msgid,targetType,resourceids,flag,targetId){ //初始化消息阅读状态
    ...
    updateMsgUnreadCountHtml:function(messageid,targetType,count){
        ...
        if(!chatWin.is(':hidden')&&chatitem.parent().is(":hidden")){
            chatitem.parent().show();
            contentHeight=chatContentdiv.find('.chatContent').outerHeight();
            chatitem.parent().hide();
        }       
        if(contentHeight==0){
    ...
    updateMsgUnreadCount:function(messageid,receiverid,targetType,isRepeat,msgFrom){ //更新消息阅读状态
        if(messageid==""||receiverid==undefined || receiverid == 'SysNotice') return;
        ...
        $.post(url,{"messageid":messageid,"receiverid":receiverid,"msgFrom":msgFrom},function(data){
            ...
            if(isInit){
                ...
                var targetid=chatWin.attr("_targetid");
                if(_targetType == 1){
                    ClientUtil.getDiscussionMemberIds(targetid,function(ids){
    ...
    getLatestMessage:function(targetid,targetType,targetName){
        ...
        ClientUtil.getHistoryMessages(targetType,targetid,5,function(value){
                var symbol = value.symbol;
                var HistoryMessages = value.HistoryMessages;
                var flag = value.isSuccess;
                var message, objName;
                var isFind=false;
                if(HistoryMessages){
                ...
                if(isFind){
                    ...
                    if(HandleInfoNtfMsg.isMsgWithdrawed(message)){
                        message.content=social_i18n('WithdrawTip', '');
                    }
    ...
    getDiscussionInfo : function(discussid,force,callback){
        ...
        if(discuss && !force){
            ...
            ClientUtil.getDiscussion(discussid,function(discuss){
                if(discuss){
    ...
    initIMAtwho:function (targetid){
        ...
        var at_config = {
            ...
            insert_tpl: "<a href='/hrm/HrmTab.jsp?_fromURL=HrmResource&id=${id}' onclick=\"IMUtil.doOpenLink('/hrm/HrmTab.jsp?_fromURL=HrmResource&id=${id}', '_blank');return false;\" class='hrmecho' _relatedid='${id}' atsome='@${id}' unselectable='off' contenteditable='false' target='_blank'>${atwho-data-value}</a>", 
            limit: 200,
            callbacks:{
    ...
    sendIMMsgSimulating: function(chatInfo, msgArray, callback) {
        ...
        showConverChatpanelNew(_acceptid,chatInfo.userName,chatInfo.userHead,chatInfo.targetType);
        if (msgArray && msgArray.length > 0) {
            ...
            if(_objeceName == "fw:customsharemsg") {
                ...
                if (_msgType == 6 || _msgType == 1) {
                        WindowDepartUtil.winDepartParm.openConArray[_acceptid]=false;
                    $("#imCenterdiv").bind('DOMNodeInserted',function(){
                        var charSendDiv = $("div[_acceptid='" + _acceptid + "']").filter('.chatSend');
                        if(charSendDiv.length==0){
                            return;
                        }
                        if(WindowDepartUtil.winDepartParm.openConArray[_acceptid]){
                                return; 
                        }
                        WindowDepartUtil.winDepartParm.openConCount++;
                        if(WindowDepartUtil.winDepartParm.openConCount==WindowDepartUtil.winDepartParm.openConNum){
                            $("#imCenterdiv").unbind('DOMNodeInserted');
                        }
                        WindowDepartUtil.winDepartParm.openConArray[_acceptid]=true;
                            ...
                            if(objectName =="FW:CustomShareMsg"){
                                    ...
                                    if(shareType=="folder"||shareType=="pdoc"){
                                        ...
                                        if(targetType =='0'){
                                            ...
                                            ClientUtil.getDiscussionMemberIds(msgobj.targetid ,function(ids){shareCrm(shareid,ids+"")});
                                        }
	...
	contentobj.html("");
        ...
        extra+=",\"countids\":\""+CountidsUtil.getMsgids(targetid)+"\"";
        extra="{"+extra+"}";
        ...
        WindowDepartUtil.winDepartParm.sendtime=sendtime;
        ClientUtil.sendIMMsg(targetid,msgObj,msgType,msgid,targetType,function(msg){
        ...
        contentobj.focus();
        // tab页置顶
        ChatUtil.setChatTabTop(targetid, targetType);
    },
    // tab页置顶显示
    setChatTabTop: function(targetid, targetType){
			...
			webSql.query("select * from social_IMCountMsg where userid ='"+M_USERID+"'",function(result){
					for (var i = 0; i < result.rows.length; i++) {
                        ...
                        if(chatdivObj.find('#'+messageid).length > 0) {
                            ChatUtil.sendCountMsg(messageid, targetid,'web');
                            ChatUtil.delCountMsg(messageid, M_USERID);
                        }
					}
			});
    ...
    publishMessage:function(msgObj){
        ...
        var msgObj={"content":params,"objectName":"FW:SyncMsg","targetType":"0"};
        ClientUtil.sendIMMsg(M_USERID,msgObj,6,messageid,msgObj.targetType,function(data){
            if(data.issuccess==0){
    ...
    sendClearUnreadCountMsg : function(targetId, targetType){
        ...
        ClientUtil.getClientIns(function(client){
        if(client){
            ClientUtil.sendMessageToUser(M_USERID, msgObj, 6,targetType);
        }else{
            ChatUtil.postFloatNotice(social_i18n('FloatNotice2'), "error", {
                "onclick": {
...
var DiscussUtil = {
    ...
    DiscussSetFunc: {
        ...
        itemMouseHand: function(obj, ismovein){
            ...
            var discussid = dsDiv.attr('discussid');
            var discuss = discussList[discussid];
            //是否创建人
        ...
        setDiscussionAdmin:function(discussid, admins,cb){
            if(admins.length<2){
                ClientUtil.setDiscussionAdmin(discussid,getIMUserId(admins[0]),cb);
            }           
        },
        //修改群名称
        changeDiscussName: function(obj){
            ...
            if(newDiscussName==""){
                ...
                newDiscussName = newDiscussName.replace(/<br>/gi, '').replace(/<\/br>/gi, '');
                
                ClientUtil.setDiscussionName(discussid, newDiscussName, function(discussid,newDiscussName){
        ...
        setNewMsgNotice: function(obj){
            ...
            $.post(url,{"targetid":targetid,"targetType":targetType,"remindType":remindType},function(data){
                ...
                //需要同步到右侧
                ClientUtil.setNewMsgNotice(settingInfo);
            })
                ...
                if (data.id != "") {
                    ...
                    DiscussUtil.checkMemCount(memList, discussid, function(isOver) {
                        //if(isOver){
                         ...
                         //   return;
                        //}else{
                        ClientUtil.addMemberToDiscussion(discussid, memList, function() {
        ...
        quitFromDiscussion: function(discussid){
            ClientUtil.quitDiscussion(discussid,function(){
               			if(!IS_BASE_ON_OPENFIRE){
					//公有云维护最近回话列表
				...
				ClientUtil.sendIMMsg(M_USERID,msgObj,6,false,targettype);
            },function(err){
            ...
            $("#discussListNav").bind('mouseleave', function() { setTimeout(function() { $("#discussListNav").hide(); }, ChatUtil.discussMenuSet.onHideDelay); });
              ...
              $.post(url, function(data){
                  ...
                  for(var i=0; i<jsonGroup.length; i++) {
                      if(jsonGroup[i].name==pName)
                          continue;
                      var li = "<li><a href='#' _target='moveToGroup' id='"+jsonGroup[i].id+"' style='max-width:130px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis' title='"+jsonGroup[i].name+"'>&nbsp;&nbsp;&nbsp;"+jsonGroup[i].name+"</a></li>";;
                      all=all+li;
                      j=j+1;
                  }
                  ul=ul+all;
                ...
                $("ul.sf-menu").find("li").each(function(index, obj) {
                    ...
                    if (target == 'moveToGroup') {
                        $(obj).unbind('click').bind('click', function() {
                            ...
                            showImConfirm(social_i18n('MoveToGroupTip', name, groupName), function() {
    ...
    _addDiscuss: function(disName,resourceids,memList,callback) {
        ...
        DiscussUtil.checkMemCount(memList, null, function(isOver){
            if(isOver){
                ...
                ClientUtil.createDiscussion(disName,
                    memList,{
...
var QuickReplyUtil = {
    openQuickreplay: function(targetid, evt, reload){
        ...
        quickreplayBlock.css("right","100px");
...
var OnLineStatusUtil = {
    getUserOnlineStatus : function(memberids,cb){
        var callback = {
            onSuccess : function(array){
                if(cb!=undefined){
                    ...
                    OnLineStatusUtil.receiveUserStatusHandle(array,memberids);
                }
            },
            onError : function(){
                //client.writeLog("获取用户状态失败,服务端拒绝请求");
        ...
        if(IS_BASE_ON_OPENFIRE){
            ...
            for(var i=0;i < length / 200;i++ ){
                ...
                //IMclient.prototype.getUserOnlineStatus(temp,callback);
            } 
            $(document).bind("click.autoHide",function(e){
    ...
    setUserOnlineStatus : function(userStatus,callback){
        var callback = {
            onSuccess : function() {
                $("#pc-headtoolbar").find("._userName").find(".edUserStatus").css('background','url("'+SOCIAL_GLOBAL.OnlineStatus.TOP_ONLINESTATUS[userStatus]+'") no-repeat center center');
                $("#pc-headtoolbar").find("._userName").find(".edUserStatus").attr('title',ONLINESTATUS[userStatus]);
                $("#pc-headtoolbar").find("._userName").find(".edUserStatus").attr('target',userStatus);
    ...
    switchUserStatus : function(obj){
        ...
        var oldStatus = $("#pc-headtoolbar").find("._userName").find(".edUserStatus").attr('target');
        if(status==oldStatus){
    ...
    showUserStatusPanel : function(obj,event ){
       ...
       $(".userStatusPanel").bind('mouseleave', function() { setTimeout(function() { $(".userStatusPanel").hide(); }, ChatUtil.discussMenuSet.onHideDelay); });
    },
    ...
    setOtherLeftStatus : function (userid,userStatus,mobileStatus){
        ...
        if(chatIMTabLeft.length>0){
            var head = chatIMTabLeft.find('.headicon').find('img.head28');
            if(chatIMTabLeft.find('.headicon').find('#userStatus').length>0){
            ...
            if(mobileStatus=='offline'){
                if(userStatus=='offline'){
                    ...
                    if(userStatus!="online"){
                        var hoverBtn = $('<div id="userStatus" target="'+userStatus+'" device="pc" style="margin-top: -13px;z-index: 10;position: relative;"><img style="position: relative;margin-left: 20px;" src='+SOCIAL_GLOBAL.OnlineStatus.LEFT_ONLINESTATUS[userStatus]+'></div>');
                        head.css('position', 'relative');
                        head.after(hoverBtn);
                    }
                }
            }else{
                var hoverBtn = $('<div id="userStatus" target="'+mobileStatus+'" device="mobile" style="margin-top: -13px;z-index: 10;position: relative;"><img style="position: relative;margin-left: 20px;" src='+SOCIAL_GLOBAL.OnlineStatus.LEFT_ONLINESTATUS[mobileStatus]+'></div>');
                head.css('position', 'relative');
                head.after(hoverBtn);
            }
        }
    ...
    setOtherRightStatus : function (userid,userStatus,mobileStatus){
        ...
        if(chatIMTabRight.length>0){
            var head = chatIMTabRight.find('img.head35');
            if(chatIMTabRight.find('.itemleft').find('#userStatus').length>0){
            ...
            if(mobileStatus=='offline'){
                if(userStatus=='offline'){
                    ...
                    if(userStatus!="online"){
                        var hoverBtn = $('<div id="userStatus" target="'+userStatus+'" device="pc" style="margin-top: -17px;z-index: 10;position: relative;"><img style="position: relative;margin-left: 25px;" src='+SOCIAL_GLOBAL.OnlineStatus.LEFT_ONLINESTATUS[userStatus]+'></div>');
                        head.css('position', 'relative');
                        head.after(hoverBtn);
                    }
                }
            }else{
                var hoverBtn = $('<div id="userStatus" target="'+mobileStatus+'" device="mobile" style="margin-top: -17px;z-index: 10;position: relative;"><img style="position: relative;margin-left: 25px;" src='+SOCIAL_GLOBAL.OnlineStatus.LEFT_ONLINESTATUS[mobileStatus]+'></div>');
                head.css('position', 'relative');
                head.after(hoverBtn);
            }
        }
    ...
    setAllStatus : function (userid,userStatus,mobileStatus){
        ...
        OnLineStatusUtil.setTreeUserStatus(userid,userStatus,mobileStatus,4);
    ...
    showUserStatusTip : function(userid,userStatus,mobileStatus){
        ...
        if(userStatus=='busy'&&mobileStatus!='online'){
            tip = '对方当前为忙碌状态，可能无法及时回复您的消息';
        }
        if(userStatus=='away'&&mobileStatus!='online'){
            tip = '对方当前为离开状态，可能无法及时回复您的消息';
        }
    ...
    setOrgTreeUserStatus : function (currentArr,memArr,type){
        var cb = {
             onSuccess:function (userid,pcStatus,mobileStatus){
                 if(type==1){
                    SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid]=currentArr[userid];
                 }else if(type==2){
                    SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid]=currentArr[userid];
                 }else if(type==3){
                    SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid]=currentArr[userid];
                 }
                 OnLineStatusUtil.setTreeUserStatus(userid,pcStatus,mobileStatus,type);
             },
             onError : function(){
                 //client.writeLog('获取在线状态失败');
             }
         };
         OnLineStatusUtil.getUserOnlineStatus(memArr,cb);
    },
    setTreeUserStatus : function(userid,pcStatus,mobileStatus,type){
        if(type==1){
           var current = SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid] ;
        }else if(type==2){
           var current = SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid];
        }else if(type==3){
           var current = SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid];
        }
        if(pcStatus=='online'||mobileStatus=='online'){
             if(type==4){
                 if(SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid].find(".orgTreeUserStatus").remove();
                 }
                 if(SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid].find(".orgTreeUserStatus").remove();
                 }
                 if(SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid].find(".orgTreeUserStatus").remove();
                 }                    
             }else{
                if(current==undefined)return;
                current.find(".orgTreeUserStatus").remove();
             }
         }else if(pcStatus!='online'&&mobileStatus!='online'){
             if(pcStatus=='offline'&&mobileStatus=='offline'){
                 var tempdiv=$('<div class="orgTreeUserStatus" style="position: absolute;right:20px;display: inline-block;">[离线]</div>');
             }else{
                 var tempdiv=$('<div class="orgTreeUserStatus" style="position: absolute;right:20px;display: inline-block;">['+ONLINESTATUS[pcStatus]+']</div>');
             }
             if(type==4){
                 if(SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid].find(".orgTreeUserStatus").remove();
                    SOCIAL_GLOBAL.OnlineStatus.M_ORG_TREE[userid].append(tempdiv);
                 }
                 if(SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid].find(".orgTreeUserStatus").remove();
                    SOCIAL_GLOBAL.OnlineStatus.M_PRIVATEGROUP_TREE[userid].append(tempdiv);
                 }
                 if(SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid]!=undefined){
                    SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid].find(".orgTreeUserStatus").remove();
                    SOCIAL_GLOBAL.OnlineStatus.M_PUBLICGROUP_TREE[userid].append(tempdiv);
                 }                    
             }else{
                if(current==undefined)return;
                current.find(".orgTreeUserStatus").remove();
    ...
    setDeptLowerUserStatus : function(userid,pcStatus,mobileStatus){
        var chatDept = $('#contactListdiv').find("#deptConb").find(".chatItem[_targetid='"+userid+"']");
        var chatLower = $('#contactListdiv').find("#lowerConb").find(".chatItem[_targetid='"+userid+"']");
        if(chatDept.length>0){
            var headDept = chatDept.find('.itemleft').find('img.head35');
            if(chatDept.find('.itemleft').find('#userStatus').length>0){
                chatDept.find('.itemleft').find('#userStatus').remove();
            }
            var flag = 0;
            if(mobileStatus=='offline'){
                if(pcStatus=='offline'){
                    ...
                    if(pcStatus!="online"){
                        var hoverBtn = $('<div id="userStatus" target="'+pcStatus+'" device="pc" style="margin-top: -17px;z-index: 10;position: relative;"><img style="position: relative;margin-left: 25px;" src='+SOCIAL_GLOBAL.OnlineStatus.LEFT_ONLINESTATUS[pcStatus]+'></div>');
                        flag = 1;
                    }
                }
            }else{
            ...
            if(flag==1){
                headDept.css('position', 'relative');
                headDept.after(hoverBtn);
            }
        }
        if(chatLower.length>0){
            var headLower = chatLower.find('.itemleft').find('img.head35');
             if(chatLower.find('.itemleft').find('#userStatus').length>0){
                chatLower.find('.itemleft').find('#userStatus').remove();
            }
            var flag = 0;
            if(mobileStatus=='offline'){
                if(pcStatus=='offline'){
            ...
            if(flag==1){
                headLower.css('position', 'relative');
