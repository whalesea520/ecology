...
Favourite.prototype = {
	...
	registerEvt : function(_favids){
		...
		jqobjs.find("div.favop span").bind("mouseover",function(){
			...
			if(topParent.length > 0){
				...
				if(that.hasClass("repeat")){
					...
					$this.operation.movefav(topParent,$this);
		...
		jqobjs.each(function(){
			var that = jQuery(this);
			var favid = that.data("id");
			that.find("div.fav_level").bind("click",function(){
				$this.operation.editLevel(favid,$this);
			});
		})
	},
	/**
	...
	operation :{
		 ...
		 * 编辑重要程度
		 */
		editLevel : function(id,$instance){
			var levelPanel = jQuery("#levelPanel");
		 ...
		 * 移动
		 */
		movefav : function(favitemObj,$instance){
			var movePanel = jQuery("#movePanel");
		 ...
		 * 批量移动
		 */
		movefavs : function(btnObj,$instance){
			var movePanel = jQuery("#movePanel");
			var objid = movePanel.data("objid");
			var favid = "movebtn";
			var favids = "";
...
function registerOtherEvt(_favourite){
	 ...
	 * 批量移动按钮的事件
	 */
	jQuery("#movebtn").bind("click",function(){
		var that = jQuery(this);
		_favourite.operation.movefavs(that,_favourite);
	});
