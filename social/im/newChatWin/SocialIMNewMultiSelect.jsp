...
<html>
	...
	try{
       var id = new String();
       var name = new String();
       jQuery('.e8_box_d').find('#dest_box_middle').find('.ui-sortable').children('tr').each(function(index,obj){
           var idNum = $(obj).find("td > input[name=id]").attr('value');
           id+=(idNum)+',';
           var nameString = $(obj).find("td:eq(2)").html();
           name+=nameString+',';
       });
       ...
       parent.parent.parent.window.Electron.ipcRenderer.send('plugin-addGroup-cb',data);
       parent.parent.parent.window.Electron.currentWindow.close();
       }catch(err){
       parent.parent.parent.window.Electron.currentWindow.close();
       }
    }
	...
	function addGroup(node){
		...
		var dialogGroup = new window.top.Dialog();
		dialogGroup.currentWindow = window;
		dialogGroup.Title = "<%=SystemEnv.getHtmlLabelNames("82,17748",user.getLanguage())%>";
	  dialogGroup.URL = "/hrm/HrmDialogTab.jsp?_fromURL=hrmGroup&method=HrmGroupAdd&type="+type;
	...
	function doEdit(id){
		var dialogGroup = new window.top.Dialog();
		dialogGroup.currentWindow = window;
		dialogGroup.Title = "<%=SystemEnv.getHtmlLabelNames("93,17748",user.getLanguage())%>";
	  dialogGroup.URL = "/hrm/HrmDialogTab.jsp?_fromURL=hrmGroup&method=HrmGroupAdd&cmd=edit&isdialog=1&id="+id;
		dialogGroup.Width = 700;
		dialogGroup.Height = 500;
		dialogGroup.Drag = true;
		dialogGroup.textAlign = "center";
								...
								<td>
									<span id="searchblockspan" style="width: 284px;display: inline-block;">
										<span class="searchInputSpan" style="position:relative;width: 288px">
											...
											<input type="text" class="searchInput middle" id="flowTitle" name="flowTitle" value="<%=qname %>" onkeyup="jsSourceSearch(this)" onfocus="onSearchFocus();$('#src_box_middle').css('height','398px');$('#dest_box_middle').css('height','399px');" onblur="onSearchFocusLost()" style="vertical-align: top;width: 258px">
...
<wea:layout needImportDefaultJsAndCss="false">
	<wea:group context="">
		<wea:item type="toolbar">
			<input type="button" class=zd_btn_submit id=btnok val="<%=SystemEnv.getHtmlLabelName(826,user.getLanguage())%>" value="<%=SystemEnv.getHtmlLabelName(826,user.getLanguage())%>" onclick="commitOk()"></input>
			<input type="button" class=zd_btn_submit id=btnclear value="<%=SystemEnv.getHtmlLabelName(311,user.getLanguage())%>" onclick="commitClear()"></input>
			<input type="button" class=zd_btn_cancle id=btncancel value="<%=SystemEnv.getHtmlLabelName(201,user.getLanguage())%>" onclick="commitCancel()"></input>
		</wea:item>
	</wea:group>
</wea:layout>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		...
		jQuery('.e8_box_slice').attr('style','height:400px;');
		jQuery('#dest_box_middle').css('height',398);
		<%if(lsConditionField!=null&&lsConditionField.size()>0){%>
	...
	function expandZH(){
		...
		jQuery("#dest_box_middle").css("height",height-135);
	...
	function commitCancel(){
	   var electron = parent.parent.parent.window.Electron;
