...
var HandleSysMsgUitl = {
    ...
    openCon : function(allContent){
        ...
        WindowDepartUtil.winDepartParm.openConArray = new Array();
        WindowDepartUtil.winDepartParm.openConNum = _targetIdList.length;
        WindowDepartUtil.winDepartParm.openConCount = 0;
            ...
            $.each(_targetIdList,function(i,result){
                if(result!=M_USERID){
                    if(result.length<32){
                        ...
                        ClientUtil.getDiscussionName(result,function(name){
                        var chatInfo = {};
...
var HandleInterceptMsgUtils = {
    ...
    clearUnreadCountNew:function(targetId,count){
...
var HandleInfoNtfMsg = {
    ...
    _handleWithDrawSuccess: function(msgObj){
        if(msgObj===undefined||msgObj==='undefined') return;
    ...
    withDrawMsg: function(msgObj, targettype, targetid, callback){
        ...
        if(targettype == '0'){
            ...
            ClientUtil.sendMessageToUser(targetid, wdMsgObj, 9,  targettype,triggerFunc);
        }else if(targettype == '1'){
        
           ClientUtil.sendMessageToDiscussion( targetid, wdMsgObj, 9,'',targettype, triggerFunc);
    ...
    caneclShareDiskMsg: function(msgObjOrMsgid, targettype, targetid, callback){
        ...
        if(targettype == '0'){
            ...
            ClientUtil.sendMessageToUser( targetid, sdMsgObj, 6, targettype,triggerFunc);
        }else if(targettype == '1'){
            
            ClientUtil.sendMessageToDiscussion( targetid, sdMsgObj, 6, '',targettype,triggerFunc);
        }
    },
    // 处理撤销消息
...
function getPushMsgType(message){
    var objName = message.objectName;
...
function getTargetType(conversationType,targetid){
        var targetType="-1";
        if(targetid){
            try{
...
function handleIMMsg(message){
    ...
    var senderUserid=message.senderUserId; //发送人id
    var targetid=message.targetId; //消息来源对象id
    ...
    var objName = message.objectName; //消息标识
    var extra=message.extra;
    ...
    var targetType=getTargetType(conversationType,targetid);
    ...
    if(message && message.objectName == 'RC:DizNtf'){
...
function updateTotalMsgCount(targetid,flag,targetType,count){
	...
	ClientUtil.getConversationCount(targetType,realTargetId,function(msgcount){
	//窗口分离版本不需要加一，直接展示即可
	var targetTotal  = msgcount;
...
function getConversationList(){
    ...
    ClientUtil.getConversationList(function(conversationList){
        ...
        for (var i = 0; i <conversationList.length; i++) {
            ...
            var targetType=getTargetType(conversationType,targetid); //0：单聊 1:群聊 -1：其他   
...
function goMounseHandler(obj, e) {
    ...
    switch(e.which) {
        ...
        case 1:     //单击    
            ...
            Client.getClientIns(function(client){
            if(!client){
...
function showConverChatpanel(obj){
     if($(obj).length > 0){
       ...
       showConverChatpanelNew(targetid,targetName,targetHead,targetType);
}

function showConverChatpanelNew(targetid,targetName,targetHead,targetType,count){
    ...
    var timerid = setTimeout(function(){
        ...
        setIMChatPanelNew(targetid,targetType);
        
        // pc对聊天面板的处理
        setIMChatPanelOfPcNew(targetid,targetType);
...
function setIMChatPanelNew(targetId, targetType){
    if(targetId == 'SysNotice') {
...
// pc端对聊天面板处理
function setIMChatPanelOfPcNew(targetId, targetType) {
    // pc端对工具栏处理
    var $chatWin = $('#chatWin_' + targetType + '_' + targetId);
    if(ChatUtil.isFromPc()) {
...
function clearMessagesUnreadStatus(targetType,targetId){
    ...
    ClientUtil.clearMessagesUnreadStatus(targetType,targetId); 
}
...
function downloads(fileid){
  ...
  function showIMChatpanel_l(chatType, acceptId,chatName,headicon) {
     //隐藏搜索结果页
     showResultDiv(0);
  ...
  function showIMChatpanel(chatType, acceptId,chatName,headicon) {
     showIMChatpanel_r(chatType, acceptId,chatName,headicon);
  }
  //显示聊天面板
  function showIMChatpanel_r(chatType, acceptId,chatName,headicon) {
            ...
            if(acceptId == "bing"){
				...
				$("#"+chatWinid).html("<iframe id ='privateframe' name='privateframe'  frameborder='0' width='100%' height='100%' scrolling ='no'></iframe>");
  				$("#"+chatWinid + " iframe[name='privateframe']").attr('src',url+"&targetName="+chatName);
                    ...
                    if(chatdivTemp.find('.chatbottom').length==0){
                    $("#chatDivFrm").load(function(){
                        var chatdivTemp = $("#chatDivFrm").contents().find("#chatdivTemp").clone();
                        if(chatType == '1'){
                            chatdivTemp.find('.imnote').show();
                            chatdivTemp.find('.imvote').show();
...
scrollTOBottom($("#"+chatWinid).find(".chatList"));
  ...
  function changeTabWin(obj){
    ...
    //更新当天聊天窗口激活的targetid
    ClientUtil.updateChatWinID(targetid);
    //同步个性签名
    if(chattype==0){
        ChatUtil.updateSignature(targetid);
        if(IS_BASE_ON_OPENFIRE){
  ...
  function closeTabWin(obj){
      ...
      if(M_SDK_VER == '1') {
        ClientUtil.resetGetHistoryMessages(targetType,targetid);
      }
      ...
      if($('#chatIMTabs').children('.chatIMTabItem').length===0){
  ...
  function initBrowerTabHandler(_win){
    ...
    var onVisibilityChange = function(){
        if (!doc[hiddenProperty]) {    
            ...
            ClientUtil.reconnect();//激活后检查链接状态，如果断开重新连接
                    ...
                    if(msg && msg.issuccess){
                        ...
                        try{
                            ...
                            if(targetType =='0'){
                                ...
                                ClientUtil.getDiscussionMemberIds(targetId ,function(ids){shareCrm(shareid,ids+"")});
                            }
                        }else{                          
