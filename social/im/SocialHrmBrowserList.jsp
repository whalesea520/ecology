...
if(tabid.equals("recent")){
	...
	String queryMemCountSql = "";
	if(IS_BASE_ON_OPENFIRE){
	    queryMemCountSql = "select groupname as targetid,count(*) from ofgroupuser where groupname in (select targetid from social_IMRecentConver where userid = '"+userid+"') group by groupname ";
	}else{
	    queryMemCountSql = "select group_id as targetid,count(*) from mobile_ronggroup where group_id in (select targetid from social_IMRecentConver where userid = '"+userid+"') group by group_id";
	}
	log.writeLog("queryMemCountSql============="+queryMemCountSql);
	...
	for(int i = 0; i < mapList.size(); ++i){
		...
		String targetPinyin = SocialUtil.getPingYin(targetName);
        String targetFirstSpell = SocialUtil.getFirstSpell(targetName);
        ...
        if(!searchName.equals("") &&targetPinyin.indexOf(searchName) == -1){
            flag +=1;
        }
        if(!searchName.equals("") &&targetFirstSpell.indexOf(searchName) == -1){
            flag +=1;
        }
        if(flag==3){
            continue;
...
<%}else if(tabid.equals("group")){ %>
	...
	for(int i=0;i<groupList.size();i++){
		...
		String targetPinyin = SocialUtil.getPingYin(targetName);
		String targetFirstSpell = SocialUtil.getFirstSpell(targetName);
		//log.writeLog("targetName============"+targetName);
		//log.writeLog("targetPinyin============"+targetPinyin);
		//log.writeLog("targetFirstSpell============"+targetFirstSpell);
		if(!searchName.equals("") && (targetName.indexOf(searchName) == -1)){
		    flag +=1;
		    //continue;
		}
