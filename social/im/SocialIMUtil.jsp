...
var countInterval=setInterval(function(){
	if(countMsgidsArray.length>0 && M_SERVERSTATUS){
		...
		if(sendCountMsgids.indexOf(msgid)==-1){
            ...
            ChatUtil.delCountMsg(msgid,toid,'send');
        }
	}
},300);
...
function handlePushMsg(pushMsgType, message){
	...
	var objectName = message.getObjectName();
	var isCmdMsg = objectName === 'FW:CMDMsg';
	switch(pushMsgType){
    	//必达
    	case 0:
    		try{
    		...
    		try{
    			...
    			client && client.error("handlePushMsg extra -->extraObj 失败！");
    		}
    		var targettype = 3;
    		var targetid="bing_"+M_USERID;
    		var msgType=6;
    		if(isCmdMsg && contentObj) {
    			...
    			var dingContent = extraObj.alert;
    			var dingid = "";
	    		var senderid = "";
	    		var messageid = "";
	    		updateFixedConversation(targetid,targettype,dingContent,sendtime);
    		}
                ...
                if(ChatUtil.isFromPc()){
                    switch(content.pushType) {
                        ...
                        case 'weaver_sendDir' :  //发送文件夹，发送系统消息通知对方
                            var userInfos = PcMainUtils.userInfos;
	...
	case 4:
	   ...
	   case 6:
			...
			if(!isWinAct) return;
			updateTotalMsgCount(targetid,true,targettype,1); 		
...
var HandleSysMsgUitl = {
    ...
    forcedOffline : function(allContent){
        ...
        if(ChatUtil.isFromPc()) {
			...
			if(clearType == 0) { // 清理缓存
				PcMainUtils.clearCache(function(){
...
var HandleInterceptMsgUtils = {
    ...
    clearUnreadCount : function(targetId){
		if(targetId.indexOf('|private')>-1){
			PrivateUtil.setConnverCache(getRealUserId(targetId),false);
		}else{
...
var HandleInfoNtfMsg = {
	...
	withDrawMsg: function(msgObj, targettype, targetid, callback){
		...
		wdMsgObj.content = "";
		var extra = {"withdrawId": msgId, "notiType": "noti_withdraw", "msgFrom": "pc"};
		if(targettype == 8){
			extra.isPrivate ="1"
		}
		wdMsgObj.extra = JSON.stringify(extra);
		wdMsgObj.objectName = "RC:InfoNtf";
		var msgItemdiv = $("#"+msgId);
		if(msgItemdiv.attr('withdraw-flag')) {
			return;
		}
		msgItemdiv.attr('withdraw-flag', '1');
		var triggerFunc = function(result){
			...
			if(result.issuccess){
				...
				var content = social_i18n('WithdrawTip', social_i18n('You'));
				var msgItemDataObj = msgItemdiv.data("msgObj");
				if(msgItemdiv.nextAll('div').length == 0&&targettype!=8){
					// 更新会话
					updateConversationList(userInfos[M_USERID],targetid,targettype,social_i18n('WithdrawTip', M_USERNAME),M_CURRENTTIME,true);
				}
				...
				msgItemdiv.removeAttr('withdraw-flag');
			}
	...
	handleWithdraw: function(notiType, targettype, targetid, message) {
		...
		// 如果已经缓存被撤回消息id，则为了防止小灰条消息不继续做处理
		if(this.withDrawMap[withDrawId]) {
			return;
		}
		// 缓存消息id
		...
		if(chatwin.length>0){
			...
			var content = social_i18n('WithdrawTip', senderName);
		...
		// 更新会话
		if(targetid!=="" && needUpdateConver && targettype !=8){
			updateConversationList(senderInfoUndo,targetid,targettype,social_i18n('WithdrawTip', senderInfoUndo.userName),sendtime);
			// 更新底部提示
			var relateddiv = $('#relatedMsgdiv_bottom_'+targetid);
			if(relateddiv.length > 0) {
				relateddiv.find(".relatedContent").html(social_i18n('WithdrawTip', senderInfoUndo.userName));
			}
		}
...
var HrmGroupUtil = {
	...
	doAdd: function(memberIdList, nid){
        ...
        var diag = this.getBrowserDiag("<%=SystemEnv.getHtmlLabelName(82, user.getLanguage()) +" "+ SystemEnv.getHtmlLabelName(17748, user.getLanguage())%>", nid); // 新建自定义组
        if(memberIdList) {
            var hrmids = "",hrmnames="";
	...
	if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
	   if(hrmids===undefined||hrmids==='undefined'){
	       var hrmids = '';
	       var hrmnames = '';
	   }
	   var args = {
              'title' : "<%=SystemEnv.getHtmlLabelName(82, user.getLanguage()) +" "+ SystemEnv.getHtmlLabelName(17748, user.getLanguage())%>", // 新建自定义组
              'width' : 800,
              'height' : 600,
              'type': type,
              'hrmids':hrmids,
            ...
            WindowDepartUtil.winDepartParm.hrmGroupAddNid=nid;
            WindowDepartUtil.openNewWindow(args,WindowDepartUtil.NewWindowList.HrmGroupAdd);
	}else{
		diag.show();
	}
	},
	doEdit: function(nid){
		// 编辑自定义组
		var diag = this.getBrowserDiag("<%=SystemEnv.getHtmlLabelName(93, user.getLanguage()) +" "+ SystemEnv.getHtmlLabelName(17748, user.getLanguage())%>", $('#'+nid).parents('li').attr('id'));
...
function isOfflineMsg(message) {
	return message.getIsOfflineMsg();
}

function handleCacheOfflineMsg() {
	console.info("启动监听器");
	SOCIAL_GLOBAL.offlineMsgHandler = setInterval(function(){
		if(SOCIAL_GLOBAL.OfflineMsgPool.length > 0) {
			console.info("缓存非空，正在处理缓存数据");
			var msgObj =  null, count = 0;
			while(count < 10) {
				msgObj = SOCIAL_GLOBAL.OfflineMsgPool.shift();
				if(msgObj && msgObj.msg){
					receiveIMMsg(msgObj.msg, true);
				}
				count++;
			}
			SOCIAL_GLOBAL.idleCount = 0;
		}else {
			console.info("缓存已空");
			SOCIAL_GLOBAL.idleCount++;
			if(SOCIAL_GLOBAL.idleCount > 1000) {
				clearInterval(SOCIAL_GLOBAL.offlineMsgHandler);
				SOCIAL_GLOBAL.offlineMsgHandler = null;
				console.info("销毁监听器");
			}   
		}
	}, 300);
...
function handleOfflineMsg(message) {
	var next = true;
	try{
		// 处理离线消息
	   	if(isOfflineMsg(message)) {
	   		// 第一条离线消息不做缓存
	   		if(SOCIAL_GLOBAL.ReceivedOfflineMsgCount > 0) {
	   			// 比较和上次收到的时间比较，如果时间间隔小于30ms，则把消息放到缓冲区
	   			var currTime = new Date().getTime();
	   			if(SOCIAL_GLOBAL.OfflineMsgPool.length == 0 || currTime - SOCIAL_GLOBAL.OfflineMsgPool[SOCIAL_GLOBAL.OfflineMsgPool.length - 1].rectime < SOCIAL_GLOBAL.MilisThrehold) {
	   				SOCIAL_GLOBAL.OfflineMsgPool.push({'rectime': currTime, 'msg': message});
	   				console.info("添加一条消息到缓冲区");
	   				if(!SOCIAL_GLOBAL.offlineMsgHandler){
	   					handleCacheOfflineMsg();
	   				}
	   				if(SOCIAL_GLOBAL.OfflineMsgPool.length > 0)
	   					next = false;
	   			} 
	   		...
	   		SOCIAL_GLOBAL.ReceivedOfflineMsgCount++;
	   	}
...
function receiveIMMsg(message, force){
	if(IS_BASE_ON_OPENFIRE && !force && !handleOfflineMsg(message)) {
		return;
	}
    try{
        if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()){
            if(WindowDepartUtil.checkIfHandleMessage(message)){
                handleIMMsg(message);
            }
            WindowDepartUtil.handleIMMsg(message);
        }else{
          handleIMMsg(message); 
        }
    }catch(e){
...
function handleIMMsg(message){
	...
	// localStorage.setItem(targetid+":FULLMSG", message.toJSONString());
	...
	if(message && message.getObjectName() == 'RC:DizNtf'){
		...
		}else if(notificationType==2 &&IS_BASE_ON_OPENFIRE){
            var admins = message.getAdmins();
            if(admins){
				var adminsArray =  admins.split(",");
				if(adminsArray.indexOf(getIMUserId(M_USERID))==-1){
	...
	try{
		...
		var msgcontent=senderName+":"+getMsgContent(message);
	  	if(extraObj.isPrivate == "1"){
			chatWinid = "chatWin_8_"+realTargetid;
			chatTabid="chatTab_7_private";
			targetType  = 7;
			msgcontent = social_i18n('NewMessages');
			PrivateUtil.calculationSeconds(message,msgid);
			if(!IS_BASE_ON_OPENFIRE || ClientSet.ifForbitPrivateChat == "1"){
				return;
			}
		  }
	  	...
	  	if(typeof notiType !=="undefined"&&notiType =="noti_voteDeadline"){
			senderInfo.userHead = "/social/images/sysnotice_head.png";
			 msgcontent ="系统提醒:"+getMsgContent(message);
		  }
		  //次账号的广播
		 if(senderUserid.indexOf("|sysnotice")!=-1&&realTargetid.indexOf("push_")!=-1){
			  msgcontent ="系统提醒:"+getMsgContent(message);
		 }
	  	//显示@提醒
	  	if(!isOfflineMsg){
	            ...
	            if(objName == 'FW:CustomMsg') {
	                if(!(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart())){
                       ShakeWindowUtils.shakeWindow(true, extraObj, senderInfo);
                        ...
                        //窗口抖动需要打开窗口
                        if(extraObj.isPrivate !='1')
                            $('#conversation_' + senderInfo.userid).click();
                    };
	            }
	            // 消息到达是否弹出窗口
	...
	// !!trace end!!
  	...
  	if(isWinAct && senderid!=M_USERID){ 
  		if(!isOfflineMsg && converTempdiv){
			...
			if(targetType == 7){
				//密聊阅读更新
				PrivateUtil.updateConversationList(realTargetid,sendtime);				
				PrivateUtil.setConnverCache(realTargetid,true,msgid);							
			}else{
			      if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()){
                     var win = WindowDepartUtil.getLeftWin();
                     if(win!==''){
                         if(win.isVisible()&&win.isFocused()){
                             if(realTargetid!=WindowDepartUtil.winDepartParm.activeChatWin){
                                 updateTotalMsgCount(realTargetid,true,targetType,1);
                             }
                         }else{
                             updateTotalMsgCount(realTargetid,true,targetType,1);
                         }
  	...
  	if($("#"+chatWinid).length==1){
			...
			chatList.data("newMsgCome",true);
	  		...
	  		if(flag) {
	  			if(!chatWin.is(":visible") || !IMUtil.isWindowActive(window.self)){
		  			...
		  			ChatUtil.postFloatNotice(social_i18n('FloatNotice1'), "normal", options);
		  		}
	  		}
  		}
		...
		if(IMUtil.isWindowActive(window.self)){
			if(versionTag=="rdeploy"&&$("#conversation_"+realTargetid).hasClass("activeChatItem")){
				...
				isWinActive=true;
			}else if($('#chatTab_7_private').hasClass('chatIMTabActiveItem')&&$('#chatTab_7_private').attr('_chatwinid')!=='chatWin_7_private'){
				isWinActive = true;
			}
		}
		if(!ChatUtil.isDisableMsgRead(targetid,targetType)){
            if(isWinActive){
                if(senderid != 'SysNotice'&& senderid != M_USERID) {
                    countMsgidsArray.push(countMsgItem);
                    ChatUtil.savaCountMsg(msgid,senderid,targetType,'send',M_USERID);
                }
            }else{
                //窗口为非激活状态，count消息id暂时存贮
                var realTargetIdTemp = realTargetid;
                if(targetType ==7||targetType==8){
                    realTargetIdTemp = realTargetIdTemp+"|private";
                }
                var localMsgArray=converMsgList[realTargetIdTemp];
                if(!localMsgArray){
                    localMsgArray=new Array();
                }
                //client.error("push msgid:"+msgid);
                localMsgArray.push(countMsgItem);
                converMsgList[realTargetIdTemp]=localMsgArray;
            }
        }
  	}
  	if(isOfflineMsg){  //补偿消息直接发送count消息
        if(senderid != 'SysNotice'&& senderid != M_USERID && !ChatUtil.isDisableMsgRead(targetid,targetType)) {
  		    countMsgidsArray.push(countMsgItem);
            ChatUtil.savaCountMsg(msgid,senderid,targetType,'send',M_USERID);
        }
  	}
  	...
  	if(message && message.getObjectName() == 'RC:DizNtf'){
	  	...
	  	if(senderid!=M_USERID){ //非同步消息
			...
			*/
			CountidsUtil.updateMsgUnreadCount(extraObj.countids,targetType);
			if(targetType==7||targetType ==8){
				realTargetid =realTargetid+"|private";
			}
			CountidsUtil.setMsgidToCIdsCaChe(realTargetid,msgid,true);
            ...
            ChatUtil.initMsgRead(msgid,targetType,receiverids,true,targetid);
	   }
   }
   //Rdeploy版提醒方式
   remindRdeploy(1);
...
function showRelateddiv(extraObj,targetid,targetType,senderinfo,msgcontent){
  	...
  	if(msg_at_userid.indexOf(","+M_USERID+",")!=-1 ||msg_at_userid.indexOf("msg_at_all") !=-1 ){  	
...
/* 
*统计当前所有的未读数，展示在最近中，或者web版本的浮动窗口
*targetid 会话标识
*flag true:更新 targetid 的count条数未读数， false:去除targetid 的未读数
*flag false: ClearUnreadCount消息，或者发送clear消息，清除未读状态，也是一个会话清空一个会话
*count :某个会话加或者减一定数目
*/
function updateTotalMsgCount(targetid,flag,targetType,count){
	var realTargetId = getRealUserId(targetid);
	var conversation = $("#"+ getConverId(targetType, realTargetId));
	targetType = targetType =="-1" ?conversation.attr('_targettype'):targetType;
	var chatTabMsgCount = $("#chatTab_" + targetType + "_" + realTargetId).find(".msgcount");
	var conversationMsgCount = conversation.find(".msgcount");
	var targetTotal  = Number(conversationMsgCount.attr("_msgcount")); 
	targetTotal = targetTotal>0? targetTotal:0;
	if(targetTotal == 0 && !flag){
		//console.log("暂时空着");
		return ;
	}
	...
	if(flag){
		total += Number(count);
		targetTotal += Number(count);
		conversationMsgCount.attr("_msgcount",targetTotal).html( targetTotal > 99 ? "99+":targetTotal).css("display","block");
		chatTabMsgCount.html( targetTotal > 99 ? "99+":targetTotal).css("display","block");
		if(targetTotal<= 0){
			conversationMsgCount.css("display","none");
			chatTabMsgCount.css("display","none");
		}
	}else{
		total -= targetTotal;
		total =  total > 0 ? total : 0;
		conversationMsgCount.attr("_msgcount",0).html(0).css("display","none");
		chatTabMsgCount.html(0).css("display","none");
	}
	//展示的未读数
	var tempTotal = total > 99 ? "99+":total;
	recentmsgcount.attr("_msgcount",total).html(tempTotal);
	if(total <= 0){
		recentmsgcount.css("display","none");
		//托盘闪动提醒 如果total 为0表示全部已读，停止闪动
		if(typeof TrayUtils !=="undefined"){
			TrayUtils.restoreTray();
		}		
	}else{
		if(!recentTab.hasClass("activeitem")){
			recentmsgcount.css("display","block");
		}
	}	
	//web版e-message 未读数显示	
	var immsgdiv=$("#immsgdiv",parent.document);
	if(immsgdiv.length > 0) {
		var unreadMsgCount=$("#unreadMsgCount",parent.document);
		var unreadMsgSpan=$("#unreadMsgSpan",parent.document);
		unreadMsgCount.html(tempTotal);
		if(total>0){
			unreadMsgSpan.show();
	...
	//窗口分离的未读数控制
	if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
            WindowDepartUtil.clearUnreadCount(targetid,tempTotal);
     }
}
...
function getConverdiv(converjs){
	...
	}else if(targettype == 7){//密聊
		targetName = social_i18n('PrivateChat');
		converjs.msgcontent = social_i18n('NewMessages');
		targetHead="/social/images/private_head.png";
	}
    else{
...
if(typeof targetid =='string'&&targetid.indexOf("push_")>=0){
		var targetInfoO = getUserInfo(targetid.substring(targetid.indexOf('_')+1));
		targetHead=targetInfoO.userHead;
		targetName=targetInfoO.deptName+"："+targetInfoO.userName;
	}
...
function getConverTempdiv(targetid,targetType,senderinfo) {
	...
	if(targetType == 8 || targetType == 7){//密聊
		targetHead="/social/images/private_head.png";
		targetName = social_i18n('PrivateChat');
		targetid = 'private_' + M_USERID;
		targetType = 7;
	}
...
function getConverId(targettype,targetid){
	...
	//密聊
	else if (targettype ==7||targettype==8){
            targetid = 'private_' + M_USERID;
		return "conversation_" + targetid;
	}
    else{
...
// 处理类似于必达的消息
function updateFixedConversation(targetid,targettype,content,sendtime) {
	var converId = getConverId(targettype,targetid);
	var converTempdiv=$("#" + converId);
	if(converTempdiv.length==0){
		converTempdiv = getConverTempdiv(targetid, targettype,null);
	}
	converTempdiv.attr("_senderid","");
...
function updateConversationList(sendinfo,targetid,targetType,content,sendtime,syncDb){
	...
	if(typeof targetid =='string'&&targetid.indexOf('push_')>=0){
		jsconverList[0].receiverids = M_USERID;
		jsconverList[0].targettype = 100;
		jsconverList[0].senderid = -1;
		ChatUtil.syncConversToLocal(jsconverList,true);
	}
	if(targetType == 7){//密聊
		jsconverList[0].targetid = "private_"+M_USERID;
		jsconverList[0].receiverids = M_USERID;
		ChatUtil.syncConversToLocal(jsconverList,true);
	}
	converTempdiv.show();
...
function loadLocalConvers(version){
	...
	$.post(url,function(data){
		...
		if(IS_BASE_ON_OPENFIRE){
            // 获取在线状态
            SOCIAL_GLOBAL.OnlineStatus.M_ALLUSERSTATUS = IMUtil.unique(SOCIAL_GLOBAL.OnlineStatus.M_ALLUSERSTATUS.concat(temptTrgetid));
            if(M_SERVERSTATUS){
...
function onRMenuClick(obj, index){
	...
	break;
	case "2":		//删除会话
		if(_targettype == 1 || _targettype == 0||_targettype==6||_targettype==7||_targettype==100){
			updateTotalMsgCount(_targetid,false,_targettype,0);
...
function showConverChatpanel_ncr(obj, _paramsObj){
    var targetid = "",targetName ="",targetHead="",targetType="",count="";
        if($(obj).length > 0){
            ...
            count=_paramsObj.msgCount?parm2.msgCount:0;
       }
       ...
       //主次切换账号    
        ...
        if(typeof targetid=='string'&&targetid.indexOf("push_")>=0){
        if(ChatUtil.isFromPc()){
        var toUserId = targetid.substring(targetid.indexOf('_')+1)
        var toUserName  = getUserInfo(toUserId).userName;
        showImConfirm("确定切换到您的次账号: "+toUserName+"？",function(){
            ...
            $.post(switchUrl, function(data){
                try{
                    ...
                    if(status){
                        ...
                        if(data.message!=''){
                            if(typeof data.isAccountFail != "undefined" && data.isAccountFail =="1"){
                                ChatUtil.removeThisConversation(100, targetid)
                            }
                            showImAlert(data.message);
                        }else{
                            showImAlert('主次账号切换失败');
                        }           
                    }
                }catch(err){
                    console.error(err, "主次账号切换失败");
                    showImAlert('主次账号切换失败');
                }finally{
                    PcModels.reconnect();
                }
    ...
    updateTotalMsgCount(targetid,false,targetType,0);
    ...
    WindowDepartUtil.openNewChatWin(args);
}
...
function showConverChatpanel(obj, _paramsObj){
    if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
        if(M_SERVERSTATUS){
            showConverChatpanel_ncr(obj,_paramsObj);
        }else{
            if(IS_BASE_ON_OPENFIRE){
                OpenFireConnectUtil._remindDisconnection();
            }else{
                if(M_SDK_VER ==2){
	...
	var timerid = setTimeout(function(){
        ...
        setIMChatPanel(targetid,targetType);
        
        // pc对聊天面板的处理
        setIMChatPanelOfPc(targetid,targetType);
        ...
        if(lastActiveChatItem && lastActiveChatItem.length > 0 && lastActiveChatItem.attr('_targetid') !== targetid){
			var lastTargetid = lastActiveChatItem.attr('_targetid');
			var lastTargetType = lastActiveChatItem.attr('_targettype');
			var lastChatwin = $('#chatWin_' + lastTargetType + '_' + lastTargetid);
			if(lastChatwin.length>0){
				ChatUtil.sendClearUnreadCountMsg(lastTargetid, lastTargetType);
			}
        }
        if(targetType==0){
            if(IS_BASE_ON_OPENFIRE){
...
function setIMChatPanel(targetId, targetType){
    if(targetId == 'SysNotice') {
...
// pc端对聊天面板处理
function setIMChatPanelOfPc(targetId, targetType) {
    // pc端对工具栏处理
    var $chatWin = $('#chatWin_' + targetType + '_' + targetId);
    if(ChatUtil.isFromPc()) {
        ...
        }else if(targetType==8& targetId != M_USERID && targetId != 'SysNotice'){
			$chatWin.find('div[type="shakeWindow_div"]').show();
		}
    }
    if(targetType==0){
        if (targetId != M_USERID && targetId != 'SysNotice' && IS_BASE_ON_OPENFIRE == true) {
            $chatWin.find('div[type="private_div"]').show();
        }
        //添加个性签名
        var signatures = "";
        try {
	...
	try{
		...
		if (message && message.getObjectName() == 'RC:DizNtf') {
			...
			if(messageType==1){
				...
				} else if(objName == "FW:Extension_Msg"){
                    var senderId = message ? message.getSenderUserId().split('|')[0] : M_USERID;
                    content = "收到红包 , 请在手机上查看";
                    if(senderId == M_USERID){
                        content = "发出红包 , 请在手机上查看";
                    }
                }else{
				...
				content=social_i18n('WithdrawTip','');  //通知
			}else{
...
function getFwCustomMsgContent(senderId, content, extra) {
    ...
    switch(extra.pushType) {
        case 'weaver_shakeMsg' :
			...
			var userName  = getUserInfo(senderId).userName;
			if(extra.isPrivate==1){
				userName ="对方";
			}
            result.content = extra.receiverids != M_USERID ? '您发送了一个窗口抖动' : userName + ' 抖了您一下';
            break;
    }
    return result;
}
...
function isScrollBottom(chatList){
	...
	var scrollTop = Math.ceil(container.perfectScrollbar("getScrollTop"));
...
function addSendtime(chatList,recorddiv,sendtime,recordType,isLast){
	...
	if(recordType=="send"||recordType=="receive"){
		...
		if(latestSendtime==undefined||(latestSendtime-sendtime)/(1000*60)>2||isLast){
			chatList.prepend("<div class='chatTime'  data-msgid='"+recorddiv.attr('id')+"' id='chatTime_"+sendtime+"'>"+senddate+"</div>");
...
function downAccFile(obj, isDefPath){
	...
	var fileSize = $(obj).attr("_filesize");
	checkFileDelete(fileId, function(data){
		if(data[fileId] == '1'){
			IM_Ext.showMsg("附件已超出"+ClientSet.taskTime+"天，已过期");
		}else{
			var context = (typeof from == 'undefined')?parent.window: self.window;
			if(context.from == 'pc'){
				var item = {};
				item['url'] = context.PcMainUtils.userInfos.currentHost + "/weaver/weaver.file.FileDownload?fileid="+fileId+"&download=1";
				item['filename'] = fileName;
				item['totalbytes'] = fileSize;
				item['fileid'] = fileId;
				context.PcDownloadUtils.willdownload(item, isDefPath);
			}else{
				downloads(fileId);
			}
		}
...
function downloads(fileid){
  ...
  function getDefaultHeadImage(targettype,targetid){
  	var defaultHeadImage = "/messager/images/icon_m_wev8.jpg";
    if(targettype == 0){
        var sex = '0';
        try{
    ...
    if(targettype==1){
        defaultHeadImage='/social/images/head_group.png';
    }
    return defaultHeadImage;
  }
  
  function showIMChatpanel_ncl(chatType, acceptId,chatName,headicon){
      WindowDepartUtil.openIMNewChatWin(chatType,acceptId,chatName,headicon);
  }
  
  //显示聊天面板
  function showIMChatpanel(chatType, acceptId,chatName,headicon) {
  if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
        showIMChatpanel_ncl(chatType, acceptId,chatName,headicon);
  }else{
  	var targettype=chatType;
  	var targetid=acceptId;
  	var defaultHeadImage;
  	defaultHeadImage = getDefaultHeadImage(targettype,targetid);
	...
	$("#chatIMTabs .chatIMTabActiveItem").removeClass("chatIMTabActiveItem");
	 if(chatType == 8 ||chatType == 7){
		$("#chatTab_7_private").attr('_chatWinid',chatWinid);
	  }
  	if($("#"+chatTabid).length==0 || (chatType == 7 && $("#"+chatWinid).length==0)){
		if(chatType == 8){
			$("#chatTab_7_private").addClass("chatIMTabActiveItem");
			chatTabid = "chatTab_7_private";
			acceptId = chatType+"_"+acceptId;
		 }
		if($("#"+chatTabid).length==0){
			var msgcount =  PrivateUtil.getAllUnReadCount()
            var tempMsgcount = msgcount >99 ? "99+":msgcount;
			$("#chatIMTabs").prepend("<div id='"+chatTabid+"' class='chatIMTabItem chatIMTabActiveItem' onclick='changeTabWin(this)' _chatWinid='"+chatWinid+"'>"+
  								...
  								"	<div class='msgcount hoverRight'>"+tempMsgcount+"</div>"+
								"</div>");
			
			if(msgcount>0){
				$("#chatTab_7_private").find(".msgcount").css({"display":"block"});
			}
		}
		...
		}else if(acceptId =="private"){
			chatjspUrl = "/social/im/SocialIMPrivate.jsp";
		}
  		if($("#"+chatWinid).length==0){					
            ...
            if(typeof from != 'undefined' && from == 'pc') {
  			...
  			if(acceptId == "bing"){
				...
				$("#"+chatWinid).css('height','100%');
				$("#"+chatWinid).html("<iframe id ='privateframe' name='privateframe'  frameborder='0' width='100%' height='100%' scrolling ='no'></iframe>");
  				$("#"+chatWinid + " iframe[name='privateframe']").attr('src',url+"&targetName="+chatName);
			} else{
  	...
  	}else if(targettype == 4 || targettype == 3||targettype==6||targettype==7){
	...
	if(!ChatUtil.isFromPc()){
		$("#"+chatWinid).find(".chattop").children(".signatures").css("width","340px");
	}
  ...
  function sendLocalCountMsg(targetid){
  	...
  	if(localMsgArray){
  		for(var i=0;i<localMsgArray.length;i++){
            ...
            ChatUtil.savaCountMsg(localMsgArray[i].msgid,localMsgArray[i].toid,0,'send',M_USERID);
  		}
  ...
  function closeTabWin(obj){
	  ...
	  //清理countids缓存
	  CountidsUtil.deleteCacheByTargetid(targetid);
	  stopEvent();
  }
  ...
  function initIMNavTop(){
  	...
  	$(".imToptab .tabitem").bind("click",function(){
  			...
  			if(target!="recent"){
				var recentmsgcount = $(".imToptab .tabitem .recentmsgcount");
				if(Number(recentmsgcount.attr('_msgcount'))>0){
					recentmsgcount.css('display', 'block');
				}			  	
		  	}
  			//处理联系人列表的滚动条显示
...
function changeChatTopTab(obj,tabtype){
	...
	if(target != "chatList"){
        ...
        //光标获取焦点
		chatdiv.find('.chatcontent').focus();
...
function refreshDiscussName(discuss){
	...
	//再把语义化的空格代码转化为空格
	discussName = discussName.replace(/&nbsp;/ig, " ");
...
function clearContentHtml(obj){
	if($(obj).length===0)return;
 	$(obj)[0].onpaste = function(event){
			...
			var leng=clipboard.items.length;
		 	if(leng==4&&$(obj).attr("_imgCapturer") && (clipboard.items[leng-1].kind == 'file' || clipboard.items[leng-1].type.indexOf('image') > -1)){
				...
				for(var i=0,len=clipboard.items.length; i<len; i++) {
					...
					else if(!ChatUtil.cache.reloadPrevImgHandler && clipboard.items[i].kind == 'string' && clipboard.items[i].type == 'text/plain'){
		...
		// 访问系统粘贴板
		if(isFilePasteEnable)
			ClipboardUtil.queryClipboard(obj, null);
...
function viewIMFile(obj) {
    ...
    if(filetype)
        $(obj).parents(".accitem").attr("_filetype", filetype);
    ...
    //是图片改用图片浏览
        ...
        for(var i = 0; i < accitems.length; ++i){
            ...
            // 消息体异常，过滤undefined
            if(typeof fileid == 'undefined' || fileid == 'undefined'){
                continue;
            }
            imgidPool[fileid] = i;
            ...
            if(data[curfileid] == '1'){
                ...
                for(var i = 0; i < imgidlist.length; ++i){
                    if(data[imgidlist[i]] == '0') {
                        ...
                        imgpool.push("/social/js/imcarousel/image_deleted.png");
                    }
                }
        ...
        checkFileDelete(fileid, function(data){
            if(data[fileid] == '1'){
                ...
                if(ClientSet.isFileToDocs==="1"){
                    $.post("/social/im/SocialIMOperation.jsp?operation=getDocid",{"fileid":fileid},function(data){
                        if($.trim(data)==""){
                            var url=getPostUrl("/social/im/SocialIMFilePreView.jsp?fileid="+fileid);
                        }else{
                            var url=getPostUrl("/docs/docs/DocDsp.jsp?id="+$.trim(data));
                        }
                            ...
                            dialogfile.Width = 700;
                            dialogfile.Height = 500;
...
function cleargroupmsg(obj){
 	var groupa = $(obj).parents('.chatgroupmsga');
 	groupa.hide();
 	groupa.next(".chatList").css("top", "61px");
}
 ...
 function hideChatPopBox(obj){
  ...
  function closeIMdiv(obj){
	...
	function showImAlert(text, callback){
	    if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
	       var args = {
              'title' : social_i18n("SystemAlert"),
              'width' : 320,
              'height' : 150,
              'content': text
            };
            WindowDepartUtil.openNewWindow(args,WindowDepartUtil.NewWindowList.SystemAlert);
            callback(); 
	    }else{
	       setTimeout(function(){
            if(typeof from != 'undefined' && from == 'pc') {
    ...
    if(ChatUtil.isFromPc()&&WindowDepartUtil.isAllowWinDepart()&&WindowDepartUtil.tabIsNull()){
           ...
           var comfirmText = '';
           if(content[1]==='4'){
               openurl = $.base64.encode(content[2]);
               comfirmText = content[3];
               //text = text+'|'+content[3];
           }
           var height = 150;
           if(languageid==="8"&&content[1]==='1'){
               height = 180;
           }
           var args = {
              'title' : social_i18n("SystemComfirm"),
              'width' : 320,
              'height' : height,
              'content' : text,
              'id' : content[1],
            ...
            WindowDepartUtil.openNewWindow(args,WindowDepartUtil.NewWindowList.SystemComfirm); 
        }else{
		setTimeout(function(){
			if(typeof from != 'undefined' && from == 'pc') {
	            DragUtils.closeDrags();
