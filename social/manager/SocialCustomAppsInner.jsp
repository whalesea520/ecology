...
if(HrmUserVarify.checkUserRight("AddSectorInfo:add", user)){
    ...
    RCMenu += "{"+SystemEnv.getHtmlLabelName(132307,user.getLanguage())+",javascript:onSync(this),''} " ;
    RCMenuHeight += RCMenuHeightStep ;
...
<table id="topTitle" cellpadding="0" cellspacing="0" >
	<tr>
		...
		<td class="rightSearchSpan">
			...
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(132307, user.getLanguage()) %>" class="e8_btn_top" onclick="javascript:onSync(this);"/>&nbsp;&nbsp;
...
<input type="hidden" name="method" value="savefieldbatch1">
...
<input type="hidden" value="" name="syncToUserIds">
<input type="hidden" value="" name="syncRowIndexs">
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0"  style="padding-bottom: 30px;">
<tr>
	<td valign="top" width="100%">
			<div id="detaildata">
				<jsp:include page="/social/manager/SocialCustomAppsDetail.jsp"></jsp:include>
			</div>
	</td>
</tr>
</table>
...
<script type="text/javascript">
			...
			switch(j){
				...
				case 2:
					...
					"<div style='display: inline-block;height: 24px;width: 24px;float: left;text-align: center;'>" +
						"<span style='height: 100%;display: inline-block;vertical-align: middle;'></span><img id='navico_"+rowindex+"' src='/social/images/pcmodels/ftb_default_wev8.png' style='max-width: 24px; max-height: 24px;vertical-align: middle;'/>" +
					"</div>" +
				...
				case 3:
					...
					var sHtml = 
					"<div style='display: inline-block;height: 24px;width: 24px;float: left;text-align: center;'>" +
						"<span style='height: 100%;display: inline-block;vertical-align: middle;'></span><img id='navhotico_"+rowindex+"' src='/social/images/pcmodels/ftb_default_h_wev8.png' style='max-width: 24px; max-height: 24px;vertical-align: middle;'/>" +
					"</div>" +
				...
				case 4:
					var oDiv = document.createElement("div");
					oDiv.innerHTML = "<input type='checkbox' name='isopen_"+rowindex+"' value='0' checked='checked' onchange='checkSelect(this,"+rowindex+");setChange("+rowindex+");' >" +
	...
	function onSave(obj){
		...
		//alert("TBVars.syncRowIndexs:"+TBVars.syncRowIndexs);
		//alert("TBVars.syncToUserIds:"+TBVars.syncToUserIds);
		...
		document.weaver.syncToUserIds.value=TBVars.syncToUserIds;
		document.weaver.syncRowIndexs.value=TBVars.syncRowIndexs;
	...
	function onSync(obj) {
		if(jQuery(":checkbox[name='check_select']").nextAll(".jNiceChecked").length == 0){
			window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(132309 ,user.getLanguage())%>");
			return;
		}
		...
		showModalDialogForBrowser(event,'/systeminfo/BrowserMain.jsp?url=/hrm/resource/MutiResourceBrowser.jsp?' +
					'selectedids=','#','resourceid',false,1,'',
			{
				...
				dialogTitle:'人员',
				arguments:'',
				_callback: _selectCallback
			});
		
		function _selectCallback(event, data, name, oldid){
			TBVars.syncToUserIds = data.id;
			var rowindex;
			var cks = jQuery(":checkbox[name='check_select']").nextAll(".jNiceChecked");
	       	for(var i=0; i<cks.length; i++){
	             ...
	             rowindex=checkSelectArray[1];
	             TBVars.syncRowIndexs +=rowindex+",";
	             setChange(rowindex);
