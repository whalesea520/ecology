...
<%
    ...
    if ("delete".equals(method)) {
        RecordSet.executeSql("delete from social_imForbitLogin where id = " + id);
    ...
    // 批量删除
        ...
        RecordSet.executeSql("delete from social_imForbitLogin where id in (" + id + ")");
        response.sendRedirect("/social/manager/SocialForbitLogin.jsp");
        return;
    }
    ...
    // 添加
        ...
        for (String shaveVaue : shaveVauesList) {
            ...
            String selsql = "select id from social_imForbitLogin where contents = '" + shaveVaue + "' and permissionType = "
        ...
        new BatchRecordSet().executeSqlBatch("insert into social_imForbitLogin(contents,permissionType,seclevel,seclevelMax,jobtitleid,joblevel,scopeid)"
                                + " values (?,?,?,?,?,?,?)", paraList);
