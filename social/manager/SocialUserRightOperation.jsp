...
<%
    ...
    if ("delete".equals(method)) {
        RecordSet.executeSql("delete from Social_IMUserRight where id = " + id);
        response.sendRedirect("/social/manager/SocialUserRightGroupChat.jsp");
        return;
    }

    // 批量删除
    else if ("batchDelete".equals(method)) {
        RecordSet.executeSql("delete from Social_IMUserRight where id in (" + id + ")");
        response.sendRedirect("/social/manager/SocialUserRightGroupChat.jsp");
        return;
    }
    ...
    // 添加
        ...
        for (String shaveVaue : shaveVauesList) {
            ...
            String selsql = "select id from Social_IMUserRight where contents = '" + shaveVaue + "' and permissionType = "
        ...
        new BatchRecordSet().executeSqlBatch("insert into Social_IMUserRight(contents,permissionType,seclevel,seclevelMax,jobtitleid,joblevel,scopeid)"
                                + " values (?,?,?,?,?,?,?)", paraList);
