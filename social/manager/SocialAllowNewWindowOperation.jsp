...
<%
    ...
    if ("delete".equals(method)) {
        RecordSet.executeSql("delete from social_imAllowWinDepart where id = " + id);
        response.sendRedirect("/social/manager/SocialForbitLogin.jsp");
        return;
    }

    // 批量删除
    else if ("batchDelete".equals(method)) {
        RecordSet.executeSql("delete from social_imAllowWinDepart where id in (" + id + ")");
        response.sendRedirect("/social/manager/SocialForbitLogin.jsp");
        return;
    }
    ...
    // 添加
        ...
        for (String shaveVaue : shaveVauesList) {
            ...
            String selsql = "select id from social_imAllowWinDepart where contents = '" + shaveVaue + "' and permissionType = "
        ...
        new BatchRecordSet().executeSqlBatch("insert into social_imAllowWinDepart(contents,permissionType,seclevel,seclevelMax,jobtitleid,joblevel,scopeid)"
                                + " values (?,?,?,?,?,?,?)", paraList);
