...
Model = (function() {
  ...
  Model.prototype.query = function(query, callback) {
    ...
    var url=this.context.get_opt("data");
    var query_cash=this.storage.data("query_cash");
    
    if (typeof url === "string"&&query!=""&&query!=query_cash) {
    	if(url.indexOf("?")==-1)
    		url=url+"?q="+query;
    	else
    		url=url+"&q="+query;
    	this.storage.data("query_cash",query);
    	this.context.model.reload(url);
    	data = this.fetch();
    	odata=data;
    }
  ...
  inserting_wrapper: function($inputor, content, suffix) {
    ...
    if ($inputor.is('textarea, input')) {
      ...
      if (/firefox/i.test(navigator.userAgent)) {
        ...
        //wrapped_content = "<span>" + content +"</span>"+suffix;
      }
      if (this.app.document.selection) {
        wrapped_content = "<span contenteditable='true'>" + content + "</span>";
      }
      return wrapped_content+"&nbsp;<span class='deleteTag'></span>";
    }
  }
};
