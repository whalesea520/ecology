...
SWFUpload.prototype.initSWFUpload = function(settings) {
	...
	function replaced_originPlace() {
		...
		if(jQuery("#" + settings.button_placeholder_id).parent().hasClass("chatapp")){
			tempParent.innerHTML = getHTML(0, "24px");
			
		}else{
			tempParent.innerHTML = getHTML(1, "80px");
		}
		targetElement.parentNode.replaceChild(tempParent.firstChild, targetElement);
	};

	function getHTML(isVisib, width) {
		return '<div  id=' + settings.button_placeholder_id + ' style="width:' + width + ';display:inline-block;color:#000;opacity:' + isVisib + ';background: url(/email/images/acc_wev8.png) center center no-repeat;cursor:pointer;" > 添加附件</div>';
	}
	...
	if(getBrowserInfo()) {
		this.newUploader = new plupload.Uploader({
			...
			filters: {
				...
				max_file_size: jQuery("#" + settings.button_placeholder_id).parent().hasClass("chatapp") ? settings.file_size_limit+"kb" : settings.file_size_limit,
...
//						var _this = SWFUpload.instances[this.movieName];
//						//console.log("up1",up,"files",files.length);
//						up.files.splice(settings.file_upload_limit, up.files.length - settings.file_upload_limit);
//						//console.log("up2",up,"files",files.length);
//						if(files.length > 1) {
//							plupload.each(files, function(file) {
//								_this.fileQueued(file);
//							});
//							_this.fileDialogComplete(up.files.length, up.files.length);
//						}
...
SWFUpload.prototype.fileDialogComplete = function(numFilesSelected, numFilesInQueue) {
	this.queueEvent("file_dialog_complete_handler", [numFilesSelected, numFilesInQueue]);
};
...
!function(e, t) {
    ...
    i(E, [u, l, f, h, p, c, w, g, v], function(e, t, n, i, r, o, a, s, u) {
        function c(r) {
            ...
            d = n.get(r.container),
            d || (d = document.body),
            "static" === n.getStyle(d, "position") && (d.style.position = "static"),
            d = f = null ,
            u.call(c),
            e.extend(c, {
                uid: e.guid("uid_"),
