...
String titlename = SystemEnv.getHtmlLabelName(129722,user.getLanguage());//维度设置
String needfav ="1";
String needhelp ="";
...
<form id=form2 name=form2 method=post action="/fna/costCenter/fccDimensionSet/fccDimensionSetInner.jsp">
...
String workflow_type_sqlStr = "CONVERT(VARCHAR, a.id)";
if("oracle".equals(rs.getDBType())){
	workflow_type_sqlStr = "to_char(a.id)";
}

String backfields = "ffd.*, b.labelname";
String fromSql = "from fnaFccDimension ffd \n" +
	" join workflow_browserurl a on ffd.type = "+workflow_type_sqlStr+" \n" +
	" join HtmlLabelInfo b on a.labelid = b.indexid";
String sqlWhere = " where b.languageid = "+user.getLanguage()+" ";
if(!"".equals(nameQuery)){
	sqlWhere += " and (ffd.name like '%"+StringEscapeUtils.escapeSql(nameQuery)+"%') ";
}
String orderby = "ffd.displayOrder, ffd.name";
String sqlprimarykey = "ffd.id";

//out.println("select "+backfields+fromSql+sqlWhere);
tableString =" <table instanceid=\"fccDimensionSetInner_LIST\" "+
			" pagesize=\"10\" tabletype=\"checkbox\" >"+
...
"			<head>";
tableString+="				<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(129735,user.getLanguage())+"\" column=\"name\" orderkey=\"name\" "+//维度显示名
								" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.doJsFunc\" otherpara=\"doOpen+column:id\"/>";
tableString+="				<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(687,user.getLanguage())+"\" column=\"labelname\" orderkey=\"labelname\" />";//表现形式
tableString+="				<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(129737,user.getLanguage())+"\" column=\"fielddbtype\" "+
								" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getFnaFccDimensionFielddbtypeName\" otherpara=\"column:type\" />";//浏览按钮名称
tableString+="			</head>";
...
function addRecord(_id){
	_fnaOpenDialog("/fna/costCenter/fccDimensionSet/fccDimensionSetAdd.jsp?id="+_id,  
			"<%=SystemEnv.getHtmlLabelName(129724,user.getLanguage()) %>", 
			660, 430);
}
...
function doOpen(_id){
	_fnaOpenDialog("/fna/costCenter/fccDimensionSet/fccDimensionSetAdd.jsp?id="+_id,  
			"<%=SystemEnv.getHtmlLabelName(129725,user.getLanguage()) %>", 
			660, 430);
}
...
function onLog(){
	...
	var url = "/systeminfo/SysMaintenanceLog.jsp?operateitem=61410002";//61410002表示类型是成本中心维度设置
