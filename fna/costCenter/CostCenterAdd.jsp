...
int supFccId = Util.getIntValue(request.getParameter("supFccId"),0);
...
String supClassName = "";
if(supFccId > 0){
	String sql = "select name from FnaCostCenter a where a.id = "+supFccId;
	rs.executeSql(sql);
	if(rs.next()){
...
<%@page import="weaver.fna.maintenance.FnaFccDimension"%><HTML><HEAD>
...
<input type="hidden" id="type" name="type" value="<%=type %>" />
	...
	<wea:layout type="2col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'><!-- 基本信息 -->
			...
			<wea:item>
			 	<%=FnaCommon.escapeHtml(supClassName) %>
			 	<input id="supFccId" name="supFccId" value="<%=supFccId %>" type="hidden" />
			</wea:item>
		...
		<wea:group context='<%=SystemEnv.getHtmlLabelName(24664,user.getLanguage())%>' attributes="{'itemAreaDisplay':'display'}"><!-- 关联对象 -->
			...
			<wea:item>
		        <brow:browser viewType="0" name="depId" browserValue='<%=depId.toString() %>' 
		                browserUrl='<%=new BrowserComInfo().getBrowserurl("57")+"%3Fshow_virtual_org=-1%26selecteds=#id#" %>'
		                hasInput="true" isSingle="false" hasBrowser = "true" isMustInput="1"
...
function doEdit2(){
  ...
  try{
		...
		var _data = "operation=add&"+getPostDataByForm("form2");
