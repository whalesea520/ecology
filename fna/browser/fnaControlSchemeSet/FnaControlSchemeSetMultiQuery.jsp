...
String workflowname = Util.null2String(request.getParameter("name"));
...
<FORM id="weaver" NAME="SearchForm" STYLE="margin-bottom:0" action="" onsubmit="return false;" method=post>
	...
	<wea:layout type="4col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(15774,user.getLanguage())%>'><!-- 搜索条件 -->
			...
			<wea:item>
        		<input class="inputstyle" id="name" name="name" value="<%=workflowname%>" />
			</wea:item>
			<wea:item><%=SystemEnv.getHtmlLabelName(33163,user.getLanguage())%></wea:item><!-- 方案编码 -->
			<wea:item>
        		<input class="inputstyle" id="code" name="code" value="<%=workflowname%>" />
			</wea:item>
        </wea:group>
	</wea:layout>
</form>
