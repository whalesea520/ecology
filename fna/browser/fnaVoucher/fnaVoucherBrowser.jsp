...
String xmlName = Util.null2String(request.getParameter("xmlName"));

%>
<BODY style='overflow-x:hidden'>
<jsp:include page="/systeminfo/commonTabHead.jsp">
   <jsp:param name="mouldID" value="fna"/>
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(84672,user.getLanguage()) %>"/>
</jsp:include>
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="/fna/browser/fnaVoucher/fnaVoucherBrowser.jsp" method=post>

<wea:layout type="4col">
	<wea:group context="<%=SystemEnv.getHtmlLabelName(20331, user.getLanguage())%>">
		<wea:item><%=SystemEnv.getHtmlLabelName(195, user.getLanguage())%></wea:item>
		<wea:item>
        	<input class="inputstyle" id="xmlName" name="xmlName" value="<%=xmlName%>" />
		</wea:item>
	</wea:group>
<wea:group context="" attributes="{'groupDisplay':'none'}">
	<wea:item attributes="{'colspan':'full','isTableList':'true'}">
		<% 
			if (!"".equals(xmlName)) {
				sqlwhere += " and xmlName like '%"+StringEscapeUtils.escapeSql(xmlName)+"%'";
			}
			String backfields = " * ";
			String formsql = " from fnaVoucherXml ";
			String orderby  = "xmlName";
			String colString=
			 "<col width=\"0%\" hide=\"true\" transmethod=\"weaver.general.KnowledgeTransMethod.forHtml\" text=\"\" column=\"id\" />"+
			 "<col width=\"40%\" text=\""+SystemEnv.getHtmlLabelName(195, user.getLanguage())+"\" column=\"xmlName\" "+
	 	     			" transmethod=\"weaver.general.Util.toScreenForWorkflow\" />"+
			 "<col width=\"60%\" text=\""+SystemEnv.getHtmlLabelName(433, user.getLanguage())+"\" column=\"xmlMemo\" "+
   						" transmethod=\"weaver.general.Util.toScreenForWorkflow\" />";
			String tableString =
			" <table pageId=\""+PageIdConst.FNA_VOUCHER_BROWSER_LIST+"\" instanceid=\"FNA_VOUCHER_BROWSER_LIST\" tabletype=\"none\" "+
			" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_VOUCHER_BROWSER_LIST,user.getUID())+"\" >"+
...
<input type="hidden" name="pageId" id="pageId" _showCol="false" value="<%=PageIdConst.FNA_VOUCHER_BROWSER_LIST %>"/>
