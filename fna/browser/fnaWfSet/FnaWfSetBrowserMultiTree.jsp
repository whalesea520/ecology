...
String optFrame3 = "/fna/browser/fnaWfSet/FnaWfSetBrowserMultiQuery.jsp";
String optFrame = "";
String type = "";
if(tabId==3){
	type = "query";
...
<div class="zDialog_div_content">
	<FORM id="weaver" NAME="SearchForm" STYLE="margin-bottom:0" action="" onsubmit="return false;" method=post>
		...
		<input type="hidden" name="fnaControlSchemeId" id="fnaControlSchemeId" value="0" />
			
		<input type="hidden" name="changeQueryType" id="changeQueryType" value="" />
		<input type="hidden" name="workflowname" id="workflowname" value="" />
...
function changeQueryType(tabId){
	jQuery("#fnaControlSchemeId").val("0");
...
function showMultiDialog(selectids){
	...
	config.srchead=["<%=SystemEnv.getHtmlLabelName(18104,user.getLanguage())%>"];//流程名称
	config.container =jQuery("#colShow");
    config.searchLabel="";
    config.hiddenfield="id";
    config.saveLazy = true;
    config.srcurl = "/fna/browser/fnaWfSet/FnaWfSetBrowserMultiAjax.jsp?src=src";
    config.desturl = "/fna/browser/fnaWfSet/FnaWfSetBrowserMultiAjax.jsp?src=dest";
    config.pagesize = 10;
...
function initQuery(){
	...
	var workflowname = optFrame.jQuery("#workflowname").val();
	
	jQuery("#workflowname").val(workflowname);
}
