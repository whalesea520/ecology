...
<body scroll="no">
	...
	<div class="e8_boxhead">
        ...
        <div>
			...
			<IFRAME name=frame2 id=frame2 src="/fna/browser/costCenter/singleType/FccBrowserList.jsp?sqltag=<%=sqltag%>&fromHrmStatusChange=<%=fromHrmStatusChange %>&tabid=<%=tabid%>&needsystem=<%=needsystem%>&from=<%=from%>&sqlwhere=<%=xssUtil.put(sqlwhere) %>&workflowid=<%=workflowid %>&fieldid=<%=fieldid %>&selectedid=<%=selectedid %>&fcctype=<%=fcctype %>" class="flowFrame" width=100%  height=345px frameborder=no scrolling="yes">
			//浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。
			</IFRAME>
        </div>
	    </div>
...
<script language=javascript>
 jQuery('.e8_box').Tabs({
	...
	function resetbanner(objid){
		...
		if(objid == 0 ){
		  window.frame1.location="/fna/browser/costCenter/singleType/FccBrowserTree.jsp?sqltag=<%=sqltag%>&needsystem=<%=needsystem%>&from=<%=from%>&fromHrmStatusChange=<%=fromHrmStatusChange%>&sqlwhere=<%=xssUtil.put(sqlwhere) %>&workflowid=<%=workflowid%>&fieldid=<%=fieldid%>&selectedid=<%=selectedid %>&fcctype=<%=fcctype %>";
		  try{
		  	$(curDoc).find("#btnsub").css("display","none");
		  	$(curDoc).find("#btnsub").next().css("display","none");
			}catch(err){}	
		}else if(objid == 2){
			window.frame1.location="/fna/browser/costCenter/singleType/FccBrowserTab2.jsp?sqltag=<%=sqltag%>&needsystem=<%=needsystem%>&from=<%=from%>&fromHrmStatusChange=<%=fromHrmStatusChange%>&sqlwhere=<%=xssUtil.put(sqlwhere) %>&workflowid=<%=workflowid%>&fieldid=<%=fieldid%>&selectedid=<%=selectedid %>&fcctype=<%=fcctype %>";
			try{
