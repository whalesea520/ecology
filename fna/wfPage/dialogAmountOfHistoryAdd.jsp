...
double amountBorrowBefore = Util.getDoubleValue(request.getParameter("amountBorrowBefore"), 0.00);
double amountBorrowAfter = Util.getDoubleValue(request.getParameter("amountBorrowAfter"), 0.00);
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
		...
		<wea:item>
	        <%=df.format(amountBorrowBefore) %>
	        <input id="amountBorrowBefore" name="amountBorrowBefore" value="<%=df.format(amountBorrowBefore) %>" type="hidden" />
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(83240,user.getLanguage())%></wea:item><!-- 调整后借款金额 -->
		<wea:item>
			<wea:required id="amountBorrowAfterSpan" required="true">
	        	<input id="amountBorrowAfter" name="amountBorrowAfter" value="<%=df.format(amountBorrowAfter) %>" type="text" 
	        	 	onchange='checkinput("amountBorrowAfter","amountBorrowAfterSpan");'/>
			</wea:required>
		</wea:item>
...
jQuery(document).ready(function(){
	...
	checkinput("amountBorrowAfter","amountBorrowAfterSpan");
	checkinput("memo1","memo1Span");
	controlNumberCheck_jQuery("amountBorrowAfter", true, 2, true, 18);
});
...
function doSave(_openEditPage){
	var amountBorrowBefore = null2String(jQuery("#amountBorrowBefore").val());
	var amountBorrowAfter = null2String(jQuery("#amountBorrowAfter").val());
	var memo1 = null2String(jQuery("#memo1").val());
	
	if(amountBorrowAfter=="" || memo1==""){
		top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(30702,user.getLanguage()) %>");//必填信息不完整
		return;
	}

	var parentWin = parent.getParentWindow(window);
	parentWin.setTzsmValue("<%=objId %>", amountBorrowBefore, amountBorrowAfter, memo1);
	onCancel();
}
