...
<form id="form2" name="form2" method="post"  action="/fna/report/expense/FnaExpenseDepartmentInner.jsp">
...
<%
	...
	sqlstr =" select tt.*, \n" +
			...
			"  	 where fbft.feetype = 2 and feiIn.status = 1 and feiIn.organizationtype = 2 and feiIn.organizationid = a.budgetorganizationid and (feiIn.occurdate >= '"+fnayearstartdate+"' and feiIn.occurdate <= '"+fnayearenddate+"')) fnaExpenseIn, "+ //实际 收入
			" 	(select sum(feiIn.amount) from FnaExpenseInfo feiIn join FnaBudgetfeeType fbft on feiIn.subject = fbft.id "+
			"  	 where fbft.feetype = 1 and feiIn.status = 1 and feiIn.organizationtype = 2 and feiIn.organizationid = a.budgetorganizationid and (feiIn.occurdate >= '"+fnayearstartdate+"' and feiIn.occurdate <= '"+fnayearenddate+"')) fnaExpenseOut, "+ //实际 支出
    ...
    if(!"".equals(nameQuery) || !"".equals(depId) || !"".equals(subId)){
    	sqlWhere += " and (1=2 ";
		if(!"".equals(nameQuery)){
			sqlWhere += " or (t1.departmentname like '%"+StringEscapeUtils.escapeSql(nameQuery)+"%' or t2.subcompanyname like '%"+StringEscapeUtils.escapeSql(nameQuery)+"%') \n"; 
		}
		if(!"".equals(depId)){
			sqlWhere += " or t1.id in ("+StringEscapeUtils.escapeSql(depId)+") \n"; 
		}
		if(!"".equals(subId)){
			sqlWhere += " or t2.id in ("+StringEscapeUtils.escapeSql(subId)+") \n"; 
		}
		sqlWhere += " ) ";
    }
	...
	String tableString=""+
       "<table instanceid=\"FNA_EXPENSE_DEPARTMENT_INNER_LIST\" pageId=\""+PageIdConst.FNA_EXPENSE_DEPARTMENT_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_EXPENSE_DEPARTMENT_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"none\">"+
       ...
       "<head>"+
			"<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"id\" orderkey=\"t1departmentname\" "+//部门
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getDepNameLink\" />"+
			"<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(141,user.getLanguage())+"\" column=\"t2id\" orderkey=\"t2subcompanyname\" "+//分部
	...
	<wea:layout type="1col">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item attributes="{'isTableList':'true'}">
				<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.FNA_EXPENSE_DEPARTMENT_INNER_LIST %>" />
...
function view_grid(budgetorganizationid){
	var url = "/fna/report/expense/RptFnaBudgetViewInner.jsp?organizationtype=2&organizationid="+budgetorganizationid+"&budgetperiods=<%=budgetperiods %>&goBackUrl=/fna/report/expense/FnaExpenseDepartmentInner.jsp";
	window.open(url);
}
