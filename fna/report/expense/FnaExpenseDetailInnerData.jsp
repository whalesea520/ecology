...
if ("".equals(budgetperiods)) {
    ...
    if (budgetperiods == null || "".equals(budgetperiods)) {
        ...
        if (rs.next()) {
            ...
            //如果未取到，取最大年
            rs.executeProc("FnaYearsPeriods_SelectMaxYear", "");
            if (rs.next()) {
                budgetperiods = rs.getString("id");
            }
        }
...
if (!"".equals(budgetperiods) && !"".equals(organizationid) && !"".equals(organizationtype)) {
    ...
    rs.executeSql(sqlstr);
    if (rs.next()) {
        fnabudgetinfoid = rs.getString("id");
        status = rs.getString("status");
    	revision = rs.getInt("revision");
        
        budgetinfoid = Util.getIntValue(fnabudgetinfoid);
    }
} else {
    canView = false;
}
...
boolean showHiddenSubject = false;
boolean subjectFilter = false;
String separator_FnaSystemSet ="";
int alertvalue_FnaSystemSet = 0;
sql = "select * from FnaSystemSet";
rs.executeSql(sql);
if(rs.next()){
	showHiddenSubject = 1==Util.getIntValue(rs.getString("showHiddenSubject"), 0);
	subjectFilter = 1==Util.getIntValue(rs.getString("subjectFilter"), 0);
	separator_FnaSystemSet = Util.null2String(rs.getString("separator"));
	alertvalue_FnaSystemSet = Util.getIntValue(rs.getString("alertvalue"), 0);
}
if("".equals(separator_FnaSystemSet)){
	separator_FnaSystemSet = "/";
}
...
<%@page import="weaver.systeminfo.systemright.CheckSubCompanyRight"%><html>
<head>
...
Map budgetPeriodMap = BudgetHandler.getBudgetPeriodMap(Util.getIntValue(budgetperiods), feeperiod+"", periodsidKeyList);
...
//得到指定范围内三级科目整期已分配预算
HashMap<String, Map> b3DistributiveBudgetTypeAmountHm = fnaBudgetInfoComInfo.getDistributiveBudgetAmountBySubjects_isEditFeeType(Util.getIntValue(budgetperiods), 
...
while(rs.next()){
	...
	if(!b2id.equals(_b3id)){
		if(b2Archive){
			b2name = FnaCommon.escapeHtml(budgetfeeTypeComInfo.getSubjectFullName(Util.getIntValues(b2id), separator_FnaSystemSet))+"<font color=\"red\">("+SystemEnv.getHtmlLabelName(22151,user.getLanguage())+")</font>";//封存
		}else{
			b2name = FnaCommon.escapeHtml(budgetfeeTypeComInfo.getSubjectFullName(Util.getIntValues(b2id), separator_FnaSystemSet));
		}
...
%>
	<tr>
		<td class="tdRowSplit" rowspan="<%=rowspan %>" style="text-align: left;vertical-align: text-top;">
			<span style="margin-left: 15px;font-weight: bold;"><%=b3name %></span>
			<%=b3ArchiveHtml %>
		</td>
		<td><%=SystemEnv.getHtmlLabelName(386,user.getLanguage()) %></td><!-- 预算 -->
	<%for(int i=1;i<=qCount;i++){
	...
	%>
		<td class="amtTd"><%= df.format(_ysze) %></td>
	<%}
	%>
		<td class="amtTd"><%= df.format(_sum_ysze) %></td>
	</tr>
	<tr>
		<td><%=SystemEnv.getHtmlLabelName(18503,user.getLanguage()) %></td><!-- 实际 -->
	<%for(int i=1;i<=qCount;i++){
        ...
        double _sj = fnaBudgetInfoComInfo.getInComeAmount(organizationid, organizationtype, startdate, enddate, b3id) 
        		- fnaBudgetInfoComInfo.getFeeAmount(organizationid, organizationtype, startdate, enddate, b3id);
        _sj = Util.getDoubleValue(df.format(_sj));

        _sum_sj = Util.getDoubleValue(df.format(_sum_sj + _sj));
	%>
		<td class="amtTd"><%= df.format(_sj) %></td>
	<%}
	
	%>
		<td class="amtTd"><%= df.format(_sum_sj) %></td>
	</tr>
	<tr>
	...
	<%for(int i=1;i<=qCount;i++){
		...
		double _spzys = fnaBudgetInfoComInfo.getApprovingAmount(organizationid, organizationtype, startdate, enddate, b3id); 

		_sum_spzys = Util.getDoubleValue(df.format(_sum_spzys + _spzys));
	%>
		<td class="amtTd"><%= df.format(_spzys) %></td>
	<%}

	%>
		<td class="amtTd"><%= df.format(_sum_spzys) %></td>
	</tr>
	<tr style="font-weight: bold;">
		<td class="tdRowSplit"><%=SystemEnv.getHtmlLabelName(15368,user.getLanguage()) %></td><!-- 偏差 -->
	<%for(int i=1;i<=qCount;i++){
    	String budgetperiodslist = (String)periodsidKeyList.get(i-1);
    	String startdate = (String)budgetPeriodMap.get(budgetperiodslist+"_startdate");
    	String enddate = (String)budgetPeriodMap.get(budgetperiodslist+"_enddate");
    	
		double tmpnum1 = Util.getDoubleValue((String)budgetTypeAmount.get(i+""), 0.00);
    	
        double tmpnum = fnaBudgetInfoComInfo.getInComeAmount(organizationid, organizationtype, startdate, enddate, b3id) 
        		- fnaBudgetInfoComInfo.getFeeAmount(organizationid, organizationtype, startdate, enddate, b3id);

		double tmpnum2 = fnaBudgetInfoComInfo.getApprovingAmount(organizationid, organizationtype, startdate, enddate, b3id); 
		
		double tmpSum2 = 0.00;
		if(tmpnum<0){
			tmpnum=-tmpnum;
		}
		if((tmpnum - tmpnum1)==0||tmpnum1==0) {
			tmpSum2 = 0;
		}else {
			tmpSum2 = (tmpnum - tmpnum1) * 100 / tmpnum1;
		}

		tmpSum2 = Util.getDoubleValue(df.format(tmpSum2));

		String tmpSum2Str = "";
		if (tmpSum2 != 0.00){
			tmpSum2Str = df.format(tmpSum2)+"%";
		}
	%>
		<td class="amtTd tdRowSplit"><%=tmpSum2Str  %></td>
	<%}
	
	double tmpnum1 = _sum_ysze;
	double tmpnum = _sum_sj;
	double tmpnum2 = _sum_spzys;
