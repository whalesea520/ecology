...
<form id="form2" name="form2" method="post"  action="/fna/report/expense/FnaExpenseCrmInner.jsp">
...
<%
	sqlstr = " select a.id, a.name, a.manager, \n" +
			...
			"  where fbft.feetype = 2 and feiIn.status = 1 and feiIn.relatedcrm = a.id and (feiIn.occurdate >= '"+fnayearstartdate+"' and feiIn.occurdate <= '"+fnayearenddate+"')) fnaExpenseIn, \n" +
			" (select sum(feiIn.amount) from FnaExpenseInfo feiIn join FnaBudgetfeeType fbft on feiIn.subject = fbft.id\n" +
			" where fbft.feetype = 1 and feiIn.status = 1 and feiIn.relatedcrm = a.id and (feiIn.occurdate >= '"+fnayearstartdate+"' and feiIn.occurdate <= '"+fnayearenddate+"')) fnaExpenseOut \n" +
			" from CRM_CustomerInfo a where 1=1 ";
	if( !canview ) {
	...
	String tableString=""+
       "<table instanceid=\"FNA_EXPENSE_CRM_INNER_LIST\" pageId=\""+PageIdConst.FNA_EXPENSE_CRM_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_EXPENSE_CRM_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"none\">"+
       ...
       "<head>"+
			"<col width=\"40%\"  text=\""+SystemEnv.getHtmlLabelName(136,user.getLanguage())+"\" column=\"id\" orderkey=\"name\" "+//客户
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getCrmNameLink\" />"+
			"<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(1278,user.getLanguage())+"\" column=\"manager\" "+//客户经理
	...
	<wea:layout type="1col">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item attributes="{'isTableList':'true'}">
				<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.FNA_EXPENSE_CRM_INNER_LIST %>" />
...
function view_grid(budgetorganizationid){
	var url = "/fna/report/expense/Rpt1FnaBudgetViewInner.jsp?organizationtype=crm&organizationid="+budgetorganizationid+"&goBackUrl=/fna/report/expense/FnaExpenseDepartmentInner.jsp";
	window.open(url);
}
