...
%>
	<tr>
		<td class="tdRowSplit" rowspan="5" style="text-align: left;vertical-align: text-top;">
	...
	<%for(int i=1;i<=qCount;i++){
		...
		if(_ysze < _ysyys){
			colorRed1 = "color: red;";
		}else if(Util.getDoubleValue(df.format(_ysze - _ysyys), 0.00) < Util.getDoubleValue(df.format((_ysze / 100) * alertvalue), 0.00)){
			colorRed1 = "color: orange;";
		}
	%>
		<td class="amtTd" style="<%=colorRed1%>"><%= df.format(_ysze) %></td>
	<%}
	
	double _sum_ysyys = Util.getDoubleValue(df.format(_sum_yfpys + _sum_yfsys + _sum_spzys), 0.00);

	String colorRed2 = "";
	if(_sum_ysze < _sum_ysyys){
		colorRed2 = "color: red;";
	}
	%>
		<td class="amtTd" style="<%=colorRed2%>"><%= df.format(_sum_ysze) %></td>
	</tr>
	<tr>
	...
	<tr>
		<td><%=SystemEnv.getHtmlLabelName(628,user.getLanguage()) %></td><!-- 实际 -->
	<%for(int i=1;i<=qCount;i++){
		Expense expense = (Expense)budgetTypeExpense.get(i+"");
		double _yfsys = 0.00;
		if(expense!=null){
	...
	for(int i=1;i<=qCount;i++){
		...
		double _sjYsze = Util.getDoubleValue(df.format((_ysze - _yfpys)), 0.00);
		_sum_sjYsze = Util.getDoubleValue(df.format(_sum_sjYsze + _sjYsze));
		
		double _pc = 0.00;
		if(_sjYsze!=0.00){
			_pc = Util.getDoubleValue(df.format((_sjYsze - _yfsys) * 100 / _sjYsze));
		}
	%>
		<td class="amtTd tdRowSplit"><%= df.format(_pc) %></td>
	<%}
	double _sum_pc = 0.00;
	if(_sum_sjYsze!=0.00){
		Util.getDoubleValue(df.format((_sum_sjYsze - _sum_yfsys) * 100 / _sum_sjYsze));
	}
	%>
		<td class="amtTd tdRowSplit"><%= df.format(_sum_pc) %></td>
	</tr>
<%
	}
}
...
function gotoPage(_addPageIndex){
	...
	window.location.href = "/fna/report/expense/RptFnaBudgetViewData.jsp?<%=commonPara %>&pageIndex="+_pageIndex;
}
...
function jumpPage(_jumpPageIndex, _pageSize){
	...
	window.location.href = "/fna/report/expense/RptFnaBudgetViewData.jsp?<%=commonPara %>&pageIndex="+_jumpPageIndex+"&pageSize="+_pageSize;
}
