...
<%
	...
	sqlstr =" select a.budgetorganizationid, max(a.status) as status, sum(b.budgetaccount) as budgetaccount, " +//c.feetype,  
			" sum(case when (c.feetype=2) then b.budgetaccount else 0.00 end) budgetaccountIn, "+ 
			" sum(case when (c.feetype=1) then b.budgetaccount else 0.00 end) budgetaccountOut, "+
	...
	String tableString=""+
       "<table instanceid=\"FNA_BUDGET_DEPARTMENT_INNER_LIST\" pageId=\""+PageIdConst.FNA_BUDGET_DEPARTMENT_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_BUDGET_DEPARTMENT_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"none\">"+
       ...
       //" sumColumns=\"budgetaccountIn,budgetaccountOut\" decimalFormat=\""+Util.toHtmlForSplitPage("%.2f|%.2f")+"\" "+
       " />"+
       "<head>"+
	...
	<wea:layout type="1col">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item attributes="{'isTableList':'true'}">
				<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.FNA_BUDGET_DEPARTMENT_INNER_LIST %>" />
