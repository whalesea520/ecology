document.write("<script type='text/javascript' src='/fna/costStandard/js/util2_wev8.js?r=3'></script>");
(function($) {
	$.fn.FuzzyQuery = function(options) {
		...
		this.startQuery = function() {
			if (this.options.data) {
				...
				if(this.val()!=""){
					...
					var keyword = URLencode(this.val());
					url += "&searchtype=" + searchtype + "&currentid=" + currentid;
					$.ajax({
						url: url,
						async: true,
						cache: false,
		...
		this.createLoadPanel = function() {
			...
			if((top+num*24)>$(window).height()){//document.body.clientHeight
		...
		this.createQueryPanel = function() {
			...
			var num = 1;
			if(_valid_data!=null){num = _valid_data.length;}
			if((top+num*22)>$(window).height()){//document.body.clientHeight
				//this.show_panel.css("top","auto");
				//this.show_panel.css("bottom",$("#main").height()-$("#"+updatename).offset().top);
				
				this.show_panel.css("top",$("#"+updatename).offset().top-num*22-4);
				this.show_panel.css("bottom","auto");
			}else{
				this.show_panel.css("top",$("#"+updatename).offset().top+$("#"+updatename).height()+4);
				this.show_panel.css("bottom","auto");
				...
				tr = $("<tr/>");
					...
					function() {
						...
						if(updatename=="objname"){
							...
							doSelectSearch(updatename,fieldname,_row_data["id"],_row_data["name"], true);
						}else{
							doSelectUpdate(updatename,fieldname,_row_data["id"],_row_data["name"]);
						}
						me.blur();
					});
				tab.append(tr)
