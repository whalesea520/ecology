...
if(mainId > 0){
	...
	promptSC = FnaLanguage.getPromptSC_FnaBorrowWfSetEditPageLogicSet(user.getLanguage());
}
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(83194,user.getLanguage())%>'><!-- 校验规则 -->
		<wea:item><%=SystemEnv.getHtmlLabelName(83194,user.getLanguage()) %></wea:item><!-- 校验规则 -->
		<wea:item>
			<input id="totalAmtVerification" name="totalAmtVerification" <%=totalAmtVerification==1?"checked=\"checked\"":"" %> value="1" type="checkbox" />
			<%=SystemEnv.getHtmlLabelName(83195,user.getLanguage()) %><!-- 借款金额合计 = 收款金额合计 -->
		</wea:item>
		...
		<wea:item>
			<font color="red">
				...
				#amount1#：<%=SystemEnv.getHtmlLabelNames("1043,358",user.getLanguage()) %><br /><!-- 借款金额合计 -->
				#amount2#：<%=SystemEnv.getHtmlLabelNames("17176,358",user.getLanguage()) %><br /><!-- 收款金额合计 -->
			</font>
		</wea:item><!-- 使用说明 -->
	</wea:group>
</wea:layout>
...
function doSave(_openEditPage){
	...
	try{
		...
		top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(32130,user.getLanguage()) %>",
			function(){
				var _data = "operation=FnaWfSetLogicEditPage&mainId="+mainId+"&workflowid="+workflowid+
					"&intensity="+intensity+"&totalAmtVerification="+totalAmtVerification+
