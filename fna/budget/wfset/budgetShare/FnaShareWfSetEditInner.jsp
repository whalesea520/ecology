...
<form id="form2" name="form2" method="post"  action="/fna/budget/wfset/budgetShare/FnaShareWfSetEditInner.jsp">
...
<%
	...
	String sqlWhere = " where a.fnaWfType = 'share' ";
	if(!"".equals(nameQuery)){
	...
	String tableString=""+
       "<table instanceid=\"FNA_SHARE_WF_SET_EDIT_INNER_LIST\" pageId=\""+PageIdConst.FNA_SHARE_WF_SET_EDIT_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_SHARE_WF_SET_EDIT_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"checkbox\">"+
       ...
       "</head>"+
		"		<operates>"+
		"			<popedom transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getFnaShareWfSetEditInner_popedom\" otherpara=\"column:enable\" ></popedom> "+
	...
	<wea:layout type="1col">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item attributes="{'isTableList':'true'}">
				<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.FNA_SHARE_WF_SET_EDIT_INNER_LIST %>" />
...
function openEditPage(id, param2, param3, tabId, parentWin){
	...
	_fnaOpenDialog("/fna/budget/wfset/budgetShare/FnaShareWfSetEditPage.jsp?id="+id+"&tabId="+tabId, 
			"<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())+SystemEnv.getHtmlLabelName(124980,user.getLanguage()) %>", 
			_w, _h);
}
...
function doAdd(){
	var _w = 600;
	var _h = 360;
	_fnaOpenDialog("/fna/budget/wfset/budgetShare/FnaShareWfSetEditAdd.jsp", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(124980,user.getLanguage()) %>", 
			_w, _h);
}
//新建
function doAddAndCreate(){
	var _w = 720;
	var _h = 580;
	_fnaOpenDialog("/fna/guide/FnaWfCreat.jsp?creatType=share", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(124980,user.getLanguage()) %>", 
			_w, _h);
}
