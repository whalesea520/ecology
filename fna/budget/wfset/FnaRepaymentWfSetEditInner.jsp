...
RCMenuHeight += RCMenuHeightStep ;
    ...
    RCMenuHeight += RCMenuHeightStep ;RCMenu += "{"+SystemEnv.getHtmlLabelName(32136,user.getLanguage())+",javascript:doBatchDel(),_self} ";//批量删除
RCMenuHeight += RCMenuHeightStep ;
%>

<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
<form id="form2" name="form2" method="post"  action="/fna/budget/wfset/FnaRepaymentWfSetEditInner.jsp">
...
<%
	...
	String sqlWhere = " where a.fnaWfType = 'repayment' ";
	if(!"".equals(nameQuery)){
	...
	String tableString=""+
       "<table instanceid=\"FNA_REPAYMENT_WF_SET_EDIT_INNER_LIST\" pageId=\""+PageIdConst.FNA_REPAYMENT_WF_SET_EDIT_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_REPAYMENT_WF_SET_EDIT_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"checkbox\">"+
       ...
       "</head>"+
		"		<operates>"+
		"			<popedom transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getFnaRepaymentWfSetEditInner_popedom\" otherpara=\"column:enable\" ></popedom> "+
...
function openEditPage(id, param2, param3, tabId, parentWin){
	...
	_fnaOpenDialog("/fna/budget/wfset/FnaRepaymentWfSetEditPage.jsp?id="+id+"&tabId="+tabId, 
			"<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())+SystemEnv.getHtmlLabelName(83183,user.getLanguage()) %>", 
			_w, _h);
}
...
function doAdd(){
	var _w = 600;
	var _h = 360;
	_fnaOpenDialog("/fna/budget/wfset/FnaRepaymentWfSetEditAdd.jsp", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(83183,user.getLanguage()) %>", 
			_w, _h);
}
//新建
function doAddAndCreate(){
	var _w = 720;
	var _h = 580;
	_fnaOpenDialog("/fna/guide/FnaWfCreat.jsp?creatType=repayment", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(83183,user.getLanguage()) %>", 
			_w, _h);
}
