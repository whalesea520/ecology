...
String promptSC = FnaLanguage.getPromptSC_FnaChangeWfSetLogicEditPage(user.getLanguage());
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(27955,user.getLanguage())%>'><!-- 触发条件 -->
		...
		<wea:item>
	        ...
	        <brow:browser viewType="0" name="subjectId" browserValue='<%=subjectId %>' 
	                ...
	                browserSpanValue='<%=FnaCommon.escapeHtml(shownameSubject.toString()) %>' width="70%" 
	                >
	        </brow:browser>
		</wea:item>
		...
		<%if(fnaBudgetCostCenter){ %>
            <span id="spanFccId" style="display: none;">
		        <brow:browser viewType="0" name="fccId" browserValue='<%=fccId %>' 
		                ...
		                browserSpanValue='<%=FnaCommon.escapeHtml(shownameFcc.toString()) %>' width="70%" 
		               	>
		        </brow:browser>
		    </span>
		<%} %>
...
function doSave(_openEditPage){
	...
	try{
		...
		top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(32130,user.getLanguage()) %>",
			function(){
				var _data = "operation=FnaWfSetLogicEditPage&mainId="+mainId+"&id="+id+"&workflowid="+workflowid+
