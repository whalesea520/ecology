...
rs.executeSql("select * from FnaRuleSet where id = "+id);
if(rs.next()){
...
int idx = 0;
rs.executeSql("select * from FnaRuleSetDtl where showidtype = 1 and mainid = "+id+" order by id");
while(rs.next()){
...
idx = 0;
rs.executeSql("select * from FnaRuleSetDtl where showidtype = 2 and mainid = "+id+" order by id");
while(rs.next()){
	String showid = Util.null2String(rs.getString("showid")).trim();
	if(idx > 0){
		depids.append(",");
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(33177,user.getLanguage())+SystemEnv.getHtmlLabelName(385,user.getLanguage()) %>"/>
</jsp:include>
...
<wea:layout type="2col">
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(33194,user.getLanguage())%>' attributes="{'itemAreaDisplay':'display'}"><!-- 可编制机构范围 -->
	<%if(fnaBudgetOAOrg){ %>
...
function doEdit(){
	...
	try{
		...
		var _data = "op=add&field6599="+field6599+"&id="+id+
			"&name="+encodeURI(name)+"&oldroleid="+oldroleid+
