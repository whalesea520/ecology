...
if(mainId > 0){
	sql = "select * from fnaFeeWfInfoLogicAdvanceR where mainId = "+mainId;
	rs.executeSql(sql);
	if(rs.next()){
	...
	//"本次预付款冲销金额：#amount1# 必须小于等于 未还金额：#amount2# ！";
	promptSC = FnaLanguage.getPromptSC_FnaWfSetEditPageLogicSetAdvanceReverse(user.getLanguage());
}
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(83194,user.getLanguage())+" 1"%>'><!-- 校验规则 -->
		...
		<wea:item>
			...
			<%=SystemEnv.getHtmlLabelName(128572,user.getLanguage()) %><!-- 报销金额合计 = 预付款冲销金额合计 + 支付金额合计-->
		</wea:item>
...
function doSave(_openEditPage){
	...
	try{
		...
		top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(32130,user.getLanguage()) %>",
			function(){
				var _data = "operation=FnaWfSetEditPageLogicSetAdvanceReverse&mainId="+mainId+"&workflowid="+workflowid+
					"&intensity="+intensity+"&totalAmtVerification="+totalAmtVerification+"&totalAmtVerification2="+totalAmtVerification2+
					"&promptSC="+promptSC+"&__multilangpre_promptSC="+__multilangpre_promptSC;
