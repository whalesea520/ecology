...
<%
RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",javascript:doAddAndCreate(),_self} ";//新建
...
<form id="form2" name="form2" method="post"  action="/fna/budget/FnaWfSetEditInner.jsp">
	...
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right; width:500px!important">
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(82,user.getLanguage()) %>" 
				class="e8_btn_top" onclick="doAddAndCreate();"/><!-- 新建 -->
			...
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(33190,user.getLanguage()) %>" class="e8_btn_top" onclick="doBatchSet1()"/><!-- 批量设置方案 -->
...
<%
	...
	String sqlWhere = " where a.fnaWfType = 'fnaFeeWf' ";
	if(!"".equals(nameQuery)){
	...
	String tableString=""+
       "<table instanceid=\"FNA_WF_SET_EDIT_INNER_LIST\" pageId=\""+PageIdConst.FNA_WF_SET_EDIT_INNER_LIST+"\" "+
      		" pagesize=\""+PageIdConst.getPageSize(PageIdConst.FNA_WF_SET_EDIT_INNER_LIST,user.getUID(),PageIdConst.FNA)+"\" tabletype=\"checkbox\">"+
       ...
       "</head>"+
		"		<operates>"+
		"			<popedom transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getFnaWfSetEditInner_popedom\" otherpara=\"column:enable\" ></popedom> "+
	...
	<wea:layout type="1col">
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item attributes="{'isTableList':'true'}">
				<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.FNA_WF_SET_EDIT_INNER_LIST %>" />
...
function doBatchSet1(){
	...
	if(ids==null||ids==""){
		top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(33191,user.getLanguage()) %>");//请先选择需要批量设置方案的数据
		return;
	}
	
	var _w = 550;
	var _h = 250;
	_fnaOpenDialog("/fna/budget/FnaWfSetBatchSet1.jsp?ids="+ids,
			"<%=SystemEnv.getHtmlLabelName(33190,user.getLanguage()) %>", 
			_w, _h);
}
...
function openEditPage(id, param2, param3, tabId, parentWin){
	...
	_fnaOpenDialog("/fna/budget/FnaWfSetEditPage.jsp?id="+id+"&tabId="+tabId, 
			"<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())+SystemEnv.getHtmlLabelName(83184,user.getLanguage()) %>", 
			_w, _h);
}
...
function doAdd(){
	var _w = 600;
	var _h = 395;
	_fnaOpenDialog("/fna/budget/FnaWfSetEditAdd.jsp", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(83184,user.getLanguage()) %>", 
			_w, _h);
}
//新建
function doAddAndCreate(){
	var _w = 720;
	var _h = 580;
	_fnaOpenDialog("/fna/guide/FnaWfCreat.jsp?creatType=fnaFeeWf", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(83184,user.getLanguage()) %>", 
			_w, _h);
}
