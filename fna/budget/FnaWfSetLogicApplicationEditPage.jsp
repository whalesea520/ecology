...
//"#orgTypeName# #orgName# 科目：#subjectName# #feeDay# 的 预申请流程中，剩余可报销金额为：#budgetavail#，申请金额为：#applyamount#";
String promptSC = FnaLanguage.getPromptSC_FnaWfSetLogicApplicationEditPage(user.getLanguage());

if(id > 0){
	sql = "select * from fnaFeeWfInfoLogic where id = "+id;
	rs.executeSql(sql);
	if(rs.next()){
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(84634,user.getLanguage()) %>"/>
</jsp:include>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(27955,user.getLanguage())%>'><!-- 触发条件 -->
		...
		<wea:item>
			<wea:required id="promptSCspan" required="true">
				<input id="promptSC" name="promptSC" value="<%=promptSC %>" maxlength="2000" style="width: 90%;" class="inputstyle" 
		...
		<wea:item>
			<font color="red">
				...
				#orgTypeName#：<%=SystemEnv.getHtmlLabelNames("83370,195",user.getLanguage()) %><br /><!-- 承担主体类型名称 -->
				#orgName#：<%=SystemEnv.getHtmlLabelNames("83371,195",user.getLanguage()) %><br /><!-- 承担主体名称 -->
				#feeDay#：<%=SystemEnv.getHtmlLabelNames("1491,97",user.getLanguage()) %><br /><!-- 费用日期 -->
				#applyamount#：<%=SystemEnv.getHtmlLabelNames("1491,534",user.getLanguage()) %><br /><!-- 费用金额 -->
...
function doSave(_openEditPage){
	...
	try{
		...
		top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(32130,user.getLanguage()) %>",
			function(){
				var _data = "operation=FnaWfSetLogicApplicationEditPage&mainId="+mainId+"&id="+id+"&workflowid="+workflowid+
					...
					"&depId="+depId+"&hrmId="+hrmId+"&fccId="+fccId+"&promptSC="+encodeURI(promptSC)+"&__multilangpre_promptSC="+encodeURI(__multilangpre_promptSC);
