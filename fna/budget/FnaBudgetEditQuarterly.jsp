...
<div id="quarterbudgetlisttable" style="display:<% if(showtab==2){ %>block<%}else{%>none<%}%>">
...
<INPUT class=text id=quarter_gopage1 style="TEXT-ALIGN: right" size=2 value="<%=topage%>" onChange="document.getElementById('topage').value=this.value;">
...
<THEAD>
    <TR class=Header>
        ...
        <th><input name="quarter_FnaBudgetfeeTypeIDs" type="checkbox"
...
<TD bgcolor="#EFEFEF">
    ...
    <input name="FnaBudgetfeeTypeSaveValueNum" type="hidden" value="4">
    <input name="quarter_FnaBudgetfeeTypeIDs" type="checkbox" value="<%=thirdlevelid%>"
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr height=20 class=datadark><td nowrap align=right>
            <%
                ...
                tmpSum1 += tmpnum;
            %>
            <span id="quarter_<%=thirdlevelid%>_canusedbudget_<%=j%>"
        ...
        <tr height=20 class=datalight><td nowrap align=right>
            <%
                ...
                tmpSum2 += tmpnum;
            %>
            <span id="quarter_<%=thirdlevelid%>_allottedbudget_<%=j%>"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpnum)%></span>
        </td></tr>
        <%}%>
        <tr height=20 class=datadark><td nowrap align=right>
            <%
                ...
                tmpSum3 += tmpnum;
            %>
            <span id="quarter_<%=thirdlevelid%>_originalbudget_<%=j%>"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpnum)%></span>
        </td></tr>
        <tr height=20 class=datalight><td nowrap align=right>
            <%
                ...
                tmpSum5 += tmpnum;
            %>
            <input type="text" name="quarter_<%=thirdlevelid%>_newbudget_<%=j%>" onblur='checknumber("quarter_<%=thirdlevelid%>_newbudget_<%=j%>");onCalculete(this);'
        ...
        <tr height=20 class=datadark><td nowrap align=right>
            <%
                ...
                tmpSum4 += tmpnum;
            %>
            <span id="quarter_<%=thirdlevelid%>_addedbudget_<%=j%>"
...
<TD>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_canusedbudget"><td nowrap align=right>
            <span id="quarter_<%=thirdlevelid%>_canusedbudget_sum"
        ...
        <tr height=20 class=datalight id="msg_<%=thirdlevelid%>_allottedbudget"><td nowrap align=right>
            <span id="quarter_<%=thirdlevelid%>_allottedbudget_sum"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum2)%></span>
        </td></tr>
        <%}%>
        <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_originalbudget"><td nowrap align=right>
            <span id="quarter_<%=thirdlevelid%>_originalbudget_sum"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum3)%></span>
        </td></tr>
        <tr height=20 class=datalight id="msg_<%=thirdlevelid%>_newbudget"><td nowrap align=right>
            <input type="text" name="quarter_<%=thirdlevelid%>_newbudget_sum" onblur="onCalculete(this);"
                   class=InputStyle maxlength=20 style="text-align:right;" size=8
        ...
        <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_addedbudget"><td nowrap align=right>
            <span id="quarter_<%=thirdlevelid%>_addedbudget_sum"
                  style="color:<%=(tmpSum4<0?"RED":"GREEN")%>"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum4)%></span>
            <input type="hidden" id="quarter_<%=thirdlevelid%>_addedbudget_sumhid" name="quarter_<%=thirdlevelid%>_addedbudget_sumhid" value="<%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum4)%>"/>
        </td></tr>
    </table>
</TD>
</TR>
...
<%=SystemEnv.getHtmlLabelName(15323, user.getLanguage())%>
<INPUT class=text id=quarter_gopage2 style="TEXT-ALIGN: right" size=2 value="<%=topage%>" onChange="document.getElementById('topage').value=this.value;">
<%=SystemEnv.getHtmlLabelName(23161, user.getLanguage())%>
</DIV>
