...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(33071,user.getLanguage()) %>"/>
</jsp:include>
	...
	<wea:layout type="2col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
			...
			<wea:item>
				<input id="code" name="code" value="" class="inputstyle" />
			</wea:item>
			...
			<wea:item>
				...
				<%
				sql = "select a.id, a.fnayear, a.status from FnaYearsPeriods a WHERE a.status in (1, -1) ORDER BY a.fnayear DESC ";
				rs.executeSql(sql);
				while(rs.next()){
				...
				%>
					<option value="<%=_id %>" <%=(_status==1)?"selected=\"selected\"":"" %>><%=_fnayear%></option>
				<%
				}
				%>
		        </select>
				...
				while(rs.next()){
					...
					int _fnayear = rs.getInt("fnayear");
				%>
					<option value="<%=_id %>"><%=_fnayear%></option>
				<%
				}
				%>
		        </select>
...
function doSave(_openEditPage){
	...
	hideRightMenuIframe();
	try{
		var _data = "operation=add&name="+encodeURI(name)+"&code="+encodeURI(code)+"&fnayearid="+fnayearid+"&fnayearidEnd="+fnayearidEnd+"&enabled="+enabled+"&FeeWfInfoIds="+FeeWfInfoIds;
