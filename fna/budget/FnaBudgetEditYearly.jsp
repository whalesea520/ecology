...
<div id="yearbudgetlisttable" style="display:<% if(showtab==4){ %>block<%}else{%>none<%}%>">
...
<INPUT class=text id=year_gopage1 style="TEXT-ALIGN: right" size=2 value="<%=topage%>" onChange="document.getElementById('topage').value=this.value;">
...
<THEAD>
    <TR class=Header>
        ...
        <th><input name="year_FnaBudgetfeeTypeIDs" type="checkbox"
...
<TR>
    ...
    <TD bgcolor="#EFEFEF">
        ...
        <input name="FnaBudgetfeeTypeSaveValueNum" type="hidden" value="1">
        <input name="year_FnaBudgetfeeTypeIDs" type="checkbox" value="<%=thirdlevelid%>"
    ...
    <TD>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr height=20 class=datadark><td nowrap><%out.println(SystemEnv.getHtmlLabelName(18604, user.getLanguage()));//上级可用预算%>
            </tr>
            <%}%>
            ...
            <tr height=20 class=datadark><td
                    nowrap><%out.println(SystemEnv.getHtmlLabelName(18569, user.getLanguage()));//原预算额%></td></tr>
            <tr height=20 class=datalight><td
                    nowrap><%out.println(SystemEnv.getHtmlLabelName(18570, user.getLanguage()));//新预算额%></td></tr>
            <tr height=20 class=datadark><td
                    nowrap><%out.println(SystemEnv.getHtmlLabelName(18571, user.getLanguage()));//预算增额%></td></tr>
        </table>
    </TD>
    <%
        tmpSum1 = 0d;
    ...
    <TD>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_canusedbudget"><td nowrap align=right>
                <span id="year_<%=thirdlevelid%>_canusedbudget_sum"
            ...
            <tr height=20 class=datalight id="msg_<%=thirdlevelid%>_allottedbudget"><td nowrap align=right>
                <span id="year_<%=thirdlevelid%>_allottedbudget_sum"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum2)%></span>
            </td></tr>
            <%}%>
            <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_originalbudget"><td nowrap align=right>
                <span id="year_<%=thirdlevelid%>_originalbudget_sum"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum3)%></span>
            </td></tr>
            <tr height=20 class=datalight id="msg_<%=thirdlevelid%>_newbudget"><td nowrap align=right>
                <input type="text" name="year_<%=thirdlevelid%>_newbudget_sum" onblur='checknumber("year_<%=thirdlevelid%>_newbudget_sum");onCalculete(this);'
                       class=InputStyle maxlength=20 style="text-align:right;" size=10
            ...
            <tr height=20 class=datadark id="msg_<%=thirdlevelid%>_addedbudget"><td nowrap align=right>
                <span id="year_<%=thirdlevelid%>_addedbudget_sum"
                      style="color:<%=(tmpSum4<0?"RED":"GREEN")%>"><%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum4)%></span>
                <input type="hidden" id="year_<%=thirdlevelid%>_addedbudget_sumhid" name="year_<%=thirdlevelid%>_addedbudget_sumhid" value="<%=FnaBudgetInfoComInfo.getStrFromDouble(tmpSum4)%>"/>
            </td></tr>
        </table>
    </TD>
</TR>
...
<%=SystemEnv.getHtmlLabelName(15323, user.getLanguage())%>
<INPUT class=text id=year_gopage2 style="TEXT-ALIGN: right" size=2 value="<%=topage%>" onChange="document.getElementById('topage').value=this.value;">
<%=SystemEnv.getHtmlLabelName(23161, user.getLanguage())%>
</DIV>
