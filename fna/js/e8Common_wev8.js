...
String.prototype._fnaEscape4Html=function(){
	return this._fnaReplaceAll("&","&amp;")._fnaReplaceAll('"',"&quot;")._fnaReplaceAll("'","&apos;")._fnaReplaceAll("<","&lt;")._fnaReplaceAll(">","&gt;");
}
String.prototype._fnaEscape4Url=function(){
	return this._fnaReplaceAll("%","%25")._fnaReplaceAll("#","%23")
		._fnaReplaceAll("&","%26")._fnaReplaceAll("\\(","%28")._fnaReplaceAll("\\)","%29")
		._fnaReplaceAll("\\+","%2B")._fnaReplaceAll("/","%2F")._fnaReplaceAll(",","%2C")._fnaReplaceAll(":","%3A")
		._fnaReplaceAll(";","%3B")._fnaReplaceAll("<","%3C")._fnaReplaceAll("=","%3D")._fnaReplaceAll(">","%3E")
		._fnaReplaceAll("\\?","%3F")._fnaReplaceAll("@","%4o")._fnaReplaceAll("\\\\","%5C")._fnaReplaceAll("\\|","%7C")
		._fnaReplaceAll('"',"%22")._fnaReplaceAll(" ","%20")
		;
}
function FnaStringBuffer(){this.buffer=[];}
FnaStringBuffer.prototype.append=function append(string){this.buffer.push(string);return this;};
FnaStringBuffer.prototype.toString=function toString(){return this.buffer.join("");};

(function(jQuery){ 
	jQuery.__fnaScrollFixed = function(el, options){
		var base = this; 
		base.$el = jQuery(el); 
		base.el = el; 
		var target = base.$el; 
		...
		var _fix_position = function(){ 
			if(base.options.fixed == 'right'){
				target.css('right', (jQuery(window).scrollLeft() + jQuery(window).width() - jQuery(document).width() + original_right) + 'px'); 
			} else if(base.options.fixed == 'left'){ 
				target.css('left', (original_left - jQuery(window).scrollLeft()) + 'px'); 
			} 
		}; 
		...
		base.init = function(){ 
		base.options = jQuery.extend({}, jQuery.__fnaScrollFixed.defaultOptions, options); 
		jQuery(window).resize(windowResize); 
		jQuery(window).scroll(windowScroll); 
		_fix_position(); 
	...
	jQuery.__fnaScrollFixed.defaultOptions = { 
		fixed:'left' 
	}; 
	
	jQuery.fn.__fnaScrollFixed = function(options){ 
		return this.each(function(){ 
			(new jQuery.__fnaScrollFixed(this, options)); 
		}); 
	}; 
})(jQuery);
...
//切换美化checkbox是否选中
function __fna___changeCheckboxStatus4tzCheckBox(obj, checked) {
	var jObj = jQuery(obj);
	jObj.attr("checked", checked);
	if (checked) {
		jObj.next("span.tzCheckBox").addClass("checked");
		jObj.next("span.jNiceCheckbox").addClass("jNiceChecked");
	} else {
		jObj.next("span.tzCheckBox").removeClass("checked");
		jObj.next("span.jNiceCheckbox").removeClass("jNiceChecked");
	}
}
function fnaUrlEncoded(_s){
	return _s._fnaEscape4Url();
}
function getPostDataByForm(formId){
	var postDataByForm = new FnaStringBuffer();
	var formObj = jQuery("form[id='"+formId+"']");
	var objArray = formObj.find("input");
	var objArrayLen = objArray.length;
	for(var i=0;i<objArrayLen;i++){
		var obj=jQuery(objArray[i]);
		var postName=null2String(obj.attr("name"));
		if(postName!=""){
			if(obj.attr("type")=="checkbox"){
				if(obj.attr("checked")){
					postDataByForm.append("&"+postName+"="+fnaUrlEncoded(obj.val()));
				}
			}else{
				postDataByForm.append("&"+postName+"="+fnaUrlEncoded(obj.val()));
			}
	...
	objArray = formObj.find("select");
	objArrayLen = objArray.length;
	for(var i=0;i<objArrayLen;i++){
		var obj=jQuery(objArray[i]);
		var postName=null2String(obj.attr("name"));
		if(postName!=""){
	...
	objArray = formObj.find("textarea");
	objArrayLen = objArray.length;
	for(var i=0;i<objArrayLen;i++){
		var obj=jQuery(objArray[i]);
		var postName=null2String(obj.attr("name"));
	...
	return postDataByForm.toString();
}
...
function setWfMainAndDetailFieldSpanValueForMobile1(valueForSpan, fieldId, isDtl, _idx){
    if(isDtl=="1"){
        jQuery("#field"+fieldId+"_"+_idx+"_span_d").html(valueForSpan);
        return;
    }else{
...
function setWfMainAndDetailFieldValueForPc(valueForField, fieldId, isDtl, _idx){
	if(isDtl=="1"){
		...
		try{
			if(jQuery("#field"+fieldId+"_"+_idx).attr("viewtype")=="1"){
				try{window.checkinput("field"+fieldId+"_"+_idx,"field"+fieldId+"_"+_idx+"span");}catch(ex1){}
				try{window.checkinput("field"+fieldId+"_"+_idx,"field"+fieldId+"_"+_idx+"spanimg");}catch(ex1){}
			}
		}catch(ex1){}
		return;
	}else{
		...
		try{
			if(jQuery("#field"+fieldId).attr("viewtype")=="1"){
				try{window.checkinput("field"+fieldId,"field"+fieldId+"span");}catch(ex1){}
				try{window.checkinput("field"+fieldId,"field"+fieldId+"spanimg");}catch(ex1){}
			}
		}catch(ex1){}
		return;
	}
...
function controlNumberCheck_jQuery(id, isDouble, pointLength, allowNegative, maxlength){
	jQuery("#"+id).bind("keypress", function(){numberCheck(document.getElementById(id), isDouble, pointLength, allowNegative, maxlength)});
	jQuery("#"+id).bind("keyup", function(){numberCheck2(document.getElementById(id), isDouble, pointLength, allowNegative, maxlength)});
	jQuery("#"+id).bind("mouseout", function(){numberCheck2(document.getElementById(id), isDouble, pointLength, allowNegative, maxlength)});
	jQuery("#"+id).bind("blur", function(){numberCheck2(document.getElementById(id), isDouble, pointLength, allowNegative, maxlength)});
}
/*限制输入时，只允许输入指定类型的数字
 * onkeypress*/
