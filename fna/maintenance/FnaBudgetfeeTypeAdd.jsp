...
sql = "select feelevel, feeperiod from FnaBudgetfeeType a where a.id = "+supsubject;
rs.executeSql(sql);
if(rs.next()){
	supfeelevel = Util.getIntValue(rs.getString("feelevel"), 3);
	feeperiod = Util.getIntValue(rs.getString("feeperiod"));
}
...
int feetype = 0;
String agreegap = "";
int feelevel = supfeelevel+1;
String alertvalue = "";
int archive = 0;
int feeCtlLevel = 0;
double displayOrder= 0.0;

sql = "select max(displayOrder) maxDisplayOrder from FnaBudgetfeeType a where a.supsubject = "+supsubject;
rs.executeSql(sql);
if(rs.next()){
	displayOrder = Util.getDoubleValue(df3.format(Util.getDoubleValue(rs.getString("maxDisplayOrder"), 0)+1));
	if(displayOrder > 999.999){
		displayOrder = 999.999;
	}else if(displayOrder < -999.999){
		displayOrder = -999.999;
	}
}
...
boolean groupCtrl = true;
if(feelevel==3){
	rs.executeSql("select "+FnaCostCenter.getDbUserName()+"fnaGetGroupCtrlKmId(a.id) groupctrlKmId \n" +
			" from FnaBudgetfeeType a \n" +
			" where a.id = "+supsubject);
	if(rs.next() && rs.getInt("groupctrlKmId") > 0){
		groupCtrl = false;
	}
}
...
if(subjectFilter){
	BudgetfeeTypeComInfo.loadFnabudgetfeetypeRuleSet(supsubject, allowZbBooleanStr, depids, depnames, fbids, fbnames, fccids, fccnames);
}
...
%>
			...
			<wea:item attributes="{'samePair':'budgetCanBeNegative1'}">
	            <input id=budgetCanBeNegative name="budgetCanBeNegative" value="1" type="checkbox" tzCheckbox="true"/>
			</wea:item>
			
			<wea:item attributes="{'samePair':'groupCtrlItem'}"><%=SystemEnv.getHtmlLabelName(128826,user.getLanguage())%></wea:item><!-- 可编制预算 -->
			<wea:item attributes="{'samePair':'groupCtrlItem'}">
				<input id="isEditFeeType" name="isEditFeeType" value="1" type="checkbox" tzCheckbox="true" />
			</wea:item>
			...
			<wea:item attributes="{'samePair':'isEditFeeType1'}">
				<input id="groupCtrl" name="groupCtrl" value="1" type="checkbox" onclick="groupCtrl_onclick();" tzCheckbox="true" />
			</wea:item>
...
function doEdit2(obj){
	try{
		...
		var _data = "operation=add&_guid1="+_guid1+"&id="+id+
			...
			"&groupCtrl="+groupCtrl+"&isEditFeeType="+isEditFeeType+
