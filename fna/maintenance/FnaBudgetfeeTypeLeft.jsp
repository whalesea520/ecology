	...
	<tr>
		<td class="leftTypeSearch">
			<div>
				...
				<!-- e8InitTreeSearch({ifrms:'#optFrame',formID:'#frmmain',conditions:'#subjectId'}); -->
				<span><%=SystemEnv.getHtmlLabelName(1462,user.getLanguage()) %></span><!-- 预算科目 -->
...
<FORM id=weaver name=frmmain action="/fna/maintenance/FnaBudgetfeeTypeView.jsp" method=post STYLE="margin-bottom:0" target="optFrame">
...
function e8_custom_search_for_tree(_searchStr_lowerCase){
	...
	if(_searchStr_lowerCase==null){
		_searchStr_lowerCase = "";
	}
	setting = {
			async: {
				enable: true,
				url:"/fna/maintenance/FnaBudgetfeeTypeLeftAjax.jsp",
				autoParam:["id"],
				otherParam:{"otherParam":"0","_searchStr_lowerCase":_searchStr_lowerCase},
				dataFilter: filter
			},
			callback: {
...
function __e8InitTreeSearch(){
	...
	parent.document.getElementById("optFrame").src = "/fna/maintenance/FnaBudgetfeeTypeView.jsp";
}
...
var global_clickId = "";
function do_reAsyncChildNodes(refresh_nodeId){
	var treeObj = jQuery.fn.zTree.getZTreeObj("fnaWfTree");
	if(refresh_nodeId==null){
		treeObj.reAsyncChildNodes(null, "refresh");
	}else{
		var nodes = treeObj.getNodesByParam("id", refresh_nodeId, null);
