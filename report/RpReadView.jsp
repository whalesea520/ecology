...
<%
    ...
    if(hasTab.equals("")){
    	response.sendRedirect("/docs/report/DocReportTab.jsp?_fromURL=11");
    	return;
    }
...
String reader = Util.null2String(request.getParameter("reader")) ;
...
String creatername = "";
String whereclause = " where operatetype='0'";
if(!fromdate.equals("")) {
...
String resourcename = "";
if(!reader.equals("")){
	whereclause+="and operateuserid in ("+reader+")";
	String[] tmpArr = reader.split(",");
	for(int i=0;i<tmpArr.length;i++){
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
	<FORM id=report name=report action="RpReadView.jsp?hasTab=1" method=post>
	
	<wea:layout type="4col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
	    <wea:item><%=SystemEnv.getHtmlLabelNames("33794",user.getLanguage())%></wea:item>
	    <wea:item>
	    ...
	    <wea:item><%=SystemEnv.getHtmlLabelNames("33795",user.getLanguage())%></wea:item>
	    <wea:item>
	    <brow:browser viewType="0" name="reader" browserValue='<%= ""+reader %>' 
				browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/MultiResourceBrowser.jsp?resourceids="
				hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
				completeUrl="/data.jsp" 
	    ...
	    <wea:item><%=SystemEnv.getHtmlLabelName(31971,user.getLanguage())%></wea:item>
	    <wea:item>
...
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
			...
			<%
				//设置好搜索条件				
				String tableString=""+
					   "<table pagesize=\""+PageIdConst.getPageSize(PageIdConst.DOC_READDOCLOGSTA,user.getUID(),PageIdConst.DOC)+"\" tabletype=\"none\">"+
					   ...
					   "<head>";
					   		tableString +=	 "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(33795,user.getLanguage())+"\" column=\"operateuserid\"  transmethod=\"weaver.splitepage.transform.SptmForDoc.getName\" otherpara=\"column:usertype\" orderkey=\"operateuserid\" />";
					   		tableString+=	 "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(31971,user.getLanguage())+"\" column=\"operatedate\"  transmethod=\"weaver.splitepage.transform.SptmForDoc.getOperateTime\" otherpara=\"column:operatetime\"  orderkey=\"operatedate,operatedatetime\" />";
					   		tableString += "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(33794,user.getLanguage())+"\" column=\"docid\" transmethod=\"weaver.splitepage.transform.SptmForDoc.getDocDownLogURL\" otherpara=\"column:docsubject\" orderkey=\"docsubject\" />";
					   		tableString += "<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(18582,user.getLanguage())+"\" column=\"doccreater\" transmethod=\"weaver.splitepage.transform.SptmForDoc.getName\" otherpara=\"column:usertype\" orderkey=\"doccreater\" />";
					   tableString += "</head>"+
					   "</table>";      
			  %>
			  <input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.DOC_READDOCLOGSTA %>"/>
