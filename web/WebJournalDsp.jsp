        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" valign=top>
          <tr>
            <td width="100%" valign=top> 
				  <table width="100%" >
				  <form id="journalForm" name="journalForm" action="/web/WebJournalDsp.jsp">
				  ...
				  <table class=form valign=top>					 
						  ...
						  <%
							...
							if (DocManager.getCount()==0) 
								{
								if (language==7)	remindMsg = "该栏目目前还没有任何资料！" ;
							...
							<table width="90%" border="0" cellspacing="0" cellpadding="0">
							  <tr> 
								...
								%>
									...
									<img border=0 width="80" src="/weaver/weaver.file.FileDownload?fileid=<%=tmppicid%>"> 
									</a> 
								<% }else if(defaultimgid!=0){ %>
									<a href="<%=webServer%>/web/WebDetailDsp.jsp?id=<%=docid%>&languageid=<%=languageid%>&isTime=<%=isTime%>" target="_top"> 
									<img border=0 width="80" src="/weaver/weaver.file.FileDownload?fileid=<%=defaultimgid%>"> 
									</a> 
								<%}%>

								 </td>
								 <td width="80%" class="p91" valign="top">
								 <strong>
								 <a href="<%=webServer%>/web/WebDetailDsp.jsp?id=<%=docid%>&languageid=<%=languageid%>&isTime=<%=isTime%>" target="_top">
								 <%=docsubject%>
								 </a>
								 ...
								 %>
								 <%if(isTime.equals("1")){%>[<%=doclastmoddate%>]&nbsp;&nbsp;<%}%><%=disptmp%>&nbsp&nbsp
								 <%if(curimgid>0){%>&nbsp;&nbsp;[图文]<%}%>
								 <BR><img border="0" src="/web/images/icon_arrow_wev8.gif">&nbsp;<a href="<%=webServer%>/web/WebDetailDsp.jsp?id=<%=docid%>&languageid=<%=languageid%>&isTime=<%=isTime%>" target="_top">
								 <%=SystemEnv.getHtmlLabelName(361,languageid)%>>></a> 
								 <br><br> 
								 </td>
							 </tr>
						  </table>
						...
						<tr> 
							<td height="1" colspan="3" background="/web/images/bg_hdotline_wev8.gif" class="p9grey"><img src="/web/images/spacer_wev8.gif" width="1" height="1"></td>
						</tr>
          ...
          <tr>
			  ...
			  <td align=center  noWrap > 
					<%linkstr = "/web/WebJournalDsp.jsp?id="+id+"&hstart="+hstart+"&journalListID="+journalListID+"&isTime="+isTime;%>
