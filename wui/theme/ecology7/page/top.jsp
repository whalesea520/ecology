...
<tr>
	<td valign="center" width="461px">
		...
		<div class="topMenuDiv_top">
			...
			<div class="menuItem" style="" onClick="dymlftenu(this);topMenuContraction();" title='<%=SystemEnv.getHtmlLabelName(31753,user.getLanguage())%>'>
			    <DIV class="menuItemIcon" style="background-position:0px 0px;"></DIV>
				<DIV class="slideItemText" style="padding-left:15px;text-align:center;"><%=SystemEnv.getHtmlLabelName(31753,user.getLanguage())%></DIV>
			</div>
			<%
			...
			<div class="menuItem" style="" onClick="clickstatictics(this);" levelid="<%=map.get("levelid")%>" id="<%=map.get("id")%>" target="<%=map.get("target")%>"  title="<%=map.get("name")%>">
				...
				<DIV class="slideItemText" style="padding-left:15px;text-align:center;"><%=tempName %></DIV>
			</div>
			<%
				topLine++;
			}
...
function topMenuExpand() {
	...
	jQuery(".topMenuDiv").css("filter", "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='/wui/theme/ecology7/skins/<%=skin %>/page/top/menu_expand_bg_wev8.jpg',sizingMethod='scale')");
