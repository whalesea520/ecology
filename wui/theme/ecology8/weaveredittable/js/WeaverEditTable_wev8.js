...
//                    ];
                    ...
                    for(var i=0;i<rsdata.length;i++)
                    {
                        ...
                        for(var m=0;m<entry.length;m++){
                            if(entry[m].type == "select" ){
                                 if(entry[m].fIndex && entry[m].fsecIndex){
                                    if(typeof(showTreenodeid) == "function"){
                                    	showTreenodeid(entry[m].fIndex,entry[m].fsecIndex,i);
                                    }
                                 }
                            }
                         }
            ...
            for(var i=0;i<checkeditems.length;i++)
            {
                 ...
                 */
				 if(!that.options.cloneConfigCheckbox){
					...
					trclone.find(".jNiceWrapper:first").remove();
    ...
    WeaverEditTable.prototype.registerEvents=function()
    {
        ...
        this.container.find("input[name='_allselectcheckbox']").click(function(){
			var groupselectboxs = that.container.find("table").find(".groupselectbox");
            if(jQuery(this).is(':checked'))
            {
                ...
                groupselectboxs.trigger("checked",true);
            }
            else
            {
                ...
                groupselectboxs.trigger("checked",false);
            }
	  ...
	  WeaverEditTable.prototype.getTableJson=function()
    {
       ...
       trs.each(function(){
           ...
           {
        	   csb.append("{\""+jQuery(inputTexts[i]).attr("name")+"\":\""+jQuery(inputTexts[i]).val().replace(/\\/g,"`#weaver#`")+"\"},");
           }
		   //隐藏域
