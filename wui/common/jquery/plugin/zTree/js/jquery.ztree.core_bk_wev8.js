...
(function($){
		...
		appendNodes: function(setting, level, nodes, parentNode, initFlag, openFlag) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				if (openFlag) {
					...
					html.push("<li id='", node.tId, "' class='level", node.level," ",(node.level>1?"e8_z_seclevel":"e8_z_toplevel"),"' treenode>","<div style='padding-left:"+paddingLeft+"px'>",
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			try{
				window.setTimeout(function(){
					jQuery("#overFlowDivTree .ulDiv").height(jQuery(".ztree").height());
	...
	$.fn.zTree = {
		...
		init: function(obj, zSetting, zNodes) {
			var div = "<div style=\"overflow:hidden;height:300px;position:relative;\" id=\"overFlowDivTree\"><div class=\"ulDiv\" style='display:none;' >"+
						"<div id='hiddenImage' style='display:none;'><img src='/images/xp_none/Tplus_wev8.png' onload='prefectScrollBar();'/></div></div></div>";
			
			obj.parent().append(jQuery(div));
			jQuery("div#overFlowDivTree").children("div.ulDiv:first").append(obj);
...
function prefectScrollBar(){
	jQuery("#overFlowDivTree .ulDiv").show();
	...
	if(!!height){
		...
		if(!!overFlowDiv && overFlowDiv.length>0){
			...
			jQuery("#overFlowDivTree").height(jQuery(window).height()-43);
			jQuery("#btncancel").closest("td").show();
		}else{
			jQuery("#overFlowDivTree").height(jQuery(window).height()-jQuery("td.leftTypeSearch").height()-5);
		}
		window.setTimeout(function(){
			jQuery("#overFlowDivTree .ulDiv").height(jQuery(".ztree").height());
