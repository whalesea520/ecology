...
(function($){
	...
	view = {
		...
		appendNodes: function(setting, level, nodes, parentNode, initFlag, openFlag) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				if (openFlag) {
					...
					var paddingLeft = 18*level;
					html.push("<li id='", node.tId, "' class='level", node.level," ",(node.level>1?"e8_z_seclevel":"e8_z_toplevel"),"' treenode>","<div class='e8HoverZtreeDiv' style='padding-left:"+paddingLeft+"px'>",
					...
					html.push("<button type='button' id='", node.tId, consts.id.ICON,
						...
						"' class='",node.fontClass,"'>",node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span>");
					if(i == 0 && setting.view.addFirstNodeAttr && setting.view.addFirstNodeAttr!==""){html.push(setting.view.addFirstNodeAttr);setting.view.addFirstNodeAttr = null;}
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			if (tools.isArray(setting.asyncParamOther)) {
				...
				for (var p in setting.async.otherParam) {
					if(typeof(setting.async.otherParam[p])=="function")continue;
			...
			$.ajax({
				...
				success: function(msg) {
					...
					tools.apply(callback);
					try{
						__zTreeNamespace__.prefectScrollBarForZtree();
					}catch(e){}
				},
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					$("#" + list[j].tId + consts.id.A).parent("div").removeClass(consts.node.CURSELECTED);
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			try{
				window.setTimeout(function(){
					jQuery("#overFlowDivTree .ztreeUlDiv").height(jQuery(".ztree").height());
					jQuery('#overFlowDivTree').perfectScrollbar("update");
				},300);
			}catch(e){
				if(window.console){
					console.log("/wui/common/jquery/plugin/zTree/js/jquery.ztree.cor.js-->"+e);
				}
			}
		},
		...
		makeNodeIcoStyle: function(setting, node) {
			...
			if (!node.isAjaxing) {
				...
				if (icon && icon!="") icoStyle.push("background:url(", icon, ") 0 0 no-repeat;");
				if (setting.view.showIcon == false || !tools.apply(setting.view.showIcon, [setting.treeId, node], true) || icon==null || icon=="") {
		...
		makeNodeLineClass: function(setting, node) {
			...
			if (setting.view.showLine) {
				...
				} else if (node.level == 0 && node.isFirstNode && node.isParent!=false) {
		...
		selectNode: function(setting, node, addFlag) {
			...
			if(node[setting.data.key.rightKey]!=="N" || node[setting.data.key.selectRight]){
				$("#" + node.tId + consts.id.A).addClass(consts.node.CURSELECTED);
				$("#" + node.tId + consts.id.A).parent("div").addClass(consts.node.CURSELECTED);
	...
	$.fn.zTree = {
		...
		init: function(obj, zSetting, zNodes) {
			var div = "<div style=\"overflow:hidden;height:708px;position:relative;\" _count=0 id=\"overFlowDivTree\"><div class=\"ztreeUlDiv\" style='display:none;' >"+
						"<div id='hiddenImage' style='display:none;'><img src='/images/xp_none/Tplus_wev8.png' onload='__zTreeNamespace__.prefectScrollBarForZtree();'/></div></div></div>";
			
			obj.parent().append(jQuery(div));
			jQuery("div#overFlowDivTree").children("div.ztreeUlDiv:first").append(obj);
...
var __zTreeNamespace__ = (function(){
	return (function(){
		return {
			e8HasData:function(){
				if(window._treeLoaded){
					if(jQuery(".ztree").children("li").length==0||jQuery(".ztree").children("li:visible").length==0){
						var emptyinfo = jQuery("#e8emptyInfo");
						if(emptyinfo.length==0){
							emptyinfo=jQuery("<div id='e8emptyInfo'></div>");
							var ulDiv = jQuery("#overFlowDivTree .ztreeUlDiv")
					...
					setTimeout(function(){e8HasData();},100);
				}
			},
			prefectScrollBarForZtree:function(){
				var overflowDivTree = jQuery("#overFlowDivTree");
				var cc = parseInt(overflowDivTree.attr("_count"))+1;
				overflowDivTree.attr("_count",cc)
				var ulDiv = jQuery("#overFlowDivTree .ztreeUlDiv")
				ulDiv.show();
				//console.log(jQuery(".ztree").children("li").length+"::"+jQuery(".ztree").children("li").css("display"));
				
				var height = jQuery("#overFlowDivTree").closest("td.flowMenusTd").height();
				...
				if(e8_searching.length==0){
					...
					e8_searching.css({
						//top:height/2,
						width:jQuery("#overFlowDivTree").closest("td.flowMenusTd").width()-12
					});
				}
				if(cc>50){
					e8HasData();
				...
				if(!height){
					 height = jQuery(".ztree").height();
				}
				if(!!height){
					var overFlowDiv = null;//jQuery("div.ps-container");
					if(!!overFlowDiv && overFlowDiv.length>0){
						if(window.FIXTREEHEIGHT){
							jQuery("#overFlowDivTree").height(window.FIXTREEHEIGHT);
						}else{
							jQuery("#overFlowDivTree").height(overFlowDiv.height());
						}
						overFlowDiv.children("div").height(overFlowDiv.height());
					}else if(jQuery("#btncancel").length>0){
						if(!window.E8EXCEPTHEIGHT){window.E8EXCEPTHEIGHT=0;}
						if(window.FIXTREEHEIGHT){
							jQuery("#overFlowDivTree").height(window.FIXTREEHEIGHT);
						}else{
							jQuery("#overFlowDivTree").height(jQuery(window).height()-43-window.E8EXCEPTHEIGHT);
						}
						...
						if(window.FIXTREEHEIGHT){
							...
							if(!window.E8EXCEPTHEIGHT){window.E8EXCEPTHEIGHT=0;}
							jQuery("#overFlowDivTree").height(jQuery(window).height()-65-window.E8EXCEPTHEIGHT);
						}
					}
					window.setTimeout(function(){
						updateZtreeUlHeight();
					...
					if(cc<=50){
						window.setTimeout(function(){
							prefectScrollBarForZtree();
						},300);
					}
				}
			},
			updateZtreeUlHeight:function(){
				var height = jQuery(".ztree").height();
				if(height){
					jQuery("#overFlowDivTree .ztreeUlDiv").height(height);
			...
			refreshTree:function(id,parentId,options){
				...
				var treeObj = $.fn.zTree.getZTreeObj(options.rootId);
				var parentNode = treeObj.getNodeByParam("id", options.idPrefix+parentId, null);
				if(options.add && !!parentNode){
					parentNode.isParent = true;
				}
				...
				window.setTimeout(function(){
					selectNode(treeObj,parentNode,id,options,100);
				},100);
			},
			selectNode:function(treeObj,parentNode,id,options, count){
					var node = treeObj.getNodeByParam("id", options.idPrefix+id, parentNode);
					if(!!node || count==0){
						treeObj.selectNode(node);
					}else{
						window.setTimeout(function(){
							selectNode(treeObj,parentNode,id,options,count-1);
						},100);
					}
			},
			expandAll:function(){
				 var imgs = jQuery("button.center_close,button.bottom_close");
				 var i=0;
				 imgs.each(function(){
				   jQuery(this).click();
				   i++;
			...
			format:function(searchStr,show){
				var as = jQuery("div a[target]");
				var hasMore = false;
				as.each(function(){
					var display = jQuery(this).closest("li").css("display");
					if(jQuery(this).text().toLowerCase().indexOf(searchStr)<0 && display!="none"){
					  ...
					  //jQuery(this).closest("li").closest("li").show()
					}
				});
				return hasMore;
			},
			...
			e8_after:function(){
				...
				try{
					...
					if(window.console)console.log("/wui/common/jquery/plugin/zTree/js/jquery.ztree.core.js-->format-->"+e);
				}
			},
			e8_search:function(searchStr,notExpand){
				var i = 0;
				if(notExpand){
					i=0;
				}else{
				...
				if(i>0){
					...
					var hasMore = format(searchStr);
					if(hasMore){
						window.setTimeout(function(){
							e8_search(searchStr,true);
						},500);
					}else{
						format(searchStr,true);
						e8_after();
					}
				}
			}
...
//@deprecated
function e8HasData(){
	__zTreeNamespace__.e8HasData();
}

//@deprecated
function prefectScrollBarForZtree(){
	__zTreeNamespace__.prefectScrollBarForZtree();
}

//@deprecated
function updateZtreeUlHeight(){
	__zTreeNamespace__.updateZtreeUlHeight();
}

//@deprecated
function e8_initTree(url){
	__zTreeNamespace__.e8_initTree(url);
}

//@deprecated
function refreshTree(id,parentId,options){
	__zTreeNamespace__.refreshTree(id,parentId,options);
}

//@deprecated
function selectNode(treeObj,parentNode,id,options, count){
	__zTreeNamespace__.selectNode(treeObj,parentNode,id,options, count);
}

//@deprecated
function expandAll(){
   return __zTreeNamespace__.expandAll();
}

//@deprecated
function format(searchStr,show){
	return __zTreeNamespace__.format(searchStr,show);
}
...
//@deprecated
function e8_after(){
	__zTreeNamespace__.e8_after();
}

//@deprecated
function e8_search(searchStr,notExpand){
	__zTreeNamespace__.e8_search(searchStr,notExpand);
}
