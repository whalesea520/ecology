...
Window.implement({
	$mt: function(B, C) {
        ...
        var A = $type(B);
        return ($mt[A]) ? $mt[A](B, C, this.document) : null;
    },
    $$: function(A) {
...
$mt.string = function(C, B, A) {
    C = A.getElementById(C);
    return (C) ? $mt.element(C, B) : null;

};
$mt.element = function(A, D) {
    $uid(A);
...
$mt.object = function(B, C, A) {
    if (B.toElement) {
        return $mt.element(B.toElement(A), C);
    }
    return null;
};
$mt.textnode = $mt.whitespace = $mt.window = $mt.document = $arguments(0);
Native.implement([Element, Document], {
    getElement: function(A, B) {
        return $mt(this.getElements(A, true)[0] || null, B);
    },
    ...
    Element.implement({
        ...
        grab: function(M, L) {
            A[L || "bottom"]($mt(M, true), this);
            return this;
        },
        inject: function(M, L) {
            A[L || "bottom"](this, $mt(M, true));
            return this;
        },
        replaces: function(L) {
            L = $mt(L, true);
...
Element.Events.domready = {
    ...
    if (Browser.Engine.trident) {
        ...
        (function() { ($try(function() {
                A.doScroll("left");
                return $mt(A).inject(document.body).set("html", "temp").dispose();
...
var Swiff = new Class({
    ...
    inject: function(A) {
        $mt(A, true).appendChild(this.toElement());
        return this;
    },
    remote: function() {
...
Fx.Tween = new Class({
    Extends: Fx.CSS,
    initialize: function(B, A) {
        this.element = this.subject = $mt(B);
        this.parent(A);
    },
    set: function(B, A) {
...
Request.HTML = new Class({
    ...
    success: function(D) {
        ...
        if (C.update) {
            $mt(C.update).empty().set("html", B.html);
        }
        if (C.evalScripts) {
            $exec(B.javascript);
        }
