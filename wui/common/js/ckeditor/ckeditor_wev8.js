            ...
            m.on('editingBlockReady', function() {
                ...
                m.addMode('source', {load: function(r, s) {
                        ...
                        p.on('blur', function() {
							...
							m.focusManager.setCheck(m.name);
        ...
        j.add('wysiwygarea', {requires: ['editingblock'],init: function(C) {
                ...
                C.on('editingBlockReady', function() {
                    ...
                    C.addMode('wysiwyg', {load: function(V, W, X) {
                            ...
                            jQuery(jQuery(ckeditIframe)[0].contentWindow.document.body).bind("focus",check);
			                jQuery(jQuery(ckeditIframe)[0].contentWindow.document.body).bind("blur",check);
			
							//如果CkEditot是应用于非签字意见框(如流程短语)，则签字意见框不存在，故需要检查签字意见框是否存在。
							if(jQuery(ckeditRemarkIframe)[0] != undefined){
								jQuery(jQuery(ckeditRemarkIframe)[0].contentWindow.document.body).bind("focus",check);
								jQuery(jQuery(ckeditRemarkIframe)[0].contentWindow.document.body).bind("blur",check);
							}
                        }});
