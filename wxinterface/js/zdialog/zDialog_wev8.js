...
var IMAGESPATH = '/wxinterface/js/zdialog/'; //图片路径配置
...
var CoDialog = function (currentwin) {
	...
	this.checkDataChange = true;//关闭时是否需要检查数据是否更改
    ...
    if (arguments.length > 0 && typeof(arguments[0]) == "string") { //兼容旧写法
        ...
        CoDialog.setOptions(this, arguments[0])
    }
	if(!this.ID)
...
CoDialog._Array = [];
CoDialog.bgDiv = null;
CoDialog.setOptions = function (obj, optionsObj) {
    if (!optionsObj) return;
    for (var optionName in optionsObj) {
        obj[optionName] = optionsObj[optionName];
    }
};
CoDialog.attachBehaviors = function () {
    if (isInternetExplorer) {
        document.attachEvent("onkeydown", CoDialog.onKeyDown);
        window.attachEvent('onresize', CoDialog.resetPosition);
    } else {
        document.addEventListener("keydown", CoDialog.onKeyDown, false);
        window.addEventListener('resize', CoDialog.resetPosition, false);
    }
};
CoDialog.prototype.attachBehaviors = function () {
...
CoDialog.prototype.displacePath = function () {
...
CoDialog.prototype.setPosition = function () {
...
CoDialog.setBgDivSize = function () {
    var bd = $bodyDimensions(topWin);
	if(CoDialog.bgDiv){
			if(isIE6){
				CoDialog.bgDiv.style.height = bd.clientHeight + "px";
				CoDialog.bgDiv.style.top=bd.scrollTop + "px";
				CoDialog.bgDiv.childNodes[0].style.display = "none";//让div重渲染,修正IE6下尺寸bug
				CoDialog.bgDiv.childNodes[0].style.display = "";
			}else{
				CoDialog.bgDiv.style.height = bd.scrollHeight + "px";
			}
		}
};
CoDialog.resetPosition = function () {
    CoDialog.setBgDivSize();
    for (var i = 0, len = topWin.CoDialog._Array.length; i < len; i++) {
        topWin.CoDialog._Array[i].setPosition();
    }
};
CoDialog.prototype.create = function () {
    var bd = $bodyDimensions(topWin);
  ...
  <table id="_DialogTable_' + this.ID + '"  width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0"  style=" font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "")+(this.hideDraghandle ? "display:none;" : "") + '">\
      <td height="30px;" style="background-color:#4f81bd;"><div style="float: left;color:#fff;"><span style="width:10px;display:inline-block;"></span><span id="_Title_' + this.ID + '" style="font-size:13px;height:30px;line-height:30px;"> ' +  this.Title  + '</span></div>\
        <div onclick="CoDialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(CoDialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(/wxinterface/js/zdialog/16_close.png)\'" onmouseover="this.style.backgroundImage=\'url(/wxinterface/js/zdialog/16_close2.png)\'" style="font-family:Verdana!important;\
        				margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background-image: url(/wxinterface/js/zdialog/16_close.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
        if(this.maxiumnable){				
			html += '\
			<div id="_maxBtn_'+this.ID+'" onclick="CoDialog.getInstance(\'' + this.ID + '\').maxOrRecoveryWindow(this);" onmouseout="CoDialog.getInstance(\'' + this.ID + '\').mouseOut(this);" onmouseover="CoDialog.getInstance(\'' + this.ID + '\').mouseOver(this);" style="font-family:Verdana!important;\
		margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background-image: url(/wxinterface/js/zdialog/max.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
		}
		html += '</td>\
    </tr>\
    <tr valign="top">\
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align:'+this.textAlign+' ; background-color:#f6f6f6;">\
                ...
                <input type="button" class="zd_btn_cancle" style="padding: 0px 18px 0px 18px;" value='+this.cancelLabel+' onclick="CoDialog.getInstance(\'' + this.ID + '\').closeByHand();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
    </tr>\
...
CoDialog.prototype.setSize = function (w, h) {
    if (w && +w > 20) {
        this.Width = +w;
...
CoDialog.prototype.mouseOut = function(obj){
	if(this.maxState){//最大化状态
		jQuery(obj).css("background-image","url(/wxinterface/js/zdialog/recovery.png)");
	}else{
		jQuery(obj).css("background-image","url(/wxinterface/js/zdialog/max.png)");
	}
}

CoDialog.prototype.mouseOver = function(obj){
	if(this.maxState){//最大化状态
		jQuery(obj).css("background-image","url(/wxinterface/js/zdialog/recovery2.png)");
	}else{
		jQuery(obj).css("background-image","url(/wxinterface/js/zdialog/max2.png)");
	}
}

//最大化或者还原窗口
CoDialog.prototype.maxOrRecoveryWindow = function(obj){
...
CoDialog.prototype.callback = function(datas){
	    ...
	    if (ids!= ""){
			...
			if(isAdd){
				...
				if(isAdd){
					...
					if(isMustInput==2){
						e8os.find("#"+fieldid+"spanimg").html("<img align='absmiddle' src='/wxinterface/js/zdialog/BacoError.gif'>");
					}
				}
		}
		if(!!userCallBack){
			try{
				...
				if(window.console)console.log(e+"-->/cowechant/js/zDialog.js-->callback()");
			}
		}
...
CoDialog.prototype.show = function () {
    this.create();
    var bgdiv = CoDialog.getBgdiv(),
		thisdialogDiv=this.getDialogDiv();
    this.zindex = thisdialogDiv.style.zIndex = parseInt(CoDialog.bgDiv.style.zIndex) + 1;
    if (topWin.CoDialog._Array.length > 0) {
        this.zindex = thisdialogDiv.style.zIndex = parseInt(topWin.CoDialog._Array[topWin.CoDialog._Array.length - 1].zindex) + 2;
    } else {
    ...
    topWin.CoDialog._Array.push(this);
    if (this.Modal) {
        bgdiv.style.zIndex = parseInt(topWin.CoDialog._Array[topWin.CoDialog._Array.length - 1].zindex) - 1;
        CoDialog.setBgDivSize();
...
CoDialog.prototype.getPageData = function($dialog,memkey,count){
	try{
		if(count>50){
			if(window.console){
				console.log("/wxinterface/js/zdialog.js---超时退出...");
			}
			return;
		}
...
CoDialog.prototype.closeManual = function(dialog){
	if(!dialog)dialog=this;
	var thisdialogDiv=dialog.getDialogDiv();
    if (dialog == topWin.CoDialog._Array[topWin.CoDialog._Array.length - 1]) {
        var isTopDialog = topWin.CoDialog._Array.pop();
    } else {
        topWin.CoDialog._Array.remove(dialog)
    }
    ...
    if(window.console && console.log)console.log(topWin.CoDialog._Array);
    if (topWin.CoDialog._Array.length > 0) {
        if (dialog.Modal && isTopDialog) CoDialog.bgDiv.style.zIndex = parseInt(topWin.CoDialog._Array[topWin.CoDialog._Array.length - 1].zindex) - 1;
    } else {
        CoDialog.bgDiv.style.zIndex = "900";
        CoDialog.bgDiv.style.display = "none";
        var topWinBody = topDoc.getElementsByTagName(topDoc.compatMode == "BackCompat" ? "BODY" : "HTML")[0];
...
*/
CoDialog.prototype.close = function(datas){
	try {
		if (!!this.callbackfun && !!datas) {
			this.callbackfun(this.callbackfunParam, datas);
		}
	} catch (e) {}
	this.closeManual();
}

CoDialog.prototype.closeByHand = function () {
	var $dialog = this;
	if(this.checkDataChange){
	...
	if(this.dataChange && false){//第一版暂时取消该提示
		top.CoDialog.confirm("数据发生更改且尚未保存，确定离开吗？",function(){
...
CoDialog.prototype.autoClose = function () {
    if (this.closed) {
...
CoDialog.getInstance = function (id) {
    var dialogDiv = topWin.$id("_DialogDiv_" + id);
    if (!dialogDiv){ 
    	if(window.console)
    	console.log("/wxinterface/js/zdialog.js-->getInstance-->没有取到对应ID的弹出框页面对象");
    }
	try{
...
CoDialog.prototype.addButton = function (id, txt, func) {
...
CoDialog.prototype.removeButton = function (btn) {
    var input0 = topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT")[0];
    input0.parentNode.removeChild(btn);
};
CoDialog.getBgdiv = function () {
    if (CoDialog.bgDiv) return CoDialog.bgDiv;
    ...
    CoDialog.bgDiv = bgdiv;
	bgdiv=null;
    return CoDialog.bgDiv;
};
CoDialog.prototype.getDialogDiv = function () {
...
CoDialog.prototype.getContainer = function(){
	var container = topWin.$id("_Container_"+this.ID);
	return container;
}

CoDialog.onKeyDown = function (event) {
...
CoDialog.close = function (id) {
    if (topWin.CoDialog._Array.length > 0) {
        var diag = topWin.CoDialog._Array[topWin.CoDialog._Array.length - 1];
        diag.cancelButton.onclick.apply(diag.cancelButton, []);
    }
};
CoDialog.alert = function (msg, func, w, h,m,_options) {
    var w = w || 300,
        h = h || 80,
	...
	var diag =null;
    if(window.top.CoDialog&&!options.isIframe){
    	diag=new window.top.CoDialog({
	        Width: w,
	        Height: h,
	        Modal:m
	    });
    ...
    if(options.isIframe){
	    ...
	    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
			<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.png" style="margin-right:10px;" width="26" height="26" align="absmiddle"></td>\
...
CoDialog.confirm = function (msg, funcOK, funcCal, w, h,m, custombtnjson, btnoklabel, btncancellabel, win, flashs) {
    var w = w || 300,
        h = h || 80,
        m=m==false?false:true;
    
    var diag = new CoDialog({
        Width: w,
        Height: h,
        Modal:m,
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_query.png" style="margin-right:10px;" width="26" height="26" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt">' + msg + '</td></tr>\
	</table>';
    diag.show();
...
CoDialog.open = function (arg) {
    var diag = new CoDialog(arg);
    diag.show();
    return diag;
};

if (isInternetExplorer) {
    window.attachEvent("onload", CoDialog.attachBehaviors);
} else {
    window.addEventListener("load", CoDialog.attachBehaviors, false);
}
...
function getDialog(win){
    ...
    for(var i=0;i<allFrame.length;i++){
    	...
    	if (!!tempFrame && tempFrame.contentWindow == win) {
    		...
    		if (tempIds.length > 0) {
    			return CoDialog.getInstance(tempIds[tempIds.length - 1]);
    		}
    	}
	}
	return null;
...
function customClickEvent(dialogid) {
	var cceDialog = CoDialog.getInstance(dialogid);
	cceDialog.custombtnjson.click();
	CoDialog.getInstance(dialogid).close();
}
