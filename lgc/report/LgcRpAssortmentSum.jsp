...
String titlename = SystemEnv.getHtmlLabelName(352,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(716,user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<%
LgcRpSumManage.setOptional("assortment") ;
...
<TABLE class=ListStyle width="100%" cellspacing=1>
  ...
  <TR class=Header> 
    <TH><%=SystemEnv.getHtmlLabelName(716,user.getLanguage())%></TH>
  ...
  <%  
	...
	%>
    <TD><%=Util.toScreen(AssetAssortmentComInfo.getAssortmentName(resultid),user.getLanguage())%></TD>
    <TD> 
      <TABLE height="100%" cellSpacing=0 width="100%">
        <TBODY> 
        <TR> 
          <a href="/lgc/search/LgcSearchOperation.jsp?assetassortment=<%=resultid%>&operation=search">
