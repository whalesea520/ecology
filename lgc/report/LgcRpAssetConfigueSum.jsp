...
String titlename = SystemEnv.getHtmlLabelName(352,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(706,user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<tr>
	...
	<td valign="top">
		...
		<td valign="top">
<%
LgcRpSumManage.setOptional("configue") ;
...
<TABLE class=ListStyle width="100%" cellspacing=1>
  ...
  <TR class=Header> 
    <TH><%=SystemEnv.getHtmlLabelName(706,user.getLanguage())%></TH>
...
<TR class=Line><TD colSpan=4></TD></TR>
  ...
  while(LgcRpSumManage.next())  { 
	...
	%>
    <TD><%=Util.toScreen(AssetRelationTypeComInfo.getAssetRelationTypename(resultid),user.getLanguage())%></TD>
    <TD> 
      <TABLE height="100%" cellSpacing=0 width="100%">
        <TBODY> 
        <TR> 
          <TD class=redgraph <%if(Util.getFloatValue(imagepercent.substring(0,imagepercent.length()-1),0)>=1){%>width="<%=imagepercent%>"<%} else {%>width="1" <%}%>>&nbsp;</TD></a>
          <TD>&nbsp;</TD>
        </TR>
        </TBODY>
      </TABLE>
