...
<%
String pageSize = PageIdConst.getPageSize(PageIdConst.DOC_SECCATEGORMOVERIGHT,user.getUID(),PageIdConst.DOC);
String pageId = PageIdConst.DOC_SECCATEGORMOVERIGHT;
String titlename = "";
...
labels[1] = 78;
int operationcode = MultiAclManager.OPERATION_MOVEDOC;
intanceid = "movePermission";
	...
	if(parentId>0){
		hasSecManageRight = am.hasPermission(Util.getIntValue(""+id), MultiAclManager.CATEGORYTYPE_SEC, user, MultiAclManager.OPERATION_CREATEDIR);
	}
...
%>
<BODY>
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.DOC_SECCATEGORMOVERIGHT %>"/>
<%if("1".equals(isDialog)){ %>
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right;">
			<%if(canAdd){ %>
			<input id="newRight" type=button class="e8_btn_top" onclick="openDialog(3);" value="<%=SystemEnv.getHtmlLabelName(611, user.getLanguage())%>"></input>
			<%} %>
			<%if(canDelete){ %>
...
<%
if(canAdd){ 
RCMenu += "{"+SystemEnv.getHtmlLabelName(611,user.getLanguage())+",javascript:openDialog(3),_top} " ;
