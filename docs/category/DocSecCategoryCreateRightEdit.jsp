...
<%
 ...
 String intanceid = "createPermission";
...
String pageSize = PageIdConst.getPageSize(PageIdConst.DOC_SECCATEGORCREATERIGHT,user.getUID(),PageIdConst.DOC);
String pageId = PageIdConst.DOC_SECCATEGORCREATERIGHT;
//是否有效
...
%>
<BODY>
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.DOC_SECCATEGORCREATERIGHT %>"/>
<%if("1".equals(isDialog)){ %>
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right;">
			<%if(canAdd){ %>
				<input type=button id="newRight" class="e8_btn_top" onclick="openDialog(1);" value="<%=SystemEnv.getHtmlLabelName(611, user.getLanguage())%>"></input>
			<%} %>
			<%if(canDelete){ %>
			<input type=button id="delRight" class="e8_btn_top" onclick="onPermissionDelShare<%=operationcode%>();" value="<%=SystemEnv.getHtmlLabelName(91, user.getLanguage())%>"></input>
			<%} %>
			<span title="<%=SystemEnv.getHtmlLabelName(23036,user.getLanguage())%>" id="cornerMenu" class="cornerMenu"></span>
		</td>
	</tr>
</table>
</form>
...
<%
if(canAdd){ 
RCMenu += "{"+SystemEnv.getHtmlLabelName(611,user.getLanguage())+",javascript:openDialog(1),_top} " ;
