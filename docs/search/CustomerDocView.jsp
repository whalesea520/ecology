...
String titlename = SystemEnv.getHtmlLabelName(58,user.getLanguage())+"-"+SystemEnv.getHtmlLabelName(136,user.getLanguage())+":"
		+"<a href='/CRM/data/ViewCustomer.jsp?CustomerID="+crmid+"'>"+CustomerInfo.getCustomerInfoname(crmid)+"</a>";
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<%
//check right
RecordSet.executeProc("CRM_Find_Creater",crmid);
...
RecordSet.first();
String Creater = Util.toScreen(RecordSet.getString(1),user.getLanguage());
boolean canview=false;
 if(HrmUserVarify.checkUserRight("ViewCustomer:View",user)) {
	 canview=true;
 }else if(userid.equals(Creater)){
	 canview=true;
 }else if(userid.equals(ResourceComInfo.getManagerID(Creater))){
	 canview=true;
 }else if(userid.equals(RecordSet.getString("Manager"))){
	 canview=true;
 }else if(userid.equals(ResourceComInfo.getManagerID(RecordSet.getString("Manager")))){
	 canview=true;
 }
...
String linkstr = "CustomerDocView.jsp?orderby="+orderby+"&dspreply="+dspreply+"&dspachive="+dspachive+"&maincategory="+
				 maincategorys+"&doctype="+doctype+"&dsptype="+dsptype+"&crmid="+crmid ;
String whereclause ="where crmid="+crmid ;
String orderclause ="" ;
...
<FORM id=docsearch name=docsearch action="CustomerDocView.jsp" method=post >
<input type=hidden name="crmid" value="<%=crmid%>">
<TABLE class=ViewForm border=0>
  <TBODY>
  <TR>
    <TD noWrap><%=SystemEnv.getHtmlLabelName(338,user.getLanguage())%></TD>
    <TD class=Field><SELECT class=InputStyle  class=FieldLongB id=orderby 
    ...
    <TD class=Field><SELECT  class=InputStyle class=FieldShort id=dspreply 
  ...
  <TR>
    ...
    <TD>&nbsp;</TD>
    <TD noWrap><%=SystemEnv.getHtmlLabelName(63,user.getLanguage())%></TD>
      <TD class=Field> 
        <select name=doctype  class=InputStyle  onChange="docsearch.submit();">
          <option value=""> </option>
          <%
			  while(DocTypeComInfo.next()){
				String isselect = "";
				String curtypeid = DocTypeComInfo.getDocTypeid();
				String curname = DocTypeComInfo.getDocTypename();
				if(curtypeid.equals(doctype)) isselect = " selected";
		  %>
          <option value="<%=curtypeid%>" <%=isselect%>><%=curname%></option>
          <%}%>
        </select>
      </TD>
    <TD>&nbsp;</TD>
