...
//处理自定义条件 begin
	...
	String whereclause = " where " + DocSearchComInfo.FormatSQLSearchSharing(user.getLanguage()) ;
	
	//whereclause+=" and t2.sharelevel=3 ";
	//whereclause+=" and exists(select 1 from DocSecCategory where DocSecCategory.id=t1.secCategory and DocSecCategory.shareable='1') "; //允许修改非默认共享
	...
	//sqlWhere = DocSearchManage.getShareManageSqlWhere(whereclause,user);//匹配共享的条件
     sqlWhere = whereclause ;
		...
		//operateString userType_userId_userSeclevel
		String popedomOtherpara=loginType+"_"+userid+"_"+userSeclevel+"_"+userType+"_"+userdepartment+"_"+usersubcomany;
		String popedomOtherpara2="column:seccategory+column:docStatus+column:doccreaterid+column:ownerid";
		...
		String userInfoForotherpara =user.getLogintype()+"+"+userid+"+"+userSeclevel+"+"+userType+"+"+userdepartment+"+"+usersubcomany;
		...
		colString +="<col name=\"id\" width=\"22%\"  text=\""+SystemEnv.getHtmlLabelName(58,user.getLanguage())+"\" column=\"id\" orderkey=\"docsubject\" target=\"_fullwindow\" transmethod=\"weaver.splitepage.transform.SptmForDoc.getDocNameAndIsNewByDocIdSharing\" otherpara=\""+userInfoForotherpara+"+column:docsubject+column:doccreaterid+column:readCount\" />";
