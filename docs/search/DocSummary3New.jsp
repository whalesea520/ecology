...
if(offical.equals("1")){//发文/收文库
	...
	String secids = "";
	if(officalType==1){
		 _sql = "select secCategoryId from Workflow_DocProp where workflowid in (select id from workflow_base where isWorkflowDoc=1 and officalType in (1,3))";
	}else if(officalType==2){
		_sql = "select secCategoryId from Workflow_DocProp where workflowid in (select id from workflow_base where isWorkflowDoc=1 and officalType in (2))";
	}
...
if(logintype.equals("1")){  //内部用户的处理
    ...
    //总的分目录下的文章
    sql = "select count(t1.id) count,t1.seccategory from DocDetail  t1, "+tables+"  t2 where ((docstatus = 7 and (sharelevel>1 or (doccreaterid="+user.getUID()+" or ownerid="+user.getUID()+"))) or docstatus in ("+docstatus+")) and t1.id=t2.sourceid and t1.subcategory="+subid+" ";
    sql+=whereclause;
    ...
    //刘煜改为总的分目录 看过的文章 
    sql = "select count(distinct t1.id) count,t1.seccategory from "+tables+"  t3 , DocDetail  t1 left join docReadTag t2 on t1.id=t2.docid where t1.id=t3.sourceid and t1.subcategory="+subid+" and ((docstatus = 7 and (sharelevel>1 or (doccreaterid="+user.getUID()+" or ownerid="+user.getUID()+"))) or docstatus in ("+docstatus+")) and ( ( t1.doccreaterid = "+user.getUID()+" and t1.usertype ='1')  or ( t2.userid="+user.getUID()+" and t2.usertype=1 ) )" ;
    sql += whereclause;
    sql += " group by t1.seccategory order by t1.seccategory ";
   ...
   /* sql = "select count(distinct t1.id) count,t1.seccategory from "+tables+"  t3 , DocDetail  t1 left join docReadTag t2 on   t1.id=t2.docid where t1.id=t3.sourceid and t1.subcategory="+subid+" and ((docstatus = 7 and (sharelevel>1 or (doccreaterid="+user.getUID()+" or ownerid="+user.getUID()+"))) or docstatus in ('1','2','5')) and ( ( t1.doccreaterid = "+user.getUID()+" and t1.usertype ='1')  or ( t2.userid="+user.getUID()+" and t2.usertype=1 ) )" ;
	...
	sql="select count(t1.id) count,t1.seccategory from DocDetail  t1";
    if(!urlType.equals("11")){
    	sql += ", "+tables+"  t2";
    }
    sql += " where  t1.subcategory="+subid+" and ((docstatus = 7 ";
    if(!urlType.equals("11")){
