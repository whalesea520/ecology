		...
		<wea:item>
			<div >
				<%if(!isWorkflowDoc.equals("1")){ %>
					<input TYPE="radio" NAME="sdoctype" <%=(docType.equals(".htm")?"checked":"")%> onClick="onChangeDocType('/docs/mouldfile/DocMouldAdd.jsp','.htm')">HTML&nbsp;<%=SystemEnv.getHtmlLabelName(16449,user.getLanguage())%>
				<%} %>
				<input TYPE="radio" NAME="sdoctype" <%=(docType.equals(".doc")?"checked":"")%> onClick="onChangeDocType('/docs/mouldfile/DocMouldAddExt.jsp','.doc')">WORD&nbsp;<%=SystemEnv.getHtmlLabelName(16449,user.getLanguage())%>
				<%if(!isWorkflowDoc.equals("1")){ %>
				<input TYPE="radio" NAME="sdoctype" <%=(docType.equals(".xls")?"checked":"")%> onClick="onChangeDocType('/docs/mouldfile/DocMouldAddExt.jsp','.xls')">EXCEL&nbsp;<%=urlfrom.equals("hr")?SystemEnv.getHtmlLabelName(15786,user.getLanguage()):SystemEnv.getHtmlLabelName(16449,user.getLanguage())%>
				<%} %>
				<input TYPE="radio" NAME="sdoctype" <%=(docType.equals(".wps")?"checked":"")%> onClick="onChangeDocType('/docs/mouldfile/DocMouldAddExt.jsp','.wps')"><%=SystemEnv.getHtmlLabelName(22359,user.getLanguage())%>&nbsp;<%=SystemEnv.getHtmlLabelName(16449,user.getLanguage())%>
				<%if("1".equals(isUseET)){%>
			<input TYPE="radio" NAME="sdoctype" <%=(docType.equals(".et")?"checked":"")%> onClick="onChangeDocType('/docs/mouldfile/DocMouldAddExt.jsp','.et')"><%=SystemEnv.getHtmlLabelName(24545,user.getLanguage())%>&nbsp;<%=urlfrom.equals("hr")?SystemEnv.getHtmlLabelName(15786,user.getLanguage()):SystemEnv.getHtmlLabelName(16449,user.getLanguage())%>
			<%}%>
			</div>
			<script language=javascript>
			...
			<span>
				<%String extraparams = "{'docType':'"+docType+"'}"; %>
				   <brow:browser viewType="0" name="docMouldExistedId" browserValue='<%= ""+docMouldExistedId %>' 
					...
					browserUrl="/systeminfo/BrowserMain.jsp?mouldID=resource&url=/docs/mouldfile/DocMouldBrowser.jsp?doctype=.doc" language='<%=""+user.getLanguage() %>'
...
function getBrowserUrlFn(){
	if("<%=urlfrom%>"=="hr"){
	  return "/systeminfo/BrowserMain.jsp?mouldID=resource&url=/docs/mouldfile/DocMouldtwoBrowser.jsp?urlfrom=<%=urlfrom%>&doctype=<%=docType%>&subcompanyid1=<%=subcompanyid1%>";
	}else{
	  return "/systeminfo/BrowserMain.jsp?mouldID=doc&url=/docs/mouldfile/DocMouldBrowser.jsp?doctype=<%=docType%>";
	}
...
function afterOnShowMould(e,id1,fieldid,params){
	if(id1!=null){
		if("<%=urlfrom%>"=="hr"){
			if(isdocexisted)
			location="/docs/mouldfile/DocMouldAddExt.jsp?isdialog=<%=isDialog%>&id="+weaver.id.value+"&subcompanyid1=<%=subcompanyid1%>&urlfrom=<%=urlfrom%>&mouldname="+document.weaver.mouldname.value+"&docType=<%=docType%>&docMouldExistedId="+id1.id;
			else if("<%=docid%>"!="")
			location="/docs/mouldfile/DocMouldAddExt.jsp?isdialog=<%=isDialog%>&id=<%=docid%>&subcompanyid1=<%=subcompanyid1%>&urlfrom=<%=urlfrom%>&mouldname="+document.weaver.mouldname.value+"&docType=<%=docType%>&docMouldExistedId="+id1.id;
			else
			location="/docs/mouldfile/DocMouldAddExt.jsp?isdialog=<%=isDialog%>&subcompanyid1=<%=subcompanyid1%>&urlfrom=<%=urlfrom%>&mouldname="+document.weaver.mouldname.value+"&docType=<%=docType%>&docMouldExistedId="+id1.id;
		}else{
