...
%>
    <HEAD>
		...
		try{
			parent.setTabObjName("<%=SystemEnv.getHtmlLabelNames("25126",user.getLanguage())%>");
		}catch(e){
...
<%
		String sqlwhere = "docSecCategoryMould.mouldID = docMould.ID AND docSecCategoryMould.mouldType in (3,7) AND docSecCategoryMould.mouldBind = 1 AND docSecCategoryMould.secCategoryID = " + seccategory+" and docSecCategoryMould.mouldID=workflow_mould.mouldid and workflow_mould.seccategory="+seccategory+" and workflow_mould.workflowid="+workflowid+" and workflow_mould.mouldType in (0,1) and workflow_mould.visible=1 and workflow_mould.mouldid in ( select docMouldId from workflow_docshow where flowId="+workflowid+" and fieldid!=-1 and secCategoryId="+seccategory+")";
		String tableString=""+
			   "<table needPage=\"false\" pagesize=\"10\" tabletype=\"none\">"+
			   "<sql backfields=\"docMould.ID, docMould.mouldName\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlwhere)+"\" sqlform=\"DocSecCategoryMould docSecCategoryMould, DocMould docMould,workflow_mould\" sqlprimarykey=\"docMould.ID\"  sqlisdistinct=\"true\" sqlsortway=\"desc\"  />"+
			   "<head>";
