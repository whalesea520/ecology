...
if(!id.equals("")){
	RecordSet.executeSql("select * from DocInstancyLevel where id = "+id);
	if(RecordSet.next()){
...
function openDialog(id){
	...
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("93,15534",user.getLanguage())%>";
		dialog.URL = "/docs/tabs/DocCommonTab.jsp?_fromURL=34&id="+id;
	}else{
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("82,15534",user.getLanguage())%>";
		dialog.URL = "/docs/tabs/DocCommonTab.jsp?_fromURL=34";
	}
	dialog.Width = 600;
	dialog.Height = 213;
	dialog.Drag = true;
...
String titlename = SystemEnv.getHtmlLabelName(15534,user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(83,user.getLanguage())+",javascript:_onViewLog(342),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
%>
...
<FORM id=weaverD  action="docInstancyLevelOperation.jsp" method=post>
...
<%
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.DOC_DOCINSTANCYLIST+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.DOC_DOCINSTANCYLIST,user.getUID(),PageIdConst.DOC)+"\" tabletype=\"checkbox\">"+
	    " <checkboxpopedom  id=\"checkbox\" showmethod=\"weaver.general.KnowledgeTransMethod.getDocNumberOperate\"  popedompara=\""+canedit+"\" />"+
	   "<sql backfields=\"*\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlform=\"DocInstancyLevel\" sqlorderby=\"showOrder\"  sqlprimarykey=\"id\" sqlsortway=\"asc\"  sqldistinct=\"true\" />"+
	   operateString+
	   "<head>"+							 
	   ...
	   "</table>";
%> 
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.DOC_DOCINSTANCYLIST %>"/>
