...
function openDialog(id){
	...
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("93,16973",user.getLanguage())%>";
		dialog.URL = "/docs/tabs/DocCommonTab.jsp?_fromURL=35&id="+id;
	}else{
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("82,16973",user.getLanguage())%>";
		dialog.URL = "/docs/tabs/DocCommonTab.jsp?_fromURL=35";
	}
	dialog.Width = 600;
	dialog.Height = 213;
	dialog.Drag = true;
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(83,user.getLanguage())+",javascript:_onViewLog(341),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
%>
...
<%
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.DOC_DOCKINDLIST+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.DOC_DOCKINDLIST,user.getUID(),PageIdConst.DOC)+"\" tabletype=\"checkbox\">"+
	    " <checkboxpopedom  id=\"checkbox\" showmethod=\"weaver.general.KnowledgeTransMethod.getDocNumberOperate\"  popedompara=\""+canedit+"\" />"+
	   "<sql backfields=\"*\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlform=\"DocSendDocKind\" sqlorderby=\"showOrder\"  sqlprimarykey=\"id\" sqlsortway=\"asc\"  sqldistinct=\"true\" />"+
	    operateString+
	   "<head>"+							 
			 "<col width=\"44%\" transmethod=\"weaver.general.KnowledgeTransMethod.forHtml\" text=\""+SystemEnv.getHtmlLabelName(195,user.getLanguage())+"\"  href=\"docKind.jsp\" column=\"name\" linkkey=\"id\" linkvaluecolumn=\"id\" target=\"_self\"/>"+
	   ...
	   "</table>";
%>
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.DOC_DOCKINDLIST %>"/>
