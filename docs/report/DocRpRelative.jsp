...
if(!department.equals("")){
	whereclause+=" and id in ("+department+") ";
...
<% 
	...
	String doccount = "select COUNT(id) from DocDetail d where t.id = d.docdepartmentid and (ishistory is null or ishistory = 0) and usertype>0 and docstatus in ('1','2','5')";
	if(issubcompanyvirid||isdepartmentvirid){
	doccount = "select COUNT(id) from DocDetail d ,(select tv2.resourceid as hid,tv1.id as did from HrmDepartmentVirtual tv1,HrmResourceVirtual tv2 where tv1.id=tv2.departmentid ) e where e.hid=d.doccreaterid and e.did=t.id  and (ishistory is null or ishistory = 0) and usertype>0 and docstatus in ('1','2','5')";
	}
	String notreply = "select COUNT(id) from DocDetail d where t.id = d.docdepartmentid and (ishistory is null or ishistory = 0) and usertype>0 and docstatus in ('1','2','5') and (d.isreply=0 or d.isreply is null)";
	if(issubcompanyvirid||isdepartmentvirid){
	notreply = "select COUNT(id) from DocDetail d, (select tv2.resourceid as hid,tv1.id as did from HrmDepartmentVirtual tv1,HrmResourceVirtual tv2 where tv1.id=tv2.departmentid ) e where e.hid=d.doccreaterid  and e.did=t.id  and (ishistory is null or ishistory = 0) and usertype>0 and docstatus in ('1','2','5') and (d.isreply=0 or d.isreply is null)";
	}
	String onlyreply = "select COUNT(id) from DocDetail d where t.id = d.docdepartmentid and (ishistory is null or ishistory = 0) and usertype>0 and docstatus in ('1','2','5') and (d.isreply=1)";
	if(issubcompanyvirid||isdepartmentvirid){
	...
	String backfields="t.id,t.departmentname,t.subcompanyid1,"+
	"("+doccount+") as doccount,"+
	"("+notreply+") as notreplydoccount,"+
	"("+onlyreply+") as replydoccount";
	String sqlform = "hrmdepartment t";
	if(issubcompanyvirid||isdepartmentvirid){
	sqlform = "HrmDepartmentVirtual t";
	
	}
 String colString="";
	if(isusenewreply){
		colString="<col width=\"20%\" column=\"id\" orderkey=\"departmentname\" transmethod=\"weaver.general.KnowledgeTransMethod.getDepartmentName\" otherpara=\""+user.getLanguage()+"\" text=\""+SystemEnv.getHtmlLabelName(27511,user.getLanguage())+"\"/>"+
			 "<col width=\"10%\" text=\""+SystemEnv.getHtmlLabelName(141,user.getLanguage())+"\" transmethod=\"weaver.general.KnowledgeTransMethod.getSubcompanyName\" otherpara=\""+user.getLanguage()+"\" column=\"subcompanyid1\"  orderkey=\"subcompanyid1\"/>"+
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.DOC_DEPARTMENTLIST+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.DOC_DEPARTMENTLIST,user.getUID(),PageIdConst.DOC)+"\" tabletype=\""+tabletype+"\">"+
	   ...
	   "</table>";
%>
	<input type="hidden" _showCol="false" name="pageId" id="pageId" value="<%= PageIdConst.DOC_DEPARTMENTLIST %>"/>
