...
if(fromdate == null){
    if(session.getAttribute("DocRpDocSum_fromdate")==null){
        session.setAttribute("DocRpDocSum_fromdate","");
        fromdate = "";
    }else{
        fromdate = session.getAttribute("DocRpDocSum_fromdate").toString();
    }
}

if(todate == null){
    if(session.getAttribute("DocRpDocSum_todate")==null){
        session.setAttribute("DocRpDocSum_todate","");
        todate = "";
    }else{
        todate = session.getAttribute("DocRpDocSum_todate").toString();
    }
}
if(fromdate.equals(""))	fromdate="0000-00-00";
session.setAttribute("DocRpDocSum_fromdate",fromdate);

if(todate.equals(""))	todate="9999-99-99";
session.setAttribute("DocRpDocSum_todate",todate);
...
<form id=report name=report method=post action="DocRpDocSum.jsp" >
...
<TR><TD class=Line colSpan=6></TD></TR>
            ...
            <td><%if(count_read!=""){%><a href="/docs/report/DocReadRp.jsp?sqlwhere=<%=java.net.URLEncoder.encode("where docid="+docid_read)%>&flag=1"><%=count_read%></a>
            <%} else {%><%=count_read%><%}%></td>
            </TR>
...
<TABLE width="100%" border=0>
  ...
  <TR>
   <TD noWrap>
   <%
	 out.println(Util.makeNavbar2(pagenum, total , prepage, "DocRpDocSum.jsp"));
   %>
   </TD>
  </TR>
  </TBODY>
