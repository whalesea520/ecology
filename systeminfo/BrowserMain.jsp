...
if(CommonBrowserIndex>=0||MultiCommonBrowserIndex>=0){
	...
	int index = queryString.indexOf("type=browser.");
	if(index>0){
		otherParam = queryString.substring(index+"type=browser.".length());
		index = otherParam.indexOf("&");
		if(index!=-1){
			otherParam = otherParam.substring(index);
		}
	}else{
		otherParam = queryString;
	}
	if(!"".equals(otherParam)&&otherParam.indexOf("&")!=0){
...
<%}else if(CommonBrowserIndex>=0||MultiCommonBrowserIndex>=0){%>
	jQuery(document).ready(function(){
		try{
			...
			<% 
			//QC：278879 [80][90]数据展现集成-标识带有希腊字母IE浏览器打开自定义浏览按钮异常
				String browserType_temp = java.net.URLEncoder.encode(browserType,"UTF-8");
				String otherParam_temp = "";
				if(!"".equals(otherParam.trim())){
					otherParam = otherParam.replace(browserType,browserType_temp);
					if(otherParam.indexOf("=") > -1 && otherParam.length() > (otherParam.indexOf("="))){
						otherParam_temp = otherParam.substring(0,otherParam.indexOf("=")+1)+java.net.URLEncoder.encode(otherParam.substring(otherParam.indexOf("=")+1),"UTF-8");
					}else{
			...
			var frombrowserid = "<%=frombrowserid%>";
			var currenttime = "<%=currenttime%>";
			url+= "<%=otherParam_temp.replaceAll("%26","&").replaceAll("%3D","=")%>";
