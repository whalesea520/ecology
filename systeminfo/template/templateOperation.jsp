...
if(request.getParameter("operationType")!=null||true){
	...
	if(operationType.equals("open")){
		...
		if(isClear.equals("true")){
			rs.executeSql("delete from SystemTemplateSubComp where subcompanyid="+subCompanyId);
		}else{
	...
	if(operationType.equals("saveAs")){
		String templateName = Util.null2String(request.getParameter("saveAsName"));
		...
		sql = "select extendtempletid  from SystemTemplate"+tempdata+" where id = "+templateid;
		rs.execute(sql);
		if(rs.next()){
			extendtempletid = rs.getString("extendtempletid");
			sql = "INSERT INTO SystemTemplate (templatetype,extendtempletid,ecology7themeid,skin,templateName,companyId,logo,topBgColor,topBgImage,toolbarBgColor,toolbarBgImage,leftbarBgColor,leftbarBgImage,leftbarBgImageH,leftbarFontColor,menubarBgColor,menubtnBgColor,menubtnBgColorActive,menubtnBgColorHover,menubtnBorderColorActive,menubtnBorderColorHover,menubtnFontColor,templateTitle,isShowMainMenu,menuborderbg,menuInBoldBorderbg,menuBottomCusbg,defaultHp,isopen) select templatetype,extendtempletid,ecology7themeid,skin,'"+templateName+"',companyId,logo,topBgColor,topBgImage,toolbarBgColor,toolbarBgImage,leftbarBgColor,leftbarBgImage,leftbarBgImageH,leftbarFontColor,menubarBgColor,menubtnBgColor,menubtnBgColorActive,menubtnBgColorHover,menubtnBorderColorActive,menubtnBorderColorHover,menubtnFontColor,templateTitle,isShowMainMenu,menuborderbg,menuInBoldBorderbg,menuBottomCusbg,defaultHp,0 from SystemTemplate"+tempdata+" where id = '"+templateid+"'";
			//out.println(sql);
			rs.execute(sql);
			
			if("2".equals(extendtempletid)){
				rs.execute("select max(id) as id from SystemTemplate");
				rs.next();
				String newtemplateid = rs.getString("id");	
				sql = "INSERT INTO extendHpWebCustom (templateId,menuid,pagetemplateid,menustyleid,leftmenuid,leftmenustyleid,defaultshow,useVoting,useRTX,useWfNote,useBirthdayNote,useDoc) select '"+newtemplateid+"',menuid,pagetemplateid,menustyleid,leftmenuid,leftmenustyleid,defaultshow,useVoting,useRTX,useWfNote,useBirthdayNote,useDoc from extendHpWebCustom"+tempdata+" where templateid = '"+templateid+"'";
				//out.println(sql);
				rs.execute(sql);
			}
		}
		
		//sql = "INSERT INTO SystemTemplate (templatetype,extendtempletid,ecology7themeid,skin,templateName,companyId,logo,topBgColor,topBgImage,toolbarBgColor,toolbarBgImage,leftbarBgColor,leftbarBgImage,leftbarBgImageH,leftbarFontColor,menubarBgColor,menubtnBgColor,menubtnBgColorActive,menubtnBgColorHover,menubtnBorderColorActive,menubtnBorderColorHover,menubtnFontColor,templateTitle,isShowMainMenu,menuborderbg,menuInBoldBorderbg,menuBottomCusbg,defaultHp) select templatetype,extendtempletid,ecology7themeid,skin,'"+templateName+"',"+subCompanyId+",logo,topBgColor,topBgImage,toolbarBgColor,toolbarBgImage,leftbarBgColor,leftbarBgImage,leftbarBgImageH,leftbarFontColor,menubarBgColor,menubtnBgColor,menubtnBgColorActive,menubtnBgColorHover,menubtnBorderColorActive,menubtnBorderColorHover,menubtnFontColor,templateTitle,isShowMainMenu,menuborderbg,menuInBoldBorderbg,menuBottomCusbg,defaultHp from SystemTemplate where id = '"+templateid+"'";
		//out.println(sql);
		//rs.executeSql(sql);
		//if(!from.equals("edit")){
			//response.sendRedirect("templateList.jsp?subCompanyId="+subCompanyId);
	...
	if(operationType.equals("delete")){
		...
		sql = "DELETE FROM SystemTemplate WHERE id in ("+templateId+")";
		rs.executeSql(sql);
		response.sendRedirect("templateList.jsp?subCompanyId="+subCompanyId);
		return;
	}
	...
	try{
		while(i.hasNext()) {
			...
			if(!item.isFormField()){
				...
				if(item.getFieldName().equals("templateTitle"))					templateTitle = Util.null2String(item.getString("UTF-8"));
				//DeleteImage
