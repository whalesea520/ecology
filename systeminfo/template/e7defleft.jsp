...
//lskin="default";
	...
	//横菜单
	homeMenus = getPortalMenus(user,from);

	List topMenus = mm.getCompanyMenus(companytype,subCompanyId, 1,user.getLanguage());
...
<%!public String grtlftMenu(List elements, User user, boolean isPotal) {
	...
	public List getPortalMenus(User user,String from) throws Exception {
		List result = new ArrayList();

		RecordSet rs = new RecordSet();

		
		String sql = "select id,infoname,subcompanyid from hpinfo h where subcompanyid>0 and isuse=1 and pid=0 order by ordernum1,id";
		...
		while (rs.next()) {
			...
			if("preview".equals(from)){
				map.put("url", "/homepage/Homepage.jsp?hpid=" + tempHpid
							+ "&subCompanyId=" + tempHpSubcompanyid
							+ "&isfromportal=1&isfromhp=0");
			}else{
				map.put("url", "/homepage/HomepageEditor.jsp?hpid=" + tempHpid
						+ "&subCompanyId=" + tempHpSubcompanyid
						+ "&isfromportal=1&isfromhp=0");
			}
			...
			map.put("child", getPortalSubMenu(tempHpid, user,from));
			result.add(map);
		}
	...
	public List getPortalSubMenu(String parenthpid, User user,String from) {
		List result = new ArrayList();
...
<script type="text/javascript" src="/js/jquery/plugins/jquery.dragsort-0.4.min_wev8.js"></script>
<script type="text/javascript">
...
function dymlftenu(ele, isOpenFirstMenu) {
    ...
    if(paramValue==536){
	   	...
	   	jQuery("#drillmenu").load("/email/new/leftmenu.jsp?"+new Date().getTime(),function(){
	   		jQuery("#mainFrame").attr("src", "/email/new/MailInBox.jsp?folderid=0&receivemailid=0&receivemail=true");
