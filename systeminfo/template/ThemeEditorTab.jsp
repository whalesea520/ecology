...
String url="";
String navName=SystemEnv.getHtmlLabelName(33673,user.getLanguage());
HashMap kv = (HashMap)pack.packageParams(request, HashMap.class);

String id = (String)kv.get("id");

rsSkin.execute("select name from ecology8theme where id ="+id);
if(rsSkin.next()){
	navName = rsSkin.getString("name");
}
...
<div class="e8_box demo2">
    ...
    <div class="tab_box">
        <div>
            <iframe src="<%="/systeminfo/template/ThemeEditor.jsp?id="+kv.get("id") %>" onload="update();" id="tabcontentframe" name="tabcontentframe" class="flowFrame" frameborder="0" height="100%" width="100%;"></iframe>
        </div>
    </div>
