...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="MenuMaintenanceList.jsp?type=<%=type%>" method=post target="contentframe">
    ...
    <%-- <BUTTON class=btnok accessKey=1 style="display:none" onclick="window.parent.close()" id=btnok><U>1</U>
        -<%=SystemEnv.getHtmlLabelName(201, user.getLanguage())%></BUTTON>
...
<%}%>
    <table height="100%" width=100% class="ViewForm" valign="top">
		...
		<tbody>
	        ...
	        <TR>
	            <td height="100%">
	                <div id="deeptree" class="cxtree" menu="f" CfgXMLSrc="/css/TreeConfig.xml"/>
	            <td>
	        </tr>
		</tbody>
...
<script language="javascript">
    function initTree() {
        CXLoadTreeItem("", "/systeminfo/menuconfig/MenuMaintenanceTreeLeftXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
