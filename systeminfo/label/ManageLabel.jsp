...
<%
	...
	if(!hasTab.equals("1")){
		response.sendRedirect("/docs/tabs/DocCommonTab.jsp?_fromURL=83");
		return;
	}
%>
<HTML><HEAD>
...
function onBtnSearchClick(){
	jQuery("#frmmain").attr("action","/systeminfo/label/ManageLabel.jsp?hasTab=<%=hasTab %>");
...
function openDialog(id){
	...
	var url = "/docs/tabs/DocCommonTab.jsp?_fromURL=86&isdialog=1";
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("26473,176",user.getLanguage())%>";
		dialog.Height = 400;
		url = "/docs/tabs/DocCommonTab.jsp?_fromURL=87&isdialog=1&id="+id;
	}else{
...
function exportLabel(){
	jQuery("#frmmain").attr("action","/systeminfo/label/LabelOperation.jsp?operation=export&hasTab=<%=hasTab %>");
	jQuery("#frmmain").submit();
}

function importLabel(){
	parent.location.href="/docs/tabs/DocCommonTab.jsp?_fromURL=97&isdialog=1"
}
</script>
</head>
<%
String imagefilename = "/images/hdMaintenance_wev8.gif";
String titlename = SystemEnv.getHtmlLabelNames("81485",user.getLanguage())+":"+SystemEnv.getHtmlLabelName(81486,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(28343,user.getLanguage())+",javascript:exportLabel(),_top} " ;
	RCMenuHeight += RCMenuHeightStep ;
	RCMenu += "{"+SystemEnv.getHtmlLabelNames("32935,81486",user.getLanguage())+",javascript:importLabel(),_top} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right;">
			...
			<input type=button class="e8_btn_top" onclick="exportLabel();" value="<%=SystemEnv.getHtmlLabelName(28343,user.getLanguage())%>"></input>
...
<form name="frmmain" id="frmmain" method="post" action="ManageLabel.jsp?hasTab=<%=hasTab %>">
...
<%
	...
	String sqlfrom = "(select * from ( "+
			"select a.id as id,b.indexdesc, cname,ename,twname,elname,flname from ("+
					"select a.indexid as id,a.labelname as cname,c.labelname as ename,d.labelname as twname,e.labelname as elname,f.labelname as flname  from (select  * from HtmlLabelInfo where languageid='#cname#')a " +
					"left join (select * from HtmlLabelInfo where languageid='#twname#')d on a.indexid = d.indexid "+ 
						"left join (select * from HtmlLabelInfo where languageid='#ename#')c on a.indexid = c.indexid "+ 
						"left join (select * from HtmlLabelInfo where languageid='#elname#')e on a.indexid = e.indexid "+
						"left join (select * from HtmlLabelInfo where languageid='#flname#')f on a.indexid = f.indexid "+
						") a,HtmlLabelIndex b where a.id=b.id) m) ff " ;
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.SYS_SYSTEM_LABEL_LIST+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.SYS_SYSTEM_LABEL_LIST,user.getUID(),PageIdConst.DOC)+"\" tabletype=\""+tabletype+"\">"+
	   ...
	   "</table>";
%> 
<input type="hidden" _showCol="false" name="pageId" id="pageId" value="<%= PageIdConst.SYS_SYSTEM_LABEL_LIST %>"/>
