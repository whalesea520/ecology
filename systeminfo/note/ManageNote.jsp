...
<%
	...
	if(!hasTab.equals("1")){
		response.sendRedirect("/docs/tabs/DocCommonTab.jsp?_fromURL=84");
		return;
	}
%>
<HTML><HEAD>
...
function onBtnSearchClick(){
	jQuery("#frmmain").attr("action","/systeminfo/note/ManageNote.jsp?hasTab=<%=hasTab %>");
...
function openDialog(id){
	...
	var url = "/docs/tabs/DocCommonTab.jsp?_fromURL=88&isdialog=1";
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelNames("26473,176",user.getLanguage())%>";
		dialog.Height = 400;
		url = "/docs/tabs/DocCommonTab.jsp?_fromURL=89&isdialog=1&id="+id;
	}else{
...
function exportNote(){
	jQuery("#frmmain").attr("action","/systeminfo/note/NoteOperation.jsp?operation=export&hasTab=<%=hasTab %>");
	jQuery("#frmmain").submit();
}

function importNote(){
	parent.location.href="/docs/tabs/DocCommonTab.jsp?_fromURL=98&isdialog=1"
}
</script>
</head>
<%
String imagefilename = "/images/hdMaintenance_wev8.gif";
String titlename = SystemEnv.getHtmlLabelNames("81485",user.getLanguage())+":"+SystemEnv.getHtmlLabelNames("24960",user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(28343,user.getLanguage())+",javascript:exportNote(),_top} " ;
	RCMenuHeight += RCMenuHeightStep ;
	RCMenu += "{"+SystemEnv.getHtmlLabelNames("32935,24960",user.getLanguage())+",javascript:importNote(),_top} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right;">
			...
			<input type=button class="e8_btn_top" onclick="exportNote();" value="<%=SystemEnv.getHtmlLabelName(28343,user.getLanguage())%>"></input>
...
<form name="frmmain" id="frmmain" method="post" action="ManageNote.jsp?hasTab=<%=hasTab %>">
...
<%
	...
	String sqlfrom = "(select * from ( "+
			"select a.id as id,b.indexdesc, cname,ename,twname,elname,flname  from ("+
					"select a.indexid as id,a.notename as cname,c.notename as ename,d.notename as twname ,e.notename as elname,f.notename as flname from (select  * from HtmlNoteInfo where languageid='#cname#')a " +
						"left join (select * from HtmlNoteInfo where languageid='#twname#')d on a.indexid = d.indexid "+ 
						"left join (select * from HtmlNoteInfo where languageid='#ename#')c on a.indexid = c.indexid "+ 
						"left join (select * from HtmlNoteInfo where languageid='#elname#')e on a.indexid = e.indexid "+
						"left join (select * from HtmlNoteInfo where languageid='#flname#')f on a.indexid = f.indexid "+
				") a,HtmlNoteIndex b where a.id=b.id) m) ff " ;
	if(!"".equals(id)){
		sqlWhere += " and id = '"+id+"'";
	}
	if(!"".equals(indexdesc)){
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.SYS_SYSTEM_NOTE_LIST+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.SYS_SYSTEM_NOTE_LIST,user.getUID(),PageIdConst.DOC)+"\" tabletype=\""+tabletype+"\">"+
	   ...
	   "</table>";
%> 
<input type="hidden" _showCol="false" name="pageId" id="pageId" value="<%= PageIdConst.SYS_SYSTEM_NOTE_LIST %>"/>
