...
String indexdesc = "";
rs.executeSql("select indexdesc from ErrorMsgIndex where id="+labelId);
if(rs.next()){
...
<wea:layout>
		...
		<wea:group context='<%= SystemEnv.getHtmlLabelNames("176,33439",user.getLanguage())%>'>
				...
				<wea:layout needImportDefaultJsAndCss="false" type="table" attributes="{'cols':'2','cws':'40%,60%'}">
					<wea:group context=""  attributes="{\"groupDisplay\":\"none\"}">
						...
						<%while(rs.next()){ %>
							...
							<wea:item>
								<%if(rs.getInt("id")==7){ %>
									<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" value="<%= SystemEnv.getErrorMsgName(labelId,rs.getInt("id"),true)%>" name="name_<%=rs.getInt("id") %>" onchange="setTWName(this);"/>
								<%}else if(rs.getInt("id")==9){ %>
									<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" value="<%= SystemEnv.getErrorMsgName(labelId,rs.getInt("id"),true)%>" name="name_<%=rs.getInt("id") %>" onchange="setCName(this,1);"/>
								<%}else{ %>
									<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" value="<%= SystemEnv.getErrorMsgName(labelId,rs.getInt("id"),true)%>" name="name_<%=rs.getInt("id") %>"/>
								<%} %>
							</wea:item>
						<%} %>
					</wea:group>
          ...
          <input type=hidden value="editerrormsg" name="operation">
</FORM>
	</div>
