...
String titlename = SystemEnv.getHtmlLabelName(82,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(25700,user.getLanguage());
String needfav ="1";
String needhelp ="";
String zh_tw = Util.null2String(rs.getPropValue("MutilLanguageProp","ZH_TW_LANGUAGE"));
String en = Util.null2String(rs.getPropValue("MutilLanguageProp","EN_LANGUAGE"));
%>
<BODY>
...
<form id="weaver" name="weaver" action="ErrorMsgOperation.jsp" method=post>
<wea:layout>
		<wea:group context='<%= SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
			...
			<wea:item>
				<wea:required id="indexdescspan" required="true" value="">
					<INPUT class=InputStyle maxLength=255 size=60 name="indexdesc" temptitle="<%=SystemEnv.getHtmlLabelName(433,user.getLanguage())%>" 
		...
		<wea:group context='<%= SystemEnv.getHtmlLabelNames("176,33439",user.getLanguage())%>'>
			<wea:item attributes="{'isTableList':'true'}">
				<wea:layout needImportDefaultJsAndCss="false" type="table" attributes="{'cols':'2','cws':'40%,60%'}">
					<wea:group context=""  attributes="{\"groupDisplay\":\"none\"}">
						...
						<wea:item>
							<%if(rs.getInt("id")==7){ %>
								<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" name="name_<%=rs.getInt("id") %>" onchange="setTWName(this);"/>
							<%}else if(rs.getInt("id")==9){ %>
								<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" name="name_<%=rs.getInt("id") %>" onchange="setCName(this,1);"/>
							<%}else{ %>
								<input class="InputStyle" type="text" id="name_<%=rs.getInt("id") %>" name="name_<%=rs.getInt("id") %>"/>
							<%} %>
						</wea:item>
						<%} %>
					</wea:group>
          ...
          <input type=hidden value="adderrormsg" name="operation">
</FORM>
	</div>
