...
<%
	...
	if("K3".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initK3(1);
	}else if("NC".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initNC(1);
	}else if("EAS".equals(typename)){
	}else if("U8".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initU8(1);
	}else if("NC5".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initNC5(1);
	}
...
%>
			<tr style="<%=tr_display %>">
				...
				<td class="field">
					<select id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>" name="D1_<%=fieldDbTbName %>_<%=fieldDbName %>" 
					...
					<%=FnaWfSet.getSelect(fieldIdListMain, fieldInfoHmMain, fieldValue_sel, user, formid, 
							"D1_"+fieldDbTbName+"_"+fieldDbName+"_sel", true, -1, true, display_k3_0_gsdm_sel, "workflowfieldValue_onchange(this);") %>
				<% 
				if(inputIsSelect==1){
				...
				%>
					<span id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_spanIpt" style="<%=display_k3_0_gsdm_ipt %>">
						<select id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" name="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" 
				...
				%>
					<input id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" name="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" 
						value="<%=FnaCommon.escapeHtml(fieldValue_ipt) %>" style="width:65%;<%=display_k3_0_gsdm_ipt %>" class="inputstyle" 
						onchange="workflowfieldValue_onchange(this);" />
				<%
				}
				%>
					<span id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_span" style="<%=display_k3_0_gsdm_span %>">
						<%=SystemEnv.getHtmlLabelName(18076,user.getLanguage()) %>：
						<select id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_datasourceid" name="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_datasourceid">
							<option></option>
							<%
						...
						<textarea id="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_sql" name="D1_<%=fieldDbTbName %>_<%=fieldDbName %>_sql" 
