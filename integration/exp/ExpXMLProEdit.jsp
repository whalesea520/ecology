...
String synType = "";//同步方式
String timeModul = "";//时间模式
String Frequency = "";//时间模式为按年时，表示第几天
 frequencyy = "";//时间模式为按年时，表示第几天
 createType = "";//计算类型，0,为表示正数，1，表示倒数
 createTime = "";//同步具体时间
String XMLType = "";//XML格式
String XMLEcodingType = "";//XML文件编码
String XMLFileType = "";//XML文件信息格式
String XMLHaveRemark = "";//XML包含流转意见
 xmltext = "";//-XML模板内容
...
if(!proId.equals("")){
	...
	String sql="select * from exp_XMLProSettings where id="+proId;
	rs.executeSql(sql);
	if(rs.next()){
		 ...
		 XMLType = Util.null2String(rs.getString("XMLType"));//XML格式
		 XMLEcodingType = Util.null2String(rs.getString("XMLEcodingType"));//XML文件编码
		 XMLFileType = Util.null2String(rs.getString("XMLFileType"));//XML文件信息格式
		 XMLHaveRemark = Util.null2String(rs.getString("XMLHaveRemark"));//XML包含流转意见
 ...
 <wea:layout>
	   ...
	   <wea:item><%=SystemEnv.getHtmlLabelNames("31691,63",user.getLanguage())%></wea:item><!-- 注册类型 -->
		<wea:item>
			<wea:required id="regitTypeimage" required="true" value="">
	           <select id="regitType" style='width:120px!important;' name="regitType" >
				  <option value=""></option>
				</select>
			</wea:required>
		</wea:item>
		  ...
		  <wea:item>
						<select id="synType" name="synType" style='width:180px!important;' onchange='changeSynType(this.value);'>
							<option value="0" <%if("0".equals(synType)){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(30107,user.getLanguage())%></option><!-- 手动同步 -->
							<option value="1" <%if("1".equals(synType)){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(32221,user.getLanguage())%></option><!-- 自动同步 -->
						</select>
		   </wea:item>
	...
	<!--  XML设置-->
	  <wea:group context="<%=SystemEnv.getHtmlLabelName(125781, user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
           <select id="XMLType" style='width:120px!important;' name="XMLType" onchange="changeXMLType();">
			  ...
			  <option value="1" <%if(XMLType.equals("1")) out.print("selected"); %>><%=SystemEnv.getHtmlLabelName(125782, user.getLanguage())%></option><!--  自由格式-->
			</select>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(125783, user.getLanguage())%></wea:item><!--  XML文件编码-->
		<wea:item>
           <select id="XMLEcodingType" style='width:120px!important;' name="XMLEcodingType" onchange="XMLchange(this);">
			  <option value="0" <%if("".equals(XMLEcodingType) || "0".equals(XMLEcodingType)) out.print("selected"); %>>UTF－8</option>
			  <option value="1" <%if("1".equals(XMLEcodingType)) out.print("selected"); %>>GB2312</option>
			    <option value="2" <%if("2".equals(XMLEcodingType)) out.print("selected"); %>>GBK</option>
			</select>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(125784, user.getLanguage())%></wea:item><!--  XML文件信息格式-->
		<wea:item>
           <select id="XMLFileType" style='width:120px!important;' name="XMLFileType" onchange="XMLchange();">
			  <option value="0"><%=SystemEnv.getHtmlLabelName(125785, user.getLanguage())%></option><!--  FTP路径 -->
			  <option value="1" <%if(XMLFileType.equals("1")) out.print("selected"); %>><%=SystemEnv.getHtmlLabelName(125786, user.getLanguage())%></option><!--  BASE64编码 -->
			</select>
		</wea:item>	
		<wea:item><%=SystemEnv.getHtmlLabelName(125787, user.getLanguage())%></wea:item><!-- XML包含流转意见 -->
	    <wea:item>
			<input class="inputstyle" type="checkbox" tzCheckbox='true' id="XMLHaveRemark" name="XMLHaveRemark" value="1" <%if(XMLHaveRemark.equals("1"))out.println("checked"); %>>
	    </wea:item>
	  ...
	  <wea:group context="<%=SystemEnv.getHtmlLabelName(125722, user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
          <input class="inputstyle" type="checkbox" tzCheckbox='true' id="ExpWorkflowFileFlag" name="ExpWorkflowFileFlag" value="1" <%if(ExpWorkflowFileFlag.equals("1"))out.println("checked"); %> onclick="showdetailToClear(this,'','ExpWorkflowFileForZipFlag');showdetail1();showdetail(this,'ExpWorkflowFileForZipFlagView');" >
		</wea:item >
		...
		<wea:item>
          <input class="inputstyle" type="checkbox" tzCheckbox='true' id="ExpWorkflowRemarkFileFlag" name="ExpWorkflowRemarkFileFlag" value="1" <%if(ExpWorkflowRemarkFileFlag.equals("1"))out.println("checked"); %> onclick="showdetailToClear(this,'','ExpWorkflowRemarkFileForZip');showdetail1();showdetail(this,'ExpWorkflowRemarkFileForZipView');">
		</wea:item>
		<!--  导出流转意见文档为ZIP-->
	  ...
	  <wea:group context="<%=SystemEnv.getHtmlLabelName(125728, user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
          <input class="inputstyle" type="checkbox" tzCheckbox='true' id="ExpWorkflowInfoFlag" name="ExpWorkflowInfoFlag" value="1" <%if(ExpWorkflowInfoFlag.equals("1"))out.println("checked"); %> onclick="showdetailToClear(this,'','');showdetail(this,'ExpWorkflowInfoPathView');">
		</wea:item>
		<!--  导出流程表单路径-->
...
function showdetail1() {
	...
	if(XMLFileType == '1'){
	 ...
	 if(checked){
	 	checked1 = $("#ExpSignFileFlag").attr("checked"); 
	 	if(checked1){
	 		showEle("ExpSignFileFlagView");
	 	}
	 }else{
	 	hideEle("ExpSignFileFlagView");
	 }
}
