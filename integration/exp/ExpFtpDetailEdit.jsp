...
String sum = Util.null2String(request.getParameter("sum"));
String oldname = Util.null2String(request.getParameter("name"));
...
rs.executeSql("select * from exp_ftpdetail where id="+sysid+"");
String name = "";
String adress = "";
String port = "";
String path = "";
...
if(rs.next()){
	...
	adress = Util.toScreenToEdit(rs.getString("adress"),user.getLanguage());
	port = Util.toScreenToEdit(rs.getString("port"),user.getLanguage());
	path = Util.toScreenToEdit(rs.getString("path"),user.getLanguage());
	ftpuser= Util.toScreenToEdit(rs.getString("ftpuser"),user.getLanguage());
	ftppwd = Util.toScreenToEdit(rs.getString("ftppwd"),user.getLanguage());
}
...
boolean usedFlag = new weaver.expdoc.ExpUtil().getShowmethod(sysid+"+0").equals("true");
%>
<BODY>
...
<wea:layout><!-- 基本信息 -->
	<wea:group context="<%=SystemEnv.getHtmlLabelName(81711,user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
            <wea:required id="adressimage" required="true" value='<%=adress%>'>
            	<input class=inputstyle type=text style='width:280px!important;' size=100 maxlength="100" name="adress"  value="<%=adress%>" onchange='checkinput("adress","adressimage")' _noMultiLang='true'>
            </wea:required>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(84629,user.getLanguage())%></wea:item>
		<wea:item>
            <wea:required id="portimage"  value='<%=port%>'>
            	<input class=inputstyle type=text style='width:140px!important;' size=10 maxlength="30" name="port" id="port" value="<%=port%>" _noMultiLang='true'>
            </wea:required>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(2072,user.getLanguage())%></wea:item>
		<wea:item>
            <wea:required id="ftpuserimage" required="true" value='<%=ftpuser%>'>
            	<input class=inputstyle type=text style='width:140px!important;' size=100 maxlength="100" name="ftpuser"  value="<%=ftpuser%>" onchange='checkinput("ftpuser","ftpuserimage")'>
            </wea:required>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(409,user.getLanguage())%></wea:item>
		<wea:item>
            <wea:required id="ftppwdimage" value='<%=ftppwd%>'>
            	<input class=inputstyle type=password style='width:200px!important;' size=100 maxlength="100" name="ftppwd"  value="<%=ftppwd%>" _noMultiLang='true'>
            </wea:required>
		</wea:item>		
...
function doAdd(){
	document.location.href="/integration/exp/ExpFtpDetailAdd.jsp?typename=<%=backto%>";
}
