...
if("exp".equals(operation)){
	...
	ExpWorkflowManager.doSendDoc(requestidstr, user.getUID()+"","");
}
int perpage=Util.getIntValue(request.getParameter("perpage"),0);
String PageConstId = "ExpLogList";
...
//out.println("select "+backfields+" "+fromSql+" "+sqlWhere+" order by t1.requestid ");
String tableString = "<table instanceid=\"archivingLogTable\" tabletype=\"checkbox\" pagesize=\""+PageIdConst.getPageSize(PageConstId,user.getUID(),PageIdConst.Browser)+"\" >"+
					...
					"           <col width=\"14%\"  text=\""+SystemEnv.getHtmlLabelName(125751 ,user.getLanguage())+"\" column=\"senddate\"  otherpara=\"column:sendtime\" orderkey=\"senddate, sendtime \"  transmethod=\"weaver.splitepage.transform.SptmForCrm.getTime\"/>"+ 
					"       </head>"+
				   	"<operates width=\"20%\">"+
...
<form action="ExpLogList.jsp" name="archivingLogForm" id="archivingLogForm">
	...
	<wea:layout type="fourCol">
		<wea:group context='<%= SystemEnv.getHtmlLabelName(26876,user.getLanguage())%>'>
			...
			<select style='width:120px!important;' id="status1" name="status">
				<option value="" <%if(status.equals("")) out.println("selected"); %>><%=SystemEnv.getHtmlLabelName(332,user.getLanguage())%></option><!-- 全部 -->
		...
		<wea:group context="" attributes="{'groupDisplay':'none'}">
			<wea:item type="toolbar">
					<!--QC:270807   [80][90]流程归档集成-调整高级搜索中按钮样式，以保持统一  e8_btn_submit-->
...
function doRefresh(){
	...
	window.location = "/integration/exp/ExpLogList.jsp?requestname="+requestname;
}
