...
String sql="select * from exp_workflowDetail where id='"+id+"'";
rs.executeSql(sql);
if(rs.next()){
...
<wea:layout><!-- 基本信息 -->
	<wea:group context="<%=SystemEnv.getHtmlLabelName(1361 ,user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item attributes="{'id':'workflow_td'}">
  			<brow:browser name="workflowid" viewType="0" hasBrowser="true" hasAdd="false" browserValue='<%=workflowid%>'  
					browserUrl="/systeminfo/BrowserMain.jsp?url=/workflow/workflow/WorkflowBrowser.jsp" isMustInput="2" isSingle="true" hasInput="true"
 					completeUrl="/data.jsp?type=workflowBrowser" width="300px" browserSpanValue='<%=workflowname%>'/>   
		</wea:item>
...
jQuery(document).ready(function () {
 ...
 $("#expid").val("<%=expid%>"); 
 	//解绑，绑定
	jQuery("#expid").selectbox("detach");
 	__jNiceNamespace__.beautySelect("#expid");
 	checkinput("expid","expidimage");
 changeProValue();
});
