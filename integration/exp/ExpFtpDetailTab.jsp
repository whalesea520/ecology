...
if(!HrmUserVarify.checkUserRight("intergration:expsetting", user)){
	...
	String url = "/integration/exp/ExpFtpDetailAdd.jsp?"+queryString;
	if("1".equals(urlType))
	{
		//navName = SystemEnv.getHtmlLabelNames("125745,87",user.getLanguage());//归档FTP注册信息
		url = "/integration/exp/ExpFtpDetailAdd.jsp?"+queryString;
	}else if("2".equals(urlType))
	{
		//navName = SystemEnv.getHtmlLabelNames("125745,87",user.getLanguage());//归档FTP注册信息
		url = "/integration/exp/ExpFtpDetailEdit.jsp?"+queryString;
	}
	
	if(id == 0){
		navName = SystemEnv.getHtmlLabelNames("126141",user.getLanguage());//归档FTP注册信息
	}else{
		rs.executeSql("select * from exp_ftpdetail where id="+id);
		if(rs.next()){
