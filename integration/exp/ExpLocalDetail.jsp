...
if(HrmUserVarify.checkUserRight("intergration:expsetting", user)){
	RCMenu += "{"+SystemEnv.getHtmlLabelName(125759,user.getLanguage())+",javascript:add(),_self} " ;
...
String backfields=" * " ;
String perpage="10";
String PageConstId = "ExpLocalDetail";
String fromSql=" exp_localdetail "; 
...
tableString += " <checkboxpopedom    popedompara=\"column:id+1\" showmethod=\"weaver.expdoc.ExpUtil.getShowmethod\"  />"+
         ...
         "       <head>"+
         "           <col width=\"50%\"  text=\""+SystemEnv.getHtmlLabelName(195,user.getLanguage())+"\" column=\"name\" orderkey=\"name\" transmethod=\"weaver.expdoc.ExpUtil.getNameLink\"  otherpara=\"column:id\" />"+
		 "           <col width=\"50%\"  text=\""+SystemEnv.getHtmlLabelName(84630 ,user.getLanguage())+"\" transmethod=\"weaver.expdoc.ExpUtil.getnbps\" column=\"path\" orderkey=\"path\" />"+
         "       </head>"+
         "<operates width=\"20%\">"+
...
<form action="ExpLocalDetail.jsp" method="post" name="datalist" id="datalist" >
...
function add(){
	...
	var title = "<%=SystemEnv.getHtmlLabelName(125759,user.getLanguage())%>";
...
function doDelete(ids){
	...
	top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(15097,user.getLanguage())%>", function (){
		self.location.href="/integration/exp/ExpLocalDetailOperation.jsp?backto=<%=backto%>&operation=delete&id="+ids;
	}, function () {}, 320, 90);	
}
function doDeleteById(id)
{
	if(id=="") return ;
	top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(15097,user.getLanguage())%>", function (){
		self.location.href="/integration/exp/ExpLocalDetailOperation.jsp?backto=<%=backto%>&operation=delete&id="+id;
	}, function () {}, 320, 90);	
}
function doEditById(id){
	if(id=="") return ;
	var url = "/integration/exp/ExpLocalDetailTab.jsp?urlType=2&isdialog=1&backto=<%=backto%>&id="+encodeURI(encodeURI(id));
	var title = "<%=SystemEnv.getHtmlLabelName(125759,user.getLanguage())%>";
	openDialog(url,title);
}
function doLog(id){
