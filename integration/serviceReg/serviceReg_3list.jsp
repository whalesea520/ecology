...
<html>
	...
	<%
		...
		tableString =   " <table instanceid=\"sendDocListTable\" tabletype=\"checkbox\" pagesize=\""+perpage+"\" >"+
                ...
                "       <head>"+
                "           <col width=\"20%\"  text=\""+ SystemEnv.getHtmlLabelName(30660,user.getLanguage()) +"\" column=\"poolname\"   />"+
                "           <col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(30672,user.getLanguage()) +"\" column=\"regname\"  transmethod=\"com.weaver.integration.util.IntegratedMethod.getRegname\"  otherpara=\"column:id\"  />"+
				"           <col width=\"20%\"  text=\""+ SystemEnv.getHtmlLabelName(30713,user.getLanguage()) +"\" column=\"funname\"    />"+
				"           <col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(30720,user.getLanguage()) +"\" column=\"serdesc\"   />"+
                "       </head>"+
                " </table>";
	%>
	<body>
...
<%
RCMenu += "{"+SystemEnv.getHtmlLabelName(81695,user.getLanguage())+",javascript:doAdd(this),_self} " ;
...
<table id="topTitle" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		...
		<td class="rightSearchSpan"
			...
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(81695,user.getLanguage())%>" id="zd_btn_submit_0" class="e8_btn_top" onclick="doAdd(this);">
...
$(document).ready(function(){
	...
	jQuery("#tabDiv").remove();
		...
		function doAdd(obj)
		{
			var url="/integration/serviceReg/serviceReg_3New.jsp?hpid=<%=hpid%>";
			var title="<%=SystemEnv.getHtmlLabelName(81695,user.getLanguage())%>";
			showDialog(title,url,500,360,false);
		}
		function doUpdate(id)
		{
			var url="/integration/serviceReg/serviceReg_3New.jsp?isNew=1&id="+id;
			var title="<%=SystemEnv.getHtmlLabelName(81695,user.getLanguage())%>";
