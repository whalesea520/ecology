...
<wea:layout><!-- 基本信息 -->
	<wea:group context="<%=SystemEnv.getHtmlLabelName(1361 ,user.getLanguage())%>" attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
            <wea:required id="workflownameimage" required="true">
            	<input class=inputstyle type=text style='width:280px!important;' size=100 maxlength="100"  id="workflowname" _noMultiLang='true'  name="workflowname"  onchange='checkinput("workflowname","workflownameimage");checkvalues(this.name);'>
            </wea:required>
		</wea:item>	
		<wea:item><%=SystemEnv.getHtmlLabelNames("18526,18015" ,user.getLanguage())%></wea:item><!--接收流程-->
		<wea:item>
	           <input <%if(sysreceivewfdata == 0){ %>disabled<%}else if(sysreceivewfdata == 1){%>checked<%} %> class="inputstyle" type="checkbox" tzCheckbox='true' id="receivewfdata" name="receivewfdata" value="1" >
		</wea:item>	
	</wea:group>
</wea:layout>
<br>
