...
<%
	...
	if("K3".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initK3(0);
	}else if("NC".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initNC(0);
	}else if("EAS".equals(typename)){
	}else if("U8".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initU8(0);
	}else if("NC5".equals(typename)){
		fieldNameList = fnaVoucherObjInit.initNC5(0);
	}
...
%>
			<tr style="<%=tr_display %>">
				...
				<td class="field" style="vertical-align: top;">
					<select id="M_<%=fieldDbTbName %>_<%=fieldDbName %>" name="M_<%=fieldDbTbName %>_<%=fieldDbName %>" 
					...
					<%=FnaWfSet.getSelect(fieldIdListMain, fieldInfoHmMain, fieldValue_sel, user, formid, 
							"M_"+fieldDbTbName+"_"+fieldDbName+"_sel", true, -1, true, display_k3_0_gsdm_sel, "workflowfieldValue_onchange(this);") %>
				<% 
				if(inputIsSelect==1){
				...
				%>
					<span id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_spanIpt" style="<%=display_k3_0_gsdm_ipt %>">
						<select id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" name="M_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" 
				...
				%>
					<input id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" name="M_<%=fieldDbTbName %>_<%=fieldDbName %>_ipt" 
						value="<%=FnaCommon.escapeHtml(fieldValue_ipt) %>" style="width:65%;<%=display_k3_0_gsdm_ipt %>" class="inputstyle" 
						onchange="workflowfieldValue_onchange(this);" />
				<%
				}
				%>
					<span id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_span" style="<%=display_k3_0_gsdm_span %>">
						<%=SystemEnv.getHtmlLabelName(18076,user.getLanguage()) %>：
						<select id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_datasourceid" name="M_<%=fieldDbTbName %>_<%=fieldDbName %>_datasourceid">
							<option></option>
							<%
						...
						<textarea id="M_<%=fieldDbTbName %>_<%=fieldDbName %>_sql" name="M_<%=fieldDbTbName %>_<%=fieldDbName %>_sql" 
