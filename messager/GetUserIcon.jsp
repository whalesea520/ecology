...
<HTML>
	...
	<form name="frmMain" method="post" action="GetUserIconOpreate.jsp?trmphomepage=<%=requestFrom%>&temploginid=<%=loginid%>" enctype="multipart/form-data">
...
<SCRIPT LANGUAGE="JavaScript">
	$(document).ready( function() {	
		$("#fileSrcUrl").change( function() {			
			...
			if(imgUrl!=''){
				if(imgUrl.toLowerCase().indexOf(".gif")==-1 && imgUrl.toLowerCase().indexOf("jpg")==-1 && imgUrl.toLowerCase().indexOf("png")==-1) {
					reSelect();
	...
	function doApply(){
		...
		if($.trim(srcUrl.value)==""&&$.trim($("#divSelected").css("display"))!="none"){
			window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(132006, user.getLanguage())%>");
			return;
		} else if (srcUrl.toLowerCase().indexOf(".gif")==-1 && srcUrl.toLowerCase().indexOf("jpg")==-1 && srcUrl.toLowerCase().indexOf("png")==-1){
			reSelect();
			window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(132006, user.getLanguage())%>");
			return;
		} else {
	...
	function reSelect(){
		frmMain.action="GetUserIconOpreate.jsp?trmphomepage=<%=requestFrom%>&temploginid=<%=loginid%>";
 ...
 if("true".equals(isclosed)){
	...
	if(requestFrom.equals("homepage")){
      if("1".equals(isManager)){
	   ...
	   out.println("<script>parent.imgWindow.hide();parent.reloadMyLogo();window.location='GetUserIcon.jsp?isManager="+isManager+"&userId="+userId+"&subcompanyid="+subcompanyid+"&loginid="+loginid+"'</script>"); 
 }
  %>
