...
String needlawcheck=RecordSet.getString("needlawcheck");
...
%>
	...
	if(fieldhtmltype.equals("3")){
		...
		if(fieldtype.equals("17")|| fieldtype.equals("18")){
			...
			if(approveid.equals(""))
				approveid="0";
			sql="select "+keycolumname+","+columname+" from "+tablename+" where "+keycolumname+" in( "+approveid+")";
			...
			while(RecordSet.next()){
				...
				if(!linkurl.equals("")){
				%>
			        <a href='<%=linkurl%><%=showid%>&isrequest=1&requestid=<%=requestid%>'><%}%>
			        <%=Util.toScreen(showname,user.getLanguage())%>
			        <%if(!linkurl.equals("")){%></a><%}
			}
			    
		}else {
			int intfieldvalue=Util.getIntValue(approveid,0);
			String tablename=BrowserComInfo.getBrowsertablename(fieldtype);
			String columname=BrowserComInfo.getBrowsercolumname(fieldtype);
			String keycolumname=BrowserComInfo.getBrowserkeycolumname(fieldtype);
			...
			if(!linkurl.equals("")){
			%>
		        <a href='<%=linkurl%><%=intfieldvalue%>&isrequest=1&requestid=<%=requestid%>'><%}%><%=Util.toScreen(showname,user.getLanguage())%><%if(!linkurl.equals("")){%></a>
		        <%}
		}
	}
	else if(fieldhtmltype.equals("4")){%>
        <input type=checkbox value=1 name="prefield_<%=fieldid%>" DISABLED <%if(needlawcheck.equals("1")){%> checked <%}%>> 
<%    }
%>
      </td>
    </tr>
