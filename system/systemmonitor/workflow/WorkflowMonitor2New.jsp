...
if("".equals(workflowid)){
	workflowid = Util.null2String((String)session.getAttribute(user.getUID()+""+"sessionMonitorWorkflowid"));
}
session.setAttribute(user.getUID()+""+"sessionMonitorWorkflowid", workflowid);
...
sqlwhere="where t1.workflowid=b.workflowid and b.monitorhrmid = " + user.getUID()+" and d.requestid=t1.requestid ";
...
//自定义结束
	...
	if(isfirst == 0){
		...
		if(!"".equals(firstWFids)){
			sqlwhere += " and t1.workflowid in ("+firstWFids+")  ";
		}else{
			sqlwhere += " and t1.workflowid in ("+workflowid+")  ";
		}
	}
...
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(91,user.getLanguage())+",javascript:deleteWorkflow(),_self}" ;
RCMenuHeight += RCMenuHeightStep ;
if(RecordSet.getDBType().equals("db2")){
	RCMenu += "{"+SystemEnv.getHtmlLabelName(83,user.getLanguage())+",/systeminfo/SysMaintenanceLog.jsp?isNew=2New&sqlwhere=where int(operateitem)=85,_self} " ;
}else{
	RCMenu += "{"+SystemEnv.getHtmlLabelName(83,user.getLanguage())+",/systeminfo/SysMaintenanceLog.jsp?isNew=2New&sqlwhere=where operateitem=85,_self} " ;
}
RCMenuHeight += RCMenuHeightStep ;
//翻页按钮
...
<form name="frmmain" method="post" action="WorkflowMonitor2New.jsp">
...
<button class=Browser  onfocus="changelevel('<%=tmpcount%>')"  onclick="onShowBrowser('<%=id%>','/systeminfo/BrowserMain.jsp?url=/hrm/company/DepartmentBrowser.jsp')"></button>
<input type=hidden name="con<%=id%>_value" value="<%=value%>">
<input type=hidden name="con<%=id%>_name" value="<%=showName%>">
<span name="con<%=id%>_valuespan" id="con<%=id%>_valuespan"><%=showName%></span>
</td>
                      ...
                      <td COLSPAN=4>
						<div id='divshowreceivied' style='background:#FFFFFF;padding:3px;width:100%' valign='top'>
                          ...
                          <%
                           String tableString = "";
                            ...
                            //String backfields = " t1.requestid, t1.createdate, t1.createtime,t1.creater, t1.creatertype, t1.workflowid, t1.requestname, t1.status,t1.requestlevel,t1.currentnodeid,b.isview ";
                            String backfields = " t1.requestid, t1.createdate, t1.createtime,t1.creater, t1.creatertype, t1.workflowid, t1.requestname, t1.status,t1.requestlevel,t1.currentnodeid,t1.currentnodetype,t1.currentstatus,b.isview ";
							//加上自定以字段
                            ...
                            String fromSql  = " from workflow_requestbase t1, workflow_monitor_bound b, "+tablename+" d ";
                            String sqlWhere = sqlwhere;
                            String para2="column:requestid+column:isview";
							//String paraOpt = "column:requestid+"+CurrentUser+"+"+logintype+"+"+userlang+"+2New";
							String paraOpt = "column:requestid+"+CurrentUser+"+"+logintype+"+"+userlang+"+column:currentstatus+column:creater+column:currentnodetype";
						...
						while (RecordSet.next()) {
                          if(RecordSet.getString("id").equals("-3")) 
                                    ...
                                    tableString+="				<col  text=\""+SystemEnv.getHtmlLabelName(1334,user.getLanguage())+"\" column=\"requestname\" orderkey=\"requestname\" transmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultFlowName\"  otherpara=\""+para2+"\"/>";
                        else  if(RecordSet.getString("id").equals("-6")) //-6
                                    tableString+="				<col  text=\""+SystemEnv.getHtmlLabelName(524,user.getLanguage())+SystemEnv.getHtmlLabelName(15586,user.getLanguage())+"\" column=\"currentnodeid\" transmethod=\"weaver.general.WorkFlowTransMethod.getCurrentNode\"/>";
                        //else  if(RecordSet.getString("id").equals("-7")) //-7
						 ...
						 tableString+="			    <col  text=\""+label+"\"  column=\""+name+"\"  otherpara=\""+para3+"\"  transmethod=\"weaver.general.WorkFlowTransMethod.getOthers\"/>";
                          ...
                          if(hasrequestname)
                                    tableString+="				<col width=\"19%\"  text=\""+SystemEnv.getHtmlLabelName(1334,user.getLanguage())+"\" column=\"requestname\" orderkey=\"requestname\" transmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultFlowName\"  otherpara=\""+para2+"\"/>";
                          if(hascurrentnode)
						  ...
						  if(hasSubWorkflow) 
									...
									tableString+="           <col width=\"8%\"  text=\""+SystemEnv.getHtmlLabelName(18361,user.getLanguage())+"\" column=\"workflowid\" transmethod=\"weaver.general.WorkFlowTransMethod.getMonitorLink\" otherpara=\""+paraOpt+"\" />";
                                    tableString+="			</head>"+   			
                                                 "</table>";
                          %>
...
function deleteWorkflow(){
	if(isdel()) {
        document.frmmain.multiRequestIds.value = _xtable_CheckedCheckboxId();
        document.frmmain.operation.value='deleteworkflow';
        document.frmmain.action='/system/systemmonitor/MonitorOperation.jsp?isNew=2New';
        document.frmmain.submit();
	}
}
</script>
