...
<!-- add by liaodong for qc63770  in  20131015  start-->
<jsp:useBean id="cateRecordSet" class="weaver.conn.RecordSet" scope="page"/>
<!--end-->
...
String sqlWhere = "";
if(!twfname.equals(""))
	sqlWhere+=" and (wb.workflowname = '"+Util.fromScreen2(twfname,7)+"' or wb.workflowname like '%"+Util.fromScreen2(twfname,7)+"%')" ; // 2012-09-07 ypc 修改
if(!wfid.equals(""))
...
while(RecordSet.next()){
  ...
  //微信提醒(QC:98106)
    ...
    String docPath=RecordSet.getString("docPath");
	 //update by liaodong for qc63770 in 2013年10月15日 start
    String selectedCateName = "";
    String catelogType = RecordSet.getString("catelogType"); //文档存放目录类型 0 固定 1选择
    String selectedCateLog = RecordSet.getString("selectedCateLog");//选择目录的编号
    if(null!=catelogType && !"".equals(catelogType) && "1".equals(catelogType)){
        String sqlSelectCatalog = "";
	         if("1".equals(isbill)){
			     sqlSelectCatalog = "select formField.id,fieldLable.labelName as fieldLable "
	                         ...
	                         + "  and formField.id = '"+selectedCateLog+"'"
			  ...
			  sqlSelectCatalog = "select formDict.ID, fieldLable.fieldLable "
	                         ...
	                         + " and formDict.ID = '"+selectedCateLog+"'"
		   ...
		   if(!"".equals(sqlSelectCatalog)){
		            cateRecordSet.executeSql(sqlSelectCatalog);
		            if(cateRecordSet.next()){
		               selectedCateName = cateRecordSet.getString("fieldLable");
		            }
		   }
    }
    //end
    ...
    RecordSet_tmp.execute(Sql);
    while(RecordSet_tmp.next()){
    	if(bill.equals("1")){//单据无法将字段名称作为查询条件，在这里进行处理
			fieldlablename += SystemEnv.getHtmlLabelName(RecordSet_tmp.getInt("fieldlabel"),user.getLanguage())+",";
		}else{
			fieldlablename += Util.null2String(RecordSet_tmp.getString("fieldlable"))+",";
		}
    }
...
%>
  <tr> 
    ...
    <tr class=header>
		...
		%>
        <td><%=SystemEnv.getHtmlLabelName(33806,user.getLanguage())%></td><!-- 工作流类型 -->
    ...
    <tr class=header>
		...
		%>
        <td class=field><%=SystemEnv.getHtmlLabelName(31494,user.getLanguage())%></td>
        <td class=field>
        	<b>
        ...
        <!-- 2012-09-2-17 ypc 添加 -->
          ...
          <!-- 退回是否提醒 -->
          <td class=field><%=SystemEnv.getHtmlLabelName(31497,user.getLanguage())%></td>
          ...
          <!-- 退回时是否可设置提醒节点  -->
           <td class=field><%=SystemEnv.getHtmlLabelName(31498,user.getLanguage())%></td>
          ...
          <%} %>
           ...
           <!-- 新建时是否可导入流程  -->
           <td class=field><%=SystemEnv.getHtmlLabelName(31499,user.getLanguage())%></td>
          ...
          <%} %>
     	  <!-- 相关流程意见不显示  -->
          <td class=field><%=SystemEnv.getHtmlLabelName(31500,user.getLanguage())%></td>
                        ...
                        case 51:
                        	...
                        	if(type == 5)
                        		...
                        		tmpdesc+=SystemEnv.getHtmlLabelName(141,user.getLanguage());
														...
														viewtype = RecordSet2.getString("viewtype");
												  		groupid = RecordSet2.getString("groupid");
												  		String name = "";
												  		if(isbill.equals("0")) 
			...
			<%
				switch (type){
					...
					case 3:
						...
						//while(RecordSet4.next()){
						//String lastname=Util.null2String(RecordSet4.getString("lastname"));
						//String uid=Util.null2String(RecordSet4.getString("id"));
					%>
