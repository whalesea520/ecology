...
<FORM id=frmain name=frmain method=post target="mainFrame">
	<tr class=Header> 
      <th height = 30 colspan=2><%=SystemEnv.getHtmlLabelName(6058,user.getLanguage())%> >>&nbsp;&nbsp;&nbsp;
	  <%=SystemEnv.getHtmlLabelName(15089,user.getLanguage())%>：
	  <BUTTON class=Browser id=SelectResourceid onClick="onShowHrmID(Resourceidspan,resourceid)"></BUTTON>
		  <span id=Resourceidspan></span> 
          <INPUT class=saveHistory type=hidden name="resourceid">
	  </th>
    </tr>	
</FORM>
</table>
...
sub onShowHrmID(getSpan,getInput)
	...
	getInput.value=id(0)
	frmain.action="/hrm/resource/HrmResourcePlan.jsp"
	frmain.submit()
	else 
