...
<form id="frmMain" name="frmMain" method="post" action="/system/sysdetach/AppDetachEditRange.jsp">
...
String sqlwhere = " a.infoid = "+mainId+" and sourcetype = 1 ";
...
String tableString=""+
	"<table pageId=\""+pageId+"\" instanceid=\"sysadminEditBatchTable\" "+
		...
		"<head>"+							 
			...
			"<col width=\"45%\" text=\""+SystemEnv.getHtmlLabelNames("19467", user.getLanguage())+"\" column=\"content\""+
...
function doBatchDel(){
	...
	var _data = "operation=rangeBatchDel&mainId=<%=mainId %>&ids="+ids;
...
function doDel_grid(id){
	var _data = "operation=rangeDel&mainId=<%=mainId %>&id="+id;
...
function doEdit_grid(id, param2, param3, tabId, parentWin){
	...
	openDialog("/system/sysdetach/AppDetachEditRangeEdit.jsp?mainId=<%=mainId %>&id="+id, 
			"<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())+SystemEnv.getHtmlLabelNames("19467,68",user.getLanguage()) %>", 
			_w, _h);
}
...
function addNewRow(){
	var _w = 500;
	var _h = 280;
	openDialog("/system/sysdetach/AppDetachEditRangeEdit.jsp?mainId=<%=mainId %>", 
			"<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelNames("19467,68",user.getLanguage()) %>", 
			_w, _h);
}
