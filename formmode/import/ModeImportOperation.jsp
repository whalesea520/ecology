...
String getImportTitle(int type,User user)
{
	...
	switch(type)
	{
		...
		case 30:
			...
			break;
		case 31:
			title = SystemEnv.getHtmlLabelName(124947,user.getLanguage());
			break;
		default:
			title = "";
			break;
...
FormmodeDataService.importFormmodeByXml(xmlfilepath,appid,isAppImport,subCompanyId);
%>
<HTML><HEAD>
...
<BODY onload="displayExcel();">
	...
	out.println("<TABLE class=liststyle id=oTable><COLGROUP><COL width='25%'><COL width='25%'><COL width='30%'><COL width='15%'><COL width='5%'>");//导入结果

   	out.println("<tr class=Title>");
    out.println("<th>"+SystemEnv.getHtmlLabelName(31830,user.getLanguage())+"id</th>");//关联
    out.println("<th>"+SystemEnv.getHtmlLabelName(21900,user.getLanguage())+"</th>");//表名
    out.println("<th>"+SystemEnv.getHtmlLabelName(104,user.getLanguage())+"</th>");//操作
    out.println("<th>"+SystemEnv.getHtmlLabelName(433,user.getLanguage())+"</th>");//描述
    out.println("<th>"+SystemEnv.getHtmlLabelName(82342,user.getLanguage())+"</th>");//导入状态
    out.println("</tr>");
