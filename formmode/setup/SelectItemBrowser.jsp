...
int fromDialog = Util.getIntValue(request.getParameter("fromDialog"), 0);
...
<input type="hidden" name="fromDialog" value="<%=fromDialog%>">
			 ...
			 <table class="e8_tblForm" style="margin: 10px 0;">
			   <TR>
				 <TD width=20% class="e8_tblForm_label"><!--  选择项名称 -->
				   <%=SystemEnv.getHtmlLabelName(82480, user.getLanguage()) %>
				 </TD>
				 <TD width=30% class="e8_tblForm_field" colspan="3">
				 	<input name="selectname" id="selectname" value="<%=selectname%>" class="InputStyle">
				 </TD>
			  ...
			  <%
			  String perpage = "12";
			  String backFields = " a.id,a.selectitemname ";
			  String sqlFrom = "from mode_selectitempage a  " ;
...
/*               if(fmdetachable.equals("1")){
			  ...
			  if(!selectname.equals("")){
				  SqlWhere += " and a.selectitemname like '%"+selectname+"%'";
			  }
			  if(appid>0){
		  	 ...
		  	 String tableString=""+
				"<table pagesize=\""+perpage+"\" tabletype=\"none\">"+
					"<sql backfields=\""+backFields+"\" sqlform=\""+sqlFrom+"\" sqlprimarykey=\"a.id\" sqlsortway=\"desc\" sqldistinct=\"true\" sqlwhere=\""+Util.toHtmlForSplitPage(SqlWhere)+"\"/>"+
						"<head>"+            //自定义表单
							"<col width=\"50%\"  text=\""+ SystemEnv.getHtmlLabelName(82480, user.getLanguage()) +"\" column=\"selectitemname\" orderkey=\"selectitemname\"  otherpara=\"column:id+column:issystem+column:issystemflag+"+user.getLanguage()+"\" />"+
						"</head>"+
				"</table>";
			  %>
...
function BrowseTable_onclick(e){
	...
	if( target.nodeName =="TD"||target.nodeName =="A"  ){
		...
		if(vstr=="<%=SystemEnv.getHtmlLabelName(33885,user.getLanguage())%>"){//虚拟表单
   	 	...
   	 	var nameValue = "<a href=\"#\" onclick=\"toformtabFormChoosed('"+idValue+"_"+isvirtualform+"')\" style=\"TEXT-DECORATION:none\">"+
