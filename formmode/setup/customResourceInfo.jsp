...
%>
<%
int resourceId = Util.getIntValue(request.getParameter("id"), 0);
...
Map<String, Object> data = customResourceService.getCustomResourceById(resourceId);
String resourcename = Util.null2String(data.get("resourcename"));
%>
<html>
<head>
	<title></title>
...
<body>
	<div class="e8_right_top">
		...
		<div class="e8_baseinfo">
			<div class="e8_baseinfo_name"><!-- 自定义页面 -->
				<%=SystemEnv.getHtmlLabelName(124947,user.getLanguage())%>/ <span class="e8_baseinfo_name"  ><%=resourcename %></span><!-- 资源面板 -->
			</div>
			<div class="e8_baseinfo_modify"><!-- 最后编辑时间 -->
				<%LogService logService = new LogService(); %>
				<%=SystemEnv.getHtmlLabelName(82013,user.getLanguage())%>：<%=logService.getLastOptTime(resourceId, Module.RESOURCE) %>
			</div>
		</div>
	...
	<div class="e8_right_center">
		<div id="tabs-content-1" class="e8_right_frameContainer">
			<iframe id="baseFrame" name="baseFrame" frameborder="0" style="width: 100%;height: 100%;" scrolling="auto" lazy-src="/formmode/setup/customResourceBase.jsp?id=<%=resourceId %>&appid=<%=appid %>">
			
			</iframe>
		</div>
		<div id="tabs-content-2" class="e8_right_frameContainer">
			<iframe id="logFrame" name="logFrame" frameborder="0" style="width: 100%;height: 100%;" scrolling="auto" lazy-src="/formmode/setup/log.jsp?objid=<%=resourceId%>&logmodule=<%=Module.RESOURCE %>">
