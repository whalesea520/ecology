...
JSONArray reportinfoData = new JSONArray();
if(fmdetachable.equals("1")){
	reportinfoData = reportInfoService.getReportInfoByAppIdWithJSONDetach(appId,currentSubCompanyId);
}else{
	reportinfoData = reportInfoService.getReportInfoByAppIdWithJSON(appId);
}
...
<head>
	...
	<script type="text/javascript">
		...
		var datas = <%=reportinfoData.toString()%>;
		
		var currentDatas;
		
		var currReportId = null;
		
		$(document).ready(function () {
			
			currReportId = FormmodeUtil.getLastId(FormModeConstant._CURRENT_REPORTINFO, datas, "id");
			currPageIndex = FormmodeUtil.getLastIdPageIndex(currReportId,datas,"id",pageSize);
			...
			var url = "";
			//if(currReportId != null){
				url = "/formmode/setup/reportinfoManage.jsp?id="+currReportId+"&appid=<%=appId%>";
			//}
			...
			currentDatas = datas;
			doSearchTextChange();
			changeRightFrameUrl(currReportId);
		});
		...
		function onSearchTextChange(text){
			...
			if(text == ""){
				...
				for(var i = 0; i < currentDatas.length; i++){
					if(currentDatas[i].reportname.toLowerCase().indexOf(text) != -1 || currentDatas[i].reportdesc.toLowerCase().indexOf(text) != -1){
		...
		function changeRightFrameUrl(id, AElement){
			...
			currReportId = id;
			FormmodeUtil.writeCookie(FormModeConstant._CURRENT_REPORTINFO, id);
			
			$("#rightFrame").attr("src", "/formmode/setup/reportinfoManage.jsp?id="+id+"&appid=<%=appId%>");
		}
		
		function onPagedCallback(){
			if(currReportId != null){
				var $currForm = $("#A_" + currReportId);
		...
		function pagedDataRender(data){
			return "<a id=\"A_"+data["id"]+"\" href=\"javascript:void(0);\" onclick=\"javascript:changeRightFrameUrl("+data["id"]+",this);\">" +
						"<div class=\"e8_data_label\">"+data["reportname"]+"</div>" +
						"<div class=\"e8_data_label2\">"+data["reportdesc"]+"</div>" +
					"</a>";
		}
		...
		function refreshData(){
			var url = "/weaver/weaver.formmode.servelt.ReportInfoAction?action=getReportByAppIdWithJSON&appid=<%=appId%>&fmdetachable=<%=fmdetachable%>&subCompanyId=<%=currentSubCompanyId%>";
		...
		function right() {
			...
			$("#rightFrame").attr("src","/formmode/setup/reportinfoManage.jsp?appid=<%=appId%>");
		}
...
<body>
	...
	<%
		if(operatelevel>0&&(fmdetachable.equals("1")&&!treelevel.equals("0")||!fmdetachable.equals("1"))){
			RCMenu += "{"+SystemEnv.getHtmlLabelName(82260,user.getLanguage())+",javascript:right(),_top} " ;//新建报表
