 ...
 <script type="text/javascript">
		...
		$(function(){
		    $('.e8_box').Tabs({
		        ...
		        staticOnLoad:true,
		        objName:"<%=SystemEnv.getHtmlLabelNames("18214,82200",user.getLanguage())%>"
		   	});
		}); 
	</script>
	<div class="e8_box demo2">
		  <div class="e8_boxhead">
		   ...
		   <div class="tab_box">
		        <div>
		            <iframe src="FormmodeFieldBrowserIframe.jsp?modeId=<%=modeId%>&type=<%=type%>&selfieldid=<%=selfieldid%>&fieldids=<%=fieldids%>&rownum=<%=rownum%>" onload="if(typeof(update)=='function'){update()}" id="tabcontentframe" name="tabcontentframe" class="flowFrame" frameborder="0" height="100%" width="100%;"></iframe>
		        </div>
		   </div>
	</div> 
		...
		<wea:layout needImportDefaultJsAndCss="false">
			<wea:group context=""  attributes="{\"groupDisplay\":\"none\"}">
				<wea:item type="toolbar">
			    	<input type="button" value="<%=SystemEnv.getHtmlLabelName(309,user.getLanguage())%>" id="zd_btn_cancle"  class="zd_btn_cancle" onclick="dialog.close();" style="width: 60px!important;">
				</wea:item>
			</wea:group>
		</wea:layout>
		</div>
