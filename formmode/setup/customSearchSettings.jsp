...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			currentDatas = datas;
			doSearchTextChange();
			changeRightFrameUrl(currCustomSearchId);
		});
		...
		function changeRightFrameUrl(id){
			...
			currCustomSearchId = id;
			FormmodeUtil.writeCookie(FormModeConstant._CURRENT_CUSTOMSEARCH, id);
			
			$("#rightFrame").attr("src", "/formmode/setup/customSearchInfo.jsp?id="+id+"&appid=<%=appId%>");
		}

		function onPagedCallback(){
			if(currCustomSearchId != null){
				var $currForm = $("#A_" + currCustomSearchId);
		...
		function refreshData(){
			var url = "/weaver/weaver.formmode.servelt.CustomSearchAction?action=getCustomSearchByAppIdWithJSON&appid=<%=appId%>&fmdetachable=<%=fmdetachable%>&subCompanyId=<%=currentSubCompanyId%>";
		...
		function refreshCustomSearch(id){
		...
		function right() {
			...
			$("#rightFrame").attr("src","/formmode/setup/customSearchInfo.jsp?appid=<%=appId%>");
		}
		...
		function copyCustomSearcSetting(){
			...
			if(rightWin.copyCustomSearcSetting){
				rightWin.copyCustomSearcSetting();
			}
		}
...
<body>
	...
	<%
		if(operatelevel>0&&(fmdetachable.equals("1")&&!treelevel.equals("0")||!fmdetachable.equals("1"))){
			RCMenu += "{"+SystemEnv.getHtmlLabelName(30543,user.getLanguage())+",javascript:right(),_top} " ;//新建查询
			RCMenuHeight += RCMenuHeightStep ;
			RCMenu += "{"+SystemEnv.getHtmlLabelName(82046,user.getLanguage())+",javascript:copyCustomSearcSetting(),_top} " ;//复制查询
			RCMenuHeight += RCMenuHeightStep ;
		}
	%>
