...
var formOperate=(function (){
	...
	function getMaxCell(ecs){
   		...
   		for(var item in ecs){
   			...
   			var cur_col = parseInt(item.split(",")[1]) + parseInt(ecObj.colspan) - 1;
   			if(cur_row > maxrow)
   				maxrow = cur_row;
   			if(cur_col > maxcol)
   				maxcol = cur_col;
	...
	function saveLayoutWindow(flag){
    	...
    	var save2Json = "{";
        	...
        	save2Json += "\"modeid\":\""+jQuery("#modeid").val()+"\",";
	...
	function resumeSheetData(symbol, excelDiv){
    	...
    	for(var cellid in ecs){
			...
			//处理面板恢复特殊字符Bug
			resumeResetCellValue(sheet, r, c);
			...
			if(etype === celltype.FNAME || etype === celltype.FCONTENT || etype === celltype.SUMFNAME || etype === celltype.SUMFCONTENT){
				if(judgeFieldExist(efieldid)){
					...
					if(etype === celltype.FCONTENT){
						...
						if(wfinfo.layouttype == "0" || wfinfo.layouttype == "3" || wfinfo.layouttype == "4")	//显示、监控、打印模式图片变灰
							fieldattr = 1;
						//根据字段属性获取对应图片
	...
	function initDataobj(symbol){
   	...
   	//针对插件对<、>、&三特殊字符保存后被转换Bug修复
   	function resumeResetCellValue(sheet, r, c){
   		var cvalue = sheet.getCell(r,c,$.wijmo.wijspread.SheetArea.viewport).value();
   		if(!!cvalue){
   			cvalue = cvalue+"";		//必须转为字符串，否则数字indexOf会报错
	   		if(cvalue.indexOf("&lt;") >-1 || cvalue.indexOf("&gt;") >-1 ||cvalue.indexOf("&amp;") >-1){
	   			cvalue = cvalue.replaceAll("&lt;", "<");
	   			cvalue = cvalue.replaceAll("&gt;", ">");
	   			cvalue = cvalue.replaceAll("&amp;", "&");
	   			sheet.getCell(r,c,$.wijmo.wijspread.SheetArea.viewport).value(cvalue);
	   		}
   		}
   	}
...
var tabOperate = (function (){
	...
	function insertTabPage(){
		var imgsrc = "/formmode/exceldesign/image/rightBtn/tabsmall_wev8.png";
	...
	function bindTabEvent(){
		...
		$(".tabdiv .changeskin").click(function(){
			...
			var url = "/formmode/exceldesign/tabStyleChoose.jsp?tabstyle="+tabstyle_obj.val();
	...
	function setDefaultTab(defindex){
		...
		tabOperArea.eq(defindex).find("span").before("<img class=\"defaultimg\" src=\"/formmode/exceldesign/image/shortBtn/tab/defaulttab_wev8.png\" />");
	}
...
var mcOperate = (function (){
	...
	function setCellToMC(sheet, r, c){
		var imgsrc = "/formmode/exceldesign/image/shortBtn/morecontent/more_wev8.png";
	...
	function setCellToBr(r){
		...
		var mc_dataobj = getCurrentDataObj();
		var c = 1;
		var imgSrc = "/formmode/exceldesign/image/shortBtn/morecontent/wrap_wev8.png";
	...
	function switchCellBrSign(r){
		...
		var imgSrc;
		if(brsign === "Y"){
			imgSrc = "/formmode/exceldesign/image/shortBtn/morecontent/wrap_wev8.png";
			mc_dataobj.ecs[r+","+c].brsign = "N";
		}else if(brsign === "N"){
			imgSrc = "/formmode/exceldesign/image/shortBtn/morecontent/wrap_hot_wev8.png";
...
var insertOperate = (function(){
	...
	function insertScriptCode(){
    	var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excelUploadCode.jsp";
		dialog.Title = "插入代码";
	...
	function insertLink(){
    	...
    	if(dataobj && dataobj.ecs && dataobj.ecs[ar+","+ac]){
    		var etype = dataobj.ecs[ar+","+ac].etype;
			if(etype==="" || etype===celltype.TEXT || etype===celltype.LINKTEXT){
			}else{
    	...
    	var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excelUploadLink.jsp";
		dialog.Title = "插入链接";
		dialog.Width = 500;
		dialog.Height = 340;
		dialog.Drag = true; 	
	...
	function portalDesign(target){
		...
		var paramStr = "&modeid="+wfinfo.modeid+"&formid="+wfinfo.formid
				+"&isbill="+wfinfo.isbill+"&layouttype="+wfinfo.layouttype;
		paramStr += "&layoutname="+encodeURI(jQuery("#modename").val()+"-"+jQuery("#layoutname").val());
		...
		dialog.callbackfun = function(paramobj, datas){
			...
			var imgsrc = "/formmode/exceldesign/image/controls/portal_wev8.png";
			var celltext = "门户元素";
			if(!!datas.elements)
	...
	function iframeDesign(target){
		...
		var c = sheet.getActiveColumnIndex();
		var cellid = r+","+c;
		var url = "/formmode/exceldesign/setIframe.jsp?firstParam=1";
		...
		dialog.callbackfun = function (paramobj, datas) {
			...
			var imgsrc = "/formmode/exceldesign/image/controls/iframeArea_wev8.png";
	...
	function scanCodeDesign(target){
		...
		var c = sheet.getActiveColumnIndex();
		var cellid = r+","+c;
		var url = "/formmode/exceldesign/setScanCode.jsp?formid="+wfinfo.formid+"&isbill="+wfinfo.isbill;
		if(target == "setting" && dataobj.ecs[cellid].jsonparam){
			var jsonparam = dataobj.ecs[cellid].jsonparam;
			for(var key in jsonparam){
		...
		dialog.callbackfun = function (paramobj, datas) {
			...
			if(datas.codetype){
				...
				var imgsrc = "/formmode/exceldesign/image/controls/scanCode_"+datas.codetype+"_wev8.png";
...
var formulaOperate = (function(){
	...
	function resumeFormulas(formulaJson){
		for(var __key in formulaJson){
			...
			//模板打开时<、>、&被特殊转译，需先恢复
			if(formulaStr.indexOf("&lt;") >-1 || formulaStr.indexOf("&gt;") >-1 || formulaStr.indexOf("&amp;") >-1){
	...
	function saveFormulas(){
		...
		for(var __key in formulas){
			...
			_formulaJson.formulatxt = replaceFormula(formulaStr.toUpperCase());
			...
			if(window.console)	console.dir(_formulaJson);
	...
	function openFormulaWin(){
		...
		var url = "/formmode/exceldesign/excelUploadFormula.jsp?isDetail="+isDetail+"&detailIdenty="+detailIdenty+"&paramtxt="+paramtxt;
	...
	//删除单元格需删除公式
	function deleteCellFormula(cellid){
		var r = parseInt(cellid.split(",")[0]);
		var c = parseInt(cellid.split(",")[1]);
		var sheet = getCurrentSheet();
		var cellletter = getColHeader(sheet, c)+(r+1);
		var __key = globalData.getCacheValue("symbol").toUpperCase()+".FORMULA"+cellletter;
		var _formulaObj = isDetail==="on" ? parentWin_Main.formulaObj : formulaObj;
		_formulaObj.delOneFormula(__key);
	}
	...
	return {
		...
		//删除单元格需删除公式
		deleteCellFormulaFace: function(cellid){
			deleteCellFormula(cellid);
		},
		//公式选择时绑定表格事件
...
var detailOperate = (function(){
	...
	function doInsertDetail(action){
		var imgsrc = "/formmode/exceldesign/image/shortBtn/detail/detailTable_wev8.png";
		var sheet = getCurrentSheet();
		var r = sheet.getActiveRowIndex();
		var c = sheet.getActiveColumnIndex();
	...
	function openDetailWin(edetail){
		...
		var url="/formmode/exceldesign/excelMain.jsp?isDetail=on&d_identy="+edetail+"&d_num="+detailNum+
				"&formid="+$("#formid").val()+"&isbill="+$("#isbill").val()+
				"&layoutid="+$("#layoutid").val()+"&layouttype="+$("#layouttype").val()+"&detaillimit="+detaillimit;
		dlg.URL= url;
		dlg.Title="新版流程模式设计器";
		dlg.show();
	}
   	...
   	function addDetailRowMark(obj){
    	...
    	var canMerge = true;	//如果新插入的行存在合并单元格，则是异常情况，addSpan会导致设计器整体无效
   	...
   	function addDetailCellMark(obj){
		var objname = $(obj).attr("name");
		var imgsrc = "/formmode/exceldesign/image/shortBtn/detail/"+objname+"_wev8.png";
		var sheet = getCurrentSheet();
		var r = sheet.getActiveRowIndex();
		var c = sheet.getActiveColumnIndex();
		restoreCellProperties("", r+","+c);
   	...
   	function clickOpenSeniorSet(){
   		if(jQuery("#openSeniorSet").attr("checked")){
			...
			dialog.Height = 200;
			dialog.Drag = true; 	
			dialog.URL = "/formmode/exceldesign/seniorSetConfirm.jsp?allowInit="+allowInit;
   	...
   	function autoExtendToSenior(mark){
   		...
   		var firstCellStyle_clone = firstCellStyle.clone();
   		firstCellStyle_clone.backgroundImage = null;
   		sheet.setStyle(0, 0, firstCellStyle_clone, $.wijmo.wijspread.SheetArea.viewport);
   		setCellProperties("0,0", "");
   		sheet.setStyle(0, 1, firstCellStyle_clone, $.wijmo.wijspread.SheetArea.viewport);
   		...
   		//合并表头、表尾标示
   		sheet.addSpan(mark.headRow, 0, 1, sheet.getColumnCount());
   		setRowMarkStyle(sheet, mark.headRow, "表头标示");
   		sheet.addSpan(mark.tailRow, 0, 1, sheet.getColumnCount());
   		setRowMarkStyle(sheet, mark.tailRow, "表尾标示");
   		//生成合计行
   		...
   		if(sumfield.length > 0){
   			...
   			for(var i=0; i<sheet.getColumnCount(); i++){
   				...
   				if(headCellStyle == null)	headCellStyle = new $.wijmo.wijspread.Style();
   				sheet.setStyle(sumRow, i, headCellStyle.clone(), $.wijmo.wijspread.SheetArea.viewport);
   				var sumcell = sheet.getCell(sumRow, i, $.wijmo.wijspread.SheetArea.viewport);
   				sumcell.backgroundImage(null);
   				sumcell.textIndent(0);
   				if(i === 0){
   				}else if(i === 1){
...
function setExcelBxValueFun(e, targetid, flag, txt, _fieldtype, _fieldtypedetail){
	...
	if(flag === celltype.FCONTENT){			//处理字段相关(字段属性、字段类型、图片)
		...
		if(wfinfo.layouttype == "0" || wfinfo.layouttype == "3" || wfinfo.layouttype == "4"){	//显示、监控、打印布局
			setfieldattr = "1";
		}else{
			if(isDetail==="on"){
		...
		if(_controltype != "")
			imgsrc = "/formmode/exceldesign/image/controls/"+_controltype+setfieldattr+"_wev8.png";
	}else if(flag === celltype.SUMFCONTENT){
		imgsrc = "/formmode/exceldesign/image/shortBtn/detail/detailSumField_wev8.png";
	}
	//判断是覆盖单元格/扩充成多字段
	var dataobj = getCurrentDataObj();
	var celltext = sheet.getCell(r, c, $.wijmo.wijspread.SheetArea.viewport).text();
...
function getCellFieldImage(cellobj, fieldattr){
	...
	try{
		...
		if(fieldattr == 0){
			bgimage = "/formmode/exceldesign/image/controls/fieldNotShow_wev8.png";
		}else{
			if(efieldid == -4)		//签字意见字段只读
				return "/formmode/exceldesign/image/controls/textarea1_wev8.png";
			if(efieldid == -1 && fieldattr == 2)	//标题字段可编辑则必填
				fieldattr = 3;
			if(wfinfo.layouttype == "0" || wfinfo.layouttype == "3" || wfinfo.layouttype == "4")	//显示、监控、打印图片置灰
				fieldattr = 1;
			if(cellobj.efieldtype===controltype.LINK || cellobj.efieldtype===controltype.POSITION)
				fieldattr = 1;
			if(!!cellobj.financial){
				if(cellobj.financial.indexOf("2-") > -1)
					bgimage = "/formmode/exceldesign/image/controls/finance"+fieldattr+"_wev8.png";
				else if(cellobj.financial == "3")
					bgimage = "/formmode/exceldesign/image/controls/financeUpper_wev8.png";
				else if(cellobj.financial == "4")
					bgimage = "/formmode/exceldesign/image/controls/thousands_wev8.png";
			}else{
				bgimage = "/formmode/exceldesign/image/controls/"+cellobj.efieldtype+fieldattr+"_wev8.png";
			}
		}
	}catch(e){}
	return bgimage;
...
function checkServer(func,param){
	if(isDetail === "on"){
		...
		jQuery.ajax({
			type:"POST",
			url:"/formmode/exceldesign/excelCheckServer.jsp",
			timeout:10000,
...
function initGlobalData(){
	...
	if(mainFields==null){
		jQuery.ajax({
			type:"POST",
			url:"excelAjaxData.jsp?src=getMainFields&modeid="+modeid+"&formid="+formid+"&layoutid="+layoutid,
			success:function(res){
				mainFields=eval('('+res+')');
			}
		});
	...
	if(detailFields==null){
		jQuery.ajax({
			type:"POST",
			url:"excelAjaxData.jsp?src=getDetailFields&modeid="+modeid+"&formid="+formid+"&layoutid="+layoutid,
			success:function(res){
			...
			if(datas.returnValue=="success"){
				...
				if(datas.curfieldid!=null&&datas.tableIndex!=null){
					try{
						...
						hiddenDivObj.append('<input type="hidden" id="datasource'+curfieldid+'" name="datasource'+curfieldid+'" value="" />');
...
function operTableHead(){
	$(".tableHead").children("div").click(function(){
		if($(this).is(".current"))
			return;
		$(".tableHead").children("div").each(function(){
...
function bindTrEvent(){
    ...
    $(".tableBody").find("table tr[name=_fieldtrinexceldesign]").hover(function(event){
		...
		var editimg = $("<div name=editfieldImg style=position:absolute;cursor:pointer;><img width=16 height=16 src='/formmode/exceldesign/image/rightBtn/edit2_wev8.png' ></div>");
		...
		editimg.on("click",function(){
			...
			$.ajax({
				url : "/formmode/exceldesign/excelGetFieldOperat.jsp",
				type : "post",
 ...
 */
function saveLayout(){
//	if(wfinfo.layouttype === "1" || wfinfo.layouttype === "2"){	//qc:334421 
		//将模板中不存在的字段属性全部置为不显示
...
function loadDetailTable(idx){
	...
	var _html=getDetailFieldHtml(1,'',idx-1);
	_tbody.find("tr[class!='thead']").remove();
	_tbody.append(_html);
	addMiddleBorder();
	bindTrEvent();
