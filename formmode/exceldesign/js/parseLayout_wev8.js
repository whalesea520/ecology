...
var detailOperate = (function(){
	...
	function addRowOperDom(groupid, addRowHtmlStr){
		...
		$G("modesnum"+groupid).value = curindex + 1;
	}
	...
	function addRowExecFun(groupid, initDetailFields){
		...
		if(!!initDetailFields){
			try{
				datainputd(initDetailFields,true);	//QC:348386
			}catch(e){}
		}
		//表单设计器，新增行触发公式计算
	...
	function delRowFun(groupid, isfromsap){
		var oTable = jQuery("table#oTable"+groupid);
		var checkObj = oTable.find("input[name='check_mode_"+groupid+"']:checked");
	...
	function copyRowFun(groupid,copyRowjsStr){
		...
		if(checkObj.size()>0){
			checkObj.each(function(){
                ...
                var length = oTable.find("input[name='check_mode_"+groupid+"']").length;
                var k = oTable.find("input[name='check_mode_"+groupid+"']").get(length - 1).value;
                eval(copyRowjsStr);
				var rowIndex = jQuery(this).val();
			});
			try{
	...
	function checkAllFun(groupid){
		...
		var checkObj = oTable.find("input[type='checkbox'][name='check_mode_"+groupid+"']").not(":disabled");
	...
	return {
		...
		copyRowFun: function(groupid,copyRowjsStr){
			return copyRowFun(groupid,copyRowjsStr);
		},
