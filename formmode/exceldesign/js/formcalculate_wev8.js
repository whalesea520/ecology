...
var calOperate = (function(){
	 ...
	 * 高级明细模式行列规则计算实现主体
	 */
	function calSumFun(groupid,dfieldid){
		...
		}catch(e){if(trifieldid=='')trifieldid=dfieldid;}
	...
	function calRowRule_allRow(groupid){
		...
		oTable.find("input[name='check_mode_"+groupid+"']").each(function(){		//循环行
	...
	function calRowRule_valChange(groupid, trifieldid){
		...
		if(!reg.test(trifieldid)&&trifieldid.indexOf('addbutton')==-1)
			return;
		var trifield_id = trifieldid.substring(5, trifieldid.indexOf("_"));
		var rowmark = trifieldid.substring(trifieldid.indexOf("_")+1);
		if(trifieldid.indexOf('addbutton')>-1){
			rowmark=parseInt($G("indexnum"+groupid).value)-1;
		}
		...
		for(var i=0; i<rowcalArr.length; i++){
			...
			if(needTri || assignStr.replace("detailfield_","") === trifield_id||trifieldid.indexOf('addbutton')>-1){
				calRowRule_single(oTable, rowmark, assignStr, evalStr);
	...
	return {
		...
		calSumFun: function(groupid,dfield){
			return calSumFun(groupid,dfield);
		},
