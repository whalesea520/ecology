...
var excelBaseOperat=(function (){
	...
	function openStyleWin(){
		...
		var url = "/formmode/exceldesign/excelSFContainer.jsp?bxsize="+bxsize;
							...
							if(type==1 || type==3){
		                   		...
		                   		if(type == 1){
		                   			if(dataobj.ecs[r+","+c].etype===celltype.FCONTENT){
		                   				var imgsrc = "/formmode/exceldesign/image/controls/"+dataobj.ecs[r+","+c].efieldtype+getFieldAttr(dataobj.ecs[r+","+c].efieldid)+"_wev8.png";//图的名称很重要
		                   				if(isDetail === "on")
		                   					imgsrc = "/formmode/exceldesign/image/controls/"+dataobj.ecs[r+","+c].efieldtype+parentWin_Main.getFieldAttr(dataobj.ecs[r+","+c].efieldid)+"_wev8.png";//图的名称很重要
										...
										sheet.getCell(r,c,sheet.sheetArea).textIndent(3);
		                   				...
		                   				var imgsrc = "/formmode/exceldesign/image/shortBtn/detail/de_btn_wev8.png";
		                   				sheet.getCell(r,c,sheet.sheetArea).backgroundImage(imgsrc);
										sheet.getCell(r,c,sheet.sheetArea).backgroundImageLayout($.wijmo.wijspread.ImageLayout.None);
		                   			}
		                   		}
					...
					if(dataobj.ecs[ar+","+ac]){
						if(!!dataobj.ecs[ar+","+ac].financial){
							if(dataobj.ecs[ar+","+ac].etype === celltype.FCONTENT){
								...
								var imgsrc = "/formmode/exceldesign/image/controls/"+_controltype+_targetstatus+"_wev8.png";//图的名称很重要
				...
				for (var j = 0; j < sel.colCount; j++) {
					...
					if(dataobj.ecs[r+","+c].etype === celltype.FCONTENT || dataobj.ecs[r+","+c].etype === celltype.NADVICE)
					{
						...
						if(efiledid == -1 && filltype == 2) {	//标题字段特殊处理
							...
							var imgsrc = "/formmode/exceldesign/image/controls/"+dataobj.ecs[r+","+c].efieldtype+3+"_wev8.png";
							sheet.getCell(r,c,sheet.sheetArea).backgroundImage(imgsrc);
							sheet.getCell(r,c,sheet.sheetArea).backgroundImageLayout($.wijmo.wijspread.ImageLayout.None);
						}else
						{
							...
							if(dataobj.ecs[r+","+c].financial)
								imgsrc = "/formmode/exceldesign/image/controls/finance"+filltype+"_wev8.png";
							else
								imgsrc = "/formmode/exceldesign/image/controls/"+dataobj.ecs[r+","+c].efieldtype+filltype+"_wev8.png";
							sheet.getCell(r,c,sheet.sheetArea).backgroundImage(imgsrc);
							sheet.getCell(r,c,sheet.sheetArea).backgroundImageLayout($.wijmo.wijspread.ImageLayout.None);
						}
					}
    ...
    return {
        ...
        initHeadSetPanel:function (operatpanel,excelDiv){
            ...
            operatpanel.find("[name=linkpic]").click(function () {
            	...
            	//var imgSrc = "/formmode/exceldesign/css/cobalt/images/wijmo-ui-icons_000000_240x112_wev8.png"; 
            	//浮动图片 start=============
	            /*if (imgSrc !== "") { 
	                var spread = excelDiv.wijspread("spread"); 
	                var sheet = spread.getActiveSheet(); 
