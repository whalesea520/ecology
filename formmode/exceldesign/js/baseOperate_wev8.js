var baseOperate=(function (){
	...
	function initModuleTab(){
		...
		if(layouttype=='0'){
			...
			clickModuleMenu('edit');
		}else if(layouttype=='3'){
			clickModuleMenu('monitor');
		}else{
		...
		leftMenu.click(function(){
			if($(this).is("#import")){
				...
				var url = "/formmode/exceldesign/excelImport.jsp?modeid="+wfinfo.modeid+"&formid="+wfinfo.formid+"&layouttype="+wfinfo.layouttype+"&layoutid="+wfinfo.layoutid
							+"&isform="+wfinfo.isform+"&isdefault="+wfinfo.isdefault;
	...
	function clickModuleMenu(menuname){
		...
		var iframeurl = "/formmode/exceldesign/showModule.jsp?modeid="+wfinfo.modeid+"&formid="+wfinfo.formid+"&isdefault="+wfinfo.isdefault;
		if(menuname==="show"){		//模板
		...
		}else if(menuname==="monitor"){
			iframeObj.attr("src",iframeurl+"&layouttype=3");
		}else if(menuname==="print"){
			iframeObj.attr("src",iframeurl+"&layouttype=4");
		}else{
			iframeObj.attr("src","");
		}
	}
            ...
            if(dataobj.ecs[cellid] != undefined){
            	...
            	if(etype === celltype.DETAIL){	
            		...
            		if(dataobj.ecs[cellid].financial){
						...
						var url = "/formmode/exceldesign/excelSetFinancial.jsp?financial="+dataobj.ecs[cellid].financial;
           		...
           		}else if(etype === celltype.DE_TITLE || etype === celltype.DE_TAIL || etype === celltype.DE_BTN
           			|| etype === celltype.DE_CHECKALL || etype === celltype.DE_CHECKSINGLE || etype === celltype.DE_SERIALNUM 
	...
	function setFinancial_head(){
    	...
    	if(dataobj && dataobj.ecs && dataobj.ecs[ar+","+ac]){
    		if(dataobj.ecs[ar+","+ac].etype === celltype.FNAME){
    			var url = "/formmode/exceldesign/excelSetFinancial.jsp?financial="+dataobj.ecs[ar+","+ac].financial;
    			var fdialog = new window.top.Dialog();
				fdialog.currentWindow = window;
				fdialog.Title = "财务格式设置";
				fdialog.Width = 450;
				...
				fdialog.show();
				fdialog.callbackfun = function (paramobj, result) {
        			sheet.getCell(ar,ac,$.wijmo.wijspread.SheetArea.viewport).backgroundImage("/formmode/exceldesign/image/controls/financeTitle_wev8.png");
    		...
    		if(dataobj.ecs[ar+","+ac].etype === celltype.FCONTENT){
				...
				fdialog.callbackfun = function (paramobj, result) {
	            	var status = getFieldAttr(dataobj.ecs[ar+","+ac].efieldid);
       				var imgsrc = "/formmode/exceldesign/image/controls/finance"+status+"_wev8.png";//图的名称很重要
		...
		for (var n = 0; n < sels.length; n++) {
			...
			for (var i = 0; i < sel.rowCount; i++) {
				for (var j = 0; j < sel.colCount; j++) {
					...
					if(dataobj && dataobj.ecs && dataobj.ecs[ar+","+ac]){
						if(dataobj.ecs[ar+","+ac].etype === celltype.FCONTENT){
							sheet.getCell(ar,ac,$.wijmo.wijspread.SheetArea.viewport).backgroundImage("/formmode/exceldesign/image/controls/thousands_wev8.png");
    						sheet.getCell(ar,ac,$.wijmo.wijspread.SheetArea.viewport).backgroundImageLayout($.wijmo.wijspread.ImageLayout.None);
    	...
    	for (var n = 0; n < sels.length; n++) {
			...
			for (var i = 0; i < sel.rowCount; i++) {
				for (var j = 0; j < sel.colCount; j++) {
					...
					if(dataobj && dataobj.ecs && dataobj.ecs[ar+","+ac]){
						if(dataobj.ecs[ar+","+ac].etype === celltype.FCONTENT){
							sheet.getCell(ar,ac,$.wijmo.wijspread.SheetArea.viewport).backgroundImage("/formmode/exceldesign/image/controls/financeUpper_wev8.png");
    						sheet.getCell(ar,ac,$.wijmo.wijspread.SheetArea.viewport).backgroundImageLayout($.wijmo.wijspread.ImageLayout.None);
								...
								delete dataobj.ecs[r+","+c].enumbric;
		                   		if(dataobj.ecs[r+","+c].etype === celltype.DE_TITLE || dataobj.ecs[r+","+c].etype===celltype.DE_TAIL){
		                   			...
		                   			if(etype === celltype.FNAME && dataobj.ecs[r+","+c].financial)
		                   				imgsrc = "/formmode/exceldesign/image/controls/financeTitle_wev8.png";
		                   			else if(etype === celltype.FCONTENT)
		                   				imgsrc = getCellFieldImage(dataobj.ecs[r+","+c]);
		                   			else if(etype === celltype.DETAIL)
		                   				imgsrc = "/formmode/exceldesign/image/shortBtn/detail/detailTable_wev8.png";
		                   			else if(etype === celltype.TAB)
		                   				imgsrc = "/formmode/exceldesign/image/rightBtn/tabsmall_wev8.png";
		                   			else if(etype === celltype.MC)
		                   				imgsrc = "/formmode/exceldesign/image/shortBtn/morecontent/more_wev8.png";
		                   			else if(etype === celltype.PORTAL)
		                   				imgsrc = "/formmode/exceldesign/image/controls/portal_wev8.png";
		                   			else if(etype === celltype.IFRAME)
		                   				imgsrc = "/formmode/exceldesign/image/controls/iframeArea_wev8.png";
		                   			else if(etype === celltype.SCANCODE){
		                   				if(dataobj.ecs[r+","+c].jsonparam && dataobj.ecs[r+","+c].jsonparam.codetype)
		                   					imgsrc = "/formmode/exceldesign/image/controls/scanCode_"+dataobj.ecs[r+","+c].jsonparam.codetype+"_wev8.png";
		                   			}else if(etype === celltype.DE_BTN || etype === celltype.DE_CHECKALL
		                   				|| etype === celltype.DE_CHECKSINGLE || etype === celltype.DE_SERIALNUM){
		                   				imgsrc = "/formmode/exceldesign/image/shortBtn/detail/"+etype+"_wev8.png";
	...
	function addPic(){
		...
		var url = "/formmode/exceldesign/excelUploadImg.jsp";
		dialog.Title = "插入图片";
		dialog.Width = 500;
		dialog.Height = 340;
		dialog.Drag = true; 	
	...
	function setCustomAttr(){
		var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/setCustomAttr.jsp?isDetail="+isDetail+"&locatetarget="+rightMenuSelectCase;
	...
	function set_Fieldpro(){
		...
		var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excel_fieldattr.jsp?efieldid="+efieldid+"&isDetail="+isDetail;
		dialog.Title = "字段属性";
		dialog.Width = 600;
		dialog.Height = 600;
		dialog.Drag = true;
	...
	function set_Morepro(){
    	...
    	var efieldid = dataobj.ecs[ar+","+ac].efieldid;
   		var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excel_fieldattrMore.jsp?efieldid="+efieldid+"&isDetail="+isDetail;
		dialog.Title = "更多属性";
		dialog.Width = 500;
		dialog.Height = 600;
		dialog.Drag = true; 	
	 ...
	 */
   	...
   	function initStyleTab(){
		...
		$(".s_style").find(".morenewBtn").click(function(){
			...
			var url = "/formmode/exceldesign/excelStyleDesign.jsp";
	...
	function bindStyleRightMenu(obj){
		jQuery(obj).contextMenu({
			...
			if(action === "editstyle"){	//编辑
				var style_dialog = new window.top.Dialog();
				style_dialog.currentWindow = window;
				var url = "/formmode/exceldesign/excelStyleDesign.jsp?styleid="+$(el).attr("target").split("styleid")[1];
	...
	function _setStyleBycjson(cjson){
			...
			if(skey === "main" || skey.indexOf("tab_") >-1){
				...
				for(var cellid in ecs){
					...
					if(ecs[cellid].etype === celltype.FNAME || ecs[cellid].etype === celltype.NNAME 
						|| ecs[cellid].etype === celltype.DETAIL || ecs[cellid].etype === celltype.TAB){
				...
				for(var cellid = 0; cellid<detail_dataobj.ec.length;cellid++){
					...
					if(detail_dataobj.ec[cellid].etype === transformerEtype(celltype.FNAME) 
						...
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.DE_CHECKALL)
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.DE_CHECKSINGLE)
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.TEXT)
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.DE_SERIALNUM)
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.SUMFNAME)
						|| detail_dataobj.ec[cellid].etype === transformerEtype(celltype.SUMFCONTENT)){
   	...
   	function getMenuJson(){
		var menuStr = "{";
			menuStr += "\"children\": [";
		if(rightMenuSelectCase === "nor" && (wfinfo.layouttype == "1" || wfinfo.layouttype == "2")){	//新建、编辑布局
	...
	function openStyleWin(){
		...
		var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excelSFContainer.jsp?bxsize="+bxsize+"&isEditMoreContent="+isEditMoreContent;
        ...
        if(rc === "r"){
    		...
    		for(var i=0;i < sel.colCount; i++){
    			if(iscolpercent){
	        		...
	        		if(i==0) 
	        			defaultvar = parseInt(colheadtxt.substring(1,colheadtxt.lastIndexOf(")")));
	        		else {
   		...
   		var dialog = new window.top.Dialog();
		dialog.currentWindow = window;
		var url = "/formmode/exceldesign/excelSetRChw.jsp?isrc="+rc+"&defaultvar="+defaultvar+"&iscolpercent="+iscolpercent;
		if(rc==="r")
			dialog.Title = "设置行高";
		else
			dialog.Title = "设置列宽";
	...
	function controlOperLimits(row, col){
		...
		if(wfinfo.layouttype == "1" || wfinfo.layouttype == "2"){	//新建、编辑布局
			var _menu_rem = getFieldREM();
			var isfield = (_menu_rem.split(",")[0]==="true")?true:false;
			var istitlefield = (_menu_rem.split(",")[1]==="true")?true:false;
			var isspecial = (_menu_rem.split(",")[2]==="true")?true:false;
		...
		if(wfinfo.layouttype == "0" || wfinfo.layouttype == "3" || wfinfo.layouttype == "4"){
			if(isSingle){
				if(dataobj.ecs[row+","+col] && dataobj.ecs[row+","+col].etype === celltype.FCONTENT){
					fieldPanel.find("[name=fieldpro]").removeClass("shortBtn_disabled").addClass("shortBtn");
				}
	...
	/**
	 * 明细表选中单一单元格时，右下角显示字段设置
	 */
	function controlDiscriptionAreaDetail(){
		var layouttype=wfinfo.layouttype;
		if(layouttype!="0"&&layouttype!="2")return;
        ...
        if(isSingle){
	    	...
	    	if(dataobj && dataobj.ecs && dataobj.ecs[cellid]){
	    		...
	    		if(etype === celltype.FCONTENT){
	    			isfield = true;
	    			fieldid = dataobj.ecs[cellid].efieldid;
	    		}
	    	}
        }
        //显示说明信息
        var obj = $("div[name=sortfielddiv]").find("tr.groupHeadHide").find(".hideBlockDiv");
        if(isfield){
        	var fieldid_old = jQuery("#sortfield").attr("fieldid",fieldid);
        	if(fieldid_old==fieldid){
        		alert("相同");
        		return;
        	}else{
        		jQuery("#issort").removeAttr("disabled").attr("fieldid",fieldid);
        		jQuery("#ordertype").removeAttr("disabled");
        		jQuery("#ordernum").removeAttr("disabled");
            	var issort = $("#issort"+fieldid, parentWin_Main.document).val();
            	var ordertype = $("#ordertype"+fieldid, parentWin_Main.document).val();
            	var ordernum = $("#ordernum"+fieldid, parentWin_Main.document).val();
        		...
        		jQuery("#issort").attr("checked",fieldchecked);
    	    	if(obj.attr("_status") === "1"){
    	    		obj.click();
    	    	}
        	}
        }else{
        	jQuery("#issort").attr("disabled","disabled").attr("checked",false).attr("fieldid","");
        	jQuery("#ordertype").attr("disabled","disabled");
        	jQuery("#ordernum").val("").attr("disabled","disabled");
        	if(obj.attr("_status") === "0"){
        		obj.click();
        	}
        }
