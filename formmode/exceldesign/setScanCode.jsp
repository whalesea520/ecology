...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="二维/条形码设置"/>
</jsp:include>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361, user.getLanguage()) %>'>
		...
		<wea:item>
			...
			<div id="barCodeDiv" class="codediv" style="display:<%=codetype==2 ? "" : "hidden" %>">
				<img class="barcodeimg" src="/createWfBarCode?msg=Weaver-20150808&type=code128&hrp=none" />
			</div>
			<div class="noteinfo">
				实际显示将根据单元格大小自动等比例缩放</br>
				信息量大时请适当调整单元格大小，否则信息密集易导致扫码出现偏差或失效
			</div>
		</wea:item>
		<%-- <wea:item>对应数据</wea:item>
		<wea:item>
		...
		<wea:item attributes="<%=barCode %>">不显示数据文本</wea:item>
...
function getChooseFieldUrl(){
	var url = "/formmode/exceldesign/chooseTextField.jsp?formid=<%=formid %>&isbill=<%=isbill %>&selfieldid="+jQuery("input#relatefield").val();
	return url;
}
