...
var win;
var triggerObj;
Ext.ux.form.Expendattr=Ext.extend(Ext.form.TriggerField,{
	defaultAutoCreate:{
	...
	,triggerClass:"x-form-search-trigger",validateOnBlur:false,multiSelect:false,showOnFocus:false,sFeatures : "dialogHeight:500px;dialogWidth:800px;status:no;center:yes;resizable:yes",url:"blank",initComponent:function(){
		Ext.ux.form.Expendattr.superclass.initComponent.call(this);
		if(this.store){
			if(!this.valueField){
	...
	,onRender:function(b,a){
		...
		Ext.ux.form.Expendattr.superclass.onRender.call(this,b,a);
		if(this.hiddenName){
	...
	,onTriggerClick:function(a){
		...
		mobileModelDlg.Width = 850;//定义长度
		mobileModelDlg.Height = 600;
		mobileModelDlg.URL = this.url;
		mobileModelDlg.Title = SystemEnv.getHtmlNoteName(4025,readCookie("languageidweaver"));
		mobileModelDlg.show();
		mobileModelDlg.onCloseCallbackFn=function(result){
			theStoreRecord.set('expendattr',result);
		};
	}
});
Ext.reg("xExpendattr",Ext.ux.form.Expendattr);