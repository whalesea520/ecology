...
FCKeditor.prototype.ReplaceTextarea = function()
{
	if ( !this.CheckBrowser || this._IsCompatibleBrowser() )
	{
		...
		if(oTextarea.style.width)
			this.Width = oTextarea.style.width;
		if(oTextarea.style.height)
			this.Height = oTextarea.style.height;//自动获取高和宽
...
function FCKeditor_IsCompatibleBrowser( enableSafari, enableOpera )
{
	...
	if ( sAgent.indexOf("msie") != -1 && sAgent.indexOf("mac") == -1 && sAgent.indexOf("opera") == -1 )
	{
	    ...
	    var startnum = version.indexOf("MSIE ");
	    var lastnum = version.substring(startnum).indexOf(";");
	    //alert(version.substring(startnum).substring(5,lastnum));
		var sBrowserVersion = version.substring(startnum).substring(5,lastnum);
	    /**
