...
(function(){
	...
	function _initEditor(formName,name,isEnglish,isNonImage,height,toolbar){
		...
		//编辑器语言		
		var oFCKeditorlanguage = 'zh-cn';
		if(isEnglish=='8'||isEnglish==true){//非中文是读取，否则默认为简体中文
			oFCKeditorlanguage="en";
			this.isEn=true;
		}
		
		//生成编辑器
		var oFCKeditor;// = CKEDITOR.replace(name,{language:oFCKeditorlanguage,height:height});
		...
		if(isNonImage == this.HtmlLayout_IMAGE){
			oFCKeditor = CKEDITOR.replace(name,{language:oFCKeditorlanguage,height:height, customConfig : '/formmode/js/ckeditor/custom/wfEditorConf_wev8.js'});
			//oFCKeditor.editorConfig="/FCKEditor/"+"wfEditorConf_wev8.js";
		}else if(isNonImage == this.MobileLayout_IMAGE){
			//oFCKeditor.Config["CustomConfigurationsPath"]="/FCKEditor/"+"wfMobileConf_wev8.js";
			oFCKeditor = CKEDITOR.replace(name,{language:oFCKeditorlanguage,height:height, customConfig : '/formmode/js/ckeditor/custom/wfEditorConf_wev8.js'});
		}else{
			//oFCKeditor.Config["CustomConfigurationsPath"]="/FCKEditor/"+"weaverEditorConf_wev8.js";
			oFCKeditor = CKEDITOR.replace(name,{language:oFCKeditorlanguage,height:height});
		}
		...
		oFCKeditor.config.enterMode = CKEDITOR.ENTER_BR;		//屏蔽换行符
		oFCKeditor.config.shiftEnterMode = CKEDITOR.ENTER_P;	//屏蔽段落符
		...
		oFCKeditor.on('instanceReady', function (event) {
			...
			if (window.location.href.indexOf("/workflow/") != -1) {
				...
				jQuery("#cke_remark").css("width", "100%");
			}
			
			if (height == "100%" && window.location.href.indexOf("/workflow/html/LayoutEditFrame.jsp") != -1) {
				var h = jQuery(document.getElementById(event.editor.name)).height(); 
				var td = document.getElementById('cke_contents_' + event.editor.name); 
				 //alert(td);
			...
			// add by cjl 2014-09-03 
			if (height == "100%" && window.location.href.indexOf("/formmode/setup/LayoutEdit.jsp") != -1) {
				var h = jQuery(document.getElementById(event.editor.name)).height(); 
				var td = document.getElementById('cke_contents_' + event.editor.name); 
				 //alert(td);
				 //var tbody = td.parentNode.parentNode;
		...
		oFCKeditor.getData=function (){
			var n=this;n.fire('beforeGetData');var l=n._.data;if(typeof l!='string'){var m=n.element;if(m&&n.elementMode==1)l=m.is('textarea')?m.getValue():m.getHtml();else l='';}l={dataValue:l};n.fire('getData',l);
			if(name==="layouttext")
				return _filterXss(l.dataValue);
			else
				return _filterXss(_stripScripts(l.dataValue));
		}	
		//完成初始化
		isInit=true;//标记初始化完成
		displayAllmenu();
	...
	function _setHtml(s,ename){
		...
		if(oEditor!=null && oEditor!=undefined){
			...
			try{
				// "#cke_contents_remark iframe"永远是签字意见的cheditor的ID号。在字段联动时候，给html格式多行文本
				// 的字段设置值时候，会重复设置值。既给该字段值设置了值，又将流程中的签字意见也设置了值。故需要删除。
				var ckeditIframe = "#cke_contents_" + ename + " iframe";
				//jQuery(jQuery(ckeditIframe)[0].contentWindow.document.body).html(s);
				//jQuery(ckeditIframe)[0].contentWindow.document.write(s);
				//不能用jQuery.html赋值，会丢失<Script>
				if (ename === "layouttext" && oEditor.dataProcessor){	//流程模板编辑器丢失js
                   s = oEditor.dataProcessor.toHtml(s);
				}
				jQuery(ckeditIframe)[0].contentWindow.document.body.innerHTML = s;
			}catch(e){}
	...
	*/		
	...
	function _Ok(ename){
		...
		if(isFile){
			jQuery("#"+imgTypeId+"_"+ename).attr("url","");
		}
	...
	function _initUploadImage(formName,isNonFile,ename){
		...
		/*以下是修正当明细行比较多的情况下，签字意见上传图片，点击上传按钮无效的问题。QC77536*/
		window.setInterval(function(){
			var top = jQuery("#imgFile_"+ename).offset().top;
			//console.log(top);
			jQuery("input[name='imgFile_"+ename+"']").closest("div").css({top:top+'px'})
		},1000);
	}
