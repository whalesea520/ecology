...
if(Util.null2String(request.getParameter("comefrom")).equals("1")){
	...
	if(fromBillManagement==1){
		//调整字段后新的读取方式开始
		String strSql = "select showcondition, showconditioncn from mode_triggerworkflowset where id="+linkid;
...
if(haspost.equals("1")){
	...
	if(fromBillManagement==1||fromself==1){
		String savenode = "update mode_triggerworkflowset set showcondition=?,showconditioncn=? where id=?";
		if(isOracle){
		    savenode = "update mode_triggerworkflowset set showcondition=empty_clob(),showconditioncn=empty_clob() where id=?";
		}
		...
		try{
			...
			if(isOracle){
				String sql = "select showcondition,showconditioncn from mode_triggerworkflowset where id = " +linkid;
...
if(isclear.equals("1")){
	...
	if(fromBillManagement==1){
		String sql = "update mode_triggerworkflowset  set showcondition ='', showconditioncn ='' where id="+linkid;
		if(rs.getDBType().equals("oracle")){
            sql = "update mode_triggerworkflowset set showcondition=empty_clob() , showconditioncn=empty_clob() where id="+linkid;
		}
		rs.executeSql(sql);
	}
}
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="ModeTriggerConditionIframe.jsp" method="post">
...
<TR class="title">
	<TH colSpan=6><%=SystemEnv.getHtmlLabelName(15364,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(68,user.getLanguage())%></TH><!-- 条件设置 -->
</TR>
...
<%
char flag=2;
rs.executeProc("workflow_selectitembyid_new",""+id+flag+isbill);
while(rs.next()){
...
<select class=inputstyle  name="con<%=id%>_opt" style="width:80px;" onfocus="changelevel('<%=tmpcount%>')" >
<!-- TD15999 -->
<%if(type.equals("162")||type.equals("257") || type.equals("194")) {%>
...
<td>
<%if(type.equals("161")) {%>
<button type="button" class=Browser  onfocus="changelevel('<%=tmpcount%>')" onclick="onShowBrowserCustomNew('<%=id%>','162','<%=UrlComInfo.getUrlbrowserurl("162")%>?type=<%=dbtype%>')"></button>
<%} else if(type.equals("162")) {%>
<button type="button" class=Browser  onfocus="changelevel('<%=tmpcount%>')" onclick="onShowBrowserCustomNew('<%=id%>','161','<%=UrlComInfo.getUrlbrowserurl("161")%>?type=<%=dbtype%>')"></button>
<%}else if(type.equals("256")||type.equals("257")) {
	...
	String urls=UrlComInfo.getUrlbrowserurl(type)+"?type="+dbtype+"_"+tempType;     // 浏览按钮弹出页面的url
%>
<button type="button" class=Browser  onfocus="changelevel('<%=tmpcount%>')" onclick="onShowBrowserCustomNew('<%=id%>','<%=type%>','<%=urls %>')"></button>
<%} else {
if(type.equals("194")){%>
	<button type="button" class=Browser  onfocus="changelevel('<%=tmpcount%>')" onclick="onShowBrowser('<%=id%>','<%=UrlComInfo.getUrlbrowserurl("164")%>')"></button>
<%}else{%>
...
function onShowBrowser(id,url){
	...
	if(url.indexOf("/MutiResourceBrowser.jsp")!=-1){ 
		datas = window.showModalDialog(url, "", "dialogHeight:600px;dialogwidth:648px");
	}else{
...
function SearchFormReset(){
	var $ = jQuery;
	jQuery("input[name$='_value']").val("");
	jQuery("input[name$='_name']").val("");
	jQuery("span[name$='_valuespan']").html("");
	jQuery("span[class='jNiceCheckbox jNiceChecked']").removeClass("jNiceChecked");
...
function onShowBrowserCustomNew(id,type1, url) {
	url+="&iscustom=1";
