...
RCMenu += "{"+ SystemEnv.getHtmlLabelName(83476,user.getLanguage()) +",javascript:onShare("+parentid+"),_self} " ;
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="rightSearchSpan" style="text-align:right; width:500px!important">
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(83476,user.getLanguage()) %>" class="e8_btn_top" onclick="onShare(<%=parentid %>);"/>
	...
	<table class="viewForm">
      <TR>
	    ...
	    <td  colspan="4" class="btnTd">
			<input type="submit" value="<%=SystemEnv.getHtmlLabelName(82529,user.getLanguage())%>" class="e8_btn_submit"/>	<!-- 搜索 -->
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(32694,user.getLanguage())%>" class="e8_btn_cancel" id="cancel"/>	<!-- 取消 -->
		</td> 
		</tr>
	</table>
</div>	
...
tableString =   " <table  pageId=\""+pageId+"\"  instanceid=\"CptCapitalAssortmentTable\" tabletype=\"checkbox\"  pagesize=\""+PageIdConst.getPageSize(pageId,user.getUID(),"uf4mode_cpt")+"\"  >"+
                ...
                "       <head>"+
                "           <col width=\"25%\"  text=\""+ SystemEnv.getHtmlLabelName(21956,user.getLanguage()) +"\" column=\"sharetype\" orderkey=\"sharetype\" otherpara=\""+"{'languageid':"+user.getLanguage()+"}"+"\" transmethod='weaver.cpt.util.CapitalTransUtil.getShareTypeName'  />"+
                "           <col width=\"25%\"  text=\""+ SystemEnv.getHtmlLabelName(106,user.getLanguage()) +"\" column=\"id\" orderkey=\"id\" otherpara=\""+"{'languageid':"+user.getLanguage()+",'sharetablename':'uf4mode_CptAssortmentShare'}"+"\" transmethod='weaver.cpt.util.CommonTransUtil.getShareObjectName' />"+
                "           <col width=\"25%\"  text=\""+ SystemEnv.getHtmlLabelName(683,user.getLanguage()) +"\" column=\"seclevel\" orderkey=\"seclevel\" otherpara=\"column:id\" transmethod='weaver.cpt.util.CapitalTransUtil.getSeclevel' />"+
                "           <col width=\"25%\"  text=\""+ SystemEnv.getHtmlLabelName(3005,user.getLanguage()) +"\" column=\"sharelevel\" orderkey=\"sharelevel\" otherpara=\""+"{'languageid':"+user.getLanguage()+"}"+"\" transmethod='weaver.cpt.util.CapitalTransUtil.getShareLevelName' />"+
                "       </head>"+
                "     <popedom column=\"id\" ></popedom> "+
                "		<operates>"+
					"		<operate href=\"javascript:onDel();\" text=\""+ SystemEnv.getHtmlLabelName(125426,user.getLanguage()) +"\" target=\"_self\" index=\"1\"/>"+
				"		</operates>"+                  
                " </table>";
%>
...
function onShare(id){
	if(id){
		...
		var title="<%=SystemEnv.getHtmlLabelName(83671,user.getLanguage())%>";
		openDialog(url,title,500,290);
	}
}
function onDel(id,assortmentid){
	if(id){
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(83672,user.getLanguage())%>",function(){
			//提示
			...
			jQuery.post(
				...
				function(data){
					diag_tooltip.close();
					window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(83472,user.getLanguage()) %>",function(){
						_table.reLoad();
					});
				}
			);
...
function batchDel(){
	...
	if(typeids=="") return ;
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(83601,user.getLanguage()) %>",function(){
		//提示
		var diag_tooltip = new window.top.Dialog();
		diag_tooltip.ShowCloseButton=false;
		diag_tooltip.ShowMessageRow=false;
