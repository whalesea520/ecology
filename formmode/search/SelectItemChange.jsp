...
function insertSelect_All(){
	try{
		...
		if(childSelObj.length>0&&childSelObj.val()==""){
			var defaultValues = childSelObj.attr("defaultValues");
			if(defaultValues!=""){
				var arr = defaultValues.split(",");
		...
		<%if(isinit==1){%>
			...
			<%}else{%>
					if(typeof(parent.changeChildSelectItemDetailField)=="function"){
						parent.changeChildSelectItemDetailField(0,childfieldid,childsel,<%=rowindex%>,1);
					}
		<%		}   
		  }%>
