...
<table id="topTitle" cellpadding="0" cellspacing="0">
	...
	<td class="rightSearchSpan" style="text-align:right;">
		...
		%>
		   <span title="<%=quickname %>" style="font-size: 12px;cursor: pointer;">
				<input name="zh" class="e8_btn_top middle" onclick="<%=quickmethod %>" type="button" value="<%=quickname %>"/>
			</span>
		<%} %>
		...
		%>
			<span title="<%=m.get("name") %>" style="font-size: 12px;cursor: pointer;">
				<input name="zh" class="e8_btn_top middle" onclick="<%=m.get("href") %>" type="button" value="<%=m.get("name") %>"/>
			</span>
		<%} %>
		<%
			...
			while(rs.next()){
				...
				%>
				<span title="<%=expendName %>" style="font-size: 12px;cursor: pointer;">
					<input name="zh" type="button" value="<%=expendName%>" id="zd_btn_save" class="e8_btn_top" onclick="<%=executionMethod%>">
				</span>
				<%
				}
			}%>
...
<input type=hidden name ="customTreeDataId" value="<%=customTreeDataId %>"><!--树节点-->
...
function toEditCallBack(){
	var url="/formmode/view/AddFormMode.jsp?isfromTab=0&modeId=<%=modeId%>&formId=<%=formId%>&type=2&billid=<%=billid%>&viewfrom=<%=viewfrom%>&opentype=<%=opentype%>&customid=<%=customid%>&isRefreshTree=<%=isRefreshTree%>&mainid=<%=mainid%>&istabinline=<%=istabinline%>&tabcount=<%=tabcount%>&tabid=<%=tabid%>&customTreeDataId=<%=customTreeDataId%>";
	var istabinline = $G("istabinline").value;
	var tabcount = $G("tabcount").value;
	if(istabinline==1){
		if(tabcount==0){
			url="/formmode/view/AddFormModeIframe.jsp?isfromTab=0&modeId=<%=modeId%>&formId=<%=formId%>&type=2&billid=<%=billid%>&viewfrom=<%=viewfrom%>&opentype=<%=opentype%>&customid=<%=customid%>&isRefreshTree=<%=isRefreshTree%>&mainid=<%=mainid%>&istabinline=<%=istabinline%>&tabcount=<%=tabcount%>&customTreeDataId=<%=customTreeDataId%>";
...
function changeshowattr(fieldid,fieldvalue,rownum,workflowid,nodeid){
    ...
    jQuery.ajax({
		...
		data: "modeId=<%=modeId%>&type=<%=ResolveFormMode.getLayoutid()%>&fieldid="+fieldid+"&fieldvalue="+fieldvalue,
		success: function(data){
            try{
...
function datainput(parfield){                <!--数据导入-->
    ...
    var StrData="modeId=<%=modeId%>&formId=<%=formId%>&type=<%=type%>&layoutid=<%=layoutid%>&detailsum=<%=detailsum%>&trg="+encodeURIComponent(parfield);
    for(var i=0;i<tempParfieldArr.length;i++){
	    ...
	    %>
	       if($GetEle("<%=temp.substring(temp.indexOf("|")+1)%>")) StrData+="&<%=temp%>="+encodeURIComponent($GetEle("<%=temp.substring(temp.indexOf("|")+1)%>").value);
	    <%
	        }
	    }
	    %>
	...
	for(var i=0;i<tempParfieldArr.length;i++){
		...
		if(!trrigerdetailfield.trim().equals("")){
			...
			for(int i=0;i<Linfieldname.size();i++){
				String temp=(String)Linfieldname.get(i);
				%>
				if($G("<%=temp.substring(temp.indexOf("|")+1)%>_"+indexid)!=null) StrData+="&<%=temp%>="+encodeURIComponent($G("<%=temp.substring(temp.indexOf("|")+1)%>_"+indexid).value);
				<%
			}
		}%>
	}
