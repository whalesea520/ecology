...
if(modeId>0&&billid>0&&isfromTab!=1){
	...
	sql = "select id from mode_pageexpand where modeid = "+modeId+" and isshow = 1 and showtype = 1 and isbatch in(0,2) and (tabshowtype=0 or tabshowtype is null) order by showorder asc";
	rs.executeSql(sql);
	while(rs.next()){
...
switch(type){
	...
	case 3:
		...
		break;
	case 4:
		status = SystemEnv.getHtmlLabelName(257,user.getLanguage());// 打印 
		break;
}
if (!status.equals("")) {
...
int layoutid = ResolveFormMode.getLayoutid();
...
function doSubmit(detailid,issystemflag){
	...
	var modenum = checkModeNumFun();
   	if(modenum>0) {
   		alert("<%=SystemEnv.getHtmlLabelName(24827,user.getLanguage())%>"+modenum+"<%=SystemEnv.getHtmlLabelName(24828,user.getLanguage())%>!");//必须填写第 xxx个明细表数据，请填写
		displayAllmenu();
   		return false;
   	}
