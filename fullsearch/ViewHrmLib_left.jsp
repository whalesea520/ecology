...
<BODY onload="initTree()">
    <FORM NAME=SearchForm STYLE="margin-bottom:0" action="ViewHrmLibTab.jsp" method=post target="contentframe">
...
function initTree(){
    ...
    */
    cxtree_id = '<%=zxnodeid%>';
    CXLoadTreeItem("", "/fullsearch/FullSearchHrmXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
    var tree = new WebFXTree();
    tree.add(cxtree_obj);
    //document.write(tree);
    document.getElementById('deeptree').innerHTML = tree;
...
function setCompany(nodeid){
    ...
    document.SearchForm.action="/fullsearch/ViewHrmLibTab.jsp?_fromURL=HrmCompanyDsp&id="+comid;
...
function setSubcompany(nodeid){ 
    ...
    document.SearchForm.action="/fullsearch/ViewHrmLibTab.jsp?_fromURL=HrmSubCompanyDsp&subCompanyId="+subid;
    document.SearchForm.submit();
}
function setDepartment(nodeid){
    ...
    document.SearchForm.action="/fullsearch/ViewHrmLibTab.jsp?_fromURL=HrmDepartmentDsp&departmentid="+deptid;
    document.SearchForm.submit();
}
