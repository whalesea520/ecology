...
<%
   ...
   for(String item:ruleInfoArray)
      baseBean.writeLog("item====>"+item);
   	//baseBean.writeLog("eid:"+eid+"ebaseid:"+ebaseid+"sbaseid:"+sbaseid);
%>
<html>
    <head>
	...
	var descisshow = false;
    	...
    	function getExpItemXmlString(expItem) {
			...
			if (!!datatype && datatype.indexOf(weaverSplit) != -1) {
		        ...
		        if (valuetype == "6") {
		        	filterid = datatypearray[3];
		        	filterformid = datatypearray[9];
		        	filterisbill = datatypearray[10];
		        }
		    }
	        ...
	        if (!!datavaluejson) {
	        	...
	        	result += "valueName=\"" + $.trim(datavaluejson.names) + "\" ";
	        } else {
        	...
        	if (valuetype == "6") {
        			...
        			result += "filterformid=\"" + filterformid + "\" ";
        			result += "filterisbill=\"" + filterisbill + "\" ";
		    }
	        result += ">";
	        result += "</expression>";
		...
		function setPublicBrow(browserid,ptype,psolo,sysid)
		{
			...
			if(sysid==='1' && (ispagedeal==='1' || <%=HPTypeEnum.HP_WORKFLOW_FORM.getName().equals(pagetype)%>)){
				...
				jQuery("#valuetype").append("<option value=\"6\"><%=SystemEnv.getHtmlLabelName(84323,user.getLanguage()) %></option>");
			...
			if(ptype === "1" || ptype === "2")
			{
				...
				if (<%=HPTypeEnum.HP_WORKFLOW_FORM.getName().equals(pagetype)%>) {
					$("#valuetype").append("<option value=\"6\"><%=SystemEnv.getHtmlLabelName(84323,user.getLanguage()) %></option>");
				}
				jQuery("#valuetype").selectbox();
				//字符串
				$("#paramvalue").show();
		...
		function getEditBrowserJson(event,datas,fieldid,params,_target,e8os)
		{
			if(datas.id)
			{
				var weaverSplit = "||~WEAVERSPLIT~||";
				var pfs = datas.pfiled.split(weaverSplit);
				var editBlock = $(e8os).parents(".editBlockClass");
    ...
    <body>
		...
		<input type="hidden" id="wfformidfilter" value="">
		<input type="hidden" id="ispagedeal" name="ispagedeal" value="<%=ispagedeal %>">
		...
		<div id="header" style="display:block;">
			<div class="headblock" style="display:block;width:100%;background:#f0f2f5;padding-top:5px;padding-bottom:5px;">
        		...
        		<%
        			...
        			String furl = "/systeminfo/BrowserMain.jsp?url=/synergy/browser/SynergyParamBrowserContent.jsp?ebaseid="+ebaseid+"&sbaseid="+sbaseid+"&stype="+pagetype+"&spagetype="+spagetype+"&saddpage="+saddpage+"&wfid="+wfid+"&pagetype="+pagetype+"&ispagedeal="+ispagedeal; %>
        			<div style="height:26px;margin-top:2px;float:left;width:680px;padding-left:10px" >
	        			...
	        			<!-- 是否为自定义变量 -->
	        			<select name="compareoption" id="compareoption" style="width:80px;margin-left:5px;">
                        ...
                        <%
				  		...
				  		//baseBean.writeLog("==========" + browserComInfo.get);
	                    ...
	                    <select id="valuetype" name="valuetype" style="width:80px;margin-left:5px;">
							...
							<% if(ispagedeal.equals("1")) { %>
			                <option value="6"><%=SystemEnv.getHtmlLabelName(84323,user.getLanguage()) %></option>
							<% } %>
	                    </select>
	                    ...
	                    <span id="browserSpan" name="browserSpan" style="display:none;float:right;">
	                    	<brow:browser name="selectids" viewType="0" hasBrowser="true" hasAdd="false" 
	                  			isMustInput="1" isSingle="true" hasInput="true"
	                  			browserOnClick="onShowBrowser('selectids','','','17', 0, document.getElementById('browsertype'), document.getElementById('valuetype'),document.getElementById('sbaseid').value,document.getElementById('ebaseid').value)"
        		...
        		<span style="margin-right:5px;float:left;">
		  		...
		  		%>
                   ...
                   <brow:browser name="eselectids" viewType="0" hasBrowser="true" hasAdd="false" 
                 			isMustInput="1" isSingle="true" hasInput="true"
                 			browserOnClick="onShowBrowser('eselectids','','','17', 0, document.getElementById('ebrowsertype'), document.getElementById('evaluetype'),document.getElementById('sbaseid').value,document.getElementById('ebaseid').value)"
                 			completeUrl="javascript:getajaxurl();" 
