CREATE OR REPLACE PROCEDURE Doc_SecCategory_Insert_New
 ...
 begin
    if  level_1=1 then
      dirtype := 0;
      insert into DocMainCategory(categoryname,categoryiconid,categoryorder,coder,norepeatedname,subcompanyid) values( categoryname_2,0, secorder,0, noRepeatedName, subcompanyId);
      select MAX(id) into dirid from DocMainCategory  where categoryname= categoryname_2;
    else
      if  level_1=2  then
         dirtype := 1;
        insert into DocSubCategory(categoryname,maincategoryid,subcategoryid,suborder,norepeatedname) values( categoryname_2,(select dirid from DocSecCategory where id= parentid),-1, secorder, noRepeatedName);
          select MAX(id) into dirid from DocSubCategory where categoryname= categoryname_2;
      end if;
    end if;
