...
INSERT INTO LeftMenuInfo (id,labelId,iconUrl,linkAddress,menuLevel,parentId,defaultIndex,relatedModuleId) VALUES (26,16401,'/images_face/ecologyFace_1/LeftMenuIcon/CRM_3.gif','/CRM/search/CRMCate/ry.jsp',2,3,3,4)
/
...
AS
 ...
 begin
  	FOR leftMenuInfo_cursor in(SELECT id, defaultIndex FROM LeftMenuInfo)
		...
		/*用户*/
	FOR hrmResource_cursor in(SELECT id FROM HrmResource order by id)
		loop
			userId :=hrmResource_cursor.id;
			FOR leftMenuInfo_cursor_1 in( 
			SELECT id, defaultIndex FROM LeftMenuInfo)
				loop
					id_1 :=leftMenuInfo_cursor_1.id;
					defaultIndex_1 :=leftMenuInfo_cursor_1.defaultIndex;
