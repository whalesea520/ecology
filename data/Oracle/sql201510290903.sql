...
BEGIN
  ...
  LOOP
    ...
    tablename := 'modeDataShare_' || tableIndex ||'_set';
      ...
      select count(*) into existsisdefault from user_tab_columns where upper(table_name)=upper('modedatashare_'||tableIndex||'_set' ) and upper(COLUMN_NAME) = upper('orgrelation');
      if existsisdefault <= 0 then
        sqltext := 'alter table modeDataShare_'||tableIndex||'_set add orgrelation int';
