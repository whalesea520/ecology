...
/
CREATE SEQUENCE ldapsetsubparam_seq
 START WITH     1
 INCREMENT BY   1
 NOCACHE
 NOMAXVALUE
/
create or replace trigger ldapsetsubparam_Tri
  before insert on ldapsetsubparam
  for each row
begin
  select ldapsetsubparam_seq.nextval into :new.id from dual;
end;
/
...
/
CREATE SEQUENCE ldapsetdepparam_seq
 START WITH     1
 INCREMENT BY   1
 NOCACHE
 NOMAXVALUE
/
create or replace trigger ldapsetdepparam_Tri
  before insert on ldapsetdepparam
  for each row
begin
  select ldapsetdepparam_seq.nextval into :new.id from dual;
end;
/
...
/
CREATE SEQUENCE ldapsetoutype_seq
 START WITH     1
 INCREMENT BY   1
 NOCACHE
 NOMAXVALUE
/
create or replace trigger ldapsetoutype_Tri
  before insert on ldapsetoutype
  for each row
begin
  select ldapsetoutype_seq.nextval into :new.id from dual;
end;
/
...
tinyintfield1 int NULL ,
tinyintfield2 int NULL ,
tinyintfield3 int NULL ,
tinyintfield4 int NULL ,
tinyintfield5 int NULL ,
...
usbstate int NULL ,
totalSpace float(53)  DEFAULT 100 ,
occupySpace float(53)  DEFAULT 0 ,
ecology_pinyin_search varchar(300) NULL ,
isADAccount char(1) NULL 
)
/
CREATE SEQUENCE HrmResourceTemp_seq
 START WITH     1
 INCREMENT BY   1
 NOCACHE
 NOMAXVALUE
/
create or replace trigger HrmResourceTemp_Tri
  before insert on HrmResourceTemp
  for each row
begin
  select HrmResourceTemp_seq.nextval into :new.id from dual;
end;
/