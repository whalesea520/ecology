...
isAccordToSubCom char (1)  DEFAULT ('0'),
childitemid varchar (2000) NULL,
cancel varchar (1)   DEFAULT ((0))
)
/
create sequence hrm_SelectItem_id
minvalue 1
maxvalue 999999999999999999999999999
start with 1
increment by 1
cache 20
/
CREATE OR REPLACE TRIGGER hrm_SelectItem_Trigger before insert on hrm_SelectItem for each row begin select hrm_SelectItem_id.nextval into :new.id from dual; end;
/


CREATE OR REPLACE PROCEDURE hrm_selectitembyid_new(id_1      VARCHAR2,
                                                        isbill_1  VARCHAR2,
                                                        flag      OUT INTEGER,
                                                        msg       OUT VARCHAR2,
...
CREATE OR REPLACE PROCEDURE cus_selectitembyid_new(id_1      VARCHAR2,
                                                        isbill_1  VARCHAR2,
                                                        flag      OUT INTEGER,
                                                        msg       OUT VARCHAR2,
                                                        thecursor IN OUT cursor_define.weavercursor) AS
