CREATE OR REPLACE PROCEDURE workplan_insertplus(type_n_1                      INTEGER,
                                                name_1                        VARCHAR2,
...
BEGIN
  ...
  SELECT MAX(ID) INTO m_id_1 FROM workplan where createrid = createrid_1;
  IF 1 <> createrid_1 THEN
    SELECT departmentid
      INTO m_deptid_1
      FROM hrmresource
  ...
  END IF;
  UPDATE workplan
     SET deptid = m_deptid_1, subcompanyid = m_subcoid_1
   WHERE ID = m_id_1;
  OPEN thecursor FOR
