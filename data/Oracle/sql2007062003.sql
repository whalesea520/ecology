...
if departmentid_1 is not null and olddepartmentid_1 is not null and ( departmentid_1 <>olddepartmentid_1 or  seclevel_1 <> oldseclevel_1 or oldseclevel_1 is null )  then
 ...
 or (t2.rolelevel=1 and t1.subcompanyid1=subcompanyid_1 )))
   ...
   for sharewp_cursor1 in(
    SELECT DISTINCT workPlanId, shareLevel FROM WorkPlanShare WHERE ((forAll = 1 AND securityLevel <= seclevel_1) OR (userId = resourceid_1) OR (deptId like  concat( concat('%,' , to_char(departmentid_1)) , ',%' ) AND securityLevel <= seclevel_1)))
    loop
