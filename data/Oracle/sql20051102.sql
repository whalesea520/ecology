...
if departmentid_1 is not null and olddepartmentid_1 is not null and ( departmentid_1 <>olddepartmentid_1 or  seclevel_1 <> oldseclevel_1 or oldseclevel_1 is null )  then
...
自已并不会默认对自己的文档具有完全控制权限,也不会默认对自已的下级具有查看权限*/
	...
	select id from docdetail where doccreaterid!= ownerid  and ownerid=resourceid_1)
		loop
			docid_2 := docid_cursor.id;  
			insert into temptablevalue values(docid_2, 3);
		end loop;
	...
	select distinct docid,sharelevel from docshare where sharetype=80 and userid=resourceid_1)
		loop
			docid_2 := docid_cursor.docid;
			sharelevel_2 := docid_cursor.sharelevel;
			insert into temptablevalue values(docid_2, sharelevel_2);
		end loop;
	  
	/*对shareType为81的 表示对自己的直接下级的文档*/
	for docid_cursor in(
	select  distinct docid,sharelevel from docshare  where sharetype=81 and userid in ( select id from HrmResource_Trigger where managerId_2 =resourceid_1))
		loop
			docid_2 := docid_cursor.docid;
			sharelevel_2 := docid_cursor.sharelevel;
			insert into temptablevalue values(docid_2, sharelevel_2);
	...
	for docid_cursor in(
	select  distinct docid,sharelevel from docshare  where sharetype=82 and userid in ( select id from HrmResource_Trigger where concat(',',managerstr) like  concat(concat('%,',chr(resourceid_1)),',%')))
		loop
			docid_2 := docid_cursor.docid;
			sharelevel_2 := docid_cursor.sharelevel;
			insert into temptablevalue values(docid_2, sharelevel_2);
	...
	select concat(managerstr,'0') into managerId_2s_2 from  HrmResource_Trigger where id =resourceid_1;
    sepindex_2 := INSTR(managerId_2s_2,',');
	while sepindex_2 != 0 
	loop     
     managerId_2 := to_number(SUBSTR(managerId_2s_2,1,(sepindex_2-1)));
	 managerId_2s_2 := SUBSTR(managerId_2s_2,(sepindex_2+1),(Length(managerId_2s_2)-sepindex_2));
	 sepindex_2 := INSTR(managerId_2s_2,',');
		for docid_cursor in(
		select  distinct docid,sharelevel from docshare  where sharetype=83 and userid =managerId_2 and  
		seclevel<=seclevel_1)
		loop
			docid_2 := docid_cursor.docid;
			sharelevel_2 := docid_cursor.sharelevel;
	...
	for docid_cursor in(
		select  distinct d1.docid,d1.sharelevel from docshare d1,HrmResource_Trigger h1   where d1.userid=h1.id and 
	...
	select id from HrmResource_Trigger where managerId_2=resourceid_1)
	loop   
		tempDownOwnerId_2 := downOwner_cursor.id;
		for docid_cursor in(
	...
	loop 
		docid_2 := sharedocid_cursor.docid;
		sharelevel_2 := sharedocid_cursor.sharelevel;
		select count(docid) into countrec_2 from temptablevalue where docid = docid_2;
		if countrec_2 = 0
			then
			insert into temptablevalue values(docid_2, sharelevel_2);
		elsif sharelevel_2 = 2
			then
			update temptablevalue set sharelevel = 2 where docid=docid_2; /* 共享是可以编辑, 则都修改原有记录 */   
		end if;
	end loop; 
	...
	loop 
		docid_2 :=alldocid_cursor.docid; 
		sharelevel_2:= alldocid_cursor.sharelevel;
		insert into docsharedetail values(docid_2, resourceid_1,1,sharelevel_2);
	end loop;
...
if ( countinsert >0 and ( countdelete = 0 or rolelevel_1  > oldrolelevel_1 ) )  then   
	...
	/* ------- DOC 部分 ------- */
        ...
        for sharedocid_cursor IN (select distinct docid , sharelevel from DocShare where roleid = roleid_1 and rolelevel <= rolelevel_1 and seclevel <= seclevel_1 and sharetype not in (80,81,82,83,84,85,-80,-81,-82) )
	...
	/* ------- DOC 部分 ------- */
		...
		t2.seclevel <= seclevel_1 and t1.docdepartmentid=t4.id and t4.subcompanyid1= subcompanyid_1 and t2.sharetype not in (80,81,82,83,84,85,-80,-81,-82) )
    ...
    if rolelevel_1 = '0'     then     /* 为新建时候设定级别为部门级 */
		...
		seclevel_1 and t1.docdepartmentid= departmentid_1 and t2.sharetype not in (80,81,82,83,84,85,-80,-81,-82))
