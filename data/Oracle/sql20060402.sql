...
/
create sequence shareinnerdoc_id
start with 1
increment by 1
nomaxvalue
nocycle
/
create or replace trigger shareinnerdoc_Tri
before insert on shareinnerdoc
for each row
begin
...
ALTER TABLE shareinnerdoc  ADD 
	CONSTRAINT PK_shareinnerdoc PRIMARY KEY   
	(
		id 
	)   
/
...
/
create sequence ShareouterDoc_id
start with 1
increment by 1
nomaxvalue
nocycle
/
create or replace trigger ShareouterDoc_Tri
before insert on ShareouterDoc
for each row
begin
select Bill_HrmResourceAbsense_id.nextval into :new.id from dual;
end;
...
ALTER TABLE ShareouterDoc  ADD 
	CONSTRAINT PK_ShareouterDoc PRIMARY KEY   
	(
		id 
	)   
/

ALTER TABLE DocSecCategoryShare add  crmid integer
/

DROP PROCEDURE  DocShareDetail_SetByDoc
/
DROP PROCEDURE  Doc_setDocShareByHrm
/
DROP PROCEDURE  Doc_setDocShareBYHrmAndDoc
/
DROP PROCEDURE  DocShareDetail_SByResourceId
/
DROP PROCEDURE  DocShareDetail_SelectByDocId
/
...
begin
    i_1 := 1;
    pagesize_1 := 500;
    select  max(id) into docMax_1 from docdetail;
    pagenum_1 := docMax_1/pagesize_1;
	if pagenum_1=0 then
		pagenum_1 := 1;
	end if;

    WHILE i_1 <= pagenum_1+1
    loop  
    for initdocid_cursor in(
		select id from docdetail where id>=(i_1-1)*pagesize_1 and id<i_1*pagesize_1)
		loop
		docid_1 := initdocid_cursor.id;
        Share_forDoc_init(docid_1);
		end loop;
         i_1:=i_1+1;
    end loop;    
end;
...
/*在此建索引初始化数据库的时间将会快些*/
/*建索引(type+content+seclevel)*/
create  index shareIndexDoc on shareinnerdoc (type, content,seclevel)
/
create  index dshareIndexDoc_sourceid on shareinnerdoc (sourceid desc)
/
/*建索引(type+content+seclevel)*/
create  index shareoutIndexDoc on ShareouterDoc (type, content,seclevel)
/
...
values (secid_1,sharetype_2,seclevel_3,rolelevel_4,sharelevel_5,userid_6,subcompanyid_7,departmentid_8,roleid_9,foralluser_10,crmid_11);
end;
/
...
AS
	...
	hrmid_2 integer;
	sharelevel_3 integer;
	count_4 integer;
begin
	delete shareinnerdoc where sourceid = docid_1;
	select  doccreaterid into count_4 from docdetail where id =  docid_1;
	if count_4 >0 then
