...
if ( departmentid_1 <>olddepartmentid_1 or  seclevel_1 <> oldseclevel_1 or oldseclevel_1 is null )  then
    ...
    for subdocid_cursor IN ((SELECT DISTINCT ID   FROM DOCDETAIL  WHERE (DOCCREATERID IN (SELECT DISTINCT ID   FROM HRMRESOURCE_TRIGGER  WHERE CONCAT(',',MANAGERSTR) LIKE  managerstr_11 )) AND USERTYPE = '1')
union
(SELECT DISTINCT ID   FROM DOCDETAIL  WHERE (OWNERID IN (SELECT DISTINCT ID   FROM HRMRESOURCE_TRIGGER  WHERE CONCAT(',',MANAGERSTR) LIKE  managerstr_11 )) AND USERTYPE = '1'))
