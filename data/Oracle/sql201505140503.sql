...
/
alter table Bill_BoHaiLeave add newLeaveType number
/
update Bill_BoHaiLeave set newLeaveType = (case when leaveType between 1 and 3 then (select id from hrm_leave_type where field004 = leaveType and field005 = 'leaveType') else (select id from hrm_leave_type where field004 = otherLeaveType and field005 = 'otherLeaveType') end) where ((leaveType between 1 and 4) or (otherLeaveType is not null))
/