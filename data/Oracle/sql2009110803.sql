...
begin
 open thecursor for
SELECT * from CRM_ShareInfo where ( relateditemid = relateditemid1 and (isdefault is null or (isdefault=1 and sharelevel=3 and sharetype=1))) order by sharetype ; 
end;
/
...
if ( flag_1 = 1 and managerstr_1 <> oldmanagerstr_1 and length(managerstr_1) > 1)  then /* 新建人力资源时候对经理字段的改变不考虑 */
    ...
    update shareinnerdoc set content = managerId_2 where srcfrom = 81 and opuser = resourceid_1;
