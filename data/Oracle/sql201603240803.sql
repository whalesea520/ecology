  ...
  if usertype_1 = 0 then
    select count(sourceid)
     ...
     (type = 6 and includesub=1 and content in (select * from TABLE(CAST(SplitStr(subcompanyidsub_1, ',') AS mytable))) and
           seclevel <= seclevel_1 and seclevelmax >=seclevel_1) or (type=10 and ( (joblevel=1 and content= jobids_1 ) or (joblevel=2 and content=jobids_1 and jobsubcompany in(select * from TABLE(CAST(SplitStr(subcompanyid_1, ',')AS mytable))))
