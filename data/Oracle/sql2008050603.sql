...
create sequence Workflow_FormSignRemark_id
start with 1
increment by 1
nomaxvalue
nocycle
...
create or replace trigger Workflow_FormSignRemark_Tri
before insert on Workflow_FormSignRemark
for each row
begin
select Workflow_FormSignRemark_id.nextval into :new.id from dual;
end;
/

insert into Workflow_FormSignRemark(requestLogId,imageFileId,remark)
select requestLogId,imageFileId,TO_LOB(remark)
