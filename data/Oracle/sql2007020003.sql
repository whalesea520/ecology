/* Formatted on 2007/04/28 16:02 (Formatter Plus v4.8.8) */
...
BEGIN
   SELECT (workplanid)
     INTO m_workplanid
     FROM workplanexchange
    WHERE workplanid = workplanid_1;
EXCEPTION
   WHEN NO_DATA_FOUND
   THEN
      workplanexchange_wp_add (workplanid_1, flag, msg, thecursor);
   WHEN TOO_MANY_ROWS
   THEN
      UPDATE workplanexchange
         SET exchangecount = exchangecount + 1
       WHERE workplanid = workplanid_1
         AND (   (memberid <> userid_1 AND membertype = usertype_1)
              OR membertype <> usertype_1
             );
END;
/
...
CREATE OR REPLACE PROCEDURE WorkPlanExchange_Insert (
   workplanid_1   IN       INTEGER,
   memberid_1     IN       INTEGER,
   membertype_1   IN       CHAR,
   flag           OUT      INTEGER,
   msg            OUT      VARCHAR2,
   thecursor      IN OUT   cursor_define.weavercursor
)
...
BEGIN
   INSERT INTO workplanexchange
               (workplanid, memberid, membertype
               )
        VALUES (workplanid_1, memberid_1, membertype_1
               );
END;
/
