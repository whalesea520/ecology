...
signdocids1 varchar2,signworkflowids1 varchar2,clientType1 char,
...
begin 
   ...
   if logtype1 = '1' then
               ...
               if count1 > 0 then
                      update workflow_requestlog SET operatedate= operatedate1, operatetime= operatetime1, remark= remark_1, clientip= clientip1, destnodeid= destnodeid1,annexdocids=annexdocids1,requestLogId=requestLogId1,signdocids=signdocids1,signworkflowids=signworkflowids1,isMobile=clientType1 WHERE ( requestid=requestid1 and nodeid=nodeid1 and logtype=logtype1 and operator = operator1 and operatortype = operatortype1);

               else
                      insert into workflow_requestlog (requestid,workflowid,nodeid,logtype, operatedate,operatetime,operator, remark,clientip,operatortype,destnodeid,receivedPersons,agentorbyagentid,agenttype,showorder,annexdocids,requestLogId,operatorDept,signdocids,signworkflowids,isMobile) values(requestid1,workflowid1,nodeid1,logtype1, operatedate1,operatetime1,operator1, remark_1,clientip1,operatortype1,destnodeid1,operate1,agentorbyagentid1,agenttype1,showorder1,annexdocids1,requestLogId1,operatorDept1,signdocids1,signworkflowids1,clientType1);
               end if;
   else
...
begin 
    ...
    agenttype1, showorder1, annexdocids1, requestLogId1, signdocids1, signworkflowids1, '0', flag, msg, thecursor);
...
select to_char(sysdate,'hh24:mi:ss') into currenttime from dual;
   ...
   if logtype1 = '1' then
               ...
               if count1 > 0 then
                      update workflow_requestlog SET operatedate= currentdate, operatetime= currenttime, remark= remark_1, clientip= clientip1, destnodeid= destnodeid1,annexdocids=annexdocids1,requestLogId=requestLogId1,signdocids=signdocids1,signworkflowids=signworkflowids1,isMobile=clientType1 WHERE ( requestid=requestid1 and nodeid=nodeid1 and logtype=logtype1 and operator = operator1 and operatortype = operatortype1);
