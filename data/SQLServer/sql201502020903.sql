	    ...
	    IF @@fetch_status = -2
	      CONTINUE
	      	...
	      	begin
				...
				select @existsisdefault=count(*) from sysobjects o,syscolumns c where o.id=c.id and upper(o.name)=upper('modedatashare_'+@TableIndex+'_set') and upper(c.name) = upper('higherlevel')
				if @existsisdefault = 0
	      		begin
	      			set @sql='alter table modeDataShare_'+@TableIndex+'_set add higherlevel int'
					exec (@sql)
					print @sql
...
go
alter table moderightinfo add higherlevel int
go