...
CREATE FUNCTION GetDocShareDetailTable  (@userid varchar(10) ,@usertype  varchar(10))
...
BEGIN  
   ...
   begin
      ...
      begin
        ...
        (select convert(VARCHAR(10),roleid)+CONVERT(VARCHAR(10),rolelevel) from hrmrolemembers where resourceid=@userid)) 
        GROUP BY sourceid
     end       
   end
   else 
...
ALTER PROCEDURE DocRpSum (
  ...
  begin 
    ...
    as t1, GetDocShareDetailTable(@userid ,1)  as t2 where t1.id=t2.sourceid and t1.docstatus in (1,2,5)  group by docdepartmentid order by resultcount desc 
