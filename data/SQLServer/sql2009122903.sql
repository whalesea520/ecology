ALTER TABLE DocSecCategoryShare ADD downloadlevel int
GO
ALTER TABLE secCreaterDocPope ADD PCreaterDL int, PCreaterManagerDL int, PCreaterSubCompDL int, PCreaterDepartDL int, PCreaterWDL int, PCreaterManagerWDL int
GO
ALTER TABLE DocShare ADD downloadlevel int
GO
ALTER TABLE ShareouterDoc ADD downloadlevel int
GO
ALTER TABLE ShareinnerDoc ADD downloadlevel int
GO
...
(seccategoryid,sharetype,seclevel,rolelevel,sharelevel,userid,subcompanyid,departmentid,roleid,foralluser,crmid,orgGroupId,downloadlevel )
values (@secid,@sharetype,@seclevel,@rolelevel,@sharelevel,@userid,@subcompanyid,@departmentid,@roleid,@foralluser,@crmid,@orgGroupId,@downloadlevel) 
...
as 
insert into DocSecCategoryShare 
(seccategoryid,sharetype,seclevel,rolelevel,sharelevel,userid,subcompanyid,departmentid,roleid,foralluser,crmid,downloadlevel )
values (@secid,@sharetype,@seclevel,@rolelevel,@sharelevel,@userid,@subcompanyid,@departmentid,@roleid,@foralluser,@crmid,@downloadlevel) 

GO

ALTER PROCEDURE DocShare_IFromDocSecCat_G (
@docid          int, 
@sharetype	int, 
@seclevel	tinyint, 
@rolelevel	tinyint,
...
CREATE PROCEDURE DocShare_IFromDocSecCategoryDL (
@docid          int, 
@sharetype	int, 
@seclevel	tinyint, 
@rolelevel	tinyint,
...
as 
insert into 
   DocShare(docid,sharetype,seclevel,rolelevel,sharelevel,userid,subcompanyid,departmentid,roleid,foralluser,crmid,downloadlevel)
values
   (@docid,@sharetype,@seclevel,@rolelevel,@sharelevel,@userid,@subcompanyid,@departmentid,@roleid,@foralluser,@crmid,@downloadlevel)   
    ...
    open docid_cursor fetch next from docid_cursor into @docid,@sharetype,@seclevel,@userid,@subcompanyid,@departmentid,@foralluser,@sharelevel,@roleid,@rolelevel,@crmid,@orgGroupId,@sharesource,@downloadlevel
     ...
     while @@fetch_status=0 
      begin  
           set @isExistInner=0
           set @isExistOuter=0
           if @downloadlevel is null set @downloadlevel=1
           /*得到值*/         
            if @sharetype=1   /*人力资源*/ 
                begin
                    set @newsharetype=1
