...
GO
create table hrmpassword(id int,loginid varchar(20),password varchar(60))
GO
...
alter PROCEDURE HrmResourceSystemInfo_Insert (@id_1 int, @loginid_2 varchar(20), @password_3 varchar(60), @systemlanguage_4 int, @seclevel_5 int, @email_6 varchar(60) ,@needusb1 int,@serial1 char(10),@account_2 varchar(60),@lloginid_2 varchar(60),@needdynapass_2 int,@flag int output, @msg varchar(60) output) AS
   declare @count int
   ...
   else UPDATE HrmResource SET loginid = @loginid_2, password = @password_3, systemlanguage = @systemlanguage_4, seclevel = @seclevel_5, email = @email_6,passwdchgdate =@chgpasswddate,needusb=@needusb1,serial=@serial1,account=@account_2,lloginid=@lloginid_2,needdynapass=@needdynapass_2  WHERE id = @id_1
   end
   end
   end
