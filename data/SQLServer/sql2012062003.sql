...
if (@departmentid_1 is not null and @subcompanyid1_1 is not null and (@olddepartmentid_1 IS NULL OR @oldsubcompanyid1_1 IS NULL OR @departmentid_1 <> @olddepartmentid_1 or @subcompanyid1_1<>@oldsubcompanyid1_1 or  @seclevel_1 <> @oldseclevel_1) )
begin 
	...
	select workflowid,type,objid,level_n,level2_n,signorder from workflow_flownode t1,workflow_nodegroup t2,workflow_groupdetail t3 where t1.nodetype='0' and t1.nodeid = t2.nodeid and t2.id = t3.groupid and signorder=2

	OPEN @detail_cursor 
	FETCH NEXT FROM @detail_cursor INTO @workflowid ,	@type ,@objid ,@level_n ,@level2_n,@signorder
