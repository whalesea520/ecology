CREATE VIEW tempdatainputindexview AS
...
UPDATE  Workflow_DataInput_entry SET detailindex = ( SELECT  a.detailindex      FROM    tempdatainputindexview a , workflow_base b   WHERE   b.id = Workflow_DataInput_entry.workflowid  AND b.formid = a.billid  AND SUBSTRING(Workflow_DataInput_entry.TriggerFieldName,6,LEN(Workflow_DataInput_entry.TriggerFieldName))=a.fieldid  AND b.isbill=a.isbill)
GO
UPDATE Workflow_DataInput_field SET pagefieldindex=( SELECT  a.detailindex FROM    tempdatainputindexview a ,  workflow_base b, Workflow_DataInput_entry c, Workflow_DataInput_main d  WHERE   b.id = c.workflowid  AND d.id = Workflow_DataInput_field.DataInputID  and d.entryID = c.id AND b.formid = a.billid AND SUBSTRING(Workflow_DataInput_field.pagefieldname,6,LEN(Workflow_DataInput_field.pagefieldname))=a.fieldid AND b.isbill=a.isbill)
GO
