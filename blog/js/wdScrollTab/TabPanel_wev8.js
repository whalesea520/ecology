...
TabPanel.prototype = {
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
      ...
      closer.attr('title', SystemEnv.getHtmlNoteName(4047,language)); 
      closer.appendTo(tab);
      ...
      if(this.tabs.length>0){
	      ...
	      seprator.addClass('seprator_div');
  ...
  kill : function(position){
  	 var language = readCookie('languageidweaver');
  	  var tabId = this.tabs[position].id;
  	  var $this=this;
  	  window.top.Dialog.confirm(SystemEnv.getHtmlNoteName(4048,language),function(){
  	  	deleteTab(tabId);
		  try{
