...
if (loginkey != null && !"".equals(loginkey)) {
	//Object obj = application.getAttribute(loginkey);
	Object obj = null;
	if(WxModuleInit.ifNewQRCodeTable){//新版本
		try{
			Class vc = Class.forName("weaver.login.VerifyLogin4QCode");
			Method m = vc.getDeclaredMethod("getUserCheck",javax.servlet.ServletContext.class,javax.servlet.http.HttpServletRequest.class,javax.servlet.http.HttpServletResponse.class);
			obj = m.invoke(vc.newInstance(),application,request,response);
			//obj =  new weaver.login.VerifyLogin4QCode().getUserCheck(request,response);
		}catch(Exception e){
			e.printStackTrace();
		}
	}else{
		obj = qrCode.getUserFromDb(loginkey);//db方式
		qrCode.delUserFromDb(loginkey);
	}
	if (obj != null) {
		User user = (User)obj;
