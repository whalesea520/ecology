...
function resetBrowser(obj){
	...
	if(_showSubUser==1||_issubcompany==1||_showDept==1||_deptids!=""){//显示下属 本分部 本部门 根据部门ID查询
		$("#page-control").hide();
		...
		$("#control-org").hide();
		$("#control-group").hide();
		if(_showSubUser==1||_issubcompany==1||_deptids!=""){
			if(_showSubUser==1){
				$("#nav-header .header-center").html("Subordinate");
			}else if(_issubcompany==1){
				$("#nav-header .header-center").html("SubCompany");
			}else if(_deptids!=""){
				$("#nav-header .header-center").html("All");
			}
			...
			if(!$("#page-control").hasClass("hrm")){
				...
				if(_showSubUser==1||_deptids!=""){
					$(".control-text",$("#page-control")).html("All");
					showTypeName = "All";
				}else{
					$(".control-text",$("#page-control")).html("SubCompany");
				}
			}
		}else{
			$("#nav-header .header-center").html("Dept");
	...
	//异步加载已选
	if(selectedIds != ""){
		var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp?action=getSelectedDatas&selectedIds=" + selectedIds;
		//window.prompt("",url);
...
function initGroupData(){
	var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp?action=getGroupData";
	$.get(url,null, function(responseText){
		var data;
		if($.isPlainObject(responseText)){
			data = responseText;
	...
	$(".group-title2", $("#group_group_wrap")).click(function(){
		...
		if($(this).hasClass("closed")){
			...
			if(ul.length>0){
				...
				var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp?action=getGroupMembers";
...
function doSearch(){
	$("#list-loadMore").hide();
	$("#list-loading").show();
	var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp?action=getListData&pageNo="+currPgNo;
	var $searchKey = $("#search-key");
	var searchKey = encodeURIComponent($searchKey.val());
	//var searchKey = $searchKey.val();
	$.get(url, {"searchKey":searchKey, "pageSize":pageSize,"showSubUser":_showSubUser,"issubcompany":_issubcompany,"deptids":_deptids}, 
...
function initLastHrm(){
	var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp";
...
function initPageControl(){
	...
	$("#control-dept").click(function(){
		...
		if(!$pageControl.hasClass("dept")){
			...
			showTypeName = "Same Dept";
			$(".control-text", $pageControl).html("Dept");
			changeHeaderTitle();
		}
	...
	$("#control-hrm").click(function(){
		...
		if(!$pageControl.hasClass("hrm")){
			...
			showTypeName = "All User";
			$(".control-text", $pageControl).html("All");
			changeHeaderTitle();
		}
	...
	$("#control-lasthrm").click(function(){
		...
		if(!$pageControl.hasClass("lasthrm")){
			...
			showTypeName = "Last Contacts";
			$(".control-text", $pageControl).html("Contacts");
			changeHeaderTitle();
		}
	...
	$("#control-org").click(function(){
		...
		if(!$pageControl.hasClass("org")){
			...
			showTypeName = "Organization";
			$(".control-text", $pageControl).html("Org");
			changeHeaderTitle();
		}
...
function buildSelectedResultPage(){
	...
	for(var i = 0; i < _selected_arr.length; i++){
		...
		$dataUL.append("<li class=\"one-data\" hrmId=\""+id+"\">"+
							...
							"<div class=\"delete-data\" hrmId=\""+id+"\">Delete</div>"+
						"</li>");
	}
}
	...
	$(".one-tree-data[data-haschild='1']", $wrap).click(function(){
		...
		if($oneTreeData.hasClass("closed")){
			...
			if($treePage.length > 0){
				...
				var url = "/mobile/plugin/plus/browser/hrmBrowserAction_en.jsp?action=getTreeData&type="+dataType+"&pid="+dataId;
	...
	<div id="page-center">
				...
				<div id="list-hrm-container" class="data-container hrm-container">
					<div id="list-hrm-srarch">
						<div class="srarch-inner">
							...
							<input type="search" id="search-key" placeholder="please input name、mobile"/>
							</form>
						</div>
					</div>
					...
					<div id="list-hrm-content">
						...
						<div id="list-loadMore">Load More</div>
						<div id="list-loading">
						</div>
					</div>
				</div>
		...
		<div id="center-footer">
			<div id="choosedResult">
				Selected(<span id="selectedNum">0</span>)
			</div>
