...
<%
	...
	String amountAdvanceBefore = df.format(Util.getDoubleValue(Util.null2String(request.getParameter("amountAdvanceBefore")),0));
	String amountAdvanceAfter = Util.null2String(request.getParameter("amountAdvanceAfter"));
...
%>
	<script type="text/javascript">
		function onSave(){
			var amountAdvanceBefore = jQuery("#amountAdvanceBefore").attr("value");
			var memo1 = jQuery("#memo1").attr("value");
			var amountAdvanceAfter = jQuery("#amountAdvanceAfter").attr("value");
			
			if(checknum(amountAdvanceAfter)){
				amountAdvanceAfter = (amountAdvanceAfter*1.0).toFixed(2);
				parent.setTzsmValue("<%=rowId%>", amountAdvanceBefore, amountAdvanceAfter, memo1);
				
				parent.closeDialog();
			}else{
				alert("<%=SystemEnv.getHtmlLabelName(19298,user.getLanguage()) %>");
				jQuery("#amountAdvanceAfter").attr("value","");
			}
		...
		function checknum(amountAdvanceAfter) { 
		　　if (isNaN(amountAdvanceAfter)) { 
		　　　　return false;
		　　}else{
				return true;
		　　}
 ...
 <TABLE style="margin-top:5px;padding:0;table-layout:fixed;border:1px solid #D8DDE4;" width="100%">
    ...
    <TBODY> 
      <tr style="padding-bottom:6px;padding-top:6px;background-color:#EFF2F6;border-bottom:1px solid #D8DDE4;"> 
      	<td><%=title1 %></td>
      	<td class=Field> 
       		<span id="oldamount_span" ><%=amountAdvanceBefore%></span>
			<input type="hidden" id="amountAdvanceBefore" name="amountAdvanceBefore" value="<%=amountAdvanceBefore%>"><br>
      </td>
    </tr>
     <tr style="padding-bottom:6px;padding-top:6px;background-color:#EFF2F6;border-bottom:1px solid #D8DDE4;"> 
     	<td><%=title2 %></td>
      	<td class=Field> 
       		<input type="text" id="amountAdvanceAfter" name="amountAdvanceAfter" value="<%=amountAdvanceAfter %>" size="12"><br>
     	 </td>
     </tr>
     <tr style="padding-bottom:6px;padding-top:6px;background-color:#EFF2F6;border-bottom:1px solid #D8DDE4;">
     	<td><%=title3 %></td>
