...
<div id="view_page">
	...
	%>
		<div style="width:100%;min-height:20px;position:relative;">
			<img alt="" src="/mobile/plugin/5/images/icon_liststyle_wev8.png"> <%=meeting.get("begintime") %> <%=meeting.get("name") %>
		</div>
	<%
		}
	}
...
function goToday() {
	...
	location = "/mobile/plugin/5/select.jsp?module=<%=module%>&scope=<%=scope%>&detailid=<%=detailid%>&title=<%=titleurl%>&month="+"&year="+"&date="+date;
}
...
function goMonth(d) {
	...
	location = "/mobile/plugin/5/select.jsp?module=<%=module%>&scope=<%=scope%>&detailid=<%=detailid%>&title=<%=titleurl%>&month="+month+"&year="+year+"&date=";
}
...
function goDate(d) {
	location = "/mobile/plugin/5/select.jsp?module=<%=module%>&scope=<%=scope%>&detailid=<%=detailid%>&title=<%=titleurl%>&month=&year=&date="+d;
}
...
function goList() {
	var date = $("#date").val();
	location = "/mobile/plugin/5/list.jsp?module=<%=module%>&scope=<%=scope%>&detailid=<%=detailid%>&title=<%=titleurl%>&month=&year=&date="+date;
}
</script>
