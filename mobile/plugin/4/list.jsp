...
Map pages = scheduleService.getScheduleList(conditionList, pageIndex, pageSize, user);
...
<html>
	...
	function doLeftButton() {
		location = "/mobile/plugin/4/select.jsp?&module=<%=module%>&scope=<%=scope%>&title=<%=titleurl%>&date=<%=date%>";
		return "0";
	}
	function doBack() {
...
<div id="view_page">
	...
	<% if(count==0) { %>
		<div style="width:100%;"><div style="padding:15px;color:#333333;"><%=SystemEnv.getHtmlLabelName(83506,user.getLanguage())%></div></div>
	<% } else { %>
		<% for(int i=0; i<list.size(); i++) { 
			Map schedule = (Map)list.get(i);
			%>
			<a href="/mobile/plugin/4/view.jsp?id=<%=schedule.get("id") %>&pageIndex=<%=pageIndex %>&date=<%=date%>&index=<%=(pageIndex-1)*pageSize+i+1%>&module=<%=module%>&scope=<%=scope %>&title=<%=titleurl%>">
			<div style="width:100%;border-top:solid 1px #FFFFFF;border-bottom:solid 1px #D2D2D2;background: -moz-linear-gradient(top, #E9E9E9, #FDFDFD);filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#E9E9E9', endColorstr='#FDFDFD');background: -webkit-gradient(linear, left top, left bottom, from(#E9E9E9), to(#FDFDFD));">
				<div style="padding-left:10px;height:10px;"></div>
				<div style="padding-left:10px;height:25px;color:#0081CC;font-size:14pt;line-height:25px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;"><%=schedule.get("name") %></div>
				<div style="padding-left:10px;height:18px;color:#A3A3A3;font-size:9pt;line-height:18px;"><%=schedule.get("creater") %>&nbsp;<%=schedule.get("createdate") %></div>
				<div style="padding-left:10px;height:18px;color:#000000;font-size:10pt;line-height:18px;"><%=SystemEnv.getHtmlLabelName(530,user.getLanguage())%>:<%=schedule.get("begindate") %> <%=schedule.get("begintime") %></div>
				<div style="padding-left:10px;height:18px;color:#000000;font-size:10pt;line-height:18px;"><%=SystemEnv.getHtmlLabelName(405,user.getLanguage())%>:<%=schedule.get("enddate") %> <%=schedule.get("endtime") %></div>
				<div style="padding-left:10px;height:10px;"></div>
			</div>
			</a>
		<%} %>
	...
	<div style="width:100%;height:30px;line-height:30px;background: -moz-linear-gradient(top, #FEFEFE, #E7E7E7);filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#FEFEFE', endColorstr='#E7E7E7');background: -webkit-gradient(linear, left top, left bottom, from(#FEFEFE), to(#E7E7E7));">
		...
		<a href="/mobile/plugin/4/view.jsp?pageIndex=1&date=<%=date%>&module=<%=module%>&scope=<%=scope%>&title=<%=titleurl%>" title="First Page"><strong>&lt;&lt;</strong> <%=SystemEnv.getHtmlLabelName(1500,user.getLanguage())%></a>
		<a href="/mobile/plugin/4/view.jsp?pageIndex=<%=prePage %>&date=<%=date%>&module=<%=module%>&scope=<%=scope%>&title=<%=titleurl%>" title="Prev Page"><%=SystemEnv.getHtmlLabelName(1258,user.getLanguage())%></a>
		<a href="/mobile/plugin/4/view.jsp?pageIndex=<%=nextPage %>&date=<%=date%>&module=<%=module%>&scope=<%=scope%>&title=<%=titleurl%>" title="Next Page"><%=SystemEnv.getHtmlLabelName(1259,user.getLanguage())%></a>
		<a href="/mobile/plugin/4/view.jsp?pageIndex=<%=pageCount %>&date=<%=date%>&module=<%=module%>&scope=<%=scope%>&title=<%=titleurl%>" title="Last Page"><%=SystemEnv.getHtmlLabelName(34191,user.getLanguage())%> <strong>&gt;&gt;</strong></a>
		</div>
	</div>
	<% } %>
