...
if (PortalUtil.isuserdeploy()) {
    request.getRequestDispatcher("/cowork/AddCoWork4Simple.jsp").forward(request, response);
    return;
}
%>
...
String prjid ="";
if(!"".equals(taskrecordid)){
	rs.executeSql("select prjid from Prj_TaskProcess where id = "+taskrecordid);
	rs.next();
...
<wea:layout>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(83210,user.getLanguage())%>'>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(16169,user.getLanguage())%>' attributes="{'itemAreaDisplay':'none'}">
		...
		<wea:item> 
			...
			<SPAN id=enddatespan style="color:#000000;font-weight:normal"><%=enddate%></SPAN>
		...
		<wea:item>
            ...
            %>
         	<brow:browser viewType="0" name="relateddoc" browserValue='<%=""+docid%>'
		              	...
		              	hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1' width="60%" browserSpanValue='<%=docName%>'> 
         	</brow:browser>
		...
		<%if(appStatusMap.containsKey("workflow")){%>
			...
			<wea:item>
				...
				<brow:browser viewType="0" name="relatedwf" browserValue=""
				               	...
				               	hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1' width="60%" browserSpanValue=""> 
               	</brow:browser>
		...
		<%if(appStatusMap.containsKey("crm")){%>
			...
			<wea:item>
				<brow:browser viewType="0" name="relatedcus"
	         					...
	         					browserSpanValue = '<%=CustomerInfoComInfo.getCustomerInfoname(CustomerID)+""%>'
				               	...
				               	hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1' width="60%" > 
               	</brow:browser>
