...
$(function() {
	$('.e8_box').Tabs({
		...
		staticOnLoad:true,
		objName:"<%=SystemEnv.getHtmlLabelName(131886,user.getLanguage())%>"
	});
	attachUrl();
...
function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		...
		$(this).attr("href","/cowork/CoworkCommentList.jsp?mainid=<%=mainid%>&typeid=<%=typeid%>&from=comment&commentStatus="+commentStatus);
