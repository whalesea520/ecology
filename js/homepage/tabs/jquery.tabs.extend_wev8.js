		...
		Tabs:function(_options){
			...
			var setIframeHeight = function(){
				...
				if(!options.iframe && !exceptHeight && jQuery("div"+options.container).closest("div.zDialog_div_content").length>0){
					...
					jQuery("div"+options.container).closest("div.zDialog_div_content").css("overflow","hidden");
				}
				...
				//console.log(jQuery("div"+options.container).find("ul.tab_menu").offset().top);
			}
			...
			var initTopTitle = function(ifrm,_this){
				...
				var __topTitle = _document.getElementById(options.topTitle);
				if(!__topTitle && topTitleCount<10){
			...
			var init = function(obj,_contentWindow){
				...
				var tabHandle = function( elem ){
					...
					update = function(){
						...
						insertImages();
						try{
							jQuery("div"+options.container).find("div.e8_outbox").css("top",jQuery("div"+options.container).find("ul.tab_menu").offset().top);
						}catch(e){
				...
				jQuery(options.container).find("div.tab_box").css("visibility","visible");
				return jQuery(obj);
			}
...
var i=0;
 	...
 	function getElement(){
 		try{
 			...
 			var topt = _document.getElementById("topTitle")
 			if(topt){
 				update2();
 			}else{
 				if(i<=50){
	 				setTimeout(function(){getElement();},10);
	 				i++;
	 			}
 			}
 		}catch(e){
