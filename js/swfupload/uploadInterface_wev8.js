var upfilesnum=0;
var uploadeErrMsg="";
var nowtarget = "_self";//frmmain.target;
...
function uploadSuccess_1(file, server_data) {
    if ($G(this.customSettings.uploadfiedid)) {
	   ...
	   for(var key in json){
		 value = json[key];
		 if(key.toLowerCase()=="id"){//直接追加到隐藏域中
		 ...
		 if(value=="0"){
			...
			msg="<font color='red'>失败</span>";
			if(value=="1"){
				reason = "数据库记录插入失败";
			}else if(value=="2"){
				reason = "数据库存在同名的Action";
			}else if(value=="3"){
				reason = "文件上传失败";
			}
		 }
		 if(value=="2"){
			if(confirm("【"+key+"】存在重名的Action，是否需要覆盖？")){
				//调用ajax方法更新部署状态
				updateDeployStatus(key);
			}else{
				 //将提示信息更新到指定的信息显示层
				appendMsg(key,msg,reason);
			}			
		 }else{
			 //将提示信息更新到指定的信息显示层
			 appendMsg(key,msg,reason);
...
function appendMsg(key,msg,reason){
	if(reason==null || reason=="")reason="&nbsp;"
	 jQuery("#message").append("<tr class='msgDiv'><td class='msgSpan'>"+key+"</td><td  class='msgSpan'>"+msg+"</td><td class='msgSpan' style='color:red;'>"+reason+"</td></tr>");
}

function updateDeployStatus(fileName){
	jQuery.ajax({
		url:"/weaver/weaver.admincenter.servlet.InterfaceServlet?src=update&name="+fileName,
		dataType:"json",
		cache:false,
		async: false,
		success: function(map,status,response){
			  var value = "";
			   var msg = "";
			   var reson = "";
			for(var key in map){
		...
		error: function(map,status,response){
		}
	});
}
...
function checkfileuploadcomplet() {
    if (upfilesnum > 0) {
		...
		jQuery("#interface",parent.document).submit();
    }
}
