	   ...
	   for(var key in json){
		 ...
		 if(value=="0"){
			...
			if(value=="1"){
				...
				reason = "缺少元素配置文件conf.xml";
			}else if(value=="3"){
				reason = "缺少文件夹;resource/js、css、image";
			}else if(value=="5"){
				reason = "文件上传失败";
			}else if(value=="6"){
				reason = "压缩包已损坏";
			}else if(value=="7"){
				reason = "conf.xml中配置不完整";
			}
...
function appendMsg(key,msg,reason){
	if(reason==null || reason=="")reason="&nbsp;"
	 jQuery("#message").append("<tr class='msgDiv' style='height:40px;'><td class='msgSpan'>"+key+"</td><td  class='msgSpan'>"+msg+"</td><td class='msgSpan' style='color:red;'>"+reason+"</td></tr>");
}
