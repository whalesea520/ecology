...
/* ******************* */
var SWFUpload;
var _savefile='';//html模板上传文件参数
var file_extension='';//html模板上传文件扩展名
var newUrl='';//html模板上传url拼参数
...
SWFUpload.prototype.initSWFUpload = function (settings) {
	...
	try {
		...
		this.initSettings();
		//载入按钮
		_getBrowserInfo() ? loadNoFlash() : this.loadFlash(); //是safari、chrome55+载入非flash
	...
	function replaced_originPlace() {
		...
		//根据页面地址判断按钮类型
		if(
			(window.location.href.indexOf("/workflow/request") >= 0 && !jQuery("#" + settings.button_placeholder_id).closest("span.phrase_btn"))
			|| (window.location.href.indexOf("/cowork/") >= 0 && !!jQuery("#" + settings.button_placeholder_id).parent().hasClass("e8fileupload"))
			) 
		{
			...
			(window.location.href.indexOf("/docs/docs") >= 0)
			||(window.location.href.indexOf("/docs/mould") >= 0 && jQuery("#" + settings.button_placeholder_id).parent().hasClass("e8fileupload"))
			|| (window.location.href.indexOf("/cpt/print/moduleedit.jsp") >= 0 && jQuery("#" + settings.button_placeholder_id).parent().hasClass("e8fileupload"))
			|| (window.location.href.indexOf("page/element/newNotice/detailsetting.jsp") >= 0 && settings.button_placeholder_id != "noticeuploadbtn")
			|| (window.location.href.indexOf("page/maint/template/news/NewsTemplateEdit.jsp") >= 0)
		) {//文档
			tempParent.innerHTML = getHTML_docs(0, "22px");
		}
		else if(window.location.href.indexOf("page/element/newNotice/detailsetting.jsp") >= 0 && settings.button_placeholder_id=="noticeuploadbtn"){
			tempParent.innerHTML = getHTML_detailsetting(0, '168px','80px');
		}
		else if(window.location.href.indexOf("/blog/") >= 0) {//微博
			tempParent.innerHTML = getHTML_weibo(1);
		}
		else if(
			(window.location.href.indexOf("/workplan/") >= 0 || window.location.href.indexOf("/meeting/") >= 0)
			|| (window.location.href.indexOf("/cowork/") >= 0 && !jQuery("#" + settings.button_placeholder_id).parent().hasClass("e8fileupload"))
		)
		{//日程
	...
	function getHTML_detailsetting(isVisib, width,height) {
		return '<div  id=' + settings.button_placeholder_id + ' style="opacity:' + isVisib + ';width:' + width + ';height:' + height + ';background-color:#1565a9;color:#fff!important;top:-80px;right:0px" >附件</div>';
	}
	...
	var getUrl = function () {
		...
		_savefile = "uploadImg_" + curdate + "." + (file_extension || 'jpg');
		// console.log('getUrl_file_extension',file_extension)
		return "/weaver/weaver.workflow.exceldesign.ExcelUploadServlet?method=uploadFile&savefile="+_savefile;
	}
	var nuMovieName = this.movieName;
	if(_getBrowserInfo()) {
		newUrl = settings.upload_url;
		// console.log('newUrl1',newUrl,window.location.href);
		if(window.location.href.indexOf('/workflow/exceldesign/excelUploadImg.jsp')>=0){
			newUrl=getUrl()+'&'+this.getH5Param()
			// console.log('newUrl',newUrl);
		}
		this.newUploader = new plupload.Uploader({
			file_data_name: "Filedata",
			browse_button: settings.button_placeholder_id, //触发文件选择对话框的按钮，为那个元素id
			url: newUrl, //服务器端的上传页面地址
...
SWFUpload.prototype.getH5Param = function(){
	var postParams = this.settings.post_params; 
	if(""!=postParams["secId"] && typeof(postParams["secId"]) != "undefined"){
		this.settings.secid = postParams["secId"];
	}
...
SWFUpload.prototype.callFlash = function (functionName, argumentArray) {
	if(_getBrowserInfo()){return false}
...
SWFUpload.prototype.startUpload = function (fileID) {
	...
	if(_getBrowserInfo() && typeof this.settings.button_placeholder_id != "object") {
		//console.log("sb1");
		//var _this = SWFUpload.instances[this.movieName];
		var _this = this;
...
SWFUpload.prototype.setUploadURL = function (url) {
	if(_getBrowserInfo()){
		//alert("setUploadURL----html5");
		return;
	}else{
		//alert("setUploadURL----flash");
...
SWFUpload.prototype.uploadComplete = function (file) {
	if(_getBrowserInfo() && window.location.href.indexOf('/workflow/exceldesign/excelUploadImg.jsp')>=0){
		file_extension = file.name.split(".")[1];
		jQuery("#imgname").text(_savefile);
		jQuery("[name=imgsrc_2]").val("/filesystem/exceldesign/uploadimg/"+_savefile);
		setPicDesc();
