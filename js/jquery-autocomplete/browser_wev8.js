	   ...
	   return {
			...
			del:function(evt,obj,isMustInput,needHidden,options){
				...
				try{		
					...
					if(newids==""){
						if(jQuery(obj).parent().parent()&&$G("needcheck")){
							var objid = jQuery(obj).parent().parent().attr("id").replace("span","");
							if(("," + $G("needcheck").value + ",").indexOf("," + objid + ",") >= 0){
								isMustInput = 2;
							}else{
								isMustInput = 1;
							}
			...
			__callback:function(id,fieldid,isMustInput,hasInput,options){
				...
				spans.each(function(){
					...
					if(a.length>0){
						...
						if(isMustInput==2 && jQuery("#"+fieldid).attr("viewtype")==1){
					...
					if(!(e8browserIsHrm(type)=="")){
						...
						*/
						...
						if(isSingle==false){
							...
							try {
								if(window.location.href.indexOf('formmode')<0){
									eval(nameAndIdHidden.attr('onpropertychange'));
								}
							} catch (e) {}
