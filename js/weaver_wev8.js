...
if (typeof numberChangeToChinese == "undefined") { 
...
// 数字金额大写转小写
if (typeof chineseChangeToNumber == "undefined") { 
    ...
    if(value != ""){
        ...
        if (!reg.test(value)){
            ...
            try{
				...
				var dType = obj.getAttribute("datavaluetype");
				if(dType!=null &&(dlength==null && dType.toString()=="5")){
                    dlength = 2;
                }else if(obj.getAttribute("datatype")=='int'){
                	dlength = 0;
...
if (typeof checkdecimal_length == "undefined") { 
...
if (typeof checkLength1 == "undefined") { 
...
function checkinput3(elementname,spanid,viewtype){
    ...
    if(viewtype==1){
        ...
        if(tmpvalue!=""){
            ...
            //注掉这个是因为两个选择框A把Ｂ做子选项后，又做显示属性联动,导致在次调onchage事件多出一个红色感叹号的原因
            //spanid.innerHTML="<IMG src='/images/BacoError_wev8.gif' align=absMiddle>";
           // elementname.value = "";
        }
    }
...
function setFieldReadOnly(fieldid,flag,fieldattr){
    if(flag){
        ...
        try{
            ...
            var tempvalue = "";
            var fieldName = jQuery(obj).attr("name");
...
if (typeof getTitleTip == "undefined") { 
...
function isEmptyfsUpdaload(fieldid){
    ...
    try{
        ...
        try{
		    ...
		    var fieldidspan = fieldid + "_span";
		    if(!!!$GetEle(fieldidspan))
		    fieldidspan="field_"+swfuploadid+"span";
		    $GetEle(fieldidspan).innerHTML="";
		    var tempouploadobj=eval("oUpload"+swfuploadid);
		    var sta = tempouploadobj.getStats();
		    if(sta.files_queued != 0){
		         result=false;
		    }
        }catch(e){
        	if(window.console)console.log("isEmptyfsUpdaload----",e);
        }
 ...
 * @param extra
 */
 if (typeof sendMsgToPCorWeb == "undefined") { 
 ...
 * @param sharejson 
 */
 if (typeof socialshareToEmessage == "undefined") { 
  function socialshareToEmessage(sharejson){
            sharejson=encodeURIComponent(sharejson);
            var url="/social/im/SocialHrmBrowserForShare.jsp?sharejson="+sharejson;
