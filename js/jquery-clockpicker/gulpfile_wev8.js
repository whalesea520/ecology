...
function js(prefix) {
	gulp.src('src/clockpicker_wev8.js')
		.pipe(rename({
			prefix: prefix + '-'
		}))
...
function css(prefix) {
	var stream;
	if (prefix === 'bootstrap') {
		stream = gulp.src('src/clockpicker_wev8.css');
	} else {
		// Concat with some styles picked from bootstrap
		stream = gulp.src(['src/standalone_wev8.css', 'src/clockpicker_wev8.css'])
			.pipe(concat('clockpicker_wev8.css'));
	}
	stream.pipe(rename({
			prefix: prefix + '-'
		}))
