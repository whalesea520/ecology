...
var webFXTreeConfig = {
	rootIcon        : '',//'/images/xp_none/foldericon_wev8.png',
	openRootIcon    : '',//'/images/xp_none/openfoldericon_wev8.png',
	folderIcon      : '',//'/images/xp_none/foldericon_wev8.png',
	openFolderIcon  : '',//'/images/xp_none/openfoldericon_wev8.png',
	fileIcon        : '/images/treeimages/Home_wev8.gif',
...
WebFXTree.prototype.toString = function() {
	//var goalFlowStr = "<button class='Browser' id='SelectFlowID' onClick=\"onShowFlowID('0','"+this.action+"')\" style=''></button>";
	...
	var str = "<table onmouseover=\"document.getElementById('"+this.id+"-anchor').style.backgroundColor='#DDDDDD';document.getElementById('"+this.id+"-anchor').style.border='1px solid #0099CC';\" onmouseout=\"document.getElementById('"+this.id+"-anchor').style.backgroundColor='';document.getElementById('"+this.id+"-anchor').style.border='1px solid #FFFFFF';\" id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\" style=\"width:100%\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=\"40%\">" +
		...
		">" + this.text + "</span></td><td width=\"40%\">"+tmpArray2+"</td><td width=\"20%\">"+planFlowStr+"</td></tr></table>" +
	...
	html = "<div style=\"overflow:hidden;height:300px;position:relative;\" id=\"overFlowDivTree\"><div class=\"ulDiv\" style='display:none;' >"+
	"<div id='hiddenImage' style='display:none;'><img src='/images/xp_none/Lminus_wev8.png' onload='prefectScrollBar();'/></div>"+html+"</div></div>";
	return html;
};
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var str = "<table onmouseover=\"document.getElementById('"+this.id+"-anchor').style.backgroundColor='#DDDDDD';document.getElementById('"+this.id+"-anchor').style.border='1px solid #0099CC';\" onmouseout=\"document.getElementById('"+this.id+"-anchor').style.backgroundColor='';document.getElementById('"+this.id+"-anchor').style.border='1px solid #FFFFFF';\" id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\" style=\"width:100%\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=\"40%\">" +
		...
		"<span style=\"cursor:hand;border:1px solid #FFFFFF;padding:0 4px 0 4px\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" +
		(this.target ? " target=\"" + this.target + "\"" : "") +
		">" + label + "</span></td><td width=\"40%\">"+tmpArray2+"</td><td width=\"20%\">"+planFlowStr+"</td></tr></table>" +
		"<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
	var sb = [];
	for (var i = 0; i < this.childNodes.length; i++) {
		sb[i] = this.childNodes[i].toString(i,this.childNodes.length);
...
function prefectScrollBar(){
	...
	var height = jQuery(".webfx-tree-container").height();
	if(!!height){
...
function refreshTree(id,parentId,options){
	...
	if(parentId=="0" || !parentId || parentId=="undefined")parentId="root";
	if(!!parentId){
		//jQuery("img[_id='img_"+parentId+"']:first").click();
		webFXTreeHandler.expandAnyway(jQuery("img[_id='img_"+parentId+"']").get(0));
		window.setTimeout(function(){
