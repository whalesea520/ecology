...
function WebFXLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon,objXmlSrc) {	
	// call super
	...
	if (this.open)
		if(objXmlSrc!=null)  _xmlFileLoaded(objXmlSrc,this);
		else  _startLoadXmlTree(this.src, this);
	else {
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon,menuid,menuparentId,linkAddress,isCustom,visible,viewIndex,ownerid,refersubid) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, menuid, menuparentId,linkAddress,isCustom,visible,viewIndex,ownerid,refersubid);
...
function _xmlTreeToJsTree(oNode) {
	...
	var menuid = oNode.getAttribute("id");
	var menuparentId = oNode.getAttribute("parentId");
	var linkAddress = oNode.getAttribute("linkAddress");
	var isCustom = oNode.getAttribute("isCustom");
	var visible = oNode.getAttribute("visible");
	var viewIndex = oNode.getAttribute("viewIndex");
	var ownerid = oNode.getAttribute("ownerid");
	var refersubid = oNode.getAttribute("refersubid");

	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon,menuid,menuparentId,linkAddress,isCustom,visible,viewIndex,ownerid,refersubid);
	else {		
		jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon,menuid,menuparentId,linkAddress,isCustom,visible,viewIndex,ownerid,refersubid);
	}
