 ...
 */
public class Mailbox extends BaseBean implements Action {

    private Log log = LogFactory.getLog(Mailbox.class.getName());
    ...
    private static String mailTittle = "合同签订";// 邮件的标题
    private static String mailText ="合同签订成功！";	// 邮件的文本内容
    private static String mail_host ="smtp.mxhichina.com";	// 邮件的服务器域名

    @Override
    public String execute(RequestInfo request) {
        /***interface action  start*****/
        this.log.error("######################   合同签订邮件发送Action     ###########################");
        ...
        try {
            ...
            String sqlMa="select bsqs1 from  "+mainTable+" where requestId='"+requestid+"'  ";
            rss.execute(sqlMa);
            if(rss.next()){
                sqs=rss.getString("bsqs1");
            }

            String sqlMb="select yx from uf_lxrxx  where sqs='"+sqs+"' ";
            rss1.execute(sqlMb);
            if(rss1.next()){
        ...
        this.log.error("######################合同签订调用邮箱Action end################################################");
        return "1";
    }
