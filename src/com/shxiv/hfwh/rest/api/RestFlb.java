 ...
 */
@Path("/M_AU_ACCPUR")
public class RestFlb extends BaseBean  {
    ...
    public String merchantsApproval(@QueryParam("key") String key, @QueryParam("timestamp") String timestamp, @QueryParam("sign") String sign) {
        ...
        String fNodeId=baseBean.getPropValue("ShxivHfwh_param","nodeFlb");
        String wfId=baseBean.getPropValue("ShxivHfwh_param","wfFlb");
        ...
        if(passEnc.equals(sign)){
            ...
            builder.append(" select m.requestId,m.bh,m.sqrq,y.gcbh as gys,m.cgzsl,m.cgzje,'LD0002' as fhc, ");
            builder.append("  m.rq,m.sj,dt.fwfbbm,dt.cgsl,dt.cgje from formtable_main_213 m ");
            ...
            while (rss.next()){
                ...
                String upDate=rss.getString("rq");
                if(upDate!=null&&!"".equals(upDate)){
                    upDate=upDate.replaceAll("-","");
                }
                jsonObject1.put("STATUSTIME",rss.getString("sj"));
                jsonObject1.put("OPERATION_DATE",upDate);


                //明细表数据封装
                String mbm="";
                String fld=rss.getString("fwfbbm");
