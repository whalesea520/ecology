...
public class HtKhSsAction extends BaseBean implements Action {

    private Log log = LogFactory.getLog(HtXgAction.class.getName());

    @Override
    public String execute(RequestInfo request) {
        /***interface action  start*****/
        this.log.error("######################开始调用合同款号转移送审控制Action###########################");
        ...
        try {
            ...
            if(rss.next()) {
                ...
                int cy = 0;
                String sqlMc = "select COUNT(*) as cy from formtable_main_150 a " +
                        " join workflow_requestbase b on a.requestId=b.requestid " +
                        " where b.currentnodetype in (0,1,2) and a.htbm='" + htbh + "'" +
                        " and a.hfkh='" + hfkh + "'";
                ...
                // log.error("cy:" + cy);
                if (cy > 0) {
                    tempMessage = "转移的款号在送审流程中还有未归档的流程,请确认该合同的送审流程是否全部已归档！";
        ...
        this.log.error("######################合同款号转移送审Action end##################################");
