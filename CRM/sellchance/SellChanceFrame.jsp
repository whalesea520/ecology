...
int sellChanceNumber = CustomerStatusCount.getSellChanceNumber(user.getUID()+"");//提醒数量
%>
		...
		<div class="e8_boxhead">
			...
			<div class="e8_ultab">
				...
				<div>
					<ul class="tab_menu">
						<li class="current" >
							<a href="" target="tabcontentframe" _labelid="my"><%=SystemEnv.getHtmlLabelName(82546,user.getLanguage())%></a>
						</li>
						<li>
							<a href="" target="tabcontentframe" _labelid="attention"><%=SystemEnv.getHtmlLabelNames("25436,32922",user.getLanguage())%></a>
						</li>
						<li>
							<a href="" target="tabcontentframe" _labelid="all"><%=SystemEnv.getHtmlLabelNames("332,32922",user.getLanguage())%></a>
						</li>
						<li>
							<a href="" target="tabcontentframe" _labelid="expire"><%=SystemEnv.getHtmlLabelNames("17497",user.getLanguage())%><%=sellChanceNumber==0?"":"("+sellChanceNumber +")" %></a>
						</li>
						<%
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	    $('.e8_box').Tabs({
			...
			staticOnLoad:true,
			objName:"<%=SystemEnv.getHtmlLabelName(82545,user.getLanguage())%>"
		});
		attachUrl();
	});
  ...
  function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		...
		var url="/CRM/sellchance/SellChanceMain.jsp?labelid="+labelid;
		$(this).attr("href",url);
	});
...
function doMouseout(obj){
	var type=$(obj).attr("_type");
	$(obj).css("background-image","url('/CRM/images/icon_"+type+".png')").css("color","#1098ff");
}
