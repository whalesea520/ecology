...
function openDialog(id){
	...
	var url = "/CRM/sellchance/AddCRMTypes.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(82538 ,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/sellchance/EditCRMTypes.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(82538 ,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = false;
...
function doDel(id,fullname)
{
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
		jQuery.post("/CRM/sellchance/CRMTypesOperation.jsp",{"method":"delete","id":id,"name":fullname},function(data){
...
function registerDragEvent(){
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "CRMTypesOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.post("/CRM/sellchance/CRMTypesOperation.jsp?method=delete&id="+idArr[i],function(){
					_table.reLoad();
				});
			}
		});
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String fromSql = " from CRM_SellTypes t1 ";
									...
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMTypesCheckbox\" />"
