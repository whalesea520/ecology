...
{
	var value=$("input[name='failFactor']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
//新建、编辑失败因素 add by Dracula @2014-2-8
function openDialog(id){
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	var url = "/CRM/sellchance/AddCRMFailfactor.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16500 ,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/sellchance/EditFailfactor.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16500 ,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = true;
...
{
	if(isdel())
		location.href="/CRM/sellchance/CRMFactorOperation.jsp?method=delete&sign=f&id="+id+"&name="+fullname;
}
...
function registerDragEvent(){
     ...
     jQuery("#failfactorTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "CRMFactorOperation.jsp?method=sort&sign=f&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.ajax({
					url:"/CRM/sellchance/CRMFactorOperation.jsp?method=delete&sign=f&id="+idArr[i],
					type:"post",
					async:true,
...
String titlename = SystemEnv.getHtmlLabelName(15104,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0" id="failfactorTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
							...
							<tr>
								...
								<%
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_FailureReason+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_FailureReason,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMFailfactortCheckbox\" />"
								...
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_FailureReason%>">
