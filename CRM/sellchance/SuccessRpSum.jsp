...
<%
	...
	String fromSql = "CRM_SellChance  t1,CRM_Successfactor  t2 ,"+leftjointable+" t4,CRM_CustomerInfo t3";
	String backfields = "t1.sufactor ,COUNT(distinct t1.id) AS resultcount";
	String sqlWhere = "t1.sufactor=t2.id  and t3.deleted=0 and t3.id= t1.customerid and t1.customerid = t4.relateditemid "+sqlwhere;
	String groupby = "t1.sufactor";
	
	String tableString =" <table instanceid=\"info\"  pageId=\""+PageIdConst.CRM_RPSuccessReason+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_RPSuccessReason,user.getUID(),PageIdConst.CRM)+"\" tabletype=\"none\">"+ 
    "<sql backfields=\""+backfields+"\" sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  "+
    	"sqlorderby=\""+orderby+"\"  sqlprimarykey=\"t1.sufactor\" sqlsortway=\"Desc\" sqlgroupby=\""+groupby+"\"  sumColumns=\"resultcount\" />"+
    "<head>"+
    "<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(15103,user.getLanguage()) +"\" column=\"sufactor\""+
    	" transmethod=\"weaver.crm.sellchance.SellsuccessComInfo.getSuStatusname\"/>"+ 
	"<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(15258,user.getLanguage()) +"\" column=\"resultcount\""+ 
		" transmethod=\"weaver.crm.report.CRMReporttTransMethod.getSellChanceInfo\" otherpara=\"column:sufactor\"/>"+ 
...
%>
<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_RPSuccessReason%>">
...
function showDetailInfo(id){
	...
	dialog.URL = "/CRM/sellchance/SellChanceReport.jsp?msg=report&sufactor="+id+"&fromdate=<%=fromdate%>&enddate=<%=enddate%>&viewer=<%=resource%>&customer=<%=customer%>&sellstatusid=<%=sellstatusid%>&endtatusid=<%=endtatusid%>&preyield=<%=preyield%>&preyield_1=<%=preyield_1%>";
	dialog.show();
}
</script>
</HTML>
