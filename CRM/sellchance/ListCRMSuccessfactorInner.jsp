...
{
	var value=$("input[name='successFactor']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
function openDialog(id){
	...
	var url = "/CRM/sellchance/AddCRMSuccessfactor.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16499 ,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/sellchance/EditSuccessfactor.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16499 ,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = true;
...
{
	if(isdel())
		location.href="/CRM/sellchance/CRMFactorOperation.jsp?method=delete&sign=s&id="+id+"&name="+fullname;
}
...
function registerDragEvent(){
     ...
     jQuery("#successfactorTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "CRMFactorOperation.jsp?method=sort&sign=s&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.ajax({
					url:"/CRM/sellchance/CRMFactorOperation.jsp?method=delete&sign=s&id="+idArr[i],
					type:"post",
					async:true,
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0" id="successfactorTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String fromSql = " from CRM_Successfactor t1 ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_SuccessReason+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_SuccessReason,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMSuccessfactortCheckbox\" />"
								...
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_SuccessReason%>">
