...
<%
	...
	if(labelid!=0){
	  String sql="select name,isUsed,labelOrder,labelColor,textColor from CRM_SellchanceLabel where id="+labelid;
...
<script type="text/javascript">
  ...
  function doSave(){
     ...
     if("<%=type%>"=="createLabel"||("<%=type%>"=="editLabel"&&"<%=name%>"!=$("#labelName").val())){
	     $.post("/CRM/sellchance/SellchanceLabelOperation.jsp?name="+name,{type:'isExist'},function(data){
	        if($.trim(data)=="true")
