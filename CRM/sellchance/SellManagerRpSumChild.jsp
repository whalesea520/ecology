...
<form name=frmmain id=weaver method=post action="SellManagerRpSumChild.jsp">
...
<%
	...
	String fromSql = "CRM_CustomerInfo t1,CRM_Contract t2,HrmResource t3  ,"+leftjointable+" t4 ";
	String backfields = "sum(t2.price) resultcount_n ,sum(t2.price) resultcount,t1.manager ";
	String sqlWhere = "t2.crmId = t4.relateditemid and  t1.id=t2.crmId and t1.manager = t3.id and t2.status >=2 "+sqlwhere;
	String groupby = "t1.manager";
	
	String tableString =" <table instanceid=\"info\"  pageId=\""+PageIdConst.CRM_RPResourceMoney+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_RPResourceMoney,user.getUID(),PageIdConst.CRM)+"\" tabletype=\"none\">"+ 
    "<sql backfields=\""+backfields+"\" sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  "+
    	"sqlorderby=\""+orderby+"\"  sqlprimarykey=\"t1.manager\" sqlsortway=\"Desc\" sqlgroupby=\""+groupby+"\"  sumColumns=\"resultcount\" />"+
    "<head>"+
    "<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(15255,user.getLanguage()) +"\" column=\"manager\""+
		" href=\"/hrm/resource/HrmResource.jsp\" linkkey=\"id\" linkvaluecolumn=\"manager\" target=\"_blank\""+
...
<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_RPResourceMoney%>">
