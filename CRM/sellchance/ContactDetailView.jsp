...
<%
	...
	rs.execute("SELECT customerid , subject FROM CRM_SellChance where id = '"+sellchanceid+"'");
	rs.next();
...
<div id="listoperate">
	<a style="margin-left:10px;" href="/CRM/sellchance/ViewSellChanceTab.jsp?isfromtab=true&targetType=blank&id=<%=sellchanceid%>" title="<%=subject %>" target="_blank"><%=getCrmNameSeptem(subject)%></a>
			/
	<a style="margin-left:10px;" href="/CRM/data/ViewCustomer.jsp?CustomerID=<%=customerid%>" title="<%=CustomerInfoComInfo.getCustomerInfoname(customerid) %>" target="_blank"><%=getCrmNameSeptem(CustomerInfoComInfo.getCustomerInfoname(customerid))%></a>
			/
	<a href="javascript:void(0)" title="<%=ResourceComInfo.getLastname(manager) %>" onclick="pointerXY(event);openhrm('<%=manager%>');return false;"><%=getCrmNameSeptem(ResourceComInfo.getLastname(manager))%></a>
</div>
<div class="tabStyle2" id="topMenu">
	<div class="tabitem" _type="message" url="/CRM/data/ViewContactLog.jsp?chanceid=<%=sellchanceid%>&isfromtab=true&fromType=sellchance">
		<div class="title"><A href="javascript:void(0)"><%=SystemEnv.getHtmlLabelName(6082,user.getLanguage())%></A></div>
		<div class="arrow"></div>
		<div class="tabseprator"></div>
	</div>
	<div class="tabitem" _type="contacter" url="/CRM/sellchance/ContacterCard.jsp?customerid=<%=customerid%>">
