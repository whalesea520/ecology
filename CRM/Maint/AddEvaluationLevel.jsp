...
function checkSubmit(){
    if(check_form(weaver,'name,levelvalue')){
        weaver.submit();
    }
}
...
if("<%=isclose%>"=="1"){
	parentWin.location = "EvaluationLevelListInner.jsp";
...
String titlename = SystemEnv.getHtmlLabelName(82,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(6070,user.getLanguage())+SystemEnv.getHtmlLabelName(324,user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(16328,user.getLanguage()) %>"/>
</jsp:include>
...
<wea:layout>
	<wea:group context='<%= SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
		...
		<wea:item><%=SystemEnv.getHtmlLabelName(6072,user.getLanguage())%></wea:item>
		<wea:item>
			<wea:required id="levelvalueimage" required="true" value="">
				<INPUT class=InputStyle maxLength=50 size=45 name="levelvalue" onchange='checkinput("levelvalue","levelvalueimage")'
				onKeyPress="ItemCount_KeyPress()"
				temptitle="<%=SystemEnv.getHtmlLabelName(6072,user.getLanguage())%>">
			</wea:required>
		</wea:item>
	</wea:group>
</wea:layout>
