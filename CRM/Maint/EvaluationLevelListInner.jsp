...
{
	var value=$("input[name='levelValue']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
//新建、编辑评估标准 add by Dracula @2014-2-7
function openDialog(id){
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	var url = "/CRM/Maint/AddEvaluationLevel.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16328 ,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/Maint/EditEvaluationLevel.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16328 ,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = true;
...
function doDel(id,fullname)
{
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
		jQuery.post("/CRM/Maint/EvaluationLevelOperation.jsp",{"method":"delete","id":id},function(){
			_table.reLoad()
		});
	});
}
...
function registerDragEvent(){
     ...
     jQuery("#evaluationLevelTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "EvaluationLevelOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.post("/CRM/Maint/EvaluationLevelOperation.jsp",{"method":"delete","id":idArr[i]},function(){
					_table.reLoad()
				});
			}
		});
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0" id="evaluationLevelTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String tableString = "";
									String backfields = " t1.id, t1.name, t1.levelvalue, t1.orderkey ";
									String fromSql = " from crm_evaluation_level t1 ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_EvaluationLevel+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_EvaluationLevel,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMEvaluationLevelResultCheckbox\" />"
									...
									+ otherpara + "\" />";
									tableString += " <col width=\"75%\"  text=\"" + SystemEnv.getHtmlLabelName(6072,user.getLanguage())
									+ "\" column=\"levelvalue\" orderkey=\"t1.levelvalue\"/>";
									
									tableString += " </head>" + "</table>";
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_EvaluationLevel%>">
