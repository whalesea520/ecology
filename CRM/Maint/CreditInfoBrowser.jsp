...
String creditamount = Util.null2String(request.getParameter("creditamount"));
String sqlwhere = " ";
int ishead = 0;
if(!sqlwhere1.equals("")){
	if(ishead==0){
...
if(!creditamount.equals("")){
	if(ishead==0){
		ishead = 1;
		sqlwhere += " where creditamount like '%";
		sqlwhere += Util.fromScreen2(creditamount,user.getLanguage());
		sqlwhere += "%'";
	}
	else{
		sqlwhere += " and creditamount like '%";
		sqlwhere += Util.fromScreen2(creditamount,user.getLanguage());
		sqlwhere += "%'";
	}
}
...
<div class="zDialog_div_content">
	<FORM NAME=SearchForm  action="CreditInfoBrowser.jsp" method=post>
	...
	<wea:layout type="4col" attributes="{'expandAllGroup':'true'}">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
			...
			<wea:item><input class=InputStyle  name=creditamount value='<%=creditamount%>'></wea:item>
		</wea:group>	
			...
			<wea:item attributes="{'isTableList':'true'}">
				<wea:layout type="table" attributes="{'cols':'3','layoutTableId':'BrowseTable'}" needImportDefaultJsAndCss="false">
					<wea:group context="" attributes="{'groupDisplay':'none'}">
						...
						<%
							sqlwhere = "select * from CRM_CreditInfo "+sqlwhere+" order by id asc";
