...
{
	var value=$("input[name='customerDesc']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
//新建、编辑获得途径 add by Dracula @2014-1-28
function openDialog(id){
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	var url = "/CRM/Maint/AddCustomerDesc.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16494,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/Maint/EditCustomerDesc.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16494,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = true;
...
function openLogDialog(id)
{
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	var url = "/systeminfo/SysMaintenanceLog.jsp?operateitem="+108;
	if(id!=0)
		url+="&relatedid="+id;
...
{
	//if(isdel())
    //location.href="/CRM/Maint/CustomerDescOperation.jsp?method=delete&id="+id+"&name="+fullname;
     window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(16344,user.getLanguage())%>",function(){
            jQuery.post("/CRM/Maint/CustomerDescOperation.jsp?method=delete&id="+id+"&name="+fullname,function(msg){
                var isb=false;
...
function registerDragEvent(){
     ...
     jQuery("#customerDescTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "CustomerDescOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.post("/CRM/Maint/CustomerDescOperation.jsp",{"method":"delete","id":idArr[i]},function(){
					_table.reLoad()
				});
			}
		});
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0" id="customerDescTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String fromSql = " from crm_customerdesc  t1 ";
									...
									operateString +=" <popedom transmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMCustomerDescListOperation\"  otherpara=\""+popedomUserpara+"\" ></popedom> ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		...
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_DescSet+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_DescSet,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMCustomerDescResultCheckbox\" />"
