...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value='<%=SystemEnv.getHtmlLabelNames("462",user.getLanguage()) %>'/>
</jsp:include>
...
<div class="zDialog_div_content">
	<FORM NAME=SearchForm  action="ContacterTitleBrowser.jsp" method=post>
			...
			<wea:item attributes="{'isTableList':'true'}">
				<wea:layout type="table" attributes="{'cols':'3','layoutTableId':'BrowseTable'}" needImportDefaultJsAndCss="false">
					<wea:group context="" attributes="{'groupDisplay':'none'}">
						...
						<%
							sqlwhere = "select * from CRM_ContacterTitle "+sqlwhere+" order by id asc";
...
function BrowseTable_onclick(e){
  ...
  if( target.nodeName =="TD"||target.nodeName =="A"  ){
      var returnValue = {id:jQuery.trim(jQuery(jQuery(target).parents("tr")[0].cells[0]).text()),name:jQuery.trim(jQuery(jQuery(target).parents("tr")[0].cells[1]).text())};
		if(dialog){
			try{
