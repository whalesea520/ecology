...
{
	var value=$("input[name='creditInfo']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
function openDialog(id){
	...
	var url = "/CRM/Maint/AddCreditInfo.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(580,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/Maint/EditCreditInfo.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(580,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 240;
	dialog.Drag = true;
...
{
	if(isdel())
		location.href="/CRM/Maint/CreditInfoOperation.jsp?method=delete&id="+id+"&type="+fullname;
}
...
function registerDragEvent(){
     ...
     jQuery("#creditInfoTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "CreditInfoOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.ajax({
					url:"/CRM/Maint/CreditInfoOperation.jsp?method=delete&id="+idArr[i],
					type:"post",
					async:true,
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0"  id="creditInfoTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String tableString = "";
									String backfields = " t1.id, t1.fullname, t1.creditamount, t1.highamount, t1.orderkey ";
									String fromSql = " from CRM_CreditInfo  t1 ";
									String orderkey = " t1.orderkey ";
									String sqlWhere = " 1=1 ";
									String fullnamepara = "column:fullname";
									String highamountpara = "column:highamount";
									String popedomUserpara = String.valueOf(user.getUID());
									String checkpara = "column:id+"+popedomUserpara;
									String operateString = "";
									operateString = " <operates>";
									operateString +=" <popedom transmethod=\"weaver.crm.Maint.CRMTransMethod.getCreditInfoListOperation\"  otherpara=\""+popedomUserpara+"\" ></popedom> ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_CreditLevel+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_CreditLevel,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMCreditInfoResultCheckbox\" />"
									...
									+ "\" column=\"creditamount\" orderkey=\"t1.creditamount\" transmethod=\"weaver.crm.Maint.CRMTransMethod.getCreditHighAmount\" otherpara=\""+highamountpara+"\"/>";
									
									tableString += " </head>" + "</table>";
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_CreditLevel%>">
