...
//add by Dracula @2014-2-7
function searchTitle()
{
	var value=$("input[name='evaluation']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
//新建、编辑评估项目 add by Dracula @2014-2-7
function openDialog(id){
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	var url = "/CRM/Maint/AddEvaluation.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16497 ,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/Maint/EditEvaluation.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93 ,user.getLanguage()) %><%=SystemEnv.getHtmlLabelName(16497 ,user.getLanguage()) %>";
	}
	dialog.Width = 420;
	dialog.Height = 220;
	dialog.Drag = true;
...
//编辑 add by Dracula @2014-2-7
function doEdit(id)
{
	openDialog(id);
}
//删除 add by Dracula @2014-2-7
function doDel(id,fullname)
{
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
		location.href="/CRM/Maint/EvaluationOperation.jsp?method=delete&id="+id+"&name="+fullname;
	});
}
...
function registerDragEvent(){
     ...
     jQuery("#evaluationTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "EvaluationOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.post("/CRM/Maint/EvaluationOperation.jsp",{"method":"delete","id":idArr[i]},function(){
					_table.reLoad()
				});
			}
		});
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0" id="evaluationTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String tableString = "";
									String backfields = " t1.id, t1.name, t1.proportion, t1.orderkey ";
									String fromSql = " from crm_evaluation t1 ";
									...
									operateString +=" <popedom transmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMEvaluationLevelListOperation\"  otherpara=\""+popedomUserpara+"\" ></popedom> ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_EvaluationList+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_EvaluationList,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMEvaluationLevelResultCheckbox_l\" />"
									...
									+ otherpara + "\" />";
									tableString += " <col width=\"75%\"  text=\"" + SystemEnv.getHtmlLabelName(6071,user.getLanguage())
									+ "\" column=\"proportion\" orderkey=\"t1.proportion\" transmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMEvaluationWithProportion\"/>";
									
									tableString += " </head>" + "</table>";
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_EvaluationList%>">
