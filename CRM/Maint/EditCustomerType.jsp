...
function attachUrl()
{
	...
	$("[name='tabcontentframe']").attr("src","EditCustomerTypeInner.jsp?id="+id+"&winfo=<%=ishowname%>");
	if("<%=ishowname%>"==="2")
	{
	...
	$("a[target='tabcontentframe']").each(function(){
		var	url = "EditCustomerTypeInner.jsp?id="+id+"&winfo="+$(this).attr("winfo");
		$(this).attr("href",url);
	});
}
...
$(document).ready(function(){
	    $('.e8_box').Tabs({
			...
			staticOnLoad:true,
			objName:"<%=SystemEnv.getHtmlLabelName(16482,user.getLanguage())%>"
		});
});
		...
		<div class="e8_ultab">
			...
			<div>
				<ul class="tab_menu">
					...
					<li>
						<a href='#' winfo='2' target="tabcontentframe"><%=SystemEnv.getHtmlLabelName(2112,user.getLanguage()) %></a>
					</li>
				</ul>
