...
{
	var value=$("input[name='tradeInfo']",parent.document).val();
	if(value.length > 0)
	{
		//还原第二列的值
...
function openDialog(id){
	...
	var url = "/CRM/Maint/AddTradeInfo.jsp";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(6146,user.getLanguage()) %>";
	if(!!id){//编辑
		url = "/CRM/Maint/EditTradeInfo.jsp?id="+id;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(6146,user.getLanguage()) %>";
	}
	dialog.Width = 600;
	dialog.Height =550;
	dialog.Drag = true;
...
{
	if(isdel())
		location.href="/CRM/Maint/TradeInfoOperation.jsp?method=delete&id="+id+"&type="+fullname;
}
...
function registerDragEvent(){
     ...
     jQuery("#tradeInfoTable").find("table.ListStyle tbody").sortable({                //这里是talbe tbody，绑定 了sortable  
    ...
    function sortOrderTitle()
    {
    	...
    	$.ajax({ 
	        type: "post",
	        url: "TradeInfoOperation.jsp?method=sort&ids="+_tableids,
	        async:true,
    ...
    function delMutli()
    {
		...
		window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
			...
			for(var i=0;i<idArr.length;i++){
				jQuery.ajax({
					url:"/CRM/Maint/TradeInfoOperation.jsp?method=delete&id="+idArr[i],
					type:"post",
					async:true,
...
String titlename = SystemEnv.getHtmlLabelName(320,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(6146,user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
<BODY>
...
<table width=100% height=100% border="0" cellspacing="0" cellpadding="0"  id="tradeInfoTable">
	<tr>
		<td valign="top">
			<TABLE class=Shadow>
				<tr>
						...
						<TABLE width="100%" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%
									...
									String tableString = "";
									String backfields = " t1.id, t1.fullname, t1.rangelower, t1.rangeupper, t1.orderkey ";
									String fromSql = " from CRM_TradeInfo  t1 ";
									String orderkey = " t1.orderkey ";
									String sqlWhere = " 1=1 ";
									String fullnamepara = "column:fullname";
									String rangeupperpara = "column:rangeupper";
									String popedomUserpara = String.valueOf(user.getUID());
									String checkpara = "column:id+"+popedomUserpara;
									String operateString = "";
									operateString = " <operates>";
									operateString +=" <popedom transmethod=\"weaver.crm.Maint.CRMTransMethod.getTradeInfoListOperation\"  otherpara=\""+popedomUserpara+"\" ></popedom> ";
									...
									operateString +="     <operate href=\"javascript:doDel();\" text=\"" + SystemEnv.getHtmlLabelName(23777, user.getLanguage()) + "\" otherpara=\""+fullnamepara+"\" index=\"1\"/>";
					 	       		operateString +=" </operates>";
									tableString = " <table instanceid=\"MaintContacterTitleListTable\"  tabletype=\"checkbox\" pageId=\""+PageIdConst.CRM_TradeLevel+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_TradeLevel,user.getUID(),PageIdConst.CRM)+"\" >"
									+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\""+checkpara+"\" showmethod=\"weaver.crm.Maint.CRMTransMethod.getCRMTradeInfoResultCheckbox\" />"
									...
									tableString += " <head>"; 
									tableString += " <col width=\"25%\"  text=\"" + SystemEnv.getHtmlLabelName(534,user.getLanguage())+SystemEnv.getHtmlLabelName(139,user.getLanguage())
									...
									+ otherpara + "\" />";
									tableString += " <col width=\"75%\"  text=\"" + SystemEnv.getHtmlLabelName(534, user.getLanguage())
									+ "\" column=\"rangelower\" orderkey=\"t1.rangelower\" transmethod=\"weaver.crm.Maint.CRMTransMethod.getTradeHighAmount\" otherpara=\""+rangeupperpara+"\"/>";
									
									tableString += " </head>" + "</table>";
								%>
									<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_TradeLevel%>">
