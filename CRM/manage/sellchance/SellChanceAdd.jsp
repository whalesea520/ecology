...
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
									...
									<td>
										...
										<table class="item_table" cellpadding="0" cellspacing="0" border="0">
											...
											<tr>
												...
												<td class="title" id="td_fileids">启动项目原因及关键需求</td>
												<td>
													<%if(hasPath){ %>
											...
											<tr>
												...
												<td class="title" rowspan="2">成功关键因素(风险)</td>
												<td rowspan="2"><textarea name="remark" class="item_input" style="height: 50px"></textarea></td>
											</tr>
											<tr>
												...
												<td>
													...
													%>
														<option value="<%=ProductTypeComInfo.getId() %>"><%=ProductTypeComInfo.getName() %></option>
													<%	} %>
													</select>
												</td>
											</tr>
											<tr>
												...
												<td>
													<select id="selltype" name="selltype" onchange="changeType(this.value)">
														<option value="1">终端客户商机</option>
														<option value="2">老客户二次商机</option>
														<option value="3">代理压货商机</option>
													</select>
												</td>
												<td class="title td_ploytype" >打单策略类型</td>
												<td class="td_ploytype">
													<div class="btn_browser" onclick="onShowPloyType('ploytype','ploytypeSpan')"></div>
													<div class="txt_browser" id="ploytypeSpan"></div>
													<input type="hidden" id="ploytype" name="ploytype" value=""/>
												</td>
											</tr>
											<tr>
												...
												<td class="title" rowspan="2" id="td_ploydesc">打单策略描述</td>
												<td rowspan="2"><textarea name="ploydesc" class="item_input" style="height: 50px"></textarea></td>
											</tr>
											<tr>
												<td class="title">复盘文件</td>
												<td></td>
											</tr>
										</table>
									</td>
		...
		<script type="text/javascript">
			...
			function changeType(val){
				...
				if(val==3){
					$("#td_fileids").html("启动代理的原因");
					$("#td_ploydesc").html("代理成交的关键策略").attr("title","代理成交的关键策略");
				}else{
					$("#td_fileids").html("启动项目原因及关键需求");
					$("#td_ploydesc").html("打单策略描述").attr("title","打单策略描述");
				}
			}
			function doSave(obj){
			...
			function onShowCRM() {
			    var datas = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/CRM/manage/util/CustomerBrowser.jsp");
			    if (datas) {
				    if(datas.id!=""){
			...
			function onShowPloyType(inputname,spanname) {
				var ids = $("#"+inputname).val();
			    var datas = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/CRM/manage/util/PloyTypeMutiBrowser.jsp?ids="+ids);
			    if (datas) {
				    if(datas.id!=""){
				    	$("#"+inputname).val(datas.id.substring(1));
				    	$("#"+spanname).html(datas.name.substring(1));
					}else{
						$("#"+inputname).val("");
						$("#"+spanname).html("");
					}
