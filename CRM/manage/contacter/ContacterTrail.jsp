	...
	<div class="traildiv">
		...
		<%
		  ...
		  String sql="SELECT id,customerid,firstname,title,phoneoffice,mobilephone,email,jobtitle,department,imcode FROM CRM_CustomerContacter where firstname='"+firstname+"'"
		             +" and (email='"+email+"' or mobilephone='"+mobilephone+"' or imcode='"+imcode+"')";
		  rs.execute(sql);
		...
		<div style="position: relative;height:180px;">
			...
			<div class="year" style="display:none;">
				<span style="margin-left:5px;">2014年</span>
			</div>
								...
								<%
								 ...
								 %>
								 	<div style="height:30px;line-height:30px;">
										<div style="float: left;cursor:pointer;" onclick="pointerXY(event);openhrm('<%=resourceid%>');return false;"><span style="color:<%=color%>"><%=ResourceComInfo.getLastname(resourceid)%></span>&nbsp;<span style="color:#939393">于</span></div>
										<div style="float: left;margin-left:35px;"><span style="color:#939393"><%=lastdate%>&nbsp;最后联系</div>
										<div style="clear:both;"></div>
									</div>
								<%}
								    if(rs2.getCounts()==0){
								%>
									<div style="color:#939393">无联系记录</div>
								<%}%>
							</div>
