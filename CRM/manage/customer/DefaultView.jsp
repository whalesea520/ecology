...
<%
	...
	if(isself.equals("2")){//查询风险预警
		...
		//此处将最近一个月的联系记录插入到临时表中，提高查询性能
		rs.executeSql("select p.id,p.description,p.begindate,p.begintime,p.createrid,p.docid,p.requestid,p.projectid,p.createdate,p.createtime,p.relateddoc,p.crmid,p.sellchanceid,p.contacterid"
	...
	if (rs.next()) {
		condition = "CRM_CustomerInfo as t";
		crmadmin = true;
	} else {
		String leftjointable = CrmShareBase.getTempTable(userid);
		condition = "(select distinct t1.id,t1.name,t1.manager,t1.type,t1.status,t1.createdate "
	...
	String fromsql = " from (select p.id,p.description,p.begindate,p.begintime,p.createrid,p.docid,p.requestid,p.projectid,p.createdate,p.createtime,p.relateddoc,p.crmid,p.sellchanceid,p.contacterid "
			+"from WorkPlan p where p.createrType = '1' and p.type_n = 3) w,"+condition;
	if(isself.equals("2")) fromsql = " from "+temptable+" w,"+condition;
	String backfield = " w.id,w.description,w.begindate,w.begintime,w.createrid,w.docid,w.requestid,w.projectid,w.createdate,w.createtime,w.relateddoc,w.crmid as customerid,w.sellchanceid,w.contacterid";

	String sqlwhere = " where w.crmid=convert(varchar,t.id)";	
	String hrmwhere1 = "";
	String hrmwhere2 = "";
	if(!"0".equals(creatertype) && !"1".equals(attention)){//非全部并且非关注
		fromsql += ",hrmresource hrm";
		sqlwhere += " and hrm.id=t.manager";
		if("-1".equals(creatertype)){//非本人
			hrmwhere1 = " and t.manager > "+creater+" and hrm.managerstr not like '%,"+creater+",%'";
			hrmwhere2 = " and t.manager < "+creater+" and hrm.managerstr not like '%,"+creater+",%'";
		}else{
			hrmwhere1 += " and t.manager ="+creater;
			hrmwhere2 += " and hrm.managerstr like '%,"+creater+",%'";
		}
	}else{
		hrmwhere1 = " and t.manager>0";
		hrmwhere2 = " and t.manager<0";
	}
	...
	if(!"".equals(nocontact)){
		sqlwhere += " and not exists (select 1 from WorkPlan t3 where t3.type_n=3 and t3.createrType = '1' and t3.crmid=convert(varchar,t.id) and t3.begindate>='"+nocontact+"'";
		if(contacttype.equals("1")){
			sqlwhere += " and t3.createrid=t.manager ";
		}
	...
	if(crmadmin) sqlwhere += " and (t.deleted=0 or t.deleted is null)";
	...
	if(isself.equals("2") && _total==0) rs.executeSql("drop table "+temptable);
	...
	<div id="righttitle" style="width: 100%;height: 30px;position: relative;
		...
		<div style="position: absolute;top: 3px;left:0px;height: 23px;width: 100%;">
			<div style="margin-left: 5px;margin-top: 3px;width: auto;text-align: left;font-family: 微软雅黑  !important;float: left;">
				<font title="所有人创建的联系反馈" style="font-family: 微软雅黑  !important;cursor:pointer;margin-left:10px;margin-right:3px;color:#B7B7B7;font-weight: bold;<%if(isself.equals("")){ %>color:#4F4F4F;<%} %>" onclick="loadDefault('<%=mtitle %>','')">全部反馈</font>|
				<font title="客户经理创建的联系反馈" style="font-family: 微软雅黑 !important;cursor:pointer;margin-left:3px;margin-right:3px;color:#B7B7B7;font-weight: bold;<%if(isself.equals("1")){ %>color:#4F4F4F;<%} %>" onclick="loadDefault('<%=mtitle %>','1')">本人反馈</font>|
				<font title="非客户经理创建的联系反馈" style="font-family: 微软雅黑  !important;cursor:pointer;margin-left:3px;margin-right:3px;color:#B7B7B7;font-weight: bold;<%if(isself.equals("0")){ %>color:#4F4F4F;<%} %>" onclick="loadDefault('<%=mtitle %>','0')">他人指导
				</font><%if(iswarn){ %>|
				<font title="近十天的联系记录风险预警" style="font-family: 微软雅黑  !important;cursor:pointer;margin-left:3px;margin-right:3px;color:#B7B7B7;font-weight: bold;<%if(isself.equals("2")){ %>color:#4F4F4F;<%} %>" onclick="loadDefault('<%=mtitle %>','2')">风险预警</font>
				<%} %>
			</div>
...
<script type="text/javascript">
	...
	function getListContact(obj){
		...
		$.ajax({
		    ...
		    data:{"operation":"get_list_contact","showtype":1,"showwarn":"<%=isself%>","keytype":"<%=keytype%>","fromcustomer":1,"currentpage":_currentpage,"pagesize":_pagesize,"total":_total,"querysql":filter(encodeURI(_querysql))}, 
