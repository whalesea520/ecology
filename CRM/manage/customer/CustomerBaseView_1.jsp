				...
				<td align="center" valign="top">
					<div style="width: 98%;height: 100%;margin: 0px auto;text-align: left;">
						<div style="width: auto;height: 28px;border: 1px #CCCCCC solid;margin-top: 4px;background: url('../images/title_bg_wev8.gif') repeat-x;color:#666666;">
							<div class="tabmenu tabmenu_click" style="margin-left: 10px;" _frameid="base">基本信息：</div>
							<div style="line-height: 28px;margin-left: 0px;margin-right: 20px;float: left;"><%=name %></div>
							<!--
							<div class="tabmenu"
								_frameid="totalframe" _url="/CRM/data/ViewCustomerTotal.jsp?isrequest=<%=isrequest %>&CustomerID=<%=customerid %>"><%=SystemEnv.getHtmlLabelName(352,user.getLanguage())+SystemEnv.getHtmlLabelName(351,user.getLanguage()) %></div>
							<div class="tabmenu"
								_frameid="sellchanceframe" _url="/CRM/sellchance/ListSellChance.jsp?isfromtab=true&CustomerID=<%=customerid %>">销售商机</div>
							<%if(canedit){ %>
							<div class="tabmenu"
								_frameid="evaluationframe" _url="/CRM/data/GetEvaluation.jsp?isfromtab=true&CustomerID=<%=customerid %>">价值评估</div>
							<div class="tabmenu"
								_frameid="remindframe" _url="/CRM/data/AddContacterLogRemind.jsp?isfromtab=true&CustomerID=<%=customerid %>">联系提醒</div>
							<%} %>
							<%if (manager.equals(userid)) {%>
							<div class="tabmenu"
								_frameid="uniteframe" _url="/CRM/data/UniteCustomer.jsp?isfromtab=true&CustomerID=<%=customerid %>">客户合并</div>
							<%}%>
							<div class="tabmenu"
								_frameid="exchangeframe" _url="/discuss/ViewExchange.jsp?isfromtab=true&types=CC&sortid=<%=customerid %>">外部交流</div>
							<div id="csmenu" class="csmenu"
								_frameid="" _url="">客服管理</div>
							<div class="tabmenu"
								_frameid="csframe7" _url="/customerservice/relateinfo/IncrementInfo.jsp?customerId=<%=customerid %>">增值购买</div>
							<div class="tabmenu"
								_frameid="csframe10" _url="/CRM/manage/customer/SignName.jsp?customerId=<%=customerid %>">签约名称</div>
							 -->	
							<div style="float: right;margin-right: 5px;">
								<!-- 
								<div style="line-height: 28px;margin-left: 40px;float: left;">由 <%=cmutil.getHrm(Creater) %> 于 <font title="<%=CreateTime %>"><%=CreateDate %></font> 创建</div>
								<div id="lastupdate" style="line-height: 28px;margin-left: 20px;float: left;">由 <%=cmutil.getHrm(Modifier) %> 于 <font title="<%=ModifyTime %>"><%=ModifyDate %></font> 最后修改</div>
								 -->
								<div class="btn_operate" style="" onclick="showRelateWF('');">相关流程</div>
								<div class="btn_operate" style="" onclick="showShare('');">共享设置</div>
								<%if(canedit){ %>
								<div class="btn_operate" style="" onclick="showLog('');">日志明细</div>
								<%} %>
								<%if(isattention){%>
									<div id="btnatt_" class="btn_operate btn_att" style="" title="取消关注" _special="0" _customerid="<%=customerid%>">取消关注</div>
								<%}else{ %>
									<div id="btnatt_" class="btn_operate btn_att" style="" title="标记关注" _special="1" _customerid="<%=customerid%>">标记关注</div>
								<%} %>
								<div class="btn_operate" onclick="doRemind(1)" style="" title="<%
//查找提醒
							...
							<table style="width: 100%;height:auto;margin-top: 0px;" cellpadding="0" cellspacing="0" border="0">
								...
								<tr>
									<td valign="top">
										...
										<!-- 一般信息开始 -->
											<td valign="top">
											<div class="item_title item_title1">一般</div>
										...
										<!-- 一般信息结束 -->
											<td valign="top">
										<!-- 分类信息开始 -->
											<div class="item_title item_title1">分类信息</div>
											<div class="item_line item_line1"></div>
											<table class="item_table" cellpadding="0" cellspacing="0" border="0">
												<colgroup><col width="125px"/><col width="*"/></colgroup>
												<tr>
												...
												<tr>
													...
													<td class="data">
														<!-- 
														<input type="hidden" id="description_val" value="<%=description %>"/>
														<div class="txtlink showcon txtlink<%=description %>" onmouseover="showdel(this)" onmouseout="hidedel(this)">
															<%if(!description.equals("0") && !description.equals("")){ %>
															<div style="float: left;"><%=CustomerDescComInfo.getCustomerDescname(description) %></div>
															<%} %>
														</div>
														<%if(canedit){ %>
												  		<input id="description" name="description" class="add_input2" />
												  		<div class="btn_browser" onclick="onShowBrowser('description','/CRM/Maint/CustomerDescBrowser.jsp')"></div>
												  		<%} %>
												  		 -->
												...
												<tr>
													<td class="title">规模</td>
													<td class="data">
														<!-- 
														<input type="hidden" id="size_n_val" value="<%=size_n %>"/>
														<div class="txtlink showcon txtlink<%=size_n %>" onmouseover="showdel(this)" onmouseout="hidedel(this)">
															<%if(!size_n.equals("0") && !size_n.equals("")){ %>
															<div style="float: left;"><%=CustomerSizeComInfo.getCustomerSizedesc(size_n) %></div>
															<%} %>
														</div>
														<%if(canedit){ %>
												  		<input id="size_n" name="size_n" class="add_input2" />
												  		<div class="btn_browser" onclick="onShowBrowser('size_n','/CRM/Maint/CustomerSizeBrowser.jsp')"></div>
												  		<%} %>
												  		-->
												...
												<%} %>
												<tr>
													<td class="title">获得途径</td>
													<td class="data" id="sourcetd">
														<%
										...
										<div id="moreinfo" style="width: 100%;overflow: hidden;height: 0px;">
											...
											<tr>
											<td valign="top">
												<div class="item_title item_title1"><%=SystemEnv.getHtmlLabelName(15125,user.getLanguage())%></div><!-- 账务 -->
												<div class="item_line item_line1"></div>
												<table class="item_table" cellpadding="0" cellspacing="0" border="0">
													<colgroup><col width="125px"/><col width="*"/></colgroup>
													<tr>
														<td class="title"><%=SystemEnv.getHtmlLabelName(6097,user.getLanguage())%></td><!-- 信用额度 -->
														<td class="data">
															<%if(canedit){ %>
													...
													<tr>
														<td class="title"><%=SystemEnv.getHtmlLabelName(6098,user.getLanguage())%></td><!-- 信用期间 -->
														<td class="data">
															<%if(canedit){ %>
													...
													<tr>
														<td class="title"><%=SystemEnv.getHtmlLabelName(17084,user.getLanguage())%></td><!-- 开户银行 -->
														<td class="data">
															<%if(canedit){ %>
													...
													<tr>
														<td class="title"><%=SystemEnv.getHtmlLabelName(571,user.getLanguage())%></td><!-- 帐户 -->
														<td class="data">
															<%if(canedit){ %>
													...
													<tr>
														<td class="title"><%=SystemEnv.getHtmlLabelName(17085,user.getLanguage())%></td><!-- 银行帐号 -->
														<td class="data">
															<%if(canedit){ %>
											...
											<td valign="top">
												<div class="item_title item_title1"><%=SystemEnv.getHtmlLabelName(570,user.getLanguage())%></div><!-- 其他信息 -->
												<div class="item_line item_line1"></div>
												<table class="item_table" cellpadding="0" cellspacing="0" border="0">
													<colgroup><col width="125px"/><col width="*"/></colgroup>
													<%
