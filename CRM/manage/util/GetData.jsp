...
if(!key_word.equals("")){
	...
	if(searchtype.equals("hrm")){
	...
	}else if(searchtype.equals("subhrm")){//下属
		sql.append("select top "+rows+" t1.id,t1.lastname,isnull((select top 1 1 from HrmResource t2 where t2.managerid=t1.id),0) as ismanager from HrmResource t1 where t1.managerstr like '%,"+userid+",%' and (t1.status =0 or t1.status = 1 or t1.status = 2 or t1.status = 3) and ("+sqlWhere.replaceAll("####","t1.lastname")+" or "+sqlWhere.replaceAll("####","t1.pinyinlastname")+") order by t1.dsporder");
		...
		if (rs.next()) {
			sql.append("select top "+rows+" id,name,createdate from CRM_CustomerInfo where (deleted=0 or deleted is null) and "+sqlWhere.replaceAll("####","name")+" order by createdate desc,id desc");
		} else {
		...
		rs.executeSql("select id from HrmRoleMembers where  roleid = 8 and rolelevel = 2 and resourceid = " + userid);
		if (rs.next()) {
			sql.append("select top "+rows+" t1.id,t1.name,t1.createdate from CRM_CustomerInfo t1 where (t1.deleted=0 or t1.deleted is null) and "+sqlWhere.replaceAll("####","t1.name"));
		} else {
			String leftjointable = CrmShareBase.getTempTable(userid);
			sql.append("select distinct top "+rows+" t1.id,t1.name,t1.createdate "
				+ " from CRM_CustomerInfo t1 left join " + leftjointable + " t2 on t1.id = t2.relateditemid "
		...
		sql.append(" and t1.type in (3,4,11,12,13,14,15,16,17,18,20,21,25,19) order by t1.createdate desc,t1.id desc");
		...
		sql.append("select A.id,A.subject,A.sellstatusid,A.customerid,A.createdate,A.endtatusid"
				...
				+ " union all"
				+ " select B.id,B.name,B.status,B.ismanager,'',0 from "
				+ " (select top "+rows+" t1.id,t1.lastname as name,-1 as status,isnull((select top 1 1 from HrmResource t2 where t2.managerid=t1.id),0) as ismanager from HrmResource t1 where (t1.status =0 or t1.status = 1 or t1.status = 2 or t1.status = 3) "
				+ " and ("+sqlWhere.replaceAll("####","t1.lastname")+" or "+sqlWhere.replaceAll("####","t1.pinyinlastname")+") order by t1.dsporder) B"
				);
	}else if(searchtype.equals("search1")){//客户伙伴管理查询
		String condition = "";
		String searchwhere = " ("+sqlWhere.replaceAll("####","t1.name")+" or exists(select 1 from CRM_CustomerTag ct where ct.customerid=t1.id and ct.tag like '%"+key_word+"%'))";
		rs.executeSql("select id from HrmRoleMembers where  roleid = 8 and rolelevel = 2 and resourceid = " + userid);
		if (rs.next()) {
			condition = "select top "+rows+" t1.id,name,t1.status,t1.createdate from CRM_CustomerInfo t1 where (t1.deleted=0 or t1.deleted is null) and "+searchwhere;
		} else {
			String leftjointable = CrmShareBase.getTempTable(userid);
			condition = "select distinct top "+rows+" t1.id,t1.name,t1.status,t1.createdate "
				+ " from CRM_CustomerInfo t1 left join " + leftjointable + " t2 on t1.id = t2.relateditemid "
				+ " where t1.id = t2.relateditemid and (t1.deleted=0 or t1.deleted is null) and "+searchwhere;
		}
		int maintype = Util.getIntValue((String)request.getSession().getAttribute("CRM_MAINTYPE"),1);
		if(maintype==1){
			condition += " and t1.type in (1,3,4,5)";
		}else if(maintype==2){
			condition += " and t1.type in (11,12,13,14,15,16,17,18,20,21,25,19)";
		}else if(maintype==3){
			condition += " and (t1.type = 26 or exists(select 1 from CRM_CustomerContacter cc where cc.customerid=t1.id and cc.isperson=1))";
		}
		sql.append("select A.id,A.name,A.status,0"
				+ " from ("+condition+" order by t1.createdate desc,t1.id desc) A" 
				+ " union all"
				+ " select B.id,B.name,B.status,B.ismanager from "
				+ " (select top "+rows+" t1.id,t1.lastname as name,-1 as status,isnull((select top 1 1 from HrmResource t2 where t2.managerid=t1.id),0) as ismanager from HrmResource t1 where (t1.status =0 or t1.status = 1 or t1.status = 2 or t1.status = 3) "
				+ " and ("+sqlWhere.replaceAll("####","t1.lastname")+" or "+sqlWhere.replaceAll("####","t1.pinyinlastname")+") order by t1.dsporder) B"
				);
		...
		while(rs.next()){
			...
			if(searchtype.equals("search")){
				...
				if("-1".equals(rs.getString(3))){
					json += ",status:'"+SubCompanyComInfo.getSubCompanyname(ResourceComInfo.getSubCompanyID(rs.getString(1)))+"',type:2,ismanager:"+rs.getString(4);
				}else{
			...
			}else if(searchtype.equals("search1")){
				if("-1".equals(rs.getString(3))){
					json += ",status:'"+SubCompanyComInfo.getSubCompanyname(ResourceComInfo.getSubCompanyID(rs.getString(1)))+"',type:2,ismanager:"+rs.getString(4);
				}else{
					json += ",status:'"+CustomerStatusComInfo.getCustomerStatusname(rs.getString("status"))+"',type:1,crmid:"+rs.getString("id")+",lastdate:'"+rs.getString("createdate")+"',crmname:'"+rs.getString("name")+"'";
				}
			}else if(searchtype.equals("subhrm")){
				json += ",ismanager:"+rs.getString(3);
			}
			json += "},";
		}
	}
