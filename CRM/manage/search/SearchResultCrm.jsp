...
String sqlWhere = "";
String orderBy = "";
String sqlvalue=CRMSearchComInfo.FormatSQLSearch(user.getLanguage());
String notmanager=(String)session.getAttribute("serachrmmanager");
//out.print(sqlvalue);
if (notmanager.equals("1"))
{
String accountmanager = CRMSearchComInfo.getAccountManager();
if (!accountmanager.equals(""))
{

sqlvalue=Util.StringReplace(sqlvalue,"t1.manager = "+accountmanager,"t1.manager not in ("+accountmanager+") ");
...
if(loginType.equals("1")){
	if (RecordSet.getDBType().equals("oracle")){
		...
		sqlWhere = sqlvalue+" and t1.id = t2.relateditemid";
	} else {
	...
	sqlWhere = sqlvalue+" and t1.agent="+userid;
	orderBy = "t1.id";
}
...
<%
RCMenu += "{处理,javascript:shareNext(),_top} " ;
...
<tr>
	...
	<td valign="top">
		<TABLE class=Shadow>
		<form name=crmshare action="" method="post" onsubmit="return false">
...
function shareNext(){
	...
	document.crmshare.customerids.value = _xtable_CheckedCheckboxId();
    crmshare.action="ShareTo.jsp";
    crmshare.submit();
}
