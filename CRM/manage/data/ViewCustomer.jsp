...
System.out.println("sharelevel=="+sharelevel);
if(sharelevel>0){
     canview=true;
     canviewlog=true;
     canmailmerge=true;
...
if(!Creater.equals(""))	{
	...
	temStr+=SystemEnv.getHtmlLabelName(623,user.getLanguage())+":"+ResourceComInfo.getClientDetailModifier(Creater,CreaterType,user.getLogintype())+" "+SystemEnv.getHtmlLabelName(401,user.getLanguage())+":";
	temStr+=CreateDate+"； ";
}
if(!Modifier.equals("")){
	...
	temStr+=SystemEnv.getHtmlLabelName(623,user.getLanguage())+":"+ResourceComInfo.getClientDetailModifier(Modifier,ModifierType,user.getLogintype())+" "+SystemEnv.getHtmlLabelName(103,user.getLanguage())+":";
	temStr+=ModifyDate+"； ";
}
...
//渠道信息begin*****************
String typeName = CustomerTypeComInfo.getCustomerTypename(CustomerInfoComInfo.getCustomerInfotype(CustomerID));
if(!"".equals(typeName)){
	rs.executeSql("select 1 from CRM_CustomerChannelMark where mark like '%{"+typeName+"}%'");
	if(rs.next()){
		arrOtherTab+=",{tid:'t26820',title:'"+SystemEnv.getHtmlLabelName(26820,user.getLanguage())+" ',url:'/CRM/channel/data/CustomerChannelInfoView.jsp?CustomerID="+CustomerID+"'}";
	}
}
//渠道信息end*****************
...
if(!user.getLogintype().equals("2")){
	//客服信息
	arrOtherTab+=",{tid:'t25129',title:'"+SystemEnv.getHtmlLabelName(25129,user.getLanguage())+"',url:'/customerservice/data/CustomerServiceAllInfo.jsp?CustomerID="+CustomerID+"'}";
	//客户情况调查
	arrOtherTab+=",{tid:'t25220',title:'"+SystemEnv.getHtmlLabelName(25220,user.getLanguage())+"',url:'/customerservice/data/InvestigateInit.jsp?CustomerID="+CustomerID+"'}";
	//已售&可售项目
	arrOtherTab+=",{tid:'t25274',title:'"+SystemEnv.getHtmlLabelName(25274,user.getLanguage())+"',url:'/customerservice/data/SellItemView.jsp?CustomerID="+CustomerID+"'}";
	//相关流程
	arrOtherTab+=",{tid:'t1044',title:'"+SystemEnv.getHtmlLabelName(1044,user.getLanguage())+"',url:'/customerservice/flowrelate/FlowDataAll.jsp?CustomerID="+CustomerID+"'}";
	//相关单位
	arrOtherTab+=",{tid:'t1329',title:'"+SystemEnv.getHtmlLabelName(522, user.getLanguage())
		+SystemEnv.getHtmlLabelName(1329, user.getLanguage())
		+"',url:'/customerservice/relateinfo/CustomerUnitInfo.jsp?customerId="+CustomerID+"'}";
	//升级记录
	arrOtherTab+=",{tid:'t17530',title:'"+SystemEnv.getHtmlLabelName(17530,user.getLanguage())+SystemEnv.getHtmlLabelName(264,user.getLanguage())
		+"',url:'/customersystem/data/CustomerUpdateRecordListSingle.jsp?customerId="+CustomerID+"'}";
	//增值购买
	arrOtherTab+=",{tid:'t-1',title:'增值购买',url:'/customerservice/relateinfo/IncrementInfo.jsp?customerId="+CustomerID+"'}";
	//维护升级
	arrOtherTab+=",{tid:'t-2',title:'维护升级',url:'/customerservice/relateinfo/ExpireUpdateInfo.jsp?customerId="+CustomerID+"'}";
}
...
<script language=javascript>
	...
	for(var item=0;item<arrOtherTab.length;item++){
		if(item==0){
			...
			iframeList+=" <iframe src='/CRM/data/ViewCustomerBase.jsp?isrequest=<%=isrequest%>&CustomerID=<%=CustomerID %>'  id='iframepage'  frameBorder=0 scrolling=auto width=100% onload='iFrameHeight(\"iframepage\")'  style='display:block;'></iframe>";
			...
			<table cellpadding="0" cellspacing="0" width="100%" border="0">
			  	<tr>
			  		<td width="6px" height="28px;" style="">
						<div id="tab-left" class="tab-left-selected" style=""></div>
					</td>
					<td>
						<div id="tab-center" >
							<ul>
...
$(function(){
	...
	function initMenuWidth(){
		...
		$("#tab-center ul").css("width",tabWidth+60);
	}
	/**
	function setTabPanelActive(sid){
		var objs = document.all("tab-center").getElementsByTagName("li");
