	...
	$(document).ready(function(){
	    $('.e8_box').Tabs({
			...
			mouldID:"<%= MouldIDConst.getID("customer")%>",
	   		staticOnLoad:true,
	   		objName:"<%=SystemEnv.getHtmlLabelName(16407,user.getLanguage())%>"
		});
		attachUrl();
  ...
  function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		...
		if("list" == datetype){
            ...
            var url = "/CRM/search/SearchResultCondition.jsp?selectType=search&CustomerName=<%=customerName%>&settype=<%=settype%>&cityIds=<%=cityId%>&provinceIds=<%=provinceId%>";
			var type = '<%=type%>';
			if('quick'==type){
                url = "/CRM/search/SearchOperation.jsp?selectType=search&CustomerName=<%=customerName%>&settype=<%=settype%>&cityIds=<%=cityId%>&provinceIds=<%=provinceId%>";
			}
