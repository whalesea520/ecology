	...
	$(document).ready(function(){
	    $('.e8_box').Tabs({
			...
			mouldID:"<%= MouldIDConst.getID("customer")%>",
	   		staticOnLoad:true,
	   		objName:"<%=ResourceComInfo.getLastname(searchHrmId)%><%=SystemEnv.getHtmlLabelNames("84383,84236",user.getLanguage())%>"
		});
		attachUrl();
	});
  ...
  function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		...
		if("list" == datetype){
			...
			var url = "/CRM/search/SearchOperation.jsp?selectType=hrmView&searchtype=hrmView&searchHrmId=<%=searchHrmId%>&CustomerName=<%=customerName%>";
