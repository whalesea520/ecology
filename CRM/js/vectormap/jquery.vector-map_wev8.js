...
(function( $ ){
    ...
    var WorldMap = function(params) {
		...
		var dataStatus = params.dataStatus;
		var map = this;
		...
		this.width = params.container.width()+60;
		this.height = params.container.height()+80;
		...
		$(window).resize(function(){
			map.width = params.container.width()+60;
			map.height = params.container.height()+80;
			map.resize();
		...
		$('<div/>').addClass('jvectormap-zoomin').attr('title','放大').appendTo(params.container);
		$('<div/>').addClass('jvectormap-zoomout').attr('title','缩小').appendTo(params.container);
		...
		$(params.container).delegate(this.canvas.mode == 'svg' ? 'path' : 'shape', 'mouseover mouseout', function(e){
			...
			$.each(dataStatus, function (i, items) {
				if (code == items.cha && items.value>0) {
					...
					if (e.type == 'mouseover') {
						...
						if (!regionMouseOverEvent.isDefaultPrevented()) {
							...
							if (params.hoverColor && clickpath!=path) {
						...
						if (!labelShowEvent.isDefaultPrevented()) {
							map.label.css({
								left: e.pageX-10-map.label.width(),
								top: e.pageY-10-map.label.height()
							}).show();
							//map.label.show();
						...
						path.setOpacity(1);
						if (path.currentFillColor && clickpath!=path) {
		...
		$(params.container).delegate(this.canvas.mode == 'svg' ? 'path' : 'shape', 'click', function(e){
			...
			$.each(dataStatus, function (i, items) {
				if (code == items.cha && items.value>0) {
					if(clickpath){
						clickpath.setFill(clickpath.currentFillColor);
					}
					if (params.clickColor){
						path.setFill(params.clickColor);//设置点击颜色
					}
					clickpath = path;       
		...
		params.container.mousemove(function(e){
			if (map.label.is(':visible')) {
				...
				map.label.css({
					...
					top: e.pageY-10-map.labelHeight
				})
			}
		});
            ...
            if (typeof key == 'string') {
                ...
                for (var code in colors) {
					if (this.countries[code]) {
						...
						this.countries[code].style.cursor='pointer';
					}
                }
            }
        },
        ...
        bindZoomButtons: function() {
            ...
            this.container.find('.jvectormap-zoomout').click(function(){
                if (map.zoomCurStep > 1) {
                    ...
                    if(map.zoomCurStep==1) $('#map').css('cursor','default');
                }
            });
        },
