...
defaultOptions = {
	...
	global: {
		useUTC: true,
		canvasToolsURL: 'http://code.highcharts.com/2.2.5/modules/canvas-tools_wev8.js'
	},
	chart: {
		//animation: true,
		//alignTicks: false,
...
SVGRenderer.prototype = {
	...
	html: function (str, x, y) {
		...
		wrapper_wev8.css = wrapper.htmlCss;
	...
	color: function (color, elem, prop) {
		...
		if (fillType) {
			...
			// Radial (circular) gradient
				...
				//}
				fillAttr = 'src="http://code.highcharts.com/gfx/radial-gradient_wev8.png" ' +
					'size="' + size + ',' + size + '" ' +
					'origin="0.5,0.5" ' +
					'position="' + cx + ',' + cy + '" ' +
					'color2="' + color2 + '" ';
...
if (useCanVG) {
	...
	/**
	 * Start with an empty symbols object. This is needed when exporting is used (exporting.src_wev8.js will add a few symbols), but 
...
MouseTracker.prototype = {
	...
	setDOMEvents: function () {
		...
		container.onmouseover=container.onmouseout = function (e) {//------------------修改----------------------------
			...
			if (!chart.cancelClick) {
				...
				if (hoverPoint && (attr(e.target, 'isTracker') || attr(e.target.parentNode, 'isTracker'))) {
					...
					if(e.type=="mouseover"){
						showLight(hoverPoint.name);
					}
					//alert(hoverPoint.code);
				} else {
