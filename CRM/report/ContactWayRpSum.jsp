...
<%
	...
	String backfields = "t1.source ,COUNT(distinct t1.id) AS resultcount";
	String sqlWhere = null;
	String groupby = "t1.source";
	...
	if(!"".equals(searchName)){
		fromSql += ", CRM_ContactWay t3";
		sqlWhere += " and t3.id = t1.source and t3.fullname like '%"+searchName+"%'";
	}
	
	String tableString =" <table instanceid=\"info\"  pageId=\""+PageIdConst.CRM_RPGetWay+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_RPGetWay,user.getUID(),PageIdConst.CRM)+"\" tabletype=\"none\">"+ 
    ...
    "<head>"+
    "<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(645,user.getLanguage()) +"\" column=\"source\""+
	...
	"<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(136,user.getLanguage())+SystemEnv.getHtmlLabelName(1331,user.getLanguage()) +"\" column=\"resultcount\""+ 
		" transmethod=\"weaver.crm.report.CRMReporttTransMethod.getCustomerInfo\" otherpara=\"contactway+column:source\"/>"+ 
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(1219,user.getLanguage()) %>"/>
</jsp:include>
...
<form action="ContactWayRpSum.jsp" method="post" name="weaver">
	<input type="hidden" name="searchName" value="<%=searchName %>">
</form>

<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_RPGetWay%>">
