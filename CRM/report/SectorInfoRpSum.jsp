...
<%
	...
	String backfields = "t1.sector ,COUNT(distinct t1.id) AS resultcount";
	String sqlWhere = null;
	String groupby = "t1.sector";
	...
	if(!"".equals(searchName)){
		fromSql += ", crm_sectorinfo t3";
		sqlWhere += " and t3.id = t1.sector and t3.fullname like '%"+searchName+"%'";
	}
	
	String tableString =" <table instanceid=\"info\"  pageId=\""+PageIdConst.CRM_RPSector+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_RPSector,user.getUID(),PageIdConst.CRM)+"\" tabletype=\"none\">"+ 
    "<sql backfields=\""+backfields+"\" sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+sqlWhere+"\"  "+
    	"sqlorderby=\""+orderby+"\"  sqlprimarykey=\"t1.sector\" sqlsortway=\"Desc\" sqlgroupby=\""+groupby+"\"  sumColumns=\"resultcount\" />"+
    "<head>"+
    "<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(575,user.getLanguage()) +"\" column=\"sector\""+
	...
	"<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(136,user.getLanguage())+SystemEnv.getHtmlLabelName(1331,user.getLanguage()) +"\" column=\"resultcount\""+ 
		" transmethod=\"weaver.crm.report.CRMReporttTransMethod.getCustomerInfo\" otherpara=\"sectorinfo+column:sector\"/>"+ 
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(16583,user.getLanguage()) %>"/>
</jsp:include>
...
<form action="SectorInfoRpSum.jsp" method="post" name="weaver">
	<input type="hidden" name="searchName" value="<%=searchName %>">
</form>

<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_RPSector%>">
