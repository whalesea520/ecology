...
<%
	...
	String backfields = "t1.size_n ,COUNT(distinct t1.id) AS resultcount";
	String sqlWhere = null;
	String groupby = "t1.size_n";
	...
	if(!"".equals(searchName)){
		fromSql += ", CRM_CustomerSize t3";
		sqlWhere += " and t3.id = t1.size_n and t3.fullname like '%"+searchName+"%'";
	}
	
	String tableString =" <table instanceid=\"info\"  pageId=\""+PageIdConst.CRM_RPSize+"\"  pagesize=\""+PageIdConst.getPageSize(PageIdConst.CRM_RPSize,user.getUID(),PageIdConst.CRM)+"\" tabletype=\"none\">"+ 
    "<sql backfields=\""+backfields+"\" sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+sqlWhere+"\"  "+
    	"sqlorderby=\""+orderby+"\"  sqlprimarykey=\"t1.size_n\" sqlsortway=\"Desc\" sqlgroupby=\""+groupby+"\"  sumColumns=\"resultcount\" />"+
    "<head>"+
    "<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(576,user.getLanguage()) +"\" column=\"size_n\""+
    	" transmethod=\"weaver.crm.Maint.CustomerSizeComInfo.getCustomerSizename\"/>"+ 
	"<col width=\"30%\"  text=\""+ SystemEnv.getHtmlLabelName(136,user.getLanguage())+SystemEnv.getHtmlLabelName(1331,user.getLanguage()) +"\" column=\"resultcount\""+ 
		" transmethod=\"weaver.crm.report.CRMReporttTransMethod.getCustomerInfo\" otherpara=\"customersize+column:size_n\"/>"+ 
...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(1285,user.getLanguage()) %>"/>
</jsp:include>
...
<form action="CustomerSizeRpSum.jsp" method="post" name="weaver">
	<input type="hidden" name="searchName" value="<%=searchName %>">
</form>

<input type="hidden" name="pageId" id="pageId" value="<%=PageIdConst.CRM_RPSize%>">
