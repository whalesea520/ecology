...
<%
	...
	if(labelid!=0){
	  String sql="select name,isUsed,labelOrder,labelColor,textColor from CRM_label where id="+labelid;
...
<wea:layout>
	<wea:group context="" attributes="{'groupDisplay':'none'}">
		...
		<wea:item><input type="text" style="width: 80%" onblur="changeLabelName(<%=labelid%>)"  maxlength="50" id="labelName" value='<%=name%>' name="name"/></wea:item>
...
<script type="text/javascript">
  ...
  function doSave(){
     ...
     if("<%=type%>"=="createLabel"||("<%=type%>"=="editLabel"&&"<%=name%>"!=$("#labelName").val())){
	     $.post("/CRM/customer/CrmLabelOperation.jsp?name="+name,{type:'isExist'},function(data){
	        if($.trim(data)=="true")
