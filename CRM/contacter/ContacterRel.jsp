			...
			<table id="reltable" class="rel_table ListStyle" cellpadding="0" cellspacing="0" border="0">
				...
				<tr id="tr_<%=contacterid%>" class="DataLight">
					...
					<td class="info" title="姓名:<%=Util.toScreen(rs.getString("firstname"), user
						.getLanguage())%>">
						<%if (canedit) {%>
						<input size="50" maxlength="50" style="width:110px;float:left;" _fieldname="firstname" _contacterid="<%=contacterid %>"
							class="rel_input" _def="姓名" _value="<%=Util.null2String(rs.getString("firstname")) %>"
							value="<%=Util.null2String(rs.getString("firstname")) %>" />
						<%} else {%>
							<a href="###" style="overflow:hidden;float:left;" onclick="openFullWindowHaveBar('/CRM/contacter/ContacterView.jsp?ContacterID=<%=contacterid%>')">
						...
						<%}%>
						<%if(contact.equals("0")){ %>
						<span class="c_view ci_add" style="display:inline;margin-top:1px;margin-right:4px;float:right;" onclick="addContact('<%=contacterid %>',2,'<%=Util.toScreen(rs.getString("firstname"), user.getLanguage())%> ')" title="添加联系记录"></span>
						<%} %>
					</td>
					<td class="info"
