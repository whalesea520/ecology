...
String querystr = "select inputid , reportdate ,inputstatus from " + thetable + " where modtype='0' and reportdate >='" + currentdate + "' and reportdate <='" + currenttodate + "' and crmid = " + crmid +
" and inputstatus<>'9' order by reportdate " ;


RecordSet.executeSql(querystr);
while(RecordSet.next()) {
    inputids.add(Util.null2String(RecordSet.getString("inputid"))) ;
...
<FORM id=frmmain name=frmmain method=post action="ReportDetailCrm.jsp">
  <div><BUTTON class=btn accessKey=R onClick="javascript:location.href='ReportStatus.jsp'"><U>R</U>-返回</BUTTON> </div> 
...
cellSpacing=0 cellPadding=0>
	...
	for(j=1;j<13;j++){
		...
		tempinputstatuss.clear() ;
            ...
            %>
			<TR>
          	<a href="ReportConfirmDetail_1.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>"><TD 
          <% if( tempinputstatus.equals("0") ) {%> title="已输入" bgColor=white
          <% } else if( tempinputstatus.equals("1") ) {%> title="已确认" bgColor=white
          <% } else if( tempinputstatus.equals("2") ) {%> title="已审核" bgColor=white <%}%>
           height=16>
              <% if( tempinputstatus.equals("0") ) {%><a href="ReportConfirmDetail_1.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>"><IMG SRC="\images\iedit.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("1") ) {%><a href="ReportConfirmDetail_1.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>"><IMG SRC="\images\BacoCheck.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("2") ) {%><a href="ReportConfirmDetail_1.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>"><IMG SRC="\images\BacoCheckName.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a><%}%>
...
cellSpacing=0 cellPadding=0>
	...
	for(i=0;i<24;i++){
        ...
        for(int k=0 ; k<inputids.size() ; k++) {
          ...
          <% } else if( tempinputstatus.equals("2") ) {%> title="已审核" bgColor=white <%}%>
           height=16>
              <% if( tempinputstatus.equals("0") ) {%><IMG SRC="\images\iedit.gif" BORDER="0" HEIGHT="16px" WIDTH="16px">
              <% } else if( tempinputstatus.equals("1") ) {%><IMG SRC="\images\BacoCheck.gif" BORDER="0" HEIGHT="16px" WIDTH="16px">
              <% } else if( tempinputstatus.equals("2") ) {%><IMG SRC="\images\BacoCheckName.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"><%}%>
           </TD></a></TR><tr><td bgColor=<%=bgcolor%> height=2></td></tr><%}%></TBODY></TABLE></TD>
			<%} else {%>
