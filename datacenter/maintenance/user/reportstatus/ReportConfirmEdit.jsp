...
<tr>
  ...
  <COL width="40%">
	...
	while(rs1.next()) {
		...
		<td><input type=text size=30 maxlength=<%=itemfieldscale%> name=<%=itemfieldname%>  value="<%=Util.toScreenToEdit(rs2.getString(itemfieldname),user.getLanguage())%>" ><%=itemfieldunit%></td><td>&nbsp;</td>
		<%} else if(itemfieldtype.equals("2")) { 
            String modfieldname = itemfieldname + "_mod" ;
        %>
		<td><input type=text size=30 name=<%=itemfieldname%> <% /*if( Util.getIntValue( rs2.getString(itemfieldname) , 0) != 0 ) */
        ...
        <td><input type=text size=30 name=<%=modfieldname%> <% if( Util.getIntValue(rs4.getString(itemfieldname),0) != 0 ) {%> value="<%=Util.toScreenToEdit(rs4.getString(itemfieldname),user.getLanguage())%>" <%}%>  onKeyPress="ItemCount_KeyPress()" onBlur='checkcount1(this)'>(前次修正)</td>

		<%} else if(itemfieldtype.equals("3")  || itemfieldtype.equals("5") ) { 
            String modfieldname = itemfieldname + "_mod" ;
        %>
		<td><input type=text size=30 name=<%=itemfieldname%> <% /*if( Util.getDoubleValue(rs2.getString(itemfieldname),0) != 0 )*/
        ...
        <td><input type=text size=30 name=<%=modfieldname%> <% if( Util.getDoubleValue(rs4.getString(itemfieldname),0) != 0 ) {%> value="<%=Util.toScreenToEdit(rs4.getString(itemfieldname),user.getLanguage())%>" <%}%> onKeyPress="ItemNum_KeyPress()" onBlur='checknumber1(this)'>(前次修正)</td>

		<%} else if(itemfieldtype.equals("4")) { %>
		<td><select name="<%=itemfieldname%>"><%=itemfieldunit%>
        ...
        <td><textarea name="<%=itemfieldname%>" rows="5" cols=60> <%=Util.toScreenToEdit(rs2.getString(itemfieldname),user.getLanguage())%> </textarea><%=itemfieldunit%></td><td>&nbsp;</td>
		<%}%>
    </tr>
	<%}}%>
    </tbody>
