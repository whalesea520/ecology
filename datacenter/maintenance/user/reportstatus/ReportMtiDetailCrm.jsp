...
String querystr = "select reportdate ,AVG(CONVERT(int, inputstatus)) as inputstatus from " + thetable + " where modtype='0' and reportdate >='" + currentdate + "' and reportdate <='" + currenttodate + "' and crmid = " + crmid + " and inputstatus<>'9' group by reportdate order by reportdate " ;
...
<FORM id=frmmain name=frmmain method=post action="ReportMtiDetailCrm.jsp">
  <div> 
    <BUTTON class=btn accessKey=R onClick="javascript:location.href='ReportStatus.jsp'"><U>R</U>-返回</BUTTON> </div> 
...
cellSpacing=0 cellPadding=0>
	...
	for(j=1;j<13;j++){
		...
		tempinputstatuss.clear() ;
            ...
            %>
			<TR>
          	<a href="ReportConfirmMtiDetail_1.jsp?thetable=<%=thetable%>&thedate=<%=tempinputdates%>&inprepid=<%=inprepid%>&crmid=<%=crmid%>"><TD 
          ...
          <% } else if( tempinputstatus.equals("2") ) {%> title="已审核" bgColor=white <%}%>
           height=16>
              <% if( tempinputstatus.equals("0") ) {%><a href="ReportConfirmMtiDetail_1.jsp?thetable=<%=thetable%>&thedate=<%=tempinputdates%>&inprepid=<%=inprepid%>&crmid=<%=crmid%>"><IMG SRC="\images\iedit.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("1") ) {%><a href="ReportConfirmMtiDetail_1.jsp?thetable=<%=thetable%>&thedate=<%=tempinputdates%>&inprepid=<%=inprepid%>&crmid=<%=crmid%>"><IMG SRC="\images\BacoCheck.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("2") ) {%><a href="ReportConfirmMtiDetail_1.jsp?thetable=<%=thetable%>&thedate=<%=tempinputdates%>&inprepid=<%=inprepid%>&crmid=<%=crmid%>"><IMG SRC="\images\BacoCheckName.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a><%}%>
