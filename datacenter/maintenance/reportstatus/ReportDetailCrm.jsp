...
cellSpacing=0 cellPadding=0>
	...
	for(j=1;j<13;j++){
		...
		tempinputstatuss.clear() ;
            ...
            %>
			<TR>
          	<a href="ReportConfirmDetail.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>&inprepbudget=<%=inprepbudget%>&bywhat=<%=bywhat%>&currentdate=<%=currentdate%>"><TD
          ...
          <% } else if( tempinputstatus.equals("4") ) {%> title="已审核" bgColor=white <%}%>
           height=16>
              <% if( tempinputstatus.equals("9") ) {%><a href="ReportConfirmDetail.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>&inprepbudget=<%=inprepbudget%>&bywhat=<%=bywhat%>&currentdate=<%=currentdate%>"><IMG SRC="\images\iedit.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("0") ) {%><a href="ReportConfirmDetail.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>&inprepbudget=<%=inprepbudget%>&bywhat=<%=bywhat%>&currentdate=<%=currentdate%>"><IMG SRC="\images\BacoCheck.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a>
              <% } else if( tempinputstatus.equals("4") ) {%><a href="ReportConfirmDetail.jsp?thetable=<%=thetable%>&inputid=<%=tempinputids.get(t)%>&inprepid=<%=inprepid%>&inprepbudget=<%=inprepbudget%>&bywhat=<%=bywhat%>&currentdate=<%=currentdate%>"><IMG SRC="\images\BacoCheckName.gif" BORDER="0" HEIGHT="16px" WIDTH="16px"></a><%}%>
