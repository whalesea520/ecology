...
String sqlWhere = " where t1.label is not null and id in (select min(id) as id from ecologyuplist group by label ) ";
if(!"".equals(label)) {
	sqlWhere = sqlWhere + " and content = '"+label+"'";
}
if(!"".equals(content1)) {
...
tableString += "<col width=\"10%\"   text=\""+"ID"+"\" column=\"id\" orderkey=\"t1.id\"/>"+
    "	<col width=\"10%\"   text=\""+"补丁包"+SystemEnv.getHtmlLabelName(15486,user.getLanguage())+"\" column=\"label\" orderkey=\"t1.label\" />"+
	"	<col width=\"20%\"   text=\""+"名称"+"\" column=\"versionNo\" orderkey=\"t1.versionNo\" />"+
	"	<col width=\"10%\"   text=\""+"补丁包内容"+"\" column=\"content\" transmethod=\"com.weaver.update.PackageUtil.getUpgradeMessage\" otherpara=\"column:id+1+"+lan+"+column:label+content\" target=\"_self\" />"+
	"	<col width=\"10%\"   text=\""+SystemEnv.getHtmlLabelName(724,user.getLanguage())+"\" column=\"configcontent\" transmethod=\"com.weaver.update.PackageUtil.getUpgradeMessage\" otherpara=\"column:id+2+"+lan+"+column:label+config\" target=\"_self\" />"+	
	"	<col width=\"10%\"   text=\""+SystemEnv.getHtmlLabelName(21663,user.getLanguage())+"\" column=\"operationdate\" orderkey=\"operationdate\" />"+
	"	<col width=\"10%\"   text=\""+"升级文件信息"+"\" column=\"label\" orderkey=\"label\"  transmethod=\"com.weaver.update.PackageUtil.getFileMessage\" otherpara=\"column:label\"/>";
tableString +="</head></table>";

String sql = "select * from ecologyuplist t1 "+sqlWhere+" order by label desc";
...
<div class="advancedSearchDiv" id="advancedSearchDiv">
	<wea:layout type="4col">
		<wea:group context="<%= SystemEnv.getHtmlLabelName(347,user.getLanguage())%>">
			<wea:item><%= SystemEnv.getHtmlLabelNames("21663",user.getLanguage())%></wea:item>
			<wea:item>
			...
			<wea:item><%= SystemEnv.getHtmlLabelName(24256,user.getLanguage())%></wea:item>
			<wea:item><input   type="text" name="versionno" value="<%=versionno %>"></wea:item>
			<wea:item><%="补丁包"+SystemEnv.getHtmlLabelName(15486,user.getLanguage())%></wea:item>
			<wea:item><input   type="text" name="label" value="<%=label %>"></wea:item>
		</wea:group>
...
String s = "select distinct(label) from ecologyuplist order by label desc";
rs.execute(s);
if(rs.next()) {
...
function openMessage(id,flage,label) {
	var v = flage;
	if(v=="1") {
		doOpen("/jsp/detail.jsp?ishistory=1&message=content&id="+id+"&label="+label,"<%=SystemEnv.getHtmlLabelName(17530,user.getLanguage())%>"+"<%=SystemEnv.getHtmlLabelName(33368,user.getLanguage())%>");
	} else {
		doOpen("/jsp/detail.jsp?ishistory=1&message=config&id="+id+"&label="+label,"<%=SystemEnv.getHtmlLabelName(724,user.getLanguage())%>");
	}
...
function downloadupgradefile(downloadtype,label) {
	if("add"==downloadtype) {
		document.getElementById("excels").src = "/jsp/upgradefilelist.jsp?downloadtype=add&label="+label;
	} else {
		document.getElementById("excels").src = "/jsp/upgradefilelist.jsp?downloadtype=update&label="+label;
	}
