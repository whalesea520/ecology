...
function showMultiDocDialog(selectids){
    ...
    config.srchead=["<%=SystemEnv.getHtmlLabelName(1867,user.getLanguage())%>","<%=SystemEnv.getHtmlLabelName(20869,user.getLanguage())%>","<%=SystemEnv.getHtmlLabelName(124,user.getLanguage())%>"];
...
function okCallbackFn(config,destMap,json,destMapKeys){
	...
	var returnValue={id:ids,name:names,email:emails};
	try{
		if(config.dialog){
			config.dialog.callback(returnValue);
		}else{
		...
		//window.parent.returnValue =  returnValue;
		//window.parent.close();
		config.dialog.close(returnValue);
	}
}
