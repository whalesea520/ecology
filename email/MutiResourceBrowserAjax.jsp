...
if(src.equalsIgnoreCase("dest")){//右侧已选择列表的sql条件
	if (!systemIds.equals("")) {
		...
		spp.setBackFields(" HrmResource.id, lastname, departmentid, subcompanyid1, jobtitle, dsporder,email ");
		...
		if("".equals(tabId)||"1".equals(tabId)||"2".equals(tabId)||"3".equals(tabId)){//鏈夋晥tab椤电
			if(!lastname.equals("")){
		...
		String moduleManageDetach= Util.null2String(request.getParameter("moduleManageDetach"));//(妯″潡绠＄悊鍒嗘潈-鍒嗘潈绠＄悊鍛樹笓鐢?
		
		String moduleManageDetachSQL="";
		if(moduleManageDetach!=null && "1".equals(moduleManageDetach)){

		//鏍规嵁鍒嗘潈绠＄悊鍛榠d鏌ヨ鍏舵墍绠¤緰鐨勫垎閮紙鍒嗘潈绠＄悊鍛樿缃〉闈㈡湁璁剧疆鍏跺垎鏉冪鐞嗗憳绠＄悊鐨勫垎閮級
