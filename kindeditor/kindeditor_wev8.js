...
KE.browser = (function() {
    ...
    return {
        ...
        IE11: ((ua.indexOf("trident/7.0")>-1||ua.indexOf("trident/8.0")>-1||ua.indexOf("rv:11")>-1) && ua.indexOf('opera') == -1),
...
KE.format = {
    ...
    getHtml : function(html, htmlTags, urlType) {
        ...
        var scriptEvents=KE.util.arrayToHash(new Array("onload","onerror","onchange","onsubmit","onreset","onselect","onblur","onfocus","onabort","onkeydown",
                                    "onkeypress","onkeyup","onclick","ondblclick","onmousedown","onmousemove","onmouseout","onmouseover","onmouseup"));                         
        ...
        html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {
            ...
            var endSlash = $5 ? ' ' + $5 : '';
            var endNewline = $6 || '';
            if (tagName== "script"||tagName== "form"||tagName== "input"||tagName== "button") return ''; //杩囨护script鏍囩
            ...
            if (attr !== '') {
                attr = attr.replace(/\s*([\w-:]+)=([^\s"'<>]+|"[^"]*"|'[^']*')/g, function($0, $1, $2) {
                    ...
                    //锟斤拷锟剿碉拷javascript锟铰硷拷
                    if (typeof scriptEvents[key] != "undefined") return ' ';
                    if (val === '') {
                        val = '""';
                    } else {
                        if (key === "style") {
...
KE.util = {
    ...
    getScrollPos : function() {
        var x, y;
        if (KE.browser.IE || KE.browser.OPERA || KE.browser.IE11) {
    ...
    createRange : function(doc) {
        ...
        return KE.util._getRng(doc);
    },
    _getRng: function (doc) {
        var sel = KE.util._getSel(doc), rng;
        try {
            if (sel.rangeCount > 0) {
    ...
    _getSel:function (doc) {
    var win = KE.util._getWin(doc);
    //return KE.browser._IERANGE ? doc.selection : win.getSelection();
    return doc.selection || win.getSelection();
    },
    _getWin:function (node) {
        if (!node) {
            return window;
        }
        var doc = KE.util._getDoc(node);
        return doc.parentWindow || doc.defaultView;
    },
    _getDoc:function (node) {
    if (!node) {
        return document;
    }
    ...
    getFullHtml : function(id) {
        ...
        html += '<link href="' + KE.g[id].skinsPath + 'common/editor_wev8.css?ver=' + escape(KE.version) + '" rel="stylesheet" type="text/css" />';
        ...
        html += '</head>';
        html += '<body class="ke-content" style="word-break:break-all;font-size: '+KE.g[id].mofontsize+'; !important;"></body>';
        html += '</html>';
        return html;
    },
    ...
    getMediaImage : function(id, type, attrs) {
        ...
        var html = '<img class="' + className + '" src="' + KE.g[id].skinsPath + 'common/blank_wev8.gif" ';
    ...
    insertHtml : function(id, html) {
        ...
        if(KE.browser.IE11) {
            var doc = g.iframeWin.document;
            var range = doc.getSelection();
            var rng = null;
            try {
                if (range.rangeCount > 0) {
                    rng = range.getRangeAt(0);
                } else {
                    rng = doc.createRange();
                }
            } catch(e) {};
...
KE.init = function(args) {
    ...
    if (g.loadStyleMode) KE.util.loadStyle(g.skinsPath + g.skinType + '_wev8.css');
}
...
KE.plugin['emoticons'] = {
    click : function(id) {
        ...
        if (allowPreview) {
            ...
            previewImg.src = path + '0_wev8.gif';
            previewImg.border = 0;
                    ...
                    if (previewDiv) {
                        cell.onmouseover = (function(j, num) {
                            return function() {
                                ...
                                previewImg.src = path + num + '_wev8.gif';;
    ...
    exec : function(id, num) {
        var src = KE.g[id].pluginsPath + 'emoticons/' + num + '_wev8.gif';
...
KE.plugin['image'] = {
    ...
    check : function(id) {
        ...
        if (type == 2&&!urlBox.value.match(/\.(jpg|jpeg|gif|bmp|png)(\s|\?|$)/i)) {
        ...
        if (type == 2) {
            ...
            var onloadFunc = function() {
                ...
                if (typeof data === 'object' && 'error' in data) {
                    if (data.error === 0) {
                        self.insert(id, data.url, title, width, height, 0, align,data.alt);
                    } else {
                        alert(data.message);
                        return false;
                    }
            ...
            var url = KE.$('url', dialogDoc).value;
            this.insert(id, url, title, width, height, 0, align,alt);
        }
    },
    insert : function(id, url, title, width, height, border, align,alt) {
        
        var html = '<img alt="' + alt + '" src="' + url + '" kesrc="' + url + '" ';
