...
<%
String isDouble = Util.null2String(request.getParameter("isDouble"));//定义一个标识符,用于区别是否重复
if("1".equals(isDouble)){
%>
<script language=javascript >
    $(function(){//弹窗效果               
        //top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(21011,user.getLanguage())%>");//二次确认弹窗
        top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(21011,user.getLanguage())%>");//系统弹窗
    });
...
if("1".equals(typename)||"2".equals(typename)||"3".equals(typename)||"4".equals(typename)){
	select_att = true;
}
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(82743,user.getLanguage())%>' attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
			<select id="typename" style='width:120px!important;' name="typename" onchange="onChangeTypeFun(this.value)" >
			  ...
			  <option value="3" <%if(typename.equals("3")) out.print("selected"); %>>U8</option>
                  <%--QC--%>
		      <option value="9" <%if(typename.equals("8")) out.print("selected"); %>>K3/Cloud</option>
		...
		<wea:item>
            <wea:required id="nameimage" required="true" value="">
            	...
            	<input class=inputstyle type=text style='width:280px!important;' size=30 maxlength="30" name="name"  id="name" value=""  onchange="isExist(this.value);checkinput('name','nameimage');">
            </wea:required>
		</wea:item>
...
function submitData() {
    ...
    if(check_form(frmMain,checkvalue)){
        ...
        if(typenametmp != 6 && typenametmp != 7){//企业邮箱之外的类型，才做其他参数的校验
...
jQuery(document).ready(function(){
    ...
    var paramnctr = "<TR id='ncpkcode_id1' <%if(!typename.equals("1")) out.print("style='display:none;'"); %>>"+
		            ...
		            "<TD><INPUT class='Inputstyle' type='text' id='paramnames_nc' name='paramnames_nc'  value='pkcorp' _noMultiLang=true onchange='checkparamnames(this)' onblur='checkParamnamesRepet(this)'></TD>"+
...
function checkParamnamesRepet(obj){
    ...
    var pkcorp = $("input[name='paramnames_nc']").val();
    if(obj != $("input[name='paramnames_nc']")[0] && obj.value == pkcorp){//针对NC、NC6情况，单独验证“公司名称”
        obj.value = "";
		isSame = true;
	}
    if(isSame){
