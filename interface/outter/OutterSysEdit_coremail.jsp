	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(20961,user.getLanguage())%>' attributes="{'samePair':'BaseInfo','groupOperDisplay':'none'}">
		...
		<wea:item>
			<%if(typename.equals("1")){%>NC<%}
			  ...
			  else if(typename.equals("8")){%><%=SystemEnv.getHtmlLabelName(129791,user.getLanguage())%><%}
		...
		<wea:item>
            ...
            <wea:required id="nameimage" required="true" value='<%=name%>'>
            	<input class=inputstyle type=text style='width:280px!important;' size=30 maxlength="30" name="name"  value="<%=name%>" onchange='checkinput("name","nameimage")'>
            </wea:required>
            <%}else{%><%=name%><%}%>
		</wea:item>
		...
		<%
			...
			String username_type = (typename.equals("8")||typename.equals("6"))?"{'samePair':'username_type','display':'none'}":"{'samePair':'username_type','display':'true'}";
		%>				
	   ...
	   <wea:item>
		...
		<%
		String client_type = (typename.equals("6") || typename.equals("8"))?"{'samePair':'client_type','display':'none'}":"{'samePair':'client_type','display':'true'}";
		%>
