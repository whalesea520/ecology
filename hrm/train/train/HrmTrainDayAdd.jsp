...
<wea:layout>
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
		...
		<wea:item>
		  ...
		  <SPAN id=dayspan ><IMG src="/images/BacoError_wev8.gif" align=absMiddle></SPAN> 
		...
		<wea:item>
			...
			<SPAN id='starttimespan'><IMG src="/images/BacoError_wev8.gif" align=absMiddle></SPAN>
			<input class=inputstyle type=hidden id='starttime' name='starttime'>
		</wea:item>
		...
		<wea:item>
			...
			<SPAN id='endtimespan'><IMG src="/images/BacoError_wev8.gif" align=absMiddle></SPAN>
			<input class=inputstyle type=hidden id='endtime' name='endtime'>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(345,user.getLanguage())%></wea:item>
		<wea:item>
		  <textarea class=inputstyle cols=50 rows=4 style="width: 80%" name=content ></textarea>
		</wea:item>
		<wea:item><%=SystemEnv.getHtmlLabelName(16142,user.getLanguage())%> </wea:item>
		<wea:item>
		  <textarea class=inputstyle  cols=50 rows=4 style="width: 80%" name=aim></textarea>            
		</wea:item>
		...
		<wea:item>
	      ...
	      browserSpanValue='<%=ResourceComInfo.getMulResourcename(actor)%>'>
	    </brow:browser>
		</wea:item>	   
</TABLE> 
	</wea:group>
...
function dosave(){
if(check_form(document.frmMain,'day,starttime,endtime')&&checkDateValidity()){
...
function checkDateValidity(){
    ...
    if(compareDate(startDate,selectDay)==1 || compareDate(selectDay,endDate)==1){
      window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(83883,user.getLanguage())%>"+startDate+"<%=SystemEnv.getHtmlLabelName(31052,user.getLanguage())%>"+endDate+"<%=SystemEnv.getHtmlLabelName(83882,user.getLanguage())%>");
        isValid = false;
    }
    return isValid;
}
