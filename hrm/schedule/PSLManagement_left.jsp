...
while(SubCompanyComInfo.next()){
	if("0".equals(SubCompanyComInfo.getCompanyiscanceled()) || "".equals(SubCompanyComInfo.getCompanyiscanceled())){
		subcompanyid=SubCompanyComInfo.getSubCompanyid();
		break;
	}
}
...
<BODY onload="initTree()">
	<FORM NAME=SearchForm STYLE="margin-bottom:0" action="/hrm/HrmTab.jsp?_fromURL=PSLManagementView&subcompanyid=<%=subcompanyid %>" method=post target="contentframe">
	...
	<table cellspacing="0" cellpadding="0" class="flowsTable" style="width:100%;height:100%;"  >
		...
		<tr>
			<td style="width:23%;" class="flowMenusTd">
				<div id="deeptree" class="cxtree" style="background-color:#F8F8F8;overflow:hidden;position:relative;" CfgXMLSrc="/css/TreeConfig.xml" />
...
function setCompany(nodeid){
  document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=PSLManagementView&subcompanyid=-1&cmd=";
...
function setSubcompany(nodeid){
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=PSLManagementView&subcompanyid="+subid+"&cmd="+cmd;
    document.SearchForm.submit();
}
function setDepartment(nodeid){
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=PSLManagementView&departmentid="+deptid+"&cmd="+cmd;;
    document.SearchForm.submit();
}
