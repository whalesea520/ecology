...
RCMenu += "{"+SystemEnv.getHtmlLabelName(1290,user.getLanguage())+",/hrm/schedule/HrmResouceSchedule2.jsp?id="+id+",_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<tr>
<td valign="top">
<FORM id=weaver name=frmmain action="HrmResouceScheduleOperation.jsp" method=post >
...
<TABLE class=ListStyle cellspacing=1 >	<COLGROUP>
	...
	<TR class=Header> 
          ...
          <%
          Resouce_id=RecordSet.getString("relatedid");
	  ...
	  String dept_name=Util.toScreen(DepartmentComInfo.getDepartmentname(dept_id),user.getLanguage());
          %>
          <TH width="70%" colspan=2><%=Resouce_name%> - <%=dept_name%>
          <input class=inputstyle type="hidden" name="Resouce_id" value="<%=Util.toScreen(Resouce_id,user.getLanguage())%>"></TH>
        </TR>
        <TR class=Header>
...
function doCopy(){
		...
		document.frmmain.action="HrmResouceScheduleCopy.jsp";
		document.frmmain.submit();
}
</script>
</body>
