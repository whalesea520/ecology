...
function initTree(){
	...
	*/
	...
	CXLoadTreeItem("", "/hrm/schedule/HrmDefaultScheduleXML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
	var tree = new WebFXTree();
	tree.add(cxtree_obj);
	//document.write(tree);
	document.getElementById('deeptree').innerHTML = tree;
...
function setCompany(nodeid){
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=LeaveTypeColorEdit";
...
function setSubcompany(nodeid){ 
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=LeaveTypeColorEdit&subcompanyid1="+subid;
    document.SearchForm.submit();
}
function setDepartment(nodeid){
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=LeaveTypeColorEdit&departmentid="+deptid+"&subcompanyid1="+subCompanyId;
    document.SearchForm.submit();
}
