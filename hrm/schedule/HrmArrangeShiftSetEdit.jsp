...
<BODY>
<%
String sql = "select sharetype, relatedId, level_from, level_to from HrmArrangeShiftSet where id = "+id;
rs.execute(sql);
rs.next();
String sharetype = rs.getString("sharetype");
String relatedId = rs.getString("relatedId");
String relatedName = ""; 
if(sharetype.equals("1")){
	...
	relatedName = RolesComInfo.getRolesname(relatedId);
...
<input id=relatedId name=relatedId type=hidden value="<%=relatedId %>">
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361 , user.getLanguage())%>'>
   	...
   	<wea:item>
			<select id="sharetype" name="sharetype" onchange="jsSharetype()">
				<option value="1" <%=rs.getInt("sharetype")==1?"selected":"" %>><%=SystemEnv.getHtmlLabelName(141 , user.getLanguage())%></option>
				<option value="2" <%=rs.getInt("sharetype")==2?"selected":"" %>><%=SystemEnv.getHtmlLabelName(27511 , user.getLanguage())%></option>
				<option value="3" <%=rs.getInt("sharetype")==3?"selected":"" %>><%=SystemEnv.getHtmlLabelName(179 , user.getLanguage())%></option>
				<option value="4" <%=rs.getInt("sharetype")==4?"selected":"" %>><%=SystemEnv.getHtmlLabelName(122 , user.getLanguage())%></option>
				<option value="5" <%=rs.getInt("sharetype")==5?"selected":"" %>><%=SystemEnv.getHtmlLabelName(1340 , user.getLanguage())%></option>
			</select>
   	</wea:item>
   	...
   	<wea:item attributes="{'samePair':'item_subcompany'}">
	  	<brow:browser viewType="0" name="subcompany" browserValue='<%=sharetype.equals("1")?relatedId:"" %>'
      browserUrl="/systeminfo/BrowserMain.jsp?mouldID=hrm&url=/hrm/company/SubcompanyBrowser.jsp?selectedids="
      hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2'
      completeUrl="/data.jsp?type=164" _callback="jsSetRelate" browserSpanValue='<%=sharetype.equals("1")?relatedName:"" %>'>
      </brow:browser>
   	</wea:item>
   	...
   	<wea:item attributes="{'samePair':'item_department'}">
   		<brow:browser viewType="0"  name="department" browserValue='<%=sharetype.equals("2")?relatedId:"" %>'
      browserUrl="/systeminfo/BrowserMain.jsp?mouldID=hrm&url=/hrm/company/DepartmentBrowser.jsp?selectedids="
      hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2'
      completeUrl="/data.jsp?type=4" _callback="jsSetRelate" browserSpanValue='<%=sharetype.equals("2")?relatedName:"" %>'>
      </brow:browser>
   	</wea:item>
   	...
   	<wea:item attributes="{'samePair':'item_resource'}">
   		<brow:browser viewType="0"  name="resource" browserValue='<%=sharetype.equals("3")?relatedId:"" %>'
      browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp?selectedids="
      hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2'
      completeUrl="/data.jsp" _callback="jsSetRelate" browserSpanValue='<%=sharetype.equals("3")?relatedName:"" %>'>
      </brow:browser>
   	</wea:item>
   	...
   	<wea:item attributes="{'samePair':'item_role'}">
   		<brow:browser viewType="0"  name="role" browserValue='<%=sharetype.equals("4")?relatedId:"" %>'
      browserUrl="/systeminfo/BrowserMain.jsp?mouldID=hrm&url=/hrm/roles/HrmRolesBrowser.jsp?selectedids="
      hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2'
      completeUrl="/data.jsp?type=65" _callback="jsSetRelate" browserSpanValue='<%=sharetype.equals("4")?relatedName:"" %>'>
      </brow:browser>
   	</wea:item>
		...
		<wea:item attributes="{'samePair':'item_level'}">
			<input name="level_from" type="text" value="<%=rs.getInt("level_from") %>" style="width: 60px">&nbsp;
			-&nbsp;<input name="level_to" type="text" value="<%=rs.getInt("level_to") %>" style="width: 60px">
		</wea:item>
	</wea:group>
</wea:layout>
 <%if("1".equals(isDialog)){ %>
...
function submitData() {
 if(check_form(frmmain,'relatedId,sharetype')){
