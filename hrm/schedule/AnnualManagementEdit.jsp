...
if(departmentid>0){
   ...
   sql = "select distinct a.*,b.annualyear,b.annualdays from (select id,lastname,subcompanyid1,departmentid,jobtitle,dsporder,startdate,enddate from hrmresource where departmentid = " + departmentid + "  and (status = 0 or status = 1 or status = 2 or status = 3) and status != 10) a left join hrmannualmanagement b on a.id = b.resourceid and annualyear = '" + tempyear + "' and b.status <> 0 order by dsporder,lastname,a.id";
}else{
   showname=SubCompanyComInfo.getSubCompanyname(""+subcompanyid)+"<b>("+SystemEnv.getHtmlLabelName(141,user.getLanguage())+")</b>";
   sql = "select distinct a.*,b.annualyear,b.annualdays from (select id,lastname,subcompanyid1,departmentid,jobtitle,dsporder,startdate,enddate from hrmresource where 1=1 "+((cmd.length()==0||subcompanyid==-1)?"":("and subcompanyid1 = " + subcompanyid))+" and (status = 0 or status = 1 or status = 2 or status = 3) and status != 10) a left join hrmannualmanagement b on a.id = b.resourceid and annualyear = '" + tempyear + "' and b.status <> 0 order by dsporder,lastname,a.id";
}
		...
		<wea:item attributes="{'isTableList':'true'}">
			<wea:layout type="table" attributes="{'cols':'5','cws':'20%,20%,20%,20%,20%'}">
			  ...
			  %>
			     ...
			     <wea:item>
					<input type=text size=4 class=inputstyle name=annualdays value="<%=Util.round(Util.getFloatValue(tempannualdays,0)+"",2)%>">
...
function onSave(obj){
	var annualdays = document.getElementsByName("annualdays");
	var patt1=new RegExp("^[0-9].*$");
	for(var i=0;i<annualdays.length;i++){//年假天数必须为正数
		var annualday = annualdays[i].value;
		if(!annualday || annualday.replace(/(^\s+)|(\s+$)/g,'') == "") continue;
		if(!patt1.test(annualday)){
			window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(129356,user.getLanguage())%>");
			return;
		}
	}
	obj.disable=true;
...
function onImport(){
    frmmain.action="AnnualManagementImport.jsp?annualyear=<%=annualyear%>&subCompanyId=<%=subcompanyid%>&departmentid=<%=departmentid%>";
    frmmain.submit();
}
</script>
</BODY>
