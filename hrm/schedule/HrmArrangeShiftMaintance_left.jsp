...
function initTree111(){
	var rti;
	var tree = new WebFXTree('');
	tree.setBehavior("explorer");
	tree.add(rti = new WebFXLoadTreeItem("<%=SystemEnv.getHtmlLabelName(141,user.getLanguage())%>", "/hrm/tree/HrmCompany_XML.jsp?rightStr=<%=rightStr%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>"));
...
function setCompany(nodeid){
    ...
    //document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=HrmArrangeShiftMaintance";
...
function setSubcompany(nodeid){ 
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=HrmArrangeShiftMaintance&subcompanyid="+subid;
    document.SearchForm.submit();
}
function setDepartment(nodeid){
    ...
    document.SearchForm.action="/hrm/HrmTab.jsp?_fromURL=HrmArrangeShiftMaintance&departmentid="+deptid+"&subcompanyid="+subCompanyId;
    document.SearchForm.submit();
}
