...
sub onShowMould(tdname,inputename)	
	...
	frmmain.action="HrmResouceScheduleAdd.jsp"
	frmmain.submit()
end sub
...
int Resouce_id=Util.getIntValue(request.getParameter("id"),0);
if(Resouce_id == 0){
  Resouce_id=Util.getIntValue(request.getParameter("Resouce_id"),0);
}
String Resouce_name=ResourceComInfo.getResourcename(Resouce_id+"");
...
<tr>
<td valign="top">
<FORM id=weaver name=frmmain action="HrmResouceScheduleOperation.jsp" method=post onsubmit="return check_form(this,'Resouce_id')">
...
<TABLE class=ListStyle cellspacing=1 >
	...
	<TR class=Header> 
          ...
          <TH width="70%" colspan=2><button class=Browser onclick="onShowMould('Resoucename','Resouce_id')"></button>
          <span id=Resoucename><%if(Resouce_id!=0){%><%=Resouce_name%><%}%></span>
          <input class=inputstyle type="hidden" name="Resouce_id" <%if(Resouce_id!=0){%>value="<%=Resouce_id%>"<%}%>></TH>
        </TR>
        <TR class=Header>
...
<%
Time time = new Time();
int scheduletype = time.getScheduleType(Resouce_id);
if(scheduletype == 0 ||scheduletype == 2){
RecordSet.executeProc("HrmSchedule_Select_Default","");
}
if(scheduletype == 1){
RecordSet.executeProc("HrmSchedule_Select_ByDepID",dept_id);
}
