...
if(!sqlwhere.equals("")) ishead = 1;
    ...
    if(sqlwhere.equals("")&&status.equals("")){
        if(ishead==0){
            ishead = 1;
            sqlwhere += " where (status=4 or status=5 or status=6 or status=7) and  subcompanyid1 in("+subcomstr+")" ;
        }
        else
            sqlwhere += " (status=4 or status=5 or status=6 or status=7) and  subcompanyid1 in("+subcomstr+")";
    }
    if(sqlwhere.equals("")&&!status.equals("")){
        if(ishead==0){
            ishead = 1;
...
<tr width=100%>
  ...
  <td align="center" valign="center" width="10%">
				...
				<br>
				<img src="/images/arrow_left_wev8.gif" style="cursor:hand" title="<%=SystemEnv.getHtmlLabelName(456,user.getLanguage())%>" onClick="javascript:one2two(from,srcList);">
				<br>
				<img src="/images/arrow_right_wev8.gif"  style="cursor:hand" title="<%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%>" onclick="javascript:two2one(from,srcList);">				
				<br>
				<img src="/images/arrow_left_all_wev8.gif" style="cursor:hand" title="<%=SystemEnv.getHtmlLabelName(17025,user.getLanguage())%>" onClick="javascript:removeAll(from,srcList);">
				<br>				
				<img src="/images/arrow_right_all_wev8.gif"  style="cursor:hand" title="<%=SystemEnv.getHtmlLabelName(16335,user.getLanguage())%>" onclick="javascript:removeAll(srcList,from);">				
				<br>
