...
if(!check_per.equals("")){
	try{
	if(check_per.indexOf(",")==0)check_per = check_per.substring(1);
...
if(!sqlwhere.equals("")) ishead = 1;
    ...
    if(sqlwhere.equals("")&&status.equals("")){
        if(ishead==0){
            ishead = 1;
            sqlwhere += " where status<4 and  subcompanyid1 in("+subcomstr+")" ;
        }
        else
            sqlwhere += " status<4 and  subcompanyid1 in("+subcomstr+")";
    }
    if(sqlwhere.equals("")&&!status.equals("")){
        if(ishead==0){
            ishead = 1;
