...
<IFRAME name=frame2 id=frame2 src="/hrm/resource/browser/hire/MultiSelect.jsp?tabid=<%=tabid%>&resourceids=<%=resourceids%>&sqlwhere=<%=xssUtil.put(sqlwhere)%>" width=100%  height=100% frameborder=no scrolling=no>
<%=SystemEnv.getHtmlLabelName(15017, user.getLanguage())%>
</IFRAME>
</td>
</tr>
...
<script language=javascript>
	function resetbanner(objid){
		...
		if(objid == 0 ){		        
		        window.frame1.location="/hrm/resource/browser/hire/SearchByOrgan.jsp?sqlwhere=<%=xssUtil.put(sqlwhere)%>";
		        try{
		        window.frame2.btnsub.style.display="none"
		        }catch(err){}
		        }			
		else if(objid == 1){
			window.frame1.location="/hrm/resource/browser/hire/SearchByGroup.jsp?sqlwhere=<%=xssUtil.put(sqlwhere)%>";
			try{
			window.frame2.btnsub.style.display="none"
			}catch(err){}
			}
		else if(objid == 2){
			window.frame1.location="/hrm/resource/browser/hire/Search.jsp?status=<%=status%>&sqlwhere=<%=xssUtil.put(sqlwhere)%>";
			try{
