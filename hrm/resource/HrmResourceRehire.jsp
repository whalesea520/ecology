...
<table id="topTitle" cellpadding="0" cellspacing="0">
	...
	<wea:layout>
		<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
			<wea:item><%=SystemEnv.getHtmlLabelName(16109,user.getLanguage())%></wea:item>
            <wea:item> 
		  <%
		  String lastname = "";
		  ...
		  String sqlwhere = " (status=4 or status=5 or status=6 or status=7) ";
		  String browserUrl = "/systeminfo/BrowserMain.jsp?url=/hrm/resource/MultiResourceBrowserByRight.jsp?rightStr=HrmResourceAdd:Add&fromHrmStatusChange=HrmResourceRehire&sqlwhere="+xssUtil.put(sqlwhere)+"&selectedids=";
			String completeUrl = "/data.jsp?whereClause="+xssUtil.put(" (status=4 or status=5 or status=6 or status=7) ");
			
			if(!"".equals(subcomstr)){
			completeUrl = "/data.jsp?whereClause="+xssUtil.put(" (status=4 or status=5 or status=6 or status=7) and ("+Util.getSubINClause(subcomstr, "t1.subcompanyid1", "in") +") ");
			}
		  %>
        <brow:browser viewType="0" name="resourceid" browserValue='<%=resourceid %>' 
            ...
            <wea:item><%=SystemEnv.getHtmlLabelName(16110,user.getLanguage())%></wea:item>
            ...
            <wea:item><%=SystemEnv.getHtmlLabelName(16111,user.getLanguage())%></wea:item>
            <wea:item><BUTTON class=Calendar type="button" id=selectcontractdate onclick="getDate(changeenddatespan,changeenddate)"></BUTTON> 
              <SPAN id=changeenddatespan ></SPAN> 
              <input type="hidden" name="changeenddate">
            </wea:item>
            <wea:item><%=SystemEnv.getHtmlLabelName(16112,user.getLanguage())%></wea:item>
            <wea:item>
              <textarea class=inputstyle rows=5 cols=40 name="changereason"></textarea>
            </wea:item>
            <wea:item><%=SystemEnv.getHtmlLabelName(16113,user.getLanguage())%></wea:item>
            <wea:item>
            ...
            <wea:item><%=SystemEnv.getHtmlLabelName(16114,user.getLanguage())%></wea:item>
            <wea:item>
            <span>
