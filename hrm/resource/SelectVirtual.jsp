...
public String getAllSubId(String subId,String ids){
	...
	rs_s.executeSql("select id from HrmSubCompanyVirtual where id <> " + subId + " and supsubcomid = "+subId);
	while(rs_s.next()){
...
public String getAllDeptId(String deptId,String ids){
	...
	rs_d.executeSql("select id from HrmDepartmentVirtual where id <> " + deptId + " and supdepid = "+deptId);
	while(rs_d.next()){
...
if(sqlwhere.length()==0) sqlwhere = " where hr.id =hrmresourcevirtual.resourceid";
...
if(tabid.equals("0")&&alllevel.equals("1")){
	...
	if(!departmentid.equals("")){
		if(ishead==0){
			ishead = 1;
			sqlwhere += " where HrmResourcevirtual.departmentid =" + departmentid +" " ;
		}
		else
			sqlwhere += " and HrmResourcevirtual.departmentid =" + departmentid +" " ;
	}
}
if(tabid.equals("0")&&alllevel.equals("1")){
	//包含下级机构不需要，
	...
	if(departmentid.equals("")&&!subcompanyid.equals("")){
		if(ishead==0){
			ishead = 1;
			sqlwhere += " where HrmResourcevirtual.subcompanyid =" + subcompanyid +" " ;
		}
		else
			sqlwhere += " and HrmResourcevirtual.subcompanyid =" + subcompanyid +" " ;
	}
}
...
if(from.equals("add")){
 sqlstr = "select hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle "+
			    "from HrmResource hr, hrmresourcevirtual " + sqlwhere+" and (accounttype is null or accounttype=0) order by dsporder,lastname"; ;

if(tabid.equals("1")&&!groupid.equals("")){
sqlstr="select hr.id,hr.lastname,hrmresourcevirtual.departmentid,hr.jobtitle from hrmresource hr,HrmGroupMembers t2, hrmresourcevirtual where hr.id = hrmresourcevirtual.resourceid and (hr.accounttype is null or hr.accounttype=0) and hr.id=t2.userid and t2.groupid="+groupid+(isNoAccount.equals("1")?"":noAccountSql)+" order by hr.dsporder,hr.lastname";
}

if(tabid.equals("0")&&!companyid.equals("")){
sqlstr="select  hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle  from hrmresource hr, hrmresourcevirtual where hr.id = hrmresourcevirtual.resourceid and (accounttype is null or accounttype=0) and (status =0 or status = 1 or status = 2 or status = 3) "+(isNoAccount.equals("1")?"":noAccountSql);
...
sqlstr+=" and hrmresourcevirtual.subcompanyid in("+allsubId+") order by dsporder,lastname";
//added by wcd 2014-07-08 end
...
sqlstr+=" and hrmresourcevirtual.departmentid in("+alldeptId+") order by dsporder,lastname";
//added by wcd 2014-07-08 end
}
}else{
 sqlstr = "select hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle "+
			    "from HrmResource hr, hrmresourcevirtual " + sqlwhere+" order by dsporder,lastname"; ;

if(tabid.equals("1")&&!groupid.equals("")){
sqlstr="select hr.id,hr.lastname,hrmresourcevirtual.departmentid,hr.jobtitle from hrmresource hr,HrmGroupMembers t2, hrmresourcevirtual where hr.id=hrmresourcevirtual.resourceid and hr.id=t2.userid and t2.groupid="+groupid+(isNoAccount.equals("1")?"":noAccountSql)+" order by hr.dsporder,hr.lastname";
}

if(tabid.equals("0")&&!companyid.equals("")){
sqlstr="select  hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle  from HrmResource hr, hrmresourcevirtual " + sqlwhere+" and (status =0 or status = 1 or status = 2 or status = 3) "+(isNoAccount.equals("1")?"":noAccountSql);
...
//added by wcd 2014-07-08 end
	...
	if(from.equals("add")){
		...
		if(tabid.equals("1")&&!groupid.equals("")){
		sqlstr="select hr.id,hr.lastname,hrmresourcevirtual.departmentid,hr.jobtitle from hrmresource hr,HrmGroupMembers t2, HrmResourcevirtual " + sqlwhere+" and (hr.accounttype is null or hr.accounttype=0) and hr.id=t2.userid and t2.groupid="+groupid+(isNoAccount.equals("1")?"":noAccountSql)+" order by hr.dsporder,hr.lastname";
		}

		if(tabid.equals("0")&&!companyid.equals("")){
		sqlstr="select  hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle  from hrmresource hr, HrmResourcevirtual " + sqlwhere+" and (accounttype is null or accounttype=0) and (status =0 or status = 1 or status = 2 or status = 3) "+(isNoAccount.equals("1")?"":noAccountSql);
		...
		if(tabid.equals("1")&&!groupid.equals("")){
		sqlstr="select hr.id,hr.lastname,hrmresourcevirtual.departmentid,hr.jobtitle from hrmresource hr,HrmGroupMembers t2 , HrmResourcevirtual" + sqlwhere+" and hr.id=t2.userid and t2.groupid="+groupid+(isNoAccount.equals("1")?"":noAccountSql)+" order by hr.dsporder,hr.lastname";
		}
...
if((tabid.equals("2") && isInit) ||(tabid.equals("0") && nodeid.equals(""))) sqlstr = "select hr.id,lastname,hrmresourcevirtual.departmentid,jobtitle from HrmResource hr WHERE 1=2";
...
<TABLE width=100% class="BroswerStyle"  cellspacing="0" STYLE="margin-top:0">
   ...
   <!-- <div id="tableList" style="overflow-y:scroll;width:100%;height:295px">  </div> -->
		...
		"<head>";
			...
			tableString += "<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"departmentids\" orderkey=\"departmentids\" transmethod=\"weaver.hrm.companyvirtual.DepartmentVirtualComInfo.getDepartmentname\"/>";
			tableString += "<col width=\"30%\"  text=\""+SystemEnv.getHtmlLabelName(141,user.getLanguage())+"\" column=\"subcompanyids\" orderkey=\"subcompanyids\" transmethod=\"weaver.hrm.companyvirtual.SubCompanyVirtualComInfo.getSubCompanyname\"/>"+
		"</head>"+
		"</table>";      
		%>
