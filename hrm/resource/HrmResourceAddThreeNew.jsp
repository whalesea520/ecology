...
<%
   ...
   %>
    ...
    <div>
    	   <input class=inputstyle type="hidden" id="customfield<%=hfm.getFieldid()%>" name="customfield<%=hfm.getFieldid()%>" value="" onchange='checkinput("customfield<%=hfm.getFieldid()%>","customfield<%=hfm.getFieldid()%>span");'>
...
<script>
var items=[
{width:"16%",colname:"<%=SystemEnv.getHtmlLabelName(231,user.getLanguage())%>",itemhtml:"<input class=inputstyle type=text style='width:98%!important;' name='language'>"},
{width:"16%",colname:"<%=SystemEnv.getHtmlLabelName(15715,user.getLanguage())%>",itemhtml:"<select class=inputstyle id=level style='width:90%!important;' name='level'><option value=0 selected ><%=SystemEnv.getHtmlLabelName(154,user.getLanguage())%></option><option value=1 ><%=SystemEnv.getHtmlLabelName(821,user.getLanguage())%></option><option value=2 ><%=SystemEnv.getHtmlLabelName(822,user.getLanguage())%></option><option value=3 ><%=SystemEnv.getHtmlLabelName(823,user.getLanguage())%></option></select>"},
...
<%
                ...
                if(fieldtype.equals("2") ||fieldtype.equals("19")){
            		...
            		%>
                    <button class=<%=classname %> type=button onclick="onShowBrowser('<%=fieldid%>_<%=subId%>_<%=recorderindex%>','<%=url%>','<%=linkurl%>','<%=fieldtype%>','<%=ismand%>')" title="閫夋嫨"></button>
        ...
        if(fieldhtmltype.equals("1")){                          // 单行文本框
            ...
            if(ismand.equals("1")) {
                ...
                fieldhtml = "<textarea class=InputStyle style='width:90%!important;' name='customfield"+fieldid+"_"+subId+"_\"+rowindex_"+subId+"+\"' rows='4' cols='40' style='width:80%'></textarea>" ;
					...
					if(fieldtype.equals("2") ||fieldtype.equals("19")){
            String url=BrowserComInfo.getBrowserurl(fieldtype);     // 娴忚鎸夐挳寮瑰嚭椤甸潰鐨剈rl
            String linkurl=BrowserComInfo.getLinkurl(fieldtype);    // 娴忚鍊肩偣鍑荤殑鏃跺€欓摼鎺ョ殑url
            String classname = "calendar";
            if(fieldtype.equals("19"))classname = "Clock";
