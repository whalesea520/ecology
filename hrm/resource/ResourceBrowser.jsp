...
int uid=user.getUID();
String ResourceBrowser=(String)session.getAttribute("ResourceBrowser");
if(ResourceBrowser==null){
	Cookie[] cks = request.getCookies();
	for (int i = 0; i < cks.length; i++) {
	  if (cks[i].getName().equals("ResourceBrowser" + uid)) {
	  	ResourceBrowser = cks[i].getValue();
	  	break;
	  }
	}
}
String tabid="0";
if(ResourceBrowser!=null&&ResourceBrowser.length()>0){
	String[] atts=Util.TokenizerString2(ResourceBrowser,"|");
	tabid=atts[0];
}
if(tabid.equals("0")){
	//如果最近使用为空，定位到同部门
			      ...
			      <li class="<%=tabid.equals("4")?"current":"" %>">
			        	...
			        	<%
					      ...
					      if(cansave){
									if(hasSuggest){	
								%>
								<img src="/hrm/group/icon/remind.png" style="vertical-align: middle;padding-left: 5px;margin-bottom: -5px;" onclick="javascript:doRemind(0);">
								<%}else{%>
								<img src="/hrm/group/icon/remind1.png" style="vertical-align: middle;padding-left: 5px;margin-bottom: -5px;" onclick="javascript:doRemind(1);">		
								<%}} %>
			      </li>
			    </ul>
