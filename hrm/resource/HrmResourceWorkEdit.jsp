...
if(CusFormSetting.getStatus()==1){
	...
	//自定义布局页面
	request.getRequestDispatcher("/hrm/resource/HrmResourceWorkEditNew.jsp").forward(request, response);
	return;
...
%>
    ...
    <wea:item>
       ...
       completeUrl="/data.jsp?type=usekind" width="120px"
       browserSpanValue='<%=UseKindComInfo.getUseKindname(usekind)%>'>
     </brow:browser>
   </wea:item>
   <wea:item ><%=SystemEnv.getHtmlLabelName(1970,user.getLanguage())%></wea:item>
   <wea:item>
			  <BUTTON class=Calendar type="button" id=selectcontractdate onclick="getstartdate()"></BUTTON>
            ...
            <wea:item><BUTTON class=Calendar type="button" id=selectcontractbegintime onclick="getDate(probationenddatespan,probationenddate)"></BUTTON>
            ...
            <wea:item> <BUTTON class=Calendar type="button" id=selectenddate onclick="getHendDate()"></BUTTON>
...
<%
                ...
                if(fieldtype.equals("2") ||fieldtype.equals("19")&& !("161".equals(fieldtype) || "162".equals(fieldtype))){
                    showname=fieldvalue; // 日期时间
