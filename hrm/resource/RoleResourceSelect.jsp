...
<%
	...
	String _param = "&lastname="+lastname+"&roleid="+roleid;
...
<HTML><HEAD>
	...
	<script type="text/javascript">
		function showMultiDocDialog(selectids){
			...
			config.srchead=["<%=SystemEnv.getHtmlLabelName(413,user.getLanguage())%>"];
			config.container =$("#colShow");
			config.searchLabel="";
			config.hiddenfield="id";
			config.saveLazy = true;//取消实时保存
			config.srcurl = encodeURI(encodeURI("/hrm/resource/RoleResourceData.jsp?src=src"));
			config.desturl = "/hrm/resource/RoleResourceData.jsp?src=dest";
			config.pagesize = 10;
	...
	<FORM id="SearchForm" NAME="SearchForm" STYLE="margin-bottom:0" action="" onsubmit="return false;" method=post>
		...
		<wea:layout type="4col">
			<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
				...
				<wea:item> 
				  <input class=inputstyle id=lastname name=lastname value="">
				</wea:item>
			</wea:group>
		</wea:layout>
	</form>
