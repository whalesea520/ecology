...
<html>
	...
	<script type="text/javascript" src="/js/hrm/ztree/js/hrmresource_wev8.js?t<%=rand %>"></script>
	...
	function showE8TypeOption(closed){
		...
		if(jQuery("#e8TypeOption").css("display")=="none"){
			...
			jQuery("#e8TypeOption").width(jQuery("span.leftType").width()+14);
			var src = jQuery("#currentImg").attr("src");
			if(src){
				jQuery("#currentImg").attr("src",src);
			}
	...
	function addHoverDom(treeId, treeNode) {
		if((canEditGroup && treeNode.nodeid=="group_-2")||treeNode.nodeid=="group_-3"){
		  var thisClass = "add";
			...
			var addStr = "<span class='button "+thisClass+"' id='addBtn_" + treeNode.tId + "' title='<%=SystemEnv.getHtmlLabelName(611, user.getLanguage())%>' onfocus='this.blur();'></span>";
			sObj.after(addStr);
	...
	<%
		...
		//记录最后一次tabid
		int uid=user.getUID();
		String rem=(String)session.getAttribute("ResourceBrowser");
		if(rem==null){
			Cookie[] cks= request.getCookies();
			for(int i=0;i<cks.length;i++){
				//System.out.println("ck:"+cks[i].getName()+":"+cks[i].getValue());
				if(cks[i].getName().equals("ResourceBrowser"+uid)){
					rem=cks[i].getValue();
					break;
				}
			}
		...
		rem=tabid;
		session.setAttribute("ResourceBrowser",rem);
		Cookie ck = new Cookie("ResourceBrowser"+uid,rem); 
	...
	<table id="topTitle" cellpadding="0" cellspacing="0">
		...
		<div id="dialog" style="width: 670px;">
			<div id="colShow">
				<div class="e8_box_s">
					<table class="e8_box_srctop e8_box_topmenu">
						<thead>
							<tr>
								<td>
									<span id="searchblockspan" style="width: 506px;display: inline-block;">
										<span class="searchInputSpan" style="position:relative;width: 509px">
											...
											<input type="text" class="searchInput middle" id="flowTitle" name="flowTitle" value="<%=qname %>" onkeyup="jsSourceSearch(this)" onfocus="onSearchFocus()" onblur="onSearchFocusLost()" style="vertical-align: top;width: 478px">
					...
					<div class="advancedSearchDiv" id="advancedSearchDiv" style="z-index: 999;position: absolute;width: 100%;margin-top: -3px; ">
						...
						<input type="hidden" id="cmd" name="cmd" value='HrmResourceSelect'>
					  ...
					  <%
						for(ConditionField conditionField: lsConditionField){
							...
							%>
								...
								<wea:item><input class=inputstyle id=lastname name=lastname <%=isReadonly?"readonly":"" %> value='<%=filedvalue %>' style="width: 177px;></wea:item>
							<%}else if(filedname.equals("virtualtype")){%>
							<%if(CompanyVirtualComInfo.getCompanyNum()>0){ %>
					     <wea:item><%=SystemEnv.getHtmlLabelName(34069,user.getLanguage())%></wea:item>
					  	 <wea:item>
							...
							%>
								...
								<wea:item>
											<input class=inputstyle id=jobtitle name=jobtitle maxlength=60 style="width:177px" value="<%=JobTitlesComInfo.getJobTitlesname(filedvalue) %>">
								<!-- 
							...
							%>
								...
								<wea:item>
									<brow:browser viewType="0" temptitle="" name="roleid" browserValue='<%=filedvalue %>'
										...
										hasInput="true"  isSingle="true" hasBrowser = "true" isMustInput='<%=isMustInput %>'
										completeUrl="/data.jsp?type=65" width="177px"
				 ...
				 <%}%>
					<%}else{ %>
						<wea:layout type="2col">
								<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>' attributes="{groupDisplay:none}">
										<wea:item><%=SystemEnv.getHtmlLabelName(413,user.getLanguage())%></wea:item>
										<wea:item><input type="text" class=inputstyle id=lastname name=lastname onkeyup="javascript:jsChangelastname()" style="width: 177px;"></wea:item>
										<%if(CompanyVirtualComInfo.getCompanyNum()>0){ %>
								     <wea:item><%=SystemEnv.getHtmlLabelName(34069,user.getLanguage())%></wea:item>
								  	 <wea:item>
								    	<select id=virtualtype name=virtualtype style="width: 153px">
							      ...
							      <wea:item>
								        ...
								        browserUrl="/systeminfo/BrowserMain.jsp?mouldID=hrm&url=/hrm/company/MutiSubcompanyBrowser.jsp?selectedids="
								        hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1' width="207px"
								        completeUrl="/data.jsp?type=164"
							      ...
							      <wea:item>
								      <brow:browser viewType="0" id="departmentid" name="departmentid" browserValue="" 
										...
										<wea:item><%=SystemEnv.getHtmlLabelName(6086,user.getLanguage())%></wea:item>
										<wea:item>
												<input class=inputstyle id=jobtitle name=jobtitle maxlength=60 style="width:177px">
							      </wea:item>
										<wea:item><%=SystemEnv.getHtmlLabelName(122,user.getLanguage())%></wea:item>
										<wea:item>
							      	<brow:browser viewType="0" id="roleid" name="roleid" browserValue="" 
								        ...
								        hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='1' width="207px"
					...
					<div style="position: absolute;top: 32px;left: 521px;padding-right: 5px;margin-top: <%=!E8EXCEPTHEIGHT?"5":"35" %>px;z-index: 11;">
					...
					<div style="position: absolute;top: 32px;left: 546px;padding-left: 2px;padding-right: 5px;margin-top: <%=!E8EXCEPTHEIGHT?"5":"35" %>px;z-index: 11">
					...
					<%} %>
					<%if(E8EXCEPTHEIGHT){ %>
					<table id="tabvirtualtype" cellspacing="0" cellpadding="0" style="width:100%;">
						<tr>
							<td class="leftTypeSearch" style="border-right: 0px;">
								<div class="topMenuTitle" style="border-bottom:none;width:568px;height: 30px;border-top: 1px solid #DADADA;border-left: 1px solid #DADADA;border-right: 1px solid #DADADA;">
									<span class="leftType" style="width: 558px;height: 0px">
										...
										<span>
											<div  id="e8typeDiv" style="width:auto;height:auto;position:relative;line-height: 0px">
												<span id="optionSpan" style="width: 510px;line-height: 30px" onclick="showE8TypeOption();" ><%=SystemEnv.getHtmlLabelName(83179,user.getLanguage())%></span>
												<%if(CompanyVirtualComInfo.getCompanyNum()>0){ %>
				...
				<%} %>
				<%} %>
					<div class="e8_box_middle ps-container" id="src_box_middle" style="width: 568px;">
...
<div id="hshadowAdvancedSearchOuterDiv" style="top: 228px; height: 300px;width: 100%;margin-left:2px;display: none;position: absolute;background:rgb(220, 226, 241);z-index:2;opacity:0.6"></div>
</body>
</html>
