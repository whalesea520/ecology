...
//added by wcd 2014-07-08 end
  ...
  if(!workID.equals("")){
	...
	if(check_per.substring(check_per.length()-1).equals(","))check_per = check_per.substring(0,check_per.length()-1);
	try{
		String strtmp = "select HrmResource.id,lastname,hrmresourcevirtual.departmentid from HrmResource, hrmresourceVirtual where HrmResource.id=HrmResourcevirtual.resourceid and HrmResource.id in ("+check_per+")";
    if(!initgroupid.equals(""))
    	strtmp = "select HrmResource.id,lastname,hrmresourcevirtual.departmentid from HrmResource, hrmresourceVirtual where HrmResource.id=HrmResourcevirtual.resourceid and HrmResource.id in (select userid from hrmgroupmembers where groupid="+initgroupid+")"; 

		if(!coworkid.equals("")&&isoracle){
      strtmp = "select HrmResource.id,lastname,hrmresourcevirtual.departmentid from HrmResource hrmresourceVirtual where HrmResource.id=HrmResourcevirtual.resourceid and exists (select 1  from cowork_items where id="+coworkid+" and dbms_lob.instr(coworkers,','||hrmresource.id||',',1,1)>0 )"; 
	}
  ...
  RecordSet.executeSql(strtmp);
    ...
    String mark=DepartmentVirtualComInfo.getDepartmentmark(department);
    ...
    String subcid=DepartmentVirtualComInfo.getSubcompanyid1(department);
    String subc= SubCompanyVirtualComInfo.getSubCompanyname(subcid);
  ...
  strtmp = "select HrmResourcemanager.id,lastname, 0 as departmentid from HrmResourcemanager,HrmResourcevirtual where HrmResource.id=HrmResourcevirtual.resourceid and HrmResource.id in ("+check_per+")";
	RecordSet.executeSql(strtmp);
	while(RecordSet.next()){
   String lastname=RecordSet.getString("lastname");
...
if(src.equalsIgnoreCase("dest")){//右侧已选择列表的sql条件	
	if(st!=null){
		while(st.hasMoreTokens()){
			...
			departmentname = DepartmentVirtualComInfo.getDepartmentname(htdp.get(id).toString());
			String tmp_subcompanyid = DepartmentVirtualComInfo.getSubcompanyid1(htdp.get(id).toString());
	...
	if(sqlwhere.length()==0)sqlwhere += " where hrmresource.id=hrmresourcevirtual.resourceid";
	if(!lastname.equals("")){
	...
	sqlwhere +=" and hrmresourcevirtual.subcompanyid in("+allsubId+") ";
	//added by wcd 2014-07-08 end
	...
	sqlwhere+=" and hrmresourcevirtual.departmentid in("+alldeptId+")";
	//added by wcd 2014-07-08 end
	}
	...
	String sqlfrom = "HrmResource, HrmResourcevirtual";
	/*
	if(mode==null||!mode.equals("ldap")){		 
		if(tabid.equals("0")&&!subcompanyid.equals("")){
	...
	spp.setBackFields(" HrmResource.id as id, lastname, hrmresourcevirtual.departmentid, hrmresourcevirtual.subcompanyid, jobtitle, dsporder ");
	...
	while(rs.next()) {
		id = rs.getString("id");
		departmentname = DepartmentVirtualComInfo.getDepartmentname(rs.getString("departmentid"));
		subcompanyname = SubCompanyVirtualComInfo.getSubCompanyname(rs.getString("subcompanyid"));
