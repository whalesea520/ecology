...
<div class="e8_box demo2">
	<div class="e8_boxhead">
        ...
        <div style="height: 100%;">
        	<IFRAME name=frame1 id=frame1   width=100% onload="update();" height=30% frameborder=no scrolling=no onload="update();">
					<%=SystemEnv.getHtmlLabelName(15017, user.getLanguage())%>
					</IFRAME>
					<IFRAME name=frame2 id=frame2 src="/hrm/resource/SelectByDec.jsp?f_weaver_belongto_userid=<%=f_weaver_belongto_userid %>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&tabid=<%=tabid%>&sqlwhere=<%=xssUtil.put(sqlwhere)%><%=wfparam %>" class="flowFrame" width=100%  height=60% frameborder=no scrolling=no>
					<%=SystemEnv.getHtmlLabelName(15017, user.getLanguage())%>
					</IFRAME>
        </div>
	    </div>
...
<script language=javascript>
	...
	function resetbanner(objid){
		...
		if(objid == 0 ){
		        window.frame1.location="/hrm/resource/SingleSearchByOrganDec.jsp?onlyselfdept=<%=onlyselfdept%>&isdetail=<%=isdetail %>&isbill=<%=isbill %>&fieldid=<%=fieldid %>&detachable=1&sqlwhere=<%=xssUtil.put(sqlwhere)%>&departments=<%=departments%>&subcompanyids=<%=subcompanyids%>&isall=<%=isall%>&bdf_wfid=<%=bdf_wfid%>&bdf_fieldid=<%=bdf_fieldid%>&bdf_viewtype=<%=bdf_viewtype%>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&isruledesign=<%=isruledesign%>";
		        try{
			...
			window.frame1.location="/hrm/resource/SingleSearchByGroupDec.jsp?sqlwhere=<%=sqlwhere%>&bdf_wfid=<%=bdf_wfid%>&bdf_fieldid=<%=bdf_fieldid%>&bdf_viewtype=<%=bdf_viewtype%>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>";
			try{
			  //window.frame2.btnsub.style.display="none" //修改之前
			  $(curDoc).find("#btnsub").css("display","none"); //ypc 修改
			}catch(err){}
			...
			window.frame1.location="/hrm/resource/SingleSearchDec.jsp?sqlwhere=<%=sqlwhere%>&bdf_wfid=<%=bdf_wfid%>&bdf_fieldid=<%=bdf_fieldid%>&bdf_viewtype=<%=bdf_viewtype%><%=wfparam%>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>";
			try{
