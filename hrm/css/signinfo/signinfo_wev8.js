function showsigninfo(ele) {
	...
	if (!!ele) {
		...
		__x = offset.left - 140/2 + target.width()/2;
		__y = offset.top + 32;
  ...
  jQuery(".SigninfoImg .SigninfoDiv").hover(function(){
    },function(){
  	window.setTimeout(function(){
  		if(jQuery("#tdSignInfo").data("isOpen")){
  			jQuery("#_browcommgroupblock").remove();
  			jQuery("#tdSignInfo").data("isOpen",false);
  		}
  	},600);
  ...
  jQuery("#_browcommgroupblock").hover(function() {
  	jQuery("#tdSignInfo").data("isOpen",false);
  }, function() {
  	jQuery("#_browcommgroupblock").remove();
  	jQuery("#tdSignInfo").data("isOpen",false);
