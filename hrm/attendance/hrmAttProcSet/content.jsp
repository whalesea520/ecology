...
<!-- Added by wcd 2015-04-22[考勤流程设置-流程概览] -->
<%
	...
	<head>
		...
		<script type="text/javascript">
			...
			function genFormCheck(){
				...
				if(type == "-1"){
					window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(83391,user.getLanguage())%>");
					return;
				} else if(type == "0"){
					title = "<%=SystemEnv.getHtmlLabelNames("670,18015",user.getLanguage())%>";
				} else if(type == "1"){
					title = "<%=SystemEnv.getHtmlLabelNames("20084,18015",user.getLanguage())%>";
				} else if(type == "2"){
					title = "<%=SystemEnv.getHtmlLabelNames("24058,18015",user.getLanguage())%>";
				} else if(type == "3" || type == "4" || type == "5"){
					if(type == "3") {
						title = "<%=SystemEnv.getHtmlLabelNames("6151,18015",user.getLanguage())%>";
					} else if(type == "4") {
						title = "<%=SystemEnv.getHtmlLabelName(127655,user.getLanguage())%>";
					} else if(type == "5") {
						title = "<%=SystemEnv.getHtmlLabelName(125803,user.getLanguage())%>";
					}
			...
			function genForm(type){
				...
				dialog.URL = "/hrm/attendance/hrmAttProcSet/genForm.jsp";
				...
				dialog.OKEvent = function(){
					...
					if(formName == "") {
						...
						jQuery.ajax({
							url : "/hrm/attendance/hrmAttProcSet/genFormSave.jsp",
							type : "post",
							async : true,
			...
			<form id="weaver" name="frmMain" action="/hrm/attendance/hrmAttProcSet/save.jsp" method="post" >
				...
				<wea:layout type="2col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
						...
						<wea:item>
							<brow:browser viewType="0" name="field002" width="255px"
								browserValue='<%=String.valueOf(bean.getField002()) %>' 
								browserSpanValue='<%=field002Name%>'
								hasAdd="true"
								addUrl="/workflow/form/addDefineForm.jsp?dialog=1&ajax=1&isformadd=1"
							...
							&nbsp;&nbsp;&nbsp;&nbsp;<span class='e8_btn_top' id="genform_span" onclick="genFormCheck();"><%=SystemEnv.getHtmlLabelNames("81855,15413",user.getLanguage())%></span>&nbsp;&nbsp;<img src="/wechat/images/remind_wev8.png" align="absMiddle" title="<%=SystemEnv.getHtmlLabelName(82815,user.getLanguage())%>" />
						</wea:item>
						<%
						...
						<wea:item>
							<wea:required id="field006Span" required='<%=bean.getField006()==-1%>'>
								<select id="field006" name="field006" class="inputStyle" onchange="checkinput('field006','field006Span');checkValue(this, 'field006Span');">
									...
									<option value="0" <%=bean.getField006() == 0 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelNames("670,18015",user.getLanguage())%></option>
									<option value="1" <%=bean.getField006() == 1 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelNames("20084,18015",user.getLanguage())%></option>
									<option value="2" <%=bean.getField006() == 2 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelNames("24058,18015",user.getLanguage())%></option>
									<option value="3" <%=bean.getField006() == 3 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelNames("6151,18015",user.getLanguage())%></option>
									<option value="5" <%=bean.getField006() == 5 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelName(125803,user.getLanguage())%></option>
									<option value="4" <%=bean.getField006() == 4 ? "selected" : ""%>><%=SystemEnv.getHtmlLabelName(127655,user.getLanguage())%></option>
								</select>
							</wea:required>
						</wea:item>
					</wea:group>
