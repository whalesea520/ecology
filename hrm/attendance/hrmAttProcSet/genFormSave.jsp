...
<%@ page import="java.util.*,weaver.hrm.attendance.domain.*"%>
  	...
  	if(formid1 < -1) {
  		...
  		try{
			...
			int minSubId = -1;
  			...
  			if(isoracle){
	  			rs.executeSql("create sequence "+formtable_main+"_Id start with 1 increment by 1 nomaxvalue nocycle");
		...
		try{
			...
			for(int i=0; i<list.size(); i++){
				...
				String insertsql="INSERT INTO workflow_billfield(billid,fieldname,fieldlabel,fielddbtype,fieldhtmltype,type,dsporder,viewtype,detailtable,textheight,textheight_2,childfieldid,imgwidth,imgheight) VALUES ("+formid1+",'"+fieldname+"','"+bean.getField003()+"','"+fielddbtype+"','"+bean.getField005()+"','"+bean.getField006()+"','"+bean.getField009()+"',"+0+",'','','','','','')";
