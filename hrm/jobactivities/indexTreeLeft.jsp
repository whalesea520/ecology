...
<%
	...
	String searchStr = URLEncoder.encode(Util.null2String(request.getParameter("searchStr")),"UTF-8");
...
<FORM NAME=SearchForm id="SearchForm" STYLE="margin-bottom: 0" action="HrmCountry.jsp" method=post target="contentframe">
	<table height="100%" width=100% class="">
		<TR>
			<td>
				<div class="e8_loading" id="e8_loading" style="width: 217px;top:0px; ">正在查询，请稍候...</div>
    ...
    <input type="hidden" name="jobactivite" id="jobactivite" value="">
</FORM>
<script type="text/javascript">
	/**
	...
	function getAsyncUrl(treeId, treeNode) {
	    ...
	    if (treeNode != undefined && treeNode.isParent != undefined && treeNode.isParent != null) {
	    	return "/hrm/jobactivities/indexTreeLeftXML.jsp?searchStr=<%=searchStr%>&selectedids=&id=" + treeNode.id + "&type="+treeNode.type+"&" + treeNode.ajaxParam + "&" + new Date().getTime() + "=" + new Date().getTime();
	    } else {
	    	//初始化时
	    	return "/hrm/jobactivities/indexTreeLeftXML.jsp?searchStr=<%=searchStr%><%if(cid!=null){%>&init=true&id=<%=cid%><%}%>";
	    }
	};
	//zTree配置信息
	var setting = {
	...
	function setCountry(nodeId) {
	    ...
	    if(cid == 0){
			jQuery("#SearchForm").attr("action", "/hrm/HrmTab.jsp?_fromURL=jobgroup");
	    }else{
			jQuery("#SearchForm").attr("action", "/hrm/HrmTab.jsp?_fromURL=jobgroup");
	    }
		jQuery("#jobgroup").val(cid);
		jQuery("#jobactivite").val('');
		jQuery("#SearchForm")[0].submit();
	}
	
	function setProvince(nodeId) {
	    var pid=nodeId.substring(nodeId.lastIndexOf("_")+1);
		jQuery("#SearchForm").attr("action", "/hrm/HrmTab.jsp?_fromURL=jobactivitiesView");
		jQuery("#jobgroup").val('');
		jQuery("#jobactivite").val(pid);
