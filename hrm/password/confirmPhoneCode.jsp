...
<HTML>
	<HEAD>
		...
		<script language="javascript">
			...
			// 手机验证码过期 
			var vNumber = Number("<%=phoneCodeSec%>");
			jQuery(document).ready(function(){
				message = "<%=SystemEnv.getHtmlLabelName(128900, languageid)%>"+result+"<br/><%=SystemEnv.getHtmlLabelName(125973, languageid)%>"; 
				showmsg(message);
				pJob();
			})
			...
			function pJob(){
				if(document.all("errorMessage")){
					document.all("errorMessage").innerHTML = "<%=SystemEnv.getHtmlLabelName(128880,languageid)+SystemEnv.getHtmlLabelName(81913,languageid)%>"+(vNumber--)+"<%=SystemEnv.getHtmlLabelName(81914,languageid)%>";
					if(vNumber <= 0){
						document.all("errorMessage").innerHTML = "<%=SystemEnv.getHtmlLabelName(132107, languageid)%>";
	...
	<BODY>
		...
		<form id="weaver" name="frmMain" action="resetPassword.jsp" method="post" onsubmit="return dosubmit();">
			...
			<wea:layout type="2col" needLogin="false" attributes="{'expandAllGroup':'true'}">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("128877,128876", languageid)%>' attributes="{'groupOperDisplay':'none'}">
					<wea:item><%=SystemEnv.getHtmlLabelName(128876, languageid)%></wea:item>
					<wea:item>
					<table style="width:100%">
							<tr>
								...
								<td style="width:15%;">
		    						<input type="button" value="<%=SystemEnv.getHtmlLabelName(125977, languageid)%>" name="sendSmsBtn" style="display:none;" class="e8_btn_top" onclick="sendSms();">
								</td>
							</tr>
						</table>
					</wea:item>
