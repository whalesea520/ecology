...
<%
	...
	String titlename = SystemEnv.getHtmlLabelName(17617,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(isShow?221:(groupid!=-1?93:365),user.getLanguage());
	String needfav ="1";
	String needhelp ="";
	...
	<HEAD>
		...
		<script type="text/javascript">
			...
			if("<%=isclose%>"=="1"){
		 	...
		 	function doSave(){
		    	if(check_form(document.frmMain,'name,hrmids')){
		...
		<table id="topTitle" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="rightSearchSpan" style="text-align:right;">
					...
					<%if(type.equals("1")){%>
					<input type="button" value="<%=SystemEnv.getHtmlLabelName(119,user.getLanguage())%>" class="e8_btn_top" onclick="doShare();">
					<%}%>
					<%}%>
		...
		<FORM id=weaver name=frmMain action="GroupOperation.jsp" method=post >
			<wea:layout type="2col">
				<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'>
					...
					<wea:item>
						<%if(isShow){out.println(hrmnames);}else{%>
						<brow:browser viewType="0" name="hrmids" browserValue='<%=hrmids%>' 
							...
							completeUrl="/data.jsp" width="60%" browserSpanValue='<%=hrmnames%>' >
						</brow:browser>
						<%}%>
					</wea:item>
				</wea:group>
