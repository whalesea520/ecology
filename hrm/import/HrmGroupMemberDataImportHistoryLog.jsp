...
<%
	...
	if("downLoad".equals(option)){
		...
		String securityFilePath = FileSecurityTool.get(filePath);
	 	...
	 	File file = new File(securityFilePath); 
	 	if(file.exists() && file.isFile()){ 
		 	try { 
				String fileName=securityFilePath.substring(securityFilePath.lastIndexOf("/")+1);
			 	BufferedReader bis=new BufferedReader(new FileReader(securityFilePath));
...
<HEAD>
	...
	<script type="text/javascript">
		...
		function downLoadLog(path){
  		jQuery("#downLoad").attr("src","/hrm/import/HrmGroupMemberDataImportHistoryLog.jsp?option=downLoad&path="+encodeURI(path));
		}

		function deleteLog(path,obj){
			window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(23271,user.getLanguage())%>",function(){
				jQuery.post("/hrm/import/HrmGroupMemberDataImportHistoryLog.jsp?option=delete&path="+encodeURI(path),function(data){
