...
<table id="topTitle" cellpadding="0" cellspacing="0">
	...
	<td class="rightSearchSpan" style="text-align:right;">
		<input type=button class="e8_btn_top" onClick="dosubmit(this)" value="<%=SystemEnv.getHtmlLabelName(86, user.getLanguage())%>">
		<span title="<%=SystemEnv.getHtmlLabelName(23036,user.getLanguage())%>" class="cornerMenu"></span>
	</td>
	</tr>
</table>
<FORM id=frmMain name=frmMain action="HrmGroupMemberDataImportProcess.jsp" method=post enctype="multipart/form-data" target="subframe">
...
<input id="userlanguage" name="userlanguage" type="hidden" value="<%=user.getLanguage() %>">
	<wea:layout type="2col">
      ...
      <wea:item>
        <%if(importtype.equals("groupmember")){ %>
        <select name="importlx" id="importlx" style="width: 80px;" onchange="changeImportlx(this)" >
           <option value="add"><%=SystemEnv.getHtmlLabelName(611, user.getLanguage())%></option>
           <option value="update"><%=SystemEnv.getHtmlLabelName(31260, user.getLanguage())%></option>
        </select>
        <span id="importlxSpan" style="display: none;color:#FF3497;margin-left:10px;" ><%=SystemEnv.getHtmlLabelName(131396, user.getLanguage()) %> </span>
      <%}else{%>
        <select name="importlx" id="importlx" style="width: 80px;" >
           <option value="add"><%=SystemEnv.getHtmlLabelName(611, user.getLanguage())%></option>
           <option value="update"><%=SystemEnv.getHtmlLabelName(31260, user.getLanguage())%></option>
        </select>
      <%} %>
      ...
      <wea:item><a href='templet/<%=importtype %>.xls' class="templetfile" style="color: #30b5ff"><%=SystemEnv.getHtmlLabelName(28576, user.getLanguage())%></a>
       请下载模板文件，填入数据后导入!
       </wea:item>
	...
	<wea:layout>
		<wea:group context="<%=SystemEnv.getHtmlLabelName(18596,user.getLanguage())+SystemEnv.getHtmlLabelName(85,user.getLanguage())%>" attributes="{'samePair':'RemarkInfo','groupOperDisplay':'none','itemAreaDisplay':'block','groupSHBtnDisplay':'none'}">
		  ...
		  模板中的字段根据选择的“重复验证字段”来判断必填，哪个作为“重复验证字段”，哪个就必填。
		</wea:item>
		</wea:group>
	</wea:layout>	
	<!-- 隐藏提交iframe -->
...
function dosubmit(obj) {
  if(check_form(document.frmMain)) {
  	var dialog = new window.top.Dialog();
		...
		dialog.Height = 400;
		dialog.Drag = true;
		dialog.URL = "/hrm/HrmDialogTab.jsp?_fromURL=HrmGroupMemberDataImportLog&importtype="+jQuery("#importtype").val();
		dialog.show();
...
function changeImportlx(obj){
	jQuery("#importlxSpan").hide();
	var type = jQuery.trim(jQuery(obj).val());
	if("update" == type){
		jQuery("#importlxSpan").show();
	}
}
