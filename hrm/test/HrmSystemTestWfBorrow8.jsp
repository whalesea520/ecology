...
String sql = "select * from (select a.id,a.field005,a.field006,a.field004, a.field001, b.formid, b.workflowname, b.custompage, b.custompage4Emoble,b.version, " +
		...
		" join workflow_base b on a.field001 = b.id  ) a  where 1 = 1 and field006 = 2 "+
...
while(rs.next()){
	...
	if(mapCurFields == null || mapCurFields.isEmpty()){
		...
		if(mapCurFields.get("15") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(124927,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("16") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(6122,user.getLanguage())+"没有对应!");
		}
	...
	int hiredate_ismandatory_nodeType0   =0;
	int hirereason_ismandatory_nodeType0   =0;
	int pmanager_ismandatory_nodeType0   =0;
	
	int resource_n_type = 0;
	int hiredate_type = 0;
	int hirereason_type = 0;
	int pmanager_type = 0;
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			if(mapCurFields.get("15") != null && mapCurFields.get("15").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("16") != null && mapCurFields.get("16").equals(""+fieldId) ){
				if(ismandatory==1){
					hiredate_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					hiredate_ismandatory_nodeType0 = 1;
				}
			...
			if(mapCurFields.get("17") != null && mapCurFields.get("17").equals(""+fieldId) ){
				if(fieldhtmltype == 2 && fieldtype == 1){
				}else{
					hirereason_type = -1;
				}
			}
			if(mapCurFields.get("18") != null && mapCurFields.get("18").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 1){
				}else{
					pmanager_type = -1;
				}
	...
	//试用-节点前附加操作
	String deductBorrowNode1Ids = getActionSetNodeIds(workflowid,1,1, "HrmResourceHire");
	//试用-节点后附加操作
	String deductBorrowNode2Ids = getActionSetNodeIds(workflowid,1,0, "HrmResourceHire");
	//试用-出口附加操作
	String deductBorrowNode3Ids = getActionSetNodeIds(workflowid,0,0, "HrmResourceHire");
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【转正流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if("".equals(deductBorrowNode1Ids) && "".equals(deductBorrowNode2Ids) && "".equals(deductBorrowNode3Ids)){
		tableSql.append("尚未配置【转正】动作，请进行动作设置，否则无法转正变更！"+brStr);
	}
	if(resource_n_ismandatory_nodeType0!=2){
		tableSql.append("【转正人员】必须是必填字段，请检查！"+brStr);
	}
	if(hiredate_ismandatory_nodeType0!=2){
		tableSql.append("【转正日期】必须是必填字段，请检查！"+brStr);
	}
	if(resource_n_type <0 ){
		tableSql.append("【转正人员】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
	if(hiredate_type <0 ){
		tableSql.append("【转正日期】必须是日期浏览按钮，请检查！"+brStr);
	}
	if(hirereason_type <0 ){
		tableSql.append("【转正备注】必须是多行文本框，请检查！"+brStr);
	}
	if(pmanager_type <0 ){
		tableSql.append("【转正通知人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow8.jsp";
}
function exception(){
