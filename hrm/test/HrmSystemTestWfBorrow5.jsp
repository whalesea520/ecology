...
while(rs.next()){
	...
	if(mapCurFields == null || mapCurFields.isEmpty()){
		...
		if(mapCurFields.get("38") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(368,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("39") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(125842,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("40") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("125843",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("41") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("125844",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("42") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("125845",user.getLanguage())+"没有对应!");
		}
	...
	int scheduleResourceId_ismandatory_nodeType0  =0;
	int fromdate_ismandatory_nodeType0      =0;
	int tilldate_ismandatory_nodeType0        =0;
	int newShift_ismandatory_nodeType0        =0;

	int resourceId_type = 0;
	int scheduleResourceId_type = 0;
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			if(mapCurFields.get("38") != null && mapCurFields.get("38").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("39") != null && mapCurFields.get("39").equals(""+fieldId) ){
				if(ismandatory==1){
					scheduleResourceId_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					scheduleResourceId_ismandatory_nodeType0 = 1;
				}
				if(fieldtype != 1){
					scheduleResourceId_type = -1;
				}
			}
			if(mapCurFields.get("40") != null && mapCurFields.get("40").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("41") != null && mapCurFields.get("41").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("42") != null && mapCurFields.get("42").equals(""+fieldId) ){
				if(ismandatory==1){
					newShift_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					newShift_ismandatory_nodeType0 = 1;
				}
				if(fieldtype != 280){
					newShift_type = -1;
				}
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【调班流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if(resourceId_ismandatory_nodeType0!=2){
		tableSql.append("【申请人】必须是必填字段，请检查！"+brStr);
	}
	if(scheduleResourceId_ismandatory_nodeType0!=2){
		tableSql.append("【调班人】必须是必填字段，请检查！"+brStr);
	}
	if(fromdate_ismandatory_nodeType0 !=2){
		tableSql.append("【调班开始日期】必须是必填字段，请检查！"+brStr);
	}
	if(tilldate_ismandatory_nodeType0 !=2){
		tableSql.append("【调班结束日期】必须是必填字段，请检查！"+brStr);
	}
	if(newShift_ismandatory_nodeType0 !=2){
		tableSql.append("【新班次】必须是必填字段，请检查！"+brStr);
	}
	if(resourceId_type<0){
		tableSql.append("【申请人】必须是单人力浏览按钮，请检查！"+brStr);
	}
	if(scheduleResourceId_type<0){
		tableSql.append("【调班人】必须是单部门浏览按钮，请检查！"+brStr);
	}
	//TODO  增加日期字段的校验
	if(fromDate_type <0 ){
		tableSql.append("【调班开始日期】必须是日期类型，请检查！"+brStr);
	}
	if(toDate_type <0 ){
		tableSql.append("【调班结束日期】必须是日期类型，请检查！"+brStr);
	}
	if(newShift_type <0 ){
		tableSql.append("【新班次】必须是班次浏览按钮，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow5.jsp";
}
function exception(){
