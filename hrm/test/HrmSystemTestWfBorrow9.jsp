...
while(rs.next()){
	...
	String fieldstatus="";
	String fieldchangedate="";
	String fieldchangeenddate="";
	String fieldchangereason="";
	String fieldchangecontractid="";
	String fieldinfoman="";
	...
	if(mapCurFields == null || mapCurFields.isEmpty()){
		...
		if(mapCurFields.get("68") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(16079,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("69") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(16080,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("70") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(15970,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("71") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(15971,user.getLanguage())+"没有对应!");
		}
	...
	int changedate_ismandatory_nodeType0   =0;
	int changeenddate_ismandatory_nodeType0   =0;
	int changereason_ismandatory_nodeType0   =0;
	int changecontractid_ismandatory_nodeType0   =0;
	int infoman_ismandatory_nodeType0   =0;
	
	int resourceid_type = 0;
	int status_type = 0;
	int changedate_type = 0;
	int changeenddate_type = 0;
	int changereason_type = 0;
	int changecontractid_type = 0;
	int infoman_type = 0;
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			if(mapCurFields.get("68") != null && mapCurFields.get("68").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("69") != null && mapCurFields.get("69").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("70") != null && mapCurFields.get("70").equals(""+fieldId) ){
				if(ismandatory==1){
					changedate_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					changedate_ismandatory_nodeType0 = 1;
				}
			...
			if(mapCurFields1.get("71") != null && mapCurFields1.get("71").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 2){
				}else{
					changeenddate_type = -1;
				}
			}
			if(mapCurFields1.get("72") != null && mapCurFields1.get("72").equals(""+fieldId) ){
				if(fieldhtmltype == 2 && fieldtype == 1){
				}else{
					changereason_type = -1;
				}
			}
			if(mapCurFields1.get("73") != null && mapCurFields1.get("73").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 279){
				}else{
					changecontractid_type = -1;
				}
			}
			if(mapCurFields1.get("74") != null && mapCurFields1.get("74").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 1){
				}else{
					infoman_type = -1;
				}
	...
	//试用-节点前附加操作
	String deductBorrowNode1Ids = getActionSetNodeIds(workflowid,1,1, "HrmResourceExtend");
	//试用-节点后附加操作
	String deductBorrowNode2Ids = getActionSetNodeIds(workflowid,1,0, "HrmResourceExtend");
	//试用-出口附加操作
	String deductBorrowNode3Ids = getActionSetNodeIds(workflowid,0,0, "HrmResourceExtend");
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【续签流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if("".equals(deductBorrowNode1Ids) && "".equals(deductBorrowNode2Ids) && "".equals(deductBorrowNode3Ids)){
		tableSql.append("尚未配置【续签】动作，请进行动作设置，否则无法续签变更！"+brStr);
	}
	
	if(resourceid_ismandatory_nodeType0!=2){
		tableSql.append("【被续签人】必须是必填字段，请检查！"+brStr);
	}
	if(status_ismandatory_nodeType0!=2){
		tableSql.append("【续签状态】必须是必填字段，请检查！"+brStr);
	}
	if(changedate_ismandatory_nodeType0!=2){
		tableSql.append("【续签日期】必须是必填字段，请检查！"+brStr);
	}
	if(changeenddate_ismandatory_nodeType0!=2){
		tableSql.append("【续签结束日期】必须是必填字段，请检查！"+brStr);
	}
	if(resourceid_type <0 ){
		tableSql.append("【被续签人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
	if(status_type <0 ){
		tableSql.append("【续签状态】必须是下拉框，请检查！"+brStr);
	}
	if(changedate_type <0 ){
		tableSql.append("【续签日期】必须是日期浏览按钮，请检查！"+brStr);
	}
	if(changeenddate_type <0 ){
		tableSql.append("【续签结束日期】必须是日期浏览按钮，请检查！"+brStr);
	}	
	if(changereason_type <0 ){
		tableSql.append("【续签备注】必须是多行文本框，请检查！"+brStr);
	}
	if(changecontractid_type <0 ){
		tableSql.append("【续签合同】必须是合同浏览按钮，请检查！"+brStr);
	}
	if(infoman_type <0 ){
		tableSql.append("【续签通知人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow9.jsp";
}
function exception(){
