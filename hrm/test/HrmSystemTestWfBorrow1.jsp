...
String sql = "select * from (select a.id,a.field005,a.field006,a.field004, a.field001, b.formid, b.workflowname, b.custompage, b.custompage4Emoble,b.version, " +
		...
		" join workflow_base b on a.field001 = b.id  ) a  where 1 = 1 and field006 = 3 "+
...
while(rs.next()){
	...
	while(rs2.next()){
		...
		//针对加班类型
		if("30".equals(rs2.getString("mfid"))){
			...
			if(rs3.next()){
				otype_min_selectvalue = rs3.getInt("min_selectvalue");
				otype_max_selectvalue = rs3.getInt("max_selectvalue");
			}
		}
	}
	...
	if(mapCurFields == null || mapCurFields.isEmpty()){
		...
		if(mapCurFields.get("26") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("6151,740",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("27") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("6151,742",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("28") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("6151,741",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("29") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelNames("6151,743",user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("30") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(6159,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("37") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(125592,user.getLanguage())+"没有对应!");
		}
	...
	int otype_ismandatory_nodeType0     =0;
	int overtimeDays_ismandatory_nodeType0  =0;

	int resourceId_type = 0;
	int departmentId_type = 0;
	int otype_type = 0;
	int overtimeDays_type = 0;
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			...
			if(mapCurFields.get("27") != null && mapCurFields.get("27").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("28") != null && mapCurFields.get("28").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("29") != null && mapCurFields.get("29").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("30") != null && mapCurFields.get("30").equals(""+fieldId) ){
				if(ismandatory==1){
					otype_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					otype_ismandatory_nodeType0 = 1;
				}
			}
			if(mapCurFields.get("37") != null && mapCurFields.get("37").equals(""+fieldId) ){
				if(ismandatory==1){
					overtimeDays_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					overtimeDays_ismandatory_nodeType0 = 1;
				}
			}
		}
	...
	//加班转调休-节点前附加操作
	String HrmPaidLeaveBorrowNode1Ids = getActionSetNodeIds(workflowid,1,1, "HrmPaidLeaveAction");
	//加班转调休-节点后附加操作
	String HrmPaidLeaveBorrowNode2Ids = getActionSetNodeIds(workflowid,1,0, "HrmPaidLeaveAction");
	//加班转调休-出口附加操作
	String HrmPaidLeaveBorrowNode3Ids = getActionSetNodeIds(workflowid,0,0, "HrmPaidLeaveAction");
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【加班流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if(formid >= 0){
		tableSql.append("当前流程使用的是系统请假单,加班不推荐使用系统表单，请重新选择！"+brStr);
	}else{
		if("".equals(custompage)){
			tableSql.append("PC端流程模板没有设置，请重新保存！"+brStr);
		}else{
			if(custompage.indexOf("/workflow/request/ext/HrmCustomOverTimeWork.jsp") < 0){
				tableSql.append("PC端流程模板值不是HrmCustomOverTimeWork.jsp，请确保没有做过二次开发，请重新保存！"+brStr);
			}
		}
		if("".equals(custompage4Emoble)){
			tableSql.append("移动端流程模板没有设置，请重新保存！"+brStr);
		}else{
			if(custompage4Emoble.indexOf("/workflow/request/ext/HrmCustomOverTimeWork4Mobile.jsp") < 0){
				tableSql.append("移动端流程模板值不是HrmCustomOverTimeWork4Mobile.jsp，请确保没有做过二次开发，请重新保存！"+brStr);
			}
		}
		if("".equals(HrmPaidLeaveBorrowNode1Ids) && "".equals(HrmPaidLeaveBorrowNode2Ids) && "".equals(HrmPaidLeaveBorrowNode3Ids)){
			tableSql.append("尚未配置【加班转调休】动作，请进行动作设置，否则无法进行加班转调休！"+brStr);
		}
	}
	...
	if(fromdate_ismandatory_nodeType0 !=2){
		tableSql.append("【加班开始日期】必须是必填字段，请检查！"+brStr);
	}
	if(fromtime_ismandatory_nodeType0 !=2){
		tableSql.append("【加班开始时间】必须是必填字段，请检查！"+brStr);
	}
	if(tilldate_ismandatory_nodeType0 !=2){
		tableSql.append("【加班结束日期】必须是必填字段，请检查！"+brStr);
	}
	if(tilltime_ismandatory_nodeType0 !=2){
		tableSql.append("【加班结束时间】必须是必填字段，请检查！"+brStr);
	}	
	if(otype_ismandatory_nodeType0  !=2){
		tableSql.append("【加班类型】必须是必填字段，请检查！"+brStr);
	}	
	if(otype_min_selectvalue <0 || otype_max_selectvalue >1){
		tableSql.append("【加班类型】可选项范围的值默认是0：关联调休；1：不关联调休，只有下拉框的值为0的时候才会转换成调休，请检查！"+brStr);
	}
	...
	//TODO  增加日期字段的校验
	if(fromDate_type <0 ){
		tableSql.append("【加班开始日期】必须是日期类型，请检查！"+brStr);
	}
	if(fromTime_type <0 ){
		tableSql.append("【加班开始时间】必须是时间类型，请检查！"+brStr);
	}
	if(toDate_type <0 ){
		tableSql.append("【加班结束日期】必须是日期类型，请检查！"+brStr);
	}
	if(toTime_type <0 ){
		tableSql.append("【加班结束时间】必须是时间类型，请检查！"+brStr);
	}
	if(mapCurFields.get("26") != null && mapCurFields.get("28") != null && (mapCurFields.get("26").equals(mapCurFields.get("28")) ) ){
		tableSql.append("【加班开始日期】和【加班结束日期】考勤字段对应的是同一个字段，请检查！"+brStr);
	}
	if(mapCurFields.get("27") != null && mapCurFields.get("29") != null && (mapCurFields.get("27").equals(mapCurFields.get("29")) ) ){
		tableSql.append("【加班开始时间】和【加班结束时间】考勤字段对应的是同一个字段，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow1.jsp";
}
function exception(){
...
function showContent(id, field006) {
	...
	closeDialog();
	dialog = common.showDialog("/hrm/attendance/hrmAttProcSet/tab.jsp?topage=content&subcompanyid=&isDialog=1&id="+id+"&field006=3", (id == "" ? "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())%>" : "<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())%>") + "<%=SystemEnv.getHtmlLabelNames("15880,18015",user.getLanguage())%>");
}
