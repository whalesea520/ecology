...
while(rs.next()){
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			...
			if(mapCurFields.get("19") != null && mapCurFields.get("19").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("20") != null && mapCurFields.get("20").equals(""+fieldId) ){
				if(ismandatory==1){
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【公出流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if(fromdate_ismandatory_nodeType0 !=2){
		tableSql.append("【公出开始日期】必须是必填字段，请检查！"+brStr);
	}
	if(fromtime_ismandatory_nodeType0 !=2){
		tableSql.append("【公出开始时间】必须是必填字段，请检查！"+brStr);
	}
	if(tilldate_ismandatory_nodeType0 !=2){
		tableSql.append("【公出结束日期】必须是必填字段，请检查！"+brStr);
	}
	if(tilltime_ismandatory_nodeType0 !=2){
		tableSql.append("【公出结束时间】必须是必填字段，请检查！"+brStr);
	}	
	...
	//TODO  增加日期字段的校验
	if(fromDate_type <0 ){
		tableSql.append("【公出开始日期】必须是日期类型，请检查！"+brStr);
	}
	if(fromTime_type <0 ){
		tableSql.append("【公出开始时间】必须是时间类型，请检查！"+brStr);
	}
	if(toDate_type <0 ){
		tableSql.append("【公出结束日期】必须是日期类型，请检查！"+brStr);
	}
	if(toTime_type <0 ){
		tableSql.append("【公出结束时间】必须是时间类型，请检查！"+brStr);
	}
	if(mapCurFields.get("20") != null && mapCurFields.get("22") != null && (mapCurFields.get("20").equals(mapCurFields.get("22")) ) ){
		tableSql.append("【公出开始日期】和【公出结束日期】考勤字段对应的是同一个字段，请检查！"+brStr);
	}
	if(mapCurFields.get("21") != null && mapCurFields.get("23") != null && (mapCurFields.get("21").equals(mapCurFields.get("23")) ) ){
		tableSql.append("【公出开始时间】和【公出结束时间】考勤字段对应的是同一个字段，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow3.jsp";
}
function exception(){
