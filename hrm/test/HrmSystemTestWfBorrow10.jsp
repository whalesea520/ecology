...
while(rs.next()){
	...
	String fieldredeploydate="";
	String fieldoldjob="";
	String fieldnewjob="";
	String fieldoldjoblevel="";
	String fieldnewjoblevel="";
	String fieldredeployreason="";
	String fieldischangesalary="";
	...
	if(mapCurFields == null || mapCurFields.isEmpty()){
		...
		if(mapCurFields.get("1") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(16001,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("2") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(6111,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("3") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(6112,user.getLanguage())+"没有对应!");
		}
		if(mapCurFields.get("4") == null){
			fieldErrorString.append(SystemEnv.getHtmlLabelName(6113,user.getLanguage())+"没有对应!");
		}
	...
	int redeploydate_ismandatory_nodeType0   =0;
	int oldjob_ismandatory_nodeType0   =0;
	int newjob_ismandatory_nodeType0   =0;
	int oldjoblevel_ismandatory_nodeType0   =0;
	int newjoblevel_ismandatory_nodeType0   =0;
	int redeployreason_ismandatory_nodeType0   =0;
	int ischangesalary_ismandatory_nodeType0   =0;
	int pmanager_ismandatory_nodeType0   =0;
	int departmentid_ismandatory_nodeType0   =0;
	int infoman_ismandatory_nodeType0   =0;
	
	int resource_n_type = 0;
	int redeploydate_type = 0;
	int oldjob_type = 0;
	int newjob_type = 0;
	int oldjoblevel_type = 0;
	int newjoblevel_type = 0;
	int redeployreason_type = 0;
	int ischangesalary_type = 0;
	int pmanager_type = 0;
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			...
			if(mapCurFields.get("2") != null && mapCurFields.get("2").equals(""+fieldId) ){
				if(ismandatory==1){
					redeploydate_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					redeploydate_ismandatory_nodeType0 = 1;
				}
			...
			if(mapCurFields.get("3") != null && mapCurFields.get("3").equals(""+fieldId) ){
				if(ismandatory==1){
					oldjob_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					oldjob_ismandatory_nodeType0 = 1;
				}
			...
			if(mapCurFields.get("4") != null && mapCurFields.get("4").equals(""+fieldId) ){
				if(ismandatory==1){
					newjob_ismandatory_nodeType0 = 2;
				}else if(isedit==1){
					newjob_ismandatory_nodeType0 = 1;
				}
				if(fieldhtmltype == 3 && fieldtype == 24){
				}else{
					newjob_type = -1;
			...
			if(mapCurFields1.get("5") != null && mapCurFields1.get("5").equals(""+fieldId) ){
				if(fieldhtmltype == 1 && fieldtype == 2){
				}else{
					oldjoblevel_type = -1;
				}
			}
			if(mapCurFields1.get("6") != null && mapCurFields1.get("6").equals(""+fieldId) ){
				if(fieldhtmltype == 1 && fieldtype == 2){
				}else{
					newjoblevel_type = -1;
				}
			}
			if(mapCurFields1.get("7") != null && mapCurFields1.get("7").equals(""+fieldId) ){
				if(fieldhtmltype == 2){
				}else{
					redeployreason_type = -1;
				}
			}			
			if(mapCurFields1.get("8") != null && mapCurFields1.get("8").equals(""+fieldId) ){
				if(fieldhtmltype == 4){
				}else{
					ischangesalary_type = -1;
				}
			}
			if(mapCurFields1.get("9") != null && mapCurFields1.get("9").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 1){
				}else{
					pmanager_type = -1;
				}
			}
			if(mapCurFields1.get("19") != null && mapCurFields1.get("19").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 4){
				}else{
					departmentid_type = -1;
				}
			}
			if(mapCurFields1.get("20") != null && mapCurFields1.get("20").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 1){
				}else{
					infoman_type = -1;
				}
	...
	//试用-节点前附加操作
	String deductBorrowNode1Ids = getActionSetNodeIds(workflowid,1,1, "HrmResourceRedeploy");
	//试用-节点后附加操作
	String deductBorrowNode2Ids = getActionSetNodeIds(workflowid,1,0, "HrmResourceRedeploy");
	//试用-出口附加操作
	String deductBorrowNode3Ids = getActionSetNodeIds(workflowid,0,0, "HrmResourceRedeploy");
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【调动流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if("".equals(deductBorrowNode1Ids) && "".equals(deductBorrowNode2Ids) && "".equals(deductBorrowNode3Ids)){
		tableSql.append("尚未配置【调动】动作，请进行动作设置，否则无法调动变更！"+brStr);
	}
	
	if(resource_n_ismandatory_nodeType0!=2){
		tableSql.append("【被调动人】必须是必填字段，请检查！"+brStr);
	}
	if(redeploydate_ismandatory_nodeType0!=2){
		tableSql.append("【调动日期】必须是必填字段，请检查！"+brStr);
	}
	if(oldjob_ismandatory_nodeType0!=2){
		tableSql.append("【原岗位】必须是必填字段，请检查！"+brStr);
	}
	if(newjob_ismandatory_nodeType0!=2){
		tableSql.append("【新岗位】必须是必填字段，请检查！"+brStr);
	}
	if(resource_n_type <0 ){
		tableSql.append("【被调动人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
	if(redeploydate_type <0 ){
		tableSql.append("【调动日期】必须是日期浏览按钮，请检查！"+brStr);
	}
	if(oldjob_type <0 ){
		tableSql.append("【原岗位】必须是岗位浏览按钮，请检查！"+brStr);
	}
	if(newjob_type <0 ){
		tableSql.append("【新岗位】必须是岗位浏览按钮，请检查！"+brStr);
	}	
	if(oldjoblevel_type <0 ){
		tableSql.append("【原职级】必须是多行文本框,整数，请检查！"+brStr);
	}
	if(newjoblevel_type <0 ){
		tableSql.append("【新职级】必须是多行文本框,整数，请检查！"+brStr);
	}
	if(redeployreason_type <0 ){
		tableSql.append("【调动原因】必须是多行文本框，请检查！"+brStr);
	}
	if(ischangesalary_type <0 ){
		tableSql.append("【是否重新设置基准工资】必须是Check框，请检查！"+brStr);
	}
	if(pmanager_type <0 ){
		tableSql.append("【调动后上级】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
	if(departmentid_type <0 ){
		tableSql.append("【新部门】必须是部门浏览按钮，请检查！"+brStr);
	}
	if(infoman_type <0 ){
		tableSql.append("【调动通知人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow10.jsp";
}
function exception(){
