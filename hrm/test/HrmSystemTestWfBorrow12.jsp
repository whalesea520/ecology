...
String sql = "select * from (select a.id,a.field005,a.field006,a.field004, a.field001, b.formid, b.workflowname, b.custompage, b.custompage4Emoble,b.version, " +
		...
		" join workflow_base b on a.field001 = b.id  ) a  where 1 = 1 and field006 = 6 "+
...
String fieldinfoman="";
	...
	while(rs1.next()){
		...
		if(mapCurFields != null && !mapCurFields.isEmpty()){
			...
			if(mapCurFields.get("75") != null && mapCurFields.get("75").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields.get("76") != null && mapCurFields.get("76").equals(""+fieldId) ){
				if(ismandatory==1){
			...
			if(mapCurFields1.get("77") != null && mapCurFields1.get("77").equals(""+fieldId) ){
				if(fieldhtmltype == 2){
				}else{
					changereason_type = -1;
				}
			}
			if(mapCurFields1.get("78") != null && mapCurFields1.get("78").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 279){
				}else{
					changecontractid_type = -1;
				}
			}
			if(mapCurFields1.get("79") != null && mapCurFields1.get("79").equals(""+fieldId) ){
				if(fieldhtmltype == 3 && fieldtype == 1){
				}else{
					infoman_type = -1;
				}
	...
	//试用-节点前附加操作
	String deductBorrowNode1Ids = getActionSetNodeIds(workflowid,1,1, "HrmResourceRetire");
	//试用-节点后附加操作
	String deductBorrowNode2Ids = getActionSetNodeIds(workflowid,1,0, "HrmResourceRetire");
	//试用-出口附加操作
	String deductBorrowNode3Ids = getActionSetNodeIds(workflowid,0,0, "HrmResourceRetire");
	...
	if(versionErrorString.length() > 0){
		tableSql.append("当前流程的以下版本未关联【退休流程】"+versionErrorString.toString()+"，请检查！"+brStr);
	}
	if(fieldErrorString.length() > 0){
	...
	if("".equals(deductBorrowNode1Ids) && "".equals(deductBorrowNode2Ids) && "".equals(deductBorrowNode3Ids)){
		tableSql.append("尚未配置【退休】动作，请进行动作设置，否则无法退休变更！"+brStr);
	}
	
	if(resourceid_ismandatory_nodeType0!=2){
		tableSql.append("【退休人】必须是必填字段，请检查！"+brStr);
	}
	if(changedate_ismandatory_nodeType0!=2){
		tableSql.append("【退休日期】必须是必填字段，请检查！"+brStr);
	}
	
	if(resourceid_type <0 ){
		tableSql.append("【退休人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
	if(changedate_type <0 ){
		tableSql.append("【退休日期】必须是日期浏览按钮，请检查！"+brStr);
	}
	if(changecontractid_type <0 ){
		tableSql.append("【退休合同】必须是合同浏览按钮，请检查！"+brStr);
	}
	if(changereason_type <0 ){
		tableSql.append("【退休备注】必须多行文本框，请检查！"+brStr);
	}	
	if(infoman_type <0 ){
		tableSql.append("【退休通知人】必须是单人力资源浏览按钮，请检查！"+brStr);
	}
...
function doRefresh(){
	window.location.href="/hrm/test/HrmSystemTestWfBorrow12.jsp";
}
function exception(){
