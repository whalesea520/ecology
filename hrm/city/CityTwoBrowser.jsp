...
<HEAD>
	...
	<script type="text/javascript">
		try{
			parent.setTabObjName("<%=SystemEnv.getHtmlLabelNames("25223",user.getLanguage())%>");
		}catch(e){
			if(window.console)console.log(e+"-->CityTwoBrowser.jsp");
		}
		var parentWin = null;
		var dialog = null;
		try{
...
<script type="text/javascript">
	...
	function getAsyncUrl(treeId, treeNode) {
	    ...
	    if (treeNode != undefined && treeNode.isParent != undefined && treeNode.isParent != null) {
	    	return "/hrm/tree/CityTwoSingleXML.jsp?" + treeNode.ajaxParam + "&" + new Date().getTime() + "=" + new Date().getTime();
	    } else {
	    	//初始化时
	    	return "/hrm/tree/CityTwoSingleXML.jsp?init=true&type=glob" + "&" + new Date().getTime() + "=" + new Date().getTime();
	    }
	};
	//zTree配置信息
	var setting = {
