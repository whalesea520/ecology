...
function doDel(id){
	...
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
		...
		for(var i=0;i<idArr.length;i++){
			ajaxNum++;
			jQuery.ajax({
				url:"SpecialityOperation.jsp?isdialog=1&operation=delete&id="+idArr[i],
				type:"post",
				async:true,
...
function openDialog(id){
	...
	var url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmSpecialityAdd&isdialog=1";
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(26473,user.getLanguage())+SystemEnv.getHtmlLabelName(803,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmSpecialityEdit&isdialog=1&id="+id;
	}else{
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(803,user.getLanguage())%>";
	}
	dialog.Width = 500;
	dialog.Height = 203;
	dialog.Drag = true;
...
function openImportDialog(){
	dialog = new window.top.Dialog();
	dialog.currentWindow = window;
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(125449,user.getLanguage())%>";;
	dialog.Width = 800;
	dialog.Height = 600;
	dialog.Drag = true;
	dialog.URL = "/hrm/HrmDialogTab.jsp?_fromURL=HrmResourceImport&importtype=special&title=125449";
	dialog.show();
}
...
if(HrmUserVarify.checkUserRight("HrmSpeciality:Log", user)){
...
String fromSql  = " from HrmSpeciality a ";
...
operateString = "<operates width=\"20%\">";
 	       operateString+=" <popedom transmethod=\"weaver.hrm.job.SpecialityComInfo.getSpecialtityOperate\" otherpara=\""+HrmUserVarify.checkUserRight("HrmSpecialityEdit:Edit", user)+"\" otherpara2=\""+HrmUserVarify.checkUserRight("HrmSpecialityEdit:Delete", user)+":"+HrmUserVarify.checkUserRight("HrmSpeciality:log", user)+":"+HrmUserVarify.checkUserRight("HrmSpecialityAdd:add", user)+"\"></popedom> ";
...
tableString =" <table pageId=\""+PageIdConst.HRM_Speciality+"\" tabletype=\""+tabletype+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Speciality,user.getUID(),PageIdConst.HRM)+"\" >"+
    ...
    operateString+
    "			<head>"+
    "				<col width=\"50%\" text=\""+SystemEnv.getHtmlLabelName(803,user.getLanguage()) + SystemEnv.getHtmlLabelName(195,user.getLanguage())+"\" column=\"name\" orderkey=\"name\" transmethod=\"weaver.hrm.HrmTransMethod.getHrmOpenDialogName\" otherpara=\"column:id\"/>"+
    "				<col width=\"50%\" text=\""+SystemEnv.getHtmlLabelName(803,user.getLanguage()) + SystemEnv.getHtmlLabelName(433,user.getLanguage())+"\" column=\"description\" orderkey=\"description\" />"+
    "			</head>"+
    " </table>";
%>
 <input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.HRM_Speciality %>"/>
