...
String companyid = SubCompanyVirtualComInfo.getCompanyid(""+id);
...
function openDialog(cmd,id){
	...
	if(cmd=="addSubCompany"){
		<%if(from.equals("HrmCompanyDsp")){%>
			url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmSubCompanyAddVirtual&companyid=<%=id%>"+"&virtualtype=<%=id%>";
		<%}else{%>
			url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmSubCompanyAddVirtual&method=addChildSubCompany&subcompanyid=<%=id%>"+"&virtualtype=<%=companyid%>";
		<%}%>
...
function openHrmComDetail(id){
	window.parent.location.href="/hrm/HrmTab.jsp?_fromURL=HrmSubCompanyDspVirtual&id="+id;
}
</script>
</head>
<body>
...
String fromSql  = " from HrmSubCompanyVirtual ";
String sqlWhere = "";
if(from.equals("HrmCompanyDsp")){
	sqlWhere+=" where companyid="+id;
}else{
	sqlWhere+=" where supsubcomid="+id;
}
...
operateString = "<operates width=\"20%\">";
				 operateString+=" <popedom transmethod=\"weaver.hrm.HrmTransMethod.getSubcompanyListVirtualOperate\" otherpara=\""+HrmUserVarify.checkUserRight("HrmSubCompanyEdit:Edit", user)+"\" otherpara2=\""+HrmUserVarify.checkUserRight("HrmSubCompanyEdit:Edit", user)+":"+HrmUserVarify.checkUserRight("HrmSubCompanyEdit:Edit", user)+"\"></popedom> ";
...
tableString =" <table pageId=\""+PageIdConst.HRM_SubCompanyListVirtual+"\" tabletype=\""+tabletype+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_SubCompanyListVirtual,user.getUID(),PageIdConst.HRM)+"\" >"+
		" <checkboxpopedom showmethod=\"weaver.hrm.HrmTransMethod.getSubCompanyVirtualCheckbox\"  id=\"checkbox\"  popedompara=\"column:id\" />"+
    ...
    " </table>";
%>
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.HRM_SubCompanyListVirtual %>"/>
