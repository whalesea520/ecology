...
<script type="text/javascript">
	...
	function getAsyncUrl(treeId, treeNode) {
	    ...
	    if (treeNode != undefined && treeNode.isParent != undefined && treeNode.isParent != null) {
	    	return "/hrm/companyvirtual/DepartmentSingleXML.jsp?virtualtype="+virtualtype+"&subcompanyid=<%=subcompanyid%>&" + treeNode.ajaxParam + "&" + new Date().getTime() + "=" + new Date().getTime();
	    } else {
	    	//初始化时
	    	return "/hrm/companyvirtual/DepartmentSingleXML.jsp?virtualtype="+virtualtype+"&subcompanyid=<%=subcompanyid%>&excludeid=<%=excludeid%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>" + "&" + new Date().getTime() + "=" + new Date().getTime();
	    }
	};
	//zTree配置信息
	var setting = {
