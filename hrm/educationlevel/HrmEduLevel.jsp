...
function doDel(id){
	...
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>",function(){
		...
		for(var i=0;i<idArr.length;i++){
			ajaxNum++;
			jQuery.ajax({
				url:"EduLevelOperation.jsp?isdialog=1&operation=delete&id="+idArr[i],
				type:"post",
				async:true,
...
function openDialog(id){
	...
	var url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmEduLevelAdd";
	if(!!id){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(26473,user.getLanguage())+SystemEnv.getHtmlLabelName(818,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmEduLevelEdit&id="+id;
	}else{
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())+SystemEnv.getHtmlLabelName(818,user.getLanguage())%>";
	}
	dialog.Width = 500;
	dialog.Height = 203;
	dialog.Drag = true;
...
function onLog(id){
	...
	var url = "";
	if(id && id!=""){
		url = "/docs/tabs/DocCommonTab.jsp?_fromURL=3&isdialog=1&secid=65&sqlwhere=<%=xssUtil.put("where operateitem=80 and relatedid=")%>&relatedid="+id;
	}else{
		url = "/docs/tabs/DocCommonTab.jsp?_fromURL=3&isdialog=1&secid=65&sqlwhere=<%=xssUtil.put("where operateitem=80")%>";
	}
...
if(HrmUserVarify.checkUserRight("HrmEducationLevelEdit:Delete", user)){
	RCMenu += "{"+SystemEnv.getHtmlLabelName(32136,user.getLanguage())+",javascript:doDel(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
}

if(HrmUserVarify.checkUserRight("HrmEducationLevel:Log", user)){
...
String fromSql  = " from HrmEducationLevel a ";
...
operateString = "<operates width=\"20%\">";
 	       operateString+=" <popedom transmethod=\"weaver.hrm.job.EducationLevelComInfo.getEducationLevelOperate\" otherpara=\""+HrmUserVarify.checkUserRight("HrmEducationLevelEdit:Edit", user)+"\" otherpara2=\""+HrmUserVarify.checkUserRight("HrmEducationLevelDelete:Delete", user)+":"+HrmUserVarify.checkUserRight("HrmEducationLevel:log", user)+":"+HrmUserVarify.checkUserRight("HrmEducationLevelAdd:add", user)+"\"></popedom> ";
...
tableString =" <table pageId=\""+PageIdConst.HRM_JobCall+"\" tabletype=\""+tabletype+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_JobCall,user.getUID(),PageIdConst.HRM)+"\" >"+
		" <checkboxpopedom showmethod=\"weaver.hrm.job.EducationLevelComInfo.getEducationLevelCheckbox\"  id=\"checkbox\"  popedompara=\"column:id\" />"+
    ...
    operateString+
    "			<head>"+
    "				<col width=\"50%\"  text=\""+SystemEnv.getHtmlLabelName(818,user.getLanguage()) + SystemEnv.getHtmlLabelName(195,user.getLanguage())+"\" column=\"name\" orderkey=\"name\" transmethod=\"weaver.hrm.HrmTransMethod.getHrmOpenDialogName\" otherpara=\"column:id\"/>"+
    "				<col width=\"50%\"   text=\""+SystemEnv.getHtmlLabelName(818,user.getLanguage()) + SystemEnv.getHtmlLabelName(433,user.getLanguage())+"\" column=\"description\" orderkey=\"description\"/>"+
    "			</head>"+
    " </table>";
%>
