...
String jobresponsibility = Util.null2String(JobTitlesComInfo.getJobresponsibility(""+id));
String jobcompetency = Util.null2String(JobTitlesComInfo.getJobcompetency(""+id));
String jobtitleremark = Util.null2String(JobTitlesComInfo.getJobtitleremark(""+id));
...
if(canDel&&HrmUserVarify.checkUserRight("HrmJobTitlesEdit:Delete", user)){
		...
		<wea:layout>
         ...
         <brow:browser viewType="0" name="jobactivityid" browserValue="<%=jobactivityid %>" 
          ...
          <%}else{%><%=JobActivitiesComInfo.getJobActivitiesname(jobactivityid)%><%}%>
				...
				<wea:item>
					<%if(canEdit){%>
		      <textarea class=inputstyle style="width: 95%" rows=4 name="jobresponsibility" value="<%=Util.toScreenToEdit(jobresponsibility,user.getLanguage())%>"><%=Util.toScreenToEdit(jobresponsibility,user.getLanguage())%></textarea>
		      <%}else{%><%=Util.toScreen(jobresponsibility,user.getLanguage())%><%}%>
				</wea:item>
            ...
            completeUrl="/data.jsp?type=9"></brow:browser>
	      	<%}else{%>
	      		 <SPAN id=departmentspan><a onclick="" href="/docs/docs/DocDsp.jsp?id=<%=jobdoc %>" target="_blank"><%=DocComInfo.getDocname(jobdoc)%></a></SPAN> 
	      	...
	      	<%} %>
				...
				<wea:item>
					<%if(canEdit){%><textarea class=inputstyle style="width: 95%" rows=4 name="jobcompetency" value="<%=Util.toScreenToEdit(jobcompetency,user.getLanguage())%>"><%=Util.toScreenToEdit(jobcompetency,user.getLanguage())%></textarea>
      		<%}else{%><%=Util.toScreen(jobcompetency,user.getLanguage())%><%}%>
				</wea:item>
				<wea:item><%=SystemEnv.getHtmlLabelName(454,user.getLanguage())%></wea:item>
				<wea:item>
					<%if(canEdit){%>
          <TEXTAREA class=inputstyle name=jobtitleremark rows=8 style="width: 95%" <%=isdisable%>><%=Util.toScreenToEdit(jobtitleremark,user.getLanguage())%></TEXTAREA>
          <%}else{%><%=Util.toScreen(jobtitleremark,user.getLanguage())%><%}%>
				</wea:item>
			</wea:group>
		</wea:layout>
