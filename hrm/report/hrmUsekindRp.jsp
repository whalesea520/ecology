	...
	<HEAD>
		...
		<script language=javascript>  
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=hrmUsekindRp&tnum=804&fromdate=<%=fromdate%>&enddate=<%=enddate%>&department=<%=department%>&location=<%=location%>&status=<%=status%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="hrmUsekindRp.jsp" >
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.report.manager.HrmUsekindRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+department:"+department+"+location:"+location+"+status:"+status+"\" pageId=\""+Constants.HRM_Q_0212+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_0212,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
							...
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(804,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
