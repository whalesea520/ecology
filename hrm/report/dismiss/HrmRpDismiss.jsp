...
function openDialog(cmd){
	...
	var url = "";
	if(cmd=="HrmRpDismissTime"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15958,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpDismissTime&isdialog=1";
	}else if(cmd=="HrmDismissReport"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15955,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmDismissReport&isdialog=1";
	}else if(cmd=="HrmRpDismissDetail"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpDismissDetail&isall=true&isdialog=1";
	}
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
...
function openDialogDetail(depid){
	...
	var fromdateselect = $GetEle("fromdateselect");
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
	url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpDismissDetail&isdialog=1&isall=false&olddepartment="
		+depid+"&fromdateselect="+fromdateselect.value;
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
	dialog.URL = url;
...
function HrmRpDismissTime(){
	openDialog("HrmRpDismissTime");
}
function HrmDismissReport(){
	openDialog("HrmDismissReport");
}
function HrmRpDismissDetail(){
	openDialog("HrmRpDismissDetail");
}
</script>
</head>
<%
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(15958,user.getLanguage())+",javascript:HrmRpDismissTime(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15955,user.getLanguage())+",javascript:HrmDismissReport(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",javascript:HrmRpDismissDetail(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
<form id=frmmain name=frmmain method=post action="HrmRpDismiss.jsp">
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		</td>
		<td class="rightSearchSpan" style="text-align:right;">
			<input type=button class="e8_btn_top" onclick="HrmRpDismissTime()" value="<%=SystemEnv.getHtmlLabelName(15958,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmDismissReport()" value="<%=SystemEnv.getHtmlLabelName(15955,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRpDismissDetail()" value="<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>"></input>
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
	<wea:layout type="4col">  
	    ...
	    <wea:item><%=SystemEnv.getHtmlLabelName(898,user.getLanguage())%></wea:item>
	    <wea:item>
...
<% 
	...
	String denominator = " select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid = t2.id "+sqlwhere;
	String tabletype="none";
	String backfields=" t2.departmentid as departmentid, COUNT(t1.id) as molecular, ("+ denominator +") as denominator";
	String sqlform = " HrmStatusHistory t1, HrmResource t2";
	sqlwhere =" where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid = t2.id and  t2.departmentid IS NOT NULL " +sqlwhere;
	String groupby = " t2.departmentid ";
	String tableString=""+
	   "<table pageId=\""+PageIdConst.HRM_RpRehire+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_RpRehire,user.getUID(),PageIdConst.HRM)+"\" tabletype=\""+tabletype+"\">"+
	   ...
	   "</table>";
%>
<input type="hidden" _showCol="false" name="pageId" id="pageId" value="<%= PageIdConst.HRM_RpRehire %>"/>
