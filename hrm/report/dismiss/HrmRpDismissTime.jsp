...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(15958,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
for(int month = 1;month<13;month++){
  ...
  sql = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where " +
  "(t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid=t2.id AND t2.departmentid IS NOT NULL "+sqlwhere;		     
  rs.executeSql(sql);
  rs.next();
...
<form id=frmmain name=frmmain method=post action="HrmRpDismissTime.jsp">
...
<wea:layout type="table" attributes="{'cols':'13','csw':'12%,8%,8%,8%,8%,8%,8%,8%,8%,8%,8%,8%,8%,8%,8%'}">
	<wea:group context="" attributes="{'groupDisplay':'none'}">
		...
		<wea:item type="thead"><%=month+1%><%=SystemEnv.getHtmlLabelName(6076, user.getLanguage())%></wea:item>
		<%} %>
