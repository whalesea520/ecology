...
//td1487, modified by xiaofeng
	...
	if("null".equals(olddepartment)) {
		sqlwhere+="  and t2.departmentid is NULL ";
	} else {
...
String sql = "select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) AND t2.departmentid IS NOT NULL and type_n = 5 "+sqlwhere;
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
String sqlstr = "select t1.*,t2.jobtitle, t2.departmentid from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) AND t2.departmentid IS NOT NULL and type_n = 5 "+sqlwhere+" order by changedate desc";
...
<form id=frmmain name=frmmain method=post action="HrmRpDismissDetail.jsp">
...
<tbody>
  ...
  <%
   ...
   er.addStringValue( SystemEnv.getHtmlLabelName(1978,user.getLanguage()) , "Header" ) ;  
   ...
   if(total!=0){
     ...
     String changereason = Util.toScreen(rs.getString("changereason"),user.getLanguage());     
     ...
     er.addStringValue(changereason) ;     
   %>
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    ...
    <TD><a href="/hrm/jobtitles/HrmJobTitlesEdit.jsp?id=<%=rs.getString("jobtitle") %>"> <%=oldjobtitlename%></a></TD>    
    <TD><%=changedate%></TD>        
    <TD><%=changereason%></TD>        
    </TR>
...
<%
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\"HrmRpDismissDetail\" pagesize=\""+PageIdConst.getPageSize("HrmRpDismissDetail",user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmRpDismissDetailList\" sourceparams=\"sqlstr:"+Util.toHtmlForSplitPage(sqlstr)+"\" tabletype=\"none\">"+
...
"<head>";
	...
	tableString+=  "<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(15961,user.getLanguage())+"\" column=\"changedate\" orderkey=\"changedate\"/>";
	tableString+=  "<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(1978,user.getLanguage())+"\" column=\"changereason\" orderkey=\"changereason\"/>"+
"</head>"+
"</table>";      
%>
<input type="hidden" name="pageId" id="pageId" value="HrmRpDismissDetail"/>
