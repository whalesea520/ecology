...
var hrmDialog = parent.parent.getDialog(parent);
function excel(){
...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(15955,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(28343, user.getLanguage())+",javascript:excel(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<form id=frmmain name=frmmain method=post action="HrmDismissReport.jsp">
...
<%
   int line = 0;
   ExcelFile.init ();
   String filename = SystemEnv.getHtmlLabelName(15956,user.getLanguage());
   ...
   String sql = "select distinct(t2.departmentid) from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) AND t2.departmentid IS NOT NULL and type_n = 5 and t1.resourceid=t2.id";
...
<%     
	 for(int month = 1;month<13;month++){ 
	   ...
	   sql = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid=t2.id and t2.departmentid = "+resultid+sqlwhere;
	   //out.println(sql+"<br>");
	   rs.executeSql(sql);
	   rs.next();	   
...
<%
  sql = "select distinct(t2.departmentid) resultid from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid=t2.id AND t2.departmentid IS NOT NULL and SUBSTRING(changedate,0,5) =  "+year;
  if(newsubcompanyid.length()>0)sql+=" and t2.subcompanyid1 = "+newsubcompanyid;
  String sql1 = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 AND t2.departmentid IS NOT NULL and t1.resourceid=t2.id and SUBSTRING(changedate,0,5) = "+year+" and t2.departmentid = ";
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\"HrmDismissReport\" pagesize=\""+PageIdConst.getPageSize("HrmDismissReport",user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmChangeStatusList\" sourceparams=\"type:HrmDismissReport+year:"+year+"+sqlstr:"+Util.toHtmlForSplitPage(sql)+"+sqlstr1:"+Util.toHtmlForSplitPage(sql1)+"\" tabletype=\"none\">"+
...
"</table>";
%>
<input type="hidden" name="pageId" id="pageId" value="HrmDismissReport"/>
...
<div id="zDialog_div_bottom" class="zDialog_div_bottom">
    ...
    <wea:group context="">
    	<wea:item type="toolbar">
    		<input type="button" value="<%=SystemEnv.getHtmlLabelName(309,user.getLanguage())%>" id="zd_btn_cancle"  class="zd_btn_cancle" onclick="hrmDialog.close();">
    	</wea:item>
   	</wea:group>
  </wea:layout>
</div>
