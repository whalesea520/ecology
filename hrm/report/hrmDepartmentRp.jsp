	...
	<HEAD>
		...
		<script language=javascript>  
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=hrmDepartmentRp&tnum=16551&fromdate=<%=fromdate%>&enddate=<%=enddate%>&location=<%=location%>&status=<%=status%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="hrmDepartmentRp.jsp" >
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table datasource=\"weaver.hrm.report.manager.HrmDepartmentRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+location:"+location+"+status:"+status+"\" pageId=\""+Constants.HRM_Q_0210+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_0210,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
								" <sql backfields=\"*\" sumColumns=\"result\" sqlform=\"temp\" sqlwhere=\"\"  sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"asc\"/>"+
							"	<head>"+
							//"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(141,user.getLanguage())+"\" column=\"subcompanyname\" orderkey=\"subcompanyname\" />"+
							"		<col width=\"40%\" text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"departmentname\" orderkey=\"departmentname\" />"+
							//"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(6086,user.getLanguage())+"\" column=\"jobtitlename\" orderkey=\"jobtitlename\" />"+
							"		<col width=\"25%\" text=\""+SystemEnv.getHtmlLabelName(1859,user.getLanguage())+"\" column=\"result\" orderkey=\"result\" />"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(31143,user.getLanguage())+"\" column=\"percent\" orderkey=\"percent\" molecular=\"result\" denominator=\"total\" />"+
							"	</head></table>";
						%>
