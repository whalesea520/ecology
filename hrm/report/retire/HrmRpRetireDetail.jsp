...
String sql = "";
//if(!olddepartment.equals("")){
sql = "select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where type_n = 6 and t1.resourceid = t2.id "+sqlwhere;
//}else{
...
String sqlstr = "";
//if(!olddepartment.equals("")){
  sqlstr = "select t1.*, t2.departmentid as departmentid from HrmStatusHistory t1,HrmResource t2 where type_n = 6 and t1.resourceid = t2.id "+sqlwhere+" order by changedate desc";
//}else{
...
<form id=frmmain name=frmmain method=post action="HrmRpRetireDetail.jsp">
	<input name="isdialog" type="hidden" value="1">
	<wea:layout type="4col">
	  <wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'> 
    <wea:item><%=SystemEnv.getHtmlLabelName(16033,user.getLanguage())%></wea:item>
    <wea:item>
    ...
    <wea:item><%=SystemEnv.getHtmlLabelName(16034,user.getLanguage())%></wea:item>
    <wea:item>
...
<%
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmRpRetireDetailList\" sourceparams=\"sqlstr:"+Util.toHtmlForSplitPage(sqlstr)+"\" tabletype=\"none\">"+
...
"<head>";
	...
	tableString+=  "<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(16035,user.getLanguage())+"\" column=\"changedate\" orderkey=\"changedate\"/>"+
"</head>"+
"</table>";      
%>
