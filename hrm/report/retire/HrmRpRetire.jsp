...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(16027,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
...
function openDialog(cmd){
	...
	var url = "";
	if(cmd=="HrmRpRetireTime"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(16028,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRetireTime&isdialog=1";
	}else if(cmd=="HrmRetireReport"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(16025,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRetireReport&isdialog=1";
	}else if(cmd=="HrmRpRetireDetail"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRetireDetail&isdialog=1";
	}
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
...
function openDialogDetail(depid){
	...
	var url = "";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
	url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRetireDetail&isdialog=1&olddepartment="+depid
...
function HrmRpRetireTime(){
	openDialog("HrmRpRetireTime");
}
function HrmRetireReport(){
	openDialog("HrmRetireReport");
}
function HrmRpRetireDetail(){
	openDialog("HrmRpRetireDetail");
}
</script>
</head>
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(16028,user.getLanguage())+",javascript:HrmRpRetireTime(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(16025,user.getLanguage())+",javascript:HrmRetireReport(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",javascript:HrmRpRetireDetail(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
<form id=frmmain name=frmmain method=post action="HrmRpRetire.jsp">
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		</td>
		<td class="rightSearchSpan" style="text-align:right;">
			<input type=button class="e8_btn_top" onclick="HrmRpRetireTime()" value="<%=SystemEnv.getHtmlLabelName(16028,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRetireReport()" value="<%=SystemEnv.getHtmlLabelName(16025,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRpRetireDetail()" value="<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>"></input>
...
<wea:layout type="4col">
    ...
    <wea:item><%=SystemEnv.getHtmlLabelName(16029,user.getLanguage())%></wea:item>
    <wea:item>
...
<% 
	String sqlwhere = " where type_n = 6 and t1.resourceid = t2.id AND t2.departmentid IS NOT NULL ";
