	...
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffLeaveDetail&tnum=20092&fromDate=<%=fromDate%>&toDate=<%=toDate%>&status=<%=status%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>";
			}
		</script>
	</head>
	<body>
		...
		<%
			...
			String dataParam = "fromDate:"+fromDate+"+toDate:"+toDate+"+subCompanyId:"+subCompanyId+"+departmentId:"+departmentId+"+resourceId:"+resourceId+"+status:"+status+"+type:0"+"+arg1:false"+"+arg2:true";
			SplitPageTagTable table = new SplitPageTagTable("HrmScheduleDiffLeaveDetail", out, user);
			...
			table.addCol("10%", SystemEnv.getHtmlLabelName(15378,user.getLanguage()), "status");
			table.addCol("12%", SystemEnv.getHtmlLabelName(828,user.getLanguage()), "leaveDays");
			table.addCol("10%", SystemEnv.getHtmlLabelName(1881,user.getLanguage()), "leaveType");
		%>
