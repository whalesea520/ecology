...
<%
	...
	String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(16559,user.getLanguage())+"—"+SystemEnv.getHtmlLabelName(20241,user.getLanguage());
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffSignInDetail&tnum=20241&fromDate=<%=fromDate%>&toDate=<%=toDate%>&status=<%=status%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>";
			}
	...
	<body>
		...
		<%
			...
			SplitPageTagTable table = new SplitPageTagTable("HrmScheduleDiffSignInDetail", out, user);
			table.setData("weaver.hrm.report.schedulediff.HrmScheduleDiffDetSignInManager.getScheduleList", dataParam);
