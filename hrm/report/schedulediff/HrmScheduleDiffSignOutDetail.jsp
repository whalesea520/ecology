...
<%
	...
	String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(16559,user.getLanguage())+"—"+SystemEnv.getHtmlLabelName(20242,user.getLanguage());
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffSignOutDetail&tnum=20242&fromDate=<%=fromDate%>&toDate=<%=toDate%>&status=<%=status%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>";
			}
	...
	<body>
		...
		<%
			...
			SplitPageTagTable table = new SplitPageTagTable("HrmScheduleDiffSignOutDetail", out, user);
			table.setData("weaver.hrm.report.schedulediff.HrmScheduleDiffDetSignOutManager.getScheduleList", dataParam);
			...
			table.addCol("10%", SystemEnv.getHtmlLabelName(20039,user.getLanguage()), "signTime");
