	...
	<head>
		...
		<script language="javascript">
			...
			function showdata(){
				...
				ajax.send("fromDate=<%=fromDate%>&toDate=<%=toDate%>&subCompanyId=<%=subCompanyId%>&departmentId=<%=departmentId%>&resourceId=<%=resourceId%>&status=<%=status%>");
				//获取执行状态
			...
			function exportExcel(){
				if(check_form(document.frmMain,"fromDate,toDate")){
					if(document.frmMain.fromDate.value > document.frmMain.toDate.value) {
						...
						document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffReport&tnum=20078&fromDate="+document.frmMain.fromDate.value+"&toDate="+document.frmMain.toDate.value+"&status="+$GetEle("status").value+"&resourceId="+$GetEle("resourceId").value+"&departmentId="+$GetEle("departmentId").value+"&subCompanyId="+$GetEle("subCompanyId").value;
					}
				}
			}
		</script>
