...
String titlename = SystemEnv.getHtmlLabelName(6140,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(16044,user.getLanguage())+":"+ SystemEnv.getHtmlLabelName(16043,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
...
String sql = "";
sql ="select id from HrmScheduleDiff";
rs.executeSql(sql);
while(rs.next()){
total += time.getTotalDayByType(rs.getString(1),sqlwhere,difftype,depid);
}
...
<form name=frmmain id="frmmain" method=post action="HrmRpScheduleDiffTypeDay.jsp">
...
<tr>
    ...
    <TD class=Field>  
      <input class="wuiBrowser" type="hidden" _url="/systeminfo/BrowserMain.jsp?url=/hrm/company/DepartmentBrowser.jsp"
...
<tbody>
  ...
  <%
   ...
   if(total!=0){
	...
	int resultcount = time.getTotalDayByType(resultid,sqlwhere,difftype,depid);
	if(resultcount < 0){
	  resultcount = 0;
	}
