	...
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffMonthAttDateDetail&tnum=15880,17463&fromDate=<%=curDate%>&toDate=<%=curDate%>&resourceId=<%=resourceId%>&status=<%=status%>";
			}
		</script>
	</head>
	<body>
		...
		<wea:layout type="table" needImportDefaultJsAndCss="false" attributes='<%=layoutAtt%>'>
			<wea:group context='<%=SystemEnv.getHtmlLabelNames("15880,17463",user.getLanguage())%>' attributes="<%=groupAtt%>">
				...
				<wea:item><%=strUtil.vString(map.get("scheduleName"))%></wea:item>
