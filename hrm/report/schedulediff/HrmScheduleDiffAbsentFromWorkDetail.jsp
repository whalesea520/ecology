...
<%
	...
	String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(16559,user.getLanguage())+"—"+SystemEnv.getHtmlLabelName(20090,user.getLanguage());
	...
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffAbsentFromWorkDetail&tnum=20090&fromDate=<%=fromDate%>&toDate=<%=toDate%>&status=<%=status%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>";
			}
		</script>
	</head>
	<body>
		...
		<%
			...
			SplitPageTagTable table = new SplitPageTagTable("HrmScheduleDiffAbsentFromWorkDetail", out, user);
			table.setData("weaver.hrm.report.schedulediff.HrmScheduleDiffDetAbsentFromWorkManager.getScheduleList", dataParam);
			...
			table.addCol("15%", SystemEnv.getHtmlLabelName(125799,user.getLanguage()), "scheduleName");
			table.addCol("15%", SystemEnv.getHtmlLabelNames("20085,81913,391,81914",user.getLanguage()), "ABSENT_realHours");
		%>
