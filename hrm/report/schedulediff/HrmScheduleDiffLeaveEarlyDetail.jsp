...
<%
	...
	String tabName = SystemEnv.getHtmlLabelName(20089,user.getLanguage());
    if(!ruleid.equals("")){
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmScheduleDiffReportExcel.jsp?cmd=HrmScheduleDiffLeaveEarlyDetail&tnum=20089&fromDate=<%=fromDate%>&toDate=<%=toDate%>&status=<%=status%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>&isRule=<%=isRule%>&ruleid=<%=ruleid%>";
			}
	...
	<body>
		...
		<%
			...
			SplitPageTagTable table = new SplitPageTagTable("HrmScheduleDiffLeaveEarlyDetail", out, user);
			table.setData("weaver.hrm.report.schedulediff.HrmScheduleDiffDetLeaveEarlyManager.getScheduleList", dataParam);
