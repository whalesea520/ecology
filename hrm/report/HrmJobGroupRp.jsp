	...
	<HEAD>
		...
		<script language=javascript>  
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=HrmJobGroupRp&tnum=805&fromdate=<%=fromdate%>&enddate=<%=enddate%>&location=<%=location%>&status=<%=status%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="HrmJobGroupRp.jsp" >
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table datasource=\"weaver.hrm.report.manager.HrmJobGroupRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+location:"+location+"+status:"+status+"\" pageId=\""+Constants.HRM_Q_0210+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_0210,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
								" <sql backfields=\"*\" sumColumns=\"result\" sqlform=\"temp\" sqlwhere=\"\"  sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"asc\"/>"+
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(805,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
