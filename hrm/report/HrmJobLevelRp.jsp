...
<%
	...
	String levelarea=Util.fromScreen(request.getParameter("levelarea"),user.getLanguage());
	
	if(levelarea.equals(""))levelarea="5";
	if(status.equals("")){
		status = "8";
	}
%>
<HTML>
	<HEAD>
		...
		<script language=javascript>  
			function submitData() {
				if(check_form(document.frmmain,'levelarea')){
					jQuery("#frmmain").submit();
				}
			}
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=HrmJobLevelRp&tnum=16546&fromdate=<%=fromdate%>&enddate=<%=enddate%>&department=<%=department%>&location=<%=location%>&status=<%=status%>&area=<%=levelarea%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="HrmJobLevelRp.jsp" >
			...
			<wea:layout type="4col">
				<wea:group context='<%=SystemEnv.getHtmlLabelName(15505,user.getLanguage())%>'>
					...
					<wea:item><%=SystemEnv.getHtmlLabelName(15871,user.getLanguage())%></wea:item>
					<wea:item>
						<wea:required id="levelareaspan" required='<%=levelarea.length()==0%>'>
							<INPUT class=inputStyle maxLength=20 size=6 id="levelarea" name="levelarea" onKeyPress="ItemCount_KeyPress();checkinput('levelarea','levelareaspan')" onBlur='checknumber("levelarea");checkinput("levelarea","levelareaspan")' onchange="checkinput('levelarea','levelareaspan')" value="<%=levelarea%>"><%=SystemEnv.getHtmlLabelName(15872,user.getLanguage())%>
						</wea:required>
					</wea:item>
				</wea:group>
			</wea:layout>
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.report.manager.HrmJobLevelRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+department:"+department+"+location:"+location+"+status:"+status+"+area:"+levelarea+"\" pageId=\""+Constants.HRM_Q_022+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_022,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
							...
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(1909,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
