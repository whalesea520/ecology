...
function openDialog(cmd){
	...
	var url = "";
	if(cmd=="HrmRpFireTime"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15975,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpFireTime&isdialog=1";
	}else if(cmd=="HrmFireReport"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15972,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmFireReport&isdialog=1";
	}else if(cmd=="HrmRpFireDetail"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpFireDetail&isdialog=1";
	}
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
...
function openDialogDetail(depid){
	...
	var url = "";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
	url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpFireDetail&isdialog=1&olddepartment="+depid
...
function HrmRpFireTime(){
	openDialog("HrmRpFireTime");
}
function HrmFireReport(){
	openDialog("HrmFireReport");
}
function HrmRpFireDetail(){
	openDialog("HrmRpFireDetail");
}
</script>
</head>
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(15975,user.getLanguage())+",javascript:HrmRpFireTime(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15972,user.getLanguage())+",javascript:HrmFireReport(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",javascript:HrmRpFireDetail(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
<form id=frmmain name=frmmain method=post action="HrmRpFire.jsp">
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		</td>
		<td class="rightSearchSpan" style="text-align:right;">
			<input type=button class="e8_btn_top" onclick="HrmRpFireTime()" value="<%=SystemEnv.getHtmlLabelName(15975,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmFireReport()" value="<%=SystemEnv.getHtmlLabelName(15972,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRpFireDetail()" value="<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>"></input>
...
<% 
	String sqlwhere = " where type_n = 1 and t1.resourceid = t2.id and t2.departmentid IS NOT NULL ";
	...
	String tableString=""+
	   "<table pageId=\""+PageIdConst.HRM_RpFire+"\" instanceid=\"docMouldTable\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_RpFire,user.getUID(),PageIdConst.HRM)+"\" tabletype=\""+tabletype+"\">"+
	   ...
	   "</table>";
%>
<input type="hidden" _showCol="false" name="pageId" id="pageId" value="<%= PageIdConst.HRM_RpFire %>"/>
