...
String sqlwhere = "";
	...
	sqlwhereyear+=" and createdate>='"+year+"-01-01'";
	if(rs.getDBType().equals("oracle")){
	sqlwhereyear+=" and createdate <='"+year+"-12-31'";
	}else{
	sqlwhereyear+=" and createdate <='"+year+"-12-31'";
	}


String sql = "select count(id) from HrmResource t1,HrmDepartment t2 where t1.departmentid = t2.id and status in (0,1,2,3) and 4 = 4 "+sqlwhereyear;
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
%>
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
	<form id=frmmain name=frmmain method=post action="HrmRpResourceAddReport.jsp">
...
<%
	sql = "select distinct(t2.id) resultid from HrmResource t1,HrmDepartment t2 where (t1.accounttype is null or t1.accounttype=0) and t1.departmentid = t2.id and t1.status in (0,1,2,3) ";
	if(newsubcompanyid.length()>0)sql+=" and t2.subcompanyid1 = "+newsubcompanyid;
	String sql1 = "select count(t1.id) resultcount from HrmResource t1 where (t1.accounttype is null or t1.accounttype=0) and t1.status in (0,1,2,3) and t1.departmentid = ";
  
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""
+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)
+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmChangeStatusList\" sourceparams=\"type:HrmRpResourceAddReport+year:"
