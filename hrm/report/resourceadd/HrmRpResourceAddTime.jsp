...
for(int month = 1;month<13;month++){
  ...
  sqlwhere=" and createdate>='"+firstday+"'";
  if(rs.getDBType().equals("oracle")){
  sqlwhere+=" and createdate <='"+lastday+"'";
  }else{
  sqlwhere+=" and createdate <='"+lastday+"'";
  }
  
  sql = "select count(t1.id) resultcount from HrmResource t1,HrmDepartment t2 where t1.departmentid = t2.id and status in (0,1,2,3) and 4 = 4 "+sqlwhere;
  //out.println(sql+"<br>");		     	   
  rs.executeSql(sql);
  rs.next();
...
<form id=frmmain name=frmmain method=post action="HrmRpResourceAddTime.jsp">
