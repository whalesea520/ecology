...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(15987,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
...
int total = 0;
int linecolor=0;
String filename = SystemEnv.getHtmlLabelName(15883,user.getLanguage())+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage()) ;
...
if(!oldjobtitle.equals("")){
	sqlwhere+=" and t1.jobtitle ="+oldjobtitle+" ";
}

if(!olddepartment.equals("")){
	sqlwhere+=" and  t1.departmentid = "+olddepartment+" ";
}

String sql = "";
sql = "select count(t1.id) hrmnum from Hrmresource t1,HrmDepartment t2 where 1=1 and t1.departmentid = t2.id and status in (0,1,2,3) "+sqlwhere;
...
String sqlstr = "";
sqlstr = "select t1.* from Hrmresource t1,HrmDepartment t2 where 1=1 and t1.departmentid = t2.id and status in (0,1,2,3) "+sqlwhere+" order by createdate desc";
...
<form id=frmmain name=frmmain method=post action="HrmRpResourceAddDetail.jsp">
...
<tbody>
   <tr class=header>
    ...
    <td><%=SystemEnv.getHtmlLabelName(81517,user.getLanguage())%></td>    
  </tr>
  <%
   ExcelFile.init ();   
   ...
   er.addStringValue( SystemEnv.getHtmlLabelName(81517,user.getLanguage()) , "Header" ) ;     
     
   if(total!=0){
     while(rs.next()){
     String departmentid = rs.getString("departmentid");	
     String resourcename = Util.toScreen(ResourceComInfo.getResourcename(rs.getString("id")),user.getLanguage());	
     ...
     er.addStringValue(createdate) ;     
   %>
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD><a href="/hrm/resource/HrmResource.jsp?id=<%=rs.getString("id")%>"> <%=resourcename%></a></TD>
...
<%
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmRpResourceAddDetailList\" sourceparams=\"sqlstr:"+Util.toHtmlForSplitPage(sqlstr)+"\" tabletype=\"none\">"+
"<sql backfields=\"*\"  sqlform=\"temp\" sqlorderby=\"createdate\"  sqlprimarykey=\"createdate\" sqlsortway=\"desc\"  />"+
"<head>";
	...
	tableString+=  "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(81517,user.getLanguage())+"\" column=\"createdate\" orderkey=\"createdate\"/>"+
"</head>"+
"</table>";      
%>
