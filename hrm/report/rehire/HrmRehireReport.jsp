...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(16002,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
String sqlwhere = "";
%>
...
<form id=frmmain name=frmmain method=post action="HrmRehireReport.jsp">
...
<%
   int line = 0;
   ExcelFile.init ();
   String filename = SystemEnv.getHtmlLabelName(16003,user.getLanguage()) ;
   ...
   String sql = "select distinct(t2.departmentid) resultid from HrmStatusHistory t1,HrmResource t2 where t1.resourceid = t2.id and type_n = 7 ";
...
<%     
     for(int month = 1;month<13;month++){   
	   ...
	   sql = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where type_n = 7 and t1.resourceid = t2.id and t2.departmentid = "+resultid+sqlwhere;
	   //out.println(sql+"<br>");		     	   
	   rs.executeSql(sql);
	   rs.next();	   
...
<%
	sql = "select distinct(t2.departmentid) resultid from HrmStatusHistory t1,HrmResource t2 where t1.resourceid = t2.id and type_n = 7 ";
  if(newsubcompanyid.length()>0)sql+=" and t2.subcompanyid1 = "+newsubcompanyid;
  String sql1 = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where type_n = 7 and t1.resourceid = t2.id and t2.departmentid = ";
  
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmChangeStatusList\" sourceparams=\"type:HrmRehireReport+year:"+year+"+sqlstr:"+Util.toHtmlForSplitPage(sql)+"+sqlstr1:"+Util.toHtmlForSplitPage(sql1)+"\" tabletype=\"none\">"+
