...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(16008,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
...
int total = 0;
int linecolor=0;
String filename = SystemEnv.getHtmlLabelName(16009,user.getLanguage()) ;
...
String sql = "";
//if(!olddepartment.equals("")){
  sql = "select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where type_n = 7 and t1.resourceid = t2.id "+sqlwhere;
//}else{
//  sql = "select count(t1.id) from HrmStatusHistory t1 where type_n = 7 "+sqlwhere;
//}
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
...
String sqlstr = "";
//if(!olddepartment.equals("")){
  sqlstr = "select t1.*, t2.departmentid as departmentid from HrmStatusHistory t1,HrmResource t2 where type_n = 7 and t1.resourceid = t2.id "+sqlwhere+" order by changedate desc";
//}else{
//  sqlstr = "select t1.* from HrmStatusHistory t1 where type_n = 7 "+sqlwhere+" order by changedate desc";
//}
...
<form id=frmmain name=frmmain method=post action="HrmRpRehireDetail.jsp">
	<input name="isdialog" type="hidden" value="1">
	<wea:layout type="4col">
		<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>'> 
    <wea:item><%=SystemEnv.getHtmlLabelName(16010,user.getLanguage())%></wea:item>
    <wea:item>
    ...
    <wea:item><%=SystemEnv.getHtmlLabelName(16011,user.getLanguage())%></wea:item>
    <wea:item>
...
<tbody>
  <tr class=header>
    ...
    <td><%=SystemEnv.getHtmlLabelName(16012,user.getLanguage())%></td>    
    <td><%=SystemEnv.getHtmlLabelName(16013,user.getLanguage())%></td>    
  </tr>
  <%
   ExcelFile.init ();   
   ...
   er.addStringValue( SystemEnv.getHtmlLabelName(16012,user.getLanguage()) , "Header" ) ; 
   er.addStringValue( SystemEnv.getHtmlLabelName(16013,user.getLanguage()) , "Header" ) ;    
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
	...
	<%
		String tableString =" <table datasource=\"weaver.hrm.HrmDataSource.getHrmRpRehireDetailList\" sourceparams=\"sqlstr:"+Util.toHtmlForSplitPage(sqlstr)+"\" pageId=\""+
		PageIdConst.Hrm_RpCareerApply+"\" pagesize=\""+
		PageIdConst.getPageSize(PageIdConst.Hrm_RpCareerApply,user.getUID(),PageIdConst.HRM)+"\" tabletype=\"none\">"+
			...
			"		<col width=\"15%\" text=\""+SystemEnv.getHtmlLabelName(16012,user.getLanguage())+"\" column=\"changedate\" orderkey=\"changedate\" />"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(16013,user.getLanguage())+"\" column=\"changeenddate\" orderkey=\"changeenddate\" />"+
		"</head>"+
		"</table>";
	%>
