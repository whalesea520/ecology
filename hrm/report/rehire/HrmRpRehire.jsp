...
function openDialog(cmd){
	...
	var url = "";
	if(cmd=="HrmRpRehireTime"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(16004,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRehireTime&isdialog=1";
	}else if(cmd=="HrmRehireReport"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(16002,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRehireReport&isdialog=1";
	}else if(cmd=="HrmRpRehireDetail"){
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
		url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRehireDetail&isdialog=1";
	}
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
...
function openDialogDetail(depid){
	...
	var url = "";
	dialog.Title = "<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>";
	url = "/hrm/HrmDialogTab.jsp?_fromURL=HrmRpRehireDetail&isdialog=1&olddepartment="+depid
	+"&fromdateselect=<%=fromdateselect %>&fromdate=<%=fromdate %>&enddate=<%=enddate %>&fromdatetoselect=<%=fromdatetoselect%>"+
	"&fromdateto=<%=fromdateto %>&enddateto=<%=enddateto %>";
	dialog.Width = 800;
	dialog.Height = 603;
	dialog.Drag = true;
	dialog.URL = url;
...
function HrmRpRehireTime(){
	openDialog("HrmRpRehireTime");
}
function HrmRehireReport(){
	openDialog("HrmRehireReport");
}
function HrmRpRehireDetail(){
	openDialog("HrmRpRehireDetail");
}
</script>
</head>
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(16004,user.getLanguage())+",javascript:HrmRpRehireTime(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(16002,user.getLanguage())+",javascript:HrmRehireReport(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",javascript:HrmRpRehireDetail(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
<form id=frmmain name=frmmain method=post action="HrmRpRehire.jsp">
<table id="topTitle" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		</td>
		<td class="rightSearchSpan" style="text-align:right;">
			<input type=button class="e8_btn_top" onclick="HrmRpRehireTime()" value="<%=SystemEnv.getHtmlLabelName(16004,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRehireReport()" value="<%=SystemEnv.getHtmlLabelName(16002,user.getLanguage())%>"></input>
			<input type=button class="e8_btn_top" onclick="HrmRpRehireDetail()" value="<%=SystemEnv.getHtmlLabelName(15729,user.getLanguage())%>"></input>
...
<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
	<wea:layout type="4col">  
	    ...
	    <wea:item><%=SystemEnv.getHtmlLabelName(16005,user.getLanguage())%></wea:item>
	    <wea:item>
	    ...
	    <wea:item><%=SystemEnv.getHtmlLabelName(16006,user.getLanguage())%></wea:item>
	    <wea:item>
...
<% 
	String sqlwhere = " where type_n = 7 and t1.resourceid = t2.id AND t2.departmentid IS NOT NULL ";
