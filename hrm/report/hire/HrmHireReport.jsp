...
String titlename = SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(15981,user.getLanguage()) ;
String needfav ="1";
String needhelp ="";
...
<form id=frmmain name=frmmain method=post action="HrmHireReport.jsp">
...
<%
   int line = 0;
   ExcelFile.init ();
   String filename = SystemEnv.getHtmlLabelName(15982,user.getLanguage()) ;
   ...
   String sql = "select distinct(t2.departmentid) resultid from HrmStatusHistory t1,HrmResource t2 where type_n = 2  and t1.resourceid = t2.id ";
...
<%     
     for(int month = 1;month<13;month++){   
	   ...
	   String sqlwhere =" and (changedate >='"+firstday +"' and changedate <= '"+lastday+"')";
	   
	   sql = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where type_n = 2 and t1.resourceid = t2.id and t2.departmentid = "+resultid+sqlwhere;
	   //out.println(sql+"<br>");		     	   
	   rs.executeSql(sql);
	   rs.next();	   
...
<%
	sql = "select distinct(t2.departmentid) resultid from HrmStatusHistory t1,HrmResource t2 where type_n = 2 and t1.resourceid = t2.id ";
  if(newsubcompanyid.length()>0)sql+=" and t2.subcompanyid1 = "+newsubcompanyid;
  String sql1 = "select count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2 where type_n = 2 and t1.resourceid = t2.id and t2.departmentid = ";
  
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmChangeStatusList\" sourceparams=\"type:HrmHireReport+year:"+year+"+sqlstr:"+Util.toHtmlForSplitPage(sql)+"+sqlstr1:"+Util.toHtmlForSplitPage(sql1)+"\" tabletype=\"none\">"+
"<sql backfields=\"*\"  sqlform=\"temp\" sqlorderby=\"depname\"  sqlprimarykey=\"depname\" sqlsortway=\"desc\"  />"+
"<head>";
	tableString+=	 "<col width=\"16%\" text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"depname\"  />";
	tableString+=	 "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1492,user.getLanguage())+"\" column=\"1\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1493,user.getLanguage())+"\" column=\"2\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1494,user.getLanguage())+"\" column=\"3\"  />";
	tableString+=	 "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1495,user.getLanguage())+"\" column=\"4\"  />";
	tableString+=	 "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1496,user.getLanguage())+"\" column=\"5\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1497,user.getLanguage())+"\" column=\"6\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1498,user.getLanguage())+"\" column=\"7\"  />";
	tableString+=	 "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1499,user.getLanguage())+"\" column=\"8\"  />";
	tableString+=	 "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1800,user.getLanguage())+"\" column=\"9\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1801,user.getLanguage())+"\" column=\"10\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1802,user.getLanguage())+"\" column=\"11\"  />";
	tableString+=  "<col width=\"7%\"  text=\""+SystemEnv.getHtmlLabelName(1803,user.getLanguage())+"\" column=\"12\"  />"+
"</head>"+
"</table>";      
%>
