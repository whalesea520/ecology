...
int total = 0;
int linecolor=0;
String filename = SystemEnv.getHtmlLabelName(15988,user.getLanguage()) ;
...
//String sql = "";
//if(!olddepartment.equals("")){
String sql = "select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where type_n = 2 and t1.resourceid = t2.id "+sqlwhere;
//}else{
//  sql = "select count(t1.id) from HrmStatusHistory t1 where type_n = 2 "+sqlwhere;
//}
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
...
//String sqlstr = "";
//if(!olddepartment.equals("")){
String  sqlstr = "select t1.resourceid as resourceid, t1.oldjobtitleid as oldjobtitleid, t2.departmentid as departmentid, " + 
  	"t1.changedate as changedate from HrmStatusHistory t1,HrmResource t2 where type_n = 2  and t1.resourceid = t2.id "+sqlwhere+" order by changedate desc";
//}else{
//  sqlstr = "select t1.resourceid as resourceid, t1.oldjobtitleid as oldjobtitleid, t2.departmentid as departmentid, " + 
//	"t1.changedate as changedate from HrmStatusHistory t1 where type_n = 2 "+sqlwhere+" order by changedate desc";
//}
...
<form id=frmmain name=frmmain method=post action="HrmRpHireDetail.jsp">
...
<tbody>
  ...
  <%
   ...
   er.addStringValue( SystemEnv.getHtmlLabelName(6122,user.getLanguage()) , "Header" ) ;     
   if(total!=0){
     while(rs.next()){
...
<%
String tableString=""+
"<table instanceid=\"BrowseTable\" pageId=\""+PageIdConst.HRM_Select+"\" pagesize=\""+PageIdConst.getPageSize(PageIdConst.HRM_Select,user.getUID(),PageIdConst.HRM)+"\"  datasource=\"weaver.hrm.HrmDataSource.getHrmRpHireDetailList\" sourceparams=\"sqlstr:"+Util.toHtmlForSplitPage(sqlstr)+"\" tabletype=\"none\">"+
...
"<head>";
	...
	tableString+=  "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(6122,user.getLanguage())+"\" column=\"changedate\" orderkey=\"changedate\"/>"+
"</head>"+
"</table>";      
%>
