...
<HTML>
	<HEAD>
		...
		<script language=javascript>  
			function submitData() {
				if(check_form(document.frmmain,'workage')){
					jQuery("#frmmain").submit();
				}
			}
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=hrmWorkageRp&tnum=16549&department=<%=department%>&location=<%=location%>&status=<%=status%>&area=<%=workage%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="hrmWorkageRp.jsp" >
			...
			<wea:layout type="4col">
				<wea:group context='<%=SystemEnv.getHtmlLabelName(15505,user.getLanguage())%>'>
					...
					<wea:item><%=SystemEnv.getHtmlLabelName(15927,user.getLanguage())%></wea:item>
					<wea:item>
						<wea:required id="workagespan" required='<%=workage.length()==0%>'>
							<INPUT class=inputStyle maxLength=20 size=6 id="workage" name="workage" onKeyPress="ItemCount_KeyPress();checkinput('workage','workagespan')" onBlur='checknumber("workage");checkinput("workage","workagespan")' onchange="checkinput('workage','workagespan')" value="<%=workage%>"><%=SystemEnv.getHtmlLabelName(445,user.getLanguage())%>
						</wea:required>
					</wea:item>
				</wea:group>
			</wea:layout>
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.report.manager.HrmWorkageRpManager.getResult\" sourceparams=\"department:"+department+"+location:"+location+"+status:"+status+"+area:"+workage+"\" pageId=\""+Constants.HRM_Q_024+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_024,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
							...
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(15878,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
