	...
	<HEAD>
		...
		<script language=javascript>  
			...
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=hrmSexRp&tnum=16548&fromdate=<%=fromdate%>&enddate=<%=enddate%>&department=<%=department%>&location=<%=location%>&status=<%=status%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="hrmSexRp.jsp" >
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.report.manager.HrmSexRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+department:"+department+"+location:"+location+"+status:"+status+"\" pageId=\""+Constants.HRM_Q_023+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_023,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
							...
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(416,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
