...
<HTML>
	<HEAD>
		...
		<script language=javascript>  
			function submitData() {
				if(check_form(document.frmmain,'agearea')){
					jQuery("#frmmain").submit();
				}
			}
			function exportExcel(){
				document.getElementById("excels").src = "HrmReportExcel.jsp?cmd=hrmAgeRp&tnum=16545&fromdate=<%=fromdate%>&enddate=<%=enddate%>&department=<%=department%>&location=<%=location%>&status=<%=status%>&area=<%=agearea%>";
			}
		</script>
	</head>
	<BODY>
		...
		<form id=frmmain name=frmmain method=post action="hrmAgeRp.jsp" >
			...
			<wea:layout type="4col">
				<wea:group context='<%=SystemEnv.getHtmlLabelName(15505,user.getLanguage())%>'>
					...
					<wea:item><%=SystemEnv.getHtmlLabelName(15860,user.getLanguage())%></wea:item>
					<wea:item>
						<wea:required id="ageareaspan" required='<%=agearea.length()==0%>'>
							<INPUT class=inputStyle maxLength=20 size=6 id="agearea" name="agearea" onKeyPress="ItemCount_KeyPress();checkinput('agearea','ageareaspan')" onBlur='checknumber("agearea");checkinput("agearea","ageareaspan")' onchange="checkinput('agearea','ageareaspan')" value="<%=agearea%>"><%=SystemEnv.getHtmlLabelName(445,user.getLanguage())%>
						</wea:required>
					</wea:item>
				</wea:group>
			</wea:layout>
			...
			<wea:layout type="diycol">
				<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
					<wea:item attributes="{'isTableList':'true','colspan':'full'}">
						<%
							String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.report.manager.HrmAgeRpManager.getResult\" sourceparams=\"fromdate:"+fromdate+"+enddate:"+enddate+"+department:"+department+"+location:"+location+"+status:"+status+"+area:"+agearea+"\" pageId=\""+Constants.HRM_Q_021+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_021,user.getUID(),Constants.HRM)+"\" tabletype=\"none\">"+
							...
							"	<head>"+
							"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(671,user.getLanguage())+"\" column=\"title\" orderkey=\"title\" />"+
