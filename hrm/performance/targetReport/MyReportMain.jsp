...
<script language=javascript>
 function resetbanner(objid,typeid){
    ...
    o.location="MyReport.jsp?type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years;
   ...
   function init(objid,typeid){
  	...
  	document.all("oTDtype_"+objid).className="cycleTDCurrent";
    var o = parent.contentframe.iframes.document;
    o.location="MyReport.jsp?type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>";
...
<td height="100%">
 <table style="width:98%;height:95%" border=0 cellspacing=0 cellpadding=0  scrolling=no id="tabPane">
	  ...
	  <td class="cycleTD" name="oTDtype_4"  id="oTDtype_4" background="/images/tab2.png" width=79px  align=center onmouseover="style.cursor='hand'" onclick="resetbanner(4,4)" ><b><%=SystemEnv.getHtmlLabelName(18059,user.getLanguage())%></b></td>
	  ...
	  <td class="cycleTD" name="oTDtype_3"  id="oTDtype_3" background="/images/tab2.png" width=79px  align=center onmouseover="style.cursor='hand'" onclick="resetbanner(3,3)" ><b><%=SystemEnv.getHtmlLabelName(1926,user.getLanguage())%></b></td>
      
      <td style="border-bottom:1px solid rgb(145,155,156)">&nbsp;</td>
	  </tr>
	 <tr>
		<td colspan="6" style="padding:0;">
		 <iframe src="MyReport.jsp?type=0&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=currentYear%>" ID="iframes" name="iframes" frameborder="0" style="width:100%;height:100%;border-right:1px solid #879293;border-bottom:1px solid #879293;border-left:1px solid #879293;padding:10px;padding-right:0" scrolling="auto"/>
		</td></tr>
  </TBODY>
</TABLE>
</td>
