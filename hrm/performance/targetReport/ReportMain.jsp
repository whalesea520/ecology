...
<IFRAME name=contentframe id=contentframe src="MyReportMain.jsp?objId=<%=objId%>&type_d=<%=type_d%>&months=<%=months%>&years=<%=years%>&type=<%=type%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>" width="100%" height="100%" frameborder=no scrolling=yes>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</IFRAME>
</td>
</tr>
  </TBODY>
