...
String sqlwhere = "";
if (sqlwhere.equals("")) sqlwhere = " where departmentid="+departmentId+" and HrmResource.id <> 0  "  ;
...
if(RecordSet.getDBType().equals("oracle")){
		sqlstr = "create table "+temptable+"  as select * from (select distinct HrmResource.id ,lastname  from HrmResource" + sqlwhere + "  order by id) where rownum<"+ (pagenum*perpage+2);
}else if(RecordSet.getDBType().equals("db2")){
		sqlstr = "create table "+temptable+"  as (select distinct HrmResource.id ,lastname from HrmResource ) definition only ";

        RecordSet.executeSql(sqlstr);

       sqlstr = "insert  into "+temptable+" (select  HrmResource.id ,lastname  from  HrmResource " + sqlwhere + "order by id   fetch  first "+(pagenum*perpage+1)+" rows only )" ;
}else{
		sqlstr = "select distinct top "+(pagenum*perpage+1)+" HrmResource.id ,lastname into "+temptable+" from HrmResource  " + sqlwhere + " order by id " ;
}
...
<TABLE class=Shadow>
		...
		<FORM id=weaver name=SearchForm style="margin-bottom:0" action="ResourceBrowser.jsp" method=post>
    ...
    <TR>
      ...
      <TD width=35% class=field>
        <input name=lastname class=Inputstyle value="<%=lastname%>">
      </TD>
    </TR>
...
<td width="60%">
	<!--############Browser Table START################-->
	<TABLE class="BroswerStyle" cellspacing="0" cellpadding="0" width="100%" height="100%">
		<TR class=DataHeader>
	  <TH width=40%><%=SystemEnv.getHtmlLabelName(413,user.getLanguage())%></TH>
	  <TH width=60%></TH>
		...
		<td colspan="2" width="100%" valign="top">
			...
			<COL width="60%">
				...
				if(RecordSet.last()){
					...
					String requestnames = Util.toScreen(RecordSet.getString("lastname"),user.getLanguage());
...
sub getTheDate(inputname,spanname)
	...
	if(e.tagName == "TD" || e.tagName == "A"){
		...
		window.parent.returnValue=new Array(e.parentElement.cells(0).innerText,e.parentElement.cells(1).innerText);
		//alert(newEntry);
		//alert(newEntry);
