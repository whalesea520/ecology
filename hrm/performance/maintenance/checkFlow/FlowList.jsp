...
if((HrmUserVarify.checkUserRight("CheckFlowInfo:Maintenance",user))||canedit_1||canedit_2||canedit_3||canedit_4||canedit_5||canedit_6||canedit_7||canedit_8||canedit_9||canedit_10){//È¨ÏÞÅÐ¶Ï
    		


RecordSet.execute("select a.*,b.workflowname from HrmPerformanceFlow a left join workflow_base  b on a.relatingFlow=b.id ");
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",FlowAdd.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
}
%>	
...
<TABLE class=ListStyle cellspacing=1 >
  ...
  <th><%=SystemEnv.getHtmlLabelName(18077,user.getLanguage())%></th>
  </tr>
...
<%		}%>
		...
		<TD><a href="FlowEdit.jsp?id=<%=RecordSet.getString("id")%>">
        <%if((RecordSet.getString("type_1")).equals("0")){%><%=SystemEnv.getHtmlLabelName(18102,user.getLanguage())%><%}%>
        <%if((RecordSet.getString("type_1")).equals("1")){%><%=SystemEnv.getHtmlLabelName(18103,user.getLanguage())%><%}%>
		</a>
