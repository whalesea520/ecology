...
if((HrmUserVarify.checkUserRight("Custom:Performance",user))||canedit_1||canedit_2||canedit_3||canedit_4||canedit_5||canedit_6||canedit_7||canedit_8||canedit_9||canedit_10){//È¨ÏÞÅÐ¶Ï
    		


RecordSet.execute("select * from HrmPerformanceCustom order by status,id desc");
...
String titlename =SystemEnv.getHtmlLabelName(18522,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
if(HrmUserVarify.checkUserRight("Custom:Performance",user)) {	
RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",CustomAdd.jsp,_self} " ;
...
<%		}else{%>
	<TR CLASS=DataDark>
<%		}%>
		<TD><a href="CustomEdit.jsp?id=<%=RecordSet.getString("id")%>"><%=Util.toScreen(RecordSet.getString("unitName"),user.getLanguage())%></a></TD>
		<TD><%=Util.toScreen(RecordSet.getString("memo"),user.getLanguage())%></TD>
		<TD> 
        <%if((RecordSet.getString("status")).equals("0")){%><%=SystemEnv.getHtmlLabelName(18095,user.getLanguage())%><%}%>
        <%if((RecordSet.getString("status")).equals("1")){%><%=SystemEnv.getHtmlLabelName(18096,user.getLanguage())%><%}%>
