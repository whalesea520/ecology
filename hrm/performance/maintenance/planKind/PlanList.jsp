...
if((HrmUserVarify.checkUserRight("PlanKindInfo:Maintenance",user))||canedit_1||canedit_2||canedit_3||canedit_4||canedit_5||canedit_6||canedit_7||canedit_8||canedit_9||canedit_10){//È¨ÏÞÅÐ¶Ï
    		


RecordSet.execute("select * from HrmPerformancePlanKind order by  sort");
...
String titlename =SystemEnv.getHtmlLabelName(18044,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",PlanAdd.jsp,_self} " ;
...
<%		}else{%>
	<TR CLASS=DataDark>
<%		}%>
		<TD><%=Util.toScreen(RecordSet.getString("headers"),user.getLanguage())%></TD>
		<TD><a href="PlanEdit.jsp?id=<%=RecordSet.getString("id")%>"><%=Util.toScreen(RecordSet.getString("planName"),user.getLanguage())%></a></TD>
		<TD> 
       <%=Util.toScreen(RecordSet.getString("sort"),user.getLanguage())%>
        </TD>
