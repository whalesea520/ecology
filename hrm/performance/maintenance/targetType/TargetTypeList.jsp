...
if((HrmUserVarify.checkUserRight("TargetTypeInfo:Maintenance",user))||canedit_1||canedit_2||canedit_3||canedit_4||canedit_5||canedit_6||canedit_7||canedit_8||canedit_9||canedit_10){//È¨ÏŞÅĞ¶Ï
    		


RecordSet.execute("select * from HrmPerformanceTargetType ");
...
String titlename =SystemEnv.getHtmlLabelName(18047,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",TargetTypeAdd.jsp,_self} " ;
...
<TABLE class=ListStyle cellspacing=1 >
  ...
  <th><%=SystemEnv.getHtmlLabelName(18114,user.getLanguage())%></th>
  </tr>
...
<%		}else{%>
	<TR CLASS=DataDark>
<%		}%>
		<TD><a href="TargetTypeEdit.jsp?id=<%=RecordSet.getString("id")%>"><%=Util.toScreen(RecordSet.getString("targetName"),user.getLanguage())%></a></TD>
		<TD><%=Util.toScreen(RecordSet.getString("memo"),user.getLanguage())%></TD>
		<TD><a title="<%=SystemEnv.getHtmlLabelName(18115,user.getLanguage())%>" href="TargetDetailList.jsp?id=<%=RecordSet.getString("id")%>">(<%=Util.toScreen(RecordSet.getString("num"),user.getLanguage())%>)</a></TD>
