...
{
RCMenu += "{"+SystemEnv.getHtmlLabelName(18266,user.getLanguage())+",javaScript:onSubmit(),_self} " ;
				...
				<input type="hidden" name="id" value=<%=pointId%>>
									...
									if (!type.equals("0")&&!type.equals("3")){%>
										<TABLE  class=ListStyle cellspacing=1 id='monthHtmlTbl'>
										  ...
										  for (int a=1;a<=12;a++)
										  {%>
										  <a href="MyCheck.jsp?type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=a%>">
										  ...
										  for (int a=1;a<=4;a++)
										  {%>
										  <a href="MyCheck.jsp?type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&quarters=<%=a%>">
										  ...
										  {
										  %>
										 <select class=inputStyle id="weeks" name="weeks" onchange="javascript:location.href='MyCheck.jsp?type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&weeks='+this.value">
										  <%for (int a=1;a<=56;a++)
										  {%>
...
<TR class=title>
   ...
   //得分修正	(hubo,20060211)
	...
	%>
	<button type="button" class="btn" accesskey="M" onclick="modifyPoint()"><u>M</u>-<%=SystemEnv.getHtmlLabelName(18070,user.getLanguage())%></button>
	<%
	}
	

   /**
   String sqlAdjust = "SELECT point_before FROM HrmPerformancePointAdjust WHERE pointId="+pointId+""; 
   String point_before = "";
   rsAdjust.executeSql(sqlAdjust);
	while(rsAdjust.next()){
		point_before =  rsAdjust.getString("point_before");
		out.println("<span style='color:#999'>"+Util.round(point_before,1)+"</span>");
	}
	*/
	%>
	</th>
...
<TABLE class=viewform >
  ...
  <TR class=title> 
  <th>
  <%=SystemEnv.getHtmlLabelName(18265,user.getLanguage())%></th>
  <th></th>
  <th style="text-align:right;cursor:hand"><span class="spanSwitch1" onclick="doSwitchx('showobjit')"><img src='/images/up.jpg'></span></th>
  </TR>
  ...
  <TEXTAREA TEXTAREA class="InputStyle" name="memo" rows="5" style="width:98%"><%=memo%>
  </TEXTAREA>
