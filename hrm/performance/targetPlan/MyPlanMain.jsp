...
if (type_d.equals("3"))
autoPlan.autoSetPlan(type_d,objName,objId ,user);
%>
...
<script language=javascript>
 function resetbanner(objid,typeid){
    ...
    o.location="MyPlan.jsp?type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years;
   ...
   function init(objid,typeid){
  	...
  	document.all("oTDtype_"+objid).className="cycleTDCurrent";
    var o = parent.contentframe.iframes.document;
    o.location="MyPlan.jsp?type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>";
 ...
 <table style="width:98%;height:95%" border=0 cellspacing=0 cellpadding=0  scrolling=no id="tabPane">
  ...
  <TBODY>
	  ...
	  <tr>
		 ...
		 <iframe src="MyPlan.jsp?type=0&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=currentYear%>" ID="iframes" name="iframes" frameborder="0" style="width:100%;height:100%;border-right:1px solid #879293;border-bottom:1px solid #879293;border-left:1px solid #879293;padding:10px;padding-right:0" scrolling="auto"/>
		</td></tr>
		</TBODY>
	  </table>
</td>
