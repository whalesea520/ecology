...
String name = "";
if(id.length()>0)name = JobTitlesTempletComInfo.getJobTitlesmark(id);
...
if(jobactivite.length()==0&&id.length()>0)jobactivite=JobTitlesTempletComInfo.getJobactivityid(id);
String[] strObj = null ;
String errorMsg ="";
...
jQuery(document).ready(function(){
	parent.setTabObjName("<%=SystemEnv.getHtmlLabelName(82662,user.getLanguage()) %>");
})
</script>
</head>
<BODY>
	...
	<%}%>
		<wea:layout type="2col">
			...
			<brow:browser name="jobdoc" viewType="0" hasBrowser="true" hasAdd="false" 
		              ...
		              completeUrl="/data.jsp?type=9"  linkUrl="/docs/docs/DocDsp.jsp" width="300px"  />		
		</wea:item>
			  <wea:item><%=SystemEnv.getHtmlLabelName(895,user.getLanguage())%></wea:item>
			  <wea:item><textarea class=inputstyle style="width:95%" rows=4 name="jobcompetency" ><%=(strObj==null?"":strObj[5])%></textarea></wea:item>
			  <wea:item><%=SystemEnv.getHtmlLabelName(454,user.getLanguage())%></wea:item>
			  <wea:item><TEXTAREA class=inputstyle name=jobtitleremark rows=8 style="width:95%"><%=(strObj==null?"":strObj[6])%></TEXTAREA></wea:item>
			</wea:group>
		</wea:layout>
