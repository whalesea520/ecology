...
<%
	...
	String titlename = SystemEnv.getHtmlLabelName(34097, user.getLanguage());
	String needfav ="1";
	String needhelp ="";
...
<HTML>
	...
	<BODY>
		...
		<form action="WorkflowMonitoring.jsp" name="searchfrm" id="searchfrm">
		...
		<%
		String wfidsql = Monitor.getwfidRightSql(fromid,RecordSet.getDBType());
		
			String para2 = "";
			String backfields = " wb.id,wb.workflowname as workflowname,wt.typename as typename"; 
			String fromSql  = " FROM workflow_base wb INNER JOIN workflow_type wt ON wt.id=wb.workflowtype";
			String sqlWhere = " WHERE wb.id IN ("+wfidsql+") AND wb.isvalid IN ('0','1','2')";
			String orderby = "" ;

			if (!qname.isEmpty()) {
				sqlWhere += " AND wb.workflowname like '%"+qname+"%'";
			} else if (!workflowName.isEmpty()) {
				sqlWhere += " AND wb.workflowname like '%"+workflowName+"%'";
			}
			if (!workflowType.isEmpty()) {
				sqlWhere += " AND wb.workflowtype="+workflowType;
			}
			String tableString =" <table pageId=\""+Constants.HRM_Z_011+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Z_011,user.getUID(),Constants.HRM)+"\" tabletype=\""+(isHidden?"none":"checkbox")+"\">"+
				" <sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\" sqlorderby=\""+orderby+"\" sqlprimarykey=\"wb.id\" sqlsortway=\"DESC\" sqlisdistinct=\"true\"/>"+
			"	<head>"+
			"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(33476,user.getLanguage())+SystemEnv.getHtmlLabelName(18499,user.getLanguage())+"\" column=\"workflowname\" orderkey=\"workflowname\" />"+
			"		<col width=\"25%\" text=\""+SystemEnv.getHtmlLabelName(33806,user.getLanguage())+"\" column=\"typename\" orderkey=\"typename\" />"+
			"	</head>"+
			" </table>";
