...
<!-- added by wcd 2014-09-25 [部门自定义字段] -->
<%@ include file="/hrm/header.jsp" %>
<jsp:useBean id="DepartmentComInfo" class="weaver.hrm.company.DepartmentComInfo" scope="page"/>
<jsp:useBean id="SubCompanyComInfo" class="weaver.hrm.company.SubCompanyComInfo" scope="page" />
<jsp:useBean id="cusFieldsManager" class="weaver.hrm.authority.manager.HrmDepartmentCusFieldsManager" scope="page" />
<%
	String imagefilename = "/images/hdSystem_wev8.gif";
	String titlename =SystemEnv.getHtmlLabelName(33646,user.getLanguage());
	String needfav ="1";
	String needhelp ="";
					...
					<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
						...
						<wea:item>
							<brow:browser viewType="0" name="departmentid" browserValue='<%=departmentid%>' 
								...
								completeUrl="/data.jsp?type=4" width="60%" browserSpanValue='<%=DepartmentComInfo.getDepartmentname(departmentid)%>'>
							</brow:browser>
						</wea:item>
						...
						<wea:item>
							<brow:browser viewType="0" name="subcompanyid" browserValue='<%=subcompanyid%>' 
								...
								hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
								completeUrl="/data.jsp?type=164" width="60%" browserSpanValue='<%=SubCompanyComInfo.getMoreSubCompanyname(subcompanyid)%>'>
							</brow:browser>
						</wea:item>
					</wea:group>
					<wea:group context="">
		...
		<wea:layout type="diycol">
			<wea:group context='<%=SystemEnv.getHtmlLabelNames("15101,356",user.getLanguage())%>' >
				<wea:item attributes="{'isTableList':'true','colspan':'full'}">
					<%
						String tableString =" <table needPage=\"false\" datasource=\"weaver.hrm.authority.manager.HrmDepartmentCusFieldsManager.getResult\" sourceparams=\"fromid:"+fromid+"+departmentid:"+departmentid+"+subcompanyid:"+subcompanyid+"+qname:"+qname+"+fieldname:"+fieldname+"\" pageId=\""+Constants.HRM_Q_021+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Q_021,user.getUID(),Constants.HRM)+"\" tabletype=\""+(isHidden?"none":"checkbox")+"\">"+
							" <sql backfields=\"*\" sqlform=\"temp\" sqlwhere=\"\" sqlorderby=\"id\"  sqlprimarykey=\"id\" sqlsortway=\"asc\"/>"+
						"	<head>"+
						"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(685,user.getLanguage())+"\" column=\"showName\" orderkey=\"showName\" />"+
						"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(15393,user.getLanguage())+"\" column=\"deptName\" orderkey=\"deptName\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmDepartmentDsp\" linkkey=\"id\" linkvaluecolumn=\"deptId\" target=\"_fullwindow\"/>"+
						"		<col width=\"35%\" text=\""+SystemEnv.getHtmlLabelName(19799,user.getLanguage())+"\" column=\"subName\" orderkey=\"subName\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmSubCompanyDsp\" linkkey=\"id\" linkvaluecolumn=\"subId\" target=\"_fullwindow\"/>"+
						"	</head></table>";
					%>
