...
<HTML>
	...
	<BODY>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
				<wea:layout type="4col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
			           ...
			           browserSpanValue='<%=hrmSubcompany.length()>0?Util.toScreen(SubCompanyComInfo.getSubcompanynames(hrmSubcompany),user.getLanguage()):""%>'>
					   	</brow:browser>
						</wea:item>
						<wea:item><%=SystemEnv.getHtmlLabelName(124,user.getLanguage())%></wea:item>
						<wea:item>
								<brow:browser viewType="0" name="hrmDepartment" browserValue='<%=hrmDepartment %>' 
			           browserUrl='/systeminfo/BrowserMain.jsp?mouldID=hrm&url=/hrm/company/MutiDepartmentBrowser.jsp?selectedids='
			           hasInput="true" isSingle="false" hasBrowser="true" isMustInput='1'
			           completeUrl="/data.jsp?type=4"
			           browserSpanValue='<%=hrmDepartment.length()>0?Util.toScreen(DepartmentComInfo.getDepartmentNames(hrmDepartment),user.getLanguage()):""%>'>
					   	</brow:browser>
						</wea:item>
		...
		<%
			String backfields = " a.id,a.lastname,a.workcode,a.departmentid,a.subcompanyid1,a.managerid,a.dsporder "; 
			String fromSql = "from hrmresource a ";
			String sqlWhere = " where a.jobtitle = "+fromid+" and a.status in (0,1,2,3) ";
			String orderby = "" ;
			...
			if(hrmSubcompany.length() > 0){
				sqlWhere += " and a.subcompanyid1 in ("+hrmSubcompany+") ";
			}
			if(hrmDepartment.length() > 0){
				sqlWhere += " and a.departmentid in ("+hrmDepartment+") ";
			}
			...
			"	<head>"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(413,user.getLanguage())+"\" column=\"lastname\" orderkey=\"lastname\" linkvaluecolumn=\"id\"  linkkey=\"id\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmResource\" />"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(141,user.getLanguage())+"\" column=\"subcompanyid1\" orderkey=\"subcompanyid1\" transmethod=\"weaver.hrm.company.SubCompanyComInfo.getSubCompanyname\" linkkey=\"id\" href=\"/hrm/company/HrmSubCompanyDsp.jsp?hasTree=false\"  target=\"_fullwindow\"  />"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"departmentid\" orderkey=\"departmentid\" transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\" linkkey=\"id\" href=\"/hrm/company/HrmDepartmentDsp.jsp?hasTree=false\" target=\"_fullwindow\" />"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(714,user.getLanguage())+"\" column=\"workcode\" orderkey=\"workcode\" />"+
			"		<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(15709,user.getLanguage())+"\" column=\"managerid\" orderkey=\"managerid\" transmethod=\"weaver.hrm.resource.ResourceComInfo.getResourcename\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmResource\" linkkey=\"id\" linkvaluecolumn=\"managerid\" target=\"_fullwindow\"/>"+
			"	</head>"+
			" </table>";
