...
<%
	...
	String titlename = SystemEnv.getHtmlLabelNames("33569,33417,15072", user.getLanguage());
	String needfav ="1";
	String needhelp ="";
...
<HTML>
	...
	<BODY>
		...
		<form action="WorkflowNodeGroup.jsp" name="searchfrm" id="searchfrm">
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
				<wea:layout type="4col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
						<wea:item><%=SystemEnv.getHtmlLabelName(15072,user.getLanguage())%></wea:item>
						<wea:item><input type="text" class=InputStyle maxLength=50 size=30 name="groupname" value='<%=groupname%>'></wea:item>
						<wea:item><%=SystemEnv.getHtmlLabelName(33417,user.getLanguage())%></wea:item>
						<wea:item><input type="text" class=InputStyle maxLength=50 size=30 name="nodename" value='<%=nodename%>'></wea:item>
		...
		<%
			...
			String para2 = "";
			String backfields = " wfng.id,wfng.groupname AS groupname,wfnb.nodename AS nodename,wfb.workflowname AS workflowname"; 
			String fromSql  = " FROM workflow_nodegroup wfng INNER JOIN workflow_nodebase wfnb ON wfnb.id=wfng.nodeid INNER JOIN workflow_flownode wffn ON wffn.nodeid=wfnb.id LEFT JOIN workflow_base wfb ON wfb.id=wffn.workflowid";
			String sqlWhere = "";
			if(gdtype.equals("3")){//人力资源特殊处理
				 sqlWhere = " WHERE wfng.id IN (SELECT DISTINCT wfgd.groupid FROM workflow_groupdetail wfgd WHERE  wfgd.type='"+gdtype+"' AND    exists(select 1 from Workflow_HrmOperator t2 where wfgd.id=t2.groupdetailid and t2.objid='"+fromid+"')) ";				
			}else if(gdtype.equals("58")){//岗位
				sqlWhere = " WHERE wfng.id IN (SELECT DISTINCT wfgd.groupid FROM workflow_groupdetail wfgd WHERE  wfgd.type='"+gdtype+"' ";
				if (rs.getDBType().equals("oracle")||rs.getDBType().equals("db2")){
				 ...
				 sqlWhere = " WHERE wfng.id IN (SELECT DISTINCT wfgd.groupid FROM workflow_groupdetail wfgd WHERE wfgd.type='"+gdtype+"' AND wfgd.objid='"+fromid+"')";
			}
			sqlWhere += " and (wfnb.IsFreeNode is null or wfnb.IsFreeNode!='1') ";
			sqlWhere += " and wfb.isvalid=1 ";
			String orderby = "" ;

			if (!qname.isEmpty()) {
				sqlWhere += " AND wfng.groupname like '%"+qname+"%'";
			} else if (!groupname.isEmpty()) {
				sqlWhere += " AND wfng.groupname like '%"+groupname+"%'";
			}
			if (!workflowType.isEmpty()) {
				sqlWhere += " AND wfb.workflowtype="+workflowType;
			}
			if (!nodename.isEmpty()) {
				sqlWhere += " AND wfnb.nodename like '%"+nodename+"%'";
			}
			...
			String tableString =" <table pageId=\""+Constants.HRM_Z_011+"\" pagesize=\""+PageIdConst.getPageSize(Constants.HRM_Z_011,user.getUID(),Constants.HRM)+"\" tabletype=\""+(isHidden?"none":"checkbox")+"\">"+
				" <sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\" sqlorderby=\""+orderby+"\" sqlprimarykey=\"wfng.id\" sqlsortway=\"DESC\" sqlisdistinct=\"true\"/>"+
			"	<head>"+
			"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(15072,user.getLanguage())+"\" column=\"groupname\" orderkey=\"groupname\" />"+
			"		<col width=\"30%\" text=\""+SystemEnv.getHtmlLabelName(33417,user.getLanguage())+"\" column=\"nodename\" orderkey=\"nodename\" />"+
			"		<col width=\"40%\" text=\""+SystemEnv.getHtmlLabelNames("33476,18499",user.getLanguage())+"\" column=\"workflowname\" orderkey=\"workflowname\" />"+
			"	</head>"+
			" </table>";
