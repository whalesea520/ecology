...
<HTML>
	...
	<BODY>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
				<wea:layout type="4col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
						...
						<wea:item><input type="text" class=InputStyle maxLength=50 size=30 name="orgGroupName" value='<%=orgGroupName%>'></wea:item>
						<wea:item><%=SystemEnv.getHtmlLabelName(24680,user.getLanguage())%></wea:item>
						<wea:item><input type="text" id="orgGroupDesc" name="orgGroupDesc" class="inputStyle" value='<%=orgGroupDesc%>'></wea:item>
					</wea:group>
					<wea:group context="">
		...
		<%
			String backfields = " a.id,a.orgGroupName,a.orgGroupDesc,a.showOrder "; 
			String fromSql = "from HrmOrgGroup a ";
			String rType = "1";
			if(_type.equals("T204IdStr") || _type.equals("C202IdStr")|| _type.equals("D201IdStr")){
				rType = "3";
			} else if(_type.equals("T303IdStr") || _type.equals("C303IdStr")|| _type.equals("D301IdStr")){
				rType = "2";
			}
			String sqlWhere = " where id in (select orgGroupId from HrmOrgGroupRelated where content = "+fromid+" and type = "+rType+") ";
			String orderby = "" ;
			
			if(qname.length() > 0 ){
				sqlWhere += " and a.orgGroupName like '%"+qname+"%' ";
			}else if(orgGroupName.length() > 0){
				sqlWhere += " and a.orgGroupName like '%"+orgGroupName+"%' ";
			}
			if(orgGroupDesc.length() > 0){
				sqlWhere += " and a.orgGroupDesc like '%"+orgGroupDesc+"%' ";
			}
			...
			"	<head>"+
			"		<col width=\"50%\" text=\""+SystemEnv.getHtmlLabelName(24679,user.getLanguage())+"\" column=\"orgGroupName\" orderkey=\"orgGroupName\" />"+
