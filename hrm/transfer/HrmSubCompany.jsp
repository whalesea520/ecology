...
<%
	...
	String subcompanycode = Util.null2String(request.getParameter("subcompanycode"));
%>
<HTML>
	<HEAD>
	...
	<BODY>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
				<wea:layout type="4col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage())%>'>
						<wea:item><%=SystemEnv.getHtmlLabelNames("141,399",user.getLanguage())%></wea:item>
						<wea:item><input type="text" class=InputStyle maxLength=50 size=30 name="subcompanyname" value='<%=subcompanyname%>'></wea:item>
						<wea:item><%=SystemEnv.getHtmlLabelNames("141,15767",user.getLanguage())%></wea:item>
						<wea:item><input type="text" id="subcompanydesc" name="subcompanydesc" class="inputStyle" value='<%=subcompanydesc%>'></wea:item>
						<wea:item><%=SystemEnv.getHtmlLabelNames("141,714",user.getLanguage())%></wea:item>
						<wea:item><input type="text" id="subcompanycode" name="subcompanycode" class="inputStyle" value='<%=subcompanycode%>'></wea:item>
					</wea:group>
					<wea:group context="">
		...
		<%
			String backfields = " a.id,a.subcompanyname,a.subcompanydesc,a.subcompanycode "; 
			String fromSql = "from HrmSubCompany a ";
			String sqlWhere = " where a.supsubcomid = "+fromid+" and (a.canceled IS NULL OR a.canceled !='1') ";
			String orderby = "" ;
			
			if(qname.length() > 0 ){
				sqlWhere += " and a.subcompanyname like '%"+qname+"%' ";
			}else if(subcompanyname.length() > 0){
				sqlWhere += " and a.subcompanyname like '%"+subcompanyname+"%' ";
			}
			if(subcompanydesc.length() > 0){
				sqlWhere += " and a.subcompanydesc like '%"+subcompanydesc+"%' ";
			}
			if(subcompanycode.length() > 0){
				sqlWhere += " and a.subcompanycode like '%"+subcompanycode+"%' ";
			}
			...
			"	<head>"+
			"		<col width=\"35%\" text=\""+SystemEnv.getHtmlLabelNames("141,399",user.getLanguage())+"\" column=\"subcompanyname\" orderkey=\"subcompanyname\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmSubCompanyDsp\" linkkey=\"id\" linkvaluecolumn=\"id\" target=\"_fullwindow\"/>"+
			"		<col width=\"35%\" text=\""+SystemEnv.getHtmlLabelNames("141,15767",user.getLanguage())+"\" column=\"subcompanydesc\" orderkey=\"subcompanydesc\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmSubCompanyDsp\" linkkey=\"id\" linkvaluecolumn=\"id\" target=\"_fullwindow\"/>"+
			"		<col width=\"25%\" text=\""+SystemEnv.getHtmlLabelNames("141,714",user.getLanguage())+"\" column=\"subcompanycode\" orderkey=\"subcompanycode\" href=\"/hrm/HrmTab.jsp?_fromURL=HrmSubCompanyDsp\" linkkey=\"id\" linkvaluecolumn=\"id\" target=\"_fullwindow\"/>"+
			"	</head>"+
			" </table>";
