...
<HTML>
	...
	<BODY>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;" >
				<wea:layout type="4col">
					<wea:group context='<%=SystemEnv.getHtmlLabelName(15774,user.getLanguage())%>'>
						...
						<wea:item><%=SystemEnv.getHtmlLabelName(17749,user.getLanguage())%></wea:item>
						<wea:item>
								<brow:browser viewType="0" name="prjtype" 
								browserValue='<%=prjtype %>' browserSpanValue='<%=Util.getIntValue( prjtype)>0?ProjectInfoComInfo .getProjectInfoname (""+prjtype):"" %>' 
		...
		<%
			String backfields = " tt1.id,tt1.subject,tt1.prjid,tt1.finish,tt1.begindate,tt1.enddate,tt1.hrmid,t1.manager,t1.name,t1.status "; 
			String fromSql = "from Prj_TaskProcess tt1 left outer join Prj_ProjectInfo t1 on t1.id=tt1.prjid ";
			String sqlWhere = " where 1=1 ";
			if(_type.startsWith("T113")){//项目任务
				sqlWhere+=" and tt1.hrmid='"+fromid+"' ";
			}
			
			
			String orderby = "tt1.begindate" ;
			
			if(qname.length() > 0 ){
				sqlWhere += " and tt1.subject like '%"+qname+"%' ";
			}else if(prjname.length() > 0){
				sqlWhere += " and tt1.subject like '%"+prjname+"%' ";
			}
			...
			if(prjtype.length()>0){
				sqlWhere += " and tt1.prjid='"+prjtype+"' ";
			}
			...
			String pageId=Util.null2String(PropUtil.getPageId("prj_hrmprjtskjshifter"));
			...
			" <sql backfields=\""+backfields+"\" sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlorderby=\""+orderby+"\"  sqlprimarykey=\"tt1.id\" sqlsortway=\"Asc\" sqlisdistinct=\"true\"/>"+
			"	<head>"+
			"		<col width=\"45%\" text=\""+SystemEnv.getHtmlLabelName(1352,user.getLanguage())+"\" column=\"id\" orderkey=\"subject\" otherpara=\"column:subject+column:status+"+user.getLanguage()+"+column:islandmark+column:prjid+column:begindate+column:enddate\"  transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskNameByStatus'   />"+
			"		<col width=\"25%\" text=\""+SystemEnv.getHtmlLabelNames("17749",user.getLanguage())+"\" column=\"prjid\" orderkey=\"prjid\" otherpara=\"column:name+column:status+"+user.getLanguage()+"+column:begindate+column:enddate\"  transmethod='weaver.proj.util.ProjectTransUtil.getPrjName' />"+
			"		<col width=\"25%\" text=\""+SystemEnv.getHtmlLabelNames("847",user.getLanguage())+"\" column=\"finish\" orderkey=\"finish\" otherpara='column:enddate' showaspercent=\"true\" transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskProgressbar' />"+
			"	</head>"+
			" </table>";
