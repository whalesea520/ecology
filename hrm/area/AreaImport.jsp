...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value='<%=SystemEnv.getHtmlLabelNames("25480,28100",user.getLanguage()) %>'/>
</jsp:include>
...
<FORM id=frmMain name=frmMain action="AreaImportProcess.jsp" method=post enctype="multipart/form-data" target="subframe">
	<input type="hidden" name="matrixid" value="<%=method %>"/>
	...
	<wea:layout type="2col">
      ...
      <wea:item>
       <a href='/hrm/area/import/templet/areatemplet.xls' class="templetfile" style="color: #30b5ff"><%=SystemEnv.getHtmlLabelName(28576, user.getLanguage())%></a>
      </wea:item>
	...
	<wea:layout>
		<wea:group context='<%=SystemEnv.getHtmlLabelName(18596,user.getLanguage())+SystemEnv.getHtmlLabelName(85,user.getLanguage())%>' attributes="{'samePair':'RemarkInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
		  <wea:item attributes="{'colspan':'2'}">
			...
			<BR><BR>
			3、<%=SystemEnv.getHtmlLabelName(126163, user.getLanguage())%>
			<BR><BR>
			4、<%=SystemEnv.getHtmlLabelName(126164, user.getLanguage())%>
			<BR><BR>
			5、<%=SystemEnv.getHtmlLabelName(126165, user.getLanguage())%>
			<BR><BR>
			6、<%=SystemEnv.getHtmlLabelName(126166, user.getLanguage())%>
			<BR><BR>				
		  </wea:item>
		</wea:group>
	</wea:layout>
...
function getData(){
		var tableHead = "<TABLE class=ListStyle cellspacing=1 >" +
		 ...
		 "</TR></TBODY>" + 
		 "</TABLE>";
	$.get("AreaImportLog.jsp?index="+index,function(data){
		//需要显示全部日志 
...
var tableHead = "  <TABLE class=ListStyle cellspacing=1 >" +
  ...
  $.get("AreaImportHistoryLog.jsp",function(data){
		 //$("#result").append(data);
	   if ($("#result").html() == "") {
...
function downLoadLog(path){
  $("#downLoad").attr("src","AreaImportHistoryLog.jsp?option=downLoad&path="+encodeURI(path));
}
/*删除日志文件*/
function deleteLog(path,obj){
  //if(window.confirm('<%=SystemEnv.getHtmlLabelName(23271, user.getLanguage())%>'+"?"))
	window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(23271,user.getLanguage())%>",function(){
		$.post("AreaImportHistoryLog.jsp",{"option":"delete","path":path},function(data){
     	if($.trim(data)=="true")
