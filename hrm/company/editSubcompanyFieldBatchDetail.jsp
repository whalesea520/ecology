...
<jsp:useBean id="DeptFieldManager22" class="weaver.hrm.company.DeptFieldManager" scope="page"/>
...
<!-- 
  <TABLE class=viewform>
	...
	<TBODY> 
    <TR class=title> 
      <td><b><%=SystemEnv.getHtmlLabelName(19516,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(15601,user.getLanguage())%></b></td>
      <td align=right>
 ...
 -->
  	  <table class=ListStyle id="oTable" cols=6  border=0 cellspacing=1>
          ...
          <tr class=HeaderForXtalbe>
            ...
            <th><input type="checkbox" name="checkallstatus" onClick="formStatusCheckAll(checkallstatus.checked)" value="ON">
...
<%
      ...
      String fieldlabel_kwd = Util.null2String(request.getParameter("fieldlabel_kwd"));
			...
			String sql = "select * from subcompanyDefineField " 
								 + " where viewtype=0 "; 
			if(qname.length()>0){
			...
			while(RecordSet.next()){
				...
				String canDeleteCheckBox = FormFieldTransMethod.getCanCheckBox2(para);
%>
     <TR class=<%=trClass%> forsort="ON">
			...
			<td NOWRAP >
				  <input type="hidden" name="itemFieldType_<%=rowsum%>" value="3">
				  <!--
				  ...
				  <div id="div3_<%=rowsum%>" <%if(fieldhtmltype.equals("3")){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				  	...
				  	<select class='InputStyle' id='broswerType_<%=rowsum%>' name='broswerType_<%=rowsum%>' onChange="onChangBroswerType(<%=rowsum%>);setChange(<%=rowsum%>)" style="width: 200px">
				  		<%
