...
function getAsyncUrl(treeId, treeNode) {
    ...
    if (treeNode != undefined && treeNode.isParent != undefined && treeNode.isParent != null) {
    	return "/hrm/tree/DepartmentSingleXMLByDec.jsp?" + treeNode.ajaxParam +"&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&fieldid=<%=fieldid%>&isbill=<%=isbill%>&isdetail=<%=isdetail%>&detachable=1&rightStr=<%=rightStr%>&deptlevel=<%=deptlevel%>&onlyselfdept=<%=onlyselfdept%>&excludeid=<%=excludeid%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>&mathrandom="+Math.random()+"&isruledesign=<%=isruledesign%>";
    } else {
    	//初始化时
    	return "/hrm/tree/DepartmentSingleXMLByDec.jsp?f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&fieldid=<%=fieldid%>&isbill=<%=isbill%>&isdetail=<%=isdetail%>&detachable=1&rightStr=<%=rightStr%>&deptlevel=<%=deptlevel%>&onlyselfdept=<%=onlyselfdept%>&excludeid=<%=excludeid%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>&mathrandom="+Math.random()+"&isruledesign=<%=isruledesign%>";
    }
};
