...
<HEAD>
   ...
   try{
		...
		if(window.console)console.log(e+"-->MultiSubcompanyBrowserByDec2.jsp");
	}
	var dialog = null;
	try{
...
<%}%>
      <FORM NAME=select STYLE="margin-bottom:0" action="MultiSubcompanyBrowserByDec2.jsp" method=post>
...
function initTree(){
	CXLoadTreeItem("", "/hrm/tree/SubcompanyMutiXMLByDec2.jsp?onlyselfdept=<%=onlyselfdept%>&excludeid=<%=excludeid%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>&mathrandom="+Math.random());
