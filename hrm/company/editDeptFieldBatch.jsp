...
function addItemDialog(obj,item_id,select_name){
	...
	var url = "/hrm/HrmDialogTab.jsp?_fromURL=EditHrmCustomFieldSelect&id=<%=scopeId%>&item_id="+item_id+"&select_name="+select_name+"&alloption="+alloption;;
...
function setSelectOption(resultDatas){
	...
	var previous = jQuery("select[name=selectOption] option").size();
	previous = parseInt(previous) - 1;//去掉“选项名称”
	...
	for(var i=0;i<resultDatas.length;i++){
		...
		if(resultDatas.length>previous){
			if(i >= previous){
				selectitemid = i;
			}
		}
...
<div id="rowSource">
	...
	<div style="DISPLAY: none" id="fname">
		<input class=InputStyle name="fieldname" type="text" style="width: 100px" onchange='checkKey(this);checkinput_char_num(this);hrmCheckinput(this);'>
	...
	<div style="DISPLAY: none" id="fhtmltype">
		<select size="1" name="fieldhtmltype" onChange = "htmltypeChange(this)" style="width: 95px" notBeauty=true onChange = "BrowserTypeChange(this)">
	...
	<div style="DISPLAY: none" id="fgroupid">
	  <select name="groupid" notBeauty=true>
	   	 ...
	   	 for(int tmp=0;lsGroup!=null&&tmp<lsGroup.size();tmp++){
	   		...
	   		Integer grouplabel = Integer.parseInt(HrmGroupComInfo.getLabel(tmpgroupid));
	  	%>
...
<FORM id="frmMain" name="frmMain" action="deptFieldOperation.jsp" method="post" >
	...
	<TABLE id="tabfieldhead" CLASS="ListStyle" valign="top" cellspacing=1 style="position:fixed;z-index:99!important;">
		<colgroup>   
	  	<col width="7%">
	  	<col width="13%">
	  	<col width="13%">
  ...
  <%
    ...
    <td>
    	...
    	<%}else{ %>
    	<input  class=InputStyle name="fieldlabel" value="<%=SystemEnv.getHtmlLabelName(Integer.parseInt(fieldlabel),user.getLanguage())%>" onchange='hrmCheckinput(this);' style="width: 100px">
			<SPAN>
