...
<HEAD>
	...
	try{
		...
		if(window.console)console.log(e+"-->SubcompanyBrowserByDec.jsp");
	}
	var dialog = null;
	try{
...
<%}%>
				    ...
				    <FORM NAME=select STYLE="margin-bottom:0" action="SubcompanyBrowserByDec.jsp" method=post>
...
function initTree(){
CXLoadTreeItem("", "/hrm/tree/SubcompanySingleXMLByDec.jsp?rightStr=<%=rightStr%>&onlyselfdept=<%=onlyselfdept%>&excludeid=<%=excludeid%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&isruledesign=<%=isruledesign%>&mathrandom="+Math.random());
...
function onSaveJavaScript(){
	...
	for(var i=0;i<select.selObj.length;i++) {
		if(select.selObj[i].checked) {
			...
			nameStr =  select.selObj[i].value.replace(",","，");
			break;
		}
	}
}
