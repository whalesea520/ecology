...
<jsp:useBean id="DeptFieldManager11" class="weaver.hrm.company.DeptFieldManager" scope="session"/>
...
function fieldlablesall0(){
	fieldlabelfrm.action="/hrm/company/subcompanyFieldOperation.jsp";
...
String titlename = SystemEnv.getHtmlLabelName(141, user.getLanguage())+SystemEnv.getHtmlLabelName(17088, user.getLanguage()) +":"+SystemEnv.getHtmlLabelName(176, user.getLanguage());
String needfav ="";
if(!ajax.equals("1"))
{
needfav ="1";
...
String needhelp ="";
    ...
    if(detachable==1){  
        ...
        operatelevel= CheckSubCompanyRight.ChkComRightByUserRightCompanyId(user.getUID(),"SubCompanyDefineInfo1:SubMaintain1",subCompanyId);
    }else{
        if(HrmUserVarify.checkUserRight("SubCompanyDefineInfo1:SubMaintain1", user))
            operatelevel=2;
    }
    ...
    String sql = "select id,fieldname,fieldlabel,viewtype from subcompanyDefineField where 1=1";
...
<%}%>
<form name="fieldlabelfrm" id=fieldlabelfrm method=post action="/hrm/company/addSubcompanyFieldlabel.jsp">
...
if(fieldids.size()==0){
%>
<DIV><font color="#FF0000"><%=SystemEnv.getHtmlLabelName(141,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(31417,user.getLanguage())%></font></DIV>
<script>
function selectall(){
